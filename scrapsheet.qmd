...
-   Data Formats for `stConstruct`
    ...
    -   Long - The full spatio-temporal information (i.e. response value) is held in a single column, and location and time are also single columns.
        ...
        -   Example: Create full grid from a long table
            ...
            yrs <- 1970:1986
            vec_time <- as.POSIXct(paste(yrs, "-01-01", sep=""), tz = "GMT")
            head(vec_time)
            ...
            -   [spatial_geom_ids]{.var-text} shows the order of the spatial geometry object (state polygons) which are state names in alphabetical order.
                ...
                -   At least in this example, the geometry object didn't store the actual state names. It just used a id index (e.g. ID1, ID2, etc.)
            ...
            -   Converting back to a dataframe adds 6 new columns:
                ...
                -   sp.ID: IDs within the spatial geomtry object
...
-   Time and Movement\
    ...
    -   Supported Time Classes: Date, POSIXt, timeDate ({timeDate}), yearmon ({zoo}), and yearqtr ({zoo})
...
-   Creation
    ...
    -   [Example]{.ribbon-highlight}: Long Table to Full Grid
