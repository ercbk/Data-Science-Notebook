...
- Join Count Test
  ...
  - The variance calculation uses the underlying constants of the chosen [listw]{.arg-text} object and the counts of levels of the input factor.
...
## [{stars}]{style="color: #990000"}

- These are all raster operations

- Basic GeoTIFF inspection
  ...
  - See <https://r-spatial.org/book/07-Introsf.html#plotting-raster-data> for images
...
- Aggregate
  ...
  - Aggregation over a temporal dimension is done by passing a time variable as the second argument (unclear to me how exactly this is done though)

  - Example: Maximum pixel values in 3 buffer zones
    ...
    - Says sample 3 points from r and create a buffer zones of 500m around each point. Output is a vector data cube.
    - `aggregate` finds the max pixel value in each buffer zone
      - Since r has six bands, this is done for all six bands.

  - Example: Find mean of all six bands for each pixel
  ...
  - Example: Compute the NDVI (normalised differenced vegetation index)
  ...
  - Example: Mean of every pixel for each band
  ...
  - Example: Quantiles for each band and for each pixel
...
- Model
  ...
  - Example: LDA
    ...
    ``` r
    # coerces bands to a predictor variable
    rs <- split(r)
    ...
    - split: Takes the bands dimension and makes it an attribute. The bands can now be used as predictors in the model
    - st_extract subsets the pts (from Sample and Extract) example to create a training set.
    - See source for basic plot of the predictions
...
- Other
  ...
  - Transforming values
    ...
    - Arithmetic operations
      ...
      - Results can be seen in the summary statistics section
