...
-   Target
    -   \$30 - Solid Performance 400 Thread Count Sheet Set
    -   \$17 - Gillette ProGlide Razor Men 5 Blade Manual Razor ,1 Handle and 2 Blade Refills ,Black
-   bbq chips
-   Restaurants
    -   Green Tea
        -   Beef and Broccoli
...
-   Questions
    -   Snowflake
        ...
        -   dev and prod are mention as environments but how are the implemented?

        -   permissions

            ``` bash
            -- dev write access 
            grant create table on schema madison_dbt to role transformer;
            grant create view on schema madison_dbt to role role_name;

            -- prod write access 
            grant create view on schema staging to role role_name;
            ```

            -   Might need to confirm that these are typos
-   Components
    -   Airbyte connects to Google Sheets and streams the data into a Snowflake warehouse
    -   dbt will be used for reverse ETL and data validation
    -   Orchestration and monitoring will be in Prefect.
-   Snowflake
    -   Interacting with Snowflake

        -   Snowflake Web Interface (UI)
            1.  Log in to your Snowflake account.
            2.  Navigate to the `Worksheets` section from the left-hand menu.
            3.  Create a new worksheet or use an existing one.
            4.  Copy and paste commands into the worksheet.
            5.  Execute the commands by selecting the commands and clicking on the `Run` button.
        -   CLI
            -   Download and install SnowSQL from Snowflake's website if you don’t already have it.

            -   Log in using your Snowflake credentials

                ``` bash
                snowsql -a <account_name> -u <username> -r <role> -w <warehouse> 
                ```

            -   Paste the commands into the terminal and execute them.
        -   Or through SQL Clients, R and Python Packages
    ...
    -   Create Schemas
        ...
        ``` bash
        -- creates schemas in analytics database
    ...
    -   Roles
        ...
        -   Each role has slightly different access permissions and can be applied to the tool corresponding to their category

            -   [Examples]{.ribbon-highlight}:
                -   A data ingestion tool like Airbyte will be assigned the *loader* role.
                -   A *analyzer* will not have write access to prod schemas like [core]{.var-text} and raw data schemas like [google_sheets]{.var-text}, but it will have read access to the [core]{.var-text} schema
                -   *Loader* will only have read and write access to raw data schemas like [google_sheets]{.var-text}, not your development schema nor prod schemas.
    ...
    -   Permissions
        ...
        -   Each role needs to be assigned permissions for the appropiate schema

        -   Make sure you understand the permissions you are assigning and don’t over-grant them.

        -   You always want to choose the **least** amount of permissions needed for the tool/person to do its job.

        -   [Example]{.ribbon-highlight}: Transformer Permissions
            ...
            -- schema usage
            grant usage on schema google_sheets in database analytics to role transformer;

            grant usage on schema staging in database analytics to role transformer;

            grant usage on schema intermediate in database analytics to role transformer;

            grant usage on schema core in database analytics to role transformer;

            -- select permissions 
            grant select on all tables in schema google_sheets to role transformer;
            grant select on future tables in schema google_sheets to role transformer;

            grant select on all views in schema staging to role transformer;
            grant select on future views in schema staging to role transformer;

            grant select on all views in schema intermediate to role transformer;
            grant select on all tables in schema intermediate to role transformer;
            grant select on future views in schema intermediate to role transformer;
            grant select on future tables in schema intermediate to role transformer;

            grant select on all tables in schema core to role transformer;
            grant select on all views in schema core to role transformer;
            grant select on future tables in schema core to role transformer;
            grant select on future views in schema core to role transformer;
            ...
            -- prod write access 
            grant create view on schema staging to role role_name;
            grant create view on schema intermediate to role transformer;
            grant create table on schema intermediate to role transformer;
            grant create view on schema core to role transformer;
            grant create table on schema core to role transformer;
            ```

        -   Why weren't grant permissions to read or write tables in the staging schema of the warehouse?

            -   Staging layers are usually materialized as views for the most part so reading or writing tables in the staging schema isn't necessary.
            -   Given they should be simple queries of the raw data tables this will result in having to store less data while still being performant.

    -   Create Users

        -   Create a user for every human and tool.

        -   You can assign a user more than one role, but only one default role.

        -   [Example]{.ribbon-highlight}

            ``` bash
            create user madison_ae PASSWORD='abc123' DEFAULT_ROLE = transformer; 
            grant role analyzer to madison_ae; 
            ```

        -   Create a *loader* role and [airbyte]{.var-text} user.

        -   Create a *transformer* role and [dbt]{.var-text} user.

        -   Create an *orchestrator* role and [prefect]{.var-text} user.

        -   Look in each tools documentation fore the permissions required for Airbyte, dbt, and Prefect to work as expected on your data.

    -   Set Airbyte variables **after connecting Airbyte to Googlesheets**

        ``` bash
        -- set variables (these need to be uppercase)
        set airbyte_role = 'LOADER';
        set airbyte_username = 'AIRBYTE_USER';
        set airbyte_database = 'ANALYTICS';
        set airbyte_schema = 'GOOGLE_SHEETS';
        ```

        -   These commands are provided by Airbyte once you set-up the connector

        -   Another one was provided, but Madison said to ignore it, `set airbyte_warehouse = 'AIRBYTE_WH';`

        -   You may have to the Snowflake account admin to successfully run some of the commands in the suggested script
-   Go back to Airbyte and set-up Set Sync Mode and Sync Schedule. Also you might want to specify how schema changes in the source data are handled.
...
-   \*Read [Diversifying Trend Following Strategies Improves Portfolio Efficiency](https://alphaarchitect.com/2024/12/portfolio-efficiency/)
