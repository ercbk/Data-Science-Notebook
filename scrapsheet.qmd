...
-   Snack
    -   chip
    -   pretzel
-   orange juice (maybe strawberry mix)
...
-   Pooled variogram
    -   First Function (irregular grid)
        -   Formula\
            $$
            \hat \gamma(h) = \frac{1}{2 N(h)} \sum_{(i,j) \in N(u)} \frac{1}{S} \sum_{s = 1}^S (Y_{s,i} - Y_{s,j})^2
            $$
        -   Pools across locations
        -   Pools across time pairs in bin
    -   Second Function (regular grid)
        -   Formula\
            $$
            \hat \gamma(h) = \frac{1}{2 N(h)} \sum_{s=1}^S \sum_{t=1}^{T-h} (Y_{s,t} - Y_{s, t+h})^2
            $$
        -   Classical Pooled Estimator
        -   Pools across locations
        -   $N(h) = S(T-u)$
    -   Or
        -   formula
        ...
        -   using weights to handle missing values

            $$
            \hat{\gamma}_1(h) = \frac{1}{2} \frac{\sum_{(i,j) \in N(h)} \sum_{s \in V_{ij}} (Y_{s,i} - Y_{s,j})^2}{\sum_{(i,j) \in N(h)} n_{ij}}
            $$
    -   Procedure
        -   Create distance pairs
        -   Calculate spatial semivariance from distance pairs (fixing time)
        -   bin distance pairs
        -   Calculate temporal semivariance
    -   overall
        -   bin width 7 is best for nugget estimation
            -   Initial detection of any patterns (e.g. seasonality)
            -   nugget estimation: \~68 units
                -   So probably some micro-scale variation or measurment error present
        -   bin width 14 and 21 best for sill and range estimation
        ...
        -   Other:
            ...
            -   nugget-sill ratio: 68/103 = 0.66 which is quite high
                -   Interpretation:
    ...
    -   bin width 7
        -   You see:
            -   Sharp increase from lag 0–\~30 days
            -   **Gradual rise toward \~100–105**
            -   **Clear leveling off around \~150–220 days (range)**
            -   *Slight oscillation after \~200 days*
        -   That is consistent with:
            -   Strong short-term temporal dependence
            -   Practical temporal range ≈ 100–200 days
            -   Sill ≈ 100–105
            -   **Nugget ≈ \~65–70**
        -   The mild oscillation around 200–350 days suggests annual seasonality.
            -   
            -   Consider detrending or Seasonal differencing
            -   option: detrend = TRUE
                -   Internally subtract location-wise mean or seasonal mean
        -   nugget is \~68 and sill \~100
            -   68/103 = 0.66 is on the higher side
            -   Interpretation:
                -   High day-to-day variability
                -   Weak persistence after short lags
                -   This is realistic for air pollution
    -   bin width 14
        -   you see:
            -   steep rise from \~0–30 days
            -   **Gradual increase through \~150–240 days (range)**
            -   **Broad plateau around \~100–105**
            -   *Slight decline after \~275 days*
        -   **nugget: First point ≈ 79**
        -   sill: Plateau ≈ 102–105
            -   empirical sill ≈ 103 (roughly)
        -   range: approx 150–200 days
            -   Temporal correlation becomes negligible after \~half a year which is plausible for air pollution with seasonal components
        -   dip after \~275 days suggests one of:
            -   Finite sample noise (most likely)
            -   Seasonality (annual structure)
            -   Reduced pair counts at large lags
        -   Modeling Implications
            -   Probable reasonable fits:
                -   Exponential
                -   Spherical
                -   Possibly Matérn (ν ≈ 0.5–1)
            -   Given the smooth rise, exponential is probably appropriate.
            -   Initial values for `vgm`:
                -   nugget ≈ 70
                -   psill ≈ 35 (psill = sill − nugget)
                -   range ≈ 180
    -   bin width 21
        -   range 220
        -   gradual increase from 30 to 220
