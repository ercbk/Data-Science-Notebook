    ...
    -   Misc
        ...
        -   Moment Matching Estimation (MME)
            ...
            -   [Example]{.ribbon-highlight}: Reinsurance Losses\
                ...
                -   The MME method underfits the loss data, and it's more pronouced for the Lognormal distribution. But if you look closely, it fits the right-tail of Lognormal better than the MLE. Both are pretty similar in the right-tail of the Pareto, but I'd probably take the MME estimation there, too.
...
-   Parameters: Shape ($\alpha$) (aka Pareto Index, aka Tail Index), Scale ($\theta$)

    -   The version with a location parameter $\mu$ is the Pareto II.
...
-   Probabilty Density\
    ...
    -   $x \gt 0$, $\alpha \gt 0$, and $\theta \gt 0$
...
-   Some theoretical statistical moments may not exist
    ...
    -   [Example]{.ribbon-highlight}: Pareto ($\alpha$ = 1.5) has a finite mean and an infinite variance
        -   Need $\alpha > 2$ for a finite variance\
            $$
            \text{Var}(X) = \left(\frac{\theta}{\alpha-1}\right)^2 \cdot \frac{\alpha}{\alpha-2}
            $$
        -   Need $\alpha > 1$ for a finite mean\
            $$
            \mathbb{E}[X] = \frac{\alpha \theta}{\alpha - 1}
            $$
...
-   [Example]{.ribbon-highlight}: Fitting a Pareto to data
    ...
    -   [order = c(1, 2)]{.arg-text} says you want the first and second moments (shape and scale) estimated (Required for [method = "mme"]{.arg-text})
    -   [lower]{.arg-text} (similar for [upper]{.arg-text}) specifies the left bounds on the parameters.
    -   [memp]{.arg-text} supplies the function for implementing empirical moments. This equation
