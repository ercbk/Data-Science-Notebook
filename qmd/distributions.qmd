...
- Packages
  - [{]{style="color: #990000"}[cdvinv](https://cran.r-project.org/web/packages/cdfinv/index.html){style="color: #990000"}[}]{style="color: #990000"} - Estimation of one- and two-sided confidence intervals for distribution parameters via the numerical inversion of the cumulative distribution function of a statistic's sampling distribution
  ...
  - [{]{style="color: #990000"}[DTS](https://cran.r-project.org/web/packages/DTS/index.html){style="color: #990000"}[}]{style="color: #990000"} - Methods for evaluating the probability mass function, cumulative distribution function, and generating random samples from discrete tempered stable distributions
    - Discrete tempered stable distributions are a large and flexible class of models for heavy tailed and overdispersed count data.
  - [{]{style="color: #990000"}[frbinom](https://github.com/leejeo25/frbinom){style="color: #990000"}[}]{style="color: #990000"} - Generates random variables of fractional binomial distribution and computes its density, cumulative distribution, and quantiles.
  - [{]{style="color: #990000"}[kerdiest](https://cran.r-project.org/web/packages/kerdiest/index.html){style="color: #990000"}[}]{style="color: #990000"} - Nonparametric Kernel Estimation of Distribution Function
    - Bandwidth selectors: plug-in selectors and cross-validation selector
    - The exceedance function, the mean return period and the return level are also computed.
  - [{]{style="color: #990000"}[kernopt](https://thomasfillon.github.io/kernopt/){style="color: #990000"}[}]{style="color: #990000"} - Implements *Discrete Symmetric Optimal Kernel* for estimating count data distributions
    - Histograms for count distributions can be difficult to interpret. This package creates a smoothed density based on your counts.
    - The density can be used to better determine a parametric distribution to model from; discover outliers, or use in Bayesian modeling (although this would require using stan code)
  - [{]{style="color: #990000"}[joker](https://thechibo.github.io/joker/){style="color: #990000"}[}]{style="color: #990000"} - Probability Distributions and Parameter Estimation
    - Parameter estimation of distribution given data
    - Loglikelihood of data belonging to a distribution
    - Methods that calculate the theoretical moments (expectation, variance and standard deviation, skewness, excess kurtosis) and other important parametric functions (median, mode, entropy, Fisher information) of a distribution
  - [{]{style="color: #990000"}[multivardiv](https://cran.r-project.org/web/packages/multvardiv/index.html){style="color: #990000"}[}]{style="color: #990000"} - Multivariate generalized Gaussian distribution, Multivariate Cauchy distribution, Multivariate t distribution. Distance between two distributions
  - [{]{style="color: #990000"}[probaverse](https://probaverse.com/){style="color: #990000"}[}]{style="color: #990000"} - A suite of tools in R for exploring the full space of possible outcomes in an analysis. (In most workflows, you'll load probaverse (`library(probaverse)`) and not individual packages)
    - [{]{style="color: #990000"}[distionary](https://distionary.netlify.app/){style="color: #990000"}[}]{style="color: #990000"} - Create a distribution object, and to make distribution calculations available even if they are not specified in the distribution’s definition
      - Provides a unified framework for defining, manipulating, and evaluating probability distributions. Because distionary only needs some distribution properties to be specified, it offers a level of flexibility.
    - [{]{style="color: #990000"}[distplyr](https://distplyr.netlify.app/){style="color: #990000"}[}]{style="color: #990000"} - Provides a grammar for their modification and reshaping distionary objects
      - Apply linear transformations (shifting (i.e. adding), reciprocal, negation), mathematical transformations (e.g. exp, logging), combining (e.g. mixing), shifting
    - [{]{style="color: #990000"}[famish](https://famish.probaverse.com/){style="color: #990000"}[}]{style="color: #990000"} - Fits a distribution to data. Plus, tools to select the best fit.
    - [{]{style="color: #990000"}[couple](https://r-couple.netlify.app/){style="color: #990000"}[}]{style="color: #990000"} - Construct realistic multivariate distributions by combining copulas and univariate distributions
  - [{]{style="color: #990000"}[Rsubbotools](https://cran.r-project.org/web/packages/Rsubbotools/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fast Estimation (C++) of Parameters for the Subbottin and AEP Distributions (Generalized Normal/Error Distribution)
    - Adds a shape parameter to the Normal Distribution
    - Also provides methods for the gamma, Laplace, and Asymmetric Laplace distributions
  - [{]{style="color: #990000"}[sn](https://cran.r-project.org/web/packages/sn/index.html){style="color: #990000"}[}]{style="color: #990000"} - Facilities to build and manipulate probability distributions of the skew-normal and some related families, notably the skew-*t* family and the unified skew-normal (SUN)) family.
  ...
  - [{]{style="color: #990000"}[WeibullModiAMR](https://cran.r-project.org/web/packages/WeibullModiAMR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fits Modified Weibull-Type Distributions Provides maximum likelihood estimation methods for eight modified Weibull-type distributions.
    - Returns parameter estimates, log-likelihood, AIC, and BIC, and also supports model fitting, validation, and comparison across different distributional forms.
- Tools
  - [The Distribution Zoo](https://ben18785.shinyapps.io/distribution-zoo/)
    - A large collection of distributions categorized by continuous, discrete, multivariate
    - Inputs: Range and Parameters
    - Features: Plot of PDF and CDF, Formulae with LaTeX code, R functions, Usages for Likelihood and Prior
  - [Probability Distributions Explorer](https://shiny.thecoatlessprofessor.com/probability-distribution-explorer/)
    - Moderate collection of distributions
    - Inputs: Parameters
    - Features: Plot for PDF and CDF, Sample Size, Add Comparison Distribution, Probabilty given a range
  - [Statistical Distributions](https://observablehq.com/collection/@mattiasvillani/distributions)
    - An extremely large collection of distributions
    - Inputs: Quantile and Parameters
    - Features: Plot of PDF, Formulae with calculated values that change as parameters and quantile inputs change
  - [{]{style="color: #990000"}[VisualStats](https://github.com/jbryer/VisualStats){style="color: #990000"}[}]{style="color: #990000"} Shiny App ([article](https://bryer.org/posts/2025-09-30-distributions.html))\
    ![](_resources/Distributions.resources/misc-tools-visualstats-shiny-1.png){.lightbox width="382"}\
...
- Location measures for asymmetric distributions ([Harrell](https://www.fharrell.com/post/aci/))
  - Experimented using n = 200 from a lognormal
  - The pseudomedian (aka Hodges-Lehmann estimator} is recommended due to its combination of robustness, efficiency, and having an accurate confidence interval.
    - See [Outliers \>\> Statistics](outliers.qmd#sec-outliers-stat){style="color: green"} \>\> Hodges-Lehmann Estimator
  - BCa bootstrap method was the most accurate for computing confidence limits for the *mean*, but the upper limit was too small (non-coverage probability of 0.092 instead of 0.025)
  - For the pseudomedian, both the classic exact Hodges-Lehmann confidence interval (which assumes symmetry of the population distribution) and the BCa bootstrap method have excellent coverage.
...
- [Example]{.ribbon-highlight}: Sampling from various distributions ([source](https://nvietto.github.io/samplezoo/#motivation))

  ``` r
  norm <- rnorm(n = 100, mean = 60, sd = 10)
...
- [Misc]{.underline}
  - Also see [Mathematics, Statistics \>\> Multivariate \>\> Misc](mathematics-statistics.qmd#sec-math-statc-desc-multiv-misc){style="color: green"} \>\> [{Compositional}]{style="color: #990000"} for multivariate compositional distributional tests
...
- [Energy Statistic (E-Statistic)]{.underline}\
  ...
  - Packages
    - [{]{style="color: #990000"}[energy](https://mariarizzo.github.io/energy/){style="color: #990000"}[}]{style="color: #990000"} - Implements several methods in multivariate analysis and multivariate inference based on the energy distance, which characterizes equality of distributions
    - [{]{style="color: #990000"}[estats](https://cran.r-project.org/web/packages/estats/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fast and memory-less computation of the energy statistics related quantities for vectors and matrices.
...
- [Kernel Maximum Mean Discrepancy (MMD)\]{.underline}
...
- [Kernel-based Quadratic Distance (KBQD)\]{.underline}
...
- [Kernel Likelihood Ratio Two-Sample Test]{.underline}
  - [Paper](https://arxiv.org/abs/2508.07982), [Github](https://github.com/leonardoVsantoro/Kernel-Likelihood-Ratio-Two-Sample-Test)
  - The approach leverages kernel embeddings of distributions to construct regularized likelihood ratio statistics. The implementation adaptively selects the kernel bandwith and regularization ridge
- [Density Ratio Estimation]{.underline}
  - [{]{style="color: #990000"}[densityratio](https://github.com/thomvolker/densityratio){style="color: #990000"}[}]{style="color: #990000"}, [Package Paper](https://arxiv.org/abs/2408.13167), [Statistic Paper](https://www.ms.k.u-tokyo.ac.jp/sugi/2010/RIMS2010.pdf)
    - The use case in the paper is assessing the quality of synthetic data, but it would seem capable of being used for other uses as well.
  - Density ratio estimation serves many purposes, for example, prediction, outlier detection, change-point detection in time-series, importance weighting under domain adaptation (i.e., sample selection bias) and evaluation of synthetic data utility.
  - Measures and tests the dissimularity between two sets of data. Handles numeric and categorical data although it just converts the categoricals to dummies. Some methods accept multivariable data.
  - Estimating densities separately is prone to error, and that error is magnified when the ratio is calculated. Estimating the ratio directily avoids this issue and is known to be the superior method.
  - Density Ratio Estimation Methods:
    - Unconstrained Least-Squares Importance Fitting
    - Kullback-Leibler Importance Estimation Procedure
    - Ratio of Estimated Densities (Naive method)
    - Ratio of Estimated Densities After Dimension Reduction
    - Least-Squares Heterodistributional Subspace Search (experimental).
  - The most appropriate estimation algorithm depends on the problem at hand, but computational efficiency and relative robustness make `ulsif` a good starting point. For high-dimensional data, `spectral` and `lhss` are presumably more appropriate.
  - The appropriate width of the kernel and the optimal value of the regularization parameter can be determined using cross-validation. Currently on the Gaussian kernel is available.
  - The divergence statistic is difficult to interpret in an absolute sense, it can be used as a relative measure of the quality of the synthetic data.
  - The p-value is calculated by comparing the observed divergence measure to the null distribution of divergence measures obtained using random permutations of the observed and synthetic data.
  - By (visually) inspecting which observations have a high, or small, estimated density ratio, we can identify the regions in which the synthetic data is most different from the observed data.
  - The predicted density ratio value can be regarded as an *importance weight*. Releasing the predicted density ratio values for each synthetic observation allows data users to reweigh analyses on the synthetic data. (i.e. reweighing the synthetic data to better match the observed data)
- [Density Equality Testing (DET)]{.underline}
  - [{]{style="color: #990000"}[denstest](https://cran.r-project.org/web/packages/denstest/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements FDET (Fourier-based Density Equality Testing) and MDET (Moment-based Density Equality Testing)
    - FDET compares groups of densities based on their global shape using Fourier transforms, while MDET tests for differences in distributional moments
...
- Misc
  ...
  - Packages
    - [{]{style="color: #990000"}[nullabor](https://dicook.github.io/nullabor/){style="color: #990000"}[}]{style="color: #990000"} ([article](https://mansthulin.se/posts/nullabor0315/)) - Generates a Null distribution from your data and the probability distribution you specify. Samples from that distribution to produce a panel of null distribution plots for comparison against your distribution plot. (See [Diagnostics, Regression \>\> Residuals](diagnostics-regression.qmd#sec-diag-reg-res){style="color: green"} for an example)
      - If your distribution plot is indistinguishable from the panel of plots generated by the Null distribution, then there's no evidence of lack-of-fit.
      - If the distribution plot is distinguishable, then there is a lack-of-fit issue
      - Histograms (`lineup_histograms`) and Q-Q plots (`lineup_qq`) available
...
- Packages
  ...
  - [{]{style="color: #990000"}[UGarima](https://cran.r-project.org/web/packages/UGarima/index.html){style="color: #990000"}[}]{style="color: #990000"} - Density, distribution function, quantile function, and random generating function of the Unit-Garima distribution. (alternative to the Beta distribution)

- Defined on the interval \[0,1\]

  - Rates, proportions, probabilities, etc.
...
- Also see [Mathematics, Statistics \>\> Multivariate \>\> Misc](mathematics-statistics.qmd#sec-math-statc-desc-multiv-misc){style="color: green"} \>\> [{Compositional}]{style="color: #990000"} and [Terms](mathematics-statistics.qmd#sec-math-statc-desc-multiv-terms){style="color: green"} \>\> Compositional Data

- Resources

  - [A Guide to Understading the Intuition Behind the Diriclet Distribution](https://www.andrewheiss.com/blog/2023/09/18/understanding-dirichlet-beta-intuition/)
...
- Papers
  ...
  - [Dirichlet kernel density estimation for strongly mixing sequences on the simplex](https://arxiv.org/abs/2506.08816)
    - [Gemini discussion](https://gemini.google.com/app/704d0d8440ce278e), [Repo](https://github.com/FredericOuimetMcGill/DirichletKDE)
    - The main contribution of this paper is that it's able model proportional time series (i.e. strongly mixing sequences) data.
    - The real data example uses monthly proportions ("market shares") of vehicle class sales by Renault. Based on this data, it calculates thresholds where under a threshold means this set of market shares is unlikely and over means it is likely
    - The threshold for ($S_A$, $S_B$, other) is 4.08 (Table 1) says if we plug in proportions for $S_A$, $S_B$ (vehicle classes A and B), and other (which is the sum of the rest of the classes) into their method and obtain an HDR less than 4.08 then it means those proportions are unlikely given the history of sales.
    - Use Cases
      - *Set realistic targets*: By knowing the long-term "normal" range (the 95% HDR) for joint shares, they can set cross-segment share targets that are ambitious but achievable.
      - *Spot unusual deviations*: If a new month's market share data falls outside this 95% HDR, it can be flagged as a significant anomaly worth investigating immediately.
      - *Optimize the model portfolio*: Understanding the typical relationships and long-term concentrations between segments (e.g., "do shares of A and E tend to be high together?") can help the team reallocate marketing budgets or R&D resources more effectively
...
- Used in zero-bounded continuous data, survival regression, prior for the mean of poisson response, etc.
...
## Laplace {#sec-distr-laplace .unnumbered}

![](_resources/Distributions.resources/laplace-1.png){.lightbox width="354"}

- Notes from [The Laplace distribution](https://rss.onlinelibrary.wiley.com/doi/pdf/10.1111/j.1740-9713.2018.01185.x)
- Packaages
  - [{]{style="color: #990000"}[ExtendedLaplace](https://cran.r-project.org/web/packages/ExtendedLaplace/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides computational tools for working with the Extended Laplace distribution, including the probability density function, cumulative distribution function, quantile function, random variate generation based on convolution with Uniform noise and the quantile-quantile plot
    - Well suited for modeling Laplace data contaminated with errors
- Figures a and b are illustrating the fatter tails
  - An “extreme” event in a Laplace population is more than five times as likely as in a normal population
- AKA Double Exponential
- pdf (wiki)\
  ![](_resources/Distributions.resources/laplace-2.png){.lightbox width="332"}\

  $$f(x) = \frac{1}{2b} e^{-\frac{|x - \mu|}{b}}$$
- The Laplace distribution can be derived via a so-called Scale Mixture of Normals\

  $$Y = \mu + \sqrt{W} Z$$
  - $W$ : An Exponential distribution with $\mu = 1$
  - $Z$ : A Standard Normal distribution
  - Can be thought of as a composite distribution where there are two different types of distributions of error contributing to an observation (e.g. the skill of an astronomer and the telescope)
- The MLE of the location parameter, $\mu$, is the median.
- The MLE of the scale parameter, $b$, is the MAD
- The variance is $2b^2$
- Bayesian LASSO regression uses a Laplacian prior for the coefficients.
- The symmetric Laplace can be used as a robust linear regression distribution. The asymmetric Laplace can be used in quantile regression.
...
- Packages
  ...
  - [{]{style="color: #990000"}[bayesestdft](https://github.com/Roy-SR-007/bayesestdft){style="color: #990000"}[}]{style="color: #990000"}: Estimating the Degrees of Freedom of the Student's t-Distribution under a Bayesian Framework
    - Bayesian routines to estimate the degrees of freedom of the Student's t-distribution with Jeffreys prior (`BayesJeffreys`), Gamma prior (`BayesGA`), and Log-normal prior (`BayesLNP`) endowed upon as the prior distribution over the degrees of freedom
