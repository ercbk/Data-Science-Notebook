...
-   [Theseus Plot: Binary Outcome (proportion) vs Categorical (2 categories) vs Categorical\
    ]{.underline}![](_resources/EDA.resources/catpred-catout-theseus-1.png){.lightbox width="432"}
    ...
    thes_bld <- ggplot_build(thes_obj)

    find_rect_layers <- function(layer) {
      cols <- names(layer)
      log_flipped <- cols == "flipped_aes"
      log_fill <- cols == "fill"
      if (any(log_flipped) == FALSE & any(log_fill) == TRUE) {
        rect_layer <- TRUE
      } else {
        rect_layer <- FALSE
      }
    ...
    ls_log_rect <- purrr::map(thes_bld$data, \(x) find_rect_layers(x))
    ind_rect <- which(ls_log_rect == TRUE)
    cols_outer <- c(ind_rect[[1]], ind_rect[[length(ind_rect)]])
    cols_water <- ind_rect[2:(length(ind_rect)-1)]


    thes_obj$layers[[cols_outer[[1]]]]$aes_params$fill <- notebook_colors[[3]]
    thes_obj$layers[[cols_outer[[2]]]]$aes_params$fill <- notebook_colors[[3]]

    purrr::walk(cols_water, \(x) thes_obj$layers[[x]]$aes_params$fill <- notebook_colors[[2]])
    ...
    -   [TheseusPlot](https://cran.r-project.org/web/packages/TheseusPlot/index.html)
