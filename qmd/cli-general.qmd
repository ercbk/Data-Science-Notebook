...
-   Misc
    ...
    -   Packages
        ...
        -   [{]{style="color: #990000"}[sessioncheck](https://sessioncheck.djnavarro.net/){style="color: #990000"}[}]{style="color: #990000"} - Provides simple tools that can be called at the top of a script, and produce warnings or errors if it detects signs that the script is not being executed in a clean R session.
            ...
            -   Also see [{]{style="color: #990000"}[conflicted](https://conflicted.r-lib.org/){style="color: #990000"}[}]{style="color: #990000"} and maybe use `conflict_scout`
        -   [{]{style="color: #990000"}[sftpR](https://mikuo0628.github.io/sftpR/){style="color: #990000"}[}]{style="color: #990000"} - Provides a high-level, object-oriented interface for Secure File Transfer Protocol (SFTP) operations built upon the 'curl' package.
            -   Safely stores user credential and a reusable `curl` handle for generic SFTP operations such as download and listing directories, and interal methods to create purpose-driven handles for more specific SFTP operations (upload, delete, rename, etc).
