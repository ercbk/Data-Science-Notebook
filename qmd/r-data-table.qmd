...
- Packages
  ...
  - [{]{style="color: #990000"}[fjoin](https://cran.r-project.org/web/packages/fjoin/index.html){style="color: #990000"}[}]{style="color: #990000"} - Extends 'data.table' join functionality, lets it work with any data frame class, and provides a familiar 'x'/'y'-style interface, enabling broad use across R. (See [link](https://trobx.github.io/fjoin/articles/fjoin.html#notes-for-sf-users) for vignette example)
    - Performs inner, left, right, full, semi- and anti-joins with equality and inequality conditions, plus cross joins
    - Specific support for 'data.table', (grouped) tibble, and 'sf'/'sfc' objects and their attributes; returns a plain data frame otherwise
...
- Other Functions
  ...
  - `fcase` ([docs](https://rdatatable.gitlab.io/data.table/reference/fcase.html)) - A fast implementation of SQL `CASE WHEN` statement for R. Comparable to [{dplyr::case_when}]{style="color: #990000"} and supports bit64's integer64 and nanotime classes.
...
## Parallelization {#sec-r-dt-para .unnumbered}

- [Example]{.ribbon-highlight}: [{future}]{style="color: #990000"} and [{mirai}]{style="color: #990000"} ([source](https://mastodon.social/@eliocamp/116045037997944692))
  ...
  async_future <- function(...) {
    tasks <- list(future::future(..., envir = parent.frame(1)))
    class(tasks) <- "tasks_future"
    tasks
  }

  async_mirai <- function(...) {
    tasks <- list(mirai::mirai(..., parent.frame(1)))
    class(tasks) <- "tasks_mirai"
    tasks
  }    
  ```
  ...
  ``` r
  collect_async <- function(x) {
    UseMethod("collect_async")
  }

  collect_async.default <- function(x) x

  collect_async.tasks_future <- function(x) {
    lapply(x, future::value) |>
      unlist()
  }

  collect_async.tasks_mirai <- function(x) {
    mirai::collect_mirai(x)
  }

  collect_async.data.table <- function(data) {
    data[, names(.SD) := lapply(.SD, collect_async)][]
  }    
  ```

  - There are default, tasks_future, tasks_mirai, and data.table methods
  - Not sure exactly how this works, but I'm guessing the data.table method gets called first and then either the future or mirai methods.

  ## Usage

  `long` refers to a long-running function that we want to parallelize

  ``` r
   data |>  # <1>
      _[, .(x = async_future(long(x))), by = .(group1, group2)] |>
      collect_async()
