...
``` r
dplyr_attr <- function(path_str, date_str, outcome, value, retbl){
...
base_r_attr <- function(path_str, date_str, outcome, value, lu) {
...
dt_attr <- function(path_str, date_str, outcome, value, lu) {
...
#> 1       base_r 18.4s  18.4s 0.05426719   43.85MB           NA
#> 2           dt 25.1s  25.1s 0.03986556    1.15GB    0.7346163
```

-   I didn't include the dplyr version because it wasn't going to win. If you want to see the comparison, see video or github links. 
-   I also didn't include the Rust version, because I currently don't know enough about Rust to fool with it right now.
-   According to Josiah and the dude who wrote the data.table version, the data.table version is supposed to be the fastest out of all of them (including Rust). I didn't get those results --- not sure why ¯\\\_(ツ)_/¯.
-   Not only did the base R version win, it had a *substantially* smaller memory allocation.
:::
