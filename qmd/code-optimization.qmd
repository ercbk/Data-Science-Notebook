...
-   Tools
    -   Some orchestration platforms provide monitoring for execution time, memory usage, and I/O operations for you and are flexible enough for you to add whatever asset or runtime metadata you need (e.g. [Dagster](https://dagster.io/blog/introducing-the-new-dagster-plus-ui))
-   Memory issues often masquerade as performance issues.
    -   A function that "takes forever" is usually thrashing swap because it's trying to load 20GB into 8GB of RAM. The fix isn't faster Python, it's chunked processing or better data architecture.
-   Common less-valuable optimizations ([source](https://dagster.io/blog/when-and-when-not-to-optimize-data-pipelines))
    -   String concatenation in small loops
    ...
    -   Micro-optimizations in code that runs once
...
-   Common valuable optiimizations ([source](https://dagster.io/blog/when-and-when-not-to-optimize-data-pipelines))
    ...
    -   Serialization/deserialization in tight loops
        -   [Example]{.ribbon-highlight}: Batching and json serializers
    ...
    -   Retry logic without exponential backoff
        -   See [APIs, Use \>\> {requests}](apis-use.html#sec-apis-use-reqlib){style="color: green"} \>\> Rate Limiting \>\> [{tenacity}]{style="color: goldenrod"} example
        -   Reduces API failures from 15% to \<1% by avoiding rate limit cascades. Makes pipelines more resilient to transient failures.
    -   Memory churn from repeated allocation
...
-   Adding Rows to a Matrix ([link](https://www.r-bloggers.com/2024/02/be-kind-dont-rbind/), [link](https://gist.github.com/jrosell/7a38c8731a4002425e5e39bce35fd0e8))
    ...
    -   `pmax`: Use `n[n<1] <- 1` ranther than `n<-pmax(n,1)` ([source](https://bsky.app/profile/bbolker.bsky.social/post/3mcb4w3ywmc2g))
...
-   Misc
    ...
    -   Packages
        -   [{]{style="color: #990000"}[debrief](https://github.com/r-lib/debrief){style="color: #990000"}[}]{style="color: #990000"} - Text-based summaries and analysis tools for profvis profiling output.
            -   Designed for terminal workflows and AI agent consumption, offering views including hotspot analysis, call trees, source context, caller/callee relationships, and memory allocation breakdowns.
...
-   [{]{style="color: goldenrod"}[cProfile](https://docs.python.org/3/library/profile.html){style="color: goldenrod"}[}]{style="color: goldenrod"}
...
-   Memory Usage
    ...
    -   [{{]{style="color: goldenrod"}[memory-profiler](https://pypi.org/project/memory-profiler/){style="color: goldenrod"}[}}]{style="color: goldenrod"}
        ...
        -   [Example]{.ribbon-highlight}: Basic usage with profile decorator
...
-   Misc
    ...
    -   Drop-in, low-dependency replacements
