...
-   Often the probability of selection is chosen to be proportional to some measure of size (i.e., sampling with probabilities proportional to size (PPS)), particularly when sampling Primary Sampling Units (PSU) in a multi-stage or multi-phase sample.
...
-   [Example]{.ribbon-highlight}: Multi-Stage, Stratified, and Cluster Sampling
    -   2016 NHIS Survey Design
        -   See [Surveys, Census Data \>\>]{style="color: green"}
            ...
            -   [IPUMS](surveys-census-data.qmd#sec-surv-cens-ipums){style="color: green"} \>\> Design Information \>\> Example: NHIS
    -   The PSUs are counties, groups of contiguous counties, or metropolitan areas
    -   The strata are based on population density, usually urban and rural
    -   The clusters are households (also something like dormroom residents)
