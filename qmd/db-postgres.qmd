...
- Tools
  ...
  - [RegreSQL](https://github.com/boringSQL/regresql) ([Intro](https://boringsql.com/posts/regresql-testing-queries/))
...
- [pg_durable]{.underline}
  - [Repo](https://github.com/microsoft/pg_durable)
  - Long-running, fault-tolerant SQL functions for teams that already keep their state in Postgres and want to stop stitching together cron jobs, workers, queues, and status tables to make background work reliable.
  - Define the workflow in SQL, let pg_durable checkpoint each step, and resume after crashes, restarts, or failed steps.
  - Use Cases
...
- [pg_textsearch]{.underline}
  - [Repo](https://github.com/timescale/pg_textsearch)
  - Performs BM25 relevance-ranked full-text search. Postgres OSS licensed.
...
- [DBMS-Optimized Cloud Storage\]{.underline}
  ![](_resources/DB-PostgreSQL.resources/distr-dbms-opt-1.avif){.lightbox width="532"}
...
- [Misc]{.underline}
  ...
  - Sometimes even adding the correct index, (correctly) *not* using a function on that index, etc., the query will still be executed using a sequential scan.
    ...
    - Solutions
      ...
      - The documentation suggests running `ANALYZE` manually with an increased [default_statistics_target]{.arg-text} value or by increasing the amount of collected statistics.
...
- EXPLAIN
  - Notes from [PostgreSQL: Query Optimization for Mere Humans](https://towardsdatascience.com/query-optimization-for-mere-humans-in-postgresql-875ab864390a)
  - Also see
    - [SQL \>\> Functions](sql.qmd#sec-sql-funs){style="color: green"} \>\> EXPLAIN, EXPLAIN ANALYZE
    - [Databases, Engineering \>\> Cost Optimizations](db-engineering.qmd#sec-db-eng-costopt){style="color: green"} \>\> EXPLAIN PLAN
  - Resources
    - [EXPLAIN Glossary](https://www.pgmustard.com/docs/explain)
  - Node Metrics\
    ![](_resources/DB-PostgreSQL.resources/perf-explain-node-metrics-1.webp){.lightbox width="582"}
  ...
  - [Example 1]{.ribbon-highlight}
    - Query\
      ![](_resources/DB-PostgreSQL.resources/perf-explain-ex1-1.webp){.lightbox width="582"}
      ...
      - The sequential scan filters out 2,487,813 rows
    ...
    - Create an index\
      ![](_resources/DB-PostgreSQL.resources/perf-explain-ex1-2.webp){.lightbox width="582"}
      ...
      - The sequential scan only takes 293.532 ms now

    - Check how many pages are read during the index scan\
      ![](_resources/DB-PostgreSQL.resources/perf-explain-ex1-3.webp){.lightbox width="582"}
      ...
      - 51,854 pages are read all from the cache (400 MB).
  ...
  - [Example 2]{.ribbon-highlight}: Applying a function to an indexed column ([source](https://aaronoellis.com/articles/postgres-enable-seqscan-off))
    - Query\
      ![](_resources/DB-PostgreSQL.resources/perf-explain-ex2-1.png){.lightbox width="582"}
      ...
      EXPLAIN ANALYZE SELECT * FROM "items" WHERE DATE("timestamp" AT TIME ZONE 'utc') = '2023-02-24';
      ```

      - There's an index on [timestamp]{.var-text}, yet a sequential scan is still used by the query planner.
      - This is because the query executing a function on an indexed column

    - Solution: Apply index to function and column\
      ![](_resources/DB-PostgreSQL.resources/perf-explain-ex2-2.png){.lightbox width="582"}
      ...
      - You could also filter a range of timestamps (that equals 1 day) instead of truncating the timestamp with a function, and that query will use the existing column index
