# General {#sec-econ-gen .unnumbered}

## Misc {#sec-econ-gen-misc .unnumbered}

- Packages
  - [CRAN Task View](https://cran.r-project.org/web/views/Econometrics.html)
  - [{]{style="color: #990000"}[Rcope](https://cran.r-project.org/web/packages/Rcope/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides flexible, alternative IV-free methods using copulas to handle endogeneity in regressors
...
- Notes from
  - [Causal design patterns for data analysts](https://emilyriederer.netlify.app/post/causal-design-patterns/#fn:10)
- ATE or LATE?
...
## Terms {#sec-econ-gen-terms .unnumbered}

- [**Disturbances**]{style="color: #009499"} - The error term in an econometric models (aka residuals in a regression)
- [**Economic Shock**]{style="color: #009499"} - Refers to any change to fundamental macroeconomic variables or relationships that has a substantial effect on macroeconomic outcomes and measures of economic performance,
  - Examples: unemployment, consumption, and inflation.
- [**Endogenous Variable**]{style="color: #009499"} - Variables that are correlated with the population error term. "Determined inside the model".
  ...
  - Also see [Glossary: DS terms](glossary-ds-terms.qmd#sec-ds-gloss){style="color: green"}
- [**Exogenous Variable**]{style="color: #009499"} - Variables that are NOT correlated with the population error term. "Determined outside the model"
...
- [**Limited Dependent Variable (LDV)**]{style="color: #009499"} - A variable whose range of possible values is "restricted in some important way."
...
## Standard Errors {#sec-econ-gen-se .unnumbered}

### Misc {#sec-econ-gen-se-misc .unnumbered}

- Packages
  - [{]{style="color: #990000"}[ceser](http://www.diogoferrari.com/ceser/){style="color: #990000"}[}]{style="color: #990000"}
  - [{]{style="color: #990000"}[clubSandwich](https://jepusto.github.io/clubSandwich/){style="color: #990000"}[}]{style="color: #990000"}
  ...
  - [{]{style="color: #990000"}[fwildclusterboot](https://s3alfisc.github.io/fwildclusterboot/){style="color: #990000"}[}]{style="color: #990000"}
  - [{]{style="color: #990000"}[hcinfer](https://cran.r-project.org/package=hcinfer){style="color: #990000"}[}]{style="color: #990000"} - HC~0~ through HC~5m~ estimators implemented plus HC~beta~ which was developed by the authors of the package
  - [{]{style="color: #990000"}[sandwich](https://sandwich.r-forge.r-project.org/articles/sandwich.html){style="color: #990000"}[}]{style="color: #990000"}
  - [{]{style="color: #990000"}[SpatialInference::conely_SE](https://cran.r-project.org/web/packages/SpatialInference/refman/SpatialInference.html#conley_SE){style="color: #990000"}[}]{style="color: #990000"} - Computes conely SEs for spatially correlated residuals
    ...
    - Vignette shows how to estimate the distance cutoff using a covariogram from [{gstat}]{style="color: #990000"}
- If you use clustered standard errors for multiple group variables, the robust standard errors also cover interactions involving these variables and variables nested within the group variables ([Thread](https://bsky.app/profile/jepusto.bsky.social/post/3lcj2xmvz622f), [Paper](https://jepusto.com/publications/competing-approaches-for-cross-classified-data/index.html))
...
### Heteroskedastic Robust Standard Errors {#sec-econ-gen-se-hrse .unnumbered}

- Discussed by Zeileis in his econometrics [book](https://www.econometrics-with-r.org/5-4-hah.html) and in White's [paper](http://qed.econ.queensu.ca/working_papers/papers/qed_wp_537.pdf)
- [Types]{.underline}
  - H~0~ is the original
  - H~1~ follows H~0~ but corrects for degrees of freedom. Only unbiased when experiment is balanced.
    ...
    - [Example]{.ribbon-highlight}
      ...
      ```
  - H~2~ is unbiased when errors are homeoskedastic
    - i.e. Don't just blindly use without checking for homeoskedastity
  - H~3~ is derived from jackknife procedure
...
### Heteroskedastic and Autocorrelation (HAC) Consistent {#sec-econ-gen-se-hac .unnumbered}

- Discussed in Hanck [book](https://www.econometrics-with-r.org/10-5-tferaaseffer.html)
...
- R^2^ and F test not affected (Wald test preferred when heterskadasticity and autocorrelation present)
...
### Clustered Standard Errors {#sec-econ-gen-se-cse .unnumbered}
...
- `vcovCL()`may be biased downwards
  ...
  - Solutions:
    ...
    - Wild Cluster Bootstrap [{clubSandwich}]{style="color: #990000"} [{fwildclusterboot}]{style="color: #990000"}
      - Computationally expensive
        - fwildclusterboot is VERY fast though
      - [{]{style="color: #990000"}[modernBoot::wild_boot_lm](https://cran.r-project.org/web/packages/modernBoot/refman/modernBoot.html#wild_boot_lm){style="color: #990000"}[}]{style="color: #990000"} - Performs wild bootstrap resampling for linear regression models to handle heteroscedasticity. Supports Rademacher and Mammen weight schemes
    ...
    - Cluster-estimated [{ceser}]{style="color: #990000"}
...
## Instrumental Variables (IV) {#sec-econ-gen-iv .unnumbered}
...
- [Misc]{.underline}
  ...
  - Packages
    - [{]{style="color: #990000"}[cragg](https://cloud.r-project.org/web/packages/cragg/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements Cragg-Donald and Stock and Yogo **tests for weak instruments** in R
    - [{]{style="color: #990000"}[endogeneity](https://cran.r-project.org/web/packages/endogeneity/index.html){style="color: #990000"}[}]{style="color: #990000"} - Various recursive two-stage models to **address the endogeneity issue of treatment variables** in observational study or mediators in experiments.
    ...
    - [{]{style="color: #990000"}[ivreg](https://zeileis.github.io/ivreg/){style="color: #990000"}[}]{style="color: #990000"} - Instrumental variable estimation for linear models by **two-stage** least-squares (2SLS) regression or by **robust**-regression via M-estimation (2SM) or MM-estimation (2SMM).
    - [{]{style="color: #990000"}[iv.sensemakr](https://cran.r-project.org/web/packages/iv.sensemakr/index.html){style="color: #990000"}[}]{style="color: #990000"} - **Sensitivity Analysis** Tools for Instrumental Variable Estimates
    - [{]{style="color: #990000"}[RPIV](https://cran.r-project.org/web/packages/RPIV/index.html){style="color: #990000"}[}]{style="color: #990000"} - **Residual Prediction Test** for Well-Specification of Instrumental Variable Models
    - [{]{style="color: #990000"}[senseR](https://cran.r-project.org/web/packages/senseR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides statistical diagnostics to **evaluate** whether **proxy indicators** reliably represent an unobservable target construct
      - Components:
...
- [Terms]{.underline}
  ...
  - [**Endogenous Variable**]{style="color: #009499"} - Variables that are correlated with u, the population error term. "Determined inside the model".
    - When the causality between X and Y goes both directions, both are endogenous.
  - [**Exogenous Variable**]{style="color: #009499"} - Variables that are NOT correlated with u. "Determined outside the model"

- [Conditions for Valid Instruments]{.underline}

  1.  Instrument Relevance: $Corr (X , Z) \neq 0$ (Predictive of X)
      - Checks (1st stage)
  ...
  3.  Exclusion Restriction: No impact on the dependent variable *directly*. It only impacts the dependent variable through its impact on the treatment variable
...
- Effects (Also see LATE in [Effects](experiments-analysis.qmd#sec-exp-anal-effs){style="color: green"}, [Calculating LATE](experiments-analysis.qmd#sec-exp-anal-late){style="color: green"} and [Compliance](experiments-analysis.qmd#sec-exp-anal-comp){style="color: green"} in Experiments, Analysis)
...
- [Caveats]{.underline}
  ...
  - $\beta_1$, the average treatment effect, assumes that all subgroups experience the roughly the same effect. If there are different subgroups of the population that are substantially affected differently, then a "weighted average of subsets" approach can be used.
    - [Example]{.ribbon-highlight}: Y = lung cancer, X = cigarettes, Z = cigarette tax. Perhaps people whose smoking behavior is sensitive to a tax may have a different β1 than other people

## Difference-in-Differences Estimator {#sec-econ-gen-did .unnumbered}

![](./_resources/Econometrics,_General.resources/excalidraw-did.png){.lightbox width="432"}
...
- Packages
  ...
  - [{]{style="color: #990000"}[didhetero](https://tkhdyanagi.github.io/didhetero/){style="color: #990000"}[}]{style="color: #990000"} - Tools to construct doubly robust uniform confidence bands (UCB) for the group-time conditional average treatment effect (CATT) function given a pre-treatment covariate of interest in the staggered difference-in-differences setup of Callaway and Sant’Anna (2021).
  - [{]{style="color: goldenrod"}[diff-diff](https://diff-diff.readthedocs.io/en/latest/){style="color: goldenrod"}[}]{style="color: goldenrod"} - Provides sklearn-like DiD estimators with statsmodels-style output for econometric analysis.
  ...
  - [{]{style="color: #990000"}[fastdid](https://tsailintung.github.io/fastdid/){style="color: #990000"}[}]{style="color: #990000"} - Implements the Difference-in-Differences (DiD) estimators in Callaway and Sant’Anna’s (2021)
  ...
  - [{]{style="color: #990000"}[fdid](https://cran.r-project.org/web/packages/fdid/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements the factorial difference-in-differences (FDID) framework for panel data settings where all units are exposed to a universal event but vary in a baseline factor G.
  ...
  - [{]{style="color: goldenrod"}[moderndid](https://github.com/jordandeklerk/moderndid){style="color: goldenrod"}[}]{style="color: goldenrod"} - Modern DiD estimators with diagnostic tools and sensitivity analysis.
  ...
  - [{]{style="color: #990000"}[pcdid](https://cran.r-project.org/web/packages/pcdid/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements factor-augmented difference-in-differences (DID) estimation.
  ...
  - [{]{style="color: #990000"}[ptetools](https://bcallaway11.github.io/ptetools/){style="color: #990000"}[}]{style="color: #990000"} - Compartmentalizes the steps needed to implement estimators of group-time average treatment effects (and their aggregations)
  - [{]{style="color: #990000"}[staggered](https://cran.r-project.org/web/packages/staggered/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2102.01291)) - Efficient Estimation Under Staggered Treatment Timing
  - [{]{style="color: #990000"}[staggR](https://cran.r-project.org/web/packages/staggR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fit Difference-in-Differences Models with Staggered Interventions

- Papers

  - [Difference-in-Differences Designs: A Practitioner's Guide](https://arxiv.org/abs/2503.13323)
  - [Using causal diagrams to assess parallel trends in difference-in-differences studies](https://arxiv.org/abs/2505.03526)
...
- Basically a standard lm with an interaction between the treatment indicator and time period indicator demarking before/after treatment.\
  ![](./_resources/Econometrics,_General.resources/image.png){.lightbox width="532"}

- Predictions\
  ![](./_resources/Econometrics,_General.resources/image.1.png){.lightbox width="432"}
...
## Synthetic Control Method (SCM) {#sec-econ-gen-scm .unnumbered}
...
- [Misc]{.underline}
  ...
  - Packages
    - [{]{style="color: #990000"}[coresynth](https://cran.r-project.org/web/packages/coresynth/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fast and Unified Synthetic Control Methods
    ...
    - [{]{style="color: #990000"}[DiSCos](https://www.davidvandijcke.com/DiSCos/){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2501.07550)) - Tools for computing counterfactual quantile functions in a Distributional Synthetic Controls (DiSco) setting
    - [{]{style="color: #990000"}[scpi](https://nppackages.github.io/scpi/){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2210.05026)) - Estimation and inference procedures for synthetic control methods
      - Also available for Python
      - Uncertainty quantification in synthetic controls with staggered treatment adoption
    - [{]{style="color: #990000"}[tidysynth](https://github.com/edunford/tidysynth){style="color: #990000"}[}]{style="color: #990000"} - A Tidy Implementation of the Synthetic Control Method
...
- [Terms]{.underline}
  - [**Donor Pool**]{style="color: #009499"} or [**Donors**]{style="color: #009499"}: The group of units that are untreated which are used to calculate the synthetic control
  - [**Unit**]{style="color: #009499"}, [**Cell**]{style="color: #009499"}, [**Case**]{style="color: #009499"}: Interchangeable names for the population level you're testing in the experiement (e.g. herd, school, store, city, state, precinct)
- [Recommended Use Cases]{.underline}
...
- [Choosing Units for the Donor Pool]{.underline}
...
- [Robustness Checks]{.underline}
  - in-time placebo test (backdating): move the treatment date backwards in the data. If the synthetic control still closely tracks the outcome variable until the actual treatment date, then this is evidence of a reliable synthetic control\
    ![](./_resources/Econometrics,_General.resources/Screenshot%20(245).png){.lightbox width="532"}
  ...
  - Robustness with alternate design
    ...
    - Methods
      1.  Remove a donor from the donor pool and refit the model and see if the results hold. Repeat with each donor.\
          ![](./_resources/Econometrics,_General.resources/Screenshot%20(247).png){.lightbox width="532"}
  ...
  - Pre-Post Error Ratio
    ...
    - P-Value ([article](https://towardsdatascience.com/understanding-synthetic-control-methods-dd9a291885a1))
      ...
      ```
- [Potential Issues and Solutions]{.underline}
...
## Interrupted Time Series (ITS) {#sec-econ-gen-its .unnumbered}

![](./_resources/Econometrics,_General.resources/1-p3YpUJuFKDBte0aoA3egBg.png){.lightbox width="432"}
...
- [Misc]{.underline}
  - Packages
    - [{]{style="color: #990000"}[gpss](https://cran.r-project.org/web/packages/gpss/index.html){style="color: #990000"}[}]{style="color: #990000"} - Gaussian Processes for Social Science
    - [{]{style="color: #990000"}[multipleITScontrol](https://cran.r-project.org/web/packages/multipleITScontrol/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides tools to perform interrupted-time series through a generalised least squares (GLS) framework on linear outcomes.
      - Allows for multiple interventions and a control with ARMA (autoregressive and moving-average) correction.
  - Notes from: [A Practitioner's Guide To Interrupted Time Series](https://towardsdatascience.com/what-is-the-strongest-quasi-experimental-method-interrupted-time-series-period-f59fe5b00b31)
- [Strengths]{.underline}
...
- [Power and Sample Size Considerations]{.underline}
...
## Regression Discontinuity Design (RDD) {#sec-econ-gen-rdd .unnumbered}

![](./_resources/Econometrics,_General.resources/excalidraw-rdd.png){.lightbox width="532"}
...
- [Misc]{.underline}
  - Packages
    - [{]{style="color: #990000"}[eqtesting](https://cran.r-project.org/web/packages/eqtesting/index.html){style="color: #990000"}[}]{style="color: #990000"} - Equivalence Testing Functions that provide statistically significant evidence that running variable (RV) manipulation around a cutoff is bounded beneath practically negligible levels.
    ...
    - [{]{style="color: #990000"}[MIRDD](https://cran.r-project.org/web/packages/MIRDD/index.html){style="color: #990000"}[}]{style="color: #990000"} - Diagnostic Tool by Multiple Imputation for Regression Discontinuity Designs
    - [{]{style="color: #990000"}[mrddGlobal](https://cran.r-project.org/web/packages/mrddGlobal/index.html){style="color: #990000"}[}]{style="color: #990000"} - Global Testing for Multivariate Regression Discontinuity Designs
    ...
    - [{]{style="color: #990000"}[rdd](https://cran.r-project.org/web/packages/rdd/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides the tools to undertake estimation in Regression Discontinuity Designs. Both sharp and fuzzy designs supported
    - [{]{style="color: #990000"}[rdhte](https://cran.r-project.org/web/packages/rdhte/index.html){style="color: #990000"}[}]{style="color: #990000"} - Heterogeneous Treatment Effects in Regression Discontinuity Designs
    - [{]{style="color: #990000"}[gpss](https://cran.r-project.org/web/packages/gpss/index.html){style="color: #990000"}[}]{style="color: #990000"} - Gaussian Processes for Social Science
  - [Gelman](https://statmodeling.stat.columbia.edu/2023/09/21/in-which-we-answer-some-questions-about-regression-discontinuity-designs/): The big mistakes seem to come from:
  ...
  - [Lakeland](https://statmodeling.stat.columbia.edu/2023/09/21/in-which-we-answer-some-questions-about-regression-discontinuity-designs/#comment-2264411) recommends using Bayesian estimation and Chebyshev Polynomials
- [Types]{.underline}
  - Sharp RDD:
...
- [Terms]{.underline}
  - [**Forcing**]{style="color: #009499"} or [**Assignment**]{style="color: #009499"} or [**Running**]{style="color: #009499"} [**Variable**]{style="color: #009499"}:
  ...
  - [**Bandwidth**]{style="color: #009499"} - The number of points selected on each side of the cutoff
...
- [Example]{.ribbon-highlight}: Corruption
  - From

    - Article: [Quasi-Experimental Design: Regression Discontinuity Design](https://taiwoahmed.com/2021/03/10/quasi-experimental-method-regression-discontinuity-design/)
    - Paper: [Businesspeople in Elected Office: Identifying Private Benefits from Firm-Level Returns](https://faculty.smu.edu/millimet/classes/eco7377/papers/szakonyi%202018.pdf)
      ...
      - Examine the balance along a range of covariates between winning and losing candidates in close elections (i.e. around the threshold).
        ...
        - Is there any coordination among firms and their candidates?
          - If so, we'd expect to see a sharing of the spoils after the election. Therefore, some conspicuous number of firm's revenue or profit margin should increase even though they lost.\
            ![](./_resources/Econometrics,_General.resources/rdd2.png){.lightbox width="532"}![](./_resources/Econometrics,_General.resources/rdd1.png){.lightbox width="532"}
  ...
  - Fit the RDD
    ...
    - Outcome: [cons\$fullturnover.e.l.d]{.var-text}
    - Running: [cons\$margin]{.var-text}
    - [c]{.arg-text} is the cutoff (default = 0)
    - [all = TRUE]{.arg-text} says to report three different methods for std.errors
    ...
    - [BW est]{.arg-text} (for Conventional estimate), [BW bias]{.arg-text} (for bias-corrected estimate) are the bandwidths used
      - `rdbwselect` can be used to calculate diffferent bandwidths and then specified in `rdrobust` with [h]{.arg-text} and [b]{.arg-text} args.
    ...
    - [p]{.arg-text} and [q]{.arg-text} args specify the order of polynomial to be used to fit the Conventional model and Bias-corrected model respectively (default = 2 , quadratic)
    - Interpretation
...
- [Example]{.ribbon-highlight}: Sometimes a rdd isn't the answer
  ...
  - Description:
    - Aliso Canyon gas leak leads many schools to install air filters. RDD study shows test scores went up after the filters were installed. What follows is how Gelman would have conducted the study.\
      ![](./_resources/Econometrics,_General.resources/Screenshot%20(308).png){.lightbox width="432"}
  - Steps
...
## Propensity Score Matching {#sec-econ-gen-psm .unnumbered}

- A [**Propensity Score**]{style="color: #009499"} is the probability of being assigned to a certain treatment, conditional on pre-treatment (or baseline) characteristics

### Misc {#sec-econ-gen-psm-misc .unnumbered}

- Also see [Survey, Analysis \>\> Weights](surveys-analysis.qmd#sec-surveys-anal-wts){style="color: green"} \>\> Types \>\> Inverse Probability Weights
- Packages
  - [{MatchIt}]{style="color: #990000"}: propensity score methods
  ...
  - [{]{style="color: #990000"}[MatchThem](https://cran.r-project.org/web/packages/MatchThem/index.html){style="color: #990000"}[}]{style="color: #990000"}
    - Provides essential tools for the pre-processing techniques of matching and weighting multiply imputed datasets.
    - [Vignette](https://journal.r-project.org/archive/2021/RJ-2021-073/index.html)
  - [{]{style="color: #990000"}[nbpMatching](https://cran.r-project.org/web//packages/nbpMatching/index.html){style="color: #990000"}[}]{style="color: #990000"} - Functions for Optimal Non-Bipartite Matching. Reweighted Mahalanobis Distance Matching
  ...
  - Others
    - [{twang}]{style="color: #990000"}, [{]{style="color: #990000"}[Matching](https://cran.r-project.org/web/packages/Matching/index.html){style="color: #990000"}[}]{style="color: #990000"}, [{optmatch}]{style="color: #990000"}, [{CBPS}]{style="color: #990000"}, [{ebal}]{style="color: #990000"}, [{WeightIt}]{style="color: #990000"}, [{designmatch}]{style="color: #990000"}, [{sbw}]{style="color: #990000"}, and [{cem}]{style="color: #990000"}
  - Viz
    - [{cobalt}]{style="color: #990000"}: balance tables and plots using output from above packages
- Notes from
  ...
  - Paper: [Choosing the Estimand When Matching or Weighting in Observational Studies](https://arxiv.org/ftp/arxiv/papers/2106/2106.10577.pdf)
    - How to choose an estimand based on your question (and how that maps to particular weighting / matching choices)\
      ![](./_resources/Econometrics,_General.resources/FBwqrVtWYAkZO1v.png){.lightbox width="632"}

### BBR Chapter 17.2 to 17.8 {#sec-econ-gen-psm-bbr-ch17 .unnumbered}

- Biostatistics for Biomedical Research (Harrell) [Ch.17.2](http://hbiostat.org/bbr/propensity.html) - 17.8: Modeling for Observational Treatment Comparisons
...
- Modeling
  ...
  - If outcome is binary and you can afford to ignore prognostic variables, use nonparametric regression, Y \~ PS, and fit a model to each treatment group's data
    - Nonparametric Regression - does not assume linearity; only assumes smoothness, Y \~ X where X is continuous
      ...
      - See BBR Ch [8.7](http://hbiostat.org/bbr/corr.html#nonparametric-regression) for details, examples (no binary outcome examples)
...
### Psuedo-Distributions After Weighting According to the Type of Estimand {#sec-econ-gen-psm-sdawatttoe .unnumbered}
...
- [No Estimand]{.underline}
  ...
  - No groups are upweighted (or equivalently, for both groups, weights = 1)\
    ![](./_resources/Econometrics,_General.resources/FBwlxCYX0AIi8qy.png){.lightbox width="432"}
...
- [Average Treatment Effect (ATE)]{.underline}
  - Target: whole population
  - Treated and Control groups are upweighted\
    ![](./_resources/Econometrics,_General.resources/FBwnUtyWYAEok4I.jpeg){.lightbox width="432"}
    - Light green and light blue show psuedo-counts that are added to the groups after applying weights
...
- [Average Treatment Effect on the Treated (ATT)]{.underline}
  - Target: treatment group
  - Control group is upweighted\
    ![](./_resources/Econometrics,_General.resources/FBwpDEiXoAAO_TQ.jpeg){.lightbox width="432"}
    - Individual $\text{unit}_i$ weights
...
- [Average Treatment Among Overlap Population (ATO)]{.underline}
  ...
  - Treated is downweighted\
    ![](./_resources/Econometrics,_General.resources/FBwqHmtXsAA7ZGi.jpeg){.lightbox width="432"}
    - Individual $\text{unit}_i$ weights
