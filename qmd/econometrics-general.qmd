...
* **Endogenous Variable** - variables that are correlated with the population error term. "Determined inside the model".
	...
	* Also see [Glossary: DS terms](Glossary_ DS terms)
...
* Also HC4, HC5, and modified HC4m
	...
	* Cribari-Neto F., Da Silva W.B. (2011). “A New Heteroskedasticity-Consistent Covariance Matrix Estimator for the Linear Regression Model.” _Advances in Statistical Analysis_, 95(2), 129--146
...
* Model![](./_resources/Econometrics,_General.resources/unknown_filename.png)
...
* Terms
	* _endogenous variable_ - variables that are correlated with u, the population error term. "Determined inside the model".
		* When the causality between X and Y goes both directions, both are endogenous.
	* _exogenous variable_ - variables that are NOT correlated with u. "Determined outside the model"
* Conditions for valid instruments
	...
	3. Exclusion restiction: no impact on the dependent variable _directly_. It only impacts the dependent variable through its impact on the treatment variable
		* Check
...
* Good instruments
	...
	* Examples
		...
		* outcome: #\_of\_kids (fertility), predictor: years\_of\_education, instrument: pre/post government policy that increases mandatory years of education
* Steps
...
* Effects (Also see LATE and compliance in [Experiments, Design](Experiments, Design))
	* Using an instrumental variable allows us to identify the impact of the treatment on _compliers_. This is known as the local average treatment effect or LATE.
		...
		* \*\*Requires an extra restriction on the instrumental variable\*\*
			* Monotonocity (no _defiers_): There is no one in the sample that does not receive the treatment because they received the instrument. This is usually a reasonable assumption to make but it can only be made based on intuition.
		...
		* LATE = ATE if _any_ of the following is true
...
![](./_resources/Econometrics,_General.resources/excalidraw-did.png)
...
* Basically a standard lm with an interaction between the treatment indicator and time period indicator demarking before/after treatment.![](./_resources/Econometrics,_General.resources/image.png)
* Predictions![](./_resources/Econometrics,_General.resources/image.1.png)
...
* Example
	...
	* We can estimate the treatment effect by taking the _difference_ between the (post-treatment _difference_ between populations)(solid lines after treatment) and (the pre-treatment _difference_ between populations) (solid lines before treatment)
	...
	* If not for the treatment, the two groups being compared would have **parallel trends** in the outcome. Note that groups are allowed to have different _levels_ but must have similar trends over time
...
* Data
	* The larger the pre-treatment period the smaller the bias of the synthetic control estimator (assuming the synthetic control closely tracks the outcome variable during the pre-treatment period.
		...
		* If the amount of pretreatment data is relatively small, then you need very good predictors of the _post-treatment_ outcome such that residual variance will be small which will reduce the chance of overfitting.
* Robustness checks
	* in-time placebo test (backdating): move the treatment date backwards in the data. If the synthetic control still closely tracks the outcome variable until the actual treatment date, then this is evidence of a reliable synthetic control![](./_resources/Econometrics,_General.resources/Screenshot (245).png)
	...
	* Robustness with alternate design
		* Methods
			1. Remove a donor from the donor pool and refit the model and see if the results hold. Repeat with each donor.![](./_resources/Econometrics,_General.resources/Screenshot (247).png)
	...
	* Pre-Post error ratio![](./_resources/Econometrics,_General.resources/image.2.png)
...
* Potential Issues and Solutions
	* Volatility of the outcome variable is low. Small or even large effects are difficult to detect if the outcome experiences a lot of _shocks_ that are larger or comparable to the size of the effect.
...
* analysis of a single time-series data before and after the intervention
	...
	![](./_resources/Econometrics,_General.resources/1-p3YpUJuFKDBte0aoA3egBg.png)
...
Regression Discontinuity Design (RDD)
![](./_resources/Econometrics,_General.resources/excalidraw-rdd.png)
		...
		* Check for manipulation of the running variable
			* Examine the balance along a range of covariates between winning and losing candidates in close elections (i.e. around the threshold).
				...
				* Is there any coordination among firms and their candidates?
					* If so, we'd expect to see a sharing of the spoils after the election. Therefore, some conspicuous number of firm's revenue or profit margin should increase even though they lost. ![](./_resources/Econometrics,_General.resources/rdd2.png)![](./_resources/Econometrics,_General.resources/rdd1.png)
			...
			* Density Test: assess the validity of the assumption of continuity around the threshold.
				* Have to check out docs + referenced papers to get a detailed idea of whats happening, but the p-value is what's _important_
...
* [Example]{.ribbon-highlight} (Sometimes a rdd isn't the answer)
	...
	* Description:
		* Aliso Canyon gas leak leads many schools to install air filters. RDD study shows test scores went up after the filters were installed. What follows is how Gelman would have conducted the study.![](./_resources/Econometrics,_General.resources/Screenshot (308).png)
	* Steps
...
* Misc
	* Also see [Survey, Analysis](Survey, Analysis) >> Weights >> Types >> Inverse Probability Weights
	* packages
	...
	* Notes from
		...
		* Paper: [Choosing the Estimand When Matching or Weighting in Observational Studies](https://arxiv.org/ftp/arxiv/papers/2106/2106.10577.pdf)
			* how to choose an estimand based on your question (and how that maps to particular weighting / matching choices)![](./_resources/Econometrics,_General.resources/FBwqrVtWYAkZO1v.png)
	* Harrell (bbr [Ch.17.2](http://hbiostat.org/bbr/propensity.html) - 17.8)
		* Adjusting for Confounders
			...
			* Doing an adjusted analysis where the adjustment variable is the PS simultaneously adjusts for all the variables in the score _insofar_ as confounding is concerned (but not with regard to outcome heterogeneity)
			* Stratifying for PS does not remove all the measured confounding
		* But adjusting _only_ for PS is inadequate.
		...
		* Types
			...
			* Inverse Probability Weighting
				...
				* Also see [Survey, Analysis](Survey, Analysis) >> Weights >> Types >> Inverse Probability Weights
		* Modeling
			* ![](./_resources/Econometrics,_General.resources/Screenshot (1062).png)
				* In biostatistics, a _prognostic factor_ or variable is a patient characteristic that can predict that patient's eventual response to an intervention
...
* Concept: Psuedo-Distributions after weighting according to the type of estemand
	...
	* No estimand
		...
		* No groups are upweighted (or equivalently, for both groups, weights = 1)![](./_resources/Econometrics,_General.resources/FBwlxCYX0AIi8qy.png)
	...
	* Average Treatment Effect (ATE)
		...
		* Treated and Control groups are upweighted![](./_resources/Econometrics,_General.resources/FBwnUtyWYAEok4I.jpeg)
			* Light green and light blue show psuedo-counts that are added to the groups after applying weights
			* Both groups now similar (i.e. comparable)
			* individual uniti weights
				* treatment\_weighti = 1 / propensity\_scorei
				* control\_weighti = 1 / (1 - propensity\_scorei)
		* Potential Issues
	...
	* Average Treatment Effect on the Treated (ATT)
		...
		* Control group is upweighted![](./_resources/Econometrics,_General.resources/FBwpDEiXoAAO_TQ.jpeg)
			* Individual uniti weights
				* treatment\_weighti = 1
				* control\_weighti = propensity\_scorei / (1 - propensity\_scorei)
		* Potential Issues
	...
	* Average Treatment Among Overlap Population (ATO)
		...
		* Treated is downweighted![](./_resources/Econometrics,_General.resources/FBwqHmtXsAA7ZGi.jpeg)
			* Individual uniti weights
				* treatment\_weighti = 1 - propensity\_scorei
				* control\_weighti = propensity\_scorei
