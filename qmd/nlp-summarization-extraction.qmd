...
- Packages
  ...
  - [{]{style="color: goldenrod"}[GLiNER](https://github.com/urchade/GLiNER){style="color: goldenrod"}[}]{style="color: goldenrod"} - Generalist and Lightweight Model for Named Entity Recognition
    - Supports incremental streaming NER, joint entity and relation extraction, and multi-task token classification.
    - GLiNER is fine-tunable, optimized to run on CPUs and consumer hardware, and has performance competitive with LLMs several times its size, like ChatGPT and UniNER.
    - Part of a broader ecosystem that includes: classification, entity linking, vLLM and spaCy integrations, Rust and C++ versions, etc.
- Named Entity Recognition (NER) is a specific type of extraction that focuses on identifying and classifying named entities (like people, organizations, and locations) within unstructured text
