...
- Notes from
  ...
  - [Demystifying causal inference estimands: ATE, ATT, and ATU:Explore why we care about the ATE, ATT, and ATU and figure out how to calculate them with observational data](https://www.andrewheiss.com/blog/2024/03/21/demystifying-ate-att-atu/) (Heiss) (Haven't read yet)
- Packages
  - [{]{style="color: #990000"}[BCDAG](https://cran.r-project.org/web/packages/BCDAG/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Vignette](https://www.jstatsoft.org/article/view/v116i05)) - **Bayesian** Structure and Causal Learning of **Gaussian** Directed Graphs
  - [{]{style="color: #990000"}[caugi](https://caugi.org/){style="color: #990000"}[}]{style="color: #990000"} - A fast and flexible toolbox for building, coercing and analyzing causal graphs
    - DAG builder with some unique graphing features. Some graph analysis tests as well.
  - [{]{style="color: #990000"}[MLCausal](https://cran.r-project.org/web/packages/MLCausal/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides an end-to-end workflow for estimating average treatment effects in clustered (multilevel) observational data.
    - Includes cluster-aware propensity score estimation using fixed effects and Mundlak-style specifications, inverse probability weighting, within-cluster nearest-neighbor matching.
    - Has covariate balance diagnostics at both individual and cluster-mean levels, outcome regression with cluster-robust standard errors, propensity score overlap visualization, and tipping-point sensitivity analysis for omitted cluster-level confounding.
  - [{]{style="color: #990000"}[DAGassist](https://cran.r-project.org/web/packages/DAGassist/index.html){style="color: #990000"}[}]{style="color: #990000"} - **Test Robustness** with Directed Acyclic Graphs
    - Given a 'dagitty' DAG object and a model specification, 'DAGassist' classifies variables by causal roles, flags problematic controls, and generates a report comparing the original model with minimal and canonical adjustment sets.
  - [{]{style="color: #990000"}[dagitty](https://cran.r-project.org/web/packages/dagitty/index.html){style="color: #990000"}[}]{style="color: #990000"} ([tool](https://www.dagitty.net/dags.html)) - A port of the web-based software 'DAGitty' for analyzing structural causal models (also known as directed acyclic graphs or DAGs). (See [{ggdag}]{style="color: #990000"})
  - [{]{style="color: #990000"}[dda](https://cran.r-project.org/web/packages/dda/index.html){style="color: #990000"}[}]{style="color: #990000"} - A collection of tests to analyze the **causal direction** of dependence in **linear models** (including interactions)
    - Perform Direction Dependence Analysis for variable distributions, residual distributions, and independence properties of predictors and residuals in competing causal models.
  - [{]{style="color: #990000"}[EconCausal](https://cran.r-project.org/web/packages/EconCausal/index.html){style="color: #990000"}[}]{style="color: #990000"} - Causal Analysis for **Macroeconomic Time Series** (ECM-MARS, BSTS, Bayesian GLM-AR(1))
  - [{]{style="color: #990000"}[fastEDM](https://edm-developers.github.io/fastEDM-r/){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://jinjingli.github.io/edm/edm-wp.pdf), [Video](https://youtu.be/kZv85k1YUVE), [Video](https://www.youtube.com/watch?v=ZYKoqt_nzfI&embeds_referring_euri=https%3A%2F%2Fedm-developers.github.io%2F))- Implements a series of ***Empirical Dynamic Modeling*** tools that can be used for causal analysis of **time series** data. (Also avaiable in [python](https://edm-developers.github.io/fastEDM-python/))
  - [{]{style="color: #990000"}[fasttreeid](https://cran.r-project.org/web/packages/fasttreeid/index.html){style="color: #990000"}[}]{style="color: #990000"} - Determines, for a structural causal model (SCM) whose directed edges form a tree, whether each parameter is unidentifiable, 1-identifiable or 2-identifiable (other cases cannot occur), using a randomized algorithm with provable running time $O(n^3 log^2 n$)
  - [{]{style="color: #990000"}[ggdag](https://r-causal.github.io/ggdag/){style="color: #990000"}[}]{style="color: #990000"} - Uses [{dagitty}]{style="color: #990000"} to **create and analyze** structural causal models (**DAGs**) and plot them using [{ggplot2}]{style="color: #990000"} and [{ggraph}]{style="color: #990000"} in a consistent and easy manner.
    - Also see [Confounding](causal-inference.qmd#sec-causinf-confound){style="color: green"} \>\> Example 2
    - Edge Types: [{ggraph}]{style="color: #990000"} [reference](https://ggraph.data-imaginist.com/reference/index.html#edges), [vignette](https://ggraph.data-imaginist.com/articles/Edges.html)
    - Change color of edges: [{ggraph}]{style="color: #990000"} [scales](https://ggraph.data-imaginist.com/reference/index.html#scales)
  - [{]{style="color: #990000"}[infocausality](https://stscl.github.io/infocausality/){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://www.nature.com/articles/s41467-024-53373-4)) - Methods for quantifying **temporal** and **spatial** causality through information flow, and decomposing it into unique, redundant, and synergistic components. (Seems to be for spatial or time series and not explicitly spatio-temporal)
  - [{]{style="color: #990000"}[spEDM](https://stscl.github.io/spEDM/){style="color: #990000"}[}]{style="color: #990000"} - **Spatial Empirical Dynamic Modeling**
- Resources
...
- Papers
  - [A Latent Causal Inference Framework for Ordinal Variables](https://arxiv.org/abs/2502.10276) ([code](https://github.com/martinascauda/OrdinalEffects))
  - [Perfect counterfactuals for epidemic simulations](https://royalsocietypublishing.org/doi/10.1098/rstb.2018.0279) ([code](https://github.com/HopkinsIDD/cfepi))
  - [Cross-Validated Causal Inference: a Modern Method to Combine Experimental and Observational Data](https://arxiv.org/abs/2511.00727) ([code](https://github.com/xyang23/cross_validated_causal))
  - [Complementary strengths of the Neyman-Rubin and graphical causal frameworks](https://arxiv.org/abs/2512.09130)
    - Specific examples of data-generating mechanisms - such as those involving undirected or deterministic relationships and cycles - where analyses using a directed acyclic graph are challenging, but where the tools from the Neyman-Rubin causal framework are readily applicable.
    - Examples of data-generating mechanisms with M-bias, trapdoor variables, and complex front-door structures, where the application of the Neyman-Rubin approach is complicated, but the graphical approach is directly usable.
- Pearl Causal Hierarchy (PCH)
  - First Level: Reasons about associations and passively observed data
  - Second Level: Investigates the effects of interventions (e.g., determining the effect of a policy change).
  - Third Level: Counterfactuals — reasoning about hypothetical situations that would have occurred under different interventions, as opposed to actual events.
...
- Engineering outcome variables using potential adjustment variables does not automatically adjust for those variables in your model
  ...
  - [Example]{.ribbon-highlight}\
    ![](./_resources/Causal_Inference.resources/image.png){.lightbox width="200"}
...
- Randomized experiments remove all paths from the treatment variable, X\
  ![](./_resources/Causal_Inference.resources/Screenshot%20(623).png){.lightbox width="154"}
...
- Table 2 fallacy (Notes from McElreath [video](https://www.youtube.com/watch?v=KNPYUVmY3NM), 2022 SR Lecture 6)\
  ![](./_resources/Causal_Inference.resources/tab2-fal.png){.lightbox width="200"}
...
- Causal Forests
  - Instead of minimizing prediction error (RFs), data is split in order to maximize the difference across splits in the relationship between an outcome variable and a “treatment” variable. This is intended to uncover how treatment effects vary across a sample.
  ...
  - Notes from
    - [Library of Statistical Techniques - Causal Forests](https://lost-stats.github.io/Machine_Learning/causal_forest.html)
  - Packages
    - [{]{style="color: #990000"}[grf](https://grf-labs.github.io/grf/){style="color: #990000"}[}]{style="color: #990000"} - Provides non-parametric methods for heterogeneous treatment effects estimation (optionally using right-censored outcomes, multiple treatment arms or outcomes, or instrumental variables), as well as least-squares regression, quantile regression, and survival regression, all with support for missing covariates.
  - Resources
...
- Synthetic Datasets
  - Notes from
    - [Position: Causal Machine Learning Requires Rigorous Synthetic Experiments for Broader Adoption](https://arxiv.org/abs/2508.08883)
  - Evaluations of Causal ML methods rely predominantly on synthetic datasets due to the fundamental problem of causal inference: for the same unit, it is impossible to observe the outcome under both conditions: when it has acted and when it has not.
  - Despite the advantages of synthetic data (e.g. controlled experiments), the reliance on synthetic data constitutes a major adoption limitation, as such datasets often fail to represent the complexities of real-world scenarios. Its inability to capture unknown unknowns—phenomena or dynamics outside the scope of the data-generating process underscores the importance of complementing synthetic data evaluations with real-world experiments and alternative methodologies to ensure comprehensive and robust assessments 
  - Principles for Rigorous Causal ML Synthetic Evaluation
    1.  Synthetic data is Necessary for Rigorous Evaluation
        - Since real-world datasets cannot provide access to counterfactual outcomes
        - Although, synthetic data alone is not necessarily sufficient or the only valid way of evaluating a Causal ML method.
        - Semi-synthetic data can introduce a degree of realism that is valuable in certain contexts.
          - However, such data also originates from Causal ML generation methods, which must themselves be rigorously evaluated before they can be relied upon.
          - Therefore, semi-synthetic data generation methods should first be rigorously evaluated using synthetic data to identify their limitations.
    2.  Synthetic Design Choices Must Be Clearly Stated to Mitigate Unconscious Bias
        - Make all experiment design choices transparent, to be clear on the domain of validity of the drawn results.
        - Experimental Choices to be Made Transparent
          1.  The *set of causal models* studied
          2.  The *set of causal queries* of interest
          3.  The *set of training data*
          4.  The *generation algorithm* producing the synthetic causal models, queries, and datasets
          5.  The *distribution* that the generation algorithm implies on the space of synthetic examples defined by the Choices one, two, and three.
        - See Section 4.2 of the paper for an example
    3.  Go Beyond Aggregated Accuracy in the Identification Domain With Comprehensive Experiments
        - Methods must also be tested in scenarios that challenge their assumptions, such as shifts in causal structures. Evaluating models both within and beyond their identification domains exposes failure modes and provides insights into their limits under real-world conditions.
          - Define large synthetic experimentation spaces, i.e., large sets of causal models and datasets as defined in Principle 1
        - Reducing evaluation to accuracy risks ignoring critical dimensions such as robustness, scalability, stability, and interpretability
        - Focusing solely on performance metrics can lead to biased evaluations and a misleading perception of progress. By systematically documenting negative results, exploring edge cases, and analyzing failure scenarios, researchers can uncover deeper knowledge that drives theoretical and practical advancements.
        - Take a progressive approach to increasing simulation complexity.
          - Begin with a simple, intuitive experiment, e.g., a small graph, simple mechanisms, binary variables, sufficient data, and no assumption violations.
          - Gradually introduce one new complexity at a time, e.g., hidden confounders, selection bias, small datasets, or mixtures of SCMs.
            - For each added complexity, carefully analyze its impact on the method’s performance before adding the next one.
          - Additional complexities to consider include the presence of outliers, violation of the ignorability assumption, erroneous causal graph, or adjustment sets
    4.  Developing Standardized Evaluation Frameworks to Promote Best Practices
        - Benchmarking Platforms for CausalAI
          - CauseME is dedicated to causal discovery in time series data
          - CausalBench encompasses a broader scope of causal tasks, such as treatment effect estimation.
        - These platforms provide pre-coded datasets, models, metrics, and evaluation algorithms, which make evaluation procedures more transparent, fair, and easy to use.
        - Neither platform currently includes counterfactual estimation tasks. In addition, information on datasets, particularly (semi-) synthetic ones, lacks sufficient details to comply with Principle 2
- Appropriate language for results based on the research question ([source](https://www.bmj.com/content/392/bmj-2025-085749))\
  ![](_resources/Causal_Inference.resources/approp-lang-for-res-quest.jpg){.lightbox}

## Causal Design {#sec-causinf-causdes .unnumbered}

### Misc {#sec-causinf-causdes-misc .unnumbered}
...
- Examples
  - [Algorithms in Judges’ Hands: Incarceration and Inequity in Broward County, Florida](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4839075) (Also in R \>\> Documents \>\> Causal Inference)
  - Causal inference, social media, and mental health [part 1](https://sjwild.github.io/blog/2024/09/21/causal-inference-social-media-mental-health-part-1.html), [part2](https://sjwild.github.io/blog/2024/09/30/causal-inference-social-media-mental-health-part-2.html), [part 3](https://sjwild.github.io/blog/2024/11/30/causal-inference-social-media-mental-health-part-3.html#building-our-dag)
...
### Creating the DAG {#sec-causinf-causdes-crdag .unnumbered}
...
- Causal Discovery
  - Misc
    - Algorithms that use observational data to fill out a DAG
    - Notes from [Making Causal Discovery work in real-world business settings](https://towardsdatascience.com/making-causal-discovery-work-in-real-world-business-settings-80e80c5f66b8)
    - Packages
      - [{]{style="color: #990000"}[abn](https://r-bayesian-networks.org/){style="color: #990000"}[}]{style="color: #990000"} - Derives a directed acyclic graph (DAG) from empirical data that describes the dependency structure between random variables. The package provides routines for structure learning and parameter estimation of additive Bayesian network models.
      - [{]{style="color: #990000"}[gadjid](https://cran.r-project.org/web/packages/gadjid/index.html){style="color: #990000"}[}]{style="color: #990000"} - Graph Adjustment Identification Distances for Causal Graphs
        - Method for evaluating causal discovery algorithms. So, probably simulate some data (ground truth) that you'll eventually want to collect, and test discovery algorithms against it using this package.
      - [{]{style="color: goldenrod"}[gCastle](https://pypi.org/project/gcastle/){style="color: goldenrod"}[}]{style="color: goldenrod"} ([paper](https://arxiv.org/abs/2111.15155)) - Causal Discovery Toolbox
        - Provides functionalities of generating data from either simulator or real-world dataset, learning causal structure from the data, and evaluating the learned graph, together with useful practices such as prior knowledge insertion, preliminary neighborhood selection, and post-processing to remove false discoveries
    - Markov Equivalence Class - A set of DAGS that are all equally likely.
  - Types
    - Constraint-based: Starts with a fully connected graph and uses conditional-independence tests to remove edges
    - Score-based: Starts with a disconnected graph and iteratively adds new edges computing a score to assess how well the last iteration modeled the distribution
    ...
    - Gradient-based: Essentially a score-based method, but treated as a continually optimized task.
  - Assumptions
    - Causal Markov Condition: *Each variable is conditionally independent of its non-descendants, given its direct causes.*
      - This tells us that if we know the causes of a variable, we don’t gain any additional power by knowing the variables that are not directly influenced by those causes. This fundamental assumption simplifies the modelling of causal relationships enabling us to make causal inferences.
    - Causal Faithfulness: *If statistical independence* holds in the data, then there is *no direct causal relationships* between the corresponding variables.
      - Testing this assumption is challenging and violations may indicate model misspecification or missing variables.
    - Causal Sufficiency: *All the variables included are sufficient to make causal claims about the variables of interest.*
      - In other words, we need all confounders of the variables included to be observed. Testing this assumption involves sensitivity analysis which assesses the impact of potentially unobserved confounders.
    - Acyclicity: *No cycles in the graph.*
  - PC (Peter and Clark) Algorithm
    - Constraint-based algorithm that uses independence tests
      - See [Association, General](association-general.qmd#sec-assoc-gen){style="color: green"} \>\> Hoeffding’s D, KCI test, dcor for continuous variables
        - See mixed types and dcor for mixed variable type comparisons
        - KCI has a conditional test and maybe the Regression kernel can handle mixed variable types.
      - See Discrete Analysis notebook for Chi-Square, Likelihood Ratio, and Fisher's Exact tests for categorical variables
        - Fisher's may have a conditional test and binning a continuous variable may make a potential option for mixed variable types.
    - Steps
      - Start with a fully connected graph. Then use conditional independence tests to remove edges and identify the undirected causal graph (nodes linked but with no direction).
      - Share this undirected graph with domain experts and use them to help add causal directions between the nodes and complete the DAG.

### Analysis Steps {#sec-causinf-causdes-anal .unnumbered}
...
2.  Create simulated data that fits the DAG (i.e. a generative model)
3.  Perform statistical analysis (i.e. SCMs) on the simulated data  to make sure you can measure the causal effect.
4.  Design experiment and collect the data
5.  Run the statistical analysis on the collected data and calculate the average causal effect (ACE) under the assumptions that your DAG and model specifications are correct.
6.  Based on your results, revise the DAG and SCM as necessary and repeat as necessary

### Bad Adjustment Variables {#sec-causinf-causdes-badadj .unnumbered}

- Code and more details included in 2022 SR, Lecture 6
...
- M-bias\
  ![](./_resources/Causal_Inference.resources/bav-mb.png){.lightbox width="150"}
...
- Post-Treatment Bias\
  ![](./_resources/Causal_Inference.resources/bav-ptb.png){.lightbox width="200"}
...
- Selection Bias\
  ![](./_resources/Causal_Inference.resources/bav-sb.png){.lightbox width="150"}
  - Same as collider bias
    - This version adds an unobserved confounder
  - If the experimenter doesn't take into account or know about U, then the dag they'd draw would include an arrow from Z to Y (without U). They'd condition on Z to try to block that path from Z to Y, not knowing it's actually a collider, which would actually open a path from Z to U to Y
  - [Example]{.ribbon-highlight}
    - Y: Income
    - X: Education
    - Z: Values
    - U: Family
- Case-Control Bias\
  ![](./_resources/Causal_Inference.resources/bav-ccb.png){.lightbox width="140"}
  ...
  - [Example]{.ribbon-highlight}
    ...
    - Z: Income
- Precision Parasite\
  ![](./_resources/Causal_Inference.resources/bav-pp.png){.lightbox width="150"}
...
- Peer Bias\
  ![](./_resources/Causal_Inference.resources/bav-pb.png){.lightbox width="150"}
...
- [**Average Causal Efffect (ACE)**]{style="color: #009499"} - average population effect that's calculated from an intervention (see Counterfactual definition for info on Individual Causal Effects)
...
- [**Backdoor Criterion**]{style="color: #009499"} - A valid causal estimate is available if it is possible to condition on variables such that all backdoor paths are closed
...
- [**Backdoor Path**]{style="color: #009499"} - A non-causal path that enters a causal variable in a DAG rather than exits it.
  - e.g. the path that connects a collider to a causal variable points from the collider to the causal variable
  - [**Sub-backdoor Path**]{style="color: #009499"} - this path begins with a frontdoor path but through conditioning on a variable, it opens a connecting backdoor path which biases the treatment effect
    - see Misc \>\> Table 1 Fallacy and Causal Design \>\> Bad Adjustment Variables \>\> Peer Bias
- The [**causal effect**]{style="color: #009499"} is the distribution of Y when we change x, averaged over the distributions of the adjustment variables (Z)![](./_resources/Causal_Inference.resources/ql_14ca188fe155c25bae7b65ee05f51391_l3.png){.lightbox width="483"}
- [**Causal Hierarchy**]{style="color: #009499"} (lowest to highest)
  - *Association*
  ...
  - *Intervention*
    - associated action ([**do-Calculus**]{style="color: #009499"}): *Doing* -  experimental; observing the value of Y after setting X = x
...
- A [**collider**]{style="color: #009499"} along a path blocks that path. However, conditioning on a collider (or any of its descendants) unblocks that path
  - When a collider is conditioned upon, the change in the association between the two nodes it separates is called [**collider bias**.]{style="color: #009499"}
    - e.g. if Z is a collider between X and Y, conditioning upon Z will induce an association between X and Y.
- A [**conditioning set**]{style="color: #009499"}, $L$, is the set of nodes we condition on (it can be empty).
- [**Confounding**]{style="color: #009499"} is the situation where a (possibly unobserved) common cause obscures the causal relationship between two or more variables.
...
- X and Y are [**d-separated**]{style="color: #009499"} by \[**L** if conditioning on all members in \[**L** blocks all paths between the nodes, X and Y.
  - Tool for checking the conditional independencies which are visualized in DAGs.
- A [**descendant**]{style="color: #009499"} is a node connected to a parent node by that parent node's outgoing arrow.
- [**Frontdoor Adjustment**]{style="color: #009499"} - In a causal chain with three nodes X→Z→Y, we can estimate the effect of X on Y *indirectly* by combining two distinct quantities: (Useful for when unobserved confounders prevent direct causal estimation)
  1.  The estimate of the effect of X on Z, P(Z\|do(X))
  2.  The estimate of the effect of Z on Y, P(Y\|do(Z), X)
- [**Frontdoor Path**]{style="color: #009499"} - a path that exits a causal variable in a DAG rather than enters it.
  - e.g. the path that connects a causal variable, X, to an outcome variable, Y, has an arrow that points from X to Y.
- [**Markov Equivalence**]{style="color: #009499"} - A set of DAGs, each with the same conditional independencies
- [**Mediation Analysis**]{style="color: #009499"} - seeks to identify and explain the mechanism or process that underlies an observed relationship between an independent variable and a dependent variable via the inclusion of a third hypothetical variable, known as a [**mediator**]{style="color: #009499"} variable (z-variable in the DAGs of "pipes" below)
...
- [**Moderation Analysis**]{style="color: #009499"} - Like mediation analysis, it allows you to test for the influence of a third variable, Z, on the relationship between variables X and Y, but rather than testing a causal link between these other variables, moderation tests for when or under what conditions an effect occurs.
- A node is a [**parent**]{style="color: #009499"} of another node if it has an outgoing arrow to that node
- A [**path**]{style="color: #009499"} from X to Y is a sequence of nodes and edges such that the start and end nodes are X and Y, respectively.
- [**Residual Confounding**]{style="color: #009499"} occurs when a confounding variable is measured imperfectly or with some error and the adjustment using this imperfect measure does not completely remove the effect of the confounding variable.
...
- [**Structural Causal Models (SCMs)**]{style="color: #009499"} - relate causal and probabilistic statements; each equation is a causal statement
...
![](./_resources/Causal_Inference.resources/unknown_filename.png){.lightbox width="600"}\
![](./_resources/Causal_Inference.resources/unknown_filename.2.png){.lightbox width="600"}
...
![](./_resources/Causal_Inference.resources/unknown_filename.1.png){.lightbox width="500"}
      ...
      - Sometimes you DO condition on these variables
        ...
        - The mediation path can have an important interpretation depending on your research question
          - e.g. Indirect Descrimination
            - i.e. Some of effect of [gender]{.var-text} (independent) on being [admitted]{.var-text} (dependent) has to do with the student's choice of [department]{.var-text} (mediator). Some [departments]{.var-text} are harder to get into because they're more popular and there are few spots.
            - Mediation Analysis can help determine how much of the effect is direct discrimination and indirect discrimination
...
- The fourth DAG is an elemental confound or relation called a "**Collider**."\
  ![](./_resources/Causal_Inference.resources/Screenshot%20(621).png){.lightbox width="285"}
  - In general, do NOT add these variables to your model
...
- The last elemental confound or relation is called a "**Descendent**."\
  ![](./_resources/Causal_Inference.resources/Screenshot%20(132).png){.lightbox width="140"}
  ...
  - Controlling for D will also control, to a lesser extent, for Z. The reason is that D has some information about Z.
    - This will (partially) open the path from X to Y, because Z is a collider.
      - See [Confounding](causal-inference.qmd#sec-causinf-confound){style="color: green"} \>\> Example 2
    - The same holds for non-colliders. If you condition on a descendent of Z in the pipe, it'll still be like (weakly) closing the pipe.

## Dual path DAGs {#sec-causinf-dualpath .unnumbered}

![](./_resources/Causal_Inference.resources/dp-dag1.png){.lightbox width="300"}
...
![](./_resources/Causal_Inference.resources/unknown_filename.3.png){.lightbox width="220"}
...
![](./_resources/Causal_Inference.resources/unknown_filename.4.png){.lightbox width="460"}
...
- The Doing row of DAGs (aka *manipulated* DAGs) represents setting X = x
  ...
  - For DAGs 2 and 3, Y is now UNaffected
    - Using the assumptions and some mathematical manipulation (See article for details):
      - ![](./_resources/Causal_Inference.resources/ql_14ca188fe155c25bae7b65ee05f51391_l3.png){.lightbox width="533" height="19"}
...
- [Example 2]{.ribbon-highlight}: Estimate the direct and total effects from A to Y ([source](https://bsky.app/profile/rmcelreath.bsky.social/post/3m6yhqbn3gc2c))

  ::: panel-tabset
  ## DAG

  ![](_resources/Causal_Inference.resources/conf-ex2-dag-1.png){.lightbox group="conf-ex2" width="482"}
  ...
  - **Note: Set-up code is folded above (including `theme_dag_notebook`)**
  - [layout = "sugiyama"]{.arg-text} is especially designed for DAGs
  - DAGS go from top to bottom by default. By interchanging [x]{.arg-text} and [y]{.arg-text}, [xend]{.arg-text} and [yend]{.arg-text}, along with adding negative signs, you able to make a DAG that flows from left to right.

  ## Paths

  ![](_resources/Causal_Inference.resources/conf-ex2-de-paths-1.png){.lightbox group="conf-ex2" width="532"}
  ...
  ggdag_paths(da_dag, 
    ...
    ggraph::scale_edge_color_manual(values = notebook_colors[[6]],
                                    labels = c("open path", "de-emphasized")) +
    theme_dag_notebook()
  ```

  - There are two direct paths from A to Y and no backdoor paths
  - [shadow = TRUE]{.arg-text} includes the de-emphasized paths so you can still see the whole DAG

  ## Adjustment Sets

  ###### Direct Effect

  ![](_resources/Causal_Inference.resources/conf-ex2-adj-1.png){.lightbox group="conf-ex2" width="532"}
  ...
  ggdag_adjustment_set(da_dag, 
                       effect = "direct", 
                       node_size = 14) +
    scale_color_discrete(palette = notebook_colors[c(5,6)]) +
    theme_dag_notebook(
  ...
  - To estimate A –direct–\> Y, must *also* adjust for B and (C or D).
  - If you adjust for C, it partly opens collider D, so non-causal path D \<– B –\> Y is opened. Therefore, B needs to adjusted for as well.
    - Easy to forget about descendants partly opening parents. Sneaky colliders.

  ###### Total Effect

  ![](_resources/Causal_Inference.resources/conf-ex2-total-1.png){.lightbox width="482"}
  ...
  ggdag_adjustment_set(da_dag, 
                       effect = "total", 
                       node_size = 14) +
    scale_color_manual(values = notebook_colors[6]) +
    geom_dag_text(color = "white") +
    theme_dag_notebook(
  ...
  - There is no additional adjustment needed to estimate the total effect of A on Y. (i.e. just fit Y \~ A)
  :::

## Application: Simpson's Paradox Example {#sec-causinf-appsimp .unnumbered}

- Sex as the adjustment variable           ![](./_resources/Causal_Inference.resources/unknown_filename.12.png){.lightbox width="478"}
  ...
  - Create DAGs\
    ![](./_resources/Causal_Inference.resources/unknown_filename.13.png){.lightbox width="329"}
    - S=1 as being female,
...
- Blood Pressure as the adjustment variable ![](./_resources/Causal_Inference.resources/unknown_filename.17.png){.lightbox width="510"}
  ...
  - ![](./_resources/Causal_Inference.resources/unknown_filename.18.png){.lightbox width="350"}
...
- [Example]{.ribbon-highlight} (2 Moms)
  ...
  - Variables:
    ...
    - B~1~ - Mother's birth order; binary, first born or not
    - B~2~ - Daughters' birth order; binary, first born or not
    - U - unobserved confounds  (shown as curved dotted line)
  - ![](./_resources/Causal_Inference.resources/bmdb.png){.lightbox width="200"}
  ...
  - Causal Effects\
    ![](./_resources/Causal_Inference.resources/Screenshot%20(641).png){.lightbox width="200"}
  ...
  - Calculate linear effect (i.e. regression coefficient) without a regression model using a linear system of equations
    ...
    - Path B~1~ → M: cov(B~1~, M) = b\*var(B~1~)
    - Path B~1~ → D: cov(B~1~, D) = b\*m\*var(B~1~)
...
- [Example]{.ribbon-highlight} (Bayesian 2 Moms)
  - See previous example for link, hypothesis, and definition of the variables\
    ![](./_resources/Causal_Inference.resources/Screenshot%20(649).png){.lightbox width="272"}
  - Functions (right side)
    - Each variable's function's inputs are variables that are causal influences (i.e. have arrows pointing at the particular variable
      - e.g. M has two arrows pointing at it in the DAG: B~1~ and u
  - Code
    ...
    - B~1~ and B~2~ are binary so they get bernoulli distributions
  ...
  - Results\
    ![](./_resources/Causal_Inference.resources/Screenshot%20(650).png){.lightbox width="385"}
    - Truth: no effect
    - 1st 3 lm models shows how the unobserved confound biases the estimate when using a typical regression model to estimate the causal effect
      - Including B~2~ adds precision to the biased estimate since it is causal to the outcome D while adding B1 increases the bias
...
- [Example]{.ribbon-highlight} (Bayesian Peer Bias)
  ...
  - Variables:
    ...
    - R~1~ and R~2~ are proxy variables for Q (e.g. test scores, lab work, extracurriculars, etc.)
  - Assumptions: System is linear
  - DAG and Code\
    ![](./_resources/Causal_Inference.resources/Screenshot%20(652).png){.lightbox width="205"}
    ...
    - R~1~ and R~2~ are shown in the DAG to be influenced by student quality, Q
    - Every prior is normal except for Q's coefficient
  - Results\
    ![](./_resources/Causal_Inference.resources/Screenshot%20(654).png){.lightbox width="401"}
    - Truth: -0.8
...
- [Example]{.ribbon-highlight}
  ...
  - ![](./_resources/Causal_Inference.resources/unknown_filename.27.png){.lightbox width="330"}
  ...
  - Simulation of the SCM (n = 1000) (code in article)\
    ![](./_resources/Causal_Inference.resources/unnamed-chunk-6-1.png){.lightbox width="600"}
...
- [Example]{.ribbon-highlight}(code in article): Test whether Grandma's home remedy can speed recovery time for the common cold
  ...
  - From fitting the model, we find μ = 7, β = -2, Τ = 0, ε~1~ = 0.78
    - Therefore, the Individual Causal Effect for patient 1\
      ![](./_resources/Causal_Inference.resources/countfact-ice.png){.lightbox width="300"}
...
![](./_resources/Causal_Inference.resources/Screenshot%20(850).png){.lightbox width="500"}
...
- Allows you to test for the influence of a third variable, the mediator, on the relationship between (i.e. effect of) the treatment variable, X, and the Outcome variable, Y. You're determining how much of X's effect on Y goes through the mediator.

- The goal of studies conducting mediation analyses, particularly in health research, is often to understand how hypothetical interventions targeting mediators might counter exposure effects in order to guide future treatment and policy decisions as well as intervention development.

- Misc

  - Notes from: [Mediation Models](https://m-clark.github.io/posts/2019-03-12-mediation-models/)
    - Overview of packages (Aug 2020)\
      ![](./_resources/Causal_Inference.resources/Screenshot%20(853).png){.lightbox width="486"}
  ...
  - Packages
    - [{]{style="color: #990000"}[bmabart](https://cran.r-project.org/web/packages/bmabart/index.html){style="color: #990000"}[}]{style="color: #990000"} - Bayesian Mediation Analysis Using BART
    - [{]{style="color: #990000"}[CHIMA](https://github.com/samuelOsarfo/CHIMA){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2508.16883))- Performs high-dimensional mediator screening and testing in the presence of high correlation among mediators using Ridge-HOLP and Approximate Orthogonalization methods.
    - [{]{style="color: #990000"}[HIMA](https://cran.r-project.org/package=HIMA){style="color: #990000"}[}]{style="color: #990000"} ([Vignette](https://jds-online.org/journal/JDS/article/1439/info)) - An R Package for High-Dimensional Mediation Analysis
    - [{]{style="color: #990000"}[ieTest](https://cran.r-project.org/web/packages/ieTest/index.html){style="color: #990000"}[}]{style="color: #990000"} - Tests if the indirect effect from linear regression mediation analysis is equal to 0
    - [{]{style="color: #990000"}[lcmmtp](https://github.com/nt-williams/lcmmtp){style="color: #990000"}[}]{style="color: #990000"} - Efficient and flexible causal mediation with longitudinal mediators, treatments, and confounders using modified treatment policies
      - Paper: [Identification and estimation of mediational effects of longitudinal modified treatment policies](https://arxiv.org/html/2403.09928v1) w/analysis code
    - [{]{style="color: #990000"}[medRCT](https://t0ngchen.github.io/medRCT/){style="color: #990000"}[}]{style="color: #990000"} ([Thread](https://bsky.app/profile/margaritamb.bsky.social/post/3ltl6lxvo2k2o), [JOSS](https://joss.theoj.org/papers/10.21105/joss.08063.pdf)) - For a causal mediation analysis that supports the estimation of interventional effects, specifically interventional effects that are defined such that they map explicitly to a “target trial.”
      - In the target trial, the treatment strategies are specified to reflect hypothetical interventions targeting and thus shifting the joint distribution of the mediators.
    - [{]{style="color: #990000"}[moderate.mediation](https://cran.r-project.org/web/packages/moderate.mediation/index.html){style="color: #990000"}[}]{style="color: #990000"} - Estimate and test the conditional and moderated mediation effects, assess their sensitivity to unmeasured pre-treatment confounding
    - [{]{style="color: #990000"}[power4mome](https://cran.r-project.org/web/packages/power4mome/index.html){style="color: #990000"}[}]{style="color: #990000"} - Power analysis and sample size determination for moderation, mediation, and moderated mediation in models fitted by structural equation modelling using the 'lavaan' package
    - [{]{style="color: #990000"}[rmedsem](https://cran.r-project.org/web/packages/rmedsem/index.html){style="color: #990000"}[}]{style="color: #990000"} - Conducts mediation analysis for structural equation models (SEM) estimated with 'lavaan', 'blavaan', 'cSEM', or 'modsem'.
    - [{]{style="color: #990000"}[RobustMediate](https://cran.r-project.org/web/packages/RobustMediate/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides tools for causal mediation analysis with continuous treatments using inverse probability weighting (IPW).
    - [{]{style="color: #990000"}[TransHDM](https://cran.r-project.org/web/packages/TransHDM/index.html){style="color: #990000"}[}]{style="color: #990000"} - High-Dimensional Mediation Analysis via Transfer Learning
      - Integrates large-scale source data to improve the detection power of potential mediators in small-sample target studies.
      - Addresses data heterogeneity via transfer regularization and debiased estimation while controlling the false discovery rate.
    - [{]{style="color: #990000"}[wsMed](https://yangzhen1999.github.io/wsMed/){style="color: #990000"}[}]{style="color: #990000"} - Designed for two condition within-subject mediation analysis, incorporating SEM models through the lavaan package and Monte Carlo simulation methods
  - Resources
    - [Statistical Rethinking \>\> Chapter 11](https://ercbk.github.io/Statistical-Rethinking-Notebook/qmd/ch11.html){style="color: green"} \>\> Conclusion of Berkeley Admissions example
      - Indirect Descrimination
        ...
        - Mediation Analysis can help determine how much of the effect is direct discrimination and indirect discrimination
      - Also Lecture 9 2022 [video](https://www.youtube.com/watch?v=nPi5yGbfxuo)
    - Recoding Introduction to Mediation, Moderation, and Conditional Process Analysis (Kurz)
      - [Ch. 3 The Simple Mediation Model](https://bookdown.org/content/b472c7b3-ede5-40f0-9677-75c3704c7e5c/the-simple-mediation-model.html)
      - [Ch. 4 Causal Steps, Confounding, and Causal Order](https://bookdown.org/content/b472c7b3-ede5-40f0-9677-75c3704c7e5c/causal-steps-confounding-and-causal-order.html)
      - [Ch. 5 More Than One Mediator](https://bookdown.org/content/b472c7b3-ede5-40f0-9677-75c3704c7e5c/more-than-one-mediator.html)
    - [Thread](https://bsky.app/profile/margaritamb.bsky.social/post/3ltl6lxvo2k2o) with links to papers that do mediation analysis
    - [Reviewer notes: That’s a very nice mediation analysis you have there. It would be a shame if something happened to it.](https://www.the100.ci/2025/03/20/reviewer-notes-thats-a-very-nice-mediation-analysis-you-have-there-it-would-be-a-shame-if-something-happened-to-it/) (Rohrer) - Article that discusses the consideration, pitfalls, and challenges of doing mediation analysis on various types of experimental (cross-sectional, longitudinal, RCT, etc.) data.
  - Papers
    - [Subsampling-based Tests in Mediation Analysis](https://arxiv.org/abs/2411.10648) ([Code](https://github.com/asmita112358/CSMT))
      - Traditional mediation tests, such as the Sobel test rely on asymptotic distributions, and can yield overly conservative results (especially in genomics data)
      - Procedure
        - Develop a test statistic for the mediation effect whose asymptotic null distribution belongs to the same parametric family under all null scenarios (H00, H01, and H10), differing only in their scaling parameters.
        - Implement a subsampling-based approach to construct a scale-invariant studentized statistic whose asymptotic null distribution remains the same across all null scenarios. This ensures the statistic’s distribution is independent of whether the null hypothesis falls under H~00~, H~01~ or H~10~.
        - Apply the Cauchy combination method to aggregate p-values obtained from various subsample splits, yielding a stable and powerful test.
    - [Statistical Approaches for Enhancing Causal Interpretation of the M to Y Relation in Mediation Analysis](https://journals.sagepub.com/doi/10.1177/1088868314542878?__cf_chl_f_tk=kuQsU7x0M.kksMk1sbW_4KYFUYRy8FT9xZfaeCeb6nc-1783379904-1.0.1.1-QlguWp4bOwYIrTG0RMR_cGZTDDjBZon9jXUTgRn0vDM) (\$)
  - Including a mediator and the independent variable in a regression will result in the independent variable not being signficant and the mediator being significant.
    - [Example]{.ribbon-highlight}: Causal effect of education on income
  ...
  - When would you want to only measure the Direct Effect?
    - [Example]{.ribbon-highlight}: Determining the amount of remuneration for discrimination
      - From [Simulating confounders, colliders and mediators](http://freerangestats.info/blog/2023/06/04/causality-sims)
      - Variables
        - Outcome: Pay Gap
        - Treatment: Gender
  ...
  - [**Average Causal Mediation Effect (ACME)**]{style="color: #009499"} (aka [**Indirect Effect**]{style="color: #009499"})- the expected difference in the potential outcome when the mediator took the value that it would have under the treatment condition as opposed to the control condition, while the treatment status itself is held constant.
    ...
    - δ(t) = E\[Y (t, M(t~1~)) − Y (t, M(t~0~))\]
      - where
        - t, t~1~, t~0~ are particular values of the treatment T such that t~1~ ≠ t~0~,
        - M(t) is the potential mediator
        - Y (t, m) is the potential outcome variable
  - [**Average Direct Effect (ADE)**]{style="color: #009499"} - the expected difference in the potential outcome when the treatment is changed but the mediator is held constant at the value that it would have if the treatment equals t.
    - ζ(t) = E\[Y (t~1~, M(t)) − Y (t~0~, M(t))\]
  - The [**Total Effect**]{style="color: #009499"} of the treatment on the outcome is ACME + ADE.
...
- Methods
  ...
  - Product-of-Paths (or difference in coefficients)
    ...
    - Effect formulas for models with an interaction between treatment and moderator ([Paper](https://imai.fas.harvard.edu/research/files/BaronKenny.pdf))
      - mediator: M = α~2~ + β~2~T~i~ + ξ^T^~2~X~i~ + ε~i2~(T\~i\`)
      - outcome: Y = α\~3 + β~3~T~i~ + γM~i~ + κT~i~M~i~ + ξ^T^~3~X~i~ + ε~i3~(T~i~, M~i~)
      - ACME = β~2~(γ + κt) where t = 0,1
      - ADE = β~3~ + κ{α~2~ + β~2~t + ξ^T^~2~Ε(X~i~)}
      - ATE = β~2~γ + β~3~ +κ{α~2~ + β~2~ + ξ^T^~2~Ε(X~i~)}
        - Alternatively, fit Y = α~1~ + β~1~T~i~ + ξ^T^~1~X~i~ + η^T^T~i~X~i~ + ε~i1~
          - Then ATE = β~1~ + η^T^E(X~i~)
      - Notes
        - Variables
          - T is treatment, M is mediator, X is a set of adjustment variables
            - The exponentiated T in ξ^T^ is to let you know it can be a set of coefficients for a set of adjustment variables (I guess)
        - Couldn't figure out why curly braces are being used
        - ACME with have two estimates (t=0, t=1)
        - ATE (average total effect)
        - Ε(X~i~) is the sample average of each adjustment variable and it's multiplied by its associated ξ~2~ coefficient
...
- [Example]{.ribbon-highlight}: Tingley, 2014 Method
  ...
  - [{mediation}]{style="color: #990000"}
    ...
    - Summary - `summary(mediation_result)`\
      ![](./_resources/Causal_Inference.resources/Screenshot%20(846).png){.lightbox width="419"}
...
- [Example]{.ribbon-highlight}: product-of-paths (or difference in coefficients)
  ...
  - [{lavaan}]{style="color: #990000"}
    ...
    - Also outputs the typical summary regression estimates, std.errors, pvals, R^2^ etc.
    - Bootstraps std.errors
    - Same results for "indirect" here as with [{mediation}]{style="color: #990000"} ACME estimate
    - R^2^s are poor for both regression models which could be why no mediation effect is detected.
  ...
  - [{brms}]{style="color: #990000"}
    ...
    - `hypothesis` tests H~0~: a\*b == 0
      - pval \< 0.05 says there is a mediation effect.

  - [{sjstats}]{style="color: #990000"}\
    ![](./_resources/Causal_Inference.resources/Screenshot%20(854).png){.lightbox width="263"}
...
![](./_resources/Causal_Inference.resources/moderation.png){.lightbox width="555"}

- Misc

  - Resources
    - Recoding Introduction to Mediation, Moderation, and Conditional Process Analysis (Kurz)
      - [Ch. 7 Fundamentals of Moderation Analysis](https://bookdown.org/content/b472c7b3-ede5-40f0-9677-75c3704c7e5c/fundamentals-of-moderation-analysis.html)
      - [Ch. 8 Extending the Fundamental Principles of Moderation Analysis](https://bookdown.org/content/b472c7b3-ede5-40f0-9677-75c3704c7e5c/extending-the-fundamental-principles-of-moderation-analysis.html)
      - [Ch. 9 Some Myths and Additional Extensions of Moderation Analysis](https://bookdown.org/content/b472c7b3-ede5-40f0-9677-75c3704c7e5c/some-myths-and-additional-extensions-of-moderation-analysis.html)
      - [Ch. 10 Multicategorical Focal Antecedents and Moderators](https://bookdown.org/content/b472c7b3-ede5-40f0-9677-75c3704c7e5c/multicategorical-focal-antecedents-and-moderators.html)
...
- Like mediation analysis, it allows you to test for the influence of a third variable, Z (moderator), on the relationship between variables X and Y, but rather than testing a causal link between these other variables, moderation tests for when or under what conditions an effect occurs.
  ...
  - Moderators are conceptually different from mediators ("when" (moderator) vs "how/why" (mediator)).
    - There can be moderated mediation effect though. (see [Mediation Analysis](causal-inference.qmd#sec-causinf-medanal){style="color: green"} \>\> Methods \>\> [{mediation}]{style="color: #990000"})
...
- [Example]{.ribbon-highlight}: What is the relationship between the number of hours of sleep (X, independent variable (IV)) a graduate student receives and the attention that they pay to this tutorial (Y, outcome) and is this relationship influenced by their consumption of coffee (Z, moderator)
  ...
  - Plot the simple slopes (1 SD above and 1 SD below the mean) of the moderating effect\
    ![](./_resources/Causal_Inference.resources/unnamed-chunk-7-1.png){.lightbox width="473"}
...
## Unobserved Confounds {#sec-causinf-unconf .unnumbered}

- Misc
  - Packages
    - [{]{style="color: #990000"}[BiTSLS](https://cran.r-project.org/web/packages/BiTSLS/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements bidirectional two-stage least squares (Bi-TSLS) estimation for identifying bidirectional causal effects between two variables in the presence of unmeasured confounding.
    - [{]{style="color: #990000"}[causens](https://cran.r-project.org/web/packages/causens/index.html){style="color: #990000"}[}]{style="color: #990000"} - Perform Causal Sensitivity Analyses Using Various Statistical Methods
    - [{]{style="color: #990000"}[confoundvis](https://cran.r-project.org/web/packages/confoundvis/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides visualization tools for sensitivity analysis to unmeasured confounding in observational studies.
      - Includes contour-based sensitivity plots, robustness curves, and benchmark-oriented graphics that help researchers assess how strong omitted confounding would need to be to attenuate, invalidate, or reverse estimated effects.
    ...
    - [{]{style="color: #990000"}[konfound](https://cran.r-project.org/web/packages/konfound/){style="color: #990000"}[}]{style="color: #990000"} ([vignette](https://joss.theoj.org/papers/10.21105/joss.05779)): Utilizes a sensitivity analysis approach that extends from dichotomous outcomes and omitted variable sensitivity of previous approaches to continuous outcomes and evaluates both changes in estimates and standard errors.
    ...
    - [{]{style="color: #990000"}[ovbsa](https://cran.r-project.org/web/packages/ovbsa/index.html){style="color: #990000"}[}]{style="color: #990000"} - Sensitivity Analysis of Omitted Variable Bias
      - An observed regressor (or a group of observed regressors) is chosen to benchmark the relative strength of association of the omitted regressor with the outcome variable and with the treatment variable.
      - By allowing these sensitivity parameters to take all permissible values, the most conservative estimate of the probability that omitted variable bias can overturn the reported results.
    - [{]{style="color: #990000"}[senseweight](https://cran.r-project.org/web/packages/senseweight/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides tools to conduct interpretable sensitivity analyses for weighted estimators (e.g. survey data)
  - Papers
    - [Hidden yet quantifiable: A lower bound for confounding strength using randomized trials](https://arxiv.org/abs/2312.03871) ([code](https://github.com/jaabmar/confounder-lower-bound))
    ...
    - [When Is Causal Inference Possible? A Statistical Test for Unmeasured Confounding](https://arxiv.org/abs/2508.20366)
      - This test also uses RCT results and Observational data.
- Handling unobserved confounds is typically done via **Partial Identification**
  ...
  - Measure proxies for the unobserved confound if it's not practical/ethical to measure
    - i.e. If the confound is ability, then test scores, letters of recommendation, etc. could be proxies.
      - [Example]{.ribbon-highlight}: 2022 SR Lecture 10 [video](https://www.youtube.com/watch?v=YrwL6t0kW2I&list=PLDcUM9US4XdMROZ57-OIRtIK0aOynbgZN&index=10), [code](https://github.com/rmcelreath/stat_rethinking_2022/blob/main/scripts_animation/10_confounds_poisson.r)\
        ![](./_resources/Causal_Inference.resources/Screenshot%20(886).png){.lightbox width="400"}
...
- Steps for using sensitivity analysis
  1.  Perform a sensitivity analysis to determine plausibly how much of the causal effect is due to confounding paths
      ...
      - [Example]{.ribbon-highlight}: 2022 SR Lecture 10 [video](https://www.youtube.com/watch?v=YrwL6t0kW2I&list=PLDcUM9US4XdMROZ57-OIRtIK0aOynbgZN&index=10), [code](https://github.com/rmcelreath/stat_rethinking_2022/blob/main/scripts_animation/10_confounds_poisson.r)\
        ![](./_resources/Causal_Inference.resources/Screenshot%20(884).png){.lightbox width="400"}\
...
- Notation
  - X is not independent of Y, i.e ![](./_resources/Causal_Inference.resources/ql_b610f9bb0614dc064c09e0854f572246_l3.png){.lightbox width="49"}
  - **conditional independence**: Y is not associated with some variable X, after conditioning on some other variable Z, i.e. ![](./_resources/Causal_Inference.resources/ql_ef21391bca130a6e9d5b600964aa3b19_l3.png){.lightbox width="67" height="16"}
...
- (*Total* ) Causal Effect and *Direct* Causal Effect\
  ![](./_resources/Causal_Inference.resources/Screenshot%20(707).png){.lightbox width="426"}
...
- [Example]{.ribbon-highlight}
  ...
  - Plausible DAGs (note: marriage cannot influence your age... technically)
    1.  ![](./_resources/Causal_Inference.resources/Screenshot%20(90).png){.lightbox width="131"}
    ...
    2.  ![](./_resources/Causal_Inference.resources/Screenshot%20(92).png){.lightbox width="159"}
  ...
  - Testable implications
    - DAG 1
      - The DAG shows all three are associated to each other, i.e. ![](./_resources/Causal_Inference.resources/ql_e69972d8fda85bb78dbb36718aecad23_l3.png){.lightbox width="179" height="15"}
    ...
    - DAG 2
      ...
      - The testable implication is that D is independent of M, conditional on A, i.e. ![](./_resources/Causal_Inference.resources/ql_1537f13742e0be4122a8d8a30a61b41a_l3.png){.lightbox width="63" height="15"}
...
- DAGs that are consistent with the data associations (M & N are associated but the causal relationship isn't known)\
  ![](./_resources/Causal_Inference.resources/Screenshot%20(111).png){.lightbox width="510"}\
...
- Recipe
  ...
  4.  If there are any backdoor paths that are also open, decide which variable(s) to condition on to close it.
      ...
      - Any frontdoor paths that lead to backdoor paths must also be closed (see [Misc](causal-inference.qmd#sec-causinf-misc){style="color: green"} \>\> Table 2 fallacy)
- [Examples]{.ribbon-highlight}:
  - ![](./_resources/Causal_Inference.resources/Screenshot%20(134).png){.lightbox width="200"}
  ...
  - ![](./_resources/Causal_Inference.resources/Screenshot%20(136).png){.lightbox width="200"}
...
- [Frontdoor Adjustment]{.underline}
  ...
  - [Example]{.ribbon-highlight}: 1
    - Our goal is to estimate P(Y\|do(X)). Unfortunately, this relationship between X and Y is confounded by the unobserved variable U, via this backdoor path: X←U→Y. Therefore, we cannot estimate the causal quantity of interest *directly.*\
      ![](./_resources/Causal_Inference.resources/unnamed-chunk-2-1.png){.lightbox width="260"}
  ...
  - [Example]{.ribbon-highlight} 2
    - Same as first example but using [{dosearch}]{style="color: #990000"} package
      ...
      - Output:![](./_resources/Causal_Inference.resources/Screenshot%20(203).png){.lightbox width="300"}
