...
* Intervention
* Confounding
* Application: Simpson's Paradox example
...
* Most of the notes from Definitions to Counterfactuals (see TOC) are from https://fabiandablander.com/r/Causal-Inference.html
...
* Engineering outcome variables using potential adjustment variables does not automatically adjust for those variables in your model
	...
	* Example![](./_resources/Causal_Inference.resources/image.png)
...
* Randomized experiments remove all paths from the treatment variable, X![](./_resources/Causal_Inference.resources/Screenshot (623).png)
...
* Table 2 fallacy (Notes from McElreath [video](https://www.youtube.com/watch?v=KNPYUVmY3NM), 2022 SR Lecture 6)![](./_resources/Causal_Inference.resources/image.png)
...
* Bad Adjustment Variables (Code and more details included in 2022 SR, Lecture 6)
	...
	* M-bias![](./_resources/Causal_Inference.resources/image.png)
	...
	* Post-Treatment Bias![](./_resources/Causal_Inference.resources/image.png)
	...
	* Selection Bias![](./_resources/Causal_Inference.resources/image.png)
		...
		* [Example]{.ribbon-highlight}
			...
			* Z: Values
			* U: Family
	* Case-Control Bias![](./_resources/Causal_Inference.resources/image.png)
		...
		* [Example]{.ribbon-highlight}
			...
			* X: Education
			* Z: Income
	* Precision Parasite![](./_resources/Causal_Inference.resources/image.png)
	...
	* Peer Bias![](./_resources/Causal_Inference.resources/image.png)
...
* **Average Causal Efffect (ACE)** - average population effect that's calculated from an intervention (see Counterfactual definition for info on Individual Causal Effects)
	* If X is binary, then  ![](./_resources/Causal_Inference.resources/unknown_filename.11.png)
		 is the average causal effect (see Simpson's Paradox example)
...
* The **causal effect** is the distribution of Y when we change x, averaged over the distributions of the adjustment variables (Z)![](./_resources/Causal_Inference.resources/ql_14ca188fe155c25bae7b65ee05f51391_l3.png)
* **Causal Hierarchy** (lowest to highest)
	* _Association_
		* associated action: _Seeing -_ observational; observing the value of Y when X = x
			* ![](./_resources/Causal_Inference.resources/unknown_filename.5.png)
				, observational distribution; What values Y would likely take on if X happened to equal x.
				
	* _Intervention_
		* associated action (**do-Calculus**): _Doing_ -  experimental; observing the value of Y after setting X = x
			* ![](./_resources/Causal_Inference.resources/unknown_filename.6.png)
				, interventional distribution; What values Y would likely take on if X would be set to x. 
				
			* Using the _do_ operator allows us to make inferences about the population but not individuals.
	...
	* _Counterfactual_
		* associated action: _Imagining_ - what would be the outcome if the alternative would've happened.
		* _Individual Causal Effects_ can be calculated but it requires stronger assumptions and deeper understanding of the causal mechanisms
...
* A **conditioning set**_,_ **L**, is the set of nodes we condition on (it can be empty).
* **Confounding** is the situation where a (possibly unobserved) common cause obscures the causal relationship between two or more variables.
	...
	* _Collider bias is a type of confounding_. When a collider is controlled for, a second (or more) path opens, and the effect is confounded
...
* **Frontdoor Adjustment** - In a causal chain with three nodes X→Z→Y, we can estimate the effect of X on Y _indirectly_ by combining two distinct quantities:
	(Useful for when unobserved confounders prevent direct causal estimation)
...
* A set of DAGs, each with the same conditional independencies is known as a **Markov Equivalence**
...
Association
![](./_resources/Causal_Inference.resources/unknown_filename.png)![](./_resources/Causal_Inference.resources/unknown_filename.2.png)
...
Single path DAGs
![](./_resources/Causal_Inference.resources/unknown_filename.1.png)
...
* The first 3 DAGs represent the scatter plots above
	...
	* The first and second DAGs are elemental confounds or relations called "**Pipes**."
			...
			* Sometimes you DO condition on these variables
				...
				* The mediation path can have an important interpretation depending on your research question
					* e.g. indirect descrimination
						* See [Statistical Rethinking](Statistical Rethinking) >> Chapter 11 >> Conclusion of Berkeley Admissions example
		...
		* Z is traditionally labelled a _mediator_
...
* The fourth DAG is an elemental confound or relation called a "**Collider**."![](./_resources/Causal_Inference.resources/Screenshot (621).png)
	* In general, do NOT add these variables to your model
	* Z blocks the path between X and Y unless conditioned upon.
	* An association between X and Y is _induced_  by conditioning on Z, lm(Y ~ X + Z)
...
* The last elemental confound or relation is called a "**Descendent**."![](./_resources/Causal_Inference.resources/Screenshot (132).png)
...
Dual path DAGs
![](./_resources/Causal_Inference.resources/Image.png)
...
![](./_resources/Causal_Inference.resources/unknown_filename.3.png)
...
Intervention
![](./_resources/Causal_Inference.resources/unknown_filename.4.png)
...
* The Doing row of DAGs (aka _manipulated_ DAGs) represents setting X = x
	* For DAGs 1 and 4, Y is still affected
		* Moving from seeing to doing didn't change anything![](./_resources/Causal_Inference.resources/unknown_filename.7.png)
	* For DAGs 2 and 3, Y is now UNaffected
		* Using the assumptions and some mathematical manipulation (See article for details):
			* ![](./_resources/Causal_Inference.resources/ql_14ca188fe155c25bae7b65ee05f51391_l3.png)
...
* The rule: After an intervention, _incoming_ arrows are cut from the node where the intervention took place.


Confounding
![](./_resources/Causal_Inference.resources/unknown_filename.3.png)

* The _backdoor criterion_ tells us which variable we need to adjust for in order to for our model to yield a causal relationship between two variables (i.e. graphically, nodes)
...
![](./_resources/Causal_Inference.resources/unknown_filename.12.png)
...
* Create DAGs
* ![](./_resources/Causal_Inference.resources/unknown_filename.13.png)
	* S=1 as being female,
	...
	* Notice that  ![](./_resources/Causal_Inference.resources/unknown_filename.21.png)
		 therefore our calculated effect will be confounded.
...
![](./_resources/Causal_Inference.resources/unknown_filename.17.png)
...
* ![](./_resources/Causal_Inference.resources/unknown_filename.18.png)
* Since Blood Pressure (B) is post-treatment, it has no effect on whether the patient takes the drug or not (D).
* Taking or not taking the drug (D) has an indirect effect on recovery (R) through Blood Pressure (B) along with a direct effect.
* ![](./_resources/Causal_Inference.resources/unknown_filename.20.png)
	 so our calculated effect will be unconfounded.
...
* [Example]{.ribbon-highlight} (2 Moms)
	...
	* Causal Effects![](./_resources/Causal_Inference.resources/Screenshot (641).png)
...
* [Example]{.ribbon-highlight} (Bayesian 2 Moms)
	* See previous example for link, hypothesis, and definition of the variables![](./_resources/Causal_Inference.resources/Screenshot (649).png)
	...
	* Results![](./_resources/Causal_Inference.resources/Screenshot (650).png)
		* Truth: no effect
...
* [Example]{.ribbon-highlight} (Bayesian Peer Bias)
	...
	* DAG and Code![](./_resources/Causal_Inference.resources/Screenshot (652).png)
	...
	* Results![](./_resources/Causal_Inference.resources/Screenshot (654).png)
		* Truth: -0.8
...
* [Example]{.ribbon-highlight}
	...
	* ![](./_resources/Causal_Inference.resources/unknown_filename.25.png)
		, 
		![](./_resources/Causal_Inference.resources/unknown_filename.26.png)
	* ![](./_resources/Causal_Inference.resources/unknown_filename.27.png)
		...
		* manipulated DAG 1 (middle) shows intervention where z is set to a constant
			* _incoming_ causal arrows get cutoff the intervening variable
		* manipulated DAG 1 (right) shows intervention where x is set to a constant
	* Simulation of the SCM (n = 1000) (code in article)
		* ![](./_resources/Causal_Inference.resources/unnamed-chunk-6-1.png)
...
* Example (code in article): Test whether Grandma's home remedy can speed recovery time for the common cold
	* SCM
		* ![](./_resources/Causal_Inference.resources/unknown_filename.29.png)
			...
			* R is recovery time
			* μ is the intercept
			* β is the average causal effect,
...
* From the model, we find μ = 7, β = -2, Τ = 0, ε1 = 0.78
* Individual Causal Effect for patient 1

![](./_resources/Causal_Inference.resources/Image.2.png)
...
Mediation Analysis
![](./_resources/Causal_Inference.resources/Screenshot (850).png)

* Figure
	* c' is the _direct effect_ of X on the outcome after the indirect path has been removed (i.e. conditioned upon, outcome ~ X + mediator)
	* c is the to _total effect_ (outcome ~ X)
	* c - c' equals the _indirect effect_
...
* Misc
	* Notes from: [Mediation Models](https://m-clark.github.io/posts/2019-03-12-mediation-models/)
		* Overview of packages (Aug 2020)![](./_resources/Causal_Inference.resources/Screenshot (853).png)
	...
	* Also see
		...
		* [Statistical Rethinking](Statistical Rethinking) >> Chapter 11 >> Conclusion of Berkeley Admissions example
			* also Lecture 9 2022 [video](https://www.youtube.com/watch?v=nPi5yGbfxuo)
	...
	* **Average Causal Mediation Effect (ACME)** (aka **Indirect Effect**)- the expected difference in the potential outcome when the mediator took the value that it would have under the treatment condition as opposed to the control condition, while the treatment status itself is held constant.
		...
		* δ(t) = E{Y (t, M(t1)) − Y (t, M(t0)){style='color: #990000'}[}]{style='color: #990000'}
			* where
	...
	* **Average Direct Effect (ADE)** - the expected difference in the potential outcome when the treatment is changed but the mediator is held constant at the value that it would have if the treatment equals t.
		* ζ(t) = E{Y (t1, M(t)) − Y (t0, M(t)){style='color: #990000'}[}]{style='color: #990000'}
	* The **Total Effect** of the treatment on the outcome is ACME + ADE.
* Conditions where you likely do NOT need mediation analysis :
	* If you cannot think of your model in temporal or physical terms, such that X _necessarily_ leads to the mediator, which then _necessarily_ leads to the outcome.
...
* Methods
	...
	* Product-of-Paths (or difference in coefficients)
		...
		* Effect formulas for models with an interaction between treatment and moderator ([Paper](https://imai.fas.harvard.edu/research/files/BaronKenny.pdf))
			...
			* ADE = β3 \+ κ{α2 + β2t + ξT2Ε(Xi){style='color: #990000'}[}]{style='color: #990000'}
			* ATE = β2γ + β3 +κ{α2 + β2 + ξT2Ε(Xi){style='color: #990000'}[}]{style='color: #990000'}
...
* `Example:` Tingley, 2014 Method
	* Equations![](./_resources/Causal_Inference.resources/Screenshot (848).png)![](./_resources/Causal_Inference.resources/Screenshot (844).png)
		* Predictions for "job\_seek" in the mediator model (top) are used as predictor values in the outcome model (bottom).
	* Data: `data(jobs, package = 'mediation')`
		...
		* job\_seek: mediator, ordinal: measures the level of job-search self-efficacy with values from 1 to 5.
		* econ\_hard: adjustment, ordinal: Level of economic hardship pre-treatment with values from 1 to 5.
...
* Summary - `summary(mediation_result)`![](./_resources/Causal_Inference.resources/Screenshot (846).png)
...
* Exact same brms syntax (except priors are specified) as in [Statistical Rethinking](Statistical Rethinking) >> Chapter 5 >> Counterfactual Plots
	* Example has a mediator DAG as well.
* `hypothesis` tests H0: a\*b == 0
	* pval < 0.05 says there is a mediation effect.
* [{sjstats}]{style='color: #990000'}![](./_resources/Causal_Inference.resources/Screenshot (854).png)

```
sjstats::mediation(med_result) %>% kable_df()
```

* mediator (b): the effect of "job\_seek" on "depress2"
* indirect (c-c'): ACME
* direct (c'): ADE
...
* Moderation can be tested by interacting variables of interest (moderator x IV) and plotting the simple slopes of the interaction, if present.
	* See [Regression, Interactions](Regression, Interactions) for simple slopes/effects analysis
...
* [Example]{.ribbon-highlight}: academic self-efficacy (moderator)(confidence in own’s ability to do well in school) moderates the relationship between task importance (independent variable (IV)) and the amount of test anxiety (outcome) a student feels (Nie, Lau, & Liau, 2011).
	* Students with high self-efficacy experience less anxiety on _important_ tests (task importance) than students with low self-efficacy while all students feel relatively low anxiety for less important tests.
...
* Plot the simple slopes (1 SD above and 1 SD below the mean) of the moderating effect![](./_resources/Causal_Inference.resources/unnamed-chunk-7-1.png)
	* For details on this plot and analysis, see [Regression, Interactions](Regression, Interactions) >> OLS >> numeric:numeric >> Calculate simple slopes for the IV at 3 representative values for the moderator variable
	* Interpretation
...
* **Misc**
	...
	* (_Total_ ) Causal Effect and _Direct_ Causal Effect![](./_resources/Causal_Inference.resources/Screenshot (707).png)
...
* **Testable Implications**
	...
	* DAGs that are consistent with the data associations (M & N are associated but the causal relationship isn't known)![](./_resources/Causal_Inference.resources/Screenshot (111).png)
		...
		* All three DAGs have no conditional independencies and therefore not testable implications
			* A set of DAGs, each with the same conditional independencies known as a _Markov Equivalence_
...
* **"Shutting the backdoor" to potential confounding paths**
	...
	* `Examples`
		* ![](./_resources/Causal_Inference.resources/Screenshot (134).png)
...
* ![](./_resources/Causal_Inference.resources/Screenshot (136).png)
...
* Evidently conditioning on A _and_ M is also a solution
...
* **Frontdoor Adjustment**
	...
	* In a causal chain with three nodes X→Z→Y, we can estimate the effect of X on Y _indirectly_ by combining two distinct quantities:
		1. The estimate of the effect of X on Z, P(Z|do(X))
		2. The estimate of the effect of Z on Y, P(Y|do(Z), X)
	* Assumptions
	...
	* [Example]{.ribbon-highlight} 1
		* Our goal is to estimate P(Y|do(X)). Unfortunately, this relationship between X and Y is confounded by the unobserved variable U, via this backdoor path: X←U→Y. Therefore, we cannot estimate the causal quantity of interest _directly._![](./_resources/Causal_Inference.resources/unnamed-chunk-2-1.png)
...
* Output![](./_resources/Causal_Inference.resources/Screenshot (203).png)
