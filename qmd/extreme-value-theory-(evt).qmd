# Extreme Value Theory {#sec-evt .unnumbered}

## Misc {#sec-evt-misc .unnumbered}

::: {.callout-tip collapse="true"}
## Packages

- [CRAN Task View](https://cran.r-project.org/web/views/ExtremeValue.html)
- [{]{style="color: #990000"}[CausalMixGPD](https://cran.r-project.org/web/packages/CausalMixGPD/index.html){style="color: #990000"}[}]{style="color: #990000"} - Bayesian Nonparametric Conditional Density Modeling in Causal Inference and Clustering with a Heavy-Tail Extension
  - Implements the semiparametric framework for Bayesian analysis of heavy-tailed outcomes by combining Dirichlet process mixture models for the body of the distribution with optional generalized Pareto tails
...
- [{]{style="color: #990000"}[GLmom](https://github.com/sygstat/GLmom){style="color: #990000"}[}]{style="color: #990000"} ([Papers](https://cran.r-project.org/web/packages/GLmom/index.html)) - Provides generalized L-moments estimation methods for the generalized extreme value ('GEV') distribution. (examples work with maximum value time series data)
...
- [{]{style="color: goldenrod"}[nsEVDx](https://github.com/Nischalcs50/nsEVDx){style="color: goldenrod"}[}]{style="color: goldenrod"} ([JOSS](https://joss.theoj.org/papers/10.21105/joss.11187)) - **Frequentist and Bayesian** inference of **non-stationary** EVD models, enabling researchers and practitioners to quantify changing risks associated with rare and extreme events.
  - Easy-to-use framework for fitting stationary and non-stationary Generalized Extreme Value (GEV) and Generalized Pareto (GPD) distributions.
...
- [**Survival Function**]{style="color: #009499"} - For a random variable $X$, the survival function (also called the complementary CDF or reliability function) is defined as:\

  $$S(x) = P(X \gt x) = 1 - F(x)$$

  where $F(x)$ is the Cumulative Distribution Function (CDF) which is $P(X\le x)$
  ...
  - [Example]{.ribbon-highlight}: Exponential Survival Function: $S_{\text{exp}}(x) = e^{-\lambda x}$

## Distribution Tail Classification {#sec-evt-distrtail .unnumbered}

![](./_resources/Extreme_Value_Theory_(EVT).resources/84f3854e3b965f3558decf7fb93a14d6.jpeg){.lightbox}

### Misc {#sec-evt-distrtail-misc .unnumbered}
...
### Heavy tails {#sec-evt-distrtail-heavy .unnumbered}

- Your random variable distribution is heavy tailed if:\

  $$\begin{align}
  &\lim_{x \rightarrow + \infty} \sup \frac{\bar F(x)}{\bar F_{\text{exp}}(x)} = \frac{\bar F(x)}{e^{-\lambda x}}, \;\; \forall \lambda \gt 0 \\
  &\text{where} \;\; \bar F_{\text{exp}} = P(X \ge x) = e^{-\lambda x}
  \end{align}$$

  - $\bar F_{\text{exp}}$ is the Exponential distribution **Survival Function**
...
### Long Tails {#sec-evt-distrtail-long .unnumbered}
...
- Your random variable distribution is long tailed if it follows the **Explosion Principle** which says, "If an extreme event manifests itself, then the probability of an even *more* extreme event approaches 1."\
  ...
  $$\lim_{x \rightarrow \infty} P(X \gt x + t|X\gt x) = 1, \;\; \forall t > 0$$

  - There is no time prediction on the next more extreme event, but extreme value theory + timeseries + conditions say extreme events tend to cluster
  - [Example]{.ribbon-highlight}: if you take a huge loss in your portfolio, it's a mistake to think that that value is an upper bound on losses or that the probability of an even larger loss is negligible
  - [This characteristic is not practical to determine from data]{.underline}

### Subexponential Tails {#sec-evt-distrtail-subexp .unnumbered}
...
- Your random variable distribution is subexponential tailed if it follows the **One-Shot** aka **Catastrophe Principle** aka \*\*Winner-Takes-All\*\*
  ...
  $$P(S_n \gt x) \approx P(M_n \gt x) \; \text{as} \; n \rightarrow \infty$$

  - $S_n$ is a partial sum of values of your random variable
  - $M_n$ is a partial maximum
  - $x$ is a large value
  - Says at some point the partial sum, $S_n$, will be dominated by one large value, $M_n$
  - [Example]{.ribbon-highlight}: If your portfolio follows this principle, then your total loss can be mostly attributed to one large loss
...
### Fat Tails {#sec-evt-distrtail-fat .unnumbered}
...
- Survival Function (in general)\
  ...
  - $L(x)$ is characterized as a slowly varying function that gets dominated by the decaying inverse power law element, $x^{-\alpha}$ , as $x$ goes to infinity
  - $\alpha$ is a shape parameter, aka **Tail Index** aka **Pareto Index**

- Distribution Examples

  - Pareto
    - Also see [Distributions \>\> Pareto](distributions.qmd#sec-distr-pareto){style="color: green"}

    - The Pareto has similar relationship with the Exponential distribution as Log-Normal does with Normal\

      $$\begin{align}
      &Y_{\text{exp}} = \log \left(\frac{X_{\text{pareto}}}{X_{m}}\right)\\
      &Y_{\text{pareto}} = x_m e^{Y_{\text{exp}}}
      \end{align}$$

      - $x_m$ is the (positive) minimum of the randomly distributed pareto variable, $X$, that has index $\alpha$
...
### Light Tails {#sec-evt-distrtail-light .unnumbered}
...
## Tail Classification Plots {#sec-evt-classplots .unnumbered}

### Misc {#sec-evt-classplots-misc .unnumbered}
...
- Ask these questions
  1.  Does the subject matter you're modeling lead you to expect a certain type of tail?
      - [Example]{.ribbon-highlight}: Does the explosion principle hold or not?
  2.  Is there an upper bound to your data (theoretical or actual)?
      - [Example]{.ribbon-highlight}: Is the upper bound due to the quality of the data
...
### Q-Q {#sec-evt-classplots-qq .unnumbered}

- Plot exponential quartiles on the y-axis and ordered data on the x-axis
  - See [EDA, General \>\> Continuous Variables](eda-general.qmd#sec-eda-gen-cont){style="color: green"} \>\> Q-Q plot for code
...
### Zipf {#sec-evt-classplots-zipf .unnumbered}

![](./_resources/Extreme_Value_Theory_(EVT).resources/zipf-plot-distr.png){.lightbox width="582"}
...
- [Example]{.ribbon-highlight}:\
  ![](./_resources/Extreme_Value_Theory_(EVT).resources/zipf-plot-real.png){.lightbox width="582"}
...
- [Example]{.ribbon-highlight}: Log-Normal\
  ![](./_resources/Extreme_Value_Theory_(EVT).resources/zipf-plot-lognorm.png){.lightbox width="582"}
...
- [Example]{.ribbon-highlight}: Aggregate and Compare\
  ![](./_resources/Extreme_Value_Theory_(EVT).resources/zipf-plot-aggr-real.png){.lightbox width="582"}
  ...
  - Examples of aggregation methods (halves the sample size)
    1.  Order data from largest to smallest; add $a_1 + a_n, a_2 + a_{n-1}, \ldots$; plot alongside original data (green)
    2.  Order data from largest to smallest; add $a_1 + a_2, a_3 + a_4, \ldots$; plot alongside original data (blue)

### Mean Excess (ME) {#sec-evt-classplots-me .unnumbered}

![](./_resources/Extreme_Value_Theory_(EVT).resources/meplot-distr.png){.lightbox width="582"}

- Calculating the Empirical Mean Excess variable:
  1.  Order the data
  2.  Calculate mean (1)
  3.  Remove the 1 data point
  4.  Calculate mean (2)
  5.  Remove data points 1 and 2
  6.  Calculate mean (3),
  7.  Continue until you run out of data (I assume)
  8.  Then plot the means
...
- [Example:]{.ribbon-highlight}\
  ![](./_resources/Extreme_Value_Theory_(EVT).resources/meplot-real.png){.lightbox width="682"}
...
### Maximum to Sum (MS) {#sec-evt-classplots-ms .unnumbered}

![](./_resources/Extreme_Value_Theory_(EVT).resources/msplot-distr.png){.lightbox width="582"}
...
- Maximum to Sum Ratio
  ...
  &R_n(p) = \frac{M_n(p)}{S_n(p)}; \;\; n\ge1,\; p\gt 0
  \end{align}$$
...
- Procedure
  ...
  1.  Choose a $p$ that you want to check
  2.  For each $n$, calculate the sum, maximum, and ratio
  3.  Plot where the y-axis is the ratio, and the x-axis is the $n$ value
...
- [Example]{.ribbon-highlight}: Real Estate Losses\
  ![](./_resources/Extreme_Value_Theory_(EVT).resources/msplot-credit1.png){.lightbox width="582"}
...
- [Example]{.ribbon-highlight}: Operational Losses\
  ![](./_resources/Extreme_Value_Theory_(EVT).resources/msplot-oper.png){.lightbox width="582"}

  - $p = 1$ is iffy, the rest don't exist
  - Interpretation: $\alpha$ might be less than 1

### Concentration Profile {#sec-evt-classplots-cp .unnumbered}

![](./_resources/Extreme_Value_Theory_(EVT).resources/concent-prof-distr.png){.lightbox width="582"}
