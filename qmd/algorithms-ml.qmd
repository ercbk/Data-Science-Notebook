...
## Misc {#sec-alg-ml-misc .unnumbered}

- Packages
  - [{]{style="color: #990000"}[nmfkc](https://ksatohds.github.io/nmfkc/){style="color: #990000"}[}]{style="color: #990000"} - Extends Non-negative Matrix Factorization (NMF) by incorporating covariates using kernel methods.
    - The same multiplicative update engine, with the same column-normalization trick on $X$ and the same tri-factorization structure is repurposed across mulitple domains by simply re-defining what the matrices mean
    - Supports advanced features like rank selection via cross-validation, time-series modeling (NMF-VAR), supervised classification (NMF-LAB), structural equation modeling with equilibrium interpretation (NMF-SEM), and mixed-effects modeling with random effects (NMF-RE)
    - Issue: Non-negativity constrait on coefficients ([Claude discussion](https://claude.ai/share/9a406804-dcb9-48b0-b73e-aee938d3f624))
      - Okay for classification with kernel method (but slower) and features aren't interpretable
      - Okay for forecasting with a non-negative outcome variable
      - Don't use for mixed effects modeling. A significant negative effect will be represented as 0 and non-significant by this model.
        - There is a way around this but it's pretty hacky.
...
- Misc
  ...
  - Resources
    ...
    - [Interactively exploring high-dimensional data and models in R, Ch.14](https://dicook.github.io/mulgar_book/14-lda.html)
