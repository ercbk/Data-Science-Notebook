    ...
    -   The algorithm proceeds in two phases:\
        ...
        1.  **Forward pass** — Greedily adds pairs of reflected hinge functions (and their products) to the model, choosing the predictor and knot location that most reduces the residual sum of squares at each step. The model is deliberately over-fit.
            ...
            -   The forward pass stops when any of the following conditions is met:
                ...
                -   Adding a term changes R² by less than `0.001` (controlled by [thresh]{.arg-text})
                -   R² reaches `0.999` or higher
                -   GRSq drops below `−10` (a pathologically poor penalized fit)
                -   No new term can increase R² (numerical accuracy limit)

        2.  **Backward pruning (GCV)** — More precisely called the *pruning* pass; `"backward"` stepwise deletion is the default but other methods are available via [pmethod]{.arg-text} (e.g., `"cv"` for cross-validation-based selection).
...
-   [Hyperparameters]{.underline}
    ...
    1.  [nk]{.arg-text}: Maximum number of terms in the forward pass (default: `min(200, max(20, 2 * ncol(x))) + 1`)
        -   if the termination condition printed by [{earth}]{style="color: #990000"} reads `Reached maximum number of terms`, consider increasing [nk]{.arg-text} — the forward pass may have stopped too early.
    ...
    2.  [degree]{.arg-text}: Maximum degree of interaction among hinge functions (default: `1`);
        -   [degree = 2]{.arg-text} is a common choice for allowing pairwise interactions
    3.  [thresh]{.arg-text}: Forward-pass stopping threshold; a term is added only if it reduces R² by more than this fraction (default: `0.001`)
        -   Set to `0` to disable most stopping conditions except [nk]{.arg-text} and numerical limits.
    4.  [nprune]{.arg-text} — *Maximum* number of terms (including the intercept) retained after pruning
    5.  [penalty]{.arg-text} — GCV penalty per knot (default: `2` for [degree = 1]{.arg-text}, `3` for [degree \> 1]{.arg-text}); higher values encourage sparser models
        -   The forward pass also stops if GRSq drops below `−10`; set [penalty = -1]{.arg-text} to disable this GRSq floor
        -   Default is equivalent to selection by AIC
    6.  [pmethod]{.arg-text} — Pruning method (default: `"backward"`); `"cv"` selects the number of terms by cross-validation rather than GCV; requires [nfold]{.arg-text} to be set
...
-   [Variable Importance]{.underline}
    ...
    -   `earth::evimp()` reports importance based on two criteria:
        -   nsubsets: The number of model subsets (during backward pruning) in which a predictor appears; this is the criterion used by `summary.earth()` to rank predictors; higher = more consistently retained
        -   gcv: The net reduction in GCV attributable to the predictor across all subsets it appears in; scaled so the largest value = 100. Can be negative if adding the variable worsened the GCV-penalized fit more often than it helped
        -   rss: Same as gcv but using RSS instead; generally more stable than gcv
    -   A `>` printed next to a gcv or rss value means the criterion *increased* (worsened) when that variable was added, i.e., the gcv/rss ranking disagrees with the nsubsets ranking
    -   Predictors listed as `-unused` appeared in the forward pass but were dropped during pruning; pass `trim = FALSE` to `evimp()` to show them
    -   Issues
...
-   [Uncertainty Quantification]{.underline}
    ...
    -   Getting CIs/PIs
        ...
        -   Confidence intervals: The uncertainty in the mean predicted response due to model variance alone; use `interval = "cint"`
        -   Prediction intervals: The uncertainty for a single future observation; adds irreducible error on top of model variance, so they are always wider than confidence intervals; use `interval = "pint"`

    -   Model options

        -   `"lm"`: Linear regression on absolute residuals vs. fitted values
            -   Assumes standard deviation increases linearly with the response; good default choice
        -   `"rlm"`: Robust linear regression; produces narrower intervals by down-weighting outlying residuals, but can underestimate error variance when errors are mostly Gaussian
        -   `"gam"`: Allows non-linear variance trends; more flexible but more prone to overfitting on the residuals
        -   `"earth"`: Uses a MARS model for the residual model; no IRLS iteration
        -   `"power"`: Estimates a power-of-the-mean model: $\sigma = \text{intercept} + \text{coef} \cdot \hat{y}^{\,\text{exponent}}$
        ...
        -   `"const"`: Assumes homoscedastic errors; no IRLS needed
        -   **General rule**: prefer non-`x.` methods (e.g., `"lm"`) which model variance as a function of the fitted response $\hat{y}$. Use `x.` variants (e.g., `"x.lm"`) only when the response is non-monotonic and variance has a many-to-one (multimapped) relationship with $\hat{y}$ that cannot be captured by modeling variance as a function of the fitted values alone

    -   Cross-validation is required to estimate model variance for prediction intervals; specify [nfold]{.arg-text} and [ncross]{.arg-text} (recommended: `ncross` ≥ 30) when using [varmod.method]{.arg-text}
    ...
    -   Checking the variance model
        ...
        -   The `summary()` of a variance model reports `iter.rsq` — the R² of the weighted residual model. Low values are normal and expected due to noisy residuals; this does not mean the variance model is useless
        -   The `response values in prediction interval` table in `summary()` is a sanity check of interval coverage on training data; it is highly recommended to also check this on held-out data via `summary(earth.mod, newdata = ...)`
        -   A `<` symbol next to a coverage percentage in the summary table (e.g., `88<` for the 90% interval) flags undercoverage and hints at possible overfitting in the variance model
    ...
    -   [Example]{.ribbon-highlight}
        ...
        -   The `stddev of predictions` table in `summary()` gives the estimated standard deviation formula (e.g., `1.562 + 0.262 * O3`); the `iter.stderr%` column shows coefficient uncertainty as a percentage of the coefficient — formal t-tests are not justified here due to the iterative estimation
...
-   [Example]{.ribbon-highlight}: Predicting fuel efficiency (`mpg`) from car attributes using the `mtcars` dataset
    ...
    -   The pruned model retained just 3 of 12 candidate terms, using only 2 of 10 predictors — disp (displacement) and carb (carburetors) — well below the nprune = 10 cap, suggesting the signal in mtcars is concentrated in very few variables.
    -   The knot at `disp = 145` divides the displacement effect into two regimes:
        -   Below the knot (`h(145-disp)`): As displacement decreases below 145, `mpg` increases by about 0.157 per unit — smaller engines get meaningfully better mileage in this range.

        -   Above the knot (`h(disp-145) * carb`): An interaction term kicks in where higher displacement *combined* with more carburetors is associated with lower `mpg` (−0.006 per unit of the product). The penalty is modest per unit but compounds across both dimensions.
    -   GRSq is
        -   Formula: (1-gcv) / gcv.null
    ...
    -   [nsubsets]{.var-text}: Number of pruning-stage model subsets in which the variable appeared — higher indicates the predictor was consistently retained
    -   [gcv]{.var-text} / [rss]{.var-text}: Percentage of total GCV/RSS reduction attributable to each variable (scaled so the top variable = 100)
