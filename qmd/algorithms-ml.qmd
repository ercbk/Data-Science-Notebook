...
## Support Vector Machines  {#sec-alg-ml-svm}

-   Misc
    -   packages: {e1071}, {kernlab}, {LiblineaR}, [{{sklearn}}]{style="color: goldenrod"}
    -   Also see [Model Building, tidymodels](Model%20Building,%20tidymodels) \>\> Model Specification \>\> Support Vector Machines
...
![](./_resources/Algorithms,_ML.resources/unknown_filename.59.png)
...
-   Regression![](./_resources/Algorithms,_ML.resources/image.png)
    -   *Stochastic* Gradient Descent is used in order minimize MAE loss
        -   Also see
            -   [Model building, sklearn](Model%20building,%20sklearn) \>\> Algorithms \>\> Stochaistic Gradient Descent (SGD)
            -   [Loss Functions](Loss%20Functions) \>\> Misc \>\> Mean Absolute Error (MAE)
    -   Epsilon Insensitive Loss - the idea is to use an "insensitive tube" where errors less than epsilon are ignored. For errors \> epsilon, the function is linear.
        ...
        -   See [Loss Functions](Loss%20Functions) \>\> Huber loss for something similar
...
-   Decision Trees
    ...
    -   *Classification*
        ...
        -   Calculate *Shannon Entropy* of dependent variable (Y): ![](./_resources/Algorithms,_ML.resources/unknown_filename.11.png) where P(Y) is the marginal probability

            -   For a binary variable, this would be ![](./_resources/Algorithms,_ML.resources/unknown_filename.12.png)

            -   In general the Shannon Entropy equation is ![](./_resources/Algorithms,_ML.resources/unknown_filename.10.png) where p is a probability and c is the number of classes for the variable and S = subset of data or the node.
        ...
        -   Calculate the entropy of the target, Y, with respect to each independent variable, x. For variable,![](./_resources/Algorithms,_ML.resources/unknown_filename.13.png) , with number of classes, c : ![](./_resources/Algorithms,_ML.resources/unknown_filename.18.png)
            ...
            -   I do NOT like the way the equation is written above. In videos, this type of entropy isn't given a name, but I think it matches conditional entropy in its description and calculation.
                ...
                -   *Conditional Entropy (for a particular x),*![](./_resources/Algorithms,_ML.resources/unknown_filename.15.png)
                    -   This definition uses ![](./_resources/Algorithms,_ML.resources/unknown_filename.19.png) where H is used to as the symbol for entropy.

            -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.14.png) is the marginal probability for that class of that variable, i.e. ratio of instances of that class in the entire dataset.

            -   example: ![](./_resources/Algorithms,_ML.resources/unknown_filename.16.png)
        ...
        -   
            ...
            Calculate information gain for variable ![](./_resources/Algorithms,_ML.resources/unknown_filename.13.png)

            :   ![](./_resources/Algorithms,_ML.resources/unknown_filename.17.png)
...
-   Random Forest![](./_resources/Algorithms,_ML.resources/1-1NOc1NgQOeXaJ48AMh-Djg.png)
    ...
    -   Several *independent* decision trees are fitted. Each tree just gets a part of the variable and then splits the outcome space according to the features in **X** When we ''drop down'' a new point **x**, it will end up in a leaf for each tree. A leaf is a set with observations *i* and taking the average over all yi in that leaf gives the prediction for one tree. These predictions are then averaged to give the final result. Thus, for a given **x** if you want to predict the conditional mean of Y given that **x**, you: "Drop down" the **x** each tree (this is indicated in red in the above figure). Since the splitting rules were made on **X**, your new point **x** will safely land somewhere in a leaf node.
        ...
        Missing Predictor Data See StatQuest: Random Forests Part 2: Missing data and clustering [video](https://www.youtube.com/watch?v=sQ870aTKqiM&list=PLblh5JKOoLUICTaGLRoHQDuF_7q2GfuJF&index=55) for more details
...
-   Isolation Forests
    ...
    -   Anomaly scores range from 0 to 1. Observations with a shorter average path length will have a larger score.
        -   Anomaly score, ![](./_resources/Algorithms,_ML.resources/unknown_filename.44.png)
        ...
        -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.42.png)
            -   where H(i) is the Harmonic number, ![](./_resources/Algorithms,_ML.resources/unknown_filename.43.png)
        -   Guidelines
...
-   Distributional Trees/Forests![](./_resources/Algorithms,_ML.resources/distforest-motivation.png)
    ...
    -   [**{partykit}**]{style="color: #990000"}
        ...
        -   Tree procedure
            ...
            2.  Take the derivative of the log-likelihood function. Plug in the value(s) of the MLE parameter(s) and a yi value to get a "score" for every value of the response variable. Repeat for each distributional parameter. (The score should fluctuate around zero.)
                -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.36.png) and ![](./_resources/Algorithms,_ML.resources/unknown_filename.1.png) where k is the number of distributional parameters and n is the number of training observations

                -   Gaussian example with ![](./_resources/Algorithms,_ML.resources/unknown_filename.34.png) :

                    1.  Solve for the MLE of the mean and calculate the value: ![](./_resources/Algorithms,_ML.resources/unknown_filename.3.png)

                    2.  Solve for the MLE of the variance and calculate the value: ![](./_resources/Algorithms,_ML.resources/unknown_filename.4.png)

                    3.  We're calculating a score for each value of the outcome variable so we can remove the summation symbol from the derivative of the log-likelihood function w.r.t. the mean. This leaves us with the mean score function: ![](./_resources/Algorithms,_ML.resources/unknown_filename.5.png)

                        -   

                    4.  Same thing but with the derivative of the log-likelihood function w.r.t. the variance:   ![](./_resources/Algorithms,_ML.resources/unknown_filename.35.png)
            3.  Null Hypothesis test each predictor variable vs the parameter score matrix where H0 = independence --- Two methods: CTree and MOB
                1.  CTree is permutation test based
                    1.  Each test statistic vector, T,  for 1,...,l predictors and n observations is calculated by: ![](./_resources/Algorithms,_ML.resources/unknown_filename.2.png) where ![](./_resources/Algorithms,_ML.resources/unknown_filename.37.png) is the 1xk row of the score matrix and v is a transformation function that depends on whether the predictor variable, Z,  is a numeric or character type.
                    ...
                    2.  T is standardized by maximum or quadratic method.
                        ...
                        -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.45.png)
                    ...
                    3.  Find the p-value associated with each predictor's c statistic
                        -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.49.png)
                            -   This says the p-value, P, is the probability, ![](./_resources/Algorithms,_ML.resources/unknown_filename.47.png) , (associated with the null hypothesis for this particular variable) that the standardized T stat is as or more extreme than the group of standardized t stats of the permuted scores.

                            -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.46.png) is the symmetric group of permutations and weights. Weights being either 0 or 1 depending on whether the observation is present in that node's data subset.
                2.  MOB stands for model based method
                    ...
                    -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.50.png)
                        -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.51.png) is a minimum amount of scores that you choose, then ![](./_resources/Algorithms,_ML.resources/unknown_filename.52.png) .
                            -   No guidelines for ![](./_resources/Algorithms,_ML.resources/unknown_filename.51.png) . In addition to choosing a minimum, the paper does mention also trimming node data points at each end by 10%.

                            -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.57.png) is called "its variance function."
                        -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.53.png)
                            -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.56.png)  means floor of nt, which means round down to the integer.

                            -   The subscript, ![](./_resources/Algorithms,_ML.resources/unknown_filename.55.png) , says the scores are ordered from highest to lowest (aka anti-rank) according to the predictor variable, Zj, values

                            -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.54.png)
                    ...
                    -   The distribution from which the p-value is calculated has something to do with a Bessel process and stuff converging to a Brownian Bridge, so I decided to shut it down here.
                        -   see [https://eeecon.uibk.ac.at/\~zeileis/papers/Zeileis+Hothorn+Hornik-2008.pdf](https://eeecon.uibk.ac.at/~zeileis/papers/Zeileis%2BHothorn%2BHornik-2008.pdf) for details
        ...
        -   Forest procedure
            ...
            -   For predictions:
                ...
                4.  Sum all tree weights, wit , for that training observation and divide by the number of trees to get the forest-weight for that training observation.
                    -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.41.png)
                    ...
                    -   In the paper, this process is described by a more compact notation:
                        -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.58.png)
                ...
                5.  The paper describes predictions being calculated by a weighted MLE as it did for a single tree, but for forests it didn't explicitly give an "in practice" description of process.
                    -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.png)
    ...
    -   [**{drf}**]{style="color: #990000"}![](./_resources/Algorithms,_ML.resources/1-A8OQlRYAyAQ0d6gmSAIEJA.png)
...
-   Gradient boosted machines (GBM) aka Gradient tree boosting
    1.  Choose a differentiable loss function, ρ, such as ![](./_resources/Algorithms,_ML.resources/unknown_filename.6.png) . In gradient boosting, 1/n is exchanged for 1/2, to make it differentiable. The mean of the loss function, SSE in this case, calculated over all observations for a model is called the "empirical risk" which is what boosting is trying to minimize.

    2.  Calculate the negative gradient, aka first derivative. For regression trees, this turns out to be ![](./_resources/Algorithms,_ML.resources/unknown_filename.7.png) which is just the residuals. Pg 360 of The Elements of Statistical Learning has other loss functions and their negative gradients. Classification uses a multinomial deviance loss function.

    3.  Initialize using the optimal constant model, which is just a single terminal node tree. Think this means the initial predictions are just mean of the target, ![](./_resources/Algorithms,_ML.resources/unknown_filename.8.png)

    4.  Calculate the negative gradient vector (residuals), r,  by plugging in the predicted values.

    5.  Fit a regression tree with the residuals, r, as the target variable. The mean of the residuals for that region (terminal node) is the *prediction*, ![](./_resources/Algorithms,_ML.resources/unknown_filename.20.png) .

    6.  Add the predicted residuals vector to the initial predictions vector, ![](./_resources/Algorithms,_ML.resources/unknown_filename.9.png) to get the next set of predictions to feed into the negative gradient equation.
...
-   XGBoost
    ...
    -   Regularization: It penalizes more complex models through both LASSO (L1) and Ridge (L2) regularization to prevent overfitting.
        -   Regularization function![](./_resources/Algorithms,_ML.resources/ql_25f93bc332e446028404fe9ff18e9293_l3.png)
...
-   Component-wise Boosting (From mboost_PKG tutorial docs)
    ...
    -   The goal is to minimize empirical risk, ![](./_resources/Algorithms,_ML.resources/unknown_filename.21.png) where ![](./_resources/Algorithms,_ML.resources/unknown_filename.22.png) is the loss function and ![](./_resources/Algorithms,_ML.resources/unknown_filename.23.png) the predictor function
    ...
    -   Steps
        1.  Compute the negative gradient of the loss function which is the negative first derivative with respect to the predictor function, ![](./_resources/Algorithms,_ML.resources/unknown_filename.24.png) .

        2.  ![](./_resources/Algorithms,_ML.resources/unknown_filename.25.png) can be thought of as the vector of predictions at the mth iteration of the algorithm. So to begin, we create an initial vector, ![](./_resources/Algorithms,_ML.resources/unknown_filename.26.png) with "offset values."

            -   For glmboost, the offset is the mean of the outcome variable. I'd guess it's probably the same for gamboost.

        3.  Compute the negative gradient vector: ![](./_resources/Algorithms,_ML.resources/unknown_filename.28.png)

            -   For the first iteration, m = 1 and ![](./_resources/Algorithms,_ML.resources/unknown_filename.27.png)
        ...
        5.  For each baselearner, calculate the residual sum of squares,![](./_resources/Algorithms,_ML.resources/unknown_filename.29.png) from it's predictions.
        ...
        6.  Whichever baselearner has the smallest RSS, scale it's predictions with a learning rate factor and combine them with the previous iteration's predictions
            ...
            -   ![](./_resources/Algorithms,_ML.resources/unknown_filename.30.png) where ![](./_resources/Algorithms,_ML.resources/unknown_filename.31.png) is the *learning rate*.
    ...
    -   Each baselearner's contribution to the final prediction vector is![](./_resources/Algorithms,_ML.resources/unknown_filename.32.png) over all iterations where that baselearner was selected
        -   I think this is the value of the ![](./_resources/Algorithms,_ML.resources/unknown_filename.33.png) on the y-axis of the partial dependence plots (pdp).
