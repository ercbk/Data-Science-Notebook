...
- Use `stringi::stri_escape_unicode` to check for hidden unicode characters
  ...
  - [Example]{.ribbon-highlight}
    ...
    stringi::stri_escape_unicode("eval_at home")
    #> [1] "eval_at\\u00a0home"
    stringi::stri_escape_unicode("eval_at home")
    #> [1] "eval_at home"
    ```
...
- Read .csv from a zipped file
  ...
  - Sometimes `curl` will fail with this error. To bypass the SSL issue, use `curl -k <url>`
...
- Check if any column has above a certain percentage of NAs

  - Names of Columns

    ``` r
    # matrix
    na_cols <- colnames(mat)[colMeans(is.na(mat)) > 0.20]

    # df
    names(df)[colMeans(is.na(df)) > 0.20]
    ```

    - Matrices: All columns must be the same type (numeric, character, etc.)
  ...
  - Removal of Columns
    ...
    ``` r
    # matrix
    mat_clean <- mat[, !colnames(mat) %in% na_cols]
    mat_clean <- mat[, colMeans(is.na(mat)) <= 0.20]

    # df
    df_clean <- df[, !names(df) %in% na_cols]
    df_clean <- df[, colMeans(is.na(df)) <= 0.20]

    df_clean <- df %>%
      janitor::remove_empty(which = "cols", prop = 0.20)
...
- Fuzzy Join
  ...
  - Packages
    ...
    - [{]{style="color: #990000"}[fozziejoin](https://cran.r-project.org/web/packages/fozziejoin/index.html){style="color: #990000"}[}]{style="color: #990000"} - A modern, performance-oriented (Rust-based) alternative to [{fuzzyjoin}]{style="color: #990000"} (NOT a drop-in replacement but should have the same functionality)
    - [{]{style="color: #990000"}[fuzzyjoin](https://cran.r-project.org/web/packages/fuzzyjoin/index.html){style="color: #990000"}[}]{style="color: #990000"} - Join tables together based not on whether columns match exactly, but whether they are similar by some comparison. Implementations include string distance and regular expression matching

  - [Example]{.ribbon-highlight}: [{fuzzyjoin}]{style="color: #990000"}
...
- Remove NULL or empty elements from a list
  ...
  ``` r
  ls_clean <- 
        ls_dirty[!sapply(ls_dirty, \(x) is.null(x) || length(x) == 0)]

  ls_clean <- Filter(Negate(is.null), ls_dirty)

  ls_clean <- purrr::discard(x, ~ is.null(.x) || length(.x) == 0)
  ```
...
## Time Series {#sec-r-snippets-ts .unnumbered}

### Misc {#sec-r-snippets-ts-misc .unnumbered}

- Resources
  - [Base R date-time manipulation with POSIXlt](https://rud.is/base-r-datetime-webr/) (hrbrmstr)
    - Arithmetic, clamping, IANA zones, flexible handling of dates/time stamps
...
### Operations {#sec-r-snippets-ts-ops .unnumbered}
...
- Create a sequence of dates
  ...
  - Base
    ...
    # Create 12 monthly dates starting from Jan 2023
    dates <- seq.Date(
      as.Date("2023-01-01"), 
      by = "month", 
      length.out = 12
    ...
    days_of_month <- c(15, 20, 10, 25, 5)

    dates <- as.Date(paste0(
      format(start_date + months(0:4), "%Y-%m"),
      "-", 
      days_of_month
    ))
    ```
  ...
  - Lubridate
    ...
    # Or with months() function
    dates <- ymd("2023-01-01") %m+% months(0:11)

    dates_last <- ceiling_date(
      seq(ymd("2023-01-01"), 
          ymd("2023-12-01"), 
          by = "month"),
      "month") - days(1)
...
- Create a sequence of datetimes
  ...
  - Base
    ...
    ``` r
    # Five hourly datetimes
    seq(
      as.POSIXct("2023-01-15 09:30:00"), 
      by = "hour", 
      length.out = 5
    )
    #> [1] "2023-01-15 09:30:00 EST" "2023-01-15 10:30:00 EST" "2023-01-15 11:30:00 EST" "2023-01-15 12:30:00 EST"
    #> [5] "2023-01-15 13:30:00 EST"

    # specified intervals
    seq(
      as.POSIXct("2023-01-15 12:00:00"),
      by = "30 min",  # or "7 days", "2 weeks", etc.
      length.out = 5
    )
    #> [1] "2023-01-15 12:00:00 EST" "2023-01-15 12:30:00 EST" "2023-01-15 13:00:00 EST" "2023-01-15 13:30:00 EST"
    #. [5] "2023-01-15 14:00:00 EST"
  ...
  - Lubridate
    ...
    ``` r
    # components
    ymd_hms("2023-01-15 09:30:00") + minutes(0:4)
    #> [1] "2023-01-15 09:30:00 UTC" "2023-01-15 09:31:00 UTC" "2023-01-15 09:32:00 UTC" "2023-01-15 09:33:00 UTC"
    #> [5] "2023-01-15 09:34:00 UTC"

    ymd_hms("2023-01-15 09:30:00") + hours(0:4) + minutes(0:4)
    #> [1] "2023-01-15 09:30:00 UTC" "2023-01-15 10:31:00 UTC" "2023-01-15 11:32:00 UTC" "2023-01-15 12:33:00 UTC"
    #> [5] "2023-01-15 13:34:00 UTC"

    # more components
    dates <- ymd(c("2023-01-15", "2023-02-20", "2023-03-10", "2023-04-25", "2023-05-05"))
    times <- c("09:30:00", "14:15:00", "18:45:00", "08:00:00", "22:30:00")
...
- Converting between Date format and Numeric format
  ...
  ``` r
  dates <- as.Date(c("1970-01-01", "2024-01-01", "1969-12-31"))
  dates
  #> [1] "1970-01-01" "2024-01-01" "1969-12-31"
...
- Converting between Datetime format and Numeric format

  ``` r
  datetime <- as.POSIXct("2026-02-19 10:35:00", tz = "UTC")
