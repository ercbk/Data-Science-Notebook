...
- Misc
  ...
  - Notes from About predictions in spatial autoregressive models: optimal and almost optimal strategies, Goulard et all 2017 (See R \>\> Documents \>\> Geospatial)
    - Breaks down TS, TC, BP, BPN, BPW and compares out-of-sample performance
  - **TL;DR** Goulard et all 2017
    - Sparse $W$ Matrix:
      - BPN is best in performance but maybe a little more computationally expensive than TC and TS
      - TC is the only method that performs notably worse than the others. The rest are very close to each other with the rank being BP, BPN, BPW, and TS.
    - Dense $W$ matrix:
      - All methods have almost identical performance
  - Check sparsity: `mean(card(nb_list))`
  ...
  - "BP" uses a weighted sum of all in-sample residuals for its correction factor. The weights are included in the form of a precision matrix (function of $W$) and reflects how strongly each in-sample unit is spatially connected to the out-of-sample unit being predicted.\
  ...
  - "BPN" does the same as "BP" but drops in-sample non-neighbors to the out-of-sample unit since their weights are very close to zero. Near identical performance when $W$ is sparse, but also still is close to "BP" even when $W$ is dense.
  - "BPW" has a more complex calculation of residual factor and the residual vector used is calculated as a difference of weighted averages (for a row-normalized $W$).
...
+-----------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["TC"]{.arg-text}; also requires [listw]{.arg-text} | Trend-corrected: the conditional mean, $(I-\hat\rho W)^{-1}X\hat\beta$ under the fitted model. Uses only covariates and the spatial weights matrix, never the observed response. No optimality property. | fit, trend         | SLM, SDM, SAC, GNM (caution\*) |
+-----------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["BP"]{.arg-text}; also requires [listw]{.arg-text} | Starts from TC, adds a correction that's meant to adjust the prediction based on the observed $y$ values of the *in-sample neighbors* of the out-of-sample units                                         | fit, trend         | SLM, SDM, SAC, GNM (caution\*) |
+-----------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
...
|                                                                           |                                                                                                                                                                                                               |               |                                |
+===========================================================================+===============================================================================================================================================================================================================+===============+================================+
| "TS"                                                                      | See In-sample and Forecast                                                                                                                                                                                    |               | SEM, SDEM, SLM, SDM            |
+---------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------+--------------------------------+
...
+---------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------+--------------------------------+
| "TC"                                                                      | See In-sample and Forecast                                                                                                                                                                                    |               | SLM, SDM, SAC, GNM             |
+---------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------+--------------------------------+
...
+---------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------+--------------------------------+
| ["BP"]{.arg-text} or ["BP1"]{.arg-text}; also requires [listw]{.arg-text} | Leave-one-out version of ["BP"]{.arg-text}                                                                                                                                                                    | Out-of-sample | SLM, SDM, SAC, GNM (caution\*) |
+---------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------+--------------------------------+
| ["BPW"]{.arg-text}\                                                       | Same as "BP" except it uses a weighted ($W$) average of neighbor y values for the correction. "BPW1" is the leave-one-out version.                                                                            | Out-of-sample | SLM, SDM, SAC, GNM (caution\*) |
| ["BPW1]{.arg-text} ["]{.arg-text}                                         |                                                                                                                                                                                                               |               |                                |
+---------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------+--------------------------------+
| ["BPN"]{.arg-text}\                                                       | Same as "BP" but drops non-neighbors. For a sparse W matrix, this means "BPN" has near identical performance as "BP" but is more computationally efficient. ["BPN1"]{.arg-text} is the leave-one-out version. | Out-of-sample | SLM, SDM, SAC, GNM (caution\*) |
| ["BPN1"]{.arg-text}                                                       |                                                                                                                                                                                                               |               |                                |
+---------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------+--------------------------------+
