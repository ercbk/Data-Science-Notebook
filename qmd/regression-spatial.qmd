...
-   Workflow (LeSage (2014), [What Regional Scientists Need to Know about Spacial Econometrics](https://rrs.scholasticahq.com/article/8081-what-regional-scientists-need-to-know-about-spatial-econometrics))
    1.  Does your DGP indicate a local or global model (i.e. SDM or SDEM)?
        ...
        -   LeSage doesn't like the SAC model (left column). See [Models](regression-spatial.qmd#sec-reg-spat-econ-mod){style="color: green"} \>\> SAC for details
...
-   **Combined Spatial Autocorrelation (SAC)**
    ...
    -   Issue: Using the same W matrix for lag and error terms can lead to identification problems.
        -   Probably best to avoid these models.
        -   HOWEVER, sometimes people mention a $\text{SARMA}(p,q)$, allowing for $p$ different $\rho_{\text{lag}} Wy$ W matrices, and $q$ different $\rho_{\text{err}} Wu$ terms likewise (normally W’s of different orders)
...
-   [Spatial Hausman Test]{.underline}
    ...
    -   Rejecting the null (p-value \< 0.05) indicates neither OLS nor SEM is appropriate and there is a spatial process not sufficiently captured by the SEM model which only spatially models the residuals. In other words, try some other spatial regression models.
        -   The literature I've seen hasn't mentioned using it with SDEM model, but Bivand used this test with a SDEM model. So, I assume it's okay.
...
-   SEM, SDEM
    ...
    -   Uses the [boston_489]{.var-text} dataset which has no aggregation or unioned geometries — only has the censored values (codes as [NA]{.arg-text}) removed from [median]{.var-text} (response) (and [CHAS]{.var-text} coerced to a factor)
        -   Therefore, this model is completely at the census tract level and not the higher ([NOX_ID]{.var-text}) level.
    -   The spatial weights list, [lw_q_489]{.var-text}, is calculated directly from [boston_489]{.var-text}
    -   Both Hausman test results are significant which suggests neither is appropriate, and that there is a spatial process not significantly being captured by these models.
...
-   Aggregated Data Set
    ...
    -   The null is no longer rejected for both models which means that these models are adequately capturing the spatial process.
