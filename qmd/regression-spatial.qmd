...
- Packages
  ...
  - [{]{style="color: #990000"}[spboost](https://cran.r-project.org/web/packages/spboost/){style="color: #990000"}[}]{style="color: #990000"} - Gradient **Boosting** for Nonlinear Spatial Autoregressive Models
    - **Flexible nonlinear** extension of spatial autoregressive (SAR), spatial error (SEM), and spatial autoregressive with autoregressive disturbances (SARAR) models with multiple regression engines
    - Available Engines: generalized additive models ('mgcv'), gradient boosting ('mboost'), multivariate adaptive regression splines ('earth'), and 'xgboost'
...
- Notes from
  ...
  - Spatial Data Science, [Ch.17](https://r-spatial.org/book/17-Econometrics.html)
...
- [[{spatialreg}]{.underline}]{style="color: #990000"} [Model Functions]{.underline}\
  ...
  - [method]{.arg-text} ([Docs](https://r-spatial.github.io/spatialreg/reference/do_ldet.html)): The method for calculating the log determinant term in the log likelihood function, and defaults to ["eigen"]{.arg-text}, which is suitable for moderately sized datasets
    - ["eigen"]{.arg-text} (default): The Jacobian is computed as the product of $1 - \rho \lambda$ where lambda is the eigenvalue using `eigenw`
      - Computes asymptotic standard error of $\rho$
    - [“Matrix”]{.arg-text}: Sparse matrix pre-computed Cholesky decomposition *with* fast updating
      - Larger numbers of observations can be handled, but the [interval]{.arg-text} may need be set when the weights are not row-standardized.
      - Set `control = list(super = as.logical(NA))` to use simplicial decomposition for the sparse Cholesky decomposition. If [TRUE]{.arg-text}, use a supernodal decomposition
    - ["Matrix_J"]{.arg-text}: Sparse matrix pre-computed Cholesky decomposition *without* fast updating
      - For strictly symmetric spatial weights lists (["B"]{.arg-text} and ["C"]{.arg-text}) or made symmetric by similarity for ["W"]{.arg-text} and ["S"]{.arg-text} (See [Geospatial, Spatial Weights \>\> Diagnostics](geospatial-spatial-weights.qmd#sec-geo-swgt-diag){style="color: green"} \>\> Symmetry)
      - Larger numbers of observations can be handled, but the [interval]{.arg-text} may need be set when the weights are not row-standardized.
      - Set `control = list(super = FALSE)` to use simplicial decomposition for the sparse Cholesky decomposition. If [TRUE]{.arg-text}, use a supernodal decomposition
    - ["spam_update"]{.arg-text}: Pre-computed Cholesky decomposition *with* updating
    - ["spam"]{.arg-text}: Standard Cholesky decomposition *without* updating
      - For strictly symmetric spatial weights lists (["B"]{.arg-text} and ["C"]{.arg-text}) or made symmetric by similarity for ["W"]{.arg-text} and ["S"]{.arg-text} (See [Geospatial, Spatial Weights \>\> Diagnostics](geospatial-spatial-weights.qmd#sec-geo-swgt-diag){style="color: green"} \>\> Symmetry)
    - ["LU_prepermutate"]{.arg-text}: Standard LU decomposition *with* updating (pre-computed fill-reducing permutation)
      - Alternative sparse matrix decomposition approach
      - Larger numbers of observations can be handled, but the [interval]{.arg-text} may need be set when the weights are not row-standardized.
    - ["LU"]{.arg-text}: Standard LU decomposition *without* updating
      - Alternative sparse matrix decomposition approach
      - Larger numbers of observations can be handled, but the [interval]{.arg-text} may need be set when the weights are not row-standardized.
    - ["MC"]{.arg-text}: Monte Carlo Approximation; approximate log-determinant method
    - ["Cheb"]{.arg-text}: Chebyshev approximation; approximate log-determinant method
    - ["moments"]{.arg-text}: Smirnov/Anselin (2009) trace approximation
    - ["SE_classic"]{.arg-text}: Emulates the behavior of Spatial Econometrics toolbox ML fitting functions. Use grids of log determinant values
    - ["SE_whichMin"]{.arg-text}, and ["SE_interp"]{.arg-text}: Attempt to ameliorate some features of ["SE_classic"]{.arg-text}. (See docs for details)
  - `summary` results for models with:
    ...
    - [correlation]{.arg-text}: if [TRUE]{.arg-text}, the correlation matrix of the estimated parameters including sigma is returned and printed.
    - [Nagelkerke]{.arg-text}: if [TRUE]{.arg-text}, the Nagelkerke pseudo R-squared is reported
  - Fitting Help
    - [interval]{.arg-text}: Search interval for autoregressive parameter
      - For sparse matrix methods, it may need be set when the weights are not row-standardized.
    - [pre_eig]{.arg-text}, [pre_eig1]{.arg-text}, [pre_eig2]{.arg-text}: For adding precalculated eigenvalue vectors which can be computed using `eigenw`. Probably useful for optimization. From the examples:
      - `lagsarlm` just uses one: [pre_eig]{.arg-text}, e.g. `control = list(pre_eig = eig_vec)`
      - `sacsarlm` can use two: [pre_eig1]{.arg-text} and [pre_eig2]{.arg-text}
        - Give the same vector to both arguments.
    - [small]{.arg-text}: default 1500; threshold number of observations for asymmetric covariances when the method is not “eigen”
      - No idea what this is about, but in the examples in the docs, it was set to 25 when [method = "Matrix"]{.arg-text}. Could just be an optimizaton. e.g. `control = list(small = 25)`
    - [tol.solve]{.arg-text}: The tolerance for detecting linear dependencies in the columns of matrices to be inverted. Passed to `solve` (default 1.0e-10).
      - If errors come from `solve`, then setting this to a lower value may be necessary to extract coefficient standard errors (for instance lowering to 1e-12)
      - Errors in `solve` could also indicate that variables have scales differing a lot from the autoregressive coefficient. Rescaling the RHS variables alleviates this better than setting [tol.solve]{.arg-text} to a very small value.
        - The values in this matrix may be very different in scale, and inverting such a matrix is analytically possible by definition, but numerically unstable.
    - [trs]{.arg-text}: Used when the numerical Hessian is poorly conditioned, and NaNs are generated in subsequent operations
...
- Misc
  ...
  - `predict`
    - [listw]{.arg-text}: Should be a spatial weights object for whole dataset
      - i.e. it needs to have all possible 'row.names'.
      - e.g. for out-of-sample, this should be a [listw]{.arg-text} created from the data before it's split into training and test.
      - This seems like this procedure wasn't considered for a production use case where there are future new unexpected units.
        - Maybe you can get around this manually just adding the new unit to the original dataset and recreating [listw]{.arg-text} object for it.
        - Maybe setting `row.names` manually to an id variable, because it looks like it uses row numbers by default. If you add the new unit it the end of the data.frame and keep the same order, maybe using row numbers doesn't matter either.
    - SLX (`lmSLX`) has it's own prediction method and doesn't have a [pred.type]{.arg-text} argument. So I'm not sure what kind of predictions it's able to do.
      - Doesn't seem to handle out-of-sample but does handle in-sample and forecast
  - Notes from
    - About predictions in spatial autoregressive models: optimal and almost optimal strategies, Goulard et all 2017 (See R \>\> Documents \>\> Geospatial)
      - Breaks down TS, TC, BP, BPN, BPW and compares out-of-sample performance
    - [The relative efficiencies of various predictors in spatial econometric models containing spatial lag, Kelejian and Prucha 2007](https://econweb.umd.edu/~prucha/Papers/RSUE37(2007).pdf)
      - Compared 5 prediction methods on two matrices which differed by sparseness
  - **TL;DR** Goulard et all 2017
    - Sparse $W$ Matrix (Spatial Weights Matrix):
      - [BPN]{.arg-text} is best in performance but maybe a little more computationally expensive than {.arg-text} and [TS]{.arg-text}
      - [TC]{.arg-text} is the only method that performs notably worse than the others. The rest are very close to each other with the rank being [BP]{.arg-text}, [BPN]{.arg-text}, [BPW]{.arg-text}, and [TS]{.arg-text}.
    - Dense $W$ matrix:
      - All methods have almost identical performance
  - **TL;DR** Kelejian and Prucha 2007
    - In both types of $W$ matrices (differing by sparseness):
      - By far, the largest mean squared errors correspond to [KP1]{.arg-text} (aka [TC1]{.arg-text})
      - If there were tiers, then [KP4]{.arg-text} (aka [TS1]{.arg-text}) would be in the next worst tier
      - [KP3]{.arg-text} (similar to [BP1]{.arg-text}) had the best results; followed by [KP2]{.arg-text} (similar to [BPW]{.arg-text}). They would be in the last tier.
  - Check sparsity: `mean(spdep::card(nb_list))`
    - [nb_list]{.var-text} is a neighbors list and I think just printing the object also gives you the average number of links
    - That gives the average number of neighbors each unit has.
    - [Example]{.ribbon-highlight}: if on average each unit has around 8 neighbors out of roughly 2500 total units, then that would be extremely sparse.
  - ["BP"]{.arg-text} uses a weighted sum of all in-sample residuals for its correction factor. The weights are included in the form of a precision matrix (function of $W$) and reflects how strongly each in-sample unit is spatially connected to the out-of-sample unit being predicted.\
    $$\hat Y_O^{BP} = \hat Y_O^{TC} - Q_{OO}^{-1}Q_{OO} \times (Y_S- \hat Y_S^{TC})$$
    - $\hat Y_{TC}$: The trend corrected prediction
    - $Q_{OO}^{-1}Q_{OO}$: Is the covariance matrix which is calculated using the precision matrix, $Q$, which is calculated using a quadratic expression that has $W$ (also a 2x2 matrix, so easily calculated).
    - $Y_S - \hat Y_S^{TC}$ is a sort of residual of the observed values and the trend corrected prediction.
    - So multiplying an easily calculated covariance matrix times a vector of residuals is the gist of the *extra* prediction correction here.
  - ["BPN"]{.arg-text} does the same as ["BP"]{.arg-text} but drops in-sample non-neighbors to the out-of-sample unit since their weights are very close to zero. Near identical performance when $W$ is sparse, but also still is close to ["BP"]{.arg-text} even when $W$ is dense.
  - ["BPW"]{.arg-text} has a more complex calculation of residual factor and the residual vector used is calculated as a difference of weighted averages (for a row-normalized $W$).
    - Replaces $Q_{OO}^{-1}Q_{OO}$ with a more complex expression that requires an extra matrix inversion.
    - Replaces $Y_S - \hat Y_S^{TC}$ with $W_{OS}Y_S - W_{OS}\hat Y^{TC}$. The authors claim this results in a loss of information, since you're calculating the difference of averages. Except you can just factor out $W_{OS}$ and include it with correction coefficient part of the expression. Then, you're left with $Y_S - Y^{TC}$ which means their loss-of-information argument is gone.
    - Anyways I do completely agree with computational efficiency aspect and in the paper
...
- Types
  ...
  - [**In-Sample**]{style="color: #009499"}: Predicting for the units the model was fitted on. The response $y$ is known, so its spatial lags can be used directly to compute the **signal**.
    - [newdata = NULL]{.arg-text}
  - [**Forecast**]{style="color: #009499"} aka [**Prevision**]{style="color: #009499"}: The [newdata]{.arg-text} has the same spatial units, different values of the covariates. This is a *temporal-style* forecast — you're projecting forward with new $X$ but the same neighborhood structure and known unit identities.
    - [newdata]{.arg-text} has the *same* row names as model's [listw]{.arg-text}'s [region.id]{.arg-text} attribute
  - [**Out-of-Sample**]{style="color: #009499"}: The [newdata]{.arg-text} has new spatial units. Now $y$ is unknown, so the **signal** cannot use observed $Wy$ and must be approximated somehow. This is where most of the [pred.type]{.arg-text} proliferation comes from.
    - [newdata]{.arg-text} has the *different* row names as model's [listw]{.arg-text}'s [region.id]{.arg-text} attribute
    - [**Single-Prediction**]{style="color: #009499"} (aka [**leave-one-out**]{style="color: #009499"}): Applies the ‘single out-of-sample’ formula to each of the out-of-sample unit separately, ignoring at each stage the remaining $p − 1$ units.
      - Computed separately per unit when newdata has multiple rows.
      - It allows the **trend**-**signal** strategy to be included which only exists in the single prediction case.
...
+=====================================================+===============================================================================================================================================================================+====================+================================+
| ["trend"]{.arg-text}                                | No spatial adjustment. Returns $X\beta$ only — ignores all spatial dependence.                                                                                                | fit                | All models                     |
+-----------------------------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["TS"]{.arg-text} (default)                         | Trend Signal Noise predictor: $\hat Y^{TS} = X \hat \beta + \hat \rho W Y$\                                                                                                   | fit, trend, signal | SEM, SDEM, SLM, SDM            |
|                                                     | signal = $\rho Wy$ (SLM, SDM) or $\lambda Wy − \lambda WX \beta$ (SEM, SDEM))\                                                                                                |                    |                                |
|                                                     | fit = trend + signal                                                                                                                                                          |                    |                                |
+-----------------------------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["TC"]{.arg-text}; also requires [listw]{.arg-text} | Trend-corrected: the conditional mean, $\hat Y^{TC} = (I-\hat\rho W)^{-1}X\hat\beta$ under the fitted model.                                                                  | fit, trend         | SLM, SDM, SAC, GNM (caution\*) |
+-----------------------------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["BP"]{.arg-text}; also requires [listw]{.arg-text} | Starts from [TC]{.arg-text}, adds a correction that's meant to adjust the prediction based on the observed $y$ values of the *in-sample neighbors* of the out-of-sample units | fit, trend         | SLM, SDM, SAC, GNM (caution\*) |
+-----------------------------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+

- [Out-of-sample]{.underline}

+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
|                                                      |                                                                                                                                                                                                                                                                 |                    |                                |
+======================================================+=================================================================================================================================================================================================================================================================+====================+================================+
| ["trend"]{.arg-text}                                 | See In-sample and Forecast                                                                                                                                                                                                                                      | fit                | All models                     |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["TS"]{.arg-text}                                    | See In-sample and Forecast                                                                                                                                                                                                                                      | fit, trend, signal | SEM, SDEM, SLM, SDM            |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["TS1"]{.arg-text} or ["KP4"]{.arg-text}             | Single-prediction version of ["TS"]{.arg-text}.                                                                                                                                                                                                                 | fit                | SLM, SDM, SAC, GNM             |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["TC"]{.arg-text}                                    | See In-sample and Forecast                                                                                                                                                                                                                                      | fit                | SLM, SDM, SAC, GNM             |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["TC1"]{.arg-text} or ["KP1"]{.arg-text}             | Single-prediction version of ["TC"]{.arg-text}. Most broadly applicable out-of-sample predictor — works for all model types                                                                                                                                     | fit                | All models                     |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["BP"]{.arg-text}                                    | See In-sample and Forecast                                                                                                                                                                                                                                      | fit                |                                |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["BP1"]{.arg-text}; also requires [listw]{.arg-text} | The single-prediction version of ["BP"]{.arg-text}                                                                                                                                                                                                              | fit                | SLM, SDM, SAC, GNM (caution\*) |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["BPW"]{.arg-text}\                                  | Same as ["BP"]{.arg-text} except it uses a weighted ($W$) average of neighbor y values for the correction. ["BPW1"]{.arg-text} is the single-prediciton version.                                                                                                | fit                | SLM, SDM, SAC, GNM (caution\*) |
| ["BPW1"]{.arg-text}                                  |                                                                                                                                                                                                                                                                 |                    |                                |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["BPN"]{.arg-text}\                                  | Same as ["BP"]{.arg-text} but drops non-neighbors. For a sparse W matrix, this means ["BPN"]{.arg-text} has near identical performance as ["BP"]{.arg-text} but is more computationally efficient. ["BPN1"]{.arg-text} is the single-prediction version.        | fit                | SLM, SDM, SAC, GNM (caution\*) |
| ["BPN1"]{.arg-text}                                  |                                                                                                                                                                                                                                                                 |                    |                                |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["KP2"]{.arg-text}                                   | $y_i = \hat \rho_{\text{lag}} w_iy + x_i\beta + \frac{\text{cov}(u_i, w_iy)}{\text{var}(w_iy)}[w_iy-\mathbb{E}(w_iy)]$\                                                                                                                                         | fit                | All models                     |
|                                                      | Starts at [TS1]{.arg-text}. Uses the correlation between error and spatial lagged responses as the correction coefficient. That coefficient is multiplied times the centered weighted average to produce the full correction term. Similar to [BPW]{.arg-text}. |                    |                                |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["KP3"]{.arg-text}                                   | $y_i = \hat \rho_{\text{lag}} w_i y + x_i \beta +\\ \text{cov}(u_i,y_-i)[\text{VC}(y_{-1})]^{-1}[y_{-i}-\mathbb{E}(y_{-i})]$\                                                                                                                                   | fit                | All models                     |
|                                                      | Both [BP1]{.arg-text} and [KP3]{.arg-text} are BLUPs, so they should give the similar predictions. Note that BP1 can't handle error models though.                                                                                                              |                    |                                |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
| ["KP5"]{.arg-text}                                   | $y_i = x_i \beta + \rho_{\text{err}} w_i [y-X\beta]$\                                                                                                                                                                                                           | fit                | SEM, SDEM, SAC, GNM            |
|                                                      | The trend plus a correction using the spatially weighted residuals of all other units.\                                                                                                                                                                         |                    |                                |
|                                                      | Biased but still performs well.                                                                                                                                                                                                                                 |                    |                                |
+------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------+--------------------------------+
...
- [Example 2]{.ribbon-highlight}: Fitting a SLM and Predicting Out-of-Sample

  ::: panel-tabset
  ## Data and Set-Up

  Same data used in Example 1

  ``` r
  library(dplyr); library(spatialreg)

  boston_506 <- sf::st_read(system.file(
  ...
  - Code splits the data and creates row-standardized spatial weights objects for the model and predict functions
  - The out-of-sample datafrrame has NAs for the response and different locations from the training dataframe.

  ## Fit

  ###### Fit Eigenvalue and Matrix methods
  ...
  #> 
  #> rho:    -0.236068    function value:    83.10345 
  #> rho:    0.236068     function value:    85.02081 
  #> rho:    0.527864     function value:    -120.8932 
  #> rho:    0.002185569    function value:  174.2687 
  #> rho:    0.00125444     function value:  174.2688 
  #> rho:    0.00170643     function value:  174.2692 
  #> rho:    0.001708617    function value:  174.2692 
  #> rho:    0.001708622    function value:  174.2692 
  #> rho:    0.001708627    function value:  174.2692 
  #> rho:    0.001708622    function value:  174.2692 

  slm_489_mat <- lagsarlm(
    form, 
    data = boston_489, 
    listw = lw_q_489,
    method = "Matrix",
  ...
  #> rho:    -0.2364499     function value:  82.85657 
  #> rho:    0.2354499    function value:    85.41977 
  #> rho:    0.5271001    function value:    -120.3583 
  #> rho:    0.00241657     function value:  174.2682 
  #> rho:    0.001177578    function value:  174.2686 
  #> rho:    0.001691703    function value:  174.2692 
  #> rho:    0.001708616    function value:  174.2692 
  #> rho:    0.001708621    function value:  174.2692 
  #> rho:    0.001708317    function value:  174.2692 
  #> rho:    0.001708502    function value:  174.2692 
  #> rho:    0.00170857     function value:  174.2692 
  #> rho:    0.001708596    function value:  174.2692 
  #> rho:    0.001708609    function value:  174.2692 
  #> rho:    0.001708616    function value:  174.2692 
  #> Computing eigenvalues ...
  ```

  - The matrix method requires more iterations but both converge to almost the same $\rho$
  ...
  #> Call:lagsarlm(formula = form, data = boston_489, listw = lw_q_489, 
  #>     method = "Matrix", quiet = FALSE, zero.policy = TRUE)
  #> 
  #> Residuals:
  #>        Min         1Q     Median         3Q        Max 
  #> -0.7332843 -0.0855992 -0.0079685  0.0915519  0.8595278 
  #> 
  #> Type: lag 
  ...
  #> (Intercept)        9.8990e+00  2.1731e-01  45.5528 < 2.2e-16
  #> crim              -9.5377e-03  1.2666e-03  -7.5304 5.063e-14
  #> zn                -5.6191e-05  4.8430e-04  -0.1160   0.90763
  #> indus              9.5042e-04  2.2586e-03   0.4208   0.67390
  #> chas1              5.5507e-02  3.3593e-02   1.6523   0.09847
  #> I((nox * 10)^2)   -5.9307e-03  1.0587e-03  -5.6018 2.121e-08
  #> I(rm^2)            7.1962e-03  1.3599e-03   5.2918 1.211e-07
  #> age               -1.4118e-04  5.0440e-04  -0.2799   0.77956
  #> log(dis)          -1.6028e-01  3.1968e-02  -5.0137 5.338e-07
  #> log(rad)           8.3988e-02  1.7992e-02   4.6680 3.041e-06
  #> tax               -4.6866e-04  1.1561e-04  -4.0538 5.039e-05
  #> ptratio           -3.0277e-02  4.7687e-03  -6.3491 2.165e-10
  #> I(bb/100)         -2.1630e-01  4.7841e-02  -4.5213 6.145e-06
  #> log(I(lstat/100)) -3.3635e-01  2.6435e-02 -12.7237 < 2.2e-16
  #> 
  #> Rho: 0.0017086, LR test value: 0.011309, p-value: 0.91531
  #> Asymptotic standard error: 0.016471
  #>     z-value: 0.10373, p-value: 0.91738
  #> Wald statistic: 0.010761, p-value: 0.91738
  #> 
  #> Log likelihood: 174.2692 for lag model
  #> ML residual variance (sigma squared): 0.028706, (sigma: 0.16943)
  #> Nagelkerke pseudo-R-squared: 0.79452 
  #> Number of observations: 489 
  #> Number of parameters estimated: 16 
  #> AIC: -316.54, (AIC for lm: -318.53)
  #> LM test for residual autocorrelation
  #> test value: 231.68, p-value: < 2.22e-16
  #> 
  #>  Correlation of coefficients 
  #>                   sigma rho   (Intercept) crim  zn    indus chas1 I((nox * 10)^2) I(rm^2) age   log(dis)
  #> rho                0.00                                                                                 
  #> (Intercept)        0.00 -0.78                                                                           
  #> crim               0.00  0.04 -0.07                                                                     
  #> zn                 0.00  0.06 -0.11       -0.11                                                         
  #> indus              0.00  0.00 -0.09        0.10  0.14                                                   
  #> chas1              0.00 -0.04  0.02        0.03  0.00 -0.08                                             
  #> I((nox * 10)^2)    0.00  0.01 -0.29        0.08 -0.01 -0.21 -0.09                                       
  #> I(rm^2)            0.00 -0.07 -0.01        0.03 -0.06  0.10 -0.05  0.07                                 
  #> age                0.00  0.05 -0.28        0.04  0.12  0.04 -0.03 -0.15           -0.30                 
  #> log(dis)           0.00 -0.10 -0.23        0.23 -0.29  0.28 -0.03  0.34            0.04    0.38         
  #> log(rad)           0.00  0.00  0.03       -0.17  0.23  0.21 -0.12 -0.11           -0.10    0.03 -0.05   
  #> tax                0.00  0.06 -0.02       -0.12 -0.32 -0.40  0.15 -0.12           -0.04    0.04  0.04   
  #> ptratio            0.00  0.12 -0.49       -0.03  0.30 -0.12  0.06  0.32            0.08   -0.06 -0.11   
  #> I(bb/100)          0.00  0.05 -0.04       -0.15  0.01 -0.02  0.04 -0.04           -0.15    0.07 -0.02   
  #> log(I(lstat/100))  0.00  0.02  0.24       -0.07  0.06 -0.10 -0.01 -0.04            0.62   -0.45 -0.09   
  #>                   log(rad) tax   ptratio I(bb/100)
  #> rho                                               
  #> (Intercept)                                       
  #> crim                                              
  ...
  #> tax               -0.68                           
  #> ptratio           -0.04    -0.21                  
  #> I(bb/100)         -0.05    -0.06  0.01            
  #> log(I(lstat/100)) -0.02    -0.06 -0.05   -0.14    
  ```

  - Psuedo-R^2^ is around 0.79.
  ...
  - The [listw]{.arg-text} object for `predict` is the spatial weights list for the entire dataset.
  - The eigenvalue and matrix model methods produce almost identical predictions
  - The difference in predictions is more noticable between the [TS1]{.arg-text} an [BPN]{.arg-text} prediction methods
  :::
