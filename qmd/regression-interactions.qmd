...
- Packages
  - [{]{style="color: #990000"}[APCinteraction](https://github.com/tranbaokhue/APCinteraction){style="color: #990000"}[}]{style="color: #990000"} - A nonparametric R package for **detecting interactions in two-way ANOVA** designs with balanced replication using all possible crossed comparisons (APC).
    - Provides 180 pre-computed null distributions (100,000 simulations each) for the APCSSA and APCSSM nonparametric interaction test statistics.
  ...
  - [{]{style="color: #990000"}[ixsurface](https://cran.r-project.org/web/packages/ixsurface/index.html){style="color: #990000"}[}]{style="color: #990000"} - **Interactive 3D Surface Plots** for Multi-Factor Interaction Visualization
    - Supports continuous, categorical, and mixed factor designs with automatic binning for continuous conditioning variables.
  ...
  - [{]{style="color: #990000"}[mlmoderator](https://cran.r-project.org/web/packages/mlmoderator/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides a unified workflow for probing, plotting, and assessing the robustness of cross-level interaction effects in two-level **mixed-effects** models fitted with [{lme4}]{style="color: #990000"}
  ...
  - [{]{style="color: #990000"}[simpr.interaction](https://cran.r-project.org/web/packages/simpr.interaction/index.html){style="color: #990000"}[}]{style="color: #990000"} - **Analytically calculate parameters for the simulation** of moderated multiple regression, based on the correlations among the predictors and outcome and the reliability of predictors.
...
- Stay away from polynomial interactions (unless there's a theoretical/physical reason). High-order polynomial interactions like $x^2y^2$ are notorious for instability at the boundaries of your data and lead to non-sensical predictions. (better to use splines instead)
