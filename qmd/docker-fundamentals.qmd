# Fundamentals {#sec-docker-fund .unnumbered}
...
-   Resources
    ...
    -   [DevOps for Data Science, Appendix](https://do4ds.com/chapters/append/cheatsheets.html#cheat-docker)
        -   Cheatsheet for popular docker, dockerfile, etc. commands/instructions
...
-   (Not sure if `container` is required now)
-   `docker container ls`
...
-   `docker container stop <name>` or `kill`
    -   Stops a container
...
-   [-p]{.arg-text}
    ...
    -   AKA *exposing* or *publishing* a port
    -   Expected to supply two ports separated by a colon
    -   [Example]{.ribbon-highlight}: [-p 5000:5000]{.arg-text}
        -   The first [5000]{.arg-text} is the host port. The port outside the container that you use to interact with the app in your browser (e.g. [localhost:5000]{.arg-text})
    ...
    -   [Example]{.ribbon-highlight}: `docker run -p 8080:3000 learncloudnative/helloworld:0.1.0`
        -   Once the image is downloaded you will see the message saying "Listening on port 3000". This is the message from the application running inside the container that's listening on port 3000.
        -   With the -p flag we mapped container (inside) port 3000 to the host (outside) port 8080.
            -   If you open your browser and navigate to <http://localhost:8080> you will get the "Hello World!" web page.
...
-   [-P]{.arg-text}
    -   Says to map the port that the application listening on to a random host port (i.e. outside the container, that you access through you're browser).
    -   To use this, you need to use the EXPOSE instruction in your dockerfile (e.g. `EXPOSE 3000`), so the application's port is known
    -   [Example]{.ribbon-highlight}: `docker run -P learncloudnative/helloworld:0.1.0`
-   [--restart]{.arg-text}
...
## Docker Compose {#sec-docker-fund-dc .unnumbered}
...
-   [Example]{.ribbon-highlight}:
    ...
    -   [version]{.arg-text} is the version of the compose api
    -   [services]{.arg-text} are the containers we're building
        -   Service names (e.g. [redis]{.var-text}, [web]{.var-text}) will end up being the container and image names (See below for the exception)
    -   Properties
        -   [image]{.arg-text} - Uses the base:tag format like the value for the FROM instruction in the dockerfile
        -   [build]{.arg-text} - '.' says build an image from the current directory - If your app has it's own folder then you need to specify the path - e.g. './web'
        -   [image]{.arg-text} and [build]{.arg-text} property in the same service
            -   Docker will build 2 of the same image
                -   One with project name ([build]{.arg-text}) and the other with the [image]{.arg-text} value as the name for the image
            -   [Example]{.ribbon-highlight} [build: '.']{.arg-text} and [image: 'ercbk/web:1.0]{.arg-text}
                -   Says build image using dockerfile in current directory and name it [ercbk/web:1.0]{.var-text}
                -   Useful if the image is going to be pushed to the docker hub
        -   [ports]{.arg-text}
            -   Ports to be used for the container (See [Running Containers \>\> Flags](docker-fundamentals.qmd#sec-docker-fund-rc-flag){style="color: green"} \>\> -p)
        ...
        -   [depends_on]{.arg-text}
            -   Necessary if one service depends on another. Indicates if one container needs to start before another.
                -   e.g. [web]{.var-text} (app) depends on [redis]{.var-text} (db)
            -   Takes a value in list format
    -   [worker]{.var-text}
        -   Use the same dockerfile for both services
            -   e.g. [worker]{.var-text} uses the same dockerfile as [web]{.var-text}
        -   Useful for background services for your app
            -   [{{celery}}]{style="color: goldenrod"} used for this task (which someone said was garbage)
        -   Differences
    ...
    -   [volumes]{.var-text}
        -   The name given here needs to match the name of the volume given in the services property
            -   e.g. For this yaml the name of the volume created is [redis]{.var-text}
...
-   [Example]{.ribbon-highlight}: Databases and Workers communicate on one network while apps communicate on both networks
    ...
    -   [volumes]{.arg-text}
        -   See [Data Volumes](docker-fundamentals.qmd#sec-docker-fund-dv){style="color: green"}
        ...
        -   In the redis service,
            -   [web2_redis]{.var-text} is the name we give to the data volume
            -   `/data` is designated by the redis people

-   Setting Environment Variables

    -   Method 1: [environment]{.arg-text}

        -   [name: value]{.arg-text} pair
    ...
    -   Method 2: [env_file]{.arg-text}

        -   Takes a list of environment files to load
        ...
        -   e.g. `.env` is the name of the file in the current directory
        ...
        -   [Example]{.ribbon-highlight}: File
            ...
            -   Can configure Docker Compose options here
                -   e.g. Supply a project name. Otherwise Docker would just use the current directory for the name of the project. Also gets added as a prefix for networks, etc. that Compose will create.
            -   The Python buffer variable is necessary if you want to see the output from the terminal through Compose

## Docker-Compose Commands {#sec-docker-fund-dcc .unnumbered}
...
-   `docker-compose up`
    -   Runs the project (everything created with project name prefix)
        -   Creates network
            -   eg [web2_defaults]{.var-text}
        -   Creates volume
            -   eg [web2_redis]{.var-text}
        -   Starts containers
            -   eg [web2_redis_1]{.var-text}, [web2_web_1]{.var-text}
                -   The `_1` suffix is in case the project calls for multiple instances of the same container
                -   Reminder: Multiple apps require different ports (binding?). Would also require setting up a load balancer.
                    -   See [Running Containers \>\> Flags](docker-fundamentals.qmd#sec-docker-fund-rc-flag){style="color: green"} \>\> -p
                    -   See [Docker, AWS \>\> Create Application Load Balancer](docker-aws.qmd#sec-docker-aws-calb){style="color: green"}
    -   `docker-compose up <service>`
        -   Starts a specific service
        -   If you start a service with a dependency ([depends_on]{.arg-text} specified in yaml), it will also start that dependency service.
        -   `docker compose web`
            -   Starts [web]{.var-text} but also [redis]{.var-text}, because [web]{.var-text} has a specified [redis]{.var-text} dependency
-   `docker-compose stop`
...
-   `docker-compose up --build -d`
    ...
    -   [-d]{.arg-text} says run in the background
-   `docker-compose ps`
    -   Similar info as `docker container ls` but presented slightly differently
-   `docker-compose logs -f`
    -   Since containers running in the background [-f]{.arg-text} needed to logs of the container activity
...
-   `docker-compose exec web`
    ...
    -   Opening a shell inside the container
        -   `docker-compose exec web sh`
            -   No [-it]{.arg-text} flag necessary like for `docker container exec` (See [Running Containers \>\> Commands](docker-fundamentals.qmd#sec-docker-fund-rc-coms){style="color: green"})
    -   `exit`
        -   Exits the shell
...
-   `docker-compose rm`
    ...
    -   These will also be removed using the `docker system prune` from [Docker Utility Functions](docker-fundamentals.html#sec-docker-fund-duf){style="color: green"}
