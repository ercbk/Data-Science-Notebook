# Fundamentals {#sec-docker-func}

## Misc {#sec-docker-func-misc}
...
## Creating a Dockerfile {#sec-docker-func-cadf}
...
-   Architecture
    ...
    -   [ARG]{.arg-text}: Defines a variable that users can pass at build-time to the builder with the docker build command using the `--build-arg <varname>=<value>` flag.
        -   Don't use for secrets since build arguments are visible in the docker history command and in max mode provenance attestations, which are attached to the image by default if you use the Buildx GitHub Actions and your GitHub repository is public. See `RUN --mount=type=secret` [docs](https://docs.docker.com/reference/dockerfile/#run---mounttypesecret) for the secure way to use secrets.
    ...
    -   `LABEL`s have key-value pairs. can be useful in production settings. The can be retrieved with a command later on.
        ...
        -   LABEL
            -   maintainer="Eric Book [ericbook\@email.com](mailto:ericbook@email.com){.email}"\\
            -   version="1.0"
...
## Running Containers {#sec-docker-func-rc}
...
## Networking Containers {#sec-docker-func-nc}
...
## Data Volumes {#sec-docker-func-dv}
...
## Optimizing {#sec-docker-func-opt}
...
## Running Scripts When a Container Starts {#sec-docker-func-arswacs}
...
## Docker Utility Functions {#sec-docker-func-duf}
...
## Docker Compose {#sec-docker-func-dc}
...
## Managing a Web App With Docker-Compose {#sec-docker-func-mawawdc}
