...
- **Outliers**: An indicator variable can also be used to account for an outlier in the data. Rather than omit the outlier, an indicator variable removes its effect. (FPP 3, [7.4](https://otexts.com/fpp3/useful-predictors.html#dummy-variables))

- **Intervention Variables** (FPP 3, [7.4](https://otexts.com/fpp3/useful-predictors.html#dummy-variables))
  ...
  - If the effect causes a level shift (i.e., the value of the series changes suddenly and permanently from the time of intervention), then use a “step” variable.
    ...
    - See [EDA, Time Series \>\> Seasonality and Trend](eda-time-series.html#sec-eda-ts-seas){style="color: green"} \>\> Level Changes in Trend
...
- Box-Cox Transformation
  - [{]{style="color: #990000"}[forecast::BoxCox and InvBoxCox](https://pkg.robjhyndman.com/forecast/reference/BoxCox.html){style="color: #990000"}[}]{style="color: #990000"} can be used to auto-transform and back-transform
  - [{]{style="color: #990000"}[forecast:: BoxCox.lambda](https://pkg.robjhyndman.com/forecast/reference/BoxCox.lambda.html){style="color: #990000"}[}]{style="color: #990000"} performs both Guerrero and Log Likelihood versions for finding lambda manually
  - [{]{style="color: #990000"}[actfts::acfinter](https://sergiofinances.github.io/actfts/reference/acfinter.html){style="color: #990000"}[}]{style="color: #990000"} performs both Guerrero and Log Likelihood versions among other tests
    - Lambda Guidelines
      - 1: No transformation.
      - 0: Natural logarithm.
      - 0.5: Square root.
...
- Calandar Variables
  ...
  - Day of the month, day of the year, week of the month, week of the year, month, and year
    ...
    - Using [{lubridate}]{style="color: #990000"} to add calendar features in a tsibble will create an ordinal variable ([source](https://ramikrispin.github.io/r-ladies-rome-workshop/05_forecasting_with_lm.html#modeling-seasonal-time-series))

      ``` r
      ts$month <- lubridate::month(ts2$index, label = TRUE)
      head(tsj)
      ...
      - This was effective in modeling seasonality in a simple regression model
  ...
  - [Example]{.ribbon-highlight}: [{fable}]{style="color: #990000"} Linear Regression ([source](https://otexts.com/fpp3/useful-predictors.html#example-australian-quarterly-beer-production-1))
    ...
    - For a simple regression model, you want to use dummies instead of one-hot
    - `season` automatically takes care of creating the dummies from a tsibble
...
- Events
  ...
  - [Example]{.ribbon-highlight}: COVID and Iran War ([source](https://freerangestats.info/blog/2026/06/14/samoa-seasonal-adjustment))
    ...
    cbind(covid_reg, war_reg)
    ```

    - Some packages, like the X-13 SEATS model, use a ts matrix adding predictor variables. (See [Forecasting, Decomposition \>\> Seasonal Adjusted Forecasting](forecasting-decomposition.qmd#sec-decomp-saf){style="color: green"} \>\> Example 1)
...
- [{slider}]{style="color: #990000"} - more sophisticated way without data leakage
  ...
  ``` r
  SCORE_recent <- slider::slide_index_dbl(
    SCORE,
    date,
    mean,
    na.rm = TRUE,
...
- Lag Matrix ([source](https://ecogambler.netlify.app/blog/distributed-lags-mgcv/#setting-up-lag-matrices))

  ``` r
  create_lag_mat <- function(x, n_lag = 6) {
    n <- length(x)
    X <- matrix(NA, n, n_lag)
    for (i in 1:n_lag){
      X[i:n, i] <- x[i:n - i + 1]
  ...
  mintemp <- do.call(
    rbind, 
    lapply(
      seq_along(levels(portal_ts$series)), 
      function(x) {
        tempdat <- portal_ts %>%
        ...
        lag_mat <- create_lag_mat(tempdat, 6)
  ...
  - Some packages like {mgcv} can take lists of variables as input, so that lag matrices can be used.
  - The first five observations are removed from each series (feel like there's a more orthodox way to this than using `tail`)

### Rolling, Window, Cumulative {#sec-feat-eng-ts-eng-roll .unnumbered}

- Packages

  - [{]{style="color: #990000"}[slider](https://slider.r-lib.org/){style="color: #990000"}[}]{style="color: #990000"} - A family of general purpose “sliding window” functions. The API is purposefully *very* similar to purrr. The goal of these functions is usually to compute rolling averages, cumulative sums, rolling regressions, or other “window” based computations.
  - [{]{style="color: #990000"}[data.table::froll\*](https://rdatatable.gitlab.io/data.table/reference/froll.html){style="color: #990000"}[}]{style="color: #990000"} - data.table rolling functions
  - [{]{style="color: #990000"}[RcppRoll](https://cran.r-project.org/web/packages/RcppRoll/index.html){style="color: #990000"}[}]{style="color: #990000"} - Routines for the efficient computation of windowed mean, median, sum, product, minimum, maximum, standard deviation and variance are provided
  - [{]{style="color: #990000"}[roll](https://github.com/jasonjfoster/roll){style="color: #990000"}[}]{style="color: #990000"} - Provides fast and efficient computation of rolling and expanding statistics for time-series data.
...
- If there isn't much variation in the variable (e.g. NDVI vegetation index), it would probably be more informative to decompose it into long-run mean, a seasonal component, an anomaly indicator, etc. variables per grouping variable (e.g. yearly mean for site 12)
...
- Moving Averages
  ...
  - slider
    ...
    ts_df |> 
      mutate(
        three_day_moving_avg = slide_mean(val, 
                                          before = 2L, 
                                          complete = TRUE)
      )
    ```
...
- Notes from
  ...
  - [FPP 3, 7.4](https://otexts.com/fpp3/useful-predictors.html#fourier-series)
...
- From [Cyclic Encoding in Machine Learning : A Practical Guide with Code and Intuition](https://medium.com/@indradeep.chatterjee/cyclic-encoding-in-machine-learning-a-practical-guide-with-code-and-intuition-049846419ccb)

  - Use sine and cosine pairs together to create a feature
    - A sine wave alone loses orientation (two points can share the same sine value).
  - Use the correct period
    - Hours → 24
    - Minutes → 60
    - Weekday → 7
    - Month → 12
    - Day-of-year → 365 (or 366)
  - Do not include both hour and hour_sin. This adds noise and redundancy.
...
- [Example]{.ribbon-highlight}: [{fable}]{style="color: #990000"} linear regression ([source](https://otexts.com/fpp3/useful-predictors.html#fourier-series))
...
- Packages
  ...
  - [{]{style="color: #990000"}[focus](https://cran.r-project.org/web/packages/focus/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides high-performance online changepoint detection in univariate and multivariate data streams.
    - Implements efficient 'C++' backends for the 'focus', 'md-focus' and 'np-focus' algorithms, with an 'R' interface for real-time monitoring and offline analysis.
  ...
  - [{]{style="color: #990000"}[L0TFinv](https://cran.r-project.org/web/packages/L0TFinv/index.html){style="color: #990000"}[}]{style="color: #990000"} - Efficient L0 trend filtering
  - [{]{style="color: #990000"}[L0cpt](https://cran.r-project.org/web/packages/L0cpt/index.html){style="color: #990000"}[}]{style="color: #990000"} - Change Point Detection with L0 Penalty
  ...
  - [{]{style="color: #990000"}[moped](https://github.com/EuanMcGonigle/moped){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/html/2509.00585v1)) - Moving window-based change point detection of pairwise extremal dependence for multivariate time series
  ...
  - [{]{style="color: #990000"}[rbreak](https://cran.r-project.org/web/packages/rbreak/index.html){style="color: #990000"}[}]{style="color: #990000"} - Methods for detecting structural breaks and estimating break locations for linear multiple regression models under general linear restrictions on the coefficient vector.
  - [{]{style="color: #990000"}[RegimeChange](https://cran.r-project.org/web/packages/RegimeChange/index.html){style="color: #990000"}[}]{style="color: #990000"} - A unified framework for detecting regime changes (changepoints) in time series data.
    - Implements both frequentist methods including Cumulative Sum (CumSum), Pruned Exact Linear Time (PELT), Binary Segmentation, and Wild Binary Segmentation, as well as Bayesian methods such as Bayesian Online Changepoint Detection (BOCPD), Shiryaev-Roberts
  - [{]{style="color: #990000"}[robcp](https://cran.r-project.org/web/packages/robcp/index.html){style="color: #990000"}[}]{style="color: #990000"} - Robust Change-Point Tests
  ...
  - [{]{style="color: #990000"}[tidychangepoint](https://beanumber.github.io/tidychangepoint/){style="color: #990000"}[}]{style="color: #990000"} - Allows you to use any number of algorithms for detecting changepoint sets in univariate time series with a common, tidyverse-compliant interface.
    - Currently, algorithms from [{changepoint}]{style="color: #990000"}, [{wbs}]{style="color: #990000"}, and several genetic algorithms made accessible via [{GA}]{style="color: #990000"} are supported.
  ...
  - [{]{style="color: #990000"}[tptest](https://cran.r-project.org/web/packages/tptest/index.html){style="color: #990000"}[}]{style="color: #990000"} - Performs turning point and inflection point tests for U-shaped and inverse U-shaped relationships in regression models.
- Resources
...
- Papers
  - [Monitoring Time Series for Relevant Changes](https://arxiv.org/abs/2509.01756)
...
- Packages
  ...
  - [hctsa](https://github.com/benfulcher/hctsa) - A Matlab software package for running highly comparative time-series analysis. It extracts thousands of time-series features from a collection of univariate time series and includes a range of tools for visualizing and analyzing the resulting time-series feature matrix. Can be ran through CLI. Calculates like 7000 features.
    - [{]{style="color: goldenrod"}[pyhctsa](https://github.com/DynamicsAndNeuralSystems/pyhctsa){style="color: goldenrod"}[}]{style="color: goldenrod"}
