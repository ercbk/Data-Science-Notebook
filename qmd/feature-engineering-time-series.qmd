...
* "date\_rounded" is a column that has the date for each quarter
...
* Package feature set comparison
	...
	* feature redundancy![](./_resources/Feature_Engineering,_Time_Series.resources/FChllqkVUAAp3rh (3).jpg)
...
* Decimal Date
	* recipe::step\_date(year\_month\_var, features = c("decimal"))
* Calendar features
...
* can also use a basis spline (step\_bs)

* Example: [{mgcv}]{style='color: #990000'}![](./_resources/Feature_Engineering,_Time_Series.resources/Screenshot (1374).png)
		...
		* spline tranformation
			...
			* degree 3 was used in the sklearn [tutorial](https://scikit-learn.org/dev/auto_examples/applications/plot_cyclical_feature_engineering.html#modeling-non-linear-feature-interactions-with-kernels)
				* sklearn also has an extrapolation="periodic" arg
					...
					* don't see this arg in step\_bs or step\_ns
			* Examples
				* hour feature (period = 24, num\_splines = 12)
				* weekday (day of the week, numeric) feature (period=7, num\_splines=3)
				* month (period=12, num\_splines=6)
		* spline transform + step\_kpca\_poly or step\_kpca\_rbf
			...
			* add kpca\_poly allows a regression model to capture non-linear (spline) interactions (kpca\_poly)
				* boosted trees naturally capture these features
			* Example in sklearn [tutorial](https://scikit-learn.org/dev/auto_examples/applications/plot_cyclical_feature_engineering.html#modeling-non-linear-feature-interactions-with-kernels) used n\_components=300 in elasticnet regression which seems crazy, but their point was that if you were to create non-linear interaction features manually it'd be in the thousands
...
* Clustering
	* See [Clustering, Time Series](Clustering, Time Series) for details
- [ ] Lags
...
* Change Point Detection
	* Persistent Homology
		Notes from [Topological Change Point Detection](https://towardsdatascience.com/topological-change-point-detection-162c6e11d3ca)
		* topological method
...
* Domain Specific
	* Net calculation
		* recipe::step\_mutate(monthly\_net = monthly\_export - monthly\_import)
