...
* BigQuery vs [Cloud SQL](https://cloud.google.com/sql/docs/introduction)
	...
	* Pricing
		...
		* CloudSQL has 2 types: Per Use and Packages
			* If usage over 450 hours monthly, then packages is a good option![](./_resources/Google,_BigQuery.resources/Cloud_SQL_vs_BigQuery_image2.png)![](./_resources/Google,_BigQuery.resources/Cloud_SQL_vs_BigQuery_image1.png)
				* Left pic is Per Use pricing; Right pic is Package pricing
		* BigQuery based on Usage![](./_resources/Google,_BigQuery.resources/Cloud_SQL_vs_BigQuery_image3.png)
...
* Using EXCEPT within SELECT![](./_resources/Google,_BigQuery.resources/image.1.png)
* PIVOT for pivot tables![](./_resources/Google,_BigQuery.resources/image.3.png)![](./_resources/Google,_BigQuery.resources/image.2.png)
...
* UNPIVOT![](./_resources/Google,_BigQuery.resources/image.7.png)
...
* It's a pivot\_longer function that puts columns, Laptop, TV, and Mobile, into Items and their values into Sales\_No
* Collapse columns into fewer categories![](./_resources/Google,_BigQuery.resources/image.8.png)
...
* GROUP BY + ROLLUP![](./_resources/Google,_BigQuery.resources/image.4.png)
	* total sales (where quarter = null) and subtotals (by quarter) by year
* QUALIFY ![](./_resources/Google,_BigQuery.resources/image.5.png)
...
* CTE
	...
	* "product\_threshold" is  set to 250
* Query
	* The CTE is called in the FROM statement, then the "product\_threshold" can be used in the WHERE expression
...
* CTE
	...
	* list of values is unnested into the variable, "product\_threshold"
...
* Value is calculated in the SELECT statement and stored as "product\_average"
...
* CTE
	* Product names with "Google" are stored in "product\_threshold"
...
* CTE
	* variable is the name of the CTE where "first\_order" and "last\_order" are stored
* Query
...
* Procedural Language
	...
	* Declare/Set
		...
		* Example: Basic![](./_resources/Google,_BigQuery.resources/image.10.png)
		* Example: SET within IF/THEN conditional![](./_resources/Google,_BigQuery.resources/image.11.png)
			* Checks if a table had the latest data before running the remaining SQL
			* Procedure
				* checks the row count of the prod\_data table where the daily\_date field is equal to 2022–11–18 and sets that value to the rowcnt variable
				* using IF-THEN conditional statements
					...
					* Else the latest\_date is set to the value of the max date in the prod\_data table and DATA DELAYED is displayed along with the value of latest\_date.
			* Result: data wasn’t found and the latest\_date field shows 2022–11–15.
	* Loop/Leave
		* Example: Loops until a condition is met before running your SQL statements![](./_resources/Google,_BigQuery.resources/image.12.png)
			* Continues from 2nd Declare/Set example
			* Procedure
				* A counter variable is added with default = -1
				* Subtract days from 2022–11–18 using the date\_sub function by the counter variable until the rowcnt variable equals 1.
...
* Use Cases
	* Invoke a model on BigQuery data and create a new table with enriched data. This also works for pre-built Google models like Google Translate and Vertex Entity Extraction
		...
		* if your ML model is in TensorFlow, I recommend that you _[directly load it as a BigQuery ML](https://cloud.google.com/bigquery-ml/docs/reference/standard-sql/bigqueryml-syntax-create-tensorflow)_ model. That approach is more efficient than Remote Functions.
...
* Where realtime\_price is a remote function
...
* Experiment
* ![](./_resources/Google,_BigQuery.resources/0-MbD4H5Gvm6j6oSVr.png)
* ![](./_resources/Google,_BigQuery.resources/0-dsIFjERRMCpXxFCZ.png)
...
* ![](./_resources/Google,_BigQuery.resources/1-2JtIjC0SmJHfak7CDh5gGg.png)
...
* Misc
	* Also see [DB, Engineering](DB, Engineering) >> Cost Optimization and [SQL](SQL) >> Best Practices
...
* `LIMIT` speeds up performance, but doesn’t reduce costs.
	...
	* Table Preview![](./_resources/Google,_BigQuery.resources/1-GUq9WkPsPxiJVsKaNDkHoA.png)
...
* Utilize PARTITIONS and/or CLUSTERS to significantly reduce amount of data that's scanned
	* Misc
		* Also see
			* [DB, Engineering](DB, Engineering) >> Cost Optimization >> Partitions and Indexes  for CLUSTER
			* [SQL](SQL) >> Partitions
		...
		* Info about partititoning and cluster located in Details tab of your table![](./_resources/Google,_BigQuery.resources/image.png)
...
* [Example]{.ribbon-highlight}: Partition by date
...
* "creation\_date" is truncated to a month which reduces the number of partitions needed for this table
	* Days would exceed the 4000 partition limit

* Partition Options
	* partition\_expiration\_days: BigQuery deletes the data in a partition when it expires. This means that data in partitions older than the number of days specified here will be deleted.
	* require\_partition\_filter: Users can’t query without filtering (WHERE clause) on your partition key.
...
* Caching
	...
	* Not all queries will be cached. Exceptions include:
			A query is not cached when it uses non-deterministic functions, such as CURRENT\_TIMESTAMP(), because it will return a different value depending on when the query is executed.
			
		* When the table referenced by the query received streaming inserts because any changes to the table will invalidate the cached results.
...
* Output![](./_resources/Google,_BigQuery.resources/image.9.png)
