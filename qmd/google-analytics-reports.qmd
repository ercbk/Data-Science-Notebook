# Analytics, Reports {#sec-goog-anal-rep .unnumbered}

## Misc {#sec-goog-anal-rep-misc .unnumbered}
...
-   In GA4, Google has deprioritized the concept of a session and tracking sessions in favor of user and event-based metrics
...
-   **Conversions** are now called **Key Events**
    ...
    -   A **key event** is an event that you mark as important to your business. Key events appear in Analytics reports but aren’t directly eligible for reporting or bidding in Google Ads.
        -   e.g. you could mark an important event like a purchase or newsletter subscription as a key event
...
-   Google Analytics data in BigQuery (info for GA360 not GA4)\
    ![](./_resources/Google,_Analytics,_Reports.resources/1-oh_x-JcDMYPO9N6ceWQGJA.png){.lightbox width="532"}
    ...
    -   [Example]{.ribbon-highlight}: Query for session level features\
        ![](./_resources/Google,_Analytics,_Reports.resources/1-dJSwEoqC5Ktjz3oMlD1ppA.png){.lightbox width="532"}
    -   [Example]{.ribbon-highlight}: Unnested query for hit level features\
        ![](./_resources/Google,_Analytics,_Reports.resources/1-qxDxW-QhPKh88000gsGSmQ.png){.lightbox width="532"}

## Terms {#sec-goog-anal-rep-term .unnumbered}

-   [**Cohort**]{style="color: #009499"} - Group of users who share a common characteristic identified by an Analytics dimension
-   [**Dimension**]{style="color: #009499"} - Categorical Value; Category to group_by in the calculation of a metric
    -   e.g. sessions per country where country is the dimension
-   [**Engagement**]{style="color: #009499"}
...
-   [**First User**]{style="color: #009499"} - Naming convention for acquisition attributes of a user (aka a first time user). There is no "first user" dimension. It's a prefix that describes dimensions that measure how the website first acquires a user. These values don't change when a user *returns* to your app or website.
-   [**Interaction**]{style="color: #009499"} - Any touch point with an element on a website
-   [**Medium**]{style="color: #009499"} - The general category of the source
    -   [Example]{.ribbon-highlight}s: organic search (*organic, i.e. unpaid search*), cost-per-click paid search (*cpc, i.e. paid search*), web referral (*referral*), \<email campaign\> (custom medium), none (direct traffic).
        -   PPC: Pay-Per-Click
        -   Referral: Any traffic coming from another website that's NOT Pay-Per-Click (PPC)/Cost-per-Thousand (CPM) (i.e. ads) or a search engine
            -   e.g. A link in an article or video description on Facebook, TripAdvisor, etc.
-   [**Metric**]{style="color: #009499"} - numeric value; usually a count or calculated value like a rate or average that's aggregated per dimension value
    -   e.g. The average conversions per device where average conversion is the metric
-   [**New Users**]{style="color: #009499"} - Users on your site or app for the first time
    -   Anyone who doesn't have both a Google Analytics cookie and Client ID from your site or App Instance ID from your app
-   [**(not set)**]{style="color: #009499"} ([docs](https://support.google.com/analytics/answer/2820717?hl=en#zippy=%2Cin-this-article)) - Google Analytics will store a default (not set) value if the data isn’t accessible, isn’t being tracked properly, or for other reasons.
-   [**Segments**]{style="color: #009499"} - Website variables that you can filter the data in order to perform calculations on various user subgroups
    ...
    -   Scopes\
        ![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(574).png){.lightbox width="332"}
        -   A user can be involved in multiple sessions and a session can contain multiple hits
        -   [**Users**]{style="color: #009499"} - People interact with your property (e.g., your website or app)
        ...
        -   [**Sessions**]{style="color: #009499"} - Interactions by a single user are grouped into sessions.
        ...
        -   [**Hits**]{style="color: #009499"} - Interactions during a session are referred to as hits. Hits include interactions like pageviews, events, and transactions.
            -   Behavior confined to a single action
            -   e.g. viewing a page or starting a video.
-   [**Sequenced Segment**]{style="color: #009499"} - Sequence-based Segment
...
-   [**Session**]{style="color: #009499"} - A single period of time a user is active on your site or app
...
-   [**Source**]{style="color: #009499"} - The origin of your traffic, such as a search engine (for example, google) or a domain (example.com)
-   [**Users**]{style="color: #009499"} - *Returning users* who have been processed by Analytics. Sometimes it's Active Users.
...
## Segments, Dimensions, and Metrics {#sec-goog-anal-rep-segdimmet .unnumbered}
...
-   Segments:
    -   Misc
        ...
        -   A maximum date range of 93 days to your reports
...
-   Metrics:
    -   **Active Users** - The number of unique users who performed sessions on your website within the exploration time frame
    ...
    -   **Average Click Rate** - The average number of times an element is interacted with, within a session
        ...
        -   Avg CR = Total Events / Unique Dimension Combinations (see [Google, Analytics, Analysis \>\> Examples](google-analytics-analysis.qmd#sec-goog-anal-anal-ex){style="color: green"} \>\> Example 6)
    -   **Average Engagement Time** - The sum of user engagement durations divided by the number of active users
    ...
    -   **Interaction Rate** - The percentage of people that interact (click, hover, tap) with an element.
        ...
        -   IR = (Unique Dimension Combinations \* 100) / Sessions (see [Google, Analytics, Analysis \>\> Examples](google-analytics-analysis.qmd#sec-goog-anal-anal-ex){style="color: green"} \>\> Example 6)
    ...
    -   **Total Events** - The total number of events with the same event category, event action, and event label across all sessions
    -   **Total Users** - Represents the number of unique users who viewed a screen or triggered an event in the exploration time frame
        -   If a user repeats an interaction with the same node, then they only count as 1 user at that node.
            -   [Example]{.ribbon-highlight}: When a user opens the home page, navigates to a product page, and then returns to the home page before navigating to another product page within the time frame selected, the path will show one home page user on the starting point and one product page user for each product in the first step
    -   **Unique Dimension Combinations** **(UDC)** - The unique number of sessions that have interacted with a combination of events.
        -   Differs from unique events in that the number depends on which dimensions the view is grouped on. (See [Google, Analytics, Analysis \>\> Examples](google-analytics-analysis.qmd#sec-goog-anal-anal-ex){style="color: green"} \>\> Example 6)
    ...
    -   **Unique Events** - The total number of *nonrepeated* events with the same event category, event action, and event label across all sessions
    -   **Unique User Scrolls** - The number of unique users who scrolled down at least 90% of the page
    -   **User Engagement** - The sum of user engagement durations divided by the number of triggered events
...
## Reports {#sec-goog-anal-rep-rep .unnumbered}

### Misc {#sec-goog-anal-rep-rep-misc .unnumbered}
...
### Reports Snapshot {#sec-goog-anal-rep-rep-repsnap .unnumbered}
...
### Realtime {#sec-goog-anal-rep-rep-realt .unnumbered}

![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(564).png){.lightbox width="532"}
...
### Lifecycle {#sec-goog-anal-rep-rep-lifcy .unnumbered}
...
#### Acquisition {#sec-goog-anal-rep-rep-acq .unnumbered}
...
-   [Misc]{.underline}
    ...
    -   Both User and Traffic acquisition tabs are similar except by the dimension the metrics are grouped by.
        -   User $\rightarrow$ Metrics about first time users
        -   Traffic $\rightarrow$ Metrics about all(?) users
...
-   [User Acquisition Tab]{.underline}
    ...
    -   Table
        ...
        -   Metrics:
            ...
            -   Conversions
            -   Total Revenue
-   [Traffic Acquisitions Tab]{.underline}
...
#### Engagement {#sec-goog-anal-rep-rep-engag .unnumbered}
...
-   [Overview]{.underline}
    ...
    -   Cards
        -   Multi Time Series
            -   The avg engagement time: sum of user engagement durations divided by the number of active users
            -   The engaged sessions per user: engaged sessions / users
            -   The avg engagement time per session: sum of user engagement durations divided by the number of sessions
        -   Users in last 30 min
            -   The top pages & screens
        ...
        -   User stickiness
            ...
            -   Ratios
                -   Daily Active Users (DAU) / Monthly Active Users (MAU)
                    -   Also see [Google, Analytics, Analysis \>\> Examples](google-analytics-analysis.qmd#sec-goog-anal-anal-ex){style="color: green"} \>\> Examples 9, 15 for different calculations of DAU
...
#### Monetization {#sec-goog-anal-rep-rep-monet .unnumbered}
...
-   [Retention]{.underline}
    ...
    -   Time Series
        ...
        -   User retention by cohort
            -   The percentage of new users who return on their second and eighth day
            -   [Example]{.ribbon-highlight}: 100 users visit your site on September 9. Ten of the users return on September 10 and two return on September 16. The Day 1 line shows 10% on September 10, while Day 7 shows 2% on September 16.
        ...
        -   User engagement by cohort
            -   The average engagement time of new users who return to your site or app on their second and eighth day.
...
### User {#sec-goog-anal-rep-rep-user .unnumbered}

-   [Demographics]{.underline}: Categorizes your users by age, location, interests, and more
...
-   [Tech]{.underline}: user adoption of app releases and the technologies used to engage your content
...
## Explore {#sec-goog-anal-rep-expl .unnumbered}

### Misc {#sec-goog-anal-rep-expl-misc .unnumbered}
...
### Templates {#sec-goog-anal-rep-expl-temp .unnumbered}
...
### Free Form {#sec-goog-anal-rep-expl-freef .unnumbered}

![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(585).png){.lightbox width="532"}
...
### Funnel {#sec-goog-anal-rep-expl-funn .unnumbered}

-   Misc
    -   [Docs](https://support.google.com/analytics/answer/9327974?hl=en&ref_topic=9266525)
-   Visuals\
    ![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(579).png){.lightbox width="532"}
...
-   Edit Funnel Steps
    -   In the Tab Settings panel \>\> Steps, click the pencil icon to edit\
        ![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(583).png){.lightbox width="532"}
...
### Path {#sec-goog-anal-rep-expl-path .unnumbered}
...
-   Alluvial Chart\
    ![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(577).png){.lightbox width="382"}
...
-   Backwards-Path Analysis\
    ![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(578).png){.lightbox width="432"}
    -   Allow you to work backwards from an event
        -   [Example]{.ribbon-highlight}: Tracing user steps backwards from "purchase" or "non-purchasers"
...
### Segment Overlap {#sec-goog-anal-rep-expl-segov .unnumbered}

![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(590).png){.lightbox width="532"}
...
-   Options
    ...
    -   Various types of Segments available\
        ![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(592).png){.lightbox group="ex-seg-opt" width="682"}\
        ![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(597).png){.lightbox group="ex-seg-opt" width="682"}\
        ![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(595).png){.lightbox group="ex-seg-opt" width="682"}

### Cohort {#sec-goog-anal-rep-expl-cohort .unnumbered}

![](./_resources/Google,_Analytics,_Reports.resources/Screenshot%20(589).png){.lightbox width="532"}
...
-   Explore the behavior of groups of users over time
    -   Also see [Google, Analytics, Analysis \>\> Examples](google-analytics-analysis.qmd#sec-goog-anal-anal-ex){style="color: green"} \>\> Example 8
...
-   Options
    ...
    -   Calculation
        -   Standard: Each cell includes all cohort users who meet the return criteria for that individual period, regardless of what they do in other periods. The metric displays the total value for that individual period.
    ...
    -   Metric Type
        ...
        -   Per User: percent
    -   Breakdown, Values
        -   See other templates for definitions
        -   When you include a breakdown dimension, users are only attributed to the first instance of the breakdown value that applies to them.
            -   [Example]{.ribbon-highlight}: *User A* first appears as a mobile user, then returns the same day as a desktop user. *User A* only appears in the mobile breakdown for that cohort.
-   Misc
    -   Demographic dimensions are subject to [thresholding](https://support.google.com/analytics/answer/9383630). If the number of users in the cohort is too small to protect their anonymity, those users won't be included in the exploration

### User Lifetime {#sec-goog-anal-rep-expl-uslif .unnumbered}

-   [Docs](https://support.google.com/analytics/answer/9947257?hl=en&ref_topic=9266525)
-   Table
-   Example Questions
...
-   Provides information about these users' entire lifetime, including data from before the start of the specified range.
    -   \*\* Some or all metrics may require enabling the [User-ID feature](https://support.google.com/analytics/answer/9213390) that tracks a unique user id generated by your website or app though log-in, device id, etc.
-   \*\* Available information for each user (available in the Metrics dropdown) Initial interactions: data associated with the first time the user was measured for a property.
    -   [Example]{.ribbon-highlight}: their first visit or purchase date, or the campaign by which they were acquired as a user.
    -   Most recent interactions: data associated with the last time the user was measured for a property.
        -   [Example]{.ribbon-highlight}: Their last activity or purchase date.
    -   Lifetime interactions: data aggregated over the lifetime of the user.
        -   [Example]{.ribbon-highlight}: Their lifetime revenue or engagement
    -   Predictive metrics: data generated through machine learning to predict user behavior:
        -   Purchase probability
        -   In app purchase probability
        -   Churn probability
    -   [Example]{.ribbon-highlight} (User-ID): Suppose a signed in user had multiple visits to your app last year, and conducted multiple transactions worth a total of \$1,000. Suppose that this same user also had 4 transactions conducted as a guest (not signed in), each worth \$50, and that each transaction was made on a different device. Finally, suppose this user visited your app while signed in at least once in the date range selected by your query.
        -   How this user appears in the user lifetime exploration depends on the reporting identity method used:
            -   With User-ID, Google signals, then device: this user is counted once, with revenue of \$1000, as only her signed in data is applicable to the exploration. Her average lifetime revenue would be \$1,000.
