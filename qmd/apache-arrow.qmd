...
- Packages
  ...
  - [{]{style="color: #990000"}[db2pq](https://cran.r-project.org/package=db2pq){style="color: #990000"}[}]{style="color: #990000"} - Tools for exporting 'PostgreSQL' tables to 'Parquet' files, with support for chunked writes, column type overrides, and timezone-aware timestamp handling.
  - [{]{style="color: goldenrod"}[geoparquet_io](https://geoparquet.io/){style="color: goldenrod"}[}]{style="color: goldenrod"} - Fast I/O and transformation tools for GeoParquet files, powered by DuckDB and PyArrow.
  ...
  - [{]{style="color: #990000"}[rcdf](https://cran.r-project.org/web/packages/rcdf/index.html){style="color: #990000"}[}]{style="color: #990000"} - A Comprehensive Toolkit for Working with Encrypted Parquet Files

- Documentation

  - [Docs](https://arrow.apache.org/docs/r/)
  - [dplyr verbs and other supported functions](https://arrow.apache.org/docs/r/reference/acero.html)
  - [libarrow function reference](https://arrow.apache.org/docs/cpp/compute.html#available-functions)
...
- Tools
  ...
  - Also see [Databases, DuckDB \>\> Misc](db-duckdb.qmd#sec-db-duckdb-misc){style="color: green"} \>\> Tools
  - [Datanomy](https://github.com/raulcd/datanomy) - A terminal-based tool for inspecting and understanding data files. It provides an interactive view of your data's structure, metadata, and internal organization.
...
- [Misc]{.underline}
  ...
  - Check size of a directory of partitioned parquet files

    ``` r
    fs::dir_info("data-parquet", recurse = TRUE) |>
  ...
  - You can't replace a *numeric* value with NA (For character vectors, it's fine). Arrow thinks you're mixing variable types. So you have to use NaN (`is.nan` and `is.na` are supported by arrow)

    ``` r
    ds |> 
      mutate(value = ifelse(value < 0, NA, value)) |> # or w/if_else
      collect()
    #> Error in `compute.arrow_dplyr_query()`:
    #> ! NotImplemented: Function 'if_else' has no kernel matching input types (bool, bool, decimal128(38, 18))

    # this works
    ds |> 
      mutate(value = ifelse(value < 0, NaN, value)) |> 
      collect()
    ```
...
- [Example 1]{.ribbon-highlight}: Foursquare Sample (2025 Mar, Windows 10)
...
- [Example 2]{.ribbon-highlight}: Convert large csv to parquet
...
- [Example 3]{.ribbon-highlight}: Lazily download subsetted dataset from S3 and locally convert to parquet with partitions
...
- [Example 4]{.ribbon-highlight}: Explore a dataset with minimal memory usage
  ...
  ds_no2_raw <- open_dataset("../../Data/no2/E1a")

  schema(ds_no2_raw)
  #> Start: timestamp[ms, tz=UTC] not null
  #> End: timestamp[ms, tz=UTC] not null
  #> ResultTime: timestamp[ms, tz=UTC] not null
  ...
  ``` r
  ebtools::skim_arrow(ds_no2_raw)
  #> ── Data Summary ──
  #> 
  #> # A tibble: 1 × 5
  #>      n_rows n_cols n_numeric n_character n_timestamp
  #>       <int>  <int>     <int>       <int>       <int>
  #>   1 3608708     12         5           4           3
  #> 
  #> ── Numeric Variables ──
  #> 
  #> # A tibble: 5 × 6
  #> variable       missing_pct   mean     sd   min   max
  #> <chr>                <dbl>  <dbl>  <dbl> <dbl> <dbl>
  #> 1 Pollutant              0  8       0        8    8 
  #> 2 Value                  0 -9.57  182.    -999  326.
  #> 3 Validity               0  0.871   3.57   -99    2 
  #> 4 Verification           0  1       0        1    1 
  #> 5 DataCapture          100 NA      NA       NA   NA 
  #> 
  #> ── Character Variables ──
  #> 
  #> # A tibble: 4 × 3
  #> variable           missing_pct n_unique
  #> <chr>                    <dbl>    <dbl>
  #> 1 Samplingpoint              0      412
  #> 2 Unit                       0        1
  #> 3 AggType                    0        1
  #> 4 FkObservationLog           0      449
  #> 
  #> ── Timestamp Variables ──
  #> 
  #> # A tibble: 3 × 4
  #> variable     missing_pct min                 max                
  #> <chr>              <dbl> <dttm>              <dttm>             
  #> 1 Start                0 2017-01-01 00:00:00 2017-12-31 22:00:00
  #> 2 End                  0 2017-01-01 01:00:00 2017-12-31 23:00:00
  #> 3 ResultTime           0 2018-08-24 09:27:05 2018-09-14 10:36:47
  ```
  ...
  ds_no2_raw |> 
    ...
    collect()
  #> # A tibble: 10 × 12
  #>    Start               End                 ResultTime          Samplingpoint                    Pollutant Value Unit  
  #>    <dttm>              <dttm>              <dttm>              <chr>                                <int> <dbl> <chr> 
  #>  1 2017-01-01 00:00:00 2017-01-01 01:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  42.2 ug.m-3
  #>  2 2017-01-01 01:00:00 2017-01-01 02:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  38.0 ug.m-3
  #>  3 2017-01-01 02:00:00 2017-01-01 03:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  31.2 ug.m-3
  #>  4 2017-01-01 03:00:00 2017-01-01 04:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  28.7 ug.m-3
  #>  5 2017-01-01 04:00:00 2017-01-01 05:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  28.4 ug.m-3
  #>  6 2017-01-01 05:00:00 2017-01-01 06:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  29.3 ug.m-3
  #>  7 2017-01-01 06:00:00 2017-01-01 07:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  34.2 ug.m-3
  #>  8 2017-01-01 07:00:00 2017-01-01 08:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  35.7 ug.m-3
  #>  9 2017-01-01 08:00:00 2017-01-01 09:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  35.1 ug.m-3
  #> 10 2017-01-01 09:00:00 2017-01-01 10:00:00 2018-08-24 09:27:05 DE/SPO.DE_DEBB007_NO2_dataGroup1         8  31.0 ug.m-3
  #>    AggType Validity Verification DataCapture FkObservationLog                    
  #>    <chr>      <int>        <int>       <dbl> <chr>                               
  #> 1  hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  #> 2  hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  #> 3  hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  #> 4  hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  #> 5  hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  #> 6  hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  #> 7  hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  #> 8  hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  #> 9  hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  #> 10 hour           1            1          NA 6e344c0b-79df-4c75-a72f-762ea0e10ed7
  ```
  ...
  ![](_resources/Apache,_Arrow.resources/ops-ex4-hist-1.png){.lightbox width="332"}
  ...
  ds_no2_clean |> 
    ...
    collect() |> 
    ggplot(aes(x = Value)) +
    geom_histogram(fill = notebook_colors[[2]]) +
    theme_notebook()    
  ```

  - I'm using [ds_no2_clean]{.var-text} instead of [ds_no2_raw]{.var-text}. See for details on the cleaning
  - The x-axis is fine. As seen in the `skim_arrow` results, the max value is 326
  ...
  - All 412 [locations]{.var-text} have the same number of observations
  - [location]{.var-text} was formerly [Samplingpoint]{.var-text} but I extracted the main location code part of the string.
  :::
...
## Geospatial {#sec-apache-arrow-geo .unnumbered}

- Also see [Databases, DuckDB \>\> Geospatial](db-duckdb.qmd#sec-db-duckdb-geo){style="color: green"}

- Packages

  - [{]{style="color: #990000"}[geoarrow](https://geoarrow.org/geoarrow-r/){style="color: #990000"}[}]{style="color: #990000"} - Leverages the features of the arrow package and larger Apache Arrow ecosystem for geospatial data
  - [{]{style="color: #990000"}[geoarrowrs](https://github.com/JosiahParry/geoarrowrs/){style="color: #990000"}[}]{style="color: #990000"} - Fast and efficient geospatial operations on GeoArrow arrays. Bindings to the geoarrow-rs, pure-Rust implementation of GeoArrow with fast “geospatial algorithms.
    - [Benchmark](https://bsky.app/profile/josiah.rs/post/3mvbpitd56c2d): Faster than [{duckspatial}]{style="color: #990000"} and competitive with [{apache.sedona}]{style="color: #990000"} in most instances.
  - [{]{style="color: #990000"}[lazysf](https://hypertidy.github.io/lazysf/){style="color: #990000"}[}]{style="color: #990000"} ([README](https://github.com/hypertidy/lazysf) has better documentation)- Provides a [{dplyr}]{style="color: #990000"} backend for any vector data source (aka shapefiles) readable by GDAL (e.g. gpkg, shp, geojson).
    - Uses `gdalraster::GDALVector` to talk to GDAL and [{dbplyr}]{style="color: #990000"} for SQL translation, giving you lazy evaluation of spatial data through familiar [{dplyr}]{style="color: #990000"} verbs.
    - For larger datasets, enable GDAL’s columnar Arrow C stream interface through nanoarrow for faster data transfer ([link](https://github.com/hypertidy/lazysf/tree/main?tab=readme-ov-file#arrow-stream-interface))

- Resources

  - [Example data files in the GeoArrow repository](https://geoarrow.org/data)

- Types

  - GEOMETRY type represents planar spatial objects. This includes points, linestrings, polygons, and multi geometries. The logical type indicates that the column contains spatial objects, while the physical storage uses a standard binary encoding.
...
- [Example]{.ribbon-highlight}: ([source](https://parquet.apache.org/blog/2026/02/13/native-geospatial-types-in-apache-parquet/))
