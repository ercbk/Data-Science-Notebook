# Forecasting, Hierarchical/Grouped

TOC

* BizSci Approach (Learning Lab 50)
* Global Modeling
...
* **Hierarchical**
	...
	* Feature engineering
		...
		* No real explicit sequencing variable like a date column used
			* row\_id and date were designated as ids in recipe step
				* e.g. update\_role(row\_id, date, new\_roll = "id")
	* date features were added
		* rolling means
		* months, days of week, etc
* **Grouped (local modeling)**
	* A group variable is used to group\_nest the datasets and modeling is done on each group's dataset.
	* Data preparation
...
3. Create train/test splits of the nested (i.e. grouped) structure
	* columns: 
		* id                         .actual\_data       .future\_data,                                      .splits
			grouping var     all data                  NAs to filled with forecasts          train/test splits
...
* Train, Test, and Forecast![](./_resources/Forecasting,_Hierarchical_Grouped.resources/nested-core-functions.jpg)
	* Train and test
...
* filter\_test\_forecasts: If TRUE, says only keep the best test forecasts for each group logged
...
* Access Forecasts and plot them![](./_resources/Forecasting,_Hierarchical_Grouped.resources/unnamed-chunk-18-1.png)
...
From paper, [Principles and Algorithms for Forecasting Groups of Time Series:Locality and Globality](https://www.monash.edu/business/ebs/research/publications/ebs/wp45-2020.pdf)
More detailed (raw) notes, [Principles and Algorithms for Forecasting Groups of Time Series: Locality and Globality](Principles and Algorithms for Forecasting Groups of Time Series_ Locality and Globality)
...
* Multivariate Time Series (see section in [Forecasting, Statistical](Forecasting, Statistical))
