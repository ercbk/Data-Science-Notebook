...
-   [Misc]{.underline}
    ...
    -   Also see
        ...
        -   [Model Building, sklearn \>\> Preprocessing](model-building-sklearn.qmd#sec-modbld-sklearn-preproc){style="color: green"} \>\> Class Imbalance
        -   [Surveys, Analysis \>\> Modeling](surveys-analysis.qmd#sec-surveys-anal-modeling){style="color: green"} \>\> Tidymodels \>\> Importance weights
    -   Packages
        -   [{]{style="color: #990000"}[themis](https://themis.tidymodels.org/){style="color: #990000"}[}]{style="color: #990000"} - Extra steps for the [{recipes}]{style="color: #990000"} for dealing with unbalanced data.
        -   [{]{style="color: #990000"}[ebmc](https://cran.r-project.org/web/packages/ebmc/index.html){style="color: #990000"}[}]{style="color: #990000"} - Four ensemble-based methods (SMOTEBoost, RUSBoost, UnderBagging, and SMOTEBagging) for class imbalance problem are implemented for binary classification.
        -   [{]{style="color: #990000"}[imbalance](https://ncordon.github.io/imbalance/){style="color: #990000"}[}]{style="color: #990000"} - Novel oversampling algorithms, filtering of instances and evaluation of synthetic instances. Methods: MWMOTE (Majority Weighted Minority Oversampling Technique), RACOG (Rapidly Converging Gibbs), wRACOG (wrapper-based RACOG), RWO (Random Walk Oversampling), and PDFOS (Probability Distribution density Function estimation based Oversampling).
        -   [{]{style="color: goldenrod"}[imbalanced-learn](https://imbalanced-learn.org/stable/){style="color: goldenrod"}[}]{style="color: goldenrod"} - Tools when dealing with [{sklearn}]{style="color: goldenrod"} classification with imbalanced classes
        -   [{]{style="color: #990000"}[randomForestSRC](https://www.randomforestsrc.org/){style="color: #990000"}[}]{style="color: #990000"} - Fast Unified Random Forests for Survival, Regression, and Classification (RF-SRC)
            -   Class Imbalanced q-classification
            -   Balanced random forests undersampling of the majority class. Performance is assesssed using the G-mean, but misclassification error can be requested.
        -   [{]{style="color: #990000"}[ROBOSRMSMOTE](https://cran.r-project.org/web/packages/ROBOSRMSMOTE/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides the ROBOSRMSMOTE (Robust Oversampling with RM-SMOTE) framework for imbalanced classification tasks
            -   Extends Mahalanobis distance-based oversampling techniques by integrating robust covariance estimators to better handle outliers and complex data distributions
    -   **Applying SMOTE without re-calibration is bad** (See van Smeden paper, [thread](https://twitter.com/MaartenvSmeden/status/1495668297630633985))
        -   Or maybe just bad in general ([StackExchange](https://datascience.stackexchange.com/questions/106461/why-smote-is-not-used-in-prize-winning-kaggle-solutions))
    -   Papers
        -   [The harm of class imbalance corrections for risk prediction models: illustration and simulation using logistic regression](https://arxiv.org/abs/2202.09101)
            -   Unless recalibration is applied, applying subsampling to correct class imbalance will lead to overpredicting the minority class (discrimination) and worse calibration when using logistic regression or ridge regression
    ...
    -   [It is perfectly ok to train a model on 1M negatives and 10K positives (i.e. plenty of events), as long as you avoid using accuracy as a metric]{.underline}
        -   1K to 10K events might be enough for the ML algorithm to learn from
    -   For deep learning
        -   Loss Functions: Focal Loss and Label Smoothing.
        -   Optimization: Sharpness-Aware Minimization (SAM)
    -   A 50/50 balance is probably not optimal. ([source](https://towardsdatascience.com/when-50-50-isnt-optimal-debunking-even-rebalancing/))
        -   One paper that tested 20 datasets found that around 43% (number of minority events / number of majority events) was optimal.
            -   Other papers have found datasets where the optimal balance was over 50%
        -   Some categories can be more informative (i.e. easier to predict than others), so fewer events are necessary
        -   Best practice is treat the balance ratio as a hyperparameter and test a range of ratios as part of tuning.
...
-   [Issues]{.underline}
    ...
    -   A potential disadvantage of upsampling is that new data is being created that could cause overfitting. Additionally, by artificially increasing the sample size standard errors will also be artificially decreased.
    -   A disadvantage of downsampling is that some data, and sometimes a lot of data, is excluded from the model.
...
-   [Downsampling/Undersampling]{.underline}
...
-   [Hybrid]{.underline}
    ...
    -   Often done by oversampling the minority class and undersampling the majority class, and as before this could be done in a random fashion.
    -   Alternative: Removing some observations from both classes that would be nearest neighbors (TOMEK links)
        -   This should improve the model’s ability to classify the observations among those that remain, as they are more clearly delineated among the features.

-   [Weighting]{.underline}

    -   Logistic Regression
        -   Weighting would essentially be equivalent to what is done with simple resampling techniques, negating the need to employ the latter.
        -   e.g. doubling the weight of an observation would be equivalent to having two copies of the observation in the dataset.
    -   Tree-based Models
        -   Weighting and resampling are not equivalent.
        -   e.g. in a random forest, resampling changes the composition of the bootstrap samples used to build the trees, while weighting changes the importance of each observation in the calculation of splits within each tree.
...
-   [Tuning parameters]{.underline}

    -   Weighting and resampling often lead to over-prediction of the minority class. This can seen by examining the predicted probabilities directly. Their mean prediction is well beyond the observed rate\
        ![](_resources/Classification.resources/imbal-pred-prob-logistic.png){.lightbox width="432"}
    ...
    -   In Python,
        -   [is_unbalance=True]{.arg-text} in [{LightGBM}]{style="color: goldenrod"}
        -   [class_weight='balanced']{.arg-text} for logistic regression in [{sklearn}]{style="color: goldenrod"}

## Calibration {#sec-class-calib .unnumbered}

-   [**Calibrated**]{style="color: #009499"}- When the predicted probabilities from a model match the observed distribution of probabilities for each class. Sometimes described as the reliability of a model's confidence in its predictions.
    -   Intuitively, if a model assigns a class with 90% probability, that class should appear 90% of the time.
...
-   An unintended consequence of applying calibration can be the worsening of calibration for models that are already well calibrated
-   If a model isn't calibrated, then the magnitude of the predicted probability cannot be interpreted as the likelihood of an event
    ...
    -   Well-Ranked models (i.e. high AUC), where predicting target categories and not interpreting probabilities (e.g. risk) is the primary goal, will benefit less from calibration. Therefore, the extra resources required for calibration may not worth the effort.
...
-   Even if a model isn't caibrated and depending on the metric, it can still be more accurate than a calibrated model.
    -   Poor calibration may make an algorithm less clinically useful than a competitor algorithm that has a lower AUC but is well calibrated ([Calibration: the Achilles heel of predictive analytics](https://bmcmedicine.biomedcentral.com/articles/10.1186/s12916-019-1466-7))
    -   Calibration doesn't affect the AUROC (does not rely on predicted probabilities) but does affect the Brier Score (does rely on predicted probabilities) ([A tutorial on calibration measurements and calibration models for clinical prediction models](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7075534/))([Harrell](https://discourse.datamethods.org/t/rms-describing-resampling-validating-and-simplifying-the-model/4790/75?u=erc_bk))
...
-   Also see
    -   [Diagnostics, Classification \>\> Calibration](diagnostics-classification.qmd#diag-class-calib){style="color: green"}
    -   [Econometrics, Propensity Score Analysis \>\> Misc](econometrics-psa.qmd#sec-econ-psa-misc){style="color: green"} \>\> Papers \>\> Calibration Strategies for Robust Causal Estimation: Theoretical and Empirical Insights on Propensity Score Based Estimators
        -   Provides insights on the calibration of RF and boosted tree models (used for propensity score creation) along with results from various CV calibration procedures. Includes code.
-   Packages
    -   [{]{style="color: #990000"}[probably](https://probably.tidymodels.org/){style="color: #990000"}[}]{style="color: #990000"} - tidymodels calibration package
        -   Calibration functions used in [{tailor}]{style="color: #990000"}. This package should be used for diagnostics of calibrated/non-calibrated probabilites
        -   Beta, Isotonic, Bootstrap Isotonic, Logistic, Multinomial, Linear (for regression models)
        -   Visualizing utilities and its evaluation metric is the average of the Brier scores (or anything in [{yardstick}]{style="color: #990000"}
    ...
    -   [{{]{style="color: goldenrod"}[venn_abers](https://github.com/ip200/venn-abers){style="color: goldenrod"}[}}]{style="color: goldenrod"} ([article](https://towardsdatascience.com/calibrating-classification-probabilities-the-right-way-da935caee18d)) - Binary and Multi-Class Venn-ABERS calibration
    -   [{]{style="color: #990000"}[rfUtilities::probability.calibration](https://github.com/jeffreyevans/rfUtilities){style="color: #990000"}[}]{style="color: #990000"} - Isotonic Regression
    -   [{]{style="color: #990000"}[platt](https://github.com/bioinf-jku/platt){style="color: #990000"}[}]{style="color: #990000"} - Platt Scaling
    -   [{]{style="color: #990000"}[MiMIR::plattCalibration](https://cran.r-project.org/web/packages/MiMIR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Genomics package with a Platt Scaling function
    -   [{]{style="color: #990000"}[betacal](https://cran.r-project.org/web/packages/betacal/index.html){style="color: #990000"}[}]{style="color: #990000"} - Beta Calibration
    -   [{]{style="color: #990000"}[calibtools](https://github.com/kenneth-lee-ch/calibtools){style="color: #990000"}[}]{style="color: #990000"} - Posterior probability calibration methods for machine learning algorithms in R. It provides calibration
        -   Methods: isotonic regression, knn-based methods, Bayesian Binning into Quantiles (BBQ), Ensemble of Linear Trend Estimation (ELiTe) in multiclass case.
        -   Utilities for calibration performance measures and reliability diagrams.
    -   [{]{style="color: #990000"}[CalibratR](https://cran.r-project.org/web//packages//CalibratR/index.html){style="color: #990000"}[}]{style="color: #990000"} - BBQ (Bayes Binning in Quantiles) model
    -   [{{]{style="color: goldenrod"}[SelfCalibratingConformal](https://github.com/Larsvanderlaan/SelfCalibratingConformal){style="color: goldenrod"}[}}]{style="color: goldenrod"} ([Paper](https://arxiv.org/abs/2402.07307)) - Combines Venn-Abers calibration and conformal prediction to deliver calibrated point predictions alongside prediction intervals with finite-sample validity conditional on these predictions.
-   Notes from:
...
-   For calibrated models, sample size affects the how well they're calibrated\
    ![](./_resources/Classification.resources/image.png){.lightbox width="486"}
...
-   Distributions of predicted probabilities\
    ![](./_resources/Classification.resources/image.1.png){.lightbox width="486"}
...
-   Deep Learning Models
    -   Notes from[Calibration in Deep Learning: A Survey of the State-of-the-Art](https://arxiv.org/abs/2308.01222)
    -   While increasing the depth and width of neural networks helps to obtain highly predictive power, it also negatively increases calibration errors. Empirical evidence has shown that this poor calibration is linked to overfitting on the negative log-likelihood (NLL).
    -   Some methods has been proposed to mitigate the impact of data noise on model calibration such as *robust loss functions* or *adversarial training*, have shown promise in improving model robustness to noisy data ([Hendrycks, Mazeika, & Dietterich, 2018](https://arxiv.org/abs/1812.04606); [Delaney, Greene, & Keane, 2021](https://arxiv.org/abs/2107.09734)). Also *Ensemble methods* have been proofed effective in identifying and mitigating the effects of noisy data on model performance ([Lakshminarayanan, Pritzel, & Blundell, 2017](https://arxiv.org/abs/1612.01474); [Malinin, Mlodozeniec, & Gales, 2019](https://arxiv.org/abs/1905.00076)).
    -   Techniques such as data augmentation ([Muller et al., 2019](https://arxiv.org/abs/1906.02629); [Zhang, Cisse, Dauphin, & Lopez-Paz, 2018](https://arxiv.org/abs/1710.09412)), bias mitigation algorithms ([Kamiran & Calders, 2012](https://link.springer.com/article/10.1007/s10115-011-0463-8)), and fairness-aware model training ([Zemel, Wu, Swersky, Pitassi, & Dwork, 2013](https://paperswithcode.com/paper/learning-fair-representations)) can help mitigate biases during the model development stage.
...
-   Process
    ...
    -   For each algorithm, train the algorithm on the *Training* set, and create the calibration model using it and the *Calibration* set.
        -   Each algorithm will have it's own calibration model
...
![](./_resources/Classification.resources/image.2.png){.lightbox width="352"}
...
-   **Platt Scaling**
    -   Misc
        ...
        -   **This method, in its standard form, may not be appropriate after undersampling** ([Using Platt's scaling for calibration after undersampling -- limitations and how to address them](https://arxiv.org/abs/2410.18144))
            -   Performs a comparison between traditional Platt’s scaling, Platt’s scaling with the logit transformation, Platt’s scaling with a logistic GAM, Platt’s scaling with both the logit transformation and a logistic GAM, analytical calibration and isotonic regression.
            -   Conditions
                -   If Platt's scaling would have been able to successfully calibrate the base model had it been trained on the entire dataset (i.e., without undersampling), then Platt's scaling might be appropriate for calibration after undersampling.
                -   If this is not the case, a modified version of Platt's scaling that fits a logistic GAM to the logit of the base model's predictions is recommended.
    ...
    -   [Example]{.ribbon-highlight}: SVM in R
        ...
        ``` r
        svm_platt_recal <- 
          svm_platt_recal_func(model_fit, 
                               calib_dat, 
                               test_preds, 
                               test_dat)

        svm_platt_recal_func <- 
          function(model_fit, calib_dat, test_preds, test_dat){
            ...
            cal_preds <- 
              as.data.frame(attr(cal_preds_obj, 
                            'probabilities')[, 2])
            # Create calibration model
            cal_obs_preds_df <- 
              data.frame(y = calib_dat$outcome_var, yhat = cal_preds[, 1])
            calib_model <- 
              glm(y ~ yhat, 
                  data = cal_obs_preds_df, 
                  family = binomial)
...
-   **Isotonic Regression**
    ...
    -   [Example]{.ribbon-highlight}: AdaBoost in Py using CV calibrator
        ...
        -   Calibration model is built using the test fold (aka validation fold)\
            ![](./_resources/Classification.resources/image.3.png){.lightbox width="386"}
        -   `ensemble=True`
...
-   Other forms
    -   For logistic regression models, adjustment using the Calibration Intercept (and Calibration Slope)
        ...
        -   See supplemental material in [Calibration: the Achilles heel of predictive analytics](https://bmcmedicine.biomedcentral.com/articles/10.1186/s12916-019-1466-7#MOESM1)
        -   Procedure
...
-   Misc
    ...
    -   Also see
        -   My notebook and bookmarks for proper algorithms (e.g. permutation importance) to specify for variable importance plots
        -   [Diagnostics, Model Agnostic](diagnostics-model-agnostic.qmd#sec-diag-modagn){style="color: green"} for various feature importance computations
    -   Packages
        -   [{]{style="color: #990000"}[diversityForest](https://cran.r-project.org/web/packages/diversityForest/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2409.08925)) - Calculates multi-class VIM which is tailored for identifying exclusively class-associated covariates. It measures the discriminatory ability of the splits performed in the respective covariates with regard to this split criterion
            -   Fits multi forests (MuFs) that use both multi-way and binary splitting. The multi-way splits generate child nodes for each class, using a split criterion that evaluates how well these nodes represent their respective classes.
                -   MuFs are primarily useful for the VIM calculations as they often have slightly lower predictive performance compared to conventional RFs
            -   Also includes a discriminatory VIM, based on the binary splits, that assesses the strength of the general influence of the covariates, irrespective of their class-associatedness.
        -   [{]{style="color: #990000"}[lvimp](https://bdwilliamson.github.io/lvimp/){style="color: #990000"}[}]{style="color: #990000"} - Variable importance of variables for predicting the response at each time point and the importance summarized over the time series. (longitudinal setting)
        -   [{]{style="color: #990000"}[rfPermute](https://cran.r-project.org/web/packages/rfPermute/index.html){style="color: #990000"}[}]{style="color: #990000"} - Estimate Permutation p-Values for Random Forest Importance Metrics
        -   [{]{style="color: #990000"}[vip](https://koalaverse.github.io/vip/){style="color: #990000"}[}]{style="color: #990000"} - Part of IML suite of packages. See [Diagnostics, Model Agnostic, \>\> IML](diagnostics-model-agnostic.qmd#sec-diag-modagn-iml){style="color: green"}
        -   [{]{style="color: #990000"}[vimp](https://bdwilliamson.github.io/vimp/){style="color: #990000"}[}]{style="color: #990000"} - Computes algorithm-agnostic estimates of population variable importance, along with asymptotically valid confidence intervals for the true importance and hypothesis tests of the null hypothesis of zero importance
            -   Types:
                -   Difference in population Classification Accuracy
                -   Difference in population AUROC
                -   Difference in population Deviance,
                -   Difference in population R^2^
            -   None of the vignettes describe the algorithm. The authors provide links to papers for more details, but the freely available ones look pretty dense.
                -   I recommend checking out this paper, [A Guide to Feature Importance Methods for Scientific Inference](https://arxiv.org/abs/2404.12862) \>\> Section 10, Statistical Inference for FI Methods \>\> WVIM including LOCO for a brief description of the [{vimp}]{style="color: #990000"}'s method. Also see other parts of the paper have details on WVIM and LOCO.
        -   [{]{style="color: #990000"}[xplainfi](https://cran.r-project.org/web/packages/xplainfi/index.html){style="color: #990000"}[}]{style="color: #990000"} - Feature Importance Methods for Global Explanations
            -   Includes permutation feature importance (PFI), conditional and relative feature importance (CFI, RFI), leave one covariate out (LOCO), and Shapley additive global importance (SAGE), as well as feature sampling mechanisms to support conditional importance methods
    -   Compute Feature Importance (FI) values on test data only. Computing on training data may lead to wrong conclusions. ([source](https://arxiv.org/abs/2404.12862))
    ...
    -   Variable Importance plots can be useful for explaining the model to clients. [**They should never be interpreted as causal**]{.underline}. Ex. Real estate: variables that are most influential in determining housing price by this model are aspects of a house that could be emphasized by Realtors to their clients.
...
-   glmnet\
    ![](./_resources/Classification.resources/Screenshot%20(336).png){.lightbox width="433"}
