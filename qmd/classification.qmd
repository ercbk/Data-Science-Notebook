...
* Also see [Regression, Logistic](Regression, Logistic)
...
* Misc
	* Also see
		* up/down-sampling in [Model Building, tidymodels](Model Building, tidymodels)
		* Importance weights in [Survey, Analysis](Survey, Analysis) \>> Modeling >> Tidymodels
	...
	* Unless recalibration is applied, applying subsampling to correct class imbalance will lead to overpredicting the minority class (discrimination) and worse calibration when using logistic regression or ridge regression ([paper](https://academic.oup.com/jamia/article/29/9/1525/6605096?login=false))
		...
		* Also see [Model Building, tidymodels](Model Building, tidymodels) >> Tune >> CV and Tune Model with Multiple Recipes for an example of how downsampling (w/o calibration) + glmnet affects class prediction and GOF metrics
	* SOTA
	...
	* Imbalanced Classification via Layered Learning (ICLL)
		...
		* A hierarchical model composed of two levels:
			* Level 1: a model is built to split easy instances from borderline ones.
				* The goal is to predict if an input instance belongs to a cluster with at least one observation from the minority class.
			* Level 2: we discard the easy cases. Then, we build a model to solve the original classification task with the remaining data.
				* The first level affects the second one by removing easy instances from the training set.
...
* It is extremely important that subsampling occurs _inside of resampling_. Otherwise, the resampling process can produce poor estimates of model performance.
...
* **Calibrated** \- When the predicted probabilities from a model match the observed distribution of probabilities for each class.
	...
	* An unintended consequence of applying calibration models can be the worsening of calibration for models that are already well calibrated
...
* Basic Workflow
	...
	* Process
		...
		* For each algorithm, train the algorithm on the _Training_ set, and create the calibration model using it and the _Calibration_ set.
		...
		* Use the _Training_ set for CV or Nested CV
		* For each split during CV or Nested CV (outer loop)
			...
			* Scores should include calibration metrics (see [Diagnostics, Classification](Diagnostics, Classification) >> Calibration >> Basic Workflow)
		* The rest is normal CV/Nested CV procedure
			* i.e. average scores across the splits then select the algorithm with the best score. Predict and score algorithm on the _Test_ set
...
* Misc
	* Also see [Diagnostics, Classification](Diagnostics, Classification) >> Calibration
	* Notes from:
	...
	* For calibrated models, sample size affects the how well they're calibrated![](./_resources/Classification.resources/image.png)
		* Even for a logistic model, N > 1000 is desired for good calibration
		* For a rf, closer to N = 10,000 is probably needed.
	* Distributions of predicted probabilities![](./_resources/Classification.resources/image.1.png)
...
* Methods![](./_resources/Classification.resources/image.2.png)
	* Misc
...
* See Istotonic Regression for "model\_fit", "calib\_dat", "test\_preds", and "test\_dat"
...
* **Isotonic Regression**
	...
	* Process
		...
		* Train classifier model on _train_ data
		* Get predictions from the classifier on the _test_ data
		* Create calibration model dataset from the _calibration_ data
			* Get predictions from the classifier on the _calibration_ data
			* Create df with observed outcome of _calibration_ data and predictions on _calibration_ data
		...
		* FIt calibration model
			...
			* Create a step function using the isotonic regression fitted values and the predictions on the _calibration_ data
...
* Calibration model is built using the test fold (aka validation fold)![](./_resources/Classification.resources/image.3.png)
* `ensemble=True`
...
* Other forms
	* For logistic regression models, adjustment using the Calibration Intercept (and Calibration Slope)
		...
		* For calculating the values, see [Diagnostics, Classification](Diagnostics, Classification) >> Calibration >> Evaluation of Calibration Levels >> Weak >> Intercept, Slope
...
* glmnet![](./_resources/Classification.resources/Screenshot (336).png)
...
* Linear Discriminant Analysis (LDA)
	...
	* Maximizing the ratio of Distance to Scatter determines the GoF of the separator![](./_resources/Classification.resources/Screenshot (1408).png)
	...
	* For multinomial outcomes, there are a couple differences:
		* A centroid between all the predictor data is chosen, and centroids within each category of the grouped predictor data are chosen. For each category, d is the distance between the group centroid and the overall centroid![](./_resources/Classification.resources/Screenshot (1411).png)
			* The chosen group predictor centroids are determined by maximizing the distance-scatter ratio
		* Using the coordinates of the chosen group predictor centroids, a plane is determined.![](./_resources/Classification.resources/Screenshot (1412).png)
	...
	* The separation can be visualized by using charting the data according to the separators![](./_resources/Classification.resources/Screenshot (1414).png)
