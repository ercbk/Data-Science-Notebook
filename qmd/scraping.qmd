...
- [{]{style="color: goldenrod"}[beautifulsoup](https://www.crummy.com/software/BeautifulSoup/bs4/doc/){style="color: goldenrod"}[}]{style="color: goldenrod"} - Like the rvest for python. Scrapes static html pages
- [{]{style="color: #990000"}[chromote](https://rstudio.github.io/chromote/){style="color: #990000"}[}]{style="color: #990000"} - Headless Chrome Web Browser Interface
...
- [{]{style="color: #990000"}[selenium](https://ashbythorpe.github.io/selenium-r/){style="color: #990000"}[}]{style="color: #990000"}, [{]{style="color: goldenrod"}[selenium](https://www.selenium.dev/documentation/){style="color: goldenrod"}[}]{style="color: goldenrod"}
- [{]{style="color: #990000"}[shadowr](https://github.com/ricilandolt/shadowr){style="color: #990000"}[}]{style="color: #990000"} - For shadow DOMs
- [{]{style="color: #990000"}[ralger](https://feddelegrand7.github.io/ralger/){style="color: #990000"}[}]{style="color: #990000"} - Contains convenient wrapper functions for extracting and tidying objects scraped from easily scraped sites (e.g. html tables, links, titles, etc.)
:::
...
- Resources
  ...
  - <https://scrapingsandbox.com/>
    - Practice website built it on Next.js with App Router, Tailwind CSS, Radix UI, ShadCN for the UI and deployed it via OpenNext on Cloudflare Workers which mimics modern e-commerce websites.
    - The header teases an API endpoint (api.agenty.com/:agent_id?limit=500) which is a reference to Agenty’s commercial scraping API, not a public endpoint on the sandbox itself.
...
- [**Event Bubbling**]{style="color: #009499"} - When an event happens on a DOM element (like a click on a button), that event "bubbles up" from the target element to its ancestors in the DOM tree — all the way up to the `<html>` element.
  - It allows for event delegation (e.g., you attach one listener to a parent element and handle clicks from all child elements).
  - e.g. if there's an event listener attached to a parent element that's listening for a "click," then once the button is clicked. that click signal travels to the parent `<div>` element where the event listener receives it and activates whatever (e.g. logging user clicks on a webpage)
...
- [Starting a Server Option 1]{.underline} (old)
...
- [Starting a Server Option 2]{.underline} ([source](https://www.r-bloggers.com/2025/09/step-by-step-guide-to-use-r-and-selenium-on-windows/))
  ...
  - Download Selenium Server
    ...
    - Version number could be important since this package is no longer maintained
...
## BeautifulSoup {#sec-scrap-bsoup .unnumbered}
...
- [Example]{.ribbon-highlight}: Scrape titles and paragraphs
  ...
  title = soup.find('title').get_text() # <4>
  ...
  1.  Send HTTP request to get web page
  2.  Check if request was successful
  3.  Parse HTML with BeautifulSoup
  4.  Extract titles and paragraphs
...
## Selenium {#sec-scrap-selpy .unnumbered}
...
- Get JS generated content ([source](https://datascienceplus.com/scraping-javascript-rendered-web-content-using-r/))

  - The goal is to pull the email address of the store which seems simple enough, but since it's generated by a js script, it's not scrapable by simple html methods.

  - Note that this website is no longer there, so the example can't be reproduced.

  - Console\
    ![](_resources/Scraping.resources/js-rendered-content-console.png){.lightbox width="532"}

    - The email link and address are below the html script tags which indicates we're probably going to have issues getting it.

  - Run JS script with [{V8}]{style="color: #990000"}
...
- Search and extract windy.com table ([source](https://www.youtube.com/watch?v=30AOpjLeOyI&ab_channel=TheCoatlessProfessor))\
  ...
  # chrome_session_windy$Runtime$evaluate('
  #     document.querySelector("#plugin-search > div:nth-child(1) > a").click();
  # ')

  chrome_session_windy$Runtime$evaluate('
    const el = document.querySelector("#plugin-search > div:nth-child(1) > a");

    if (el) {
      // First, move the mouse over the element (hover)
  ...
  - Modern front-end libraries sometimes "hijack" events — if they don’t see a full mouse interaction, they ignore the `.click()` (commented out). The JS code shows how to more closely imitate a human mouse click which the website will then respond to by clicking the button.
  - The [mouseover]{.var-text} event may not be necessary.
  - [cancelable: true]{.arg-text} says that if there is some code attached to the page that is listening for this event and calls `event.preventDefault()`, it can block the event's default behavior.
    - You don't actually want the event blocked. This is just to mimic a website's typical behavior.
    - Some real browser events are cancelable (like clicks, form submits, keypresses). If you left it out (or used `false`), the page's JavaScript wouldn’t be able to intercept and prevent the action — which might cause weird behavior if the site expects to be able to.
  ...
  - [#detail-data-table]{.var-text} is the CSS selector but he added [table]{.var-text} in front of that. [table]{.var-text} specifies the html tag under which to look for that id
  - The
  :::

- Adding delays to events

  - [Example]{.ribbon-highlight}: From windy.com example \>\> Full Mouse Click
    ...
    chrome_session_windy$Runtime$evaluate('
      ...
      if (el) {
        ...
        setTimeout(() => {
          ...
          // Step 2: Mouse Down (after hover)
          setTimeout(() => {
            const mouseDownEvent = new MouseEvent("mousedown", { bubbles: true, cancelable: true, view: window });
            el.dispatchEvent(mouseDownEvent);

            // Step 3: Mouse Up (after mouse down)
            setTimeout(() => {
              const mouseUpEvent = new MouseEvent("mouseup", { bubbles: true, cancelable: true, view: window });
              el.dispatchEvent(mouseUpEvent);

              // Step 4: Click (after mouse up)
              setTimeout(() => {
                const clickEvent = new MouseEvent("click", { bubbles: true, cancelable: true, view: window });
                el.dispatchEvent(clickEvent);
              }, 100); // 100ms after mouseup
            }, 100); // 100ms after mousedown
          }, 100); // 100ms after mouseover
        }, 100); // Initial delay
      }
    ')
    ```

    - Cascade of `setTimeout` functions isn't nice to look at. See the Async tab for cleaner code
    ...
    chrome_session_windy$Runtime$evaluate('
      ...
      if (el) {
        ...
        async function simulateClickSequence() {
          // Hover
        ...
        simulateClickSequence();
      }
    ')
    ```

    - Events aren't dispatched asynchronously. This format just allows you to use the `await` + `wait` combo and write code that's easier to read than the cascade of `setTimeout` functions
    - `wait(ms)` is a tiny helper that pauses for the number of milliseconds you give it.
    - `async`lets you `await` pauses between each event.
    :::
