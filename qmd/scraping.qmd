...
-   [**Static Web Page**]{style="color: #14adff"}: A web page (HTML page) that contains the same information for all users. Although it may be periodically updated, it does not change with each user retrieval.

-   [**Dynamic Web Page**]{style="color: #14adff"}: A web page that provides custom content for the user based on the results of a search or some other request. Also known as "dynamic HTML" or "dynamic content", the "dynamic" term is used when referring to interactive Web pages created for each user.
