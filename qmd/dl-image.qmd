...
* Image (or Data) Boosting
	Notes from [How I Won Andrew Ng’s First Data-Centric AI Competition](https://towardsdatascience.com/how-i-won-andrew-ngs-very-first-data-centric-ai-competition-e02001268bda)
...
Convolutional Neural Network (CNN)
![](./_resources/DL,_Image.resources/image.png)
...
* Images![](./_resources/DL,_Image.resources/image.1.png)
...
* Layers
	* Convolutional layers
		...
		* Components and Operations (fig: Gray scale, 1 Filter)![](./_resources/DL,_Image.resources/image.2.png)
			...
			* **Image section**
				...
				* The number of image sections depends on the _stride_ we use.
			...
			* **Image Input** is the input matrix
				* **Padding** adds additional pixels with zero values to each side of the image which helps to keep the feature map of the same size as the input![](./_resources/DL,_Image.resources/image.3.png)
			...
			* [Example]{.ribbon-highlight}: Gray Scale, Multiple Filters![](./_resources/DL,_Image.resources/image.4.png)
				* Feature map is now an array
			* [Example]{.ribbon-highlight}: RGB image![](./_resources/DL,_Image.resources/image.5.png)
			...
			* [Example]{.ribbon-highlight}: RGB, mulitple filters![](./_resources/DL,_Image.resources/image.6.png)
				* Feature map is now an array
	* Pooling layers
		...
		* Components and Operations (max pooling, stride = 2)![](./_resources/DL,_Image.resources/image.7.png)
			* The **Pooling Operation** happens between a section of the feature map and the filter. It outputs the pooled feature map
				...
				* Types
					...
					* **global-average-pooling:** similar to average-pooling but instead of using N×N patches of the feature maps, it uses all feature maps area at once![](./_resources/DL,_Image.resources/image.10.png)
			* **Filter**: empty so no transformation, it only sets the size of the _image section_ where the pooling operation takes place
			...
			* **Feature Map**: Input to the pooling layer
				* Size of the feature map _sections_ should be equal to the size of the filter we choose
			...
			* The **Flatten Operation** flattens the pooled feature map output to a single column so that it can be fed the Multiayer Perceptron (MLP) and the image can be classified
				...
				* Example: Grayscale![](./_resources/DL,_Image.resources/image.8.png)
				* Example: RGB![](./_resources/DL,_Image.resources/image.9.png)
...
* Architectures
	* AlexNet![](./_resources/DL,_Image.resources/image.11.png)
