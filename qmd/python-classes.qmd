...
-   Can be an instance or class type of method
    ...
    -   `__iter__` method - [**generator function**]{style="color: #14adff"} that creates one element at a time, yields it to the caller, and allows the caller to process it
...
-   Inheritance models what is called an "is a" relationship. This means that when you have a [**Derived (aka subclass, child) class**]{style="color: #14adff"} that *inherits* from a [**Base (aka super, parent) class**]{style="color: #14adff"}, you created a relationship where Derived is a specialized version of Base.
