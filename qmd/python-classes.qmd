...
-   Others
    ...
    -   `__repr__` - Creates a string representation of the class object
        ...
        -   You can set `__repr__ = __str__` means that `print(person)` and `person` should print the same string which is kind of like what happens in R with some objects
...
-   Django: Managing database tables ([source](https://www.pythonmorsels.com/when-are-classes-used/))
...
-   Email: Connect and download messages ([source](https://www.pythonmorsels.com/when-are-classes-used/))
    ...
    class IMAPChecker:
        ...
        def get_message_uids(self):
            ...
            _, data = self.server.uid("search", None, "ALL")
            return data[0].split()
    ...
    for u in server.get_message_uids():
        messages.append(server.get_message(u))
