...
-   Packages
    ...
    -   [{]{style="color: #990000"}[git2r](https://docs.ropensci.org/git2r/){style="color: #990000"}[}]{style="color: #990000"} - Programmatic access to Git repositories from R. Internally the package uses the [libgit2 library](http://libgit2.github.com/) which is a pure C implementation of the Git core methods.
    -   [{]{style="color: #990000"}[reprex](https://reprex.tidyverse.org/){style="color: #990000"}[}]{style="color: #990000"} - Prepare reprexes for posting to GitHub issues,StackOverflow, in Slack messages or snippets, or even to paste into PowerPoint or Keynote slides.
        ...
        -   Then load the packages and run `sessioninfo::session_info()` and copy console output. Paste the output underneath:
            ...
            -   Try pasting within a code block for better readability
    -   [{]{style="color: #990000"}[GitAI](https://cran.r-project.org/web/packages/GitAI/index.html){style="color: #990000"}[}]{style="color: #990000"} - Scan multiple 'Git' repositories, pull specified files content and process it with large language models.
        -   You can summarize the content in specific way, extract information and data, or find answers to your questions about the repositories.
        -   The output can be stored in vector database and used for semantic search or as a part of a RAG (Retrieval Augmented Generation) prompt.
    -   [{]{style="color: #990000"}[multideploy](https://r-pkg.thecoatlessprofessor.com/multideploy/){style="color: #990000"}[}]{style="color: #990000"} - Tools for deploying file changes across multiple GitHub repositories. It’s designed to help you manage standardized configurations, CI/CD workflows, and other common files that need to be synchronized across multiple repositories.
    -   [{]{style="color: #990000"}[gitcreds](https://gitcreds.r-lib.org/){style="color: #990000"}[}]{style="color: #990000"}
    ...
    -   [{]{style="color: #990000"}[IssueTrackeR](https://tanguybarthelemy.github.io/IssueTrackeR/){style="color: #990000"}[}]{style="color: #990000"} - Designed to retrieve and manage GitHub issues directly within R. This package allows users to efficiently track and handle issues from their GitHub repositorie

-   Resources

    -   [Happy Git and GitHub for the useR](https://happygitwithr.com/)
    -   [Building reproducible analytical pipelines with R, Ch.4](https://raps-with-r.dev/git.html)
    -   Docs: [About GitHub's IP addresses](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/about-githubs-ip-addresses)
        -   [List of the IP addresses for GitHub.com](https://api.github.com/meta)
        -   [CIDR to IPv4 and Vice Versa](https://www.ipaddressguide.com/cidr)

-   Tools

    -   [Git for Windows](https://gitforwindows.org/) - Git Bash, Git GUI, Shell Integration, and Git Credential Manager
...
-   Clone and Fork ([source](https://happygitwithr.com/fork-and-clone.html?q=fork#fork-and-clone-create-from-github))

    ``` r
    usethis::create_from_github(
      repo_spec = "https://github.com/joekirincic/performance-optimization-for-plumber-apis.git",
      destdir = "~/Documents/R/Code/APIs/",
      fork = TRUE
    )
    ```

    -   Work in a new branch, not main (or whatever the default branch is called)
    -   Do not make commits to main of a repo you have forked. If you commit to main in a repo you don’t own, it creates a divergence between that branch’s history in the source repo and in your repo.
    -   Arguments
        -   [repo_spec]{.arg-text} - Use the https or ssh address depending on what you're set-up for
        -   [fork]{.arg-text} = FALSE would just clone the repo
        -   [rstudio]{.arg-text} - Initiate an RStudio Project?
            -   Defaults to [TRUE]{.arg-text} if in an RStudio session and project has no pre-existing .Rproj file.
            -   Defaults to [FALSE]{.arg-text} otherwise (but note that the cloned repo may already be an RStudio Project, i.e. may already have a .Rproj file).
        -   [open]{.arg-text} - If [TRUE]{.arg-text}, activates the new project:
        ...
        -   [protocol]{.arg-text} - One of "https" or "ssh". Automatically detects through `git_protocol` if not specified
...
-   Change from SSH to HTTPS
    ...
    ``` bash
    git remote set-url origin https://repo-url.git
    git remote set-url --push origin https://repo-url.git
    ```
...
-   `git blame` ([Docs](https://git-scm.com/docs/git-blame), [Github Docs](https://docs.github.com/en/repositories/working-with-files/using-files/viewing-and-understanding-files#viewing-the-line-by-line-revision-history-for-a-file))- A command that shows which revision and author last modified each line of a file, helping you track changes and understand the history of the code. It annotates each line with information about the last modification, making it easier to identify who made specific changes.

    -   On the github code page, there's a tab in above the file view arean that says, "Blame." Click it and the file will be annotated on th left side with commit details and a link to the commit.
    ...
    -   All revisions specified in the `.git-blame-ignore-revs` file, which must be in the root directory of your repository, are hidden from the blame view using Git's `git blame --ignore-revs-file` configuration setting.
        ...
        -   This can be useful when a few commits make extensive changes to your code (e.g. when first applying a linter on a repo)
...
-   Resources
    ...
    -   [Building a Robust .gitconfig](https://www.jumpingrivers.com/blog/recommended-gitconfig/) - Guide with the basics to get started
