...
- Also see
  ...
  - [EDA \>\> Correlation](eda-general.qmd#sec-eda-gen-corr){style="color: green"} for visualization
...
- **Independence** - Two random variables are independent if the product of their individual probability density functions equals the joint probability density function
  ...
  - "Correlation is a summary measure of stochastic dependence, but absence of correlation does not necessarily indicate independence." ([{spatstat}]{style="color: #990000"} book, see [Geospatial, Point Patterns \>\> Misc](geospatial-point-patterns.qmd#sec-geo-ptpat-misc){style="color: green"} \>\> Resources)
...
- Correlation\
  ...
  - If $x$ and $y$ are (modeled as) jointly normally distributed (following a bivariate normal distribution), then their relationship is completely characterized by their correlation coefficient.

- Sample size required to well-estimate a *single* correlation coefficient is $n = 400$ ([Harrell](https://hbiostat.org/hdata/#to-be-able-to-do-feature-selection-you-must-be-able-to-estimate-feature-importance))
...
- Misc
  ...
  - Packages
    ...
    - [{]{style="color: #990000"}[BDgraph](https://cran.r-project.org/web/packages/BDgraph/){style="color: #990000"}[}]{style="color: #990000"} ([Vignette](https://www.jstatsoft.org/article/view/v089i03)) - Bayesian Structure Learning in Graphical Models
    ...
    - [{]{style="color: #990000"}[clusterGGM](https://cran.r-project.org/web/packages/clusterGGM/index.html){style="color: #990000"}[}]{style="color: #990000"} - Sparse Gaussian Graphical Modeling with Variable Clustering
    - [{]{style="color: #990000"}[corpcor](https://cran.r-project.org/web/packages/corpcor/index.html){style="color: #990000"}[}]{style="color: #990000"} - Efficient Estimation of Covariance and (Partial) Correlation
      - `pcor.shrink` - Shrinkage estimates of partial correlation. Returns the partial correlation matrix
    - [{]{style="color: #990000"}[dpcid](https://cran.r-project.org/web/packages/dpcid/index.html){style="color: #990000"}[}]{style="color: #990000"} - Differential partial correlation identification with the ridge and the fusion penalties
    - [{]{style="color: #990000"}[dormancy](https://danymukesha.github.io/dormancy/){style="color: #990000"}[}]{style="color: #990000"} - Detecting and analyzing dormant patterns. Identifies statistical patterns that exist but remain inactive until specific trigger conditions emerge
    ...
    - [{]{style="color: #990000"}[pdcor](https://cran.r-project.org/web/packages/pdcor/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fast and memory-less computation of the partial distance correlation for vectors and matrices.
      - Permutation-based and asymptotic hypothesis testing for zero partial distance correlation
    - [{]{style="color: #990000"}[ppcor](https://cran.r-project.org/web/packages/ppcor/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4681537/)) - Calculates partial and semi-partial (part) correlations along with p-value
    - [{]{style="color: #990000"}[rpc](https://cran.r-project.org/web/packages/rpc/index.html){style="color: #990000"}[}]{style="color: #990000"} - Computes the ridge partial correlation coefficients in a high or ultra-high dimensional linear regression problem

  - Papers

    - [Robust Bayesian graphical modeling using $\gamma$ - divergence](https://arxiv.org/abs/2312.07262)

  - Resources

    - [A Tutorial on Regularized Partial Correlation Networks](https://eiko-fried.com/wp-content/uploads/Epskamp-Fried-2018-Tutorial-partial-corr.pdf)
...
- [Misc]{.underline}
  ...
  - Packages
    - [{]{style="color: #990000"}[AUKtest](https://cran.r-project.org/web/packages/AUKtest/index.html){style="color: #990000"}[}]{style="color: #990000"} - Computes the Area Under the Kendall (AUK) estimator for multivariate independence
    - [{]{style="color: #990000"}[MarginalMaxTest](https://cran.r-project.org/web/packages/MarginalMaxTest/index.html){style="color: #990000"}[}]{style="color: #990000"} - Test the marginal correlation between a scalar response variable with a vector of explanatory variables using the max-type test with bootstrap
    - [{]{style="color: #990000"}[matrixCorr](https://cran.r-project.org/web/packages/matrixCorr/index.html){style="color: #990000"}[}]{style="color: #990000"} - Lightweight, high-performance (C++) package for computing correlation matrices
    ...
    - [{]{style="color: #990000"}[multiCCA](https://cran.r-project.org/web/packages/multiCCA/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements methods for multiple canonical correlation analysis (CCA) for more than two data blocks (e.g. each subject has a block (matrix) of repeated measures), with a focus on multivariate repeated measures and functional data.
      - Implements multiple kernel CCA, which embeds each data block into a reproducing kernel Hilbert space to capture nonlinear dependencies
      - Implements multiple functional CCA, which represents repeated measurements as smooth functions and performs analysis in a Hilbert space framework
    - [{]{style="color: #990000"}[QuantileOnQuantile](https://cran.r-project.org/web/packages/QuantileOnQuantile/index.html){style="color: #990000"}[}]{style="color: #990000"} - QQ regression estimates the effect that quantiles of one variable have on quantiles of another, capturing the dependence between distributions.
    - [{]{style="color: #990000"}[wbCorr](https://cran.r-project.org/web/packages/wbCorr/index.html){style="color: #990000"}[}]{style="color: #990000"} - Separates supplied variables into within- and between-cluster components and calculates bivariate correlations for each level separately. Intended for longitudinal or otherwise clustered data where researchers need transparent correlation matrices before fitting more complex multilevel models
...
- [Misc]{.underline}
  ...
  - Packages
    - [{]{style="color: #990000"}[PAsso](https://cran.r-project.org/web/packages/PAsso/index.html){style="color: #990000"}[}]{style="color: #990000"} - Assesses the Partial Association Between Ordinal Variables
  ...
  - Binary vs Binary Similarity measures ([paper](https://cedar.buffalo.edu/papers/articles/CVPRIP03_propbina.pdf))
    ...
    - Packages
      - [{]{style="color: #990000"}[FastHamming](https://cran.r-project.org/web/packages/FastHamming/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fast Computation of Pairwise Hamming Distances
...
- [Correspondence Analysis (CA)]{.underline} - An extension of the principal component analysis suited to analyse a large contingency table formed by two *qualitative variables* (or categorical data).
  ...
  - Resources
    ...
    - [STHDA CA Tutorial](https://www.sthda.com/english/articles/31-principal-component-methods-in-r-practical-guide/113-ca-correspondence-analysis-in-r-essentials/)
    - [STHDA MCA Tutorial](https://www.sthda.com/english/articles/31-principal-component-methods-in-r-practical-guide/114-mca-multiple-correspondence-analysis-in-r-essentials/)
  - Packages
...
- Misc
  - Packages
    - [{]{style="color: #990000"}[gcor](https://cran.r-project.org/web/packages/gcor/index.html){style="color: #990000"}[}]{style="color: #990000"} - Generalized correlation and related measures for assessing nonlinear relationships between variables, including mixed-type data
      - Generalized Correlation Guidelines
        - Range: $0\le r \le 1$
        - $r = 0$, then X and Y are independent
        - $r = 1$, then X and Y are mutually completely dependent
        - $r=|\rho|$, then X and Y follow the bivariate Normal distribution (where $\rho$ is the pearson correlation coefficient)
      - Generalized $\chi^2$ Correlation Guidelines
        - Includes $r$ guidelines
        - Symmetry: $r_{\chi^2}(X,Y) = r_{\chi^2}(Y, X)$
        - Invariance under transformation: $r_{\chi^2}(u(X),v(Y)) = r_{\chi^2}(X,Y)$
    - [{]{style="color: #990000"}[IVCor](https://cran.r-project.org/web/packages/IVCor/index.html){style="color: #990000"}[}]{style="color: #990000"} - A integrated variance correlation is proposed to measure the dependence between a categorical or continuous random variable and a continuous random variable or vector
      - Includes parametric and nonparametric approaches and a test of independence
    - [{]{style="color: #990000"}[manydist](https://cran.r-project.org/web/packages/manydist/index.html){style="color: #990000"}[}]{style="color: #990000"} - Unbiased Distances for Mixed-Type Data
...
- Packages
  - [{]{style="color: #990000"}[covercorr](https://cran.r-project.org/web/packages/covercorr/index.html){style="color: #990000"}[}]{style="color: #990000"} - Coverage Correlation Coefficient and Testing for Independence
    ...
    - Computes $V_n$, the total uncovered area after taking the union of these cubes. It uses $V_n$ to calculate the CCC ($\kappa_n^{X,Y}$)\
      ![](_resources/Association-General.resources/nonlin-covcorr-1.png){.lightbox}\

      $$\kappa_n^{X,Y} = \frac{V_n - e^{-1}}{1 - e^{-1}}$$
  - [{]{style="color: #990000"}[GTRT](https://cran.r-project.org/web/packages/GTRT/index.html){style="color: #990000"}[}]{style="color: #990000"} - Graph Theoretic Randomness Tests
    ...
    - $H_0$ : randomness
  - [{]{style="color: #990000"}[utest](https://cran.r-project.org/web/packages/utest/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://mpra.ub.uni-muenchen.de/4823/1/MPRA_paper_4823.pdf)) - Tests for the presence of a U shaped or inverted U shaped relationship between variables in (generalized) linear models.
...
### ξ (xi) and CODEC {#sec-assoc-gen-nonlin-zhi .unnumbered}
...
- Misc
  ...
  - Packages
    - [{]{style="color: #990000"}[XICOR](https://cran.r-project.org/web/packages/XICOR/index.html){style="color: #990000"}[}]{style="color: #990000"}
    ...
    - [{]{style="color: #990000"}[FOCI](https://cran.r-project.org/web/packages/FOCI/index.html){style="color: #990000"}[}]{style="color: #990000"} - Feature Ordering by Conditional Independence

      - `FOCI::codec` - Calculations the marginal contribution of a variable, $Z$, given a variable, $X$, in explaining a response variable, $Y$.
        - Calculates $\xi$ if only two variables are provided
  - Resources
    - Papers:

      - [A New Coefficient of Correlation](https://arxiv.org/abs/1909.10140)

      - [A simple measure of conditional dependence](https://arxiv.org/abs/1910.12327)
...
- Estimator\
  ...
  $$\begin{aligned}
  &\xi_n(Y,X) = 1-\sum_{i=1}^{n-1} \sum_{j\neq i} w_{\xi_n} \mathbb{1}\{r_j \in \mathcal{K}\} \\
  &\begin{aligned}
  \text{where}\;\; &w_{\xi_n} = 3/(n^2-1) \\
  &\mathcal{K} := [\min\{r_i, r_{i+1}\}, \max\{r_i, r_{i+1}\}]
  \end{aligned}
  \end{aligned}$$

  - $r_i$ is rank of $Y_i$, that is, the number of $j$ such that $Y_j \le Y_i$
...
- Bias correction for "roon-n cosistency" ([Paper](https://arxiv.org/abs/2508.09040), [Code](https://github.com/chenleihaomars/Simulation-for-ACbc/tree/main))

  - $\xi$ "includes a bias term that may vanish at a rate slower than root-n, preventing root-n consistency in general. In this article, we propose a bias correction approach that overcomes this limitation, yielding an NN-based estimator that is both root-n consistent and asymptotically normal."

  - The results of the simulation study shows "that as the dimension or correlation increases, the performance of \[bias-corrected version\] clearly dominates that of \[original\]. For small dimensions or correlation, the two (original and bias-corrected) correlation estimators have more comparable performance."

- [Example]{.ribbon-highlight}: Bias-Corrected $\xi$ for "root-n consistency"
  ...
  T_n_bc <- T_n_orig - 6 * Lhat
  ```

  - For [n]{.arg-text}, you just need 1 value, but I just decided to include the values that were tested in the experiment
  - [lambda]{.arg-text} is the ridge penalty parameter
  - [d]{.arg-text} has the polynomial orders that are used to make polynomial equations that is required to compute [Lhat]{.arg-text}
    - I'm guessing [d]{.arg-text} is some kind of assumption made about the shape of relationship between X and Y(???)
    - Need to read the paper more thoroughly to figure what this is and why it's needed.

  </Details>

- [Example]{.ribbon-highlight}: Biased-Corrected $\xi$ for small n
...
- Packages
  ...
  - [{]{style="color: #990000"}[dHSIC](https://cran.r-project.org/web/packages/dHSIC/index.html){style="color: #990000"}[}]{style="color: #990000"} - Independence Testing via Hilbert Schmidt Independence Criterion
...
- [Misc]{.underline}
  ...
  - Packages
    - [{]{style="color: #990000"}[dcortools](https://cran.r-project.org/web/packages/dcortools/index.html){style="color: #990000"}[}]{style="color: #990000"} - Tons of options, covariance test; Can use [{RFast}]{style="color: #990000"} for calculations
    ...
    - [{]{style="color: #990000"}[rdcor](https://cran.r-project.org/web/packages/rdcor/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/1909.10024))- Rank Distance Correlation Coefficient
...
### Integrated R^2^ Dependence Coefficient (IRDC) {#sec-assoc-gen-nonlin-intr2 .unnumbered}

- [{]{style="color: #990000"}[FORD](https://cran.r-project.org/web/packages/FORD/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2505.18146)) - Feature Ordering by Integrated R square Dependence (FORD) is a variable selection algorithm based on the new measure of dependence: Integrated R2 Dependence Coefficient (IRDC)
...
- It's of similar construction to $\xi(Y,X)$
  ...
  - They perform best (and similarly), compared to other statistics, on the same types of data patterns
    - "W", sinusoid, and heteroskedastic + sinusoid
  - $\nu$ "excels at detecting smooth, mid-rank oscillatory patterns, whereas $\xi$ is more sensitive to sharp transitions at the extremes."
...
- Has the same interpetation as R^2^
...
- Estimator (under the assumption there are no ties between $X_i$ and $Y_i$)\
  ...
  $$\begin{aligned}
  &\nu_n(Y,X) = 1-\sum_{i=1}^{n-1} \sum_{j\neq i, \\ i+1, \\ r_j \ne 1, n} w_{\xi_n} \mathbb{1}\{r_j \in \mathcal{K}\} \\
  &\begin{aligned}
  \text{where}\;\; &w_{\nu_n} = 1/(2(r_j -1)(n-r_j)) \\
  &\mathcal{K} := [\min\{r_i, r_{i+1}\}, \max\{r_i, r_{i+1}\}]
  \end{aligned}
