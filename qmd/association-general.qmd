...
-   [dcor]{.underline}
    ...
    -   distance covariance and distance correlation share a parallel with product-moment covariance and correlation. However, unlike the classical definition of correlation, distance correlation is zero solelywhen the random vectors are independent. Besides, the distance correlation can be used to evaluate both linear and nonlinear correlations between variables.
    -   Notes from [Improved Distance Correlation Estimation](https://arxiv.org/abs/2405.01958)
    -   {energy} implements `dcov` $\mathcal V_n(X,Y)$ and `dcor` $\mbox{dCorV}(X, Y)$, `dcovU` $\mathcal{U}^2_n (X, Y)$ and `bcdcor` $\mbox{dCorU}^2(X, Y)$ "that is, they do not take the square root" (can yield negative estimates of the squared covariance in cases of independence or very low levels of dependence and small sample sizes due to the fact that $\mathcal{U}^2_n (X, Y)$ is an unbiased estimator of the squared distance covariance \$\\mathcal V_n\^
    -   Solutions:
        -   Replace the values of $\mathcal{U}^2_n (X, Y)$ with their absolute value. This U-estimator of the distance correlation is denoted by $\mbox{dCorU}(A)$. Under the independence scenario, both $\mbox{dCorU}$ and $\mbox{dCorU}(A)$ yield identical MSE.
        -   Consider $\max \{\mathcal{U}^2_n (X, Y), 0\}$ so negative values of $\mathcal{U}^2_n (X, Y)$ are truncated to zero. Denote this U-estimator of R as $\mbox{dCorU}(T)$.
    -   Under independence, dCorU looks preferable over dCorV. The superior estimation is in general when truncating.
    -   Under dependence
        -   Linear: dCorV `dcortools::distcor(X,Y)` estimator aligns with the best results in terms of MSE (Farlie-GumbelMorgenstern (FGM) copula and bivariate normal).

        -   Nonlinear: dCorU `dcortools::distcor(X,Y,bias.corr = T)` estimator provides better estimates, however the differences are not relevant. The optimal estimator appeared to be dCorU(A), especially in cases of weak dependence.

            $$
            \mbox{dCorU} = \mbox{sign}(\mbox{dCorU}^2)\sqrt{|\mbox{dCorU}^2|}
            $$
    -   Computation Time: dCorU and dCorV are similar.
    -   Instead of choosing one or the other, use a linear combination. The proposed estimator with the parameter λ0 estimated using bootstrap, dCorλˆ0, give better results than compared to using only dCorU or dCorV. However, it is necessary to take into account that the computation time will increase as the number of bootstrap iterations increases.
        -   a convex linear combination of the former estimators
        ...
        -   Formula for $\lambda_0$ using values calculated from 1000 bootstraps\
            $$
            \begin{aligned}
            ...
            &\text{where} \quad \hat \theta^U = \mbox{dCorU}(X,Y) \;and\; \hat\theta^V = \mbox{dCorV}(X,Y)
            \end{aligned}
            $$
    -   {dcortools} implements `distcov` and `distcor`. When [bias.corr = FALSE]{.arg-text}, it allows to use the V-estimator, otherwise the U-estimator is used.
    -   {Rfast} implements `dvar`, `dcov`, `dcor`, and `bcdcor`, using the fast method proposed by . And `bcdcor` computes the bias-corrected distance correlation of two matrices.
