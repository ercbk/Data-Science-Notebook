...
-   [dcor]{.underline}
    ...
    -   [Guidelines]{.underline}
        ...
        -   Unless using a squared version (See Misc \>\> Packages), values are between 0 and 1 where 0 means indpendence and 1 means perfect association.
    ...
    -   [Misc]{.underline}
        ...
        -   Packages
            -   Squared Measures: The distance calculation requires positive covariance values, since there's a square root in the calculation. This can be an issue for associations close to 0 (i.e. independence) and small sample sizes, because they can result in negative value estimates. Some package functions do not take the square root as a workaround. As a result, their values will not be bounded by \[0,1\].
            -   {[dcortools](https://cran.r-project.org/web/packages/dcortools/index.html)} (Used by Paper) - Tons of options; Can use {RFast} for calculations
    ...
    -   [Notation]{.underline}
        ...
        -   Biased Distance Covariance , $\mathcal{V}_n(X,Y)$ (V-Statistic)
            -   Biased since it always provides positive results even close to 0.
        -   Biased Distance Correlation, $\mbox{dCorV}(X, Y)$ (V-Statistic)
        -   Unbiased Distance Covariance , $\mathcal U_n(X,Y)$ (U-Statistic)
        -   Unbiased Distance Correlation, $\mbox{dCorV}(X, Y)$ (U-Statistic)
        ...
        -   A convex linear combination
            ...
            -   Formula for $\lambda_0$\
                ...
                \text{where} \quad &\hat \theta^U = \mbox{dCorU}(X,Y) \;\mbox{and}\; \hat\theta^V = \mbox{dCorV}(X,Y) \\
                                   &\mbox{Bias}(\hat\theta^U) = \frac{1}{B} \sum_{b=1}^B \hat\theta^U - \hat \theta
                \end{aligned}
                \end{aligned}
                $$

                -   The smoothed bootstrap relies on a kernel function (typically a symmetric density around zero), and h \> 0, a smoothing parameter referred to as the bandwidth. In this case, h~1~, h~2~ --- one for each variable.
