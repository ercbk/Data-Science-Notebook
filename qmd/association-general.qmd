...
-   Misc
    ...
    -   Also see
        ...
        -   [Nonlinear](association-general.qmd#sec-assoc-gen-nonlin){style="color: green"} \>\> dcor for packages that implement partial distance covariance/correlation methods
...
-   [Example]{.ribbon-highlight}: [source](https://github.com/raz1470/causal_ai/blob/main/notebooks/making%20causal%20discovery%20work%20in%20real%20world%20business%20settings.ipynb?source=post_page-----80e80c5f66b8--------------------------------)
    ...
    -   Accept Null Hypothesis: There is no association between ice cream sales and shark attacks
...
-   [Robust Versions]{.underline}
    ...
    -   Preprocessing the variables can result in robust version of dcor

    -   Comparing the classical dCor with the robust version helps to identify whether the dependence was mainly in the tails or rather in the center of the data.

    -   Transformations are only for continuous variables but transformed variable can still be used with binary variables for mixed distance correlations

    -   3 types of transformations were considered

        -   Z-Score - Standardizing the variable
        -   Rank - Converting the variable values to ranks
        -   Biloop - A trigonometric transformation that standardizes the variable to have median of 0 and MAD of 1 + other stuff
    ...
    -   Recommendations based on the paper
        ...
        -   Heavy tails with concern about outliers $\rightarrow$ z-score (better for tails) or rank (better for outliers)
        -   Light tails with concerns about outliers $\rightarrow$ biloop
