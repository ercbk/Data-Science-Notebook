...
* EU\[D(a)\] : Expected utility of a decision-making situation (aka lottery) for a particular action, a![](./_resources/Decison_Intelligence.resources/Screenshot (657).png)
...
* Enabling Factors
	...
	* An organization’s decision _culture_ (values, desire for transparency in decision-making, culture of continuous improvement, etc.) is a critical enabling factor that supports a DI approach to decision-making
...
* Pre-Decision Activities
	* Decision Framing
		...
		* Questions to address before looking at the data
		...
		Decision Mapping
			Create the CDD by graphically represent causal relationships between actions, intermediaries, and desired outcomes
				See link above for the basics of CDDs
	...
	* Identifying Uncertainty
		...
		* In the CDD, look at each causal link
			* Identify uncertainty from a Rumsfeldian perspective ![](./_resources/Decison_Intelligence.resources/image.3.png)
	...
	* Approach![](./_resources/Decison_Intelligence.resources/image.4.png)
...
* Expected Utility with prior information![](./_resources/Decison_Intelligence.resources/Screenshot (660).png)
	* To calculate the Expected Utility this equation gets manipulated into![](./_resources/Decison_Intelligence.resources/Screenshot (667).png)
		...
		* Basically there are two parts:
			...
			* where![](./_resources/Decison_Intelligence.resources/Screenshot (668).png)
				* Says Z is the parent variable to the Action and W is the rest of the variables that aren't Z
		* Step function the Decision Rule![](./_resources/Decison_Intelligence.resources/Screenshot (669).png)
...
* [Example]{.ribbon-highlight} (Decision Rule based on prior information)![](./_resources/Decison_Intelligence.resources/Screenshot (662).png)
	...
	* The maximized expected utility = max\_utility(s0) + max\_utility(s1) + max\_utility(s2) = 0 + 1.15 + 2.10 = 3.25
...
* [Example]{.ribbon-highlight} (Risk Averse Curve)![](./_resources/Decison_Intelligence.resources/Screenshot (669).1.png)
...
* **Decision Curve Analysis (DCA)**
	...
	* General interpretations for all Net Benefit vs Threshold Probability Charts
		* Where the "treat everyone" base model's net benefit = 0 (i.e line crosses the x-axis), that threshold probability is the (disease) **prevalence**.
			* If you have a representative sample, then the prevalence can also be calculated by number\_with\_disease / sample size
...
* **(model based) net benefit** = \[probability\_threshold / (1 - probability\_threshold)\] \* \[(TP/N) - (FP/N)\]
...
* Data for examples![](./_resources/Decison_Intelligence.resources/Screenshot (673).png)
	* Marker is a numeric
	* cancerpredmarker is fitted probabilities from a logistic regression
* [Example]{.ribbon-highlight}: Compare Multivariable model to Univariate model![](./_resources/Decison_Intelligence.resources/Screenshot (679).png)
...
* [Example]{.ribbon-highlight}: A Bad Model![](./_resources/Decison_Intelligence.resources/image.1.png)
	* Interpretation
...
* [Example]{.ribbon-highlight}: Conditional and Joint application
	* Approaches (besides base models):
		...
		* Measure the marker for everyone, then biopsy anyone who is **either** at high risk of cancer **or** who was determined to have a probability of cancer past a certain level, based on the marker
			* joint approach part is risk\_group == "high risk" or marker probability > some\_amount)
		* Biopsy everyone at high risk; measure the marker for patients at intermediate risk **and** biopsy those with a probability of cancer past a certain level, based on the marker
			* conditional approach part is risk\_group == "intermediate" and marker probability > some\_amount)
...
* Run dca `dca(data=data.set, outcome="cancer", predictors=c("high_risk", "joint", "conditional"), xstop=0.35)`![](./_resources/Decison_Intelligence.resources/Screenshot (681).png)
* Interpretation
...
* harm arg
	* harm = 0 for predictor == high\_risk (i.e. harm not applied)
	* harm\_marker for predictor == joint (i.e. constant)
	* harm\_conditional for predictor == conditional (i.e. proportion of patients who have immediate risk multiplied by the harm\_marker)
* ![](./_resources/Decison_Intelligence.resources/Screenshot (683).png)
* Interpretation
...
* [Example]{.ribbon-highlight} Unnecessary interventions avoided![](./_resources/Decison_Intelligence.resources/Screenshot (685).png)
...
* Comparing Traditional Decision Analysis with Model-Based DCA ([article](https://www.fharrell.com/post/dca/))![](./_resources/Decison_Intelligence.resources/vickers3.png)
...
* Steps
	* Conduct a market research study
		...
		* Stratify sample and survey on these attributes![](./_resources/Decison_Intelligence.resources/1-ANJqIxaziH-98UXsxEJrUw.jpeg)
		...
		* Record answers![](./_resources/Decison_Intelligence.resources/1-TeZH1gmCDTYwwW6Wmd-mwg.png)
...
* Summarize utilities for each attribute![](./_resources/Decison_Intelligence.resources/1-te8LLeCcM2KDsbHBrAkvAw.png)
...
* Calculate the total utility and probability of purchase![](./_resources/Decison_Intelligence.resources/1-LmExSIr8MMauNHXIDgnDPQ.png)
...
* **Concept Map**
		Allows us to visually display a system and pinpoint how the linkages between its parts
		
	* Steps
		* Formulate a problem question
			* What are the exact questions that you need to answer to be able to visually represent the system in which the problem is situated?
					“How does X work?”, “What’s the context of X in which it exists?” and “How is X linked to Y?”
					
				[Example]{.ribbon-highlight}: App plays music to induce mood of user. App not performing as well in production as it did during CV
		...
		* Identify key entities and sort them
				Create a list of the key entities that impact the problem and are linked to it. These entities might be people, algorithms, processes, places, protocols, and more. (commonly around 20 entities)
				
				Sort the list by specificity and/or importance.
				
				Helps you to uncover the hierarchy
				
			Outline the map and fill it in
				Start adding entities according to your hierarchy and understanding of the problem
				
			* Write the actual action of the said connection by adding phrases to the arrows like “adds to”, “creates”, “selects from”, “picks according to”
			...
			* Example (see example description above)![](./_resources/Decison_Intelligence.resources/1-1024x630.png)
* **Iceberg Model**
	...
	* Components
		...
		[Example]{.ribbon-highlight}: Bank needs forecast of the amount of money taken out from their ATMs, 30 days in advance, and daily
			Events
				Follow-up questions: Why? For what use case? Why 30 days? Why daily? How often do they provision the ATM?
...
* **Six Thinking Hats**
		Approaching the problem from different standpoints should lead us to the right decision
...
* **The Cynefin Framework**
	* Problem categories
		...
		* Chaotic - causal relationships are unclear
				The goal is to bring the problem down to the Complex category
...
* **First Principles**
	...
	* Process:
		* Break the problem down into basic truths
			* Socratic Method: you start with one question and pose another one when you get the answer and you go until you reach the basic truth.
				5 Whys: Ask ourselves 5 times Why is something the case
...
* **Inversion Approach**
	* View the problem from a different angle and/or consider the worst possible scenarios
		...
		[Example]{.ribbon-highlight}: Imagine a project/solution failed. Ask why did it fail.
			Ask the team to brainstorm together why it failed, what was done/gone wrong, what mistakes we made, what we didn’t consider, and more.
