...
- Packages
  - [{]{style="color: goldenrod"}[pyjanitor](https://github.com/pyjanitor-devs/pyjanitor){style="color: goldenrod"}[}]{style="color: goldenrod"} - A Python implementation of the R package janitor. It provides a clean user-friendly API for extending pandas with powerful and readable data-cleaning functions.
- Resources
  - [Arel-Bundock Tutorial](https://arelbundock.com/posts/dt_tb_df/index.html)
    - Shows most common processing operations
    - Tabs for data.table, base, dplyr, polars, pandas
...
- Wrap text of categories with long names
...
- Dataframes
  ...
  - Create a copy of a df: `df2 = df1.copy()`
    ...
    - [deep=True]{.arg-text} (default) means it's a deep rather than shallow copy. A deep copy is its own object and manipulations to it don't affect the original.
...
- By group
  ...
  - FIlters each [country]{.var-text} by the `median` of its [amount]{.var-text} times 10.
  - The result of `apply` is a dataframe, but that dataframe has the index, [country]{.var-text} (i.e. rownames). The problem is that the result also has a [country]{.var-text} column. Therefore using `reset_index` would move the index to a column, but since there's already a country column, it will give you a [ValueError]{.arg-text}. Including [drop=True]{.arg-text} fixes this by dropping the index entirely.
...
- `apply`  an operation to column(s)
  ...
  - Within a chain
    ...
    - When the result of the `apply` function is coerced to a dataframe by `reset_index`, the column will be named "0". `rename` is used to rename it [total]{.var-text}.
...
- `agg` can only operate on one column at time, so for transformations involving multiple columns (e.g. col3 = col1 - col2), see [Mutate](python-pandas.qmd#sec-py-pandas-ops-mutate){style="color: green"} section.
...
- Group and aggregate on more than one column
...
- Filter by Group, then Summarize by Group ([link](https://www.sumsar.net/blog/pandas-feels-clunky-when-coming-from-r/))
  ...
  - Option 1
    ...
    - FIlters each [country]{.var-text} by the `median` of its [amount]{.var-text} times 10.
    - For each [country]{.var-text}, subtracts a [discount]{.var-text} from [amount]{.var-text}, then `sums` and `renames` the column from "0" to [total]{.var-text}.
  ...
  - Option 2
    ...
    - Here the `median` [amount]{.var-text} per [country]{.var-text} is calculated first and assigned to each row in [purchases]{.var-text}.
...
### Case When and Ifelse {#sec-py-pandas-ops-caseif .unnumbered}

- `where` docs: [Dataframe](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.where.html)

- `replace` docs: [Dataframe](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.replace.html)

- `map` docs: [Series](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.map.html#pandas.Series.map), [Dataframe](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.map.html#pandas.DataFrame.map)
...
- `where`
  ...
  - Single condition
    ...
    ``` python
    df["x"] = df["x"].where(df["x"] > 0, 0)

    # with numpy, you can specify a specific True value
    df["x"] = np.where(df["x"] > 0, df["x"], 0)
    ```

    - Keeps the current value if False, and it's 0 if True
...
- In the whole df
  ...
  - 1 value
    ...
    - [inplace=True]{.arg-text} says modify orginal df (default = False)
...
- Only in specific columns
  ...
  - One column
    ...
    - Notes from [SO post](https://stackoverflow.com/a/75933438/8820422)
    ...
    - `replace`
      ...
      conversion_dict = {'negative': -1, 'neutral': 0}
    ...
    - `map`
      ...
      ``` python
      df['converted_column2'] = df['col2'].map(conversion_dict).fillna(df['col2'])
      df
      #>    col1      col2 converted_column converted_column2
      #> 0     1  negative               -1              -1.0
      #> 1     2  positive         positive          positive
      #> 2     2   neutral                0               0.0
      #> 3     3   neutral                0               0.0
      #> 4     1  positive         positive          positive
      ```

      - Faster than `replace` for *large* replacement dictionaries
    ...
    - `np.select`
      ...
      # Apply the logic
      df['converted_column3'] = np.select(ls_conditions, ls_choices, default=df['col2'])
      df
      #>    col1      col2 converted_column converted_column2 converted_column3
      #> 0     1  negative               -1              -1.0                -1
      #> 1     2  positive         positive          positive          positive
      #> 2     2   neutral                0               0.0                 0
      #> 3     3   neutral                0               0.0                 0
      #> 4     1  positive         positive          positive          positive
      ```

      - If the remapping dictionary is very large, this may run into memory issues
...
### Replace NAs {#sec-py-pandas-ops-repna .unnumbered}
...
- Also see
  - [Feature Engineering, Time Series \>\> Misc](feature-engineering-time-series.qmd#sec-feat-eng-ts-misc){style="color: green"} has a list of python libraries for preprocessing
...
- Fill in gaps
  ...
  - Explode interval between 2 date columns into a column with all the dates in that interval
    ...
    - [Example]{.ribbon-highlight}: all rows\
      ![](./_resources/Python,_Pandas.resources/image.png){.lightbox width="479"}
    ...
    - [Example]{.ribbon-highlight}: pivot_longer\
      ![](./_resources/Python,_Pandas.resources/image.1.png){.lightbox width="141"}
...
- Aggregation
  ...
  - Step-Window
    - [Example]{.ribbon-highlight}: Mean temperature every 3 hours\
      ![](./_resources/Python,_Pandas.resources/image.3.png){.lightbox width="217"}
    ...
    - [Example]{.ribbon-highlight}: Max sunshine every 3 hrs\
      ![](./_resources/Python,_Pandas.resources/image.4.png){.lightbox width="283"}
...
- [Example]{.ribbon-highlight}: simulation, gaussian\
  ![](./_resources/Python,_Pandas.resources/image.9.png){.lightbox width="585"}
...
- Standard GARCH time-series that's frequently encountered in econometrics
  ...
  - [Example]{.ribbon-highlight}: simulation, beta\
    ![](./_resources/Python,_Pandas.resources/image.10.png){.lightbox width="585"}
...
- Bad performance by iteratively creating rows in a dataframe
  ...
  - Use `itertuples` instead of `iterrows` in loops![](./_resources/Python,_Pandas.resources/image.11.png){.lightbox width="376"}
...
- Variable datatypes\
  ![](./_resources/Python,_Pandas.resources/image.5.png){.lightbox width="586"}
  ...
  - Byte ranges (same bit options for floats)\
    ![](./_resources/Python,_Pandas.resources/image.6.png){.lightbox width="407"}
  ...
  - Use sparse types for variables with NaNs\
    ![](./_resources/Python,_Pandas.resources/image.2.png){.lightbox width="299"}
