...
-   `match`
    ...
    -   [{]{style="color: #990000"}[fastmatch](https://cran.r-project.org/web/packages/fastmatch/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides a fast `match` replacement for cases that require repeated look-ups.

        -   It is slightly faster that R's built-in `match` function on first match against a table, but extremely fast on any subsequent lookup as it keeps the hash table in memory.
...
-   `try`
    ...
    -   [Example]{.ribbon-highlight}: [Geospatial, Spatio-Temporal \>\> EDA \>\> Spatial Dependence](geospatial-spat-temp.qmd#sec-geo-sptemp-eda-spdep){style="color: green"} \>\> Example 1
...
-   `tryCatch`
    ...
    -   [Example 6]{.ribbon-highlight}: Save output; Model fitting ([source](geospatial-spat-temp.qmd#sec-geo-sptemp-krig-stval){style="color: green"} \>\> Starting Values \>\> Example 1)
        ...
        -   This chunk of code is from inside a function — hence the `return` functions
        -   By saving the output of the `tryCatch`, I can return a NULL. Therefore, in the final resultant tibble, there won't be any evidence of this fitted object or its error.
        -   There will be a message in console about the parameter settings that lead to the error and the error message itself.
