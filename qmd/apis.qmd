...
-   Tools
    -   [Postman](https://www.postman.com/) - Platform for building APIs
-   Design questions
...
## Optimization {#sec-apis-opt .unnumbered}
...
-   [Serialization]{.underline}
    -   Types
        ...
        -   Text
            -   e.g. json, csv, xml
        -   Binary
            -   e.g. parquet, feather, protobuf, rds, Rdata
    ...
    -   Comparison
        ...
        | [Serialization/deserialization speed]{.underline} | Often **faster**, since it doesn’t need to convert to/from text strings. | Often **slower**, because it requires parsing or formatting text. |
        | [Human readability]{.underline} | Not human-readable. | Human-readable, easy for debugging. |
        | [Interoperability]{.underline} | Certain types are require same system (e.g. python to python) | Universally readable by other languages and APIs. |
        | [Best use case]{.underline} | Within same-system communication. (e.g. R pipelines) | Between systems (APIs, web clients, JavaScript). |
    ...
    -   [Example]{.ribbon-highlight}: [{plumber2}]{style="color: #990000"} and [{yyjsonr}]{style="color: #990000"} ([source](https://joekirincic.com/posts/performance-optimization-for-plumber-apis-serialization/))
        ...
        -   2 endpoints:
            -   [optimized-payload]{.var-text} which uses the [{yyjsonr}]{style="\"color:"} optimized json serializer
            -   [default-payload]{.var-text} which uses the plumber2 default json serializer from [{jsonlite}]{style="\"color:"}
...
## FastAPI {#sec-apis-fastapi .unnumbered}
