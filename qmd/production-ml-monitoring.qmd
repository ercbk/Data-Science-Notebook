...
![](./_resources/Production,_ML_Monitoring.resources/tableofissues.png)
...
* Misc
	* DL Monitoring points![](./_resources/Production,_ML_Monitoring.resources/image.3.png)
	...
	* Other tracking
		...
		* [SHAP for Drift Detection: Effective Data Shift Monitoring](https://towardsdatascience.com/shap-for-drift-detection-effective-data-shift-monitoring-c7fb9590adb0)
				Feeds distribution of SHAP values into a logistic regression, then applies ks test on probability predictions for Y==1, Y==0
	...
	* Route alerts (slack) for a particular table only to teams that regularly use or maintain that table
		* As alerts arrive, they can use emoji reactions to classify their response to alerts.
	...
	* Alerts should have contextual information
		* Example
			...
			* What # and % of user\_id values are affected?
			* How often has this alert failed in the recent past?
			* Who configured this alert, and why?
			* What dashboards or ML models depend on fact\_table?
			* What raw data source contributed user\_id to fact\_table ?
	...
	* Root cause analysis
		* Uses statistical method to find out where the issue is
			* group\_by(cat) %>% summarize(pct\_bad = ... whatever)
...
* data required for monitoring
	...
	* Data size required to be able to measure a metric accurately
		...
		* Example:
			...
			* shot\_noise <- ((pos\_rate \* data\_size) \* (1-recall)) / data\_size
				* I don't see how to solve this for data\_size (i.e. data\_size cancels out) so I guess this is an optimization problem
* Changes in the distribution of input and output features
	* Triggers for retraining model (if possible, use both)
		...
		* By measuring changes in the input data, i.e. changes in the distribution of features that are used by the model, you can detect data drifts that indicate your model may be outdated and needs to be retained on fresh data.
			* _Missing values_ can occur regularly at ML model inference. Even when missing values are allowed in features, a model can see a lot more missing values than in the training set. An example of a missing value error is an ML model making an inference based on a form input where a previously optional field is now always sending a null value input due to a code error.
			* _Range violation_ happens when the model input exceeds the expected range of its values. It is quite common for categorical inputs to have typos and cardinality mismatches to cause this problem, e.g. free form typing for categories and numerical fields like age, etc. An unknown product SKU, an incorrect country, and inconsistency in categorical values due to pipeline state are all examples of range violation.
			* _Type mismatch_ arises when the model input type is different from the one provided at inference time. One way types get mismatched is when column order gets misaligned during some data wrangling operations.
* Track distance metrics between reference variable distribution and a production variable distribution
	...
	* Thresholds
		...
		* Computationally intensive method (may not be practical)
			...
			* Threshold =  mean(drift) of all the simulations
				If any production/inference dataset has a drift > threashold then that should trigger an alert
				
		* Closed method for PSI
			* Supposedly can be calculated for other distance measures.
			* Final Solution for the threshold 🥴![](./_resources/Production,_ML_Monitoring.resources/Screenshot (555).png)
				* K is the number of bins (numeric) or levels (categorical)
				* Pk is the percentage of total _training_ observations == level\_K or the percent of total training observations in bin K
				* αk = 1 + Nq + Pk
...
* Packages
		See [article](https://www.r-bloggers.com/2022/09/kantorovich-distance-with-the-ompr-package/) for examples of computing wasserstein (aka Kantorovich) distance in R, py, Julia.
		
	* [{philentropy}]{style='color: #990000'} has a ton of distribution distance measures + some helper functions, [Docs](https://drostlab.github.io/philentropy/index.html)
		[{{cinnamon}}]{style='color: goldenrod'} LIB handles py models and calculates wasserstein and kolmogorov-smirnov
...
* Population Stability Index (PSI)
	* Formula![](./_resources/Production,_ML_Monitoring.resources/ql_1aa491ea1b2fec3248d2bd2bdb05e93f_l3.png)
		* i ∈ length(bins)
	...
	* Steps
		1. Divide the reference variable variable range into 10 bins (arbitrary but seems to be common) or however many bins you want depending on how fine a resolution you want.
			...
			* numeric
				* slicing the _range_ of the reference variable into sections of the same interval length
				* slicing the reference variable into _quantiles_ where each bin has the same number of observations
		...
		5. Calculate PSI
	* Guidelines
		Also see Thresholds section above
		from https://scholarworks.wmich.edu/cgi/viewcontent.cgi?article=4249&context=dissertations
		![](./_resources/Production,_ML_Monitoring.resources/Screenshot (160).png)
	...
	* Example: model predictions![](./_resources/Production,_ML_Monitoring.resources/0-xdkcpmdk_NvfW88U.png)![](./_resources/Production,_ML_Monitoring.resources/1-nhVqI5AJC-DeTCoP4XvraQ.png)
	...
	* Example
		from [arize ai](https://arize.com/take-my-drift-away/) (model monitoring platform)
		![](./_resources/Production,_ML_Monitoring.resources/0-qz6NhHDCS3FYbONl.png)
...
![](./_resources/Production,_ML_Monitoring.resources/ql_39dea25ce67b8612aaabfee7409061f5_l3.png)

* where pmix = 0.5(pref + pprod) and![](./_resources/Production,_ML_Monitoring.resources/ql_ac1542a437de134369e4c2fd6712bda0_l3.png)
	* i ∈ length(bins)
	* p is the percent of total observations in bin i
	* Similar for KL(pprod || pmix)
* Steps: Same preparation steps as with the PSI distance metric (binning, proportions, etc.)
* Symmetric version of K-L divergence ( see [Information Theory](Information Theory) >> K-L Divergence) which means it satisfies the triangle inequality which means it's a true distance metric
...
* Invariance testing
	...
	* Example![](./_resources/Production,_ML_Monitoring.resources/image.4.png)
* Directional Testing
	...
	* See [{{]{style='color: goldenrod'}[behave](https://github.com/behave/behave){style='color: goldenrod'}[}}]{style='color: goldenrod'}, [Write Readable Tests for Your Machine Learning Models with Behave](https://towardsdatascience.com/write-readable-tests-for-your-machine-learning-models-with-behave-ec4a27b91490)
	* Example![](./_resources/Production,_ML_Monitoring.resources/image.5.png)
...
* Testing for drift![](./_resources/Production,_ML_Monitoring.resources/image.png)
	* Option 1: You'll already have a trained candidate ready for deployment if model 2 outperforms model 1.
		* Example: An aggregated dataset consists of 45,000 timestamped observations which we spilt into 20,000 references, 20,000 current, and 5,000 most recent observations for the test.
			* Score Models![](./_resources/Production,_ML_Monitoring.resources/image.1.png)
...
* Architectures
	* Azure ML![](./_resources/Production,_ML_Monitoring.resources/image.2.png)
...
* Misc
	* Also see [Project, Management](Project, Management) >> Event Auditing
	...
	* If your model is triggering alerts to often, generate additional rules with users to limit the number of alerts
		* Example: model that predicts patient risk of death can only trigger a _re-alert_ to the clinician if:
			* it's been 48 hrs since the previous alert
			* if the patient has _not_ just came from the ICU
* Email
	* Example: to developers![](./_resources/Production,_ML_Monitoring.resources/Screenshot (1185).png)
		* "jarvis" is an internal package and this function probably wraps a template and email package function
	* Example: to users![](./_resources/Production,_ML_Monitoring.resources/Screenshot (1188).png)
		* "CHARTWatch" is the name of the data product
		* Alerts user that the product is down and other pertinent information
* Slack
	* Example![](./_resources/Production,_ML_Monitoring.resources/Screenshot (1186).png)
