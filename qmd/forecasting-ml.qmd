...
* Algorithm Specifications
	...
	* LightGBM
		* arg: linear\_tree fits a piecewise linear model for each leaf. Helps to extrapolate linear trends in forecasting
...
* Misc
	* Notes from: [6 Methods for Multi-step Forecasting](https://towardsdatascience.com/6-methods-for-multi-step-forecasting-823cbde4127a)
		* Example dataset![](./_resources/Forecasting,_ML.resources/Screenshot (1091).png)
...
* Recursive (aka iterative) - training a single model for one-step ahead forecasting. Then, the model's one-step ahead forecast is used as data to get the 2nd-step ahead forecast.
	* The one-step ahead forecast is NOT _added_ to the training data and the model refitted. It _replaces_ the 0th lag value in the training data, and the previous 0th lag data becomes the 1st lag data, etc. The final lag (e.g t-3 in the example data) that was used in training the model is discarded. Therefore, the same number of lags that was used in training the model remains the same. This _updated_ dataset becomes "new data" and along with the original model are used as input for `predict`
...
* "Y\_tr" contains lead variables for each step in the forecast horizon
...
* [Example]{.ribbon-highlight}: Preprocessing training data for 1 then 2 steps ahead and forecasting ([post](https://github.com/WinVector/Examples/blob/main/TS/TS_example.md),  Code >> Time Series >> direct-multistep-forecasting-regression.R)
	...
	* "y\_observed" is the original outcome column
	* "y\_f" is y\_observed but stepped forward
	* "x\_0" is a copy of y\_observed
	* "x\_1" to "x\_5" are lags of y\_observed
	* 1-Step Ahead Training Data
		* Before Preprocessing![](./_resources/Forecasting,_ML.resources/1aheadorig.1.png)
			* "y\_f" has been stepped forward 1-step
			* "y\_observed" and rows with NAs are deleted
			* Even though 6 rows are deleted from the training data, all values are used for training
		* After Preprocessing![](./_resources/Forecasting,_ML.resources/1ahead.png)
			* The model is fit using this data
	* 2-Step Ahead Training Data
		* Before Preprocessing![](./_resources/Forecasting,_ML.resources/2aheadorig.1.png)
			* "y\_f" has been stepped forward 2-steps
			* "y\_observed" and rows with NAs are deleted
			* Even though 7 rows are deleted from the training data, all values are used for training
		* After Preprocessing![](./_resources/Forecasting,_ML.resources/2ahead.1.png)
			* The model is fit using this data
	* Forecast![](./_resources/Forecasting,_ML.resources/pred.png)
...
* Direct-Recursive (aka chaining) - Combo of Recursive and Direct
	...
	* Direct - The previous forecasts are _added_ to the training data and the model is retrained for each horizon.
...
* Difference until stationary
	...
	* Forecasts will need to be back-transformed![](./_resources/Forecasting,_ML.resources/image.png)
		* forecast = differenced forecast + previous value
		* Then recursively for the next forecasts in the horizon![](./_resources/Forecasting,_ML.resources/image.1.png)
* log transform
...
* Window Statistics (See [Python, Pandas](Python, Pandas) >> Time Series >> Aggregation)
	* Sliding window features - at each time point, a summary of values over a fixed window of _prior_ timesteps
	* Rolling window statistics - a range of values that includes the time point itself as well as some specified number of data points _before and after_ (?) the time point used
...
* External Features
	...
	* unemployment rate.
* From Hyndman local vs global models paper (see Algorithm Specifications section)
	* Used
