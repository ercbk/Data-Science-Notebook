...
- [API Wrappers]{.underline}
  ...
  - [{]{style="color: #990000"}[jevr](https://github.com/simxnherrera/jevr){style="color: #990000"}[}]{style="color: #990000"} - Wrapper around the TypeSafe Jev System One (aka Decision Model)
    - Articles on System One (or Decision Models): [Willison](https://simonwillison.net/2026/Sep/21/jev/), [Bite Code!](https://www.bitecode.dev/p/whats-all-this-noise-about-jev?), [Outcome School](https://outcomeschool.com/blog/jev-and-system-one-models-explained?) (deeper dive)
    - Essentially one-shot classifier (and scorer) that takes a prompt and outputs probabilities for each type (if all are desired).
    ...
    - Processes each of these types in parallel which along with the output being a number means it's very cheap to run.
    - Calibrates probabilities via reinforcement learning
    - Jev cannot hallucinate (i.e. invent an answer that does not exist). For example, it can't mathematically choose an invalid choice (type error).
...
- Benchmarks
  - Companies that suggest their agents have something like 85% success rate don't tell you that they're talking about single-step evaluations measured using controlled benchmarks on carefully selected tasks
    - [Example]{.ribbon-highlight}: Agent w/"85%" success rate on a ten-step task actually has a 20% overall success rate: $0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 = 0.197$\$0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 × 0.85 = 0.197\$
  ...
  - 2026 Aug DataBench on Frontier Models ([source](https://hex.tech/blog/databench-agentic-analytics-benchmark/#performance-on-databench))
    - Models are best at gathering evidence (75% on Q&A tasks) (Small models are close to frontier modesl on these tasks)
      - Examples
        ...
        - "What subscription changes did we process by month in Q4?"
      - Suprisingly handles messy data environments such as having multiple definitions of the same metric, fragmented definitions that live across difficult-to-join tables, stale and half-deleted datasets when the intent and guardrails of a task are relatively clear.
      - Unless prompted aggressively and repeatedly, models are unlikely to do broad “explore and exploit” reorientation and saturation of an analytical problem space (i.e. that don't look at numbers and think, "ah, that doesn't look right."), preferring to attack head on, sometimes in an overcommitted fashion. They also don’t reliably sanity check their work before presenting it (can asking for a sanity check be included in the prompt?).
    - Models are worse at open-ended delegated decisions (66%)
      - The questions require the agent not just to obtain an accurate data point (Q&A), but usually to obtain multiple data points and synthesize them alongside additional context and make judgment calls— often doing multiple iterations of investigation to arrive at the right answer. They also require much more careful framing to the user in order to be honest, accurate, and helpful.
    ...
    - Models are worst on the specific “trap” failure modes (which require intuition and reasoning) where there is an obvious and plausible but wrong easy answer and success requires going deeper (54%).
      - Example: "I need the January 15 Collections call list. Commerce and Stripe disagree on some subscription statuses — which accounts should we call, and how much MRR is at risk"
        - The obvious answer —call everyone the systems disagree on. But it’s wrong: the delinquency flags are stale sync artifacts, and the separate cash ledger shows most of those accounts already paid.
      - Agents can run all the numbers "correctly" under very specific assumptions of what “correct” means, but still draw the wrong second or third-order conclusion. This doesn't generally come from trusting bad evidence — it comes from manufacturing certainty.
        - Example: "What share of November and December orders were split into multiple shipments, and did those orders generate more suppport contacts?"
          - The model correctly gets the counts/rates, but then confidently gives a cause and recommendation that's wrong.
    - Models don't like to throw in the towel
      - Models sometimes talk themselves past a correct simple answer to get the user a more complicated and wrong (or just confusing) answer. Agents are getting better at this, but still seem to be impaired by an overpowering desire to complete their task at higher effort levels. (Claude Fable 5 is the exception)
      - e.g. "Opus 5 tops our charts at high effort but gets devastated by this at xhigh and max."
      - Example: Customer ID cs_1279869346 paid \$92K for the year and canceled four days later. Do I put them in July churn, and when do they lose access?"
        - Opus 5 at medium effort: States the correct, simple answer plainly— access runs through the end of the paid term.
        - Opus 5 at max effort: It does triple the work, categorically proves the billing system never truncates a paid period across all 916 cancellations— and then still hedges, offering the cancellation date as a possible access cutoff anyway.
    - Takeaways
  ...
  - Sites
    ...
    - [DataBench](https://hex.tech/databench/) ([Intro](https://hex.tech/blog/databench-agentic-analytics-benchmark/)) - Benchmark for business-related querying
      - Tracks frontier models. Best model for spend is the dotted line.
  - Extracting data from a .pdf or .jpg of a table ([source](https://kschaul.com/llm-evals/evals/extract-fema-incidents/))
    ...
    - Claude 3.5 and 3.7 sonnet only got 1 or the two requests correctA
- Use Cases
...
- Usage Optimization
  - Data cleaning to cut down input sizes (for example, removing links, images, and other characters in an email chain that's not relevant to the model) 
...
- Claude
  - Tools
    - Design - For visual design work. Better at text alignment than regular chat.
      - [Example creating a slide deck](https://towardsdatascience.com/4-claude-skills-every-data-scientist-needs-in-2026/)
      - Examples include:
        - Slide decks
        - Mobile app design
        - HTML emails
