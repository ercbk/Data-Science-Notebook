...
- Append multiple lines to a file
  ...
  - [Bash Heredoc: Complete Guide with Examples](https://linuxize.com/post/bash-heredoc/)

    - **heredoc** (here document) - A type of redirection that allows you to pass multiple lines of input directly to a command.

  - Syntax

    | Command | Input Redirect | Delimiter Label | Output Redirect | File    |
    |---------|----------------|-----------------|-----------------|---------|
    | cat     | \<\<           | 'some_name'     | \>\>            | my_file |

    - `cat` reads from [stdin]{.arg-text} (lines from input redirect) and writes to [stdout]{.arg-text} (the file because of the output redirect)
    - The delimiter label is just a name that indicates the end of the lines (**heredoc**) you want to append to the file
      - Quoting the delimiter label tells the shell to treat the following lines as text and not to evaluate them as they are written to the file (e.g. looking for a variable value defined in the lines)
    - Using `>>` instead of `>` is so that the contents of the file are appended to and not replaced

  - [Example]{.ribbon-highlight}: Add lines to `activate` so python can find some package locations

    ``` bash
    cat >> .venv/bin/activate << 'EOF' # <1>

    _SITE=$(python3 -c "import site; print(site.getsitepackages()[0])") # <2>
    export LD_LIBRARY_PATH=$(find "$_SITE/nvidia" -name "lib" -type d | tr '\n' ':')$LD_LIBRARY_PATH
    unset _SITE
    EOF # <3>
    ```

    1.  The heredoc line which says add the following lines to the activate file until you see "EOF" then stop

    2.  The three lines that get appended to .venv/bin/activate

    3.  The delimiter label which indicates the end of the heredoc
