...
-   Also see [Regression, Linear \>\> Transformations](regression-linear.qmd#sec-reg-lin-tran){style="color: green"}
...
#### Splines {#sec-feat-eng-gen-cont-tran-spl .unnumbered}
...
-   [Misc]{.underline}
    -   Also see
        ...
        -   [Generalized Additive Models (GAM) \>\> Splines](generalized-additive-models-(gam).qmd#sec-gam-spl){style="color: green"}
    ...
    -   **K**: Number of knots for Regression Splines
        ...
        -   Also an option for knot positions
            ...
            -   Place knots where data exist e.g. fixed quantiles of predictor's marginal distribution (See B-Splines for examples)
                -   From Harrell's RMS\
                    ![](./_resources/Feature_Engineering,_General.resources/Screenshot%20(1370).png){width="500"}
...
-   [Regression Splines]{.underline}
    ...
    -   [B-splines]{.underline}
        ...
        -   [Example]{.ribbon-highlight}: `bs(x, knots = c(min(x), median(x), max(x)))`
            ...
            -   6 dof since d + K = 3 + 3
                -   d = 3 (default)
    -   [Natural cubic and cardinal splines]{.underline} (aka [Restricted Cubic Splines]{.underline})
        ...
        -   [Example]{.ribbon-highlight}: [{mgcv}]{style="color: #990000"}: `s(time, bs = "cr")`
...
-   [Smoothing Splines]{.underline} (aka [Penalized Splines]{.underline})
...
### Embedding {#sec-feat-eng-gen-cats-embed .unnumbered}
