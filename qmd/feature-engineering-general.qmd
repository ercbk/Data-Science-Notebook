...
- Packages
  - [{]{style="color: #990000"}[evoFE](https://cran.r-project.org/web/packages/evoFE/index.html){style="color: #990000"}[}]{style="color: #990000"} - Automates feature engineering using evolutionary algorithms inspired by genetic programming. Starting from raw input features, the package evolves candidate transformation recipes through selection, crossover, and mutation, evaluating fitness via cross-validation or train/validation splits with gradient-boosted tree models ('LightGBM' or 'XGBoost').
...
- Also see
  - [EDA, General \>\> Packages](eda-general.qmd#sec-eda-gen-pkgs){style="color: green"} \>\> Hmisc::describe
    - [Info]{.arg-text} gives a measure of how much information is lost by binning.
  - [Clustering, General](clustering-general.qmd#sec-clust-gen){style="color: green"}
- Packages
  - [{]{style="color: #990000"}[binr](https://cran.r-project.org/web/packages/binr/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implementation of algorithms for cutting numerical values exhibiting a potentially highly skewed distribution into evenly distributed groups (bins)
  - [{]{style="color: #990000"}[cutpointr](https://github.com/thie1e/cutpointr){style="color: #990000"}[}]{style="color: #990000"} - Uses CV and bootstrap in determining and validating optimal cutpoints in binary classification
  - [{]{style="color: #990000"}[optbinningR](https://cran.r-project.org/web/packages/optbinningR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Optimal Binning Methods for Predictive Modeling and Analytics
  - [{]{style="color: #990000"}[smbinning](https://cran.r-project.org/web/packages/smbinning/index.html){style="color: #990000"}[}]{style="color: #990000"} ([article](https://blog.revolutionanalytics.com/2015/03/r-package-smbinning-optimal-binning-for-scoring-modeling.html)) - Requiress binary response. Optimal Binning for Scoring Modeling
  - [{]{style="color: #990000"}[santoku](https://hughjonesd.github.io/santoku/){style="color: #990000"}[}]{style="color: #990000"} - Functions create intervals in many ways, using quantiles of the data, standard deviations, fixed-width intervals, equal-sized groups, or pretty intervals for use in graphs
...
- Dichotomizing is bad ([paper](https://onlinelibrary.wiley.com/doi/10.1002/sim.70402) + [discussion](https://discourse.datamethods.org/t/dichotomization/26337), [post](https://www.bmj.com/content/332/7549/1080.1), [list of papers](https://docs.google.com/document/d/14Z0twsQ82CHzZTmcYwddfeRLNPkuInF0-wOKfKHdTXM/edit))
...
- Harrell
  ...
  - A better approach that maximizes power and that only assumes a smooth relationship is to use a restricted cubic spline (regression spline; piecewise cubic polynomial) function for predictors that are not known to predict linearly. Use of flexible parametric approaches such as this allows standard inference techniques (*P* -values, confidence limits) to be used (See [Feature Engineering, Splines](feature-engineering-splines.qmd#sec-feat-eng-spl){style="color: green"})
...
- [Bivand](https://rsbivand.github.io/eqc25_talk/bivand_eqc25_pres.pdf) suggests that a skewed continuous variable (or maybe just a skewed bounded continuous variable like potentially a proportion) might be binned and transformed into an ordered factor.
  - "... a house variable, which is the proportion of garbage pick-up points that are residential, rather than business. Only one district — City of London — has a low proportion, and a number of others, mostly tourist destinations, have proportions under 0.9; consequently, the variable might better be represented as an ordered factor."
    ...
    ``` r
    fhouse <- cut(eng324$house, c(0.0, 0.25, 0.90, 1))
    table(fhouse)
    ## fhouse
    ## (0,0.25] (0.25,0.9] (0.9,1]
    ##        1         23     300
    ```

  - Also see [Regression, Linear \>\> Contrasts](regression-linear.qmd#sec-reg-lin-contrasts){style="color: green"} \>\> `contr.poly`. Bivand actually uses `contr.diff` in his model
- They might help with prediction using ML or DL models though
  ...
  - Naive Bayes likes binned variables ([source](https://medium.com/towards-data-science/bernoulli-naive-bayes-explained-a-visual-guide-with-code-examples-for-beginners-aec39771ddd6))
- Types
  - Notes from [Discretization, Explained: A Visual Guide with Code Examples for Beginners](https://towardsdatascience.com/discretization-explained-a-visual-guide-with-code-examples-for-beginners-f056af9102fa)
  - Equal-Width
...
- [Examples]{.ribbon-highlight}
  ...
  - [Example]{.ribbon-highlight}: [`step_discretize`](https://recipes.tidymodels.org/reference/step_discretize.html)
...
- Packages
  - [{]{style="color: #990000"}[osktnorm](https://cran.r-project.org/web/packages/osktnorm/index.html){style="color: #990000"}[}]{style="color: #990000"} - A Moment-Targeting Normality Transformation Based on Tukey g-h Distribution
...
- [`step_best_normalize`](https://petersonr.github.io/bestNormalize/reference/step_best_normalize.html)

  - Requires [{]{style="color: #990000"}[bestNormalize](https://petersonr.github.io/bestNormalize/){style="color: #990000"}[}]{style="color: #990000"} and has `bestNormalize` for use outside of [{tidymodels}]{style="color: #990000"}
...
- Divide by Median then log2 transform ([Source](https://amynang.github.io/posts/20240907-roots-stems-leaves/))

  ``` r
  height_standardized <- log2(height/median(height))
  ```

  - The first step makes individuals of median height equal to 1.
  - The second step pushes those individuals equal to 0 (`log2(1) = 0`)
...
- If there's zero-inflation:
  ...
  - \* Don't use these for outcome variables \*
    - See [Regression, Other \>\> Zero-Inflated/Truncated](regression-other.qmd#sec-reg-other-zeroinftrunc){style="color: green"} \>\> Continuous for details and alternatives
      - The scale of the outcome matters. The [thread](https://x.com/jiafengkevinc/status/1602403378809647104) links to a discussion of a paper on log transforms.
...
- Backtransforming Options
  ...
  - There's a backtransformation bias when exponentiating a logged variable which comes from Jenson's inequality because exponentiation is a convex function:\
    $\mathbb{E}[\exp(X)] \ne \exp(\mathbb{E}[X])$
  - Median $\rightarrow$ Use the exponential function: `exp(prediction`)
  ...
  - Mean $\rightarrow$ Use the Gaussian bias correction or Global or Local (Duan) Smearing provide unbiasted estimates of the mean
    ...
    - Gaussian Bias Correction
      ...
      - Parametric: Assumes the residuals you get from your logged outcome model are Normal
        - The Mean Error should be relatively (log scale) close to zero: `mean(residual)`
      - `predicted = exp(predicted_log + (pred_var / 2))` where [pred_var]{.var-text} is the prediction variance.
      - Obtaining [pred_var]{.var-text} depends on the model
        - `gstat::krigeST(computeVar = TRUE)`
        - `predict(lm_mod, newdata, se.fit = TRUE)`

    - Clobal (Duan) Smearing\

      $$\begin{align}
      &\hat Y = \exp(\hat \mu) \cdot \text{SF} \\
      &\text{where} \;\; \text{SF} = \frac{1}{n} \sum_{i = 1}^n \exp(e_i)
      \end{align}$$
      ...
      - [Example]{.ribbon-highlight}:
        ...
        cv_krige_log <- tib_res_cv_krige_fin |>
          summarize(
            ...
            .by = c(station_id, date) # stratifying vars
          ) |>
          mutate(
            residual_log = observed - predicted
          )

        smear_factor_global <- cv_krige_log |>
          summarize(
            smear = mean(exp(residual_log), na.rm = TRUE)
          ) |>
          pull(smear)

        cv_krige_bc <- cv_krige_log |> 
          mutate(pred_smear_global = exp(predicted) * smear_factor_global)
        ```

        - Based on [Geospatial, Spatio-Temporal \>\> Kriging \>\> Examples](geospatial-spat-temp.qmd#sec-geo-sptemp-krig-ex){style="color: green"} \>\> Example 2

    - Local Smearing

      - Same as global but you calculate different smearing factors for different groups

      - [Example]{.ribbon-highlight}: Smearing by Season
        ...
        seasonal_smear <- cv_krige_log |>
          summarize(
            smear_season = mean(exp(residual_log), na.rm = TRUE),
            .by = season
          )

        cv_krige_bt <- cv_krige_log |> 
            left_join(seasonal_smear, by = "season") |> 
          mutate(pred_smear_global = exp(predicted) * smear_season)
        ```
    ...
    - Test how types affect your performance and bias
      ...
      - [Example]{.ribbon-highlight}:
        ...
        ## Calculate Factors and Evaluate
        ...
        # seasonal smear factor
        smear_factor_seasonal <- cv_log |>
          summarize(
            smear_season = mean(exp(residual_log), na.rm = TRUE),
            .by = c(season, model)
          )
        ...
        cv_bt <- cv_log |>
          ...
          left_join(smear_factor_seasonal, by = c("model", "season")) |> 
          mutate(
            # observed on original scale
            observed_orig = exp(observed),
            # backtransform to median
            pred_median = exp(predicted),
            # gaussian lognormal correction
            pred_gaussian = exp(predicted + (pred_var / 2)),
            # global smearing
            pred_smear_global = exp(predicted) * smear_global,
            # seasonal (local) smearing
            pred_smear_seasonal = exp(predicted) * smear_season
          ) |> 
          select(-pred_var)

        # residual processing for metrics
        cv_eval <- cv_bt |>
          select(
            model,
            station_id,
          ...
          mutate(
            residual = observed_orig - prediction,
            residual_abs = abs(residual),
            residual_log = log(observed_orig) - log(prediction)
          )
        ...
        metrics_tbl <- cv_eval |>
          summarize(
            ...
            MAE = mean(residual_abs, na.rm = TRUE),
            RMSE = sqrt(mean(residual^2, na.rm = TRUE)),
            # log scale (after backtransformation)
            ME_log = mean(residual_log, na.rm = TRUE),
            RMSLE = sqrt(mean(residual_log^2, na.rm = TRUE)),
            .by = c(method, model)
          ) |>
          arrange(RMSE) |> 
        ...
        metrics_tbl
        #> # A tibble: 8 × 7
        #>   model    method                   ME    MAE   RMSE   ME_log   RMSLE
        #>   <chr>    <chr>                 <dbl>  <dbl>  <dbl>    <dbl>   <dbl>
        #> 1 krigeST  pred_smear_global    -0.449   5.58   8.22  -0.113    0.505
        #> 2 krigeST  pred_median           1.14    5.41   8.22  -0.0125   0.493
        #> 3 krigeST  pred_smear_seasonal  -0.459   5.58   8.23  -0.112    0.507
        #> 4 krigeST  pred_gaussian        -2.89    6.42   8.92  -0.249    0.552
        #> 5 Catboost pred_smear_global    -0.358   6.52   9.40  -0.145    0.570
        #> 6 Catboost pred_smear_seasonal  -0.365   6.52   9.40  -0.144    0.571
        #> 7 Catboost pred_median           2.28    6.28   9.63   0.0290   0.552
        #> 8 Catboost pred_gaussian         NaN     NaN    NaN    NaN      NaN  
        ```

        - Based on [Geospatial, Spatio-Temporal \>\> Kriging \>\> Examples](geospatial-spat-temp.qmd#sec-geo-sptemp-krig-ex){style="color: green"} \>\> Example 2
        - For kriging
          - [pred_median]{.var-text} should win on MAE since that is median-based and it does.
            - Also outperforms [pred_smear_seasonal]{.var-text} and [pred_gaussian]{.var-text} on RMSE even though those are mean-based
          - [pred_smear_global]{.var-text} outperforms [pred_median]{.var-text} on ME (bias) since it's closer to zero and ties it for RMSE (predictive performance).
          - [pred_gaussian]{.var-text} really sucks. Normal assumption must fail hard.
        - For Catboost
          - [pred_smear_global]{.var-text} has the lowest ME and lowest RMSE
          - No performance (RMSE) benefit for using [pred_smear_seasonal]{.var-text} and it has a worse bias (ME)
        - [pred_smear_global]{.var-text} is the best choice for both models for this use case (daily pollution interpolation)
...
- Packages
  - [{]{style="color: goldenrod"}[category_encoders](https://github.com/scikit-learn-contrib/category_encoders){style="color: goldenrod"}[}]{style="color: goldenrod"} - Various methods
...
- "glmm encoding with 5-fold-CV, which ranked first place for all ML algorithms except SVM (where glmm w/o CV performed best). Performance often did not improve with glmm-10CV, suggesting that 5 folds might be a good regularization default in practice." ([source](https://arxiv.org/abs/2104.00629))

  - It doesn't look like a cv process is involved in the [{embed}]{style="color: #990000"} functions though
...
- [Target Encoding]{.underline}
  ...
  - [{]{style="color: #990000"}[collinear](https://blasbenito.github.io/collinear/reference/target_encoding_lab.html){style="color: #990000"}[}]{style="color: #990000"}
    ...
    - [Rank]{.underline} (`target_encoding_rank`): Returns the rank of the group as a integer, starting with 1 as the rank of the group with the lower mean of the response variable
      - [white_noise]{.arg-text} argument might be able to used.
    - [Mean]{.underline} (`target_encoding_mean`): Replaces each value of the categorical variable with the mean of the response across the category the given value belongs to.
      - The argument, [white_noise]{.arg-text}, limits potential overfitting. Must be a value between 0 and 1. The value added depends on the magnitude of the response. If response is within 0 and 1, a [white_noise]{.arg-text} of 0.25 will add to every value of the encoded variable a random number selected from a normal distribution between -0.25 and 0.25
    - [LOO]{.underline} (`target_encoding_loo`): Replaces each categorical value with the mean of the response variable across the *other* cases within the same group.
      - The argument, [white_noise]{.arg-text}, limits potential overfitting.

  - [{{]{style="color: goldenrod"}[category_encoders](https://pypi.org/project/category-encoders/){style="color: goldenrod"}[}}]{style="color: goldenrod"}
  ...
  - Hierarchical Blending
    ...
    - Notes from
      ...
      - [Extending Target Encoding](https://towardsdatascience.com/extending-target-encoding-443aa9414cae) discusses the concepts
      - [Exploring Hierarchical Blending in Target Encoding](https://towardsdatascience.com/exploring-hierarchical-blending-in-target-encoding-fea4c59b305b) experiments with the method and provides a link to the code
...
- [Catboost Encoder]{.underline}
  ...
  pip install category_encoders
  import category_encoders as ce
  catboost_encoder = ce.CatBoostEncoder(cols=['cat_col_1', 'cat_col_2'])
  catboost_encoder.fit(X, y)
  X_transformed = catboost_encoder.transform(X_pre_encoded)
  ```
...
- [Hashing]{.underline}
  ...
  - See [Privacy](privacy.qmd#sec-priv){style="color: green"} for hashing packages
  ...
  - `textrecipes::step_dummy_hash` - Dimension Reduction. Create dummy variables, but instead of giving each level its own column, you run the level through a hashing function (MurmurHash3) to determine the column.
    - [num_terms]{.arg-text}: Tuning parameter tha controls the number of indices that the hashing function will map to. Since the hashing function can map two different tokens to the same index, will a higher value of num_terms result in a lower chance of collision.
...
- [Likelihood Encodings]{.underline}
  ...
  - [Example]{.ribbon-highlight}: Likelihood (aka Effect) Encoding
    ...
    - [step_lencode_glm](https://embed.tidymodels.org/reference/step_lencode_glm.html) fits a glm for each(?) level of the cat var uses its estimated effect as the encoded value

    - *mixed* linear model ([`step_lencode_mixed`](https://embed.tidymodels.org/reference/step_lencode_mixed.html)) and *bayesian model* ([`step_lencode_bayes`](https://embed.tidymodels.org/reference/step_lencode_bayes.html)) are also available instead of a glm

    - These type of encodings use the average estimated effect as a value for any new levels that show-up in future data
