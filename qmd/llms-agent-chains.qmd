...
- Packages
  ...
  - [{]{style="color: #990000"}[commons](https://posit-dev.github.io/commons/r/){style="color: #990000"}[}]{style="color: #990000"}, [{]{style="color: goldenrod"}[commons](https://posit-dev.github.io/commons/py/){style="color: goldenrod"}[}]{style="color: goldenrod"} ([Intro](https://posit-dev.github.io/commons/))- Helps answer data analysis questions reliably by controlling agent behavior and defining trusted calculations, using citations to back up claims, providing confidence in answers, and adding determinism into the process
  ...
  - [{]{style="color: #990000"}[routerchainR](https://github.com/lazasaurus-ai/routerchainR){style="color: #990000"}[}]{style="color: #990000"} - Route prompts to the right LLM automatically using a cheap router model. (Inspired by LangChain's[RouterChain](https://docs.langchain.com/oss/python/langchain/multi-agent/router#router))
    - Lets you define multiple model "destinations" (clients) and a small, cheap router model that classifies incoming prompts and forwards them to the most appropriate client.
    - Good for saving cost and for routing code vs prose vs short factual queries.
- Resources
  - [Large Language Model tools for R, Language Model Agents and AI assistants](https://luisdva.github.io/llmsr-book/r-pkgs.html#language-model-agents-and-ai-assistants) - R packages
  - [RDS (R Data Science) module for BMAD](https://github.com/GiulSposito/r-bmad-data-scientist) — a comprehensive framework that implements the complete lifecycle of Data Science projects in R, following modern ecosystem best practices (Tidyverse + Tidymodels + Quarto + Vetiver).
- Frameworks
...
- Classify Task Reversibility Before Automating
  - Map every step in your agent’s workflow as either reversible or irreversible.
  - Apply one rule without exception: an agent must require explicit human confirmation before executing any irreversible action.
  - Examples of irreversable actions: deleting records; initiating purchases; sending external communications; modifying permissions
