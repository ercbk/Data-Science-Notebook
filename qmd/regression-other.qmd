...
-   [**Censored Data**]{style="color: #009499"} - Arise if exact values are only reported in a restricted range. Data may fall outside this range but those data are reported at the range limits (i.e. at the minimum or maximum of the range)
    ...
    -   [Tobit Regression]{.underline} - gaussian response, assumes constant (homoskedastic variance)
        -   Log-Likelihood function\
            ![](./_resources/Regression,_Other.resources/image.png)
        ...
        -   Marginal Effect\
            ![](./_resources/Regression,_Other.resources/image.1.png)
...
-   [Zero-Inf Poisson/Neg.Binomial]{.underline}\
    ![](./_resources/Regression,_Other.resources/1-Q9dtw-96AUhEFD920nV3-w.png){width="403"}
...
-   Find location of modes: `multimode::locmodes(dat, mod0 = 2, display = TRUE)`\
    ![](./_resources/Regression,_Other.resources/1-7pU7wdm3l50-oXcFbvthig.png){width="332"}
...
-   Commonly used to overcome binomial or "megaphone-shaped" types of heteroskedacity of OLS residuals\
    ![](./_resources/Regression,_Other.resources/1-lJYcKOFR88JFo76QPSdvPQ.png){width="400"}\
    ![](./_resources/Regression,_Other.resources/1-lK2Ho8Lu_gpUXFR5bh3GPw.png){width="400"}
-   [Misc]{.underline}
    -   Also see
...
-   The residual error to be minimized becomes:\
    ![](./_resources/Regression,_Other.resources/1-TF5S8V2buIpQs4S9AWshqA.png)
    -   Where w~i~ is the weight assigned to observation i
-   $\hat \beta$ becomes\
    ![](./_resources/Regression,_Other.resources/1-ytaH-RgtlEbKNLpnhqRdsg.png)
...
-   For "megaphone-shaped" and binomial types of heteroskedacity, it's common to set the weights to equal to each observation's squared residual error\
    ![](./_resources/Regression,_Other.resources/1-AhxHEQ8fOkPK3Xg5OkRhvQ.png)
...
-   Models that are used when individual observations are correlated within groups. Often used when repeated measures (panel data) for an individual are collected over time.
    ...
    -   Other option is Generalized Linear Mixed Model (GLMM), but GLMMs require some parametric assumptions (See [Econometrics, Mixed Effects](econometrics-mixed-effects-frequentist.html#sec-econ-me-freq){style="color: green"})
