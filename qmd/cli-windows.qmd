...
-   Iterate over the output of `Get-ChildItem`
    ...
    -   [-f]{.arg-text} flag stands for format. Says to replace [{0}]{.arg-text} and [{1}]{.arg-text} in the `$ffmpegCommand` template with these properties.
        -   `[string]::Format` casts the file properties, [FullName]{.arg-text} and [Name]{.arg-text}, into strings.
...
-   Read in name of servers and ping each of them\
    ...
    -   `try` tests the connection and `catch` outputs an error message if a server fails.
        -   The error message line has an interesting syntax that I don't quite understand. I think [\$server]{.arg-text} is the input for [\$\_]{.arg-text}.
