...
- Update apt library metadata repo. Then update libraries
...
- Create a user
  ...
  ``` bash
  # Create a new user (replace 'myusername' with your preferred username)
  adduser myusername

  # Add the user to sudo group so they can use sudo
  ...
  # set as default user
  ubuntu2204.exe config --default-user myusername
  ```
...
- Check list of users
  ...
  ``` bash
  # Shows users who can actually log in
  cat /etc/passwd | grep -E '/bin/(bash|sh|zsh)$'

  # Shows all users (including system accounts)
  cat /etc/passwd | cut -d: -f1

  # Or with more details (username:home_directory:shell)
  cat /etc/passwd | cut -d: -f1,6,7

  # Shows users in the sudo group
  getent group sudo
  ```
...
- Paths
  ...
  - Structure
    ...
    ```         
    /           ← Linux root (the "WSL branch")
    ├── home/
    │   └── ercbk/       ← your Linux home (~)
    │       └── projects
    ├── usr/
    |   └── bin/
    │       └── python3.12
    └── mnt/
        └── c/           ← Windows C: drive (the "Windows branch")
