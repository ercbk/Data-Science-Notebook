...
-   Words
    ...
    $ffmpegCommand = "-i {0} -c:v libx264 -c:a aac {0}_encoded.mp4"

    # Get all files in the directory
    $files = Get-ChildItem -Path $directory -Filter "*.mkv" # Adjust filter for your file type

    # Loop through each file and apply the FFmpeg command
    foreach ($file in $files) {
      # Construct the full command with the current file path
      $fullCommand = $ffmpegCommand -f [string]::Format($file.FullName)

      # Execute the FFmpeg command
      Start-Process -FilePath "ffmpeg.exe" -ArgumentList $fullCommand -Wait -NoNewWindow
    }
