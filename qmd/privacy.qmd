...
- [{]{style="color: #990000"}[GRANDpriv](https://cran.r-project.org/web/packages/GRANDpriv/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements a GRAND (Graph Release with Assured Node Differential) method for privatizing **network data** using differential privacy
...
- [{]{style="color: #990000"}[privacyR](https://cran.r-project.org/web/packages/privacyR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Tools for **anonymizing sensitive patient and research data**. Helps protect privacy while keeping data useful for analysis. Anonymizes IDs, names, dates, locations, and ages while maintaining referential integrity.
...
- [{]{style="color: #990000"}[rsdv](https://kvenkita.github.io/rsdv/){style="color: #990000"}[}]{style="color: #990000"} - Generates **synthetic tabular data** from real datasets using Gaussian copula models, with parametric marginal selection for numerical columns and a cumulative-frequency embedding that brings categorical and boolean columns into the same joint copula. Includes built-in quality and privacy evaluation.
...
- [{]{style="color: #990000"}[staticryptR](https://cran.r-project.org/web/packages/staticryptR/index.html){style="color: #990000"}[}]{style="color: #990000"} - **Encrypt HTML Files** Using staticrypt
...
- [{]{style="color: #990000"}[trustmebro](https://kuuuwe.github.io/trustmebro/){style="color: #990000"}[}]{style="color: #990000"} - Provides functions that make it easy to inspect various subject-generated ID codes (SGIC) for plausibility.
  ...
  - **SGIC** - Often consists of a set of stable participant characteristics that enable a respondent’s answers to be matched across multiple points in time, while preserving participant anonymity (i.e., no personal identifiers such as name, date of birth, or address are collected) ([source](https://journals.sagepub.com/doi/10.1177/01632787211031625))
- [{]{style="color: #990000"}[uuidx](https://cran.r-project.org/web/packages/uuidx/index.html){style="color: #990000"}[}]{style="color: #990000"} - Generate, parse, and validate RFC 9562 UUIDs (**Universally Unique Identifiers**) from R using the Rust 'uuid' crate
  - RFC 9562 is a specification, which are 128-bit numbers intended to guarantee uniqueness across space and time. It was published in May 2024 and supersedes the earlier RFC 4122.
  - Use Cases:
    - Databases: Used as unique keys for records, improving indexing and retrieval.
    - Web Applications: Commonly employed for session identifiers and resource identification.
- [{]{style="color: #990000"}[vvbitwarden](https://cran.r-project.org/web/packages/vvbitwarden/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides functions to **securely retrieve secrets from a 'Bitwarden** Secrets Manager' vault using the 'Bitwarden CLI', enabling secret and configuration management within R packages and workflows[{]{style="color: #990000"}
:::

- Also see [Simulation, Data](simulation-data.qmd#sec-sim-data){style="color: green"}
- Tools
  - [Dotenvx](https://dotenvx.com/) ([Overview](https://dailydrop.hrbrmstr.dev/2026/02/19/drop-774-2026-02-19-secured-saved-savvied-and-sublimed/)) - Encrypts your .env files — limiting their attack vector while retaining their benefits.
    - Encrypts environment files using ECIES for secure version control and supports multi-environment configurations through a standalone binary that works with any programming language
  ...
  - [staticrypt](https://github.com/robinmoisson/staticrypt){style="color: #990000"} - Password protect a static HTML page, decrypted in-browser in JS with no dependency. No server logic needed. (Useful for HTML reports) (Also see **R package**)
    - [hrbrmstr](https://fosstodon.org/@hrbrmstr@mastodon.social/113135958788120988): "Please use long, "complex" \*passphrases\* for this tool/docs. These documents are susceptible to brute-force attacks, so you gotta make it hard for the attacker. It uses solid encryption practices (AES-256 encryption; PBKDF2 for password hashing w/decent iterations); but — unlike public/private key-based message exchanges — once the password leaks, there's no access revocation"
  - [VeraCrypt](https://github.com/veracrypt/VeraCrypt) - Encrypt files before cloud upload for extra security
    ...
    - Cloud Workflow (ChatGPT)
      1.  Create a VeraCrypt Encrypted Container
          ...
          - Set a file name and location for your container (e.g., `secure_docs.vc`).
      ...
      2.  Mount the VeraCrypt Volume and Add Files
          - Open VeraCrypt and select an unused drive letter (e.g., `Z:`).
          - Mount your encrypted volume file (e.g., `secure_docs.vc`).
      ...
      3.  Upload the Encrypted Container to the Cloud
          - Upload the `.vc` file to a cloud service like Dropbox, Google Drive, OneDrive, or a remote server via SFTP/rsync.
          - Alternatively, store the `.vc` file as a binary object (BLOB) in a database.
          - Note: The entire container is encrypted. No need to encrypt files individually.
      4.  Access or Update the Files Later
          - Download the `.vc` file from your cloud storage or database.
          - Mount the volume with VeraCrypt using your password.
...
- [{]{style="color: #990000"}[secretbase](https://shikokuchuo.net/secretbase/){style="color: #990000"}[}]{style="color: #990000"} - Fast and memory-efficient streaming hash functions, binary/text encoding and serialization.
