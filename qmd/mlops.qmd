...
-   [{]{style="color: #990000"}[futurize](https://futurize.futureverse.org/){style="color: #990000"}[}]{style="color: #990000"} - Parallelize your existing map-reduce and domain-specific calls with one function
    ...
    -   [Example]{.ribbon-highlight}: Adding a progress bar ([source](geospatial-spat-temp.qmd#sec-geo-sptemp-eda-spdep){style="color: green"})
        ...
        handlers(global = TRUE)                  # for progress bar (optional)
        handlers("progress")                     # for progress bar (optional)

        tib_vario_params_full <- purrr::map(
          c("Exp", "Mat"),
          \(i) {
            p <- progressor(along = 1:n_samples) # for progress bar (optional)
            tib_vario_params <- purrr::map2(
              asplit(block_idxs, 1),
              1:n_samples,                       # for progress bar (optional)
              \(x, iter) {
                p(sprintf("sample=%g", iter))    # for progress bar (optional)
                estim_params_vgm(
                  mod_type = i, 
                  x, 
                  dat = r5to10
