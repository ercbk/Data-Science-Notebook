...
- [{]{style="color: #990000"}[collateral](https://collateral.jamesgoldie.dev/){style="color: #990000"}[}]{style="color: #990000"} - Captures side effects like errors to prevent them from stopping execution. Once your analysis is done, you can see a tidy view of the groups that failed, the ones that returned a results, and the ones that threw warnings, messages or other output as they ran.
- [{]{style="color: #990000"}[config](https://rstudio.github.io/config/){style="color: #990000"}[}]{style="color: #990000"} - Makes it easy to manage environment specific configuration values.
- [{]{style="color: #990000"}[frontmatter](https://posit-dev.github.io/frontmatter/){style="color: #990000"}[}]{style="color: #990000"} - Extracts and parses structured metadata ('YAML' or 'TOML') from the beginning of text documents. (Uses [{tomledit}]{style="color: #990000"}, [{yaml12}]{style="color: #990000"})
...
- [{]{style="color: #990000"}[yaml12](https://posit-dev.github.io/r-yaml12/){style="color: #990000"}[}]{style="color: #990000"}, [{]{style="color: goldenrod"}[yaml12](https://posit-dev.github.io/py-yaml12/){style="color: goldenrod"}[}]{style="color: goldenrod"} - Fast, correct, and safe YAML 1.2 parsing and emitting, built in Rust.
:::

- Tools
  - [Ray](https://www.ray.io/)
  - [Weights and Biases](https://wandb.ai/site) - Experiment tracking for Python
    - Monitoring Dashboard\
      ![](_resources/MLOps.resources/wandb-dash-1.webp){.lightbox width="432"}
      - Allows you to compare the results of different experiments, the hyperparameters used, and the training curves. It also tracks system metrics such as GPU and CPU usage.
    - Integrates with Huggingface
    - Free Tier (Paid tier also includes notification, team collaboration, CI/CD)
      ...
      - Community Support
    - Supports [Hydra](https://hydra.cc/) - YAML config system for experimentation ([How to use Hydra with W&B](https://wandb.ai/adrishd/hydra-example/reports/Configuring-W-B-Projects-with-Hydra--VmlldzoxNTA2MzQw))
- Resources
  - [Applied HPC with R](https://book-hpc.ggvy.cl/)
  - [DevOps for Data Science](https://do4ds.com/)
- Python model storage formats and their issues
  - Notes from [{PyMilo}]{style="color: goldenrod"} JOSS
  - Pickle
    - Loading a pickle file may execute arbitrary code contained within it, a known vulnerability that can be exploited if the file is maliciously crafted. Serialized files are opaque binary blobs that cannot be inspected without loading
    - Pickle files often depend on specific library versions, which may hinder long-term reproducibility.
  - Open Neural Network Exchange (ONNX) and Predictive Model Markup Language (PMML) convert trained models into framework agnostic representations.
    ...
    - Exporting complex pipelines to ONNX or PMML often leads to structural approximations, missing metadata, or unsupported components, especially for customized models. As a result, the exported model may differ in behavior, resulting in performance degradation or loss of accuracy
    - ONNX uses a binary protocol buffer format that is not human-readable, which limits interpretability and makes auditing difficult. PMML, although XML-based and readable, is verbose and narrowly scoped, supporting only a limited subset of scikit-learn models
    - PMML does not provide a mechanism to restore exported models into Python, making it a one-way format that limits reproducibility across ML workflows
  - SKOPS improves the safety of scikit-learn model storage by enabling limited inspection of model internals without requiring code execution.
    - Only supports scikit-learn models, lacks compatibility with other frameworks, and does not provide a fully transparent or human-readable structure.
  - TensorFlow.js converts TensorFlow or Keras models into a JSON configuration file and binary weight files for execution in the browser or Node.js
    - Has been shown to introduce compatibility issues, performance degradation, and inconsistencies in inference behavior due to backend limitations and environment-specific faults.
    ...
    - Running complex models in JavaScript runtimes introduces memory and performance limitations, often making the deployment of large neural networks prohibitively slow or even infeasible in browser environments
- Logging
  - Also see R \>\> Code \>\> mlops \>\> manual-logging.R for more details/use cases

  - [Example]{.ribbon-highlight}: Manual Logging ([source](https://github.com/joekirincic/burst-reporting/blob/2164be1289d897070905f4d33d8a3b24306e3da0/burst-report.R#L103C1-L197C2))
    ...
    # level is the type of message (e.g. info, error)
    ...
    # use cases
    log_msg("Starting burst report generation")

    log_msg(sprintf(
      "Found %d store-month combinations with >= %d orders",
      nrow(store_months),
      MIN_ORDERS
    ))
    ...
    - See [R, Base \>\> Error Handling](r-base-r.qmd#sec-r-baser-err){style="color: green"} \>\> `tryCatch` \>\> Example 3 for usage in logging error messages
...
- [Misc]{.underline}
  - Packages
    ...
    - [{]{style="color: #990000"}[bakerrr](https://anirbanshaw24.github.io/bakerrr/){style="color: #990000"}[}]{style="color: #990000"}
      - S7 Class System: Type-safe, modern R object system
      - Parallel Processing: Efficient daemon-based parallelization via [{mirai}]{style="color: #990000"}
      - Background Execution: Non-blocking job execution with [{callr::r_bg}]{style="color: #990000"}
    ...
    - [{]{style="color: #990000"}[mori](https://shikokuchuo.net/mori/){style="color: #990000"}[}]{style="color: #990000"} - Shared memory for R objects
      - It places an R object once into OS-level shared memory and lets every process on the machine read the same physical pages directly — with no data copying between processes.

      - [Example]{.ribbon-highlight}: just use `share`
    ...
    - [{]{style="color: #990000"}[progressify](https://cran.r-project.org/web/packages/progressify/index.html){style="color: #990000"}[}]{style="color: #990000"} - Simplifies [{progressr}]{style="color: #990000"} syntax and generalizes to all major looping methods. Similar to what [{futurize}]{style="color: #990000"} does for [{future}]{style="color: #990000"}.
      - [Example]{.ribbon-highlight}: [{purrr::map}]{style="color: #990000"} and [{furrr::future_map}]{style="color: #990000"}
    ...
    - [{]{style="color: #990000"}[progressr](https://progressr.futureverse.org/){style="color: #990000"}[}]{style="color: #990000"} - Provides a minimal API for reporting progress updates in R.
      - Designed to work out-of-the-box also for parallel and distributed processing, especially with the futureverse ecosystem.

      - I have had problems with this on my Windows machine. It'll stop being able to connect to "gzfile" at random times for some reason and stop your loop with an error. I haven't looked into this too much yet though.

      - [Example]{.ribbon-highlight}: Adding a progress bar ([source](geospatial-spat-temp.qmd#sec-geo-sptemp-eda-spdep){style="color: green"})
        ...
        ``` r
        library(futurize); library(progressr)

        plan(future.mirai::mirai_multisession, workers = 10L)
        handlers(global = TRUE)                  # for progress bar
        handlers("progress")                     # for progress bar

        run_boot <- function(idxs, n_samples) {
          p <- progressor(along = 1:n_samples) # for progress bar
          tib_params_mat <- purrr::map2(
            asplit(idxs, 1),
            1:n_samples,                       # for progress bar
            \(x, iter) {
              p(sprintf("sample=%g", iter))    # for progress bar
              estim_params_vgm(
                mod_type = "Mat", 
                x, 
                dat = r5to10
          ...
          return(tib_params_mat)
        }

        tib_params <- run_boot(block_idxs, n_samples)
        ```

        - `progressor` has to be in a localized environment like a function
        - Can also use `with_progress`

        </Details>

      - [Example]{.ribbon-highlight}: Adding a progress bar to a nested loop ([source](geospatial-spat-temp.qmd#sec-geo-sptemp-eda-spdep){style="color: green"})
    ...
    - [{]{style="color: #990000"}[SafeMapper](https://zaoqu-liu.github.io/SafeMapper/){style="color: #990000"}[}]{style="color: #990000"} - Provides drop-in replacements for 'purrr' and 'furrr' mapping functions with built-in fault tolerance, automatic checkpointing, and seamless recovery capabilities.
    ...
    - [{]{style="color: #990000"}[shard](https://bbuchsbaum.github.io/shard/){style="color: #990000"}[}]{style="color: #990000"} - Zero-copy parallelism and efficient worker restarting. Focuses on three things that are often painful in R parallelism:
...
- [{]{style="color: #990000"}[future](https://future.futureverse.org/){style="color: #990000"}[}]{style="color: #990000"}
  ...
  - Making a cluster out of SSH connected machines ([Thread](https://mastodon.social/@rmflight/112051925958441474))
    ...
    - With [{renv}]{style="color: #990000"}
      ...
      future_map(...)
      ```
- [{]{style="color: #990000"}[futurize](https://futurize.futureverse.org/){style="color: #990000"}[}]{style="color: #990000"} - Parallelize your existing map-reduce and domain-specific calls with one function
  - Currently supports:
  ...
  - Domain specific: boot, caret, glmnet, lme4, mgcv, and tm

  - [Example]{.ribbon-highlight}:

    ``` r
    # use any supported future backend
    plan(future.mirai::mirai_multisession)
    plan(future.batchtools::batchtools_slurm)
    plan(future.p2p::cluster, cluster = "alice/friends")

    ys <- lapply(xs, fcn) |> futurize() # requires {future.apply}
    ys <- purrr::map(xs, fcn) |> futurize() # requires {furrr}

    b <- boot(data, statistic, R = 999) |> futurize() # requires {future}
    cv <- glmnet::cv.glmnet(x, y) |> futurize() # requires {doFuture}
    b <- mgcv::bam(y ~ s(x0, bs = bs) + s(x1, bs = bs), data = dat) |> futurize() # requires {future}
    m <- lme4::allFit(models) |> futurize() # requires {future}
    ```

  - Also see [{progressr}]{style="color: #990000"} [docs](https://progressr.futureverse.org/articles/progressr-22-parallel-processing.html#future_map---parallel-purrrmap)
- [{]{style="color: #990000"}[future.mirai](https://future.mirai.futureverse.org/){style="color: #990000"}[}]{style="color: #990000"} - Interface with futureverse with mirai backend
  - [Example]{.ribbon-highlight}
...
- [{]{style="color: #990000"}[mirai](https://mirai.r-lib.org/){style="color: #990000"}[}]{style="color: #990000"} - Modern networking and concurrency, built on nanonext and NNG (Nanomsg Next Gen), ensures reliable and efficient scheduling over fast inter-process communications or TCP/IP secured by TLS. Distributed computing can launch remote resources via SSH or cluster managers.
  - [DeepWiki](https://deepwiki.com/r-lib/mirai) - LLM to ask questions about the package
  ...
  - Daemons are persistent background processes for receiving mirai requests
    ...
    - Profiles for using different types of compute for different types of tasks
      ...
      - [.compute = "gpu"]{.arg-text} is just an example of a name that can give to a daemon profile
  ...
  - Incorporaation of OpenTelemetry enables monitoring of entire process\
    ![](_resources/MLOps.resources/para-mirai-otel-1.png){.lightbox width="432"}

    - See [OpenTelemetry](https://mirai.r-lib.org/articles/v05-opentelemetry.html) for details
    - Works seamlessly with any OpenTelemetry-compatible observability platform, including:
      - Grafana
      - Pydantic Logfire
      - Jaeger
  ...
  - [Example]{.ribbon-highlight}:
    ...
    - `library(selenider)` must be called inside of `scrape` for `selenider_session` to be found
      - I think you could have a `load_packages` function and a `load_functions` function with the necessary packages and functions you need inside the main function. Then add `load_packages` and `load_functions` to [.arg]{.arg-text}
    - `moo` will not be found unless it's included in [.args]{.arg-text}
    - [x]{.arg-text} is the input for `c(1:10)`. The input for the vector doesn't have to be the first argument. I think it just has to be one not included in [.args]{.arg-text}

  - [Example]{.ribbon-highlight}:

    ``` r
    log_msg(sprintf("Starting parallel processing with %d workers", NUM_WORKERS))
    ...
    log_msg("Parallel processing complete")
    ```
- [{]{style="color: #990000"}[purrr](https://purrr.tidyverse.org/){style="color: #990000"}[}]{style="color: #990000"}
  - Uses [{mirai}]{style="color: #990000"} for parallelization, so `daemons` and `in_parallel` needed
  ...
  - Using nested functions
    ...
    - Also see [ebtools::add_spatial_lags](https://github.com/ercbk/ebtools/blob/df7999f331850db71cec33fa0913fb3bd580fa9b/R/add-spatial-lags.R#L207) for a similar but more complex example
  ...
  - Nested Functions ([source](https://github.com/tidyverse/purrr/issues/1200#issuecomment-3061893103))
...
- [Website](https://mlflow.org/), [Repo](https://github.com/mlflow/mlflow), [Docs](https://mlflow.org/docs/latest), [R API](https://mlflow.org/docs/latest/api_reference/R-api.html), [Python API](https://mlflow.org/docs/latest/api_reference/python_api/index.html)
- Resources
...
- Packages
  ...
  - [{]{style="color: #990000"}[tarchives](https://uchidamizuki.github.io/tarchives/){style="color: #990000"}[}]{style="color: #990000"} - Bundle targets pipelines inside your R package, instead of the data itself.
    - The pipeline runs once on the user's machine and results are cached locally — ready-to-use data without rerunning anything, refreshed just by releasing a new package version

- Tools

  - [tarborist](https://open-vsx.org/extension/tylermorganwall/tarborist) - VScode/Positron extension. Static analysis for navigating and debugging complex R targets pipelines
    - Hover over a target, and it tells you how many targets depend on it.
    - Click on that number, and it lists them all.
...
- Resources
  ...
  - Building Reproducible Analytical Pipelines in R, [Ch.13](https://raps-with-r.dev/targets.html)
    - Other chapters also mention targets

- Modularizing Large Pipelines

  - Using `source` to load scripts where each may have multiple targets all associated with a particular component or step of the pipeline (e.g. loading mulitple datasets, fitting mulitple models, etc. (See [discussion](https://github.com/ropensci/targets/discussions/850))

### Set-Up {#sec-mlops-targets-setup .unnumbered}
...
- Project Structure Example\
  ![](_resources/MLOps.resources/tar-proj-struct-1.png){.lightbox width="332"}
...
- Loading multiple packages and functions ([source](https://github.com/morgankain/serology_methods_sims/blob/main/_targets.R))
...
### Operations {#sec-mlops-targests-ops .unnumbered}

- Check Pipeline

  - `tar_manifest(fields = command)`
    - lists names of targets and the functions to execute them
  - `tar_visnetwork(targets_only = TRUE)`![](_resources/MLOps.resources/tar-visnet-1.png){.lightbox width="532"}
...
- [Docs](https://docs.dask.org/en/stable/index.html)
...
- Misc
  ...
  - Available in Python ([Tutorial](https://www.jumpingrivers.com/blog/vetiver-mlops-python-deployment/))
