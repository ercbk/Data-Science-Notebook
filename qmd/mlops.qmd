...
-   [{]{style="color: goldenrod"}[daft](https://github.com/Eventual-Inc/Daft){style="color: goldenrod"}[}]{style="color: goldenrod"} - A distributed query engine for large-scale data processing using Python or SQL, implemented in Rust.
    -   [Example]{.ribbon-highlight}: Lazily aggregate delta lake files in AWS S3 ([source](https://dataengineeringcentral.substack.com/p/650gb-of-data-delta-lake-on-s3-polars))
        ...
        -   Reads the Delta Lake table using Daft's I/O API.
        -   Explicitly sets AWS credentials via environment variables.
        -   Parses the timestamp and extracts the date component.
        -   Groups by date and counts rows to compute daily totals.
        -   Writes a single CSV file using Daft’s `repartition().write_csv()`.
