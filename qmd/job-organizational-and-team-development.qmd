...
- A lot of traditional companies are not trained on working with the scientific process or even dealing with failure. Organizations that are too risk averse or don’t understand how they can use scientific method to their advantage will have issues deploying production level models with Machine Learning and extract value. ([source](https://towardsdatascience.com/organizations-machine-learning-investment-is-or-should-be-incremental-6e3658bb8b19))
  - Experimenting with non-critical use cases and building an understanding for how AI and automation add value to them is normally a better way than selecting use cases based on certain directors’ enthusiasm.
  - Setting expectations that are too high for processes that contain a non-trivial amount of error is extremely unfair to any Machine Learning Process.
...
- Open Source vs. Proprietary Software
  - Easier to hire people with open source
  - Cheaper
  - Viewed as a more legitimate option given the greater adoption of open source in such highly regulated areas as finance, healthcare, and banking.
...
- Resources
  - [Hiring Engineers](https://leanpub.com/hiring-engineers)
  - [Hiring Data Scientists and Machine Learning Engineers A practical guide](https://dshiring.com/) (\$)
...
- Push for centralization in the reporting structure, but keeping the work management decentralized.\
  ![](./_resources/Job,_Organizational_and_Team_Development.resources/image.2.png){.lightbox width="432"}
...
- [Development]{.underline}
  - Reproducibility
    ...
    - Resources
      - [Building Reproducible Pipelines in R](https://raps-with-r.dev/)
    - Packages, Libraries
  ...
  - Develop internal packages
    - Also see [Package Development](package-development.qmd#sec-pkgdev){style="color: green"}
...
- [Security]{.underline}
  ...
  - Posit Package Manager - repository management server that allows you download packages while being disconnected from the internet
    - [Posit Public Package Manager](https://packagemanager.posit.co/) - a free service that provides complete mirrors of CRAN, Bioconductor, and PyPI, including historic package snapshots. It is based on [Posit Package Manager](https://posit.co/products/enterprise/package-manager/), the professional product for teams who need greater control over how they expose R, Python, and Bioconductor packages within their organizations.
...
- If data is decentralized, create a data warehouse that houses all data\
  ![](./_resources/Job,_Organizational_and_Team_Development.resources/image.1.png){.lightbox width="432"}
...
- Work with every department team and make sure they have their own dashboard with the top set of metrics they care about.\
  ![](./_resources/Job,_Organizational_and_Team_Development.resources/image.3.png){.lightbox width="532"}
...
- [Write clear documentation of all processes and standards]{.underline}
  - Resources
    - [Code Management Guidelines: R and GitHub Starter Kit for New Team Members](https://stringhinilab.github.io/GitHubProceduresLab/)
      - Example of an onboarding manual
...
## Data Culture {#sec-job-orgdev-datcult .unnumbered}

- Notes from [Data Culture Is the Symptom, Not the Solution](https://towardsdatascience.com/data-culture-is-the-symptom-not-the-solution/)
- Focuses on the shared habits, values, and informal rules that decide how we use data to create or protect business value
- Organizations should use data culture to identify barriers that block the use of data as an asset, and then probe small interventions which gradually remove those obstacles.
- The real value of data cultural work lies in its diagnostic power:
...
- Success patterns that often appear when cultural barriers are being addressed successfully:
  - Data Awareness: Leaders and employees understand both the opportunities *and* limitations of data-driven value creation
  - Data Leadership: Leaders actively demand and champion data-informed ways of working  —  where they add value
  - Data Literacy: All members of the organization possess the relevant interdisciplinary competencies to use data in a value-creating way
  - Insight-Based Way of Working: Everyone in the organization is willing to explore and exploit data’s potential for value creation
  - Collaborative Way of Working: Data and insights are shared willingly and proactively across boundaries
  - Data Availability: Users can access the data relevant to them  —  easily, securely, and in time
- A strong data culture brings the formal, complicated structures of governance to life. Examples:
  - Data Quality :  Data governance can define a data quality rule, but a culture of accotability motivates an employee to proactively report an anomaly observation. 
...
- Process for probing barries and designing interventions
  1.  [Business Need]{.underline}: Frame your guiding organizational problem clearly. Then trace it to its root cause to avoid treating symptoms. Document the business need to ensure a shared understanding between stakeholders. Hint: becoming data-driven is not your business need, it might be your solution.
  2.  [Identify]{.underline}: Analyze the data culture in light of the business need. What cultural patterns are in its way? The six fields of analysis from above can help to spot concrete barriers.
  3.  [Sense]{.underline}: Distill, integrate and document the barriers you have discovered to focus on what matters.
  4.  [Creating]{.underline}: Design connectable interventions that are likely to address the barriers.
  5.  [Implement]{.underline}: Anchor the interventions effectively and sustainably in the organization, then observe and evaluate their impact.
  6.  [Iterate]{.underline} as often as needed until the the barriers are resolved.
- [Example]{.ribbon-highlight}
  - A healthcare business has a so-called Critical Incident Reporting System (CIRS) in place to capture and learn from near-misses and errors in patient treatment. However, it remains largely unused. The few reports submitted are often too vague for meaningful analysis.
  - [Business Need]{.underline}: To fulfill its strategic ambition and meet regulatory requirements, the hospital must reduce its error rate. This requires a systematic way to learn from incidents. The business need is not just to increase the number and quality of reports, but to create a reliable feedback loop that measurably improves patient safety.
  - [Identify]{.underline}: Interviews with medical staff reveal several powerful, unspoken rules (i.e. culture) that block the use of the CIRS:
  ...
  - [Sense]{.underline}: Distilling these findings makes the core problem visible: The organization’s context is incompatible with the desired learning ambition.
    - While management officially asks for learning from incidents, the system’s structures, incentives, and routines actually punish the required behavior. The problem is not the mindset of the staff; it is the context in which they work.
  - [Creating]{.underline}: Instead of trying to force doctors to better document incidents or initiating a ‘culture change program’, implement three precise, structural interventions are designed to address the identified barriers:
  ...
  - [Implement]{.underline}: Results:
    - Time to document incidents reduces significantly. As the first reports are handled under the new, protected process, staff see that it is safe and start providing detailed incident reports.
    - The learning team can develop and optimize a surgical checklist based on early reports, which demonstrably reduces a specific class of error.
  - [Iterate]{.underline}
    - This tangible success is widely communicated, proving the CIRS’s value, reinforcing report documentation further which creates a positive feedback loop
    - A new culture of psychological safety and data-based learning begins to emerge — not as a planned goal, but as a side effect of the tailored change in structural conditions..
...
::: {layout-ncol="2"}
![](_resources/Job,_Organizational_and_Team_Development.resources/collect-data-12.jpg){.lightbox width="332"}\

![](_resources/Job,_Organizational_and_Team_Development.resources/data-hierarchy-table-14.jpg){.lightbox width="332"}
:::
...
- Planning
  ...
  - Workflow Example: B2C (business-to-consumer) model\
    ![](./_resources/Job,_Organizational_and_Team_Development.resources/image.png){.lightbox width="632"}
...
- Also see
  ...
  - [Product Development \>\> Metrics](https://ercbk.github.io/Domain-Knowledge-Notebook/qmd/product-development.html#sec-proddev-metrics){style="color: green"}
  - [KPIs](https://ercbk.github.io/Domain-Knowledge-Notebook/qmd/kpis.html){style="color: green"}
...
- [Mission and Theory of Change]{.underline}
  ...
  - Example\
    ![](./_resources/Job,_Organizational_and_Team_Development.resources/1-Ap92XjwEYV-kuAou_2nicg.png){.lightbox width="420"}
...
- [Matrix Format]{.underline}\
  ![](./_resources/Job,_Organizational_and_Team_Development.resources/1-0PEOIumIzBMwOK5gqFANjQ.png){.lightbox width="432"}
...
- [Example]{.ribbon-highlight}: SaaS software startup\
  ![](./_resources/Job,_Organizational_and_Team_Development.resources/1-BwZR-KuOkYDhFppIF_zGKQ.png){.lightbox width="532"}
- [Example]{.ribbon-highlight}: Uber-like company\
  ![](./_resources/Job,_Organizational_and_Team_Development.resources/1-nC3CFFylcrebHiebn8Roig.png){.lightbox width="532"}

## Determining a Data Team's ROI {#sec-job-orgdev-dtroi .unnumbered}

- Misc
  - Example of model ROI calculation: [Banking/Credit \>\> Fraud](https://ercbk.github.io/Domain-Knowledge-Notebook/qmd/banking-credit.html#sec-bank-cred-fraud){style="color: green"} \>\> Misc
  - For a blurb or whatever, a project's ROI could also be presented as "hours saved."
  ...
  - [Visualizing Machine Learning Thresholds to Make Better Business Decisions](https://blog.insightdatascience.com/visualizing-machine-learning-thresholds-to-make-better-business-decisions-4ab07f823415)\
    ![](./_resources/Job,_Organizational_and_Team_Development.resources/1-vSxUtKZDg6PcRKWFU4bpSw.png){.lightbox width="432"}
...
- If department heads aren't willing to go to bat for your team, then examine three areas
      ...
      - Solutions
        ...
        - Track Decision Impact (DI) metrics (\$), which doesn't focus on intrinsic error (e.g. RMSE, MAE, F1, etc.), but rather on the quality of the decisions made that involved your data product.
          ...
          - Example with forecasting\
            ![](./_resources/Job,_Organizational_and_Team_Development.resources/0-l5tNmIVCL_C-samu.png){.lightbox width="432"}
  ...
  2.  [Planning]{.underline}
      - *Objectives* and *key results* (OKR) is a goal-setting framework for defining and tracking objectives (O) and their outcomes (KR).\
        ![](./_resources/Job,_Organizational_and_Team_Development.resources/unknown_filename.png){.lightbox width="432"}
...
![](./_resources/Job,_Organizational_and_Team_Development.resources/My-Post-Copy-2-819x1024.png){.lightbox width="332"}

- Priority is from top to bottom

### Misc

- Components should be modular and replaceable ([source](https://substack.com/app-link/post?publication_id=3044966&post_id=196528539&utm_source=post-email-title&utm_campaign=email-post-title&isFreemail=true&r=1fkzrh&token=eyJ1c2VyX2lkIjo4NjYzOTg4NSwicG9zdF9pZCI6MTk2NTI4NTM5LCJpYXQiOjE3NzgwODQyNjYsImV4cCI6MTc4MDY3NjI2NiwiaXNzIjoicHViLTMwNDQ5NjYiLCJzdWIiOiJwb3N0LXJlYWN0aW9uIn0.CpWmpJR8TqxLJnr_rnRuM-fde6ERMXvSIELy9Brnrgw))
  - Storage should be separable from compute.
  - Your transformation layer shouldn’t be locked to your orchestration layer.
  - Your query engine should be replaceable without rebuilding everything upstream.
  - The goal isn’t just to build something that works today. It’s to build something you can migrate away from smoothly when the time comes.
- Open source checklist: ([source](https://substack.com/app-link/post?publication_id=3044966&post_id=196528539&utm_source=post-email-title&utm_campaign=email-post-title&isFreemail=true&r=1fkzrh&token=eyJ1c2VyX2lkIjo4NjYzOTg4NSwicG9zdF9pZCI6MTk2NTI4NTM5LCJpYXQiOjE3NzgwODQyNjYsImV4cCI6MTc4MDY3NjI2NiwiaXNzIjoicHViLTMwNDQ5NjYiLCJzdWIiOiJwb3N0LXJlYWN0aW9uIn0.CpWmpJR8TqxLJnr_rnRuM-fde6ERMXvSIELy9Brnrgw))
  - GitHub stars: A quick way to gauge adoption and community trust.
  - Community activity: Are people actually using it, discussing it, and building with it?
  - Maintenance: How recently was the tool updated? Are issues being addressed promptly?
  - Support: If you run into a bug, can you get help? Is the community responsive?
  - Ongoing development: Are there new features or announcements, or has the project gone quiet?
...
- Also see
  - [Production, ML Monitoring](production-ml-monitoring.qmd#sec-prod-mlmon){style="color: green"}
  - [Building a Robust Data Observability Framework to Ensure Data Quality and Integrity](https://towardsdatascience.com/building-a-robust-data-observability-framework-to-ensure-data-quality-and-integrity-07ff6cffdf69)
...
![](./_resources/Job,_Organizational_and_Team_Development.resources/image.4.png){.lightbox width="284"}
...
![](./_resources/Job,_Organizational_and_Team_Development.resources/image.7.png){.lightbox width="632"}
...
![](./_resources/Job,_Organizational_and_Team_Development.resources/image.8.png){.lightbox width="632"}
...
![](./_resources/Job,_Organizational_and_Team_Development.resources/image.9.png){.lightbox width="632"}
