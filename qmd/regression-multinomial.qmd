...
-   Example: [{mlogit}]{style="color: #990000"}
...
-   Notation
    ...
    -   $TU_{tj}(i_1, i_2, \ldots, i_{t-1})$ - Total Utility (current and all future time periods) obtained from choosing alternative $j$ in period $t$, assuming the optimal choice is made in a ll future periods. (aka **Conditional Value Function**)
...
-   Bellman Equation
    ...
    -   Work backwards from the last period to the first period to calculate the overall Total Utility. At each period (except the last period), you are using the utility calculated for the future period directly after it.
    -   Choice Probabilty
    -   Assumptions
    ...
    -   Control Function estimation
        -   When to use Control Functions instead of BLP
            -   If any market shares are zero
                -   Constant terms in BLP are identified for zero market shares
...
-   Use instruments to isolate exogenous variation in explanatory variables
...
-   Utility Model
    ...
    $$
    U_{njm} = V(p_{jm}, \boldsymbol{x}_{jm}, \boldsymbol{s}_n, \boldsymbol{\beta}_n) + \xi_{jm} + \epsilon_{njm}
    $$

    -   $p_{jm}$ - Price of product $j$ in market $m$
        -   1 product can be the "outside product" or purchase nothing category which could be a vector of 0s
