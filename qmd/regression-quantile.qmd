...
- [{]{style="color: #990000"}[copulaSQM](https://cran.r-project.org/web/packages/copulaSQM/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements maximum likelihood estimation of quantile regression models allowing flexible dependence structures between error components through various copula families (e.g., Gaussian and Student-t).
  - Models heterogeneous effects across quantiles while capturing nonlinear dependence structures between variables.
...
- [{]{style="color: #990000"}[qarPI](https://cran.r-project.org/web/packages/qarPI/index.html){style="color: #990000"}[}]{style="color: #990000"} - **Prediction Intervals** for Quantile Autoregression
  - Implements percentile-based and predictive-root-based bootstrap procedures for constructing multi-step-ahead prediction intervals.
- [{]{style="color: #990000"}[QuantileOnQuantile](https://cran.r-project.org/web/packages/QuantileOnQuantile/){style="color: #990000"}[}]{style="color: #990000"} - **Quantile-on-Quantile** Regression Analysis
  - Takes into account that the relationship between the dependent and independent variable(s) may vary depending on whether the independent variable takes extreme or moderate values.
  - Models the quantile of Y (the dependent variable) as the outcome
  - Examines the effect of different quantiles of X (the independent variable)
...
- [{]{style="color: #990000"}[SelectBoost.quantile](https://cran.r-project.org/web/packages/SelectBoost.quantile/index.html){style="color: #990000"}[}]{style="color: #990000"} - A 'SelectBoost'-inspired **variable selection** workflow for sparse quantile regression.
  - Builds correlation neighborhoods, perturbs correlated predictors with a directional sampler inspired by the original 'SelectBoost' internals, refits penalized quantile regression models on the perturbed designs, and aggregates variable-selection frequencies across a path of correlation thresholds.
