...
- Resources
  - [Stan/brms prior distributions](https://mc-stan.org/docs/functions-reference/index.html)
  ...
  - Slides: [Weakly Informative Priors](https://sites.stat.columbia.edu/gelman/presentations/wipnew2_handout.pdf) (Gelman)
  - [Prior Choice Recommendations](https://github.com/stan-dev/stan/wiki/prior-choice-recommendations) (Vehtari)
- Papers
  - [Bayesian Regression Using a Prior on the Model Fit: The R2-D2 Shrinkage Prior](https://arxiv.org/abs/1609.00046)
  - [The R2D2 Prior for Generalized Linear Mixed Models](https://arxiv.org/abs/2111.10718)
...
- Prior sensitivity analysis
  - [{]{style="color: #990000"}[priorsense](https://github.com/n-kall/priorsense){style="color: #990000"}[}]{style="color: #990000"}
    - [Video](https://www.youtube.com/watch?v=TBXD3HjcIps&list=PLUEhQLNNQrgndd7_ZY-U5x9lYssl25jmd&index=14), [Thread](https://twitter.com/avehtari/status/1421041868909985793), [BW Example](https://avehtari.github.io/Bayesian-Workflow/sleep_study/sleep_study.html)
...
- Shrinkage and Regularization ([source](https://academic.oup.com/jrsssb/article/81/4/781/7048377?login=false))
  - Also see R2D2 and Horseshoe papers in Papers section
...
- Notes from [Visualization in Bayesian Workflow](https://academic.oup.com/jrsssa/article/182/2/389/7070184?login=false)

- If we specify proper priors for all parameters in the model, a Bayesian model yields a joint prior distribution on parameters and data, and hence a *prior marginal distribution for the data*, i.e. Bayesian models with proper priors are generative models.

  - We cannot fully understand the prior by fixing all except one parameter and assessing the effect of the unidimensional marginal prior. Instead, we need to assess the effect of the prior as a multivariate distribution.

- The main advantage to assessing priors based on the *prior marginal distribution for the data* is that it reflects the interplay between the prior distribution on the parameters and the likelihood. This is a vital component of understanding how prior distributions actually work for a given problem

- [It is important that this prior predictive distribution for the data has at least some mass around extreme but plausible data sets. However, there should be no mass on completely implausible data sets.]{.underline}

- [{]{style="color: #990000"}[performance::check_priors](https://easystats.github.io/performance/reference/check_priors.html){style="color: #990000"}[}]{style="color: #990000"} takes a fitted model (stan, brms, rstanarm) and the name of a predictor.

- [Example]{.ribbon-highlight}:

  ![](_resources/Bayes,_priors.resources/simppd-pm25-1.jpeg){.lightbox}

  - Left: Shows data generated from "vague" priors. The Y-Axix is shows the generated data from these priors are completely outside the range of possible PM 2.5 values
  - Center: Shows a data generated from weakly informative priors which are plausible.
  - Right: Showing both on the same chart which emphasizes how the exteme the difference is between the generated data.
...
- Misc
  ...
  - Packages
    ...
    - [{]{style="color: #990000"}[bayprior](https://ndohpenngit.github.io/bayprior/){style="color: #990000"}[}]{style="color: #990000"} - Bayesian Prior Elicitation and Diagnostics for Clinical Trials
  ...
  - Papers
    ...
    - [Expert Knowledge Elicitation: Accessing the Big Data in Experts' Brains](https://apsjournals.apsnet.org/doi/10.1094/PHYTO-06-25-0220-FI) (\$) ([Github](https://github.com/jrobledob/Expert_Elicitation_in_Plant_Pathology))
