...
- Also see
  ...
  - Styling a ggplot object produced by another package
    ...
    - [EDA, General \>\> Categorical Predictor vs Outcome \>\> Categorical Outcome](eda-general.qmd#sec-eda-gen-catout-cat){style="color: green"} \>\> Theseus Plot
      - Uses `ggplot_build` to hack the ggplot2 object and change the fill values of `geom_rects`
    - [Geospatial, Spatio-Temporal \>\> Kriging \>\> Examples](geospatial-spat-temp.qmd#sec-geo-sptemp-krig-ex){style="color: green"} \>\> Example 2 \>\> Model-specific \>\> Catboost CP Profiles
      - Looks at the ggplot2 object to change a line color and facet subplot titles.
      - Confirms facet variable by looking at the gtable(?) in the output from `ggplot_build`
...
- Packages
  ...
  - [{]{style="color: goldenrod"}[brand_yml](https://posit-dev.github.io/brand-yml/){style="color: goldenrod"}[}]{style="color: goldenrod"} - Create reports, apps, dashboards, plots and more that match your company’s brand guidelines with a single `_brand.yml` file
    ...
    - [Claude Skill](https://github.com/stephenturner/skill-brand-yml) ([intro](https://blog.stephenturner.us/p/brand-yml-claude-skill-uva-sds-quarto)) for generating brand.yml
  - [{]{style="color: #990000"}[ggannotate](https://github.com/MattCowgill/ggannotate){style="color: #990000"}[}]{style="color: #990000"} - RStudio addin that makes annotating plots essentially painless
  ...
  - [{]{style="color: #990000"}[ggpointgrid](https://github.com/nevrome/ggpointgrid){style="color: #990000"}[}]{style="color: #990000"} - Alternative to `geom_jitter` for points and [{ggrepel}]{style="color: #990000"} for annotation ([article](https://nevrome.de/blog/posts/2026-05-18-labelling-with-ggpointgrid.html))
  ...
  - [{]{style="color: #990000"}[ggview](https://github.com/idmn/ggview){style="color: #990000"}[}]{style="color: #990000"} - Allows you to lock the dimensions of your plot in the viewer and save it.
    - [Example]{.ribbon-highlight}:
      ...
      p <- 
        ...
        geom_point(aes(y = ve_mean)) +
        facet_wrap(~id) +
        ggview::canvas(width = 220,
                       height = 220 * 2/3,
                       units = "mm",
                       dpi = 300)
      p
      ...
      ```
  - [{]{style="color: #990000"}[GoldenVizR](https://cran.r-project.org/web/packages/GoldenVizR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Rule-Based Quality Checks for 'ggplot2' Visualizations Based on the 25 Golden Rules of Data Visualization
...
- Functions and Loops
  ...
  - See [Geospatial, Spatio-Temporal \>\> Kriging \>\> Starting Values \>\> Joint Models](geospatial-spat-temp.qmd#sec-geo-sptemp-krig-stval-joint){style="color: green"} \>\> Example 1 \>\> EDA

    - Uses tidyeval and patchwork + purrr::reduce
  ...
  - [Example]{.ribbon-highlight}: ([source](https://www.tjmahr.com/notes/2026-05-04-ggplot2-for-loops/))
...
- Autofixing Axis Label Overlap ([source](https://github.com/nrennie/tidytuesday/blob/3ce055ea969792929c10358ddda5f17028a8c9ad/2026/2026-06-16/20260616.R#L108-L109))
...
- Make title flush with left side of plot

  ``` r
  theme(plot.title.position = "plot)
  ```

  - Useful when Y-axis labels are long\\

- Order bars in horizontal bar plot according to variable ([source](geospatial-spat-temp.qmd#sec-geo-sptemp-krig-ex){style="color: green"} \>\> Example 2 \>\> Model-specific)
...
## Legends {#sec-vis-ggplot-legends .unnumbered}
...
- Legend title
  ...
  ``` r
  labs(fill = "Fill's legend title")
  ```

- Control the order that multiple legends are displayed (top to bottom)
...
- Reverse the order of the keys (shapes) displayed in a legend

  ``` r
  guides(size = guide_legend(reverse = TRUE)) # e.g. largest to smallest
  ```

- Change the color of the keys in a legend

  ``` r
  guides(size = guide_legend(override.aes = list(color = "#fcde9c")))
  ```

- Change the location of the legends
...
- 4.0
  ...
  - Attributes for built-in themes ([source](https://www.tidyverse.org/blog/2025/09/ggplot2-4-0-0/))\
    ...
    - [paper]{.arg-text} is background color, [ink]{.arg-text} is the foreground color (fill, color), [accent]{.arg-text} is additional [ink]{.arg-text} for special cases/highlights like `geom_smooth`
    ...
    theme_minimal() +
      theme(
        ...
        legend.box.background = element_rect(fill='#FFFDF9FF', 
                                             color = '#FFFDF9FF'),
        legend.text = element_text(family = "Anybody", size = 12),
        legend.title = element_text(family = "Anybody", size = 12),
...
- Steven Ponce ([repo](https://github.com/poncest/personal-website/blob/master/R/themes/base_theme.R), [article](https://stevenponce.netlify.app/data_visualizations/MakeoverMonday/2026/mm_2026_05.html))
...
- `theme_notebook_map`
  ...
  ``` r
  theme_notebook_map <- function(...) {
    theme_minimal() +
      theme(
        # Land/Panel Background
        panel.background = element_rect(fill = "#FFFDF9FF", color = NA),
        # Plot/Outer Background
        ...
        plot.subtitle = element_text(family = "Anybody", size = 14),
        # Cleaner look for dark maps
        axis.title = element_blank(),
        axis.text = element_blank(),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        # Legend
        legend.background = element_rect(fill = "#FFFDF9FF", color = NA),
        legend.key = element_rect(fill = "#FFFDF9FF", color = NA),
...
- For a map with a cooler palette that has blues (also for high-contrast colors like neon green, bright orange, or pure white)

  ``` r
  theme_charcoal_map <- function(...) {
    theme_minimal() +
      theme(
        # Land/Panel Background
        panel.background = element_rect(fill = "#1a1a14", color = NA),
        # Plot/Outer Background
        plot.background = element_rect(fill = "#1a1a14", color = NA),
        # Text Styles
        ...
        panel.grid.minor = element_blank(),
        # Legend
        legend.background = element_rect(fill = "#1a1a14", color = NA),
        legend.key = element_rect(fill = "#1a1a14", color = NA),
        legend.text = element_text(
          family = "Anybody", 
          size = 12,
          face = "bold"
...
- For a map with a warm color palette (e.g. gold, coral, or yellow)

  ``` r
  theme_slate_map <- function(...) {
    theme_minimal() +
      theme(
        # Land/Panel Background
        panel.background = element_rect(fill = "#1a2327", color = NA),
        # Plot/Outer Background
        ...
        panel.grid.minor = element_blank(),
        # Legend
        legend.background = element_rect(fill = "#1a2327", color = NA),
        legend.key = element_rect(fill = "#1a2327", color = NA),
        legend.text = element_text(
          family = "Anybody", 
          size = 12,
          face = "bold"
...
- Barcode Plot ([source](https://www.youtube.com/watch?v=bp1SMhLoz_M&ab_channel=PositPBC))\
