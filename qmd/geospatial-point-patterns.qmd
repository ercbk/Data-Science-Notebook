...
-   [EDA]{.underline}
    ...
    -   Estimate by the $\hat \rho$ Function\
        $$
        \lambda(u) = \rho(Z(u))
        $$

        -   Tells us how the intensity of points depends on the value of the covariate.

        -   If a covariate is predictive of intensity, then it should agree pretty well with the kernel density estimate.

        -   $\hat \rho$ is a non-parametric estimate of intensity.
...
-   [Tests]{.underline}
    ...
    -   [Cumulative Distribution Tests]{.underline}
        -   Tests whether the values of the variable at the *observed* locations are a sample from the distribution of the variable values at *all* locations in the window.
