# A/B Testing {#sec-exp-ab .unnumbered}

## Misc {#sec-exp-ab-misc .unnumbered}
...
-   Packages
    ...
    -   [{]{style="color: #990000"}[avseqmc](https://cran.r-project.org/web/packages/avseqmc/index.html){style="color: #990000"}[}]{style="color: #990000"} - Anytime-Valid Sequential Estimation of Monte-Carlo p-Values
-   Papers
    -   [Bridging Control Variates and Regression Adjustment in A/B Testing: From Design-Based to Model-Based Frameworks](https://arxiv.org/abs/2509.13944)
        -   Review of CUPED frameworks in industry and recommendations for control variables and variance estimators
...
-   Google Analytics switched in 2017 from exact user counts to estimates of user counts by default. Estimates have Stderr and have significant effects on A/B testing. Think this can be switched to exact users in settings, see link
    -   [The Perils of Using Google Analytics User Counts in A/B Testing](https://towardsdatascience.com/the-perils-of-using-google-analytics-user-counts-in-a-b-testing-e50b5dfc5f6c){.uri}
...
-   Often a good idea to limit outcome measurement to events that happen (or don't happen) within a reasonable *attribution window* from exposure or assignment.
    ...
    -   [Example]{.ribbon-highlight}: A simple exposure of seeing "sign up" for the control and "sign up today!" for the treatment
...
-   When running experiments on as few users as possible to reach a specified level of statistical significance, experiments may end up being statistically significant but with little precision in terms of the estimate\
    ...
    -   Issues with this approach (See [Universal Holdout](experiments-a_b-testing.qmd#sec-exp-ab-univhold){style="color: green"} section for solution)
...
-   Optimal Stopping
    -   Also See [Multi-Armed Bandit](experiments-a_b-testing.qmd#sec-exp-ab-mbandit){style="color: green"} section
...
-   Use gated dial-up to minimize risk to business
    ...
    -   \*\* Don't use gradual dial-up over the whole population\*\*
        -   [Example]{.ribbon-highlight}: Start with a 1% treatment, dial up to 5% after a week, then to 10%, 25%, and finally to 50%
        -   Issues:
...
## Terms {#sec-exp-ab-terms .unnumbered}

-   [**A/A Testing**]{style="color: #009499"} - Uses A/B testing to test two identical versions of a page against each other. Typically, this is done to check that the tool being used to run the experiment is statistically fair. In an A/A test, the tool should report no difference in conversions between the control and variation, if the test is implemented correctly. Useful for exposing experiments with high false positive rates. ([link](https://www.optimizely.com/optimization-glossary/aa-testing/), [article](https://towardsdatascience.com/measure-a-b-testing-platform-health-with-simulated-a-a-and-a-b-tests-7a68b800d0e1)) (Also see [Workflow \>\> Check A/A Variation](experiments-a_b-testing.qmd#sec-exp-ab-wkflw-chkaa){style="color: green"})
-   [**Exploitation**]{style="color: #009499"} - One exploits a single action without knowing the rewards of the other actions.
    -   e.g. A person finds a good restaurant and repeatedly goes to that restaurant instead of trying others
-   [**Exploration**]{style="color: #009499"} - Always exploring the other actions to acquire as much information as possible to finally choose the best action
    -   e.g. A person repeatedly goes to a new restaurant instead of always going to the same restaurant
-   [**Minimum Detectable Effect (MDE)**]{style="color: #009499"} - The smallest improvement over the baseline we are willing to detect in a controlled experiment. How large the difference should be in order to generate statistically significant outcomes (generally provided by the A/B testing platform)
-   [**Sample Ratio Mismatch (SRM)**]{style="color: #009499"} - Refers to the mismatch between the sample ratio set by the experimenter and the observed sample ratio ([example](https://www.microsoft.com/en-us/research/group/experimentation-platform-exp/articles/diagnosing-sample-ratio-mismatch-in-a-b-testing/))
...
## Workflow {#sec-exp-ab-wkflw .unnumbered}

### Misc {#sec-exp-ab-wkflw-misc .unnumbered}
...
### Pre-Experiment {#sec-exp-ab-wkflw-preex .unnumbered}
...
### Design {#sec-exp-ab-wkflw-design .unnumbered}

-   Also see [Design](experiments-a_b-testing.qmd#sec-exp-ab-dsn){style="color: green"} and [Metrics](experiments-a_b-testing.qmd#sec-exp-ab-met){style="color: green"} sections
-   Evaluation Metrics: What are our north star metrics, directly-influenced metrics, guardrail metrics, and adoption metrics in the treatment group?
    ...
    -   [Example]{.ribbon-highlight}: Add Top 10 movies/shows feature to Netflix homepage
...
-   Unit of Diversion: Shall we randomize the test based on user_id or session_id. (See [Assignment](experiments-a_b-testing.qmd#sec-exp-ab-ass){style="color: green"})
...
### Check A/A Variation {#sec-exp-ab-wkflw-chkaa .unnumbered}
...
### Run Experiment {#sec-exp-ab-wkflw-rnexp .unnumbered}
...
-   Check the Invariant Metrics & Experiment Set-up
    ...
    -   Check that the diversion of traffic is random (Also see [Assignment](experiments-a_b-testing.qmd#sec-exp-ab-ass){style="color: green"} \>\> SRM Check)
...
### Analyze the Results {#sec-exp-ab-wkflw-analres .unnumbered}
...
-   Collect the descriptive differences as well as the p-value between the treatment groups and control groups
    -   Also see
        ...
        -   [Post-Hoc Analsyis, A/B](post-hoc-analysis-ab.qmd#sec-phoc-ab){style="color: green"}
-   Do the results align with the hypothesis?
    ...
    -   Insignificant results can because of the sample size or high variance in the metrics being measured
        -   For reducing variance, see [Experiments, Planning \>\> Decreasing the Sampling Variance of the Treatment Effect](experiments-planning.qmd#sec-exp-plan-dsvte){style="color: green"}
...
### Conclusion {#sec-exp-ab-wkflw-conc .unnumbered}

-   If results are mixed/suggestive but not conclusive, run a second A/B experiement based on learnings from the first test
    -   [Example]{.ribbon-highlight}: Half of the feature variants had a positive effect, but the other half did not.
...
## Design {#sec-exp-ab-dsn .unnumbered}
...
## Assignment {#sec-exp-ab-ass .unnumbered}

-   Misc
    -   [SpookyHash](https://en.wikipedia.org/wiki/Jenkins_hash_function#SpookyHash) - This hash function meets all 4 characteristics of a good assignment algorithm + it's fast (See below for characteristics)
...
-   Potential Issues
    ...
    -   Crossovers - These are users that manage to experience both variants.
        -   [Example]{.ribbon-highlight}: Users may come across our site on mobile with the *"sign up today!"* (exposure) text, and then switch to desktop and see the *"sign up"* (control) message.
    ...
    -   Assignment Without Exposure - Due to implementation constraints, we may not be guaranteed that assigned users are actually exposed to the treatment and control.
        -   [Example]{.ribbon-highlight}: it may be that the assignment is done on the backend while exposure happens conditionally and asynchronously on the frontend -- some users may bounce in the gap between assignment and exposure, and never see the exposure (e.g. call-to-action text, *sign up today!"* ).
    -   Multiple Exposures - Once a user has been assigned, they may get exposed to the treatment and control multiple times (without crossing over).
        -   [Example]{.ribbon-highlight}: Users may visit the landing page repeatedly and see the *"sign up"* (control) or *"sign up today!"* (exposure) text multiple times before deciding to sign up.
...
-   [Example]{.ribbon-highlight}: Wish AI ([article](https://towardsdatascience.com/assign-experiment-variants-at-scale-in-a-b-tests-e80fedb2779d))\
    ...
    -   Steps
        ...
        4.  Divide R by 100. If R/100 $\ge$ exposure_rate (e.g., 10%) times 100, we assign ignore to this user, and the user will be excluded in any calculations for this experiment.
...
-   Sample Ratio Mismatch (SRM) Check\
    ![](./_resources/Experiments,_A_B_Testing.resources/1-g5-iM03tkiAoNQiB1Knrtw.png){.lightbox width="532"}
    -   Misc
...
## Potential Biases {#sec-exp-ab-pb .unnumbered}
...
-   [Condorcet Voting Paradox]{.underline} ([article](https://www.johndcook.com/blog/2022/10/31/a-b-testing-paradox/))
    ...
    -   Customer segments or groups may not have identical transitive preferences
        -   [Example]{.ribbon-highlight}: Customer Segment preferences for treatments X, Y, Z are different (all segments have equal sample sizes)
            -   Segment 1: X \> Y \> Z
            -   Segment 2: Y \> Z \> X
            -   Segment 3: Z \> X \> Y
...
-   [Primacy or Novelty Effect]{.underline}
...
-   [Interference Between Variants]{.underline}
...
-   Utilize guard-rail metrics to make sure that if your experiment metric is positive that it isn't similtaneously driving down another important metric.
    -   [Example]{.ribbon-highlight}: Recommendation module change cannabalizes usage of other tools
        -   A typical A/B test of a recommendation module commonly involves the change in the underlying machine learning algorithm, its user interface, or both.  The recommendation change significantly increased users' clicks on the recommendation while significantly decreasing users' clicks on organic search results.\
            ![](./_resources/Experiments,_A_B_Testing.resources/image.png){.lightbox width="432"}
...
## Interleaving Experiments {#sec-exp-ab-interlv .unnumbered}
...
-   [Example]{.ribbon-highlight}: Team-Draft Interleaving for recommender models\
    ![](./_resources/Experiments,_A_B_Testing.resources/image.1.png){.lightbox width="532"}
...
## Universal Hold-Out {#sec-exp-ab-univhold .unnumbered}
...
-   How long should we run each holdout for?
    ...
    -   [Example]{.ribbon-highlight}: Disney Streaming/Hulu resets the universal holdout group every 3 months (i.e. 1 quarter)
-   Power Analysis
    -   Decide on the size of the effect size you want to be able to detect
        -   [Example]{.ribbon-highlight}: A 1% change is large enough to drive a financially meaningful ad revenue impact for Hulu.
-   Holdout Then Evaluate\
    ![](./_resources/Experiments,_A_B_Testing.resources/0-g8t8Y8rBtLFTiyhE.png){.lightbox width="432"}
    -   Steps
...
## Test and Roll {#sec-exp-ab-tandr .unnumbered}
...
## Multi-Armed Bandit (MAB) Algorithms {#sec-exp-ab-mbandit .unnumbered}
...
-   [Misc]{.underline}
    ...
    -   Packages
        ...
        -   [{]{style="color: #990000"}[whatifbandit](https://cran.r-project.org/web/packages/whatifbandit/index.html){style="color: #990000"}[}]{style="color: #990000"} - Analyzing Randomized Experiments as Multi-Arm Bandits
    -   Resources
...
-   [A/B Experiment Issues]{.underline}
...
-   [Epsilon Greedy (ε-greedy)]{.underline}
    -   After an initial number of purely exploratory trials, a random lever is pulled a fraction ε of the time. The rest of the time (1 - ε), the lever with the highest known payoff is pulled.
        -   [Example]{.ribbon-highlight}: if we set ε to 0.10, 10% of the time the algorithm will explore random alternatives, and 90% of the time it will exploit the variant that is performing the best.
...
-   [Upper Confidence Bounds (UCB)]{.underline}
