...
- Also see
  ...
  - [Apache, Arrow \>\> DuckDB](apache-arrow.qmd#sec-apache-arrow-duckdb){style="color: green"}
  - [Databases, Lakes \>\> Ducklake](db-lakes.qmd#sec-db-lakes-dklk){style="color: green"}
- Packages
  ...
  - [{]{style="color: #990000"}[RDucks](https://sounkou-bioinfo.github.io/Rducks/){style="color: #990000"}[}]{style="color: #990000"} - Registers R UDFs as DuckDB SQL functions using a DuckDB C extension
    - [Example](https://bsky.app/profile/did:plc:hmk4f5yf3ohtsvw32sbtwvk3/post/3mma2ucmib226) within [{duckplyr}]{style="color: #990000"} using its `dd$` method
- Resources
  ...
  - [DuckDB R Tips](https://github.com/eriksquires/DuckDB-With-R)
    - Using a config for development and production
    - Concurrency and multiple read but single write friction point
    - dbplyr vs. duckplyr
    - DuckDB with a Remote DB (e.g. postgres)
...
- Call duckdb SQL functions using `dd$` ([docs](https://duckplyr.tidyverse.org/dev/articles/duckdb.html)) within dplyr pipelines
...
- Misc
  ...
  - Also see [Apache, Arrow \>\> Geospatial](apache-arrow.qmd#sec-apache-arrow-geo){style="color: green"}
