...
* Create Cluster: EC2 method (user-managed)
* Create Cluster: EC2 method with SSH access
* Create Cluster: Fargate method (AWS-managed)
...
* Update service to include autoscaling
* Troublshooting
...
* **Elastic Container Registry (ECR)**
	...
	* In console
		* hit refresh (mid-right) to see that the image is loaded into the repo
...
* **Create a CodeBuild project to build the CodeCommit repo into a docker container**
	...
	* Goto IAM console -- left panel -- Roles
		* When the "build project" was created a role was also created
			...
			3. Search for _AmazonEC2ContainerRegistryPowerUser_
...
* **Create Cluster: EC2 method with SSH access (not possible with a Fargate cluster) and connect to it**
	1. Find your ssh public key
		1. go into git bash and type "cat ~/.ssh/id\_ed25519.pub"
			* options
				* id\_rsa.pub
				* id\_ecdsa.pub
				* id\_ed25519.pub
		...
		* right-click EC2 Instance id and open in new tab
			...
			* click security group link (lower left)
				...
				* click edit
					...
					* click save
	5. Open terminal
		* ssh -i ~/.ssh/id\_rsa ec2-user@<public ip address>
...
* **Create Cluster: Set-up instructions for running containers using** **Fargate method**
	* Search for ECS
...
* **Update task definition to share data volumes between containers (fargate cluster example)**
...
* **Running a Task with an available task definition**
	* left panel -- Clusters
...
* **Create application load balancer (ALB) (also see AWS notebook -- configure load balancer ports)**
	* search ec2
		...
		* Search ec2 -- left panel -- load balancing -- load balancer 
			...
			4. test it out by clicking description tab (mid left)
				*  get DNS (Domain Name Service) name
					* example [blahblahamazonaws.com](http://blahblahamazonaws.com/)
...
* **Add ECS service and task (also see create task definition and run task sections above)**
