...
## Misc {#sec-diag-llm-misc .unnumbered}
...
- [groundlens](https://github.com/groundlens-dev/groundlens) - Geometric LLM grounding verification — deterministic, auditable, no second LLM. Python library for measuring how faithfully model outputs reflect their sources.
  - It embeds the question, the candidate answer, and the source on a vector hypersphere and reads the angles between them. A grounded answer sits close to its source; a hallucinated one drifts toward the question and away from the source.
  - Metrics
    - The Semantic Grounding Index (SGI) is a ratio of two such angles
    - DGI is a distributional grounding measure calibrated on held-out grounded pairs.
    - Both are pure geometry over a fixed encoder, so they are deterministic by construction.
  - Geometric verification can discriminate hallucinations. On the HaluEval QA benchmark, scoring grounded against hallucinated answers, a combined metric (SGI + DGI) was able to discrimated successfully 94.9% (CI \[92.6%, 97.1%\]) ([source](https://towardsdatascience.com/design-loops-not-prompts/))
