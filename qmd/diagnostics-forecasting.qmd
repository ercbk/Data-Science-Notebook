...
- **Response Residuals** - The difference between the observations and the fitted values (equivalent to h-step forecasts).

- **Innovation Residuals** - If the outcome variable has been transformed (e.g. log), then these are the residuals on the transformed scale. If there was no transformation, then the (regression) residuals equal the innovation residuals. (Also used for multiplicative ETS model, Feed Forward Neural Networks)
...
- **Regression Residuals** - Equal to the original data minus the effect of the regression variables.

  - Also available for regression models with ARIMA errors
  - If there are no regression variables, the errors will be identical to the original series (possibly adjusted to have zero mean).
...
- Packages
  - [{]{style="color: #990000"}[RCtest](https://cran.r-project.org/package=RCtest){style="color: #990000"}[}]{style="color: #990000"} - Implements a comprehensive suite of statistical tests for evaluating the accuracy of forecasting models against a benchmark
...
- VIF ([source](https://otexts.com/fpp2/causality.html#multicollinearity-and-forecasting))
  - Doesn't typically affect forecasts, but does affect coefficients. So anything regarding them (e.g. std.errors, t-stats, p-values, predictor importance)
  - [If future values of a predictor, which is involved in the multicollinearity, are outside the range of the fitted data, then forecasts do become unreliable]{.underline}.
