...
- [{]{style="color: #990000"}[dtms](https://github.com/christiandudel/dtms){style="color: #990000"}[}]{style="color: #990000"} ([Papers](https://cran.r-project.org/web/packages/dtms/index.html)) - Discrete-Time Multistate Models
...
- [{]{style="color: #990000"}[hdMTD](https://cran.r-project.org/web/packages/hdMTD/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2509.01808)) - An R Package for High-Dimensional Mixture Transition Distribution Models
...
- [{]{style="color: #D55E00"}[HiddenMarkovModels](https://gdalle.github.io/HiddenMarkovModels.jl/stable/){style="color: #D55E00"}[}]{style="color: #D55E00"} - A Julia package for simulation, inference and learning of Hidden Markov Models with discrete states and discrete time.
...
- Resources
  - [Statistical Rethinking Lecture B10 - Hidden Markov Models](https://www.youtube.com/watch?v=fuonUuKTOl4)
...
- Misc
  - Notes from
    ...
    - [Tidy simulation: Designing robust, reproducible, and scalable Monte Carlo simulations](https://arxiv.org/abs/2509.11741) ([Github](https://github.com/vankesteren/tidy_simulation))
  - Packages
  ...
  - Papers
    - [Using simulation studies to evaluate statistical methods](https://arxiv.org/abs/1712.03198)
...
- Components
  - **Simulation Grid**: A tidy data frame indicating the settings for each iteration of a simulation.
    ...
    - Variables (aka Factors)
      ...
      - Continuous Values (e.g. as standardized effect size or proportion of variation (R^2^))
    ...
    - Since each row in the simulation grid should represent a single simulation according to the tidy data principles, the full factorial design should be stacked $N$ times, where $N$ is the number of iterations. `expand_grid` functions can carry this out.
    - Many millions of rows are not uncommon in larger simulations, so storage of the grid in an efficient file format like parquet is recommended where packages with lazy exectuion (duckdb, polars) can be taken advantage of.
  ...
  - **Generation Function**: Takes in parameters from the simulation grid and outputs a simulated dataset – ideally in a tidy format.
    ...
    - R: It's built-in distribution functions, e.g. `rnorm`
    - Python: `numpy.random` and `scipy.stats`
    - Julia: The Distributions.jl package
  ...
  - **Results Table**: A tidy data frame with one row per simulation setting in the simulation grid, and one or more columns containing the outcomes of interest.
    ...
    - Choose structured column names, which indicate the analysis setting and the outcome value of interest, separated by strings which are easy to parse.
      ...
      - Example: \[outcome\] \[correction\] \[value\], e.g. [changescore_uncorrected_pvalue]{.var-text}
- Optimizations
...
- [Example 4]{.ribbon-highlight}: Analyze the treatment effect in a RCT with a binary treatment and two measurement occasions: pre-treatment and post-treatment
  - Python
    ...
    - Results in 352000 individual simulations even for this small simulation with 500 repetitions and only four factors.
    ...
    - Base Function
      ...
      - pre and [post]{.var-text} are continuous measures and I'm guessing [treated]{.var-text} is a logical?(`//2` is integer division)

    - If multiple DGPs need to be tested, then create subfunctions for each DGP and call them in a conditional

      ``` python
      def generate_data(process: str, sample_size: int, effect_size: float, seed: int):
        if process == "a":
          return generate_a(sample_size, effect_size, seed)
        if process == "b":
          return generate_b(sample_size, effect_size, seed)
        if process == "c":
          return generate_c(sample_size, effect_size, seed)
      ```

    - Depending on the experiment, include argument tests and error handling

      ``` python
      def generate_data(num_groups: int, sample_size: int):
       if num_groups >= sample_size:
        raise ValueError("Sample size should be larger than groups.")
      ```

    - Flexible outputs may be needed depending other required input format of the analysis function, e.g. tuple
    ...
    - The eigenvalues serve as an indicator for multicollinearity and can be used to explain the failure of the model for a certain generated dataset or group of parameters
    ...
    ## Results Parallel Chunked

    Run the simulation in a sliced way using multiprocessing and produce Component 4: the results table
