...
- Packages
  - [{]{style="color: #990000"}[csdm](https://cran.r-project.org/web/packages/csdm/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides estimators and utilities for large panel-data models with cross-sectional dependence (plus related inference and diagnostics)
    - Includes mean group (MG), common correlated effects (CCE) and dynamic CCE (DCCE) estimators, and cross-sectionally augmented ARDL (CS-ARDL) specifications,
  ...
  - [{]{style="color: #990000"}[fixest](https://lrberge.github.io/fixest/){style="color: #990000"}[}]{style="color: #990000"} - Fast estimation, has parallel option, glm option and many other features
  ...
  - [{]{style="color: #990000"}[LeaveOutKSS](https://cran.r-project.org/web/packages/LeaveOutKSS/index.html){style="color: #990000"}[}]{style="color: #990000"} - Leave-Out Variance Component Estimation for Two-Way Fixed Effects Model
  ...
  - [{]{style="color: #990000"}[plm](https://github.com/ycroissant/plm){style="color: #990000"}[}]{style="color: #990000"} - Functions for model estimation, testing, robust covariance matrix estimation, panel data manipulation and information.
  ...
  - [{]{style="color: #990000"}[selectTWFE](https://cran.r-project.org/package=selectTWFE){style="color: #990000"}[}]{style="color: #990000"} - Estimates both a vanilla two-way fixed effects (TWFE) model and an extended TWFE (ETWFE) model, then selects between them using Cochran's Q test for heterogeneity.
    - When ETWFE wins, reports the heterogeneity fraction (I-squared) and cohort-time estimates with empirical Bayes shrinkage and Bonferroni multiplicity correction.
  - [{]{style="color: #990000"}[twfeivdecomp](https://cran.r-project.org/web/packages/twfeivdecomp/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements a decomposition of the two-way fixed effects instrumental variable estimator into all possible Wald difference-in-differences estimators.
  ...
  - [{]{style="color: #990000"}[xtbhst](https://cran.r-project.org/web/packages/xtbhst/index.html){style="color: #990000"}[}]{style="color: #990000"} - Bootstrap Slope Heterogeneity Test for Panel Dat
    - Tests the null hypothesis that slope coefficients are homogeneous across cross-sectional units.
    - Provides both standard and adjusted Delta statistics with bootstrap p-values.
    - Supports partialling out of control variables and cross-sectional averages for dealing with cross-sectional dependence.
  - [{]{style="color: #990000"}[xtdml](https://cran.r-project.org/web/packages/xtdml/index.html){style="color: #990000"}[}]{style="color: #990000"} - Double Machine Learning for Static Panel Models with Fixed Effects
- Papers
...
- Perform sensitivity analysis when grouping variable(s) may be colliders ([Paper](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=7268258), [Thread](https://bsky.app/profile/p-hunermund.com/post/3mtbjvpk5xk23))
  - FE estimators can generate unintended bias when the FE grouping structure is itself shaped by variables related to both the explanatory variable and unobserved determinants of the outcome or the outcome variable itself.
  - When group assignment depends on both the explanatory variable and unobserved outcome determinants, conditioning on group-level fixed effects can induce correlation between the explanatory variable and the regression error term.
  - The same logic can arise in two-way and high-dimensional FE settings when the absorbed groups are generated by endogenous sorting, group assignment, or equilibrium processes.
  - In many settings, attenuated point estimates after adding FE are interpreted as evidence that confounding variation has been successfully removed. Similar attenuation patterns can also arise from collider bias when additional FE condition on collider-defined group membership.
  - [Example]{.ribbon-highlight}\
    ![](_resources/Econometrics,_Fixed_Effects.resources/pitfalls-collider-bias-ex1-1.webp){.lightbox width="432"}
    - A study of how entrepreneurial team composition affects start-up performance. Incubators select start-ups based on team composition, but also on private signals about idea quality. Adding incubator fixed effects seems like the obvious way to account for differences across programs.
    - However, because incubator assignment depends on both team composition and unobserved idea quality, including incubator fixed effects conditions on a collider-defined grouping structure. This conditioning can induce correlation between team composition and unobserved determinants of performance
  - Diagnostic
    - Aggregate bias increases when the collider exhibits high between-group variance and limited within-group variance
    - The Gini coefficient approaches zero when the within-group variance is evenly distributed and approaches one when the within-group variance is highly concentrated.\
      $$
      \text{Gini} = \frac{\sum_{g=1}^G \sum_{h=1}^G |v_g - v_h|}{2G \sum_{g=1}^G v_g}
      $$
      - $G$ is the number of groups
      - $v_g = \text{Var}(X \;|\; G = g)$ is the within-group variance of $X$ (regressor of interest) in $G$
        - I think $v_h$ is the within-group variance of another group that is *not* $g$ or since this is a summation, it probably doesn't matter if $g = h$.
      - The Gini coefficient approaches zero when the within-group variance is evenly distributed and approaches one when the within-group variance is highly concentrated.
    - Simulations
      - In the low-Gini scenarios (Gini ≤ 0.25), bias reaches values as large as 2.0 and is widely dispersed, regardless of the covariance structure. This implies that low-Gini configurations are compatible with large bias across a wide range of covariance structures.
      - In the high-Gini scenarios (Gini ≥ 0.75), the upper tail of the bias distribution is compressed, and extreme bias values appear less frequently. That is, as the Gini coefficient increases, the distribution of bias is reduced and becomes more concentrated.
  - The procedure doesn't detect Collider Bias, but instead assesses whether the coefficient of interest is sensitive to changes in the distribution of identifying variation across FE groups.
  - The diagnostics reveal which groups drive the FE estimate but the causal structure determines whether those influential within-group comparisons are likely to induce spurious correlation.
  - Sensitivity Analysis Procedure
    1.  Assess the causal origin of the FE grouping variable.
        - Determine whether the grouping variable is plausibly predetermined or whether it may be downstream of the regressor and latent determinants of the outcome.
        - Some examples of concerning FE group formation processes include sorting, assignment, rankings, thresholds, platform tiers, risk scores, or other data-driven classifications.
    2.  Compute within-group identifying variation.
        - For the regressor of interest, compute $v_g = \text{Var}(X \;|\; G_i = g)$ for each FE group.
        - Groups with $v_g \approx 0$ contain little within group variation in $X$ and therefore receive little FE weight. Their local slope or bias ratios may be unstable, but they typically have limited influence on the aggregate FE estimate.
    3.  Summarize variance and weight concentration.
        - Report the Gini coefficient of the within-group variances. This statistic summarizes whether identifying variation in X is evenly distributed across FE groups or concentrated in a small number of groups.
        - Three other statistics may also be useful to include:
          - Maximum FE weight:\
            $$
              \begin{align}
              &\max \; \{\omega_g\} \\ 
              &\text{where} \;\; \omega_g = \frac{p_g v_g}{\sum_{h\in G} p_h v_h}
              \end{align}
              $$
          - $p_h$ is $\text{Pr}(G_i = g)$ is the "group share." ( ¯\\\_(ツ)\_/¯ )
          - $\sum \omega_g = 1$
          - Describes how much each group contributes to the aggregate FE estimate and when groups are equal sized, \$\\omega_g \\propto v_g \$
          - See footnotes on pg 24 in paper for normalization procedure to use in visualizations.
          - Effective number of identifying groups: $N_{\text{eff}} = 1\;/\; \sum_{g\in G} \; \omega_g^2$
          - Top-K identifying share: $S_K = \sum_{g\in G_K} \; \omega_g$
            - $G_K$ denotes the set of the $K$ groups with the largest FE weights.
    4.  Inspect the distribution of within-group variances and FE weights.
        - Plot or tabulate both the within-group variances and the FE weights.
        - Ranked-weight plots show whether individual groups receive unusually high identifying weight.
        - Cumulative-weight plots show how quickly identifying weight accumulates among the highest-weight groups
    5.  Conduct transparent sensitivity and influence checks.
        - Re-estimate the model under alternative group definitions, coarser FE partitions, and leave-out checks for influential high-weight groups.
