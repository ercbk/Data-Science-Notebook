...
- Packages
  - [{]{style="color: #990000"}[ggplot2qgis](https://yutannihilation.github.io/ggplot2qgis/){style="color: #990000"}[}]{style="color: #990000"} - Takes a ggplot2 plot whose layers are backed by sf objects and writes a QGIS project.
    - Along with [Kumoy](https://www.kumoy.io/en#pricing) allows you to easily publish maps online
  ...
  - [{]{style="color: #990000"}[mapgl](https://walker-data.com/mapgl/){style="color: #990000"}[}]{style="color: #990000"} - WebGL Maps in R with Mapbox and MapLibre
    - See [Geospatial, General \>\> Grid Systems \>\> Hexagonal Grids](geospatial-general.qmd#sec-geo-gen-gridsys-hex){style="color: green"} \>\> Example 2 for details on how to create a nice title box and pop-ups with html
    - [{]{style="color: #990000"}[deckglgeoarrow](https://r-spatial.github.io/deckglgeoarrow/){style="color: #990000"}[}]{style="color: #990000"} - Provides functionality to efficiently visualize potentially very large geospatial data as Deck.gl layers on top of a maplibre/mapbox map created with package mapgl.
  ...
  - [{]{style="color: #990000"}[tmapverse](https://cran.r-project.org/web/packages/tmapverse/index.html){style="color: #990000"}[}]{style="color: #990000"}
    - Resources
      - [Geocomputation with R, Ch.9](https://r.geocompx.org/adv-map#static-maps)
      - [Elegant and informative maps with tmap](https://r-tmap.github.io/tmap-book/) (syntax prior to tmap4)
      - [Theme-like settings](https://r-tmap.github.io/tmap/articles/adv_options)
      - Use `cols4all::c4a_palettes` for a list of available palettes.
        - Insert hyphen in front of the palette string to reverse the color scale, e.g. ["-scico.batlow"]{.arg-text}
    ...
    - [{]{style="color: #990000"}[tmap.cartogram](https://cran.r-project.org/web/packages/tmap.cartogram/index.html){style="color: #990000"}[}]{style="color: #990000"} - A cartogram is a type of thematic map in which geographic areas are resized or distorted based on a quantitative variable, such as population. The goal is to make the area sizes proportional to the selected variable while preserving geographic positions as much as possible
    - [{]{style="color: #990000"}[tmap.glyphs](https://cran.r-project.org/web/packages/tmap.glyphs/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides new layer functions to 'tmap' for drawing glyphs. A glyph is a small chart (e.g., donut chart) shown at specific map locations to visualize multivariate or time-series data
    ...
    - [{]{style="color: #990000"}[tmap.networks](https://cran.r-project.org/web/packages/tmap.networks/index.html){style="color: #990000"}[}]{style="color: #990000"} - Extension to 'tmap' for Creating Network Visualizations
    - [{]{style="color: #990000"}[tmaptools](https://r-tmap.github.io/tmaptools/){style="color: #990000"}[}]{style="color: #990000"} - Functions to supply the workflow to create thematic maps, e.g. read shape files, set map projections, append data, calculate areas and distances, and query OpenStreetMap (extension, not technically part of the verse)
- Resources
...
- openstreetmap
  - 3-D buildings in [{mapgl}]{style="color: #990000"}:
    - Default for mapbox
    - maplibre: `maplibre(style = openfreemap_style("liberty"))`
  - [{]{style="color: #990000"}[cartographr::get_osmdata](https://da-wi.github.io/cartographr/reference/get_osmdata.html){style="color: #990000"}[}]{style="color: #990000"} - Retrieves OpenStreetMap (OSM) data based on geographic coordinates or a bounding box.
    - Allows for the specification of distances and aspect ratios to define the area of interest.
    - Returns data in simple features (sf) format and has options to operate quietly.
    - Returned features for various OSM tags, e.g. landuse (feature): forest, farmland (tags):
      - highway, water, building, natural, amenity, man-made, railway, place, boundary, waterway, landuse, and leisure
- [{tmap}]{style="color: #990000"} themes
  - Examples

    - `tmap_theme_slate`: [Association, Spatial \>\> Global \>\> Examples](association-spatial.qmd#sec-assoc-spat-global-ex){style="color: green"}
    - `tmap_theme_charcoal`: [Association, Spatial \>\> Local \>\> Examples](association-spatial.qmd#sec-assoc-spat-local-ex){style="color: green"}
    - `tmap_theme_notebook`: [Association, Spatial \>\> Local \>\> Examples](association-spatial.qmd#sec-assoc-spat-local-ex){style="color: green"}

  - Make sure to:

    - **Reset options** before trying new setttings: `tmap_options_reset`
    - **Have plot viewer panel dimensions set at a large enough size** before running the plot code. If it's too small, you might get warnings, settings won't render correctly — the rendering is very finnicky.
    - When saving to image, **check the positioning of the components (e.g. legend) in the save-view** before saving. The save-view panel and plot view panel will probably show different spacing, positioning of components, etc.

  - `tmap_theme_charcoal` (theme for cool palettes)
    ...
    ``` r
    tmap_theme_charcoal <- function(...) {
      tmap_options(
        component.autoscale = FALSE,
        frame = FALSE,
        bg.color = "#191a1a",
        outer.bg = TRUE,
        outer.bg.color = "#191a1a",
        text.fontfamily = "Anybody",
        ...
        legend.title.size = 1,
        legend.title.fontfamily = "Anybody",
        legend.frame = FALSE,
        legend.bg.color = "#191a1a",
        title.color = "white",
        title.fontfamily = "Anybody",
        panel.label.size = 1.25,
        panel.label.fontfamily = "Anybody",
        panel.label.fontface = "bold",
        panel.label.color = "white",
        panel.label.bg = FALSE,
        panel.label.frame = TRUE,
        panel.label.height = 3,
        panel.label.frame.color = "#191a1a",
        ...
      )
    }
    ```

    - The panel label frame is necessary so the subplot titles in the top row don't get really close to the subtitle. Tried every type of margin, padding setting possible and nothing else worked.
    - Can't fix title text size or fontface, because the title and subtitle both use the same function for some reason (`tm_title`). Authors are anti-subtitle I guess.

    </Details>

  - `tmap_theme_slate` (theme for warm palettes)
    ...
    ``` r
    tmap_theme_slate <- function(...) {
      tmap_options(
        component.autoscale = FALSE,
        frame = FALSE,
        bg.color = "#1a2327",
        outer.bg = TRUE,
        outer.bg.color = "#1a2327",
        text.fontfamily = "Anybody",
        legend.text.color = "white",
        legend.text.fontface = "bold",
        legend.text.size = 0.85,
        ...
        panel.label.frame = TRUE,
        panel.label.height = 3,
        panel.label.frame.color = "#1a2327",
        ...
      )
    }
    ```

    - See `tmap_theme_charcoal` for details
  ...
  - `tmap_theme_notebook`
    ...
    ``` r
    tmap_theme_notebook <- function(...) {
      tmap_options(
        component.autoscale = FALSE,
        frame = FALSE,
        bg.color = "#FFFDF9FF",
        outer.bg = TRUE,
        outer.bg.color = "#FFFDF9FF",
        text.fontfamily = "Anybody",
        ...
        panel.label.frame = TRUE,
        panel.label.height = 3,
        panel.label.frame.color = "#FFFDF9FF",
        ...
      )
    }
    ```
...
- Counts run the risk that the information is related with the polygon size, it is better to use *intensive variables*. As such they are (still) associated with the polygon area but their *support* is associated with the population total. (See [Geospatial, Analysis \>\> Terms](geospatial-analysis.qmd#sec-geo-anal-terms){style="color: green"})

  - e.g. For a population count variable, one would divide by the polygon area to show the (intensive) variable population density,
  - e.g. Disease incidences over a time period are usually converted to probabilities or incidence rates by dividing over the population size of the associated polygons.
...
- [Example]{.ribbon-highlight}: [{tmap}]{style="color: #990000"} ([source](https://dosull.github.io/posts/2024-11-16-tmap-vs-ggplot/tmap4-vs-ggplot2.html#choropleth-maps-tmaps-killer-app))\
  ![](_resources/Geospatial-Mapping.resources/choro-tmap-basic-1.png){.lightbox width="432"}
  ...
  - Use `tmap_mode("view")` at the top of the code to get a leaflet interactive map

- [Example]{.ribbon-highlight}: [{ggplot}]{style="color: #990000"} ([source](https://dosull.github.io/posts/2024-11-16-tmap-vs-ggplot/tmap4-vs-ggplot2.html#choropleth-maps-tmaps-killer-app))\
  ![](_resources/Geospatial-Mapping.resources/choro-gg-projected-1.png){.lightbox width="432"}
  ...
  ggplot() +
    ...
    scale_fill_continuous_c4a_seq(
      palette = "brewer.reds") +
    coord_sf(datum = st_crs(ak)) # projected coords
  # theme_void() # remove grid
  ```

  - [{ggplot2}]{style="color: #990000"} defaults are geographic degrees regardless of the data and the color scale goes from dark (low values) to light (high values).
  - `scale_fill_continuous_c4a_seq` is from [{]{style="color: #990000"}[cols4all](https://cols4all.github.io/cols4all-R/){style="color: #990000"}[}]{style="color: #990000"} which is what [{tmap}]{style="color: #990000"} uses

- [Example]{.ribbon-highlight}: Scale Intervals ([source](https://dosull.github.io/posts/2024-11-16-tmap-vs-ggplot/tmap4-vs-ggplot2.html#choropleth-maps-tmaps-killer-app))\
  ![](_resources/Geospatial-Mapping.resources/choro-tmap-scale-intervals-1.png){.lightbox width="582"}
  ...
  - More difficult to do this in [{ggplot2}]{style="color: #990000"}. You'd have to manually create the scale and the nice interval labels in the legend.
...
- [Example]{.ribbon-highlight}: [{mapview}]{style="color: #990000"} Over 65 in Iowa by census tract\
  ...
  ``` r
  library(mapview); library(viridisLite)
...
## Raster {#sec-geo-map-rast .unnumbered}

- [Example]{.ribbon-highlight}: Basic ([source](https://dosull.github.io/posts/2024-11-16-tmap-vs-ggplot/tmap4-vs-ggplot2.html#raster-data-tmaps-other-killer-app))\
  ![](_resources/Geospatial-Mapping.resources/raster-tmap-basic-1.png){.lightbox width="432"}
...
- [Example]{.ribbon-highlight}: Hillshade ([source](https://dosull.github.io/posts/2024-11-16-tmap-vs-ggplot/tmap4-vs-ggplot2.html#raster-data-tmaps-other-killer-app))\
  ![](_resources/Geospatial-Mapping.resources/raster-tmap-hillshade-1.png){.lightbox width="432"}
