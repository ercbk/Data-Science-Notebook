...
- Tools
  - [Rclone](https://rclone.org/) - A command-line program to manage files on cloud storage. It is a feature-rich alternative to cloud vendors' web storage interfaces. [Over 70 cloud storage products](https://rclone.org/#providers) support rclone including S3 object stores
    - [{]{style="color: #990000"}[rcloner](https://cran.r-project.org/web/packages/rcloner/index.html){style="color: #990000"}[}]{style="color: #990000"} - Interface to 'rclone' Cloud Storage Utility
    - Beefed up, performant replacement for [rsync](https://rsync.samba.org/). If you don't want backing-up data or other tasks taking up computer resources, then rsync may still be the way to go. ([article](https://veronicaexplains.net/everyday-rsync/))
...
## Terms {#sec-db-lakes-term .unnumbered}

- [**Medallion Architecture**]{style="color: #009499"} - A data design pattern used to logically organize data in a lakehouse, with the goal of incrementally and progressively improving the structure and quality of data as it flows through each layer of the architecture (from Bronze ⇒ Silver ⇒ Gold layer tables). ([source](https://www.databricks.com/glossary/medallion-architecture))\
  ![](_resources/DB,_Engineering.resources/medallion-architecture-1.png){.lightbox width="532"}

  - Also see
    - [Ducklake \>\> R](db-lakes.qmd#sec-db-lakes-dklk-r){style="color: green"} \>\> Example 2
  - Medallion architectures are sometimes also referred to as "**multi-hop**" architectures.
  - Compatible with the concept of a data mesh. Bronze and silver tables can be joined together in a "one-to-many" fashion, meaning that the data in a single upstream table could be used to generate multiple downstream tables
  - [Bronze]{.underline}: Data from external source systems.
  ...
  - [Silver]{.underline}: Data from the Bronze layer is matched, merged, conformed and cleansed ("just-enough") so that the Silver layer can provide an "Enterprise view" of all its key business entities, concepts and transactions. (e.g. master customers, stores, non-duplicated transactions and cross-reference tables).
    - Enables self-service analytics for ad-hoc reporting, advanced analytics and ML.
    - Only minimal or "just-enough" transformations and data cleansing rules are applied while loading the Silver layer
      - Has more 3rd-Normal Form like data models.
      - Data Vault-like where write-performant data models can be used in this layer
  - [Gold]{.underline}: Consumption-ready "project-specific" databases
    - For reporting and uses more de-normalized and read-optimized data models with fewer joins. Data quality rules are applied here.
    ...
    - Kimball style star schema-based data models or Inmon style Data marts are often used
...
- Azure
  - [Data Lake Storage (ADLS)](https://azure.microsoft.com/en-us/products/storage/data-lake-storage/)
    - Packages
      - [{]{style="color: #990000"}[quak](https://github.com/pedrobtz/quak){style="color: #990000"}[}]{style="color: #990000"} - Provides convenience utilities for using 'DuckDB' directly over datasets stored in 'Azure Data Lake Storage Gen2'
...
- [MinIO](https://min.io/) (Put into maintenance mode)
...
- [RustFS](https://github.com/rustfs/rustfs) - An open-source, S3-compatible high-performance object storage system supporting migration and coexistence with other S3-compatible platforms such as MinIO and Ceph
- [SeaweedFS](https://github.com/seaweedfs/seaweedfs) - A fast distributed storage system for blobs, objects, files, and data lake, for billions of files
  - Blob store has O(1) disk seek, cloud tiering. Filer supports Cloud Drive, xDC replication, Kubernetes, POSIX FUSE mount, S3 API, S3 Gateway, Hadoop, WebDAV, encryption, Erasure Coding.
  - Enterprise version is at seaweedfs.com.
...
- Pipeline\
  ![source](_resources/DB-Lakes.resources/lakehouse-pipeline-1.webp){.lightbox width="432"}
  ...
  - Method: Organizes data into three layers (Medallion Architecture):
...
- Frozen Ducklakes
  - Notes from
    - [Frozen DuckLakes for Multi-User, Serverless Data Access](https://ducklake.select/2025/10/24/frozen-ducklake/)
      - Article shows an example of creating a read-only DuckDB database file and how to create new revisions of a Frozen DuckLake.
  - A read-only and has no moving parts other than a cloud storage system
  - Advantages
  ...
  - Versioning
    - Data can still be updated by creating a *new* Frozen DuckLake.
    - Older versions can be accessed by retaining revisions of the DuckDB database file — or by simply using [time travel](#0).

### R {#sec-db-lakes-dklk-r .unnumbered}

- Packages
  - [{]{style="color: #990000"}[ducklake](https://tgerke.github.io/ducklake-r/){style="color: #990000"}[}]{style="color: #990000"} - Provides ACID transactions, automatic versioning, time travel queries, and complete audit trails
...
- [Example 1]{.ribbon-highlight}: Basic Workflow
  - Local Set-Up
...
- [Example 2]{.ribbon-highlight}: Medallion Architecture ([source](https://discindo.org/posts/2025-09-20-r-ducklake/))
  - Set-up
    ...
    - Then to open in the browser UI: `duckdb -ui metadata.ducklake`
  ...
  - Create tables
    ...
    - Running these queries will create this file structure
  ...
  - Create relationships between intial tables
  ...
  - Add relationships after creating more tables
    ...
    - This is done after creating 2 more silver tables and 1 more gold table

  - Visualize relationships\
    ![](_resources/DB-Lakes.resources/ducklake-r-ex2-rel-1.png){.lightbox width="432"}
    ...
    - This plot could be part of a Quarto report or a Shiny app with some interactivity to keep track of the transformations that happen to the data you are managing.

    - Could be improved upon. See [Databricks Unity Catalog view](https://docs.databricks.com/aws/en/data-governance/unity-catalog/data-lineage)
