...
-   [Example]{.ribbon-highlight}:
    ...
    -   Pooled spatial variograms are analyzed. Spatial dependence is calculated per sampled time instance and pooled across all sampled time instances.
    -   Bootstrap distributions for the partial sill and range are calculated. These could give us a potential range of starting values if we use the same model (exponential) in the spatio-temporal variogram.
    ...
    ## Pooled Spatial Variogram Model
    ...
    -   After fitting variograms with a few different amounts of sampled time slices/instances a number of times, the variograms were substantially different — making it difficult to gain a feel for the partial sill and range parameters. Applying the bootstrap should give a better sense for a range of starting values for each of those parameters.
    ...
    2.  Iterates through the number of bootstrap replicates specified by [n_samples]{.var-text}
    3.  Generates blocks until the length of the replicate reaches the fixed length ([time_slices]{.var-text})
    4.  A matrix is created with `make_block_indices` where each row is indices of PM10 observations which are organized in blocks of 14 days. Each row of that matrix is fed into `estim_parmas_vgm` where a variogram model is fit. The calculated [psill]{.arg-text} and [range]{.arg-text} parameters of the fitted model are collected in a tibble
    ...
    ![](_resources/Geospatial-Spatio-Temporal.resources/eda-spdep-ex1-boot-1.png){.lightbox group="eda-spdep-ex1" width="532"}
    ...
    -   Maybe more than 300 days (out of 1826 days) for the number of sampled time instances (slices) would help, but this is likely do to there only being 53 stations taking measurements.
    ::::
...
-   [Misc]{.underline}
    -   See [Geospatial, Modeling \>\> Interpolation \>\> Kriging \>\> Variograms](geospatial-modeling.qmd#sec-geo-krig-vario){style="color: green"} \>\> Terms for descriptions of sill, nugget, and range.
    -   For `vgm`, you can just use `vgm("exponential")` and not worry about setting parameters like sill, nugget, etc. This might also be the case with `vgmST`. (See [Geospatial, Models \>\> Interpolation \>\> Kriging \>\> Variograms](geospatial-modeling.qmd#sec-geo-krig-vario){style="color: green"}\>\> Example 1)
...
-   "The selection of a spatio-temporal covariance model should not only be made based on the (weighted) mean squared difference between the empirical and model variogram surfaces (`attributes(fit_vario)$value`), but also on conceptional choices and visual judgement of the fits. (`plot(emp_vario, c(fit_vario1, fit_vario2, fit_vario3), wireframe = TRUE, all = TRUE)`) "
-   `plot` - Shows the sample and fitted variogram surfaces next to each other as colored level plots. ([Docs](https://cran.r-project.org/web/packages/gstat/refman/gstat.html#plot.gstatVariogram))
    ...
    -   [all = TRUE]{.arg-text} plots sample and model variogram(s) in single wireframes
...
-   spat-temp vig
    ...
    -   Parameters of the spatial and temporal variograms can be assessed from the spatio-temporal surface fixing the counterpart at 0. (Done the spatial. How to do the temporal?)
...
-   statninja vid
    -   vid 3 on st vig
        -   Empirical variogram
            -   cutoff and tlags specified
            -   heatmap - plot(vario)
                -   The shades of color are too hard to distinguish to get any sort of quality numeric estimate
            -   line plot - plot(vario, map = FALSE)
                -   100 semi-variance (no spdep) at lag 4 at around 40km
                -   Should 40 km be our spatial range starting value and 4 be out time range starting value?
        -   Metric Model
            -   starting values: psill, range, nugget = 0, stAni
            -   heatmap - plot(vario, mod)
                -   shows much more variability in the time axis than the spatial one
                    -   really smooths out the colors in the spatial dimension.
                -   Don't really like it as a way to tell how well it fits the empirical variogram
            -   no line chart
        -   Separable
            -   starting values: sill (overall) and psill, range, nugget (for time and space models)
                -   weird that the psill and nugget add to 1 but the sill is set to 100
            -   fit - boundaries (lower and upper) set - five parameters
            -   heatmap comparison w/metric
                -   less smoothing which better captures the empircal vario
            -   3d wireframe comparison
                -   pretty difficult to make any kind of judgement either way
            -   no line chart
        -   specialized prediction plot looks mostly uninformative (weird ordering, no polys)
    -   vid 4 fires
        -   convex hull
        -   time only variogram
        -   Marginal variogram starting values
            -   spatial cutoff
            ...
            -   temporal cutoff
                ...
                -   I don't think this was a sensible value. I believe the dates were aggregated to months, but the x-axis looked like days was used by the variogram function. I think this value was in the thousands. So, is it plausible there's temporal dependencies in the thousands of days or months range? I don't think so.
                    -   Maybe if you're trying to take into account some kind of el nino effect?

            -   Both widths are cutoff / 10
        -   vgmST
            -   doesn't specify model types in each vgm for time and space
                -   The chosen models are the same as the marginal ones he fit. I feel like he took this result and fit marginal ones after the fact but whatever.
            -   Sets the nugget for each model *and* an overall nugget
                -   uses the psill value of each mariginal variogram
                -   chooses min between two as the overall nugget
            -   overall siil set as the var of the response (count): `var(STIDF_fires@data$Fires)`
                -   This was actually the exact overall sill estimated by the model
            -   sets k = 15 which is the weighting of the product in the product-sum model
                -   dumb since he's fitting a separable model
                -   maybe he thinks it's for the matern in case of the marginal models chooses matern which is wrong.
            -   evidently this function does some optimatization, because range and nugget estimates for each of time and space are available from the object and also a sill estimate
