...
-   Packages
    ...
    -   [SemiparMF](https://cran.r-project.org/web/packages/SemiparMF/index.html) - Fits a **semiparametric** spatiotemporal (**regression?**) model for data with **mixed frequencies**, specifically where the response variable is observed at a lower frequency than some covariates.
...
-   [Example]{.ribbon-highlight}: Counts of pairwise distances between locations\
    ...
    -   For kriging, ideally we'd want at least around 100 pairwise distances in the first few bins (short pairwise distances) for a stable range estimate, but no pairwise distance bin has that many. (See [Geospatial, Kriging \>\> Bin Width](geospatial-kriging.html#sec-geo-krig-bw){style="color: green"})
