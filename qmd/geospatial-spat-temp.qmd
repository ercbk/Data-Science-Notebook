...
-   Weighting Options\
    ...
    -   [fit.method = 6]{.arg-text} (default)
    -   Depending on the target neighbourhood size of the desired interpolation, it might be beneficial to narrow down the spatial and temporal distances and to introduce a cutoff.
...
-   Fitting
    -   A subset of data is chosen using k-NN for an prediction location. The interpolation performs iteratively for each spatiotemporal prediction location with a local subset of the data set.
