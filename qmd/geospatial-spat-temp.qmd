...
-   [Example]{.ribbon-highlight}:
    ...
    -   This example is from the [{gstat}]{style="color: #990000"} vignette which they seem to have specified the model incorrectly given the package documentation. The vignette uses [dX = 0]{.arg-text} which I can only guess is because they thought [dX]{.arg-text} is *less than equal to* the 2-norm when it's *strictly less than* (See [Geospatial, Kriging \>\> Variograms](https://ercbk.github.io/Data-Science-Notebook/qmd/geospatial-kriging.qmd#sec-geo-krig-vario){style="color: green"} \>\> Example). Consequently, they produced a bad fit.
