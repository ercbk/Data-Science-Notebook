...
-   Spatio-Temporal vs Purely Spatial
    -   By examining the spatio-temporal variogram plot, you
        -   [Example]{.ribbon-highlight} (gstat vignette): A temporal lag of one or a few days leads already to a large variability compared to spatial distances of few hundred kilometres, implying that the temporal correlation is too weak to considerably improve the overall prediction.
...
-   Weighting Options for `fit.variogram`\
...
-   `krigeST`
    ...
    -   [bufferNmax = 1]{.arg-text} (default) is used to increase the search radius for neighbors (e.g. 2 would be double the default radius.)
...
-   EDA
    -   Autocorrelation\
        ![](_resources/Geospatial-Spatio-Temporal.resources/krig-eda-ac-ex1-1.png){.lightbox width="532"}
        ...
        par(mfrow=c(2,2))
        for (i in rn) {
          x <- na.omit(as.matrix(r5to10[i])[,1])
          acf(x, main = i)
        }
        ```
        ...
        -   `r5to10@sp` is the sp class object with location names and coordinates
        -   Which lags present autocorrelation?

    -   Cross-(Auto)Correlation\
        ![](_resources/Geospatial-Spatio-Temporal.resources/krig-eda-cac-ex1-1.png){.lightbox width="632"}
        ...
        -   For each pair of locations, is the correlation between location $a$ at time $t$ and location $b$ at time $t + h$
        ...
        -   If there is no autocorrelation between a pair of locations, is it because there's a large distance between those locations? Maybe this can give a hint about the appropriate neighborhood size or area
            ...
            # sample 5 location indicies that aren't low cross-corr locations
            mat_clean_idx_samp <- sort(sample(mat_clean_idx[-no_ac_idx], 5))
            # combine sample indicies and low cross-corr indicies
            mat_clean_idx_comb <- c(no_ac_idx, mat_clean_idx_samp)


            # Convert the SpatialPoints to sf
            sf_locs <- st_as_sf(r5to10[mat_clean_idx_comb,]@sp)
            # Calculate pairwise distances (in meters by default for geographic coordinates)
            mat_dist_sf <- st_distance(sf_locs)
            # Convert to kilometers and round to match your output
