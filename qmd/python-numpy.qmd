...
* Terms
	...
	* **memory views**: Working with views can be highly desirable since it avoids making unnecessary copies of arrays to save memory resources
		* `np.may_share_memory(new_array, old_array)` - if the result is TRUE, then new\_array is a memory view
...
* "other\_array" the array we want the constant array to do arithmetic with
* `.shape` method outputs other\_array's dimensions
...
* Any changes to "first\_row" also change "ary"
...
* \*\* produces a copy of the array. So takes-up more memory \*\*Boolean masking
