...
-   Packages
    -   Cumulative Link Models (CLMs)
        -   [{]{style="color: #990000"}[ordinal](https://cran.r-project.org/web/packages/ordinal/index.html){style="color: #990000"}[}]{style="color: #990000"}
...
-   Examine distributions of the variables to get an idea of potential relationships
    ...
    -   Can plot a histogram or compute percentages for each level of the variable
        ...
        -   [Example]{.ribbon-highlight}: $\mbox{lvl}_1 = \mbox{lvl}_{1/N} = 0.2219$, $\mbox{lvl}_2 = 0.2488$, $\mbox{lvl}_3 = 0.25$, and $\mbox{lvl}_4 = 0.2794$
...
-   Bi-variate boxplots
    ...
    -   [Example]{.ribbon-highlight}: Ordinal vs Ordinal\
        ![](./_resources/Regression,_Ordinal.resources/Screenshot%20(1141).png){.lightbox width="432"}
...
-   GOF (rule of thumb): If the deviance about the same size as the difference in the number of parameters (i.e. $p_{\mbox{full}} - p_{\mbox{reduced}}$), there is NOT evidence of lack of fit. (ordinal [vignette](http://people.vcu.edu/~dbandyop/BIOS625/CLM_R.pdf), pg 14)
    ...
    -   [Example]{.ribbon-highlight} (Have doubts this is correct)
        -   Looking at the number of parameters ([no.par]{.arg-text}) for [fm1]{.var-text} in Example: [{ordinal}]{style="color: #990000"}, model selection with LR tests below and the model summary in Proportional Odds (PO) \>\> Example: [{ordinal}]{style="color: #990000"}, response = [wine rating]{.var-text} (1 to 5 = most bitter), the number of parameters for the reduced model is the $\mbox{number of regression parameters} (2) + \mbox{number of thresholds} (4)$
...
-   [Example]{.ribbon-highlight}: [{ordinal}]{style="color: #990000"}, model selection with LR tests
    ...
    -   For [fm1]{.var-text} model, see Proportional Odds (PO) \>\> Example [{ordinal}]{style="color: #990000"}, response = wine rating (1 to 5 = most bitter)
...
## Logistic Regression {#sec-reg-ord-log}

-   Skipped this for now.

## Cumulative Link Models (CLM) {#sec-reg-ord-clm}
...
-   [Latent Variable Concept]{.underline}
    ...
    -   A response occurs in category $j$ ($Y = j$) if the latent response process $y^*$ exceeds the threshold value, $\theta_{j-1}$ , but does not exceed the threshold value, $\theta_j$ .
        -   I think $y^*$ is continous on the scale of logits. $\theta_j$s are also intercepts in the model equations and also on the scale of logits (see Proportional Odds (PO) \>\> Example: {MASS::polr})
...
-   [Structured Thresholds]{.underline}
    ...
    -   [Example]{.ribbon-highlight}: Is the reponse scale being treated by judges as equidistant between values.
        ...
        -   Compare spacing parameter with your model's average spacing
            ...
            -   [fm1]{.var-text} is from an example in the Proportional Odds (PO) section
...
## Proportional Odds (PO) {#sec-reg-ord-po}
...
-   Assumption: The independent variable effect is the same for all levels of the ordinal outcome variable
    ...
    -   Testing the assumption
        ...
        -   You can test this assumption by fitting a PO model and PPO model. Then, comparing both models via LR Test or using {ordinal::nominal_test} (See below, [Partial Proportional Odds (PPO)](regression-ordinal.qmd#sec-reg-ord-ppo){style="color: green"} \>\> Examples)
...
-   [Example 1]{.ribbon-highlight}: [{ordinal}]{style="color: #990000"}
    ...
    -   Regression Coefficients
        ...
        -   Interpretation:
            ...
            -   Me: The odds of the [rating]{.var-text} being higher are 4.61 times greater when [contact = yes]{.var-text} than when [contact = no]{.arg-text}.
...
-   [Example 2]{.ribbon-highlight}: [{ordinal}]{style="color: #990000"}, weighted complimentary log-log regression, comparing links
...
## Partial Proportional Odds (PPO) {#sec-reg-ord-ppo}
...
-   Model\
    ![](./_resources/Regression,_Ordinal.resources/Screenshot%20(1138).png)
...
## Generalized Ordinal Regression {#sec-reg-ord-gor}
