...
* Packages
	* Cumulative Link Models (CLMs)
		...
		* [[{mvord}]{style='color: #990000'}](file:///C:/Users/tbats/Downloads/v93i04.pdf):
...
* Harrell summary and comparison of a PO model vs Logistic in the case of an ordinal outcome![](./_resources/Regression,_Ordinal.resources/Screenshot (763).png)
...
* Example: lvl\_1% = lvl\_1/N =  0.2219, lvl\_2% = 0.2488, lvl\_3% = 0.25, and lvl\_4% = 0.2794
...
* Bi-variate boxplots
	...
	* Example: ordinal vs ordinal![](./_resources/Regression,_Ordinal.resources/Screenshot (1141).png)
...
* Deviance: -2\*(LogLik Reduced Model - LogLik Saturated model)
	...
	* Saturated model also called the Full model (Also see [Regression, Discrete](Regression, Discrete) >> Misc)
...
* Types
	...
	* proportional hazards model - CLM with a log-log link, for grouped survival times![](./_resources/Regression,_Ordinal.resources/Screenshot (1130).png)
...
* Link functions![](./_resources/Regression,_Ordinal.resources/Screenshot (1139).png)
	* Note the shape of the distributions of the response (see EDA) to help choose a link function
		...
		* Kurtotic, see [Statistical Concepts](Statistical Concepts) >> Fundamentals >> Kurtosis (i.e. higher sharper peaks w/short tails, flatter peaks w/long tails)
			* Default parameter values fit a symmetric heavy tailed distribution (high, sharp peak)
* Model![](./_resources/Regression,_Ordinal.resources/Screenshot (1136).png)
...
* Response variable follows the Multinomial distribution![](./_resources/Regression,_Ordinal.resources/Screenshot (1132).png)
...
* Cumulative Logits (Logit Link)![](./_resources/Regression,_Ordinal.resources/Screenshot (1129).png)
...
* Latent Variable Concept
	...
	* To set the _location_ of the latent variable, it is common to set a threshold to zero. Usually, the first of the threshold parameters (θ1) is set to zero.
...
* Assumption: The independent variable effect is the same for all levels of the ordinal outcome variable
	...
	* Example:
		* outcome = health\_status (1 for poor, 2 for average, 3 for good and 4 for excellent)
		* independent variable = family\_income (1 for above avg, 0 for below average)
	...
	* Testing the assumption
		...
		* You can test this assumption by fitting a PO model and PPO model. Then, comparing both models via LR Test or using {ordinal::nominal\_test} (see below, Partial Proportional Odds (PPO) examples)
...
* Interpretation
	* "If p1960(x) is proportion of the population with an income larger than $x in 1960 and p1970(x) is the equivalent in 1970, then approximately"![](./_resources/Regression,_Ordinal.resources/Screenshot (1164).png)
...
* Explanatory:
	...
	* "gpa" - numeric
* model![](./_resources/Regression,_Ordinal.resources/Screenshot (1142).png)
...
* Interpretation:
	...
	* In odds ratios (aka proportional odds ratios): (w/CIs `exp(cbind(OR = coef(m), ci = confint(m))`
		...
		* pared:
			* "For students whose parents did have a grad degree, the odds of being more likely (i.e., _very_ or _somewhat likely_ versus _unlikely_) to apply is 2.85 times that of students whose parents did NOT have a grad degree, holding constant all other variables."
			* "For students whose parents did have a grad degress, the odds of being less likely to apply (i.e., _unlikely_ versus _somewhat_ or _very likely_) is 2.85 times that of students whose parents did NOT have a grad degree, holding constant all other variables."
		* public
			* For students in public school, the odds of being more likely (i.e., _very_ or _somewhat_ _likely_ versus _unlikely_) to apply is 5.71% lower \[i.e., (1 -0.943) x 100%\] than private school students, holding constant all other variables.
			...
			* For students in private school, the odds of being less likely to apply (i.e., _unlikely_ versus _somewhat_ or _very likely_) is 5.71% lower than public school students, holding constant all other variables.
		...
		* gpa
			* For every one unit increase in student’s GPA the odds of being more likely to apply (_very_ or _somewhat likely_ versus _unlikely_) is multiplied 1.85 times (i.e., increases 85%), holding constant all other variables.
			* For every one unit decrease in student’s GPA the odds of being less likely to apply (_unlikely_ versus _somewhat_ or _very likely_) is multiplied 1.85 times, holding constant all other variables.
...
* [Example]{.ribbon-highlight}: [{ordinal}]{style='color: #990000'}, PPO _for "contact"_
...
* [Example]{.ribbon-highlight}: [{ordinal}]{style='color: #990000'}, Test the PO assumption for _all_ variables w/LR Test
...
* model![](./_resources/Regression,_Ordinal.resources/Screenshot (1138).png)
