...
- Packages
  ...
  - [{]{style="color: #990000"}[leprechaun](https://leprechaun.opifex.org/#/){style="color: #990000"}[}]{style="color: #990000"} - Generates basic code for a shiny application within the package.
    - From the root of a package, scaffold a leprechaun application. This generates basic code for a shiny application within the package
    - Differes from golem in that it generates code and does not make itself a dependency of the application you build; this means applications are leaner, and smaller
    - Does not require importing external dependencies, other than shiny and bslib
...
- Debugging
  - Use `browser` to walk through reactives as they're being evaluated
...
- Alternative Frameworks
  ...
  - [{]{style="color: #990000"}[htmxr](https://cran.r-project.org/web/packages/htmxr/index.html){style="color: #990000"}[}]{style="color: #990000"} ([website](https://htmx.org/)) - A lightweight framework for building server-driven web applications in 'R'. 'htmxr' combines the simplicity of 'htmx' for partial page updates with the power of 'plumber2' for non-blocking HTTP endpoints.
    ...
    - Features
      ...
      - *Surgical DOM updates*: htmx updates exactly the targeted element, nothing more. The result is smoother interfaces, less visual flicker, and a faster perceived experience.
        - Shiny reloads entire output blocks (uiOutput, renderUI)
      - *Scalability and infrastructure cost*: Deployment is standard: nginx, Docker, any VPS, any cloud platform. A pool of shared plumber2 workers handles far more concurrent users on the same hardware.
        - Shiny’s persistent session model means memory consumption scales linearly with the number of connected users. Shiny Server license required.
      - *Native HTTP: cache, load balancers, SEO*: Responses are plain HTML
      ...
      - *Progressive enhancement*: Works even when JavaScript is disabled — links and forms remain functional. You can enrich a static HTML page progressively, or add dynamic behaviour to an existing app without rewriting it from scratch
      - *Interoperability*: Standard HTML is generated and backed by a standard HTTP API. The same plumber2 endpoints that serve your htmxr frontend can be consumed by mobile apps, other services, or scripts in any language — without translation or adaptation.
        - Valuable when a proof-of-concept built in R needs to grow into a production system: the business logic stays in R, while other parts of the stack can call the same endpoints.
...
- Packages
  - [{]{style="color: #990000"}[shinytest2](https://rstudio.github.io/shinytest2/){style="color: #990000"}[}]{style="color: #990000"} - Provides a streamlined toolkit for unit testing Shiny applications and seamlessly integrates with the popular testthat framework for unit testing R code
  - [{]{style="color: #990000"}[shinytesters](https://ashbaldry.github.io/shinytesters/){style="color: #990000"}[}]{style="color: #990000"} ( [Intro](https://www.r-bloggers.com/2025/08/shinytesters-updating-inputs-in-testserver/)) - Makes it easier to test update functions in Shiny packages when using `testthat::testServer`
- Notes from [Five pre-flight checks for your dashboard](https://www.jumpingrivers.com/blog/five-preflight-checks-dashboard/)
...
- Final Checklist
  - Is the app design responsive?
    - If a user clicks on a button and nothing happens for a second or two, things “feel” broken even if, in reality, the browser is just processing things silently in the background.
    - Use animataed loading icons or even better would be some kind of progress indicator if possible for interactions that take over a second or two.
  - Is the app performant?
    - See [Shiny, Production \>\> Optimization](shiny-production.qmd#sec-shiny-prod-opt){style="color: green"}
  - Does the design work at different screen dimensions (desktop, laptop, tablet, and mobile)?
    - Stack elements and make sure there's scrolling. I think there are packages for this now though.
  - Does the app support touch screens?
    ...
    - Is it obvious what interactions are actually possible?
      - If not, add a common icon that indicates interactability, e.g. the expand icon.
      - Then, add text that explains each feature with the icon is interactable.
  - Is the app accessible?
    - If the button-like interactive elements are not HTML button elements, then they don’t have appropriate interactions and semantics to meet WCAG requirements.
    - Can everything that can be clicked/tapped be interacted with by a keyboard user?
    - Do the modals (i.e. pop-up graphs, etc.) that appear when selecting interactive elements implement focus trapping, meaning a keyboard user can lose focus somewhere in the background behind the modal window?
    - Using html buttons for various cards and the "dialog element" for the modal windows, can make these features accessible.
      - Side note: Use different colors for when features are in "focus" (i.e. like hover) and and "selected" (i.e. interacted with).
