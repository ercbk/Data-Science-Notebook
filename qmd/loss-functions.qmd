...
* Make sure your Loss Function and Performance metrics correlate
	...
	* Example: Cross-Entropy vs. Accuracy![](./_resources/Loss_Functions.resources/Screenshot (1071).png)
...
* Mean Absolute Error (MAE) isn't typically used (instead of MSE) because it is not differentiable, thus gradient descent can't be used to find its minimum. **Stochastic Gradient Descent** is used to overcome this problem. (See [Algorithms, ML](Algorithms, ML) >> SVM >> Regression)
	...
	* **Stochastic Gradient Descent**: the gradient of the loss is estimated each sample at a time and the model is updated along the way with a decreasing strength schedule (aka learning rate)
		* Also see [Model building, sklearn](Model building, sklearn) >> Algorithms >> Stochaistic Gradient Descent (SGD)



**0/1 Loss (aka binary loss)**
![](./_resources/Loss_Functions.resources/Screenshot (1086).png)
...
* A combination of squared loss and absolute loss
	...
	* The loss is the square of the usual residual (_y_ — f(_x_)) only when the absolute value of the residual is smaller than a fixed parameter, δ.
...
* {yardstick::huber\_loss}



**Asymmetric Huber Loss**
![](./_resources/Loss_Functions.resources/Screenshot (742).png)
...
* Potentially a more programmer-friendly version![](./_resources/Loss_Functions.resources/Screenshot (741).png)
...
**Mean Absolute Log Error (MALE)** ([article](https://towardsdatascience.com/mean-absolute-log-error-male-a-better-relative-performance-metric-a8fd17bc5f75))
![](./_resources/Loss_Functions.resources/Screenshot (1204).png)

* f is the forecast and y is the observed
* Similar to RMSLE
* Requirement: data are _strictly positive_: they do not take zero or negative values
...
**Mean Absolute Scaled Log Error (MASLE)**
![](./_resources/Loss_Functions.resources/Screenshot (1207).png)
...
* Non-seasonal ts scaled error![](./_resources/Loss_Functions.resources/Screenshot (338).png)
* Seasonal ts scaled error![](./_resources/Loss_Functions.resources/Screenshot (339).png)
* MASE = mean(|qj|)



**Mean Squared Error (MSE)** - A quadratic loss function that's calculation is based on the squared errors, so it is more sensitive to the larger deviations associated with tail events
...
* ![](./_resources/Loss_Functions.resources/Screenshot (259).png)
...
* ![](./_resources/Loss_Functions.resources/0-O3r1ZyUpXdG4c2Uh.png)
