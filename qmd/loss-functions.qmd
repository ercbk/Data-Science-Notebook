...
- Packages
  - [{]{style="color: #990000"}[SLmetrics](https://slmetrics-docs.gitbook.io/v1){style="color: #990000"}[}]{style="color: #990000"} ([github](https://github.com/serkor1/SLmetrics))- Very fast, written in C++
...
## Area Under the Minimum {#sec-lossfun-aum .unnumbered}

- Short for Area Under Min(FP, FN)
- A loss function that optimizes for AUC in classification models
- Paper: [Optimizing ROC Curves with a Sort-Based Surrogate Loss for Binary Classification and Changepoint Detection](https://www.jmlr.org/papers/v24/21-0751.html)
- [{]{style="color: #990000"}[torchMAUM](https://cran.r-project.org/web/packages/torchMAUM/index.html){style="color: #990000"}[}]{style="color: #990000"} - Multi-Class Area Under the Minimum in Torch
...
- [{]{style="color: #990000"}[SLmetrics::rmsle or weighted.rmsle](https://slmetrics-docs.gitbook.io/v1/reference/regression-metrics/root-mean-squared-logarithmic-error/rmsle){style="color: #990000"}[}]{style="color: #990000"}

  - Weighted case for heteroskedastic residuals

- Extend [{yardstick}]{style="color: #990000"} by creating a custom metric to calculate RMSLE
...
## Root Mean Squared Scaled Error (RMSSE) {#sec-lossfun-rmsse .unnumbered}

$$\begin{align}
&\text{RMSSE} = \frac{1}{\sqrt{\frac{1}{T-1}\sum_{t=1}^{T-1} \Delta^2_t}}\text{RMSE} \\
&\text{where}\;\; \text{RMSE} = \sqrt{\frac{1}{h}\sum_{j=1}^h e^2_{t+j}}
\end{align}$$

- $\Delta_t$ is the first differences of the in-sample values
- Used in the M5 Competition
- [{]{style="color: #990000"}[greybox::RMSSE](https://cran.r-project.org/web/packages/greybox/refman/greybox.html#ME){style="color: #990000"}[}]{style="color: #990000"}

## Scaled Absolute Mean Error (SAME) {#sec-lossfun-same .unnumbered}

$$\begin{align}
&\mathrm{SAME} = \frac{1}{\frac{1}{T-1} \sum_{t=1}^{T-1} |\Delta_t|} \mathrm{AME} \\
&\text{where}\;\;\mathrm{AME}= \left| \frac{1}{h} \sum_{j=1}^h e_{t+j} \right|
\end{align}$$

- $\Delta_t$ is the first differences of the in-sample values
- Similar to MASE, but measures bias
- [{]{style="color: #990000"}[greybox::SAME](https://cran.r-project.org/web/packages/greybox/refman/greybox.html#ME){style="color: #990000"}[}]{style="color: #990000"}
