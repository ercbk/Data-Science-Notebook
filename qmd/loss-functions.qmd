...
-   Misc
-   0/1 Loss (aka binary loss)
-   Brier Loss
-   Huber Loss
-   Mean Log Loss (aka Cross-Entropy)
-   Mean Balanced Log Loss (aka balanced cross-entropy)
-   Focal Loss
-   Mean Absolute Log Error (MALE)
-   Mean Absolute Scaled Log Error (MASLE)
...
-   Make sure your Loss Function and Performance metrics correlate
    ...
    -   Example: Cross-Entropy vs. Accuracy![](./_resources/Loss_Functions.resources/Screenshot%20(1071).png)
...
-   Mean Absolute Error (MAE) isn't typically used (instead of MSE) because it is not differentiable, thus gradient descent can't be used to find its minimum. **Stochastic Gradient Descent** is used to overcome this problem. (See [Algorithms, ML](Algorithms,%20ML) \>\> SVM \>\> Regression)
    ...
    -   **Stochastic Gradient Descent**: the gradient of the loss is estimated each sample at a time and the model is updated along the way with a decreasing strength schedule (aka learning rate)
        -   Also see [Model building, sklearn](Model%20building,%20sklearn) \>\> Algorithms \>\> Stochaistic Gradient Descent (SGD)

**0/1 Loss (aka binary loss)** ![](./_resources/Loss_Functions.resources/Screenshot%20(1086).png)
...
**Brier Loss** \[\](./\_resources/Loss_Functions.resources/0Z9I5AbSBpL88zQAP.gif\]\]
...
![](./_resources/Loss_Functions.resources/300px-Huber_loss.png)![](./_resources/Loss_Functions.resources/1-ZdCzdOtHoWOMbxNzDF_lXg.png)
...
**Asymmetric Huber Loss** ![](./_resources/Loss_Functions.resources/Screenshot%20(742).png)
...
-   Potentially a more programmer-friendly version![](./_resources/Loss_Functions.resources/Screenshot%20(741).png)

**Mean log loss** (aka **cross-entropy)** \[\](./\_resources/Loss_Functions.resources/90149Capture0.PNG\]\]
...
**Mean balanced log loss** (aka **balanced cross-entropy**) ![](./_resources/Loss_Functions.resources/ql_ec44337a8672432ed34d89ae6e003bab_l3.png)
...
**Focal Loss** ![](./_resources/Loss_Functions.resources/0-1-tjJ3J48YMut6Hm.png)
...
**Mean Absolute Log Error (MALE)** ([article](https://towardsdatascience.com/mean-absolute-log-error-male-a-better-relative-performance-metric-a8fd17bc5f75)) ![](./_resources/Loss_Functions.resources/Screenshot%20(1204).png)
...
-   Comparison with MAPE, sMAPE![](./_resources/Loss_Functions.resources/image.png)
...
**Mean Absolute Scaled Log Error (MASLE)** ![](./_resources/Loss_Functions.resources/Screenshot%20(1207).png)
...
-   Non-seasonal ts scaled error![](./_resources/Loss_Functions.resources/Screenshot%20(338).png)
-   Seasonal ts scaled error![](./_resources/Loss_Functions.resources/Screenshot%20(339).png)
-   MASE = mean(\|qj\|)

**Mean Squared Error (MSE)** - A quadratic loss function that's calculation is based on the squared errors, so it is more sensitive to the larger deviations associated with tail events ![](./_resources/Loss_Functions.resources/image10.png)

-   A custom asymmetric MSE loss function can better account for large deviations in errors![](./_resources/Loss_Functions.resources/image11.png)
    -   with α∈(0,1) being the parameter we can adjust to change the degree of asymmetry

**Quantile loss function**, which is a linear function. Benefit of allowing you to predict a certain percentile of the outcome variable

-   not effective at predicting tail events![](./_resources/Loss_Functions.resources/image9-2.png)
...
**Root Mean Squared Logarithmic Error (RMSLE)** Also see [Understanding the metric: RMSLE](https://www.kaggle.com/carlolepelaars/understanding-the-metric-rmsle)
...
**Symmetric Mean Absolute Percentage Error (SMAPE)** SMAPE = (1/n) \* Σ(\|forecast -- actual\| / ((\|actual\| + \|forecast\|)/2) \* 100

```         
smape <- function(a, f) {  return (1/length(a) * sum(2*abs(f-a) / (abs(a)+abs(f))*100)){style='color: #990000'}[}]{style='color: #990000'}
```
