...
-   Packages
    ...
    -   [{]{style="color: goldenrod"}[clevercsv](https://github.com/alan-turing-institute/CleverCSV){style="color: goldenrod"}[}]{style="color: goldenrod"} - Provides a drop-in replacement for the [{csv}]{style="color: goldenrod"} package with improved dialect detection for messy CSV files. It also provides a handy command line tool that can standardize a messy file or generate Python code to import it.
    -   [{]{style="color: goldenrod"}[tabbed](https://github.com/mscaudill/tabbed){style="color: goldenrod"}[}]{style="color: goldenrod"} ([JOSS](https://joss.theoj.org/papers/10.21105/joss.08964)) - Similar to [{clevercsv}]{style="color: goldenrod"} except uses row length and type consistency measures to automatically detect metadata, header and data sections of a D(delimiter)SV file. Furthermore, tabbed provides a value-based conditional reader for reading these irregular DSV files at scale.

-   Convert datetime to local NYC time ([source](https://towardsdatascience.com/how-to-build-an-ai-powered-weather-etl-pipeline-with-databricks-and-gpt-4o-from-api-to-dashboard/))
    ...
    -   Also [video](https://www.youtube.com/watch?v=Zekos7T2KI4) on handling this in pandas
