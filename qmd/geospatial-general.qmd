...
- [**Mesh**]{style="color: #009499"} - Divides space into connected elements, usually triangles in 2D spatial stochastic partial differential equation (SPDE) models. The triangles can vary in size and shape depending on where you need more detail.
  - Features
    - Very small triangles near dense observation locations
    - Larger triangles in sparse region
    - Irregular boundaries that follow coastlines, lakes, or study-area edges
    - Spherical mesh functions to avoid distortion from projecting the Earth onto a plane (e.g. climate models)
    - Metric graph meshes
      - Builds meshes along networks instead of open 2D regions.
      - Useful for roads, rivers, pipelines, electrical networks, or stream systems.
  - Issues
    - Poorly chosen meshes can create skinny triangles or unstable numerical behavior.
    - Finite element method (FEM) calculations on irregular triangles are usually slower and more memory-intensive than operations on a regular lattice.
    - Meshes can be harder to explain because triangle sizes vary across the domain
    - Post Processing
      - Some raster-based workflows, GIS layers, convolution methods, image processing methods, and spatial indexing methods assume a regular grid.
      - If your final output needs to be a raster map anyway, you may end up interpolating mesh results back onto a grid.
  - Grid vs Mesh
    - Grid: Regular spacing, easy to index, often inefficient if you need high resolution only in part of the domain
    - Mesh: Irregular spacing, usually triangles, can follow complex boundaries, more computationally efficient for irregular domains
  - Use Cases
    - Grid: The domain is simple, Data are already rasterized, You need easy indexing and visualization, Computational speed is more important than geometric flexibility.
    - Mesh: The domain is irregular, need boundary constraints, observation density varies, want SPDE random fields, have coastlines, holes, barriers, or networks.
...
- Resources
  ...
  - [Cloud-Optimized Geospatial Formats Guide](https://guide.cloudnativegeo.org/)
    ...
    - R Code using [{stars}]{style="color: #990000"}, [{terra}]{style="color: #990000"} and Python code
...
- Packages
  - [{]{style="color: #990000"}[a5R](https://github.com/belian-earth/a5R){style="color: #990000"}[}]{style="color: #990000"} - R bindings for the A5 pentagonal discrete global grid system (DGGS) via Rust crate.
  ...
  - [{]{style="color: #990000"}[chopin](https://docs.ropensci.org/chopin/){style="color: #990000"}[}]{style="color: #990000"} - Creates regular and padded grids
  ...
  - [{]{style="color: #990000"}[weightedVoronoi](https://harriraven.github.io/weightedVoronoi/){style="color: #990000"}[}]{style="color: #990000"} - Generates **weighted spatial partitions** that respect boundaries, landscape structure, and heterogeneous point influence using Euclidean or geodesic distance.
...
- Padded grids\
  ![](_resources/Geospatial,_General.resources/grid-reg-vs-padded-grid.png){.lightbox width="282"}
...
### Regular Grids {#sec-geo-gen-gridsys-reg .unnumbered}

- Grids with square or rectangular cells

- [Example 1]{.ribbon-highlight}: Square grid that covers Germany w/10km cells\
  ![](_resources/Geospatial,_General.resources/grid-reg-ex1.png){.lightbox}
  ...
  # German shapefile
  shp_de <- read_sf(
    "https://raw.githubusercontent.com/edzer/sdsr/main/data/de_nuts1.gpkg"
  ) |> 
    st_transform(crs)

  cell <- 10000   # 10 km cell

  grid_de <- st_make_grid(
    shp_de,
    cellsize = cell,
    what = "polygons",
    square = TRUE
...
- [Example 2]{.ribbon-highlight}: Create an irregular shaped grid based on point geometries\
  ![](_resources/Geospatial,_General.resources/grid-reg-ex2.png){.lightbox width="267"}

  ``` r
  library(sf)

  # German population for 100m grid cells w/only point geometry
  sf_pop <- local({
  ...
  - Creates a grid in the shape of Germany by using the point geometry as centroids for each square cell created
  ...
  - `st_buffer` arguments
    ...
    - `dist = 50` creates a 50m buffer around each point (giving you 100m × 100m cells)
    - `endCapStyle = "SQUARE"` ensures square corners instead of rounded
    - `nQuadSegs = 1` minimizes the number of vertices per corner (creates true squares rather than approximations)
      - i.e. the higher the integer, the more vertices used to form the corner, the more curved/smooth the corner becomes.

      - We don't want extra space between our square cells, so we want them as pointed as possible

- [Example 3]{.ribbon-highlight}: Manually
  ...
  # sf_pop has: columns x, y, Einwohner, and POINT geometry

  cell_half <- 50

  # Create one template square centered at (0,0)
  template_square <- matrix(
    c(-cell_half, -cell_half,
      cell_half, -cell_half,
      cell_half,  cell_half,
      -cell_half,  cell_half,
      -cell_half, -cell_half),
  ...
  # Extract coordinates of points (vectorized)
  coords <- st_coordinates(sf_pop)

  # For each point, add its (x,y) to the template vertices
  # Create array: [n_points x 5 vertices x 2 coords]
  verts <- array(
    data = c(
      template_square[,1] + rep(coords[,1], each = 5),
      template_square[,2] + rep(coords[,2], each = 5)
    ),
    dim = c(5, nrow(coords), 2)
  )

  # Build polygons (vectorized)
  polys <- st_sfc(
    lapply(1:nrow(coords), function(i) st_polygon(list(verts[, i, ]))),
    crs = st_crs(sf_pop)
  )

  # Replace geometry efficiently
  grid_de_pop <- sf_pop
  st_geometry(grid_de_pop) <- polys
  ```

  - Manually creates the grid from Example 2. Who knows — might come in useful if you want a bespoke shaped grid cell.

  - Btw, this method took roughly twice as long (91.48 sec vs 42.61 sec) as just using `st_buffer`

  - Notice that the matrix, [template_square]{.var-text}, is 5 x 2. Each row is the coordinate of a vertex of the square cell. There are five vertices/coordinates (instead of 4 like you'd think), because the fifth one is the same as the first and it acts to "close" the polygon. (i.e. the starting point must be repeated as the ending point.)
...
- Misc
  ...
  - Tools
    - [H3 Viewer](https://clupasq.github.io/h3-viewer/) - Provide latitude and longitude or just manually search a global map. You can zoom in and out to figure out which H3 resoulution you want.
...
- [Example 2]{.ribbon-highlight}: [{h3o}]{style="color: #990000"}, [{mapgl}]{style="color: #990000"} ([source](https://bsky.app/profile/kylewalker.bsky.social/post/3m6htj7mzys22))
  ...
  - `h3_from_points` takes an [sfc_POINT]{.arg-text} object and creates a vector of H3 cells
  - `h3_from_strings` converts a character vector of cell indexes to a H3 vector
