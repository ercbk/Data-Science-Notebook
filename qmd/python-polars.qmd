...
-   [Example 1]{.ribbon-highlight}: Basic `pipe` at column-level
    ...
    -   Use `name.keep` when selecting more than one variable (e.g. `cs.numeric`) when using `pipe`. It prevents the output of the manipulated columns from having the same name.
...
-   [Example 3]{.ribbon-highlight}: `pipe` at df-level w/args
    ...
    -   Column names are quoted in the function arguments, but not in the fuction code.
...
-   [Example 4]{.ribbon-highlight}: Basic `map_columns`
    ...
    -   See Example 1 for `cap` udf definition
