...
- Packages
  ...
  - [{]{style="color: #990000"}[blockmodels](https://cran.r-project.org/web/packages/blockmodels/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/1602.07587)) - Fast estimation of Latent Block Models and Stochastic Block Models, with various probability functions, with or without covariates
  - [{]{style="color: #990000"}[dress](https://cran.r-project.org/web/packages/dress.graph/index.html){style="color: #990000"}[}]{style="color: #990000"} - Compute per-edge similarity values on graphs using the DRESS (Diffusive Recursive Structural Similarity) algorithm. Supports weighted/unweighted and directed/undirected graphs
  - [{]{style="color: #990000"}[graphonmix](https://sevvandi.github.io/graphonmix/){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2505.13864)) - Fits a graphon mixture, enabling generation of sequences of graphs where each graph is a combination of sparse and dense graphs
    - Social networks have a small number of large hubs, and a large number of small dense communities. A graphon mixture captures both hub and dense structures.
  ...
  - [{]{style="color: #990000"}[INCVCommunityDetection](https://cran.r-project.org/web/packages/INCVCommunityDetection/index.html){style="color: #990000"}[}]{style="color: #990000"} - Inductive Node-Splitting Cross-Validation for Community Detection in Stochastic Block Models.
  ...
  - [{]{style="color: #990000"}[L0ggm](https://cran.r-project.org/web/packages/L0ggm/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Github](https://github.com/AlexChristensen/L0ggm)) - Smooth L0 Penalty Approximations for Gaussian Graphical Models
  - [{]{style="color: #990000"}[multinet](https://cran.r-project.org/web/packages/multinet/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Vignette](https://www.jstatsoft.org/article/view/v098i08)) - Creation/generation and analysis of multilayer social networks
    - Multiplex social networks are characterized by a common set of actors connected through multiple types of relations.
  - [{]{style="color: #990000"}[netrics](https://cran.r-project.org/web/packages/netrics/index.html){style="color: #990000"}[}]{style="color: #990000"} - Many tools for calculating network, node, or tie marks, measures, motifs and memberships of many different types of networks
  ...
  - [{]{style="color: #990000"}[sparsecommunity](https://cran.r-project.org/web/packages/sparsecommunity/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements spectral clustering algorithms for community detection in sparse networks under the stochastic block model ('SBM') and degree-corrected stochastic block model ('DCSBM')
  ...
  - [{]{style="color: #990000"}[textNet](https://github.com/ucd-cepb/textNet){style="color: #990000"}[}]{style="color: #990000"} ([JOSS](https://joss.theoj.org/papers/10.21105/joss.07615)) - Input one or more PDF documents and create directed, multiplex, and multimodal network graphs. This enables analysis of the relationships between verb attributes and tenses, entity types, structural motifs, and other network characteristics.
    - Entities mentioned within the input text become nodes, and the verbs connecting them in the sentences of the text become directed edges. This generates network data based on the syntactic relationships between entities within a sentence.
    - Use Cases
      - Identify which actors are involved in start-up versus ongoing management tasks, characterize patterns of information and funding flows, and compare the distribution of management tasks in networks from regions known to have contrasting characteristics
      - Works on arbitrarily long documents, making it well suited for analyzing legal documents, court proceedings, plans and policy documents, media publications, and other long-form textual data containing events and entity relationships
  - [{]{style="color: #990000"}[textnets](https://github.com/cbail/textnets){style="color: #990000"}[}]{style="color: #990000"} - Generates networks based on co-occurrence of words within documents
    - Requires input to be formatted as a dataframe, where each row represents a document. The text of each document is contained in a column, with other columns including meta data.
    - Seems less sophisticated than [{textNet}]{style="color: #990000"} and based on a 2016 paper
- Resources
...
- Papers
  ...
  - [Network Dynamics in the Colombian Armed Conflict: Armed Structures and Municipalities](https://arxiv.org/abs/2508.09051)
    - No code but uses the [{blockmodels}]{style="color: #990000"} package and looks like it describes an analysis using the package pretty clearly.
- Analysis Questions
...
- **Blockmodeling**
  - Notes from R for Social Network Analysis, [Ch. 5.4](https://schochastics.github.io/R4SNA/descriptive/clustering.html#blockmodeling)
  - Instead of focusing on the density of connections, it categorizes the relationships between different groups (or blocks) of nodes according to the roles they play in the network.
  ...
  - Partitioning of the network into blocks is done through conventional (deterministic) or stochastic approaches, including k-block modeling and stochastic blockmodeling.
  - Useful in sociology for role analysis and in organizational studies, where it’s important to understand how different groups (e.g., departments, hierarchies) interact, regardless of the density of the connections within each group.
  - Computationally expensive and best suited for small networks
- **Spring**
...
- **Bipartite** - Nodes are members of two sets and edges between nodes are only between nodes of different sets. They do not contain odd-length cycles.
