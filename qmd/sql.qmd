...
* Order of Operations![](./_resources/SQL.resources/1-MCyFDF35OZ3n0dyZqX6_Tw.png)
...
* **Index** \- a quick lookup table (e.g. field or set of fields) for finding records users need to search frequently. An index is small, fast, and optimized for quick lookups. It is very useful for connecting the relational tables and searching large tables. (also see [DB, Engineering](DB, Engineering) >> Cost Optimizations)
...
* **Surrogate key** - very similar to a primary key in that it is a unique value for an object in a table. However, rather than being derived from actual data present in the table, it is a field generated by the object itself. It has no business value like a primary key does, but is rather only used for data analysis purposes. Can be generated using different columns that already exist in your table or more often from two or more tables. [dbt function definition](https://github.com/dbt-labs/dbt-utils#surrogate_key-source)
	* Examples: PostgreSQL serial column, Oracle sequence column, or MySQL auto\_increment column, Snowflake \_file + \_line columns
	* Example: Each employee id is concatenated with a department id (e.g. marketing or finance)![](./_resources/SQL.resources/1-upKcdpMyn4wxcKk8wuHj4A.png)
...
* **Views** - database objects that represent saved SELECT queries in “virtual” tables.
	...
	* Some views are updateable but under certain conditions (1-1 mapping of rows in view to underlying table, no group\_by, etc.)
...
* Create Tables
	* If you don't include the schema as part of the table name (e.g. schema\_name.table\_name), pgadmin automatically places it into the "public" schema directory
...
* "students" is the name of the table; "id", "name", and "classroom\_id" are the fields
* Create a foreign key that points to the "classrooms" table
	* Expression
		* fk\_classrooms is the name of the `CONSTRAINT`
		* "classroom\_id" is the field that will be the `FOREIGN KEY`
		* `REFERENCES` points the foreign key to the classrooms table's primary key, "id"
			* foreign keys can point to any table
	* Postgres won’t allow you to insert a row into students with a "classroom\_id" that doesn’t exist in the "id" field of classrooms but will allow you to use a NULL placeholder
...
* "current\_date" is a function that will insert the current date as a value
...
* Add Data
	* [Example]{.ribbon-highlight}: Copy/Paste table values into chatGPT to get the query![](./_resources/SQL.resources/image.11.png)
...
* "assignments" is the table; "category", "name", "due\_date", "weight" are fields that you want to import from the csv file
...
* Subqueries
	* \*\*Better to used CTEs instead of subqueries\*\*
	...
	* Tables used in examples
		* Store A (store\_a)![](./_resources/SQL.resources/image.10.png)
			* Store B is similar
...
* filters the rows with products that have been bought at least three times in store\_b
...
* can’t apply an average directly, as our table is oriented to product\_ids and not to transaction\_ids
...
* Joins![](./_resources/SQL.resources/FPggFYUUcAszeHX.jpeg)![](./_resources/SQL.resources/FPggFYRVkAMc93Z.jpeg)
	* Cross Join -  acts like an expand\_grid; where each value in the join key column gets all combinations of rows in both tables (also see above pic)![](./_resources/SQL.resources/image.9.png)
		* Efficient join
...
* Can use map\_dfr if all the tables have the same columns

* Dynamic queries with [{glue}]{style='color: #990000'}
	...
	* General Optimizations
		...
		* Replace complex code with window functions
			* Example: Replace `GROUP_BY` + `TOP` with a partition + `FIRST_VALUE()`
				* e.g. `FIRST_VALUE(test_score) OVER(PARTITION BY student_name ORDER BY test_score DESC)`
...
* Joins
	...
	* Avoid using"Self-Joins." Use window functions instead (see [Google, BigQuery](Google, BigQuery) >> Optimization for details on self-joins)
	...
	* Break-up joins using OR into UNION because SQL uses nested operations for JOIN + OR queries which slow things.
		* Bad![](./_resources/SQL.resources/1-9K5I9iIKl5gUoYe9mw2CXw.png)
		* Good![](./_resources/SQL.resources/1-rXsv2pEUKL3qBkM7DK8bVQ.png)
...
* Style Guide![](./_resources/SQL.resources/1-HOcSBns8ec2jIO5A0gGHEw.png)
...
* "score\_index" is the name of the index
...
* _Invalid_ usage of multiple column index
...
* The "name" index won't be used in these queries since
	* first\_name is NOT the left-most column specified in the index
...
* Note:
		idx\_a on column\_p, column\_q desc is not the same as an
		* Index idx\_a on column\_q desc, column p or,
		* Index idx\_b on column\_p desc, column q
...
* Misc
	* Also see
		...
		* [Google, BigQuery](Google, BigQuery) >> Optimizations >> Partitioning and Clustering
		* [DB, Engineering](DB, Engineering) >> Cost Optimizations
...
* List views![](./_resources/SQL.resources/image.4.png)
...
* "table\_name" has the names of the views
* "view\_definition" shows the query stored in the view
...
* DATE\_TRUNC - pulls a component of a date object.
...
* UNNEST - BigQuery - takes an ARRAY and returns a table with a row for each element in the ARRAY ([docs](https://medium.com/firebase-developers/how-to-use-select-from-unnest-to-analyze-multiple-parameters-in-bigquery-for-analytics-5838f7a004c2))
	* [Google, Analytics](Google, Analytics) >> Analysis >> Example 17
...
* "@exponent" is the input variable
...
* Cursors ([Docs](https://www.postgresql.org/docs/current/plpgsql-cursors.html))
	...
	* Example ([article](https://towardsdatascience.com/5-advanced-sql-techniques-for-real-life-projects-f2db9b6680e2) (do not pay attention dynamic sql. it's for embedding sql in C programs))
...
* A table called “products” that contains information about all products, including the product ID, product name, and current inventory. You can use a cursor to iterate through all orders that contain a specific product and update its inventory.
	A cursor called “cur\_orders” that selects all order details that contain a specific product ID. We then define a variable called “product\_inventory” to store the current inventory of the product.
...
* Unlike GROUP BY, keeps original columns after an aggregation![](./_resources/SQL.resources/1-KHkgYYK0Up46b8BunVMYoQ.png)
...
* 3 Types of Window Functions![](./_resources/SQL.resources/1-u8SPxze2b1OdCdPo1maVGw.png)
	...
	* FIRST\_VALUE() returns the first value in an ordered, partitioned data output.
* General Syntax![](./_resources/SQL.resources/1-NKUDO4a5fu5kVW-OlkHUJw.png)
	* window\_function is the name of the window function we want to use (e.g. see above)
		* expression is the name of the column that we want the window function operated on.
			* May not be necessary depending on what window\_function is used
	...
	* PARTITION BY divides the rows into partitions so we can specify which rows to use to compute the window function
		* partition\_list is the name of the column(s) we want to partition by (i.e. group\_by)
	* ORDER BY is used so that we can order the rows within each partition. This is optional and does not have to be specified
		* order\_list is the name of the column(s) we want to order by
	* ROWS (optional; typically not used) used to subset the rows _within each partition_.
		* frame\_clause defines how much to offset from our current row
		* Syntax: ROWS BETWEEN <starting\_row> AND <ending\_row>
...
* [Example]{.ribbon-highlight}: Average Salary by Job Title![](./_resources/SQL.resources/1-GVX7Ws5Dk_f9-a6o9Wy_kQ.png)![](./_resources/SQL.resources/1-KHkgYYK0Up46b8BunVMYoQ.1.png)
* **Tables for Examples**![](./_resources/SQL.resources/1-vz3DggeLfupObUnYlCa11A.png)
* [Example]{.ribbon-highlight}: Average Unit Price for each CustomerId![](./_resources/SQL.resources/1-c9eFD04nSDvxmzg2zza8oA.png)
...
* [Example]{.ribbon-highlight}: Average Unit Price for each group of CustomerId AND EmployeeId![](./_resources/SQL.resources/1-NEoVEmqBzFSlX3NHNK9dUg.png)
...
* [Example]{.ribbon-highlight}: Create a new column that ranks Unit Price in descending order for each CustomerId![](./_resources/SQL.resources/1-s7wtLz5LVZleovQtIFgQJg.png)
...
* Substituting RANK in place of ROW\_NUMBER should produce the same results
* Note that ranks are skipped (e.g. rank 3 for ALFKI) when there are rows with the same rank
	* If you don't want ranks skipped, use DENSE\_RANK for the window function

* [Example]{.ribbon-highlight}: Create a new column that provides the previous order date’s Quantity for each ProductId![](./_resources/SQL.resources/1-U5Pb6kyZYtMvbhHw13o3YA.png)
...
* Use LEAD for the _following_ quantity

* [Example]{.ribbon-highlight}: Create a new column that provides the very first Quantity ever ordered for each ProductId![](./_resources/SQL.resources/1-crsYDwLQoWTRX1Ilrkwusw.png)
...
* [Example]{.ribbon-highlight}: Calculate a cumulative moving average UnitPrice for each CustomerId![](./_resources/SQL.resources/1-FK8pEUSQhqKZv4FGNekZFA.png)
...
* Running Totals/Averages (Cumulative Sums)
	...
	* [Example]{.ribbon-highlight}![](./_resources/SQL.resources/1-vZiZM7MBwfeHhi8GP3QrtQ.png)
		* Generate a new dataset grouped by month, instead of timestamp. (CTE)
			* Only include three fields: account\_id, occurred\_month and total\_amount\_usd
			* Only computed for the following accounts: 1041 , 1051, 1061, 10141.
		* Compute a running total ordered by occurred\_month, without collapsing the rows in the result set.
			* Display 2 columns: occurred\_month and cum\_amnt\_usd\_by\_month![](./_resources/SQL.resources/1-KcELs2mkbYN3XKk0Gzk-jg.png)![](./_resources/SQL.resources/1-2u7h_5kxKinnn15C7VGmiw.png)
		* Because no partition was specified, the running total is applied on the full dataset and ordered by (ascending) occurred\_month
	* [Example]{.ribbon-highlight} running total by grouping variable
		* Using previous CTE
			* Compute a running total by account\_id, ordered by occurred\_month, and account\_id (i.e. a separate running total for each account\_id.)
				* Display 3 columns: account\_id, occurred\_month, and cum\_mon\_amnt\_usd\_by\_account![](./_resources/SQL.resources/1-Axkp3Gp-fk5iWysxLJXklg.png)![](./_resources/SQL.resources/1-0jfZNXzNQ0jVT_bIW_KpCw.png)
		* Same as previous example except a partition column (account\_id) is added
	* [Example]{.ribbon-highlight} Running total over various window lengths
		* Using previous CTE
			...
			* Compute a 7 months rolling running total using a window where the current month is always the middle month.![](./_resources/SQL.resources/1-lbxUJId6dY1fJHz44A1RKQ.png)![](./_resources/SQL.resources/1-UXXt7TGvkdFFqm-opBx1mg.png)
		* First case uses 2 PRECEDING rows and the CURRENT\_ROW
		* Second case uses 3 PRECEDING rows and 3 FOLLOWING rows and the CURRENT\_ROW
...
* Also see
	...
	* [Google, Analytics](Google, Analytics) >> Analysis >> Examples 12-15, 18, 19
* Use Cases
...
* Improves readability and _usually_ no performance difference
...
* Query is creating a new categorical column, "page\_title\_adjusted," that is "PDP" when a substring in "page\_location" is one of a set of words, and "PLP" when it's not, and the value of page\_title otherwise.
* SPLIT splits the string by separator, '/'
* CONTAINS\_SUBSTR is looking for substring with a "+"
* \[SAFE\_OFFSET(3)\] pulls the 4th substring (think this indexes by 0?)
	* after it's been reversed via ARRAY\_REVERSE (?)
* ELSE says use the value for page\_title when length of the substrings after splitting page\_location is 5 or less
* "unnested\_events" is a CTE
...
* Create a table with Arrays (Postgres)![](./_resources/SQL.resources/image.5.png)
...
* Subset an array (postgres)![](./_resources/SQL.resources/image.7.png)
...
* Slice an array (postgres)![](./_resources/SQL.resources/image.8.png)
...
* Unnest an array (postgres)![](./_resources/SQL.resources/image.6.png)
...
* Only rows with arrays that have "product\_c" will be returned
...
* First update: all arrays where cart\_id == 1 are set to \['product\_a','product\_b','product\_e'\]
* Second update: all array first values where cart\_id == 2 are set to 'product\_f'
...
* arrays in product column where cart\_id == 1 get "product\_x" appended to the end of their arrays
...
* arrays in product column where cart\_id == 2 get "product\_x" prepended to the beginning of their arrays
...
* arrays in product column where cart\_id == 1 get "product\_e" removed from their arrays
...
* `ARRAY_CONTAINS` checks if a _variant_ is included in an array and returns a boolean value. ([doc](https://docs.snowflake.com/en/sql-reference/functions/array_contains.html))
...
* "@>" example returns all rows with arrays containing product\_a and product\_b
...
* Rank product categories by shipping cost for each shipping address![](./_resources/SQL.resources/image.png)
...
* `RANK()` is retrieves ranked rows based on the condition of `ORDER BY` clause. As you can see there is a tie between 1st two rows i.e. first two rows have same value in Shipping\_Cost column (which is mentioned in ORDER BY clause).
...
* Rank resets to 1 when "Shipping\_Address" changes location
...
* For one particular month, then create an indicator column with "if posting\_publish\_date ≤ 2022–01–01 and (posting\_closed\_date is null or posting\_closed\_date > 2022–01–31) then True" and then filter for True and count.
...
* Pivot Wider![](./_resources/SQL.resources/image.1.png)
...
* `strftime` codes![](./_resources/SQL.resources/image.2.png)
...
* Use a hash column as id column, then test for duplicates, remove them or investigate them (BigQuery)![](./_resources/SQL.resources/image.3.png)
...
* No duplicate records found, since "records" = 9 and "unique\_records" = 9
	* if records > unique\_records, duplicates exist
* Can select distinct hex\_row\_hash if you want to remove duplicates
* Can count hex\_row\_hash then filter where hex\_row\_hash > 1 to find which rows are duplicates
...
* Description
	...
	* zoo\_elephants is the dataset
* Steps
	* TO\_JSON\_STRING - creates column with json string for each row
	* MD5 hashes that string
	* TO\_HEX makes it alpha-numeric and gets rid of the symbols in the hash
		* Easier to deal with in BigQuery
		* Assume this is still unique (?)
* Note: By adding "true" value, `TO_JSON_STRING(inbound_zoo_elephants, true)` , TO\_JSON\_STRING adds line breaks to the json string for easier readability.
...
* `UNION ALL`combines the results of both statements.
	* Example is defined by 2 Select statements
		* Anchor Member: First SELECT statement selects the root nodes of the category tree (nodes with no parent)
			* Root node is indicated by "parent\_id" = NULL
...
* The "depth" column is used to keep track of the depth of each category node in the tree.
	...
	* "category\_tree.depth + 1" in the second statement
...
* Preprocess Time Series with 4 Lags ([article](https://towardsdatascience.com/concurrently-train-multiple-time-series-models-over-spark-with-xgboost-c6d5ec4a6430))
