...
-   Order of Operations\
    ![](./_resources/SQL.resources/1-MCyFDF35OZ3n0dyZqX6_Tw.png){width="370"}
...
-   [**Batch**]{style="color: #14adff"} - a set of sql statements e.g. statements between BEGIN and END
-   [**Compiled object**]{style="color: #14adff"} - you can create a function written in C/C++ and load into the database (at least in postgres) to achieve high performance.
-   [**Correlated Columns**]{style="color: #14adff"} - tells how good the match between logical and physical ordering is.
-   [**Correlated/Uncorrelated** **Subqueries**]{style="color: #14adff"}
    ...
    -   uncorrelated - a type of sub-query where inner query doesn't depend upon the outer query for its execution.
        ...
        ```
-   [**Functions**]{style="color: #14adff"} execute at a different level of priority and are handled differently than Views. You will likely see better performance.
-   [**Index**]{style="color: #14adff"} - a quick lookup table (e.g. field or set of fields) for finding records users need to search frequently. An index is small, fast, and optimized for quick lookups. It is very useful for connecting the relational tables and searching large tables. (also see [DB, Engineering](db-engineering.qmd#sec-db-eng-costopt) \>\> Cost Optimizations)
-   [**Migrations (schema)**]{style="color: #14adff"} - version control system for your database schema. Management of incremental, reversible changes and version control to relational database schemas. A schema migration is performed on a database whenever it is necessary to update or revert that database's schema to some newer or older version.
-   [**Physical Ordering**]{style="color: #14adff"} - A PostgreSQL table consists of one or more files of 8KB blocks (or "pages"). The order in which the rows are stored in the file is the physical ordering.
-   [**Predicate**]{style="color: #14adff"} - defines a logical condition being applied to rows in a table. (e.g. IN, EXISTS, BETWEEEN, LIKE, ALL, ANY)
-   [**Scalar/Non-Scalar Subqueries**]{style="color: #14adff"}
...
-   [**Selectivity**]{style="color: #14adff"} - the fraction of rows in a table or partition that is chosen by the predicate
...
-   [**Surrogate key**]{style="color: #14adff"} - very similar to a primary key in that it is a unique value for an object in a table. However, rather than being derived from actual data present in the table, it is a field generated by the object itself. It has no business value like a primary key does, but is rather only used for data analysis purposes. Can be generated using different columns that already exist in your table or more often from two or more tables. [dbt function definition](https://github.com/dbt-labs/dbt-utils#surrogate_key-source)
    ...
    -   Example: Each employee id is concatenated with a department id (e.g. marketing or finance)\
        ![](./_resources/SQL.resources/1-upKcdpMyn4wxcKk8wuHj4A.png){width="470"}
-   [**Transaction**]{style="color: #14adff"} - a set of queries tied together such that if one query fails, the entire set of queries are rolled back to a pre-query state if the situation dictates.
...
-   [**Views**]{style="color: #14adff"} - database objects that represent saved SELECT queries in "virtual" tables.
...
-   [Example]{.ribbon-highlight}: Copy/Paste table values into chatGPT to get the query\
    ![](./_resources/SQL.resources/image.11.png){width="423"}
...
-   Tables used in examples
    ...
    -   Store A (store_a)\
        ![](./_resources/SQL.resources/image.10.png){width="300"}
        -   Store B is similar
...
![](./_resources/SQL.resources/FPggFYUUcAszeHX.jpeg){width="354"}\
![](./_resources/SQL.resources/FPggFYRVkAMc93Z.jpeg){width="354"}
...
-   Cross Join -  acts like an expand_grid; where each value in the join key column gets all combinations of rows in both tables (also see above pic)\
    ![](./_resources/SQL.resources/image.9.png){width="296"}
...
-   Dynamic queries with [{glue}]{style="color: #990000"}
    ...
    -   [Example]{.ribbon-highlight}: MS SQL Server
...
-   Joins
    ...
    -   Break-up joins using OR into UNION because SQL uses nested operations for JOIN + OR queries which slow things.
        -   Bad\
            ![](./_resources/SQL.resources/1-9K5I9iIKl5gUoYe9mw2CXw.png){width="335"}
        -   Good\
            ![](./_resources/SQL.resources/1-rXsv2pEUKL3qBkM7DK8bVQ.png){width="328"}
...
-   Style Guide\
    ![](./_resources/SQL.resources/1-HOcSBns8ec2jIO5A0gGHEw.png){width="752"}
...
-   List views\
    ![](./_resources/SQL.resources/image.4.png){width="500"}
...
-   Unlike GROUP BY, keeps original columns after an aggregation\
    ![](./_resources/SQL.resources/1-KHkgYYK0Up46b8BunVMYoQ.png){width="370"}
...
-   3 Types of Window Functions\
    ![](./_resources/SQL.resources/1-u8SPxze2b1OdCdPo1maVGw.png){width="322"}
...
-   General Syntax\
    ![](./_resources/SQL.resources/1-NKUDO4a5fu5kVW-OlkHUJw.png){width="440"}
...
-   [Example]{.ribbon-highlight}: Average Salary by Job Title\
    ![](./_resources/SQL.resources/1-GVX7Ws5Dk_f9-a6o9Wy_kQ.png){width="370"}\
    ![](./_resources/SQL.resources/1-KHkgYYK0Up46b8BunVMYoQ.1.png){width="200"}

-   **Tables for Examples**\
    ![](./_resources/SQL.resources/1-vz3DggeLfupObUnYlCa11A.png){width="279"}

-   [Example]{.ribbon-highlight}: Average Unit Price for each CustomerId\
    ![](./_resources/SQL.resources/1-c9eFD04nSDvxmzg2zza8oA.png){width="215"}
...
-   [Example]{.ribbon-highlight}: Average Unit Price for each group of CustomerId AND EmployeeId\
    ![](./_resources/SQL.resources/1-NEoVEmqBzFSlX3NHNK9dUg.png){width="180"}
...
-   [Example]{.ribbon-highlight}: Create a new column that ranks Unit Price in descending order for each CustomerId\
    ![](./_resources/SQL.resources/1-s7wtLz5LVZleovQtIFgQJg.png){width="225"}
...
-   [Example]{.ribbon-highlight}: Create a new column that provides the previous order date's Quantity for each ProductId\
    ![](./_resources/SQL.resources/1-U5Pb6kyZYtMvbhHw13o3YA.png){width="257"}
...
-   [Example]{.ribbon-highlight}: Create a new column that provides the very first Quantity ever ordered for each ProductId\
    ![](./_resources/SQL.resources/1-crsYDwLQoWTRX1Ilrkwusw.png){width="213"}
...
-   [Running Totals/Averages (Cumulative Sums)]{.underline}
    ...
    -   [Example]{.ribbon-highlight}\
        ![](./_resources/SQL.resources/1-vZiZM7MBwfeHhi8GP3QrtQ.png){width="575"}
        ...
        -   Compute a running total ordered by occurred_month, without collapsing the rows in the result set.
            -   Display 2 columns: occurred_month and cum_amnt_usd_by_month\
                ![](./_resources/SQL.resources/1-KcELs2mkbYN3XKk0Gzk-jg.png){width="322"}![](./_resources/SQL.resources/1-2u7h_5kxKinnn15C7VGmiw.png)
        ...
        -   Using previous CTE
            -   Compute a running total by account_id, ordered by occurred_month, and account_id (i.e. a separate running total for each account_id.)
                -   Display 3 columns: account_id, occurred_month, and cum_mon_amnt_usd_by_account\
                    ![](./_resources/SQL.resources/1-Axkp3Gp-fk5iWysxLJXklg.png){width="685"}![](./_resources/SQL.resources/1-0jfZNXzNQ0jVT_bIW_KpCw.png)
    ...
    -   [Example]{.ribbon-highlight} Running total over various window lengths
        ...
        -   Using previous CTE
            ...
            -   Compute a 7 months rolling running total using a window where the current month is always the middle month.\
                ![](./_resources/SQL.resources/1-lbxUJId6dY1fJHz44A1RKQ.png)![](./_resources/SQL.resources/1-UXXt7TGvkdFFqm-opBx1mg.png){width="775"}
...
-   Create a table with Arrays (Postgres)\
    ![](./_resources/SQL.resources/image.5.png){width="300"}
...
-   Subset an array (postgres)\
    ![](./_resources/SQL.resources/image.7.png){width="220"}
...
-   Slice an array (postgres)\
    ![](./_resources/SQL.resources/image.8.png){width="236"}
...
-   Unnest an array (postgres)\
    ![](./_resources/SQL.resources/image.6.png){width="208"}
...
-   [Example]{.ribbon-highlight}: Rank product categories by shipping cost for each shipping address\
    ![](./_resources/SQL.resources/image.png){width="450"}
...
-   Misc
    ...
    -   Also see [Terms](sql.qmd#sec-sql-terms){style="color: green"} \>\> Transaction
...
-   [Pivot Wider]{.underline}\
    ![](./_resources/SQL.resources/image.1.png){width="366"}
...
-   [Extract components from date-time columns]{.underline}
    ...
    -   `strftime` codes\
        ![](./_resources/SQL.resources/image.2.png){width="480"}

-   [Duplicated Rows]{.underline}
    ...
    -   Use a hash column as id column, then test for duplicates, remove them or investigate them (BigQuery)\
        ![](./_resources/SQL.resources/image.3.png){width="294"}
