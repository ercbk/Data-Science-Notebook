...
-   Order of Operations\
    ![](./_resources/SQL.resources/1-MCyFDF35OZ3n0dyZqX6_Tw.png)
...
-   **Index** - a quick lookup table (e.g. field or set of fields) for finding records users need to search frequently. An index is small, fast, and optimized for quick lookups. It is very useful for connecting the relational tables and searching large tables. (also see [DB, Engineering](db-engineering.qmd#sec-db-eng-costopt) \>\> Cost Optimizations)
...
-   **Surrogate key** - very similar to a primary key in that it is a unique value for an object in a table. However, rather than being derived from actual data present in the table, it is a field generated by the object itself. It has no business value like a primary key does, but is rather only used for data analysis purposes. Can be generated using different columns that already exist in your table or more often from two or more tables. [dbt function definition](https://github.com/dbt-labs/dbt-utils#surrogate_key-source)
    ...
    -   Example: Each employee id is concatenated with a department id (e.g. marketing or finance)\
        ![](./_resources/SQL.resources/1-upKcdpMyn4wxcKk8wuHj4A.png)
...
-   [Example]{.ribbon-highlight}: Copy/Paste table values into chatGPT to get the query\
    ![](./_resources/SQL.resources/image.11.png)
...
-   Tables used in examples
    ...
    -   Store A (store_a)![](./_resources/SQL.resources/image.10.png)
        -   Store B is similar
...
-   Style Guide\
    ![](./_resources/SQL.resources/1-HOcSBns8ec2jIO5A0gGHEw.png){width="912"}
...
-   List views\
    ![](./_resources/SQL.resources/image.4.png)
...
-   `UNNEST` - BigQuery - takes an ARRAY and returns a table with a row for each element in the ARRAY ([docs](https://medium.com/firebase-developers/how-to-use-select-from-unnest-to-analyze-multiple-parameters-in-bigquery-for-analytics-5838f7a004c2))
    -   [Google Analytics, Analysis >> Example 17](google-analytics-analysis.qmd#sec-goog-goog-anal){style='color: green'}
...
-   Unlike GROUP BY, keeps original columns after an aggregation  
    ![](./_resources/SQL.resources/1-KHkgYYK0Up46b8BunVMYoQ.png)
...
-   3 Types of Window Functions  
    ![](./_resources/SQL.resources/1-u8SPxze2b1OdCdPo1maVGw.png)
...
-   General Syntax  
    ![](./_resources/SQL.resources/1-NKUDO4a5fu5kVW-OlkHUJw.png)
...
-   [Example]{.ribbon-highlight}: Average Salary by Job Title  
    ![](./_resources/SQL.resources/1-GVX7Ws5Dk_f9-a6o9Wy_kQ.png)  
    ![](./_resources/SQL.resources/1-KHkgYYK0Up46b8BunVMYoQ.1.png)

-   **Tables for Examples**  
    ![](./_resources/SQL.resources/1-vz3DggeLfupObUnYlCa11A.png)

-   [Example]{.ribbon-highlight}: Average Unit Price for each CustomerId  
    ![](./_resources/SQL.resources/1-c9eFD04nSDvxmzg2zza8oA.png)
...
-   [Example]{.ribbon-highlight}: Average Unit Price for each group of CustomerId AND EmployeeId  
    ![](./_resources/SQL.resources/1-NEoVEmqBzFSlX3NHNK9dUg.png)
...
-   [Example]{.ribbon-highlight}: Create a new column that ranks Unit Price in descending order for each CustomerId  
    ![](./_resources/SQL.resources/1-s7wtLz5LVZleovQtIFgQJg.png)
...
-   [Example]{.ribbon-highlight}: Create a new column that provides the previous order date's Quantity for each ProductId  
    ![](./_resources/SQL.resources/1-U5Pb6kyZYtMvbhHw13o3YA.png)
...
-   [Example]{.ribbon-highlight}: Create a new column that provides the very first Quantity ever ordered for each ProductId  
    ![](./_resources/SQL.resources/1-crsYDwLQoWTRX1Ilrkwusw.png)
...
-   [Running Totals/Averages (Cumulative Sums)]{.underline}
    ...
    -   [Example]{.ribbon-highlight}  
        ![](./_resources/SQL.resources/1-vZiZM7MBwfeHhi8GP3QrtQ.png)
        ...
        -   Compute a running total ordered by occurred_month, without collapsing the rows in the result set.
            -   Display 2 columns: occurred_month and cum_amnt_usd_by_month  
                ![](./_resources/SQL.resources/1-KcELs2mkbYN3XKk0Gzk-jg.png)![](./_resources/SQL.resources/1-2u7h_5kxKinnn15C7VGmiw.png)
        ...
        -   Using previous CTE
            -   Compute a running total by account_id, ordered by occurred_month, and account_id (i.e. a separate running total for each account_id.)
                -   Display 3 columns: account_id, occurred_month, and cum_mon_amnt_usd_by_account  
                    ![](./_resources/SQL.resources/1-Axkp3Gp-fk5iWysxLJXklg.png)![](./_resources/SQL.resources/1-0jfZNXzNQ0jVT_bIW_KpCw.png)
    ...
    -   [Example]{.ribbon-highlight} Running total over various window lengths
        -   Using previous CTE
            ...
            -   Compute a 7 months rolling running total using a window where the current month is always the middle month.  
                ![](./_resources/SQL.resources/1-lbxUJId6dY1fJHz44A1RKQ.png)![](./_resources/SQL.resources/1-UXXt7TGvkdFFqm-opBx1mg.png)
...
-   Also see
    ...
    -   [Google, Google Analytics, Analysis]((google-analytics-analysis.qmd#sec-goog-goog-anal)) \>\> Examples 12-15, 18, 19
-   Use Cases
...
-   `RANK()` is retrieves ranked rows based on the condition of `ORDER BY` clause. As you can see there is a tie between 1st two rows i.e. first two rows have same value in Shipping_Cost column (which is mentioned in ORDER BY clause).
