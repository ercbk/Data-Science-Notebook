...
- For WSL/linux users, if installing odbc for R via `install.packages('odbc')` throws an error, try installing unixodbc in a bash terminal, `sudo apt install unixodbc` ([source](https://bsky.app/profile/edenian-prince.bsky.social/post/3lhm6fqkry22j))
...
- Queries in examples
  ...
  - Window Functions
    ...
    - Three-day moving average of the temperature in each city
  - CTE
  ...
  - Business Queries
    - Total Sales, Customers, Sales per Customer per Region
...
- Comments
  ...
  - Multi-Line
    ...
    ```
- Counting Rows\
  ![](_resources/SQL.resources/count-rows-bork-1.png){.lightbox width="270"}
...
- Update Target Table by transaction date (BQ) ([link](https://towardsdatascience.com/advanced-sql-techniques-for-beginners-211851a28488))
  ...
  - [\_partitiontime]{.arg-text} is a field BQ creates to record the row's ingestion time (See [Databases, BigQuery \>\> Optimization](db-bigquery.qmd#sec-db-bigq-opt){style="color: green"} \>\> Partitions
...
- Misc
  - Be cautious when using CTEs with `DELETE` (or `UPDATE`) and `LIMIT`, especially when relying on the `RETURNING` clause for atomicity. The planner’s optimization choices might surprise you and lead to behavior counter to your intent (e.g. unnecessary loops).
- DELETE FROM
  - [Example]{.ribbon-highlight}: postgres ([source](https://www.shayon.dev/post/2025/119/a-postgresql-planner-gotcha-with-ctes-delete-and-limit/))
    ...
    - Find one available task row (`LIMIT 1`), delete it, and return its associated `item_id`.
    - Use `WHERE id =` for a single expected ID
    - If you strictly need to handle the case where zero rows might be found by the subquery without erroring, using `id IN (...)` might be necessary.
      - Make sure to test the plan, because the planner has been known to choose a less efficient loop plan where the subquery gets ran more than once.
- `DELETE CASCADE`
...
### Misc {#sec-sql-r-misc .unnumbered}

- [Thread](https://fosstodon.org/@sckottie/113319547849135263) regarding querying when permissions/privileges only allow partial access to a table (e.g. specific columns)
- Packages
  - [{]{style="color: #990000"}[dbi.table](https://cran.r-project.org/web/packages/dbi.table/index.html){style="color: #990000"}[}]{style="color: #990000"} - Database Queries Using 'data.table' Syntax
  - [{]{style="color: #990000"}[SQLFormatteR](https://github.com/dataupsurge/SQLFormatteR/){style="color: #990000"}[}]{style="color: #990000"} - Provides an easy-to-use wrapper for the sqlformat-rs Rust-based SQL formatter. It allows you to format SQL queries with support for various SQL dialects directly in R.
- Quarto
...
- Commands
  ...
  - Use `compute()` to run the query and save the results in a temporary table in the database
  - Use `collect()` to retrieve the results to R.
  - Use `show_query` to see the query.
  - Use `explain` to describe the query plan
  - `copy_inline` - An alternative to `copy_to` that does not need write access and is faster for small data.
...
- Joins
  ...
  - Avoid using"Self-Joins." Use window functions instead (see [Databases, BigQuery \>\> Optimization](db-bigquery.qmd#sec-db-bigq-opt){style="color: green"} for details on self-joins)
...
- Misc
  - Also see
    ...
    - [Databases, BigQuery \>\> Optimization \>\> Partitioning and Clustering](db-bigquery.qmd#sec-db-bigq-opt){style="color: green"}
...
- `EXPLAIN` - a means of running your query as a what-if to see what the planner thinks about it. It will show the process the system goes through to get to the data and return it.
  ...
  */
  ...
  - Also see
    ...
    - [Databases, Postgres \>\> Performance Tuning](db-postgres.qmd#sec-db-pstgr-perftun){style="color: green"} \>\> EXPLAIN
...
- `EXPLAIN ANALYZE` - tells the planner to not only hypothesize on what it would do, but actually run the query and show the results.
  ...
  - **Don’t use EXPLAIN ANALYZE on destructive operations like DELETE/UPDATE,EXPLAIN will suffice and it doesn’t run the query**
  - **Don’t use EXPLAIN ANALYZE when resources are scarce like production monitoring, and when a query never finishes*,* EXPLAIN will suffice and it doesn’t run the query**
  - Also see [Databases, Engineering \>\> Cost Optimizations](db-engineering.qmd#sec-db-eng-costopt){style="color: green"}

- `FILTER` - Only the input rows for which the filter clause evaluates to true are fed to the aggregate function; other rows are discarded ([postgres docs](https://www.postgresql.org/docs/14/sql-expressions.html))

  - Especially useful when you want to perform multiple aggregations in your query.
  - More readable than using CASE WHEN
  - Also see
    - [Window Functions](sql.qmd#sec-sql-winfun){style="color: green"} \>\>
      - Calculate the number consecutive days spent in each country
      - Three-day moving average of the temperature in each city
    - [Arrays](sql.qmd#sec-sql-arr){style="color: green"} \>\> `ARRAY_AGG`
  - [Example]{.ribbon-highlight}: Multiple Aggregations ([source](https://www.timescale.com/learn/understanding-filter-in-postgresql-with-examples))
    - With FILTER
...
- [Example]{.ribbon-highlight}: Three-day moving average of the temperature in each city (postgres, [source](https://www.timescale.com/learn/understanding-filter-in-postgresql-with-examples))

  ``` sqlpostgresql
  SELECT city, day, temperature,
      MAX(temperature) FILTER (WHERE temperature > 70)
      OVER (
        PARTITION BY city
        ORDER BY day ASC
...
- `ARRAY_AGG` - gather values of a group by variable into an array ([doc](https://docs.snowflake.com/en/sql-reference/functions/array_agg.html))
  ...
  - [Example]{.ribbon-highlight}: With FILTER in a CTE (postgres?, [source](https://towardsdatascience.com/measuring-cross-product-adoption-using-dbt-set-similarity-fdf7c1f88bc2))
    ...
    1.  I think the author wanted an array output, but to *also* use `FILTER`. `FILTER` can only be used with aggregate functions. This shows that `ARRAY_AGG` doesn't necessarily need `GROUP BY` function

    2.  Customers are aggregated by the storage they bought and their user category
...
- Total Sales, Customers, Sales per Customer per Region
  ...
  - DuckDB SQL allows you to read directly from a csv
...
- [databow](https://github.com/columnar-tech/databow) ([docs](https://docs.columnar.tech/databow/), [intro](https://columnar.tech/blog/introducing-databow//)) - A open source command-line tool for querying any database that has an ADBC (Arrow Database Connectivity) driver. Built in Rust, databow gives you one fast, modern interface to the SQL systems across your entire data stack
  - Includes:
...
- [ggsql](https://ggsql.org/) - Allows pure SQL users to create ggplot2 style visualizations via a sql query. Based on grammar of graphics. More reproducible than using a gui-based tool
  - Works inside of positron, vs code, jupyter, quarto
  - Easier for LLMs to use since they're very good at sql
  - Embedding a small executable in other tools is much easier than bundling R/Python (or requiring them to be installed)
    - e.g. lighter to use in remote production environment than installing R/Python
  - A smaller scope makes it easier to sandbox and prevent malicious code execution (either deliberately or in error)
- [SQL Crack](https://marketplace.visualstudio.com/items?itemName=buvan.sql-crack) - VS Code extension that visualizes SQL queries as interactive execution flow diagrams. Understand complex queries at a glance, track data lineage across your entire workspace, and identify optimization opportunities with professional-grade visual analysis.
- [SQLFluff](https://github.com/sqlfluff/sqlfluff) - A rust-based, dialect-flexible, configurable SQL linter. Designed with ELT applications in mind and will auto-fix most linting errors.
  - Also works with Jinja templating and dbt. SQLFluff
  - [{]{style="color: #990000"}[sqlfluffr](https://cran.r-project.org/web/packages/sqlfluffr/index.html){style="color: #990000"}[}]{style="color: #990000"} - An R interface to the 'Python' 'sqlfluff' 'SQL' linter and formatter via the 'reticulate' package.
    - Includes special handling for 'glue' 'SQL' syntax with curly-brace placeholders
- [{]{style="color: goldenrod"}[sqlglot](https://github.com/tobymao/sqlglot){style="color: goldenrod"}[}]{style="color: goldenrod"} - no dependency Python SQL parser, transpiler, optimizer, and engine
  ...
  - [Example]{.ribbon-highlight}: : translate duckdb to hive
    ...
    ```
- [sqlit](https://github.com/Maxteabag/sqlit) - A user friendly Terminal UI (TUI) for SQL databases
  ...
  - Automatically finds running database containers
