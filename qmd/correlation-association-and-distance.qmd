# Correlation, Association, and Distance
...
* **Polychoric** - Suppose each of the ordinal variables was obtained by _categorizing a normally distributed underlying variable_, and those two unobserved variables follow a bivariate normal distribution. Then the (maximum likelihood) estimate of that correlation is the polychoric correlation.
...
* **Biserial** - correlation between a continuous variable and binary variable, which is _assumed to have resulted from a dichotomized normal variable_
...
* **ξ (xi) coefficient**
	...
	* n > 20 necessary
		* n larger than about 250 probably sufficient to get a _good_ estimate
...
* **Copula**
	* Misc
		...
		* Also see [Forecasting, Nonlinear](Forecasting, Nonlinear) >> Misc >> packages, copulas
	...
	* Understanding Inverse Transform Sampling (**[video](https://www.youtube.com/watch?v=rnBbYsysPaU)**) will help with understanding the copula mathematics
		* Relationship between PDF and CDF (e.g. exponential distribution)![](./_resources/Correlation,_Association,_and_Distance.resources/Screenshot (1310).png)
			* Probability x ≤ 2 is
		...
		* The inverse of the CDF give you the value of x for any probability![](./_resources/Correlation,_Association,_and_Distance.resources/Screenshot (1312).png)
...
* Sklar’s Theorem
	...
	* For two random variables S1, S2 in \[-inf, inf\]. S1 and S2 have their own fixed, continuous CDFs,  F1, F2. Consider their (cumulative) joint distribution ![](./_resources/Correlation,_Association,_and_Distance.resources/ql_9d88a0b0a8745bc599be54ed5aa8a2a8_l3.png)
		Now take the uniformly distributed quantile random variable U1(S1), U2(S2). For every pair, (u1, u2), drawn from the pair's quantile, we define the _bivariate copula_ C: \[0,1\] x \[0,1\] --> \[0,1\] as:
		![](./_resources/Correlation,_Association,_and_Distance.resources/ql_bf7df5b5b5f0877e29c212286a5c487e_l3.png)
...
* ![](./_resources/Correlation,_Association,_and_Distance.resources/copula_marginal_dist_demo-1-1030x814.png)
	...
	* Mathematically the _cumulative conditional probabilities_ shown in the scatter plot are given by taking partial derivatives of the joint inverse CDF:![](./_resources/Correlation,_Association,_and_Distance.resources/ql_f33a4928dfb8981a6bb9503e7bad03eb_l3.png)
		* aside: taking the derivative of a (not inverse) marginal (not joint) CDF is the pdf
	* The _copula density_ is defined as:![](./_resources/Correlation,_Association,_and_Distance.resources/ql_0ab910a450fad934b5dda1293c24b142_l3.png)
...
* [Example]{.ribbon-highlight}
	...
	* Defining a Gaussian Copula for these 2 random variables![](./_resources/Correlation,_Association,_and_Distance.resources/image.6.png)
		* Format: Copula = joint cdf(quantile(cdf(gamma\_random\_variable), cdf(gamma\_random\_variable))
	* Construct a Copula
...
* **CCF ** (cross-correlation function)
	...
	* the set of sample correlations between xt+h and yt for _h_ = 0, ±1, ±2, ±3, and so on. A negative value for h is a correlation between the _x_\-variable at a time before t and the _y_\-variable at time t. For instance, consider h = −2. The CCF value would give the correlation between xt−2 and yt.
...
* Components
	...
	* Window Constraints
		...
		* Sakoe-Chiba window creates a calculation region along the diagonal of the LCM
			* ![](./_resources/Correlation,_Association,_and_Distance.resources/Screenshot (46).png)
	...
	* Lower Bounds (LB)
		...
		* uses a lower bound the dtw distance to speed up computation
			* a considerably large dataset would be needed before the overhead of DTW becomes much larger than that of dtw\_lb’s iterations
