...
- Packages
  ...
  - [{]{style="color: #990000"}[BORG](https://gillescolling.com/BORG/){style="color: #990000"}[}]{style="color: #990000"} - Automatically detects and enforces valid model evaluation by identifying information reuse between training and evaluation data. Guards against data leakage, look-ahead bias, and invalid cross-validation schemes that inflate performance estimates. Supports temporal, spatial, and grouped evaluation structures
  - [{broom} supported model classes](https://broom.tidymodels.org/articles/available-methods.html)
  - [{]{style="color: #990000"}[fastml](https://cran.r-project.org/web/packages/fastml/index.html){style="color: #990000"}[}]{style="color: #990000"} - Wrapper around tidymodels for quick iteration. Also includes diagnostic packages (e.g. dalex, naniar)
  - [{]{style="color: #990000"}[leakr](https://cran.r-project.org/web/packages/leakr/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides utilities to detect common data leakage patterns including train/test contamination, temporal leakage, and data duplication, enhancing model reliability and reproducibility in machine learning workflows
  ...
  - [{]{style="color: #990000"}[rtemis](https://docs.rtemis.org/r/){style="color: #990000"}[}]{style="color: #990000"} - Not quite automl but somewhere between mlr3/tidymodels and automl. Has all the main algorithms for regression (including tabnet), classification, and clustering. Uses plotly for visualization (eda and model diagnostics). Has data check function (missingness, character vectors, etc.). The preprocessing function seems a little basic but includes imputing. The imputing function uses a couple packages that include PMM, so not bad. Overall it's hard to criticize this package too much.
  - [{]{style="color: goldenrod"}[TanML](https://github.com/tdlabs-ai/tanml){style="color: goldenrod"}[}]{style="color: goldenrod"} ([JOSS](https://joss.theoj.org/papers/10.21105/joss.11253)) - Combines model evaluation, explainability, drift analysis, and stakeholder-ready reporting in one UI-driven system
    - Intended for practitioners working in regulated or documentation heavy settings (e.g. banking, insurance, etc.) who need reviewable validation artifacts.
  - [{]{style="color: #990000"}[tidyAML](https://www.spsanderson.com/tidyAML/){style="color: #990000"}[}]{style="color: #990000"} - Automated Machine Learning (AutoML) to the tidymodels ecosystem.

- Papers

  - [There is only one correct analysis](https://osf.io/preprints/psyarxiv/hvjxf_v1) (Lakens et al 2026)
...
- Make ML model pipelines reusable and reproducible\
  ...
  - [Consistent Structures]{.underline} - Consistency in project structures and naming can reduce human error, improve communication, and just make things easier to find.
    ...
    - [Example]{.ribbon-highlight}: ([source](https://towardsdatascience.com/automated-testing-a-software-engineering-concept-data-scientists-must-know-to-succeed/))
