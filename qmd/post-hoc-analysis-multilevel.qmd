...
-   Packages
    ...
    -   [{]{style="color: #990000"}[pecanr](https://cran.r-project.org/web/packages/pecanr/index.html){style="color: #990000"}[}]{style="color: #990000"} - Computes partial eta-squared effect sizes for fixed effects in linear mixed models fitted with the [{lme4}]{style="color: #990000"}. Both general and operative effect sizes are provided
        -   Supports crossed and nested random effects structures with any number of grouping factors.
        -   Random slope variances are translated to the outcome scale using a variance decomposition approach, correctly accounting for predictor scaling and interaction terms.
