...
* Types
	* Embedded aka In-Process (see [DB, Engineering](DB, Engineering) >> Terms): DuckDB (analytics) and SQLite (transactional)
...
* MySQL
	...
	* See SQL notebook
* [Cloud SQL](https://cloud.google.com/sql/docs/introduction) - Google service to provide hosting services for relational dbs (see [Google, BigQuery](Google, BigQuery) >> Misc). Can use postgres, mysql, etc. on their machines.
...
* [AWS RDS](https://aws.amazon.com/rds/) for db instances (see [DB, postgres](DB, postgres) >> AWS RDS)
	...
	* **S3** is like googledrive or dropbox
		* Con: only contains data _about_ the files, not what’s inside them, i.e. no querying
		* Ideal use cases
			* backup for logs,
...
* Apache Avro
	...
	* 4x _slower_ reading than csv but 1.5x _faster_ writing than csv
	* 1.7x _smaller_ file size than csv
* Benchmarks
	* Example
		* Data
		...
		* Operation: read, filter, group\_by, summarize
		* Results
...
* Start documenting the spreadsheets
	...
	* Map the spreadsheets relationships to one another
		* see [DB, Warehouses](DB, Warehouses) >> Design a Warehouse
