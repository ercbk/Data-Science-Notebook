...
- Also see
  - [Geospatial, Spatio-Temporal \>\> EDA \>\> Temporal Dependence](geospatial-spat-temp.qmd#sec-geo-sptemp-eda-ac){style="color: green"}
- Resources
...
- Packages
  ...
  - [{]{style="color: #990000"}[daiquiri](https://ropensci.github.io/daiquiri/){style="color: #990000"}[}]{style="color: #990000"}
  ...
  - [{]{style="color: #990000"}[ggtime](https://pkg.mitchelloharawild.com/ggtime){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/pdf/2510.25656)) - **Various EDA ggplots** for tsibble objects (Huang, Kay, Hyndman, O’Hara-Wild)

  - [{]{style="color: #990000"}[lomb](https://cran.r-project.org/web/packages/lomb/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/1703.09824)) - Computes the **Lomb-Scargle Periodogram** (aka **Least-Squares Spectral Analysis (LSSA)**) and actogram

    - Analyzes **periodicity/seasonality for evenly or unevenly sampled** time series (e.g. missing data on weekends/holidays).
    - Effectively fits sine and cosine waves of specific frequencies to the data exactly where the data points exist, ignoring the gaps. It calculates how well different frequencies fit the observed data.
    - It is invariant to time shifts and handles uneven sampling (like weekends) natively without needing imputation.

  - [{]{style="color: #990000"}[mists](https://github.com/earowang/mists){style="color: #990000"}[}]{style="color: #990000"} - A suite of 1d, 2d, and **visual tools for exploring and polishing missing values** residing in temporal data.
  ...
  - [{]{style="color: #990000"}[MultivariateTrendAnalysis](https://cran.r-project.org/web/packages/MultivariateTrendAnalysis/index.html){style="color: #990000"}[}]{style="color: #990000"} - Univariate and Multivariate **Trend Testing**

  - [{]{style="color: #990000"}[paneldesc](https://cran.r-project.org/web/packages/paneldesc/index.html){style="color: #990000"}[}]{style="color: #990000"} - Descriptive statistics EDA for panel data.

    - Looks a little frustrating given the website (just create a pkgdown and some vignettes) and the fact that it uses the term "shares" instead of, what I think it means, proportions. Could just be my mood though.

  - [{]{style="color: #990000"}[SignalY](https://cran.r-project.org/web/packages/SignalY/index.html){style="color: #990000"}[}]{style="color: #990000"} - Signal Extraction from **Panel Data** via Bayesian Sparse Regression and Spectral Decomposition (Handles **nonpanel data** as well)

    - This package has **multiple use cases** with methods for just about everything except actual forecasting, so I'm just sticking it here.
    - Includes spectral decomposition methods for frequency analysis, bayesian sparse regression for feature selection, dimensionality reduction methods, unit root tests for stationarity
  ...
  - [{]{style="color: #990000"}[trendtestR](https://cran.r-project.org/web/packages/trendtestR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Exploratory **Trend Analysis** and Visualization for Time-Series and Grouped Data

    - Uses model-based trend analysis using generalized additive models (GAM) for count data, generalized linear models (GLM) for continuous data, and zero-inflated models (ZIP/ZINB) for count data with potential zero-inflation.
    - Also supports time-window continuity checks, cross-year handling in `compare_monthly_cases`, and ARIMA-ready preparation with stationarity diagnostics
...
## Seasonality and Trend {#sec-eda-ts-seas .unnumbered}
...
- [STL Decomposition]{.underline}
  ...
  - Decompositions that are between additive and multiplicative can be obtained using a Box-Cox transformation of the data with $0\lt \lambda \lt 1$.

    - $\lambda = 0$ gives a multiplicative decomposition while $\lambda = 1$ gives an additive decomposition.
  ...
  - [Example]{.ribbon-highlight}: [{fable}]{style="color: #990000"} ([source](https://otexts.com/fpp3/components.html#example-employment-in-the-us-retail-sector))\
    ![](_resources/EDA,_Time_Series.resources/seas-stl-ex2-1.png){.lightbox width="532"}
    ...
    - For more flexible output, two main parameters can be chosen when using STL are the trend-cycle window [trend(window = ?)]{.arg-text} and the seasonal window [season(window = ?)]{.arg-text}. (See source for details)
...
- [Level Changes in Trend]{.underline}\
  ![](_resources/EDA,_Time_Series.resources/seas-struct-break-1.png){.lightbox width="532"}
  - In green circle, you can see the abrupt change in level

  - If the cause behind this change is something that can be determined beforehand, then it can be used as a feature in the model

  - If you have a monotone series that's expected to continue the new trend, then adding an indicator that will be true for all future data may be useful. ([source](https://ramikrispin.github.io/r-ladies-rome-workshop/05_forecasting_with_lm.html#structural-break-feature))

    ``` r
    ts$level_change <- ifelse(ts$date >= as.Date("2018-09-01"), 1, 0)
    ```
...
- [Outliers]{.underline}
  ...
  - A indicator variable can also be used to account for an outlier in the data. Rather than omit the outlier, an indicator variable removes its effect. (FPP 3, [7.4](https://otexts.com/fpp3/useful-predictors.html#dummy-variables))

  - If deemed a mistake, outliers can be recoded as NA. Then an imputation/interpolation method can be applied.
  ...
  - [Example]{.ribbon-highlight}: Using quantiles of STL remainder ([source](https://otexts.com/fpp3/missing-outliers.html#outliers))\
    ![](_resources/EDA,_Time_Series.resources/stat-out-ex2-1.png){.lightbox width="532"}
...
- [Lag Scatter Plots]{.underline}
  - Packages
    ...
    - [{]{style="color: #990000"}[astsa::lag1&2.plot](https://github.com/nickpoison/astsa/tree/master/astsa_build){style="color: #990000"}[}]{style="color: #990000"}
    - [{]{style="color: #990000"}[feasts::gg_lag](https://feasts.tidyverts.org/reference/gg_lag.html){style="color: #990000"}[}]{style="color: #990000"}
