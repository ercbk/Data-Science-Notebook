...
- [{]{style="color: #990000"}[bigMICE](https://cran.r-project.org/web/packages/bigMICE/index.html){style="color: #990000"}[}]{style="color: #990000"} - For handling missing values in **large datasets** with the Multiple Imputation by Chained Equations (MICE) by using **Apache Spark**
- [{]{style="color: #990000"}[booami](https://cran.r-project.org/web/packages/booami/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2507.21807)) - Component-Wise **Gradient Boosting after Multiple Imputation**
...
- [{]{style="color: #990000"}[deepImp](https://cran.r-project.org/web/packages/deepImp/index.html){style="color: #990000"}[}]{style="color: #990000"} - Imputation of **mixed-type and compositional data with neural networks**. The architecture (number and size of hidden layers, dropout, activation, optimiser) is user-configurable.
...
- [{]{style="color: #990000"}[HCPclust](https://cran.r-project.org/web/packages/HCPclust/index.html){style="color: #990000"}[}]{style="color: #990000"} - Hierarchical Conformal Prediction for Clustered Data with Missing Responses
  - This package has a lot going on which makes it interesting, but the documentation doesn't have a vignette or non-toy examples.
  - It seems to be that the main purpose is **conformal prediction for mixed effects type data with missing values (MAR) in the response**.
- [{]{style="color: #990000"}[LRMiss](https://cran.r-project.org/web/packages/LRMiss/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides methods for **linear regression** in the presence of missing data, including **missingness in covariates and responses**.
...
- [{]{style="color: #990000"}[miapack](https://cran.r-project.org/web/packages/miapack/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements methods to **estimate conditional outcome means** in settings with missingness-not-at-random (**MNAR**) and incomplete auxiliary variables.
- [{]{style="color: #990000"}[mimar](https://cran.r-project.org/web/packages/mimar/index.html){style="color: #990000"}[}]{style="color: #990000"} - implements a **compact chained-imputation workflow** in R for missing-data analysis, artificial amputation, native and learner-backed single and multiple imputation, diagnostic evaluation, and post-imputation pooling
  - Provides a compact workflow: describe the missingness, create benchmark amputations when needed, impute with native or learner-backed update rules, inspect diagnostics, evaluate recovered cells when truth is available, and pool post-fit quantities.
  - The chained-equation formulation, sometimes called fully conditional specification, makes multiple imputation tractable for mixed-type data by modeling each incomplete variable conditionally on all others, cycling through variables until the chained summaries appear stable
- [{]{style="color: #990000"}[missPLS](https://cran.r-project.org/web/packages/missPLS/index.html){style="color: #990000"}[}]{style="color: #990000"} - Methods and Reproducible Workflows for **Partial Least Squares** with Missing Data
- [{]{style="color: #990000"}[missRanger](https://cran.r-project.org/web/packages/missRanger/index.html){style="color: #990000"}[}]{style="color: #990000"} - Uses [{ranger}]{style="color: #990000"} for imputing mixed-type data sets by chaining **random forests**
  - Option of using **predictive mean matching**
    - Avoids imputation with values not already present in the original data (like a value 0.3334 in 0-1 coded variable)
    - Tries to raise the variance in the resulting conditional distributions to a realistic level. This would allow, e.g., to do multiple imputation when repeating the call to missRanger(). Out-of-sample application is supported as wel
...
- [{]{style="color: #990000"}[rMIDAS2](https://cran.r-project.org/web/packages/rMIDAS2/index.html){style="color: #990000"}[}]{style="color: #990000"} - Multiple Imputation with 'MIDAS2' Denoising Autoencoders
  - Generates multiply-imputed datasets, computes imputation means, and runs Rubin's rules regression analysis.
...
- [{]{style="color: #990000"}[slideimp](https://cran.r-project.org/web/packages/slideimp/index.html){style="color: #990000"}[}]{style="color: #990000"} - Numeric Matrices **K-NN** and **PCA** Imputation for **high-dimensional numeric matrices**
  - A sliding window approach for **ordinal** features
  - Include group-wise imputation, hyperparameter tuning with repeated cross-validation, multi-core parallelization, and optional subset imputation
...
- [{]{style="color: #990000"}[survlab](https://cran.r-project.org/web/packages/survlab/index.html){style="color: #990000"}[}]{style="color: #990000"} - **Survival** Model-Based Imputation for **Laboratory Non-Detect** Data
...
- [{]{style="color: #990000"}[vismi](https://cran.r-project.org/web/packages/vismi/index.html){style="color: #990000"}[}]{style="color: #990000"} - A comprehensive suite of static and interactive **visual diagnostics** for assessing the quality of multiply-imputed data obtained from packages such as 'mixgb' and 'mice'
:::
...
- Resources
  ...
  - [Best Practices in Handling Missing Data in Psychological Research](https://journals.sagepub.com/doi/full/10.1177/25152459261444273?af=R) (2026)
- Papers
  ...
  - [Inference with Imputed Data: The Allure of Making Stuff Up](https://arxiv.org/abs/2205.07388) by Manski ([Thread](https://bsky.app/profile/stephenjwild.bsky.social/post/3ma43f34qss2q))
    - Argues that Random Multiple Imputation (RMI) creates an illusion of certainty by producing point estimates that rely on strong, typically untestable assumptions (MAR) about the distribution of missing data. When those assumptions lack credibility—which is common—RMI leads to inference with “incredible certitude.”
    - Reporting point estimates without acknowledging weak identification is misleading
    - Frequentist validity (randomization) requires the imputation model to be objectively correct, which is rarely verifiable.
    - When covariates-only are missing:
      - Imputation generally fails to recover the correct conditional expectation, even asymptotically. The probability limit of imputation estimators typically differs from the target parameter
      - In extreme cases (e.g., ecological inference problems), imputing covariates is shown to be completely uninformative.
    - Alternatives to RMI
      - Assumption-free interval estimation when little is known.
      - Interval tightening using weak but credible assumptions (e.g., monotonicity).
      - Decision-theoretic point estimates (e.g., midpoint estimators) when decisions require a single number.
      - Direct modeling of joint distributions without fabricating data when assumptions are defensible.
- `na.*` functions (+ `complete.cases`)
  - `na.omit`: Removes any observation containing a NA. Rows with missing values are dropped. Residuals and fitted values are returned only for the rows actually used in the fit.
  - `na.exclude`: Removes incomplete rows when fitting the model. Returns a residuals a vector with the original length, inserting NAs where rows were excluded
  - `na.fail`: Throws an error if any missing values are present
  - `na.pass`: Leaves the data unchanged
  - `complete.cases`: Returns a logical vector indicating rows without any missing values.
    - e.g. `df[complete.cases(df), ]`
- Recommendations ([article](https://towardsdatascience.com/what-is-a-good-imputation-for-missing-values-e9256d45851b))
  - N \> 200 $\rightarrow$ Nonparametric
    ...
    - [{]{style="color: #990000"}[mice-drf](https://github.com/JeffNaef/MARimputation/tree/c1f5a1e48e8a60db95c727876086db5b7305f614/Useable){style="color: #990000"}[}]{style="color: #990000"} (distributional random forest)
      ...
      - [{missForest}]{style="color: #990000"} doesn't doesn't do this. It just uses the predictions as the imputed values which essentially a conditional mean method and not a distributional method. (See [Choosing a Method](missingness.qmd#sec-missing-caim){style="color: green"} regarding conditional mean methods)
    - Also see [{missRanger}]{style="color: #990000"} (includes PMM) and other ML packages above
...
- Transformations and Interactions
  - Notes from Introduction to Regression Methods for Public Health Using R, Ch. [9.6.3](https://www.bookdown.org/rwnahhas/RMPH/mi-mlr.html#mi-polynomial), [9.6.4](https://www.bookdown.org/rwnahhas/RMPH/mi-mlr.html#mi-interaction)
    - Includes coded examples
  - If you are going to include non-linear functions of variables (e.g., Box-Cox, log, quadratic) and interactions in the regression model, your imputation model should include those non-linearities.
  - This ensures that your imputation model includes at least the same level of complexity as your regression model.

## Types {#sec-missing-type .unnumbered}
...
- Examples ([source](https://www.r-bloggers.com/2025/08/handling-missing-data-in-r-a-comprehensive-guide/))
  ...
  - MAR — e.g., BMI missingness varies by observed Age or Gender, or people with lower income are less likely to report their weight, but we observe income (i.e. weight missingness varies by income).
...
- Packages
  - [{]{style="color: #990000"}[citestR](https://cran.r-project.org/web/packages/citestR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Tests whether missingness in explanatory variables is conditionally independent of the outcome, given observed data.
    - Uses multiply-imputed datasets and cross-validated classifiers to produce a test statistic and p-value, with a sensitivity parameter (kappa) for calibrating interpretation.
...
- Misc
  - Resources
    ...
    - [Ch. 6.3 Bayesian Imputation](https://theissbendixen.com/dag-book/chapter-6.html#bayesian-imputation), A Data Analyst's Guide to Cause and Effect
...
- [{]{style="color: #990000"}[forecast::na_interp](https://pkg.robjhyndman.com/forecast/reference/na.interp.html){style="color: #990000"}[}]{style="color: #990000"}
  - By default, uses linear interpolation for non-seasonal series.
  - For seasonal series, a robust STL decomposition is first computed. Then a linear interpolation is applied to the seasonally adjusted data, and the seasonal component is added back.
  - For many [{forecast}]{style="color: #990000"} models, this is done automatically if your data has missing values
- [{]{style="color: #990000"}[fable::interpolate](https://fable.tidyverts.org/reference/interpolate.ARIMA.html){style="color: #990000"}[}]{style="color: #990000"}
  - Seems to have methods for ARIMA, ETS, and Linear models
  - The ARIMA method uses kalman smoothing
  - I'm not sure exactly what's happening with the other models (doc is garbage), but it doesn't look too complex. (From [{forecast}]{style="color: #990000"} [9.0 release](https://robjhyndman.com/hyndsight/forecast9.html#new-forecasting-models), "it computes the likelihood and fitted values correctly using all available data")
