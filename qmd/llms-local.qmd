# Local {#sec-llm-local .unnumbered}
...
- Resources
  - [How to run a local coding agent with Gemma 4 and Pi](https://patloeber.com/gemma-4-pi-agent/)
    - Step-by-step tutorial for setting up LM Studio and Pi
  - [Running local models is good now (Boykis)](https://vickiboykis.com/2026/06/15/running-local-models-is-good-now/)
    - Runs LM Studio, Pi, and Gemma-4-12b-qat in a docker container with no access to run code or search the internet
    - Gives Pi config, Docker Compose file, and bash script
  - [Use Models from the Hugging Face Hub in LM Studio](https://huggingface.co/blog/yagilb/lms-hf)
  - Cost
    - [How Much Does It Actually Cost to Run a Local LLM? (Euros per Million Tokens, Measured)](https://towardsdatascience.com/how-much-does-it-actually-cost-to-run-a-local-llm-e-per-million-tokens-measured/)
      - Measured GPU electricity for eight local models on one RTX 3090
    - [How Much Does a Local LLM Actually Cost to Run? I Measured Every Watt on Apple Silicon](https://towardsdatascience.com/how-much-does-a-local-llm-actually-cost-to-run-i-measured-every-watt-on-apple-silicon/)
      - Five models, sustained generation, real wall-socket energy at \$0.31/kWh
- Components
  - Local model inference engine - Runs/hosts LLM
    - [Jan](https://www.jan.ai/)

    - [llama.cpp](https://llama.app/)

    - [LLM](https://llm.datasette.io/en/stable/) (Simon Willison)
  ...
  - Agentic harness - Provides the environment, context, and tooling for agents to operate
    - [Pi](https://pi.dev/)
    - [rowboat](https://github.com/rowboatlabs/rowboat) ([website](https://www.rowboatlabs.com/), [hackernews](https://news.ycombinator.com/item?id=48819808))
...
- Trusted Benchmarks
  ...
  - LiveCodeBench
- Models
  - Qwen3.8-27B ([source](https://venturebeat.com/technology/qwen3-8-27b-runs-frontier-class-coding-agents-and-reasoning-locally-no-cloud-api-required))
    - Includes native image and video understanding, a 262,144-token context window, configurable reasoning and support for coding and agentic workflows
    - Requirements
      - 16-bit precision requires roughly 56GB of GPU memory
      - FP8 version needs about 28GB of GPU memory
      - 4-bit quantization cuts the model itself to roughly 17GB of GPU memory
        - i.e. M5 Max MacBook Pro with Nvidia DGX Spark (Willison)
    - Benchmarks
      - 61.7 on SWE-bench Pro (Beats Claude Opus 4.6 Max)
      - 90.3 on LiveCodeBench v6 (Beats Claude Opus 4.6 Max)
      - 70.7 on its CoWorkBench office-work benchmark
      - 84.3 on OSWorld-Verified
      - 52 on Artificial Analysis' Intelligence Index, a composite of nine evaluations spanning coding, science, reasoning and professional tasks (Equal to GPT-5.6 Luna Max)
      - 51 Artificial Analysis' Agentic Index measuring agentic task performance (Beats Claude Opus 4.8)
    - Performance comes from thinking
