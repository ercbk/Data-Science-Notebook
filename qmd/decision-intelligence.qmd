...
-   Packages
    -   [{]{style="color: #990000"}[andorR](https://cran.r-project.org/web/packages/andorR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Optimization of the Analysis of AND-OR Decision Trees
    -   [{]{style="color: #990000"}[DecisionDrift](https://cran.r-project.org/web/packages/DecisionDrift/index.html){style="color: #990000"}[}]{style="color: #990000"} - Detecting, Decomposing, and Stress-Testing Temporal Change in Repeated Decision Systems
        -   Complements the [{decisionpaths}]{style="color: #990000"} by shifting focus from path construction to system-level change over time.
    -   [{]{style="color: #990000"}[decisionpaths](https://cran.r-project.org/web/packages/decisionpaths/index.html){style="color: #990000"}[}]{style="color: #990000"} - Tools for constructing and auditing longitudinal decision paths from panel data.
        -   Implements a decision infrastructure framework for representing institutional AI systems as generators of time-ordered binary decision sequences.
        -   Provides functions to build path objects from panel data, summarise per-unit descriptors (dosage, switching rate, onset, duration, longest run), compute the Decision Reliability Index (DRI)
    -   [{]{style="color: #990000"}[DUToolkit](https://cran.r-project.org/web/packages/DUToolkit/index.html){style="color: #990000"}[}]{style="color: #990000"} - Visualizing and Quantifying Decision Uncertainty. A suite of tools to help modelers and decision-makers effectively interpret and communicate decision risk when evaluating multiple policy options
    -   [{]{style="color: #990000"}[fastpolicytree](https://cran.r-project.org/web/packages/fastpolicytree/index.html){style="color: #990000"}[}]{style="color: #990000"} - Constructs optimal policy trees which provide a rule-based treatment prescription policy. Input is covariate and reward data, where, typically, the rewards will be doubly robust reward estimates.
    -   [ISMtools](https://cran.r-project.org/web/packages/ISMtools/index.html) - Interpretive Structural Modelling Analysis Tools
    -   [{]{style="color: #990000"}[Pinference](https://pglpm.github.io/Pinference/){style="color: #990000"}[}]{style="color: #990000"} - Computes probabilities for logical expressions. Can combine multiple expressions to calculate probabilities of complex scenarios given some initial probabilities of events.
    -   [{]{style="color: #990000"}[RMCDA](https://cran.r-project.org/web/packages/RMCDA/index.html){style="color: #990000"}[}]{style="color: #990000"} - Multi-Criteria Decision Analysis Methods
    -   [{]{style="color: #990000"}[twig](https://hjalal.github.io/twig/){style="color: #990000"}[}]{style="color: #990000"} - Provides tools for building decision and cost-effectiveness analysis models. It enables users to write these models concisely, simulate outcomes—including probabilistic analyses—efficiently using optimized vectorized processes and parallel computing, and produce results
-   Interpretive Structural Modeling (ISM) is a powerful qualitative tool used to unravel the complexity of messy, interconnected problems. Instead of looking at variables in isolation, it helps you visualize how different factors—like barriers to a project or drivers of a business—influence one another in a hierarchical structure. Relies on the collective wisdom of experts to map out relationships through several distinct steps
    -   Process
        -   Variable Identification: List all factors relevant to the problem (e.g., "Lack of funding," "Poor communication," "Technical debt").
        -   Structural Self-Interaction Matrix (SSIM): Experts determine the direction of the relationship between any two variables ($i$ and $j$). For example:
            -   Does $i$ lead to $j$?
            -   Does $j$ lead to $i$?
            -   Do they influence each other?
        -   Reachability Matrix: The SSIM is converted into a binary matrix (1s and 0s). This matrix is then checked for transitivity—the logical rule that if factor A leads to B, and B leads to C, then A must lead to C.
        -   Level Partitioning: The factors are grouped into different levels. Factors that don't influence anything else go to the top (the "symptoms"), while factors that drive everything else sink to the bottom (the "root causes").
        -   Building the Diagraph: A visual map is drawn based on these levels, showing the flow of influence from the bottom up.
...
-   [**Stochastic Dominance**]{style="color: #009499"} - A framework for comparing probability distributions without making specific assumptions about their shape.. It is often used in decision-making under uncertainty, economics, and finance. There are multiple orders of stochastic dominance, each based on different assumptions about the preferences or utility functions of decision-makers. The dominating distribution in all 4 orders is superior in terms of local mean (more of whatever), variance (less risk), skewness (more extreme positive outcomes), and kurtosis (fewer extreme negative outcomes)
    -   [{]{style="color: #990000"}[generalCorr::stochdom2](https://cran.r-project.org/web/packages/generalCorr/index.html){style="color: #990000"}[}]{style="color: #990000"} - Calculates stochastic dominance for 4 orders
    -   [First Order]{.underline}\
        $$
        \begin{align}
        &F_A(x) \leq F_B(x), \quad \forall x, \\
        &\text{with strict inequality for at least one} \; x
        \end{align}
        $$
        -   Focuses on simple probability comparisons
        ...
        -   Also see [Post-Hoc Analysis, General \>\> EDA \>\> Charts](post-hoc-analysis-general.qmd#sec-phoc-gen-eda-charts){style="color: green"}
    -   [Second Order]{.underline}\
        $$
        \begin{align}
        &\int_{-\infty}^x F_A(t) \, dt \leq \int_{-\infty}^x F_B(t) \, dt, \quad \forall x, \\
        &\text{with strict inequality for at least one} \; x
        \end{align}
        $$
        -   Considers both probability and overall risk
    ...
    -   [Third Order]{.underline}\
        $$
        \begin{align}
        &\int_{-\infty}^x \int_{-\infty}^t [F_A(s) - F_B(s)] \, ds \, dt \leq 0, \quad \forall x, \\
        &\text{with strict inequality for at least one} \; x
        \end{align}
        $$
        -   Accounts for preferences related to skewness or "peakedness" of distributions.
    ...
    -   [Fourth Order]{.underline}\
        $$
        \begin{align}
        &\int_{-\infty}^x \int_{-\infty}^t \int_{-\infty}^u [F_A(s) - F_B(s)] \, ds \, du \, dt \leq 0, \quad \forall x, \\
        &\text{with strict inequality for at least one} \; x
        \end{align}
        $$
        ...
        -   [Example]{.ribbon-highlight}: Pension Fund Manager
...
-   Misc
    -   Also see [Experiments, Design \>\> Conjoint Analysis](experiments-designs.qmd#sec-exp-des-conjoint){style="color: green"}
    -   Notes from
    ...
    -   Packages
        -   [{]{style="color: #990000"}[cjoint](https://cran.r-project.org/web/packages/cjoint/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implementation of the Average Marginal Component-specific Effects (AMCE) estimator (See papers listed for details)
        -   [{]{style="color: #990000"}[projoint](https://yhoriuchi.github.io/projoint/){style="color: #990000"}[}]{style="color: #990000"} - Provides tools for analyzing data generated from conjoint survey experiments
            -   Implements estimation of marginal means (MMs) and average marginal component effects (AMCEs), with corrections for measurement error.
            -   Methods include profile-level and choice-level estimators, bias correction using intra-respondent reliability (IRR), and visualization utilities
-   Assumptions
...
-   [Misc]{.underline}
    ...
    -   Packages
        -   [{]{style="color: #990000"}[bayesDCA](https://giulianonetto.github.io/bayesdca/){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://onlinelibrary.wiley.com/doi/full/10.1002/sim.10277)) - Bayesian Decision Curve Analysis for clinical prediction models and diagnostic tests.
        -   [{]{style="color: #990000"}[BCEA](https://n8thangreen.github.io/BCEA/){style="color: #990000"}[}]{style="color: #990000"} - Produces an economic evaluation of a sample of suitable variables of cost and effectiveness / utility for two or more interventions, e.g. from a Bayesian model in the form of MCMC simulations
            -   Cost-effectiveness analysis plots, such as CE planes and CEAC
            -   Summary statistics and tables
            -   EVPPI calculations and plots
    -   Resources
        -   [Cost-Effectiveness Analysis](https://rworks.dev/posts/ESDM-2/) - One of a series of articles that implement JAGS models from *Evidence Synthesis for Decision Making in Healthcare* by Welton et al. (2012)
    -   A *weighted* Brier Score can meaningfully measure the clinical utility ([paper](https://arxiv.org/abs/2408.01626))
