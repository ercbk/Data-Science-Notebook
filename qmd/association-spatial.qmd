...
- Other flavors
  - `localmoran.sad`
    - Also see [Diagnostics, Geospatial \>\> Spatial Autocorrelation](diagnostics-geospatial.qmd#sec-diag-geo-sauto){style="color: green"} \>\> Moran’s I for more details
    - The chief advantage of the saddlepoint approximation is that it takes a fitted linear model rather than simply a numerical variable, so the residuals are analysed
