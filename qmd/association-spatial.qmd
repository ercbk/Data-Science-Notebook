...
- Packages
  ...
  - [{]{style="color: #990000"}[sddr](https://cran.r-project.org/web/packages/sddr/index.html){style="color: #990000"}[}]{style="color: #990000"} - Spatial Distribution Dynamics
    - Provides discrete-time, spatial, rank and local indicator of spatial association ('LISA') Markov transition estimation, ergodic analysis (steady-state, mean first passage and sojourn times), rank-mobility measures (Kendall's tau and the Theta statistic) and Markov mobility indices.
  - [{]{style="color: #990000"}[spdep](https://r-spatial.github.io/spdep/){style="color: #990000"}[}]{style="color: #990000"} - **OG**
- Resources
...
- [Example 2]{.ribbon-highlight}: Moran's I
  ...
  nb_poly_pol <- pol_pres15 |> poly2nb(queen = TRUE)
  lw_b_poly_pol <- nb_poly_pol |> nb2listw(style = "B")
  lw_w_pol <- nb_poly_pol |> nb2listw(style = "W")
  ```
  ...
  ![](_resources/Association-Spatial.resources/global-moran-plot-ex2-infl-map.png){.lightbox width="532"}
...
- $G_i^*$ refers to the statistic allowing a location to count itself as its neighbor.
- High-Low, Low-High
  - Interesting Contexts:
    - A wealthy neighborhood surrounded by poor ones
    - A disease hotspot surrounded by healthy areas
  - Other Causes
    - Measurement or boundary issues — an administrative unit that straddles two different underlying processes
    - The modifiable areal unit problem — an artifact of how polygons were drawn rather than a real phenomenon
    - Noise — a polygon that just happens to be different from its neighbors by chance
