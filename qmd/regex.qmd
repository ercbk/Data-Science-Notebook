...
- [{]{style="color: #990000"}[stringr::fixed](https://stringr.tidyverse.org/reference/modifiers.html){style="color: #990000"}[}]{style="color: #990000"}

  - Compare literal bytes

  - [Example]{.ribbon-highlight}: ([source](https://bsky.app/profile/cghlewis.bsky.social/post/3mukrswp7o22n))
    ...
    #> 2 Rosie 2               
    #> 3 Frank 1, 3            
    #> 4 Rudy  sausage (ground)
    #> 5 Baloo 5    

    # Using \\ to escape () in sausage (ground)
    codes_escape <- c("pepperoni" = "1", "cheese" = "2", "onion" = "3", "sausage \\(ground\\)" = "4", "pineapple, with ham" = "5")

    df |> 
      mutate(pizza_toppings = str_replace_all(pizza_toppings, codes_escape))


    # Using fixed()
    codes <- c("pepperoni" = "1", "cheese" = "2", "onion" = "3", "sausage (ground)" = "4", "pineapple, with ham" = "5")

    df |> 
      mutate(pizza_toppings = str_replace_all(pizza_toppings, fixed(codes)))
    #> # A tibble: 5 × 2
    #> name  pizza_toppings
    #> <chr> <chr>         
    #> 1 Bear  1, 2          
