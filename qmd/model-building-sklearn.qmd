...
- Extensions
  ...
  - [{]{style="color: goldenrod"}[DeepChecks](https://github.com/deepchecks/deepchecks){style="color: goldenrod"}[}]{style="color: goldenrod"} - For model diagnostics
  ...
  - [{]{style="color: goldenrod"}[sklearn-migrator](https://github.com/anvaldes/sklearn-migrator){style="color: goldenrod"}[}]{style="color: goldenrod"} ([JOSS](https://joss.theoj.org/papers/10.21105/joss.10374)) - Cross-version migration of scikit-learn models for reproducible MLOps
    - Safely migrating scikit-learn models across versions while preserving inference behavior and remaining robust to internal attribute changes
...
- Class Imbalance
  ...
  - SMOTE with [{{imblearn}}]{style="color: goldenrod"}
    ...
    - Don't apply SMOTE without recalibration (See [Classification, Class Imbalance](classification.qmd#sec-class-imbal){style="color: green"} \>\> Misc)

- Splines

  - [Example]{.ribbon-highlight}: Pipeline ([source](https://towardsdatascience.com/mastering-non-linear-data-a-guide-to-scikit-learns-splinetransformer/))
    ...
    # not including data simulation step

    # Build a pipeline: Splines + Linear Model
    ...
    - [n_knots]{.arg-text}: number of joints in the curve. The more you have, the more flexible the curve gets.
    - [degree]{.arg-text}: This defines the “smoothness” of the segments. It refers to the degree of the polynomial used between knots ([1]{.arg-text} is a line; [2]{.arg-text} is smoother; [3]{.arg-text} is the default).
    - [knots]{.arg-text}: This one tells the model *where* to place the joints. For example, uniform separates the curve into equal spaces, while quantile allocates more knots where the data is denser.
      - Use ['quantile']{.arg-text} if your data is clustered.
    - [extrapolation]{.arg-text}: Tells the model what it should do when it encounters data *outside* the range it saw during training.
      - Use ['periodic']{.arg-text} for cyclic data, such as calendar or clock.
    - [include_bias]{.arg-text}: Whether to include a “bias” column (a column of all ones). If you are using a [LinearRegression]{.arg-text} or [Ridge]{.arg-text} model later in your pipeline, those models usually have their own [fit_intercept=True]{.arg-text}, so you can often set this to [False]{.arg-text} to avoid redundancy.
