...
-   [Example]{.ribbon-highlight}: Random (Varying) Slopes and Intercepts
    ...
    -   [usl]{.var-text} is UK sociological survey data
        ...
        -   `(1 + wave0 | pidp)`
            -   Says let both the intercept ("1") and [wave0]{.var-text} vary by [pidp]{.var-text}
                -   Which means that average [log income]{.var-text} varies by person and the rate of change (slope) in [log income]{.var-text} over time ([wave0]{.var-text} fixed effect) varies by person.
...
-   [Example]{.ribbon-highlight}: [{ggeffects}]{style="color: #990000"} Error Bar Plot\
    ![](./_resources/Econometrics,_Mixed_Effects,_Frequentist.resources/staticunnamed-chunk-6-1.png){width="432"}
...
-   [Interpretation]{.underline}
    ...
    -   See 2-stage model for definitions for $a_i$ and $b_i$
        ...
        -   Orchestral Instrumentalists ([orch~i~]{.var-text} [= 1]{.var-text})
            ...
            $$
            \begin{align}
            ai = (\alpha_0 + \alpha_1) + u_i \\
            bi = (\beta_0 + \beta_1) + v_i
            \end{align}
            $$
    -   Mean Performance Anxiety (outcome)
        -   Keyboardists or Vocalists ([orch = 0]{.var-text}) playing solos or small ensembles ([large = 0]{.var-text}): $\alpha_0$
        -   Keyboardists or vocalists ([orch = 0]{.var-text}) playing large ensembles ([large = 1]{.var-text}): $\alpha_0 + \beta_0$
        -   Orchestral instrumentalists ([orch = 1]{.var-text}) playing solos or small ensembles ([large = 0]{.var-text}): $\alpha_0 + \alpha_1\alpha_0 + \alpha_1$
        -   Orchestral instrumentalists ([orch = 1]{.var-text}) playing large ensembles ([large = 1]{.var-text}): $\alpha_0 + \alpha_1 + \beta_0 + \beta_1$
    ...
    -   Interpretations
        -   Fixed effects:
            ...
            -   $\hat \beta_1 = −1.4$ --- Orchestral instrumentalists ([orch = 1]{.var-text}) have an estimated mean decrease in [performance anxiety]{.var-text} (outcome) of 2.3 points when playing in large ensembles ([large = 1]{.var-text}) instead of solos or small ensembles ([large = 0]{.var-text}), 1.4 points greater than the mean decrease among keyboard players and vocalists ([orch = 0]{.var-text}).
                -   He's calculating simple slope/marginal effect (See [Regression, Interactions](regression-interactions.html#sec-reg-inter){style="color: green"}) and choosing to use that as the interpretation for the interaction term
...
-   **Pseudo-R^2^** (Not always a reliable performance measure)\
    ![](./_resources/Econometrics,_Mixed_Effects,_Frequentist.resources/Screenshot%20(941).png)
    ...
    -   Values of \^σ2u and \^σ2v from Model B cannot be compared to between-person variability from Model A using pseudo-R^2^, since the inclusion of performance type has changed the interpretation of these values
    -   Issues
...
-   **Random Slopes and Intercepts Model (with 2 covariates)**
    ...
    -   Pseudo-R^2^![](./_resources/Econometrics,_Mixed_Effects,_Frequentist.resources/Screenshot%20(945).png)
