...
-   Pseudo-R^2^
    ...
    \text{Psuedo R}^2_{L1_v} &= \frac{\hat \sigma^2_v (\text{Model A}) - \hat\sigma^2_v(\text{Model B})}{\hat \sigma^2_v(\text{Model A})} = \frac{0.74 - 0.45}{0.74} = 0.392
    \end{align}
    $$
...
-   [LR-Test for comparing nested models]{.underline}
    ...
    -   [model.c]{.arg-text} is the less complex model, Random Slopes and Intercepts Model (with 2 covariates)
    -   Process
