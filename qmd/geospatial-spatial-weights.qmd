...
-   [Connectedness]{.underline}
    ...
    -   [Example 1]{.ribbon-highlight}: Orders and Tracts Visually
    ...
    -   [Example 2]{.ribbon-highlight}: Analyze via Heatmap and Table
        ...
        2.  I might've been able to use `purrr::pluck` here, but I went ahead and used the hacky way to extract "results" from the list output of function that quietly creates the tibble (base pipe doesn't allow you to (cleanly) subset ¯\\\_(ツ)\_/¯). `quietly` just doesn't stop the messages, it collects errors, too. So, I needed to subset the list element with the tibble in it.
        3.  This whole chunk shapes the data into a long format of the connectedness matrix in the [{spdep}]{style="color: #990000"} vignette. Long format is needed for the heatmap function.
        4.  Bivand includes in his node tallies, the number of nodes with 0 neighbors. I need to remove these values for the heatmap to make sense.
        5.  I need each order category to hava full range of neighbor values. Otherwise, there'd be blank spots (NAs) in the heatmap. 0 values are used to fill in the blank spots. This function is similar to `expand`.

        ## Heatmap

        ![](_resources/Geospatial-Spatial-Weights.resources/gwgt-diag-conn-heat-1.png){width="582"}
        ...
        -   Each row of the heatmap shows the distribution of nodes (census tracts) among the potential neighbor totals for each lag (i.e. order of neighbors)
            -   [Example]{style="color: green"}: For the first lag (first row), the brightest yellow cell says there around 16 census tracts that have 6 First Order neighbors (other census tracts) linked to them.
        ...
        ![](_resources/Geospatial-Analysis.resources/gwgt-diag-conn-tbl-1.png){.lightbox width="482"}
        ...
        -   The table shows the actual number of nodes (census tracts) for each Order (Y-axis) for each potential neighbors total (X-axis).
