...
- Also see
  - [Regression, Spatial](regression-spatial.qmd#sec-reg-spat){style="color: green"}
  - [Diagnostics, Geospatial](diagnostics-geospatial.qmd#sec-diag-geo){style="color: green"}
- Packages
  - [{]{style="color: #990000"}[ArchipelagoEngine](https://cran.r-project.org/web/packages/ArchipelagoEngine/index.html){style="color: #990000"}[}]{style="color: #990000"} - Spatial Weight Construction for Archipelagic Geographies
  ...
  - [{]{style="color: #990000"}[sfdep](https://sfdep.josiahparry.com/){style="color: #990000"}[}]{style="color: #990000"} - A [{sf}]{style="color: #990000"} and [{tidyverse}]{style="color: #990000"} friendly interface to [{spdep}]{style="color: #990000"} along with some additional functionality
  - [{]{style="color: #990000"}[sfisland](https://horankev.github.io/sfislands/){style="color: #990000"}[}]{style="color: #990000"} - Similar to [{sfdep}]{style="color: #990000"} with its [{tidyverse}]{style="color: #990000"} and [{sf}]{style="color: #990000"} friendly interface. It makes it easier to deal with geographic datasets which contain islands
...
- Workflows
  ...
  - Distance-based
    - k-NN: Create a points list object (e.g. `st_centroid`) $\rightarrow$ If necessary, convert to geographical coordinates (via `st_transform`) $\rightarrow$ Fit neighbors algorithm (e.g. `knearneigh`) $\rightarrow$ Convert to neighbors list (e.g. `knn2nb`) $\rightarrow$ Add spatial weights (e.g. `nb2listw`)
    ...
    - Distance Band Method: Go through k-NN process to create a k-NN neighbors list (via `knn2nb`) $\rightarrow$ Find minimal distance needed for a graph w/no no-neighbor observations (via `nbdists`) $\rightarrow$ Check if graph is fully connected by printing neighbor list object or using `n.comp.nb`. If disjoint subgraphs are reported. Add more distance to upper distance parameter ([d2]{.arg-text}) in `dnearneigh`, and repeat until graph is connected $\rightarrow$ Add spatial weights (via `nb2listw`)
...
- You can use `igraph::diameter` to answer how many steps might be needed to traverse the graph
  - [Example]{.ribbon-highlight}: Find unit from which it takes the maximum amount of steps to traverse the graph

    ``` r
    library(spdep)
    data(pol_pres15, package = "spDataLarge")

    gr_wgt_b_poly <- pol_pres15 |> 
      poly2nb(queen = TRUE) |> 
      nb2listw(style = "B") |> 
      as("CsparseMatrix") |> 
      igraph::graph_from_adjacency_matrix()

    igraph::diameter(gr_wgt_b_poly)
    #> [1] 52

    idx_max_dist_unit <- gr_wgt_b_poly |> 
      igraph::distances() |> 
      apply(2, max) |> 
      which.max()

    pol_pres15$name0[idx_max_dist_unit]
    #> [1] "Lutowiska"
    ```
...
- These are lists of length equal to the number of observations with integer vector elements.

  - Observations with *no-neighbors* are encoded as an integer vector with a single element [0L]{.arg-text}
  - Observations with neighbors as sorted integer vectors containing values in [1L:n]{.arg-text} pointing to the neighboring observations.

- `union.nb` can cumulate neighbor lists
...
- Types of Neighbor Lists
  ...
  - `poly2nb` - Takes a [sf object with (multi-)polygons]{.underline} or a list of polygon objects with class SpatialPolygon
    - Builds a neighbors list based on regions with contiguous boundaries, that is sharing one or more boundary point.
    ...
    - The SDS book explains it this way, "For each observation, the function checks whether at least one ([queen = TRUE]{.arg-text}, default), or at least two (*rook*, [queen = FALSE]{.arg-text}) points are within snap distance units of each other."
...
- [Misc]{.underline}
  ...
  - A [listw]{.arg-text} object is a list with three components: a nb object (neighbor list), a matching list of numerical weights, and a single element character vector containing the single letter name of the way in which the weights were calculated (normalization style).
  - Functions require that your sf object has the same coordinate system that you used to create your neighbors list.
  - Normalizing your weights matrix is always a good idea. Otherwise, the spatial parameters might blow up – if you can estimate the model at all. It also ensures easy interpretation of spillover effects. ([source](https://ruettenauer.github.io/Geodata_Spatial_Regression/03_weights.html#normalization-of-boldsymbolmathbfw))
...
- `nb2listwdist` - Supplements `nb2listw` with additional options
  ...
  - Types
    ...
    - [Inverse Distance Weighting (IDW)]{.underline}\
      $$w_{ij} = d_{ij}^{-\alpha}$$
      - [type = "idw"]{.arg-text}
      ...
      - There's also a way to do this with `nb2listw` by using the [glist]{.arg-text} argument (See [SDS Ch.14.5](https://r-spatial.org/book/14-Areal.html#weights-specification))
    - [Exponential Distance Decay]{.underline}\
      $$w_{ij} = e^{-\alpha \cdot d_{ij}}$$
      - [type = "exp"]{.arg-text}
    - [Double-Power Distance]{.underline}\
      $$w_{ij} = \left[1 - \left(\frac{d_{ij}}{d_{\textrm{max}}}\right)^{\alpha}\right]^{\alpha}$$
      - [type = "dpd"]{.arg-text}
      - [dmax]{.arg-text} value required; the distance value where you want the weights become zero
        - This depends on your coordinate system/CRS (i.e. some CRS are in feet), but probably meters if planar distances/projected CRS and kilometers if great circle distances/geographic CRS
      - $w_{ij} = 0$ when $d_{ij} \gt d_{\text{max}}$

  - Visualizing the Types

    ::: {layout-ncol="2"}
    ![$\alpha = 1.0$](_resources/Geospatial-Spatial-Weights.resources/spwgt-types-alpha-1.png){.lightbox group="wgt-types" width="332"}

    ![$\alpha = 1.5$](_resources/Geospatial-Spatial-Weights.resources/spwgt-types-alpha-1-5.png){.lightbox group="wgt-types" width="332"}

    ![$\alpha = 2.0$](_resources/Geospatial-Spatial-Weights.resources/spwgt-types-alpha-2.png){.lightbox group="wgt-types" width="332"}

    ![$\alpha = 2.5$](_resources/Geospatial-Spatial-Weights.resources/spwgt-types-alpha-2-5.png){.lightbox group="wgt-types" width="332"}
    :::

    - As $\alpha$ increases:

      - IDW's and Exponential Decay's slopes increase and weights (Y-axis) reach zero more quickly as distance (X-axis) increases
      - Double-Power Distance's slope flattens out more at mid-distance values and the slope increases more quickly as the distance approach $d_{\text{max}}$

    - IDW produces very large weight values for small distances and small $\alpha$

    - DPD goes to exactly 0 at $d_{\text{max}}$, while the others approach 0 asymptotically

    - At $\alpha = 0.3$, IDW and Exponential Decay are giving weight to large distance values, but at $\alpha = 0.5$, Exponential Decay weights get very close to zero at $d_{\text{max}}$

      ::: {layout-ncol="2"}
      ![$\alpha = 0.3$](_resources/Geospatial-Spatial-Weights.resources/spwgt-types-alpha-0-3.png){.lightbox group="wgt-types" width="332"}

      ![$\alpha = 0.5$](_resources/Geospatial-Spatial-Weights.resources/spwgt-types-alpha-0-5.png){.lightbox group="wgt-types" width="332"}
      :::
- [Normalization Styles]{.underline}
  - **Row-Standardized (W)**
    ...
    - Most popular as it makes results comparable, i.e. one node's set of relationships is comparable to another node's set of relationships (rows)
      ...
      - *Seems to be preferred in econometric spatial models (and probably others) as it provides intuitive interpretability of impacts.*
    ...
    - Characteristics
      ...
      - Proportions between units such as distances get lost
      - Can induce asymmetries, $w_{ij}\ne w_{ji}$

    - Issue: Has some **undesirable properties when some non-contiguity based neighbor methods** are used, such as distance based neighbours. ([source](https://ruettenauer.github.io/Geodata_Spatial_Regression/03_weights.html#row-normalization))

      - It obscures the proportion due to dividing by a row-specific value.
      - e.g. The strength of the relation for distant locations can be similar to more proximate units
  ...
  - **Binary (B)**
    ...
    - Gives a weight of unity to each neighbor relationship (i.e. ignores strength of the link), and typically up-weights units with no boundaries on the edge of the study area, having a higher count of neighbors.
  ...
  - **Globally Standardized (C)**
    ...
    - [Example]{.ribbon-highlight}: Investigating voting patterns in densely populated districts vs. sparsely populated districts in a state or region. Here you want the total influence to be standardized across the entire state or region.
  - **Unstandardized (U)**
  ...
  - **Min/Max**
    - Maximum eigenvalues normalization divides each non-zero weight by the overall maximum eigenvalue $\lambda_{\text{max}}$. Each element of $W$ (weights matrix) is divided by the same scalar parameter, which preserves the relations, $W/\lambda_{\text{max}}$.
    - Interpretation may become more complicated
    - Keeps proportions of connectivity strengths across units (relevant esp. for distance based methods) (i.e. **best for distance based methods**)

## Neighbor Algorithms {#sec-geo-swgt-nbalg .unnumbered}

### Graph-based {#sec-geo-swgt-nbalg-graph .unnumbered}
...
- [Misc]{.underline}
  ...
  - Modify a neighbor list: `subset.nb(nb_q_96, !is.na(boston_96$median))` ([source](https://r-spatial.org/book/16-SpatialRegression.html#boston-house-value-dataset))
    - Both objects have 96 observations. It removes elements of the list where [median]{.var-text} is [NA]{.arg-text} (i.e. all [FALSEs]{.arg-text})
...
- **Sphere of Influence Graph (SOI)**
  ...
  - Takes triangulated neighbors and prunes off neighbor relationships represented by edges that are unusually long for each point, especially around the convex hull
  - Guarantees symmetry
...
### Distance-based {#sec-geo-swgt-nbalg-distb .unnumbered}

- [k-NN]{.underline}
  - Creates a graph where all units have at least k neighbors (when symmetry is coerced). A small k should be used.

  - Does NOT guarantee symmetry (almost always asymmetric)

    - `knn2nb` has a [sym]{.arg-text} argument to permit the imposition of symmetry
...
- [Distance-band]{.underline}
  ...
  - Specifying thresholds may be useful (to get rid of very small non-zero weights)
    ...
    - [Example]{.ribbon-highlight}: Reduce sparseness ([source](https://ruettenauer.github.io/Geodata_Spatial_Regression/03_weights.html#distance-based-weights))
  ...
  - If you set the the distance threshold (i.e. upper bound, [d2]{.arg-text}) to be too large:
  ...
  - If you set the distance threshold to be too small:
    ...
    - This can lead to disjoint subgraphs. (See [Diagnostics](geospatial-spatial-weights.qmd#sec-geo-swgt-diag){style="color: green"} \>\> Disjointedness)

  - Regularity of Spacing

    - If the spacing between locations was "regular," then all locations would be equal distant from each other.
  ...
  - [Example 1]{.ribbon-highlight}: ([source](https://r-spatial.github.io/spdep/articles/nb_sf.html#distance-neighbours))\
    ...
    - `nbdists` returns the Euclidean distances along the links of k-NN graph
      ...
      - The max of these distances is 26,564.68 meters or 26.6 km which is the *minimum distance* needed to make sure that all the areas are linked to at least one neighbor.
      - Even though there will no no-neighbor observations (their presence is reported by the print method for nb objects), the graph may not be connected, e.g. a pair of observations are each others’ only neighbors.
        - When printing the neighbor list object, if any "disjoint connected subgraphs" are reported, then your graph is not connected
      - If graph is not connected at that distance (e.g. 26.6 km), than add to it until the graph is connected.
    - The 75^th^ percentile of the max of those distances is used as the upper threshold for the distance neighbors algorithm. As expected, using this threshold, there are disjointed subgraphs. (Also see [Diagnostics](geospatial-spatial-weights.qmd#sec-geo-swgt-diag){style="color: green"} \>\> Disjointedness)
...
- [Differences]{.underline}
  - Plot differences between neighbor algorithms
    ...
    - [Example]{.ribbon-highlight}: From Example: SOI and GG\
      ...
      # graph-based neighbors list
      NY85_nb_gg <- 
        graph2nb(gabrielneigh(coords = NY8_ct_sf),
                 sym = TRUE)

      # find neighbor diffs bt soi and gg graphs
...
- [Connectedness]{.underline}
  - **Spatial Lags of Neighbors** are the number of steps away from a node. The neighbors at a particular spatial lag are that lag's **Order** **of Neighbors** (e.g. 4th lag has 4th order neighbors)
    ...
    - `nblag` creates neighbor lists for specified lag orders, e.g. `nb_lag_ls <- nblag(nb_list, 2)`
    - `nblag_cumul` cumulates the list of neighbors for the whole list of lags, e.g. `nblag_cumul(nb_lag_ls)`
    - `union.nb` can cumulate specified neighbor lists, e.g. `union.nb(nb_lag_ls[[1]], nb_lag_ls[[2]])`
      - Note that this is the same object as created by `nblag_cumul`
  ...
  - [Example 1]{.ribbon-highlight}: Explaining Spatial Lags and Neighbor Orders Visually
    ...
    org_lgt_tle <- prismatic::clr_lighten("#F76900", shift = 0.30)
    ...
    ggplot() +
      ...
      # inactive nodes
      geom_point(aes(x = st_coordinates(sy_coords)[, 1], 
                     y = st_coordinates(sy_coords)[, 2]),
                 color = "#000E54") +
      # active nodes
      with_outer_glow(
    ...
    - Above is a plot of the 3rd order neighbors. The gif is 10.7MB which is too big for here given how massive this notebook already is, but it is posted on [bluesky](https://bsky.app/profile/ercbk.bsky.social/post/3moifcm4vok2r).
    - The animated gif goes through all the orders of neighbors in the previous section.
    - [{gganimate}]{style="color: #990000"} is magical. Two lines: one to animate and one to change the subtitle. But it wasn't completely painless.
      - To keep your font family and face settings, you need to use [device = "ragg_png"]{.arg-text} in `animate`.
      - To slow down the animation, I messed with the [fps]{.arg-text} and [duration]{.arg-text} settings
  ...
  - [Example 2]{.ribbon-highlight}: Analyze via Heatmap and Table
    ...
    - Each row of the heatmap shows the distribution of nodes (census tracts) among the potential neighbor totals for each lag (i.e. order of neighbors)
      - [Example]{style="color: green"}: For the first lag (first row), the brightest yellow cell says there are around 16 census tracts that have 6 First Order neighbors (other census tracts) linked to them.
...
- [Disjointedness]{.underline}
  ...
  - Alternative to snap: see this Bivand [example](https://github.com/rsbivand/eqc25_talk/blob/9d0601ff663154a5fdaa7f441f749981467463fa/bivand_eqc25_pres.qmd#L246) where he manually connects five areas ([Slides](https://rsbivand.github.io/eqc25_talk/) for context)

  - `n.comp.nb` gives the number of disjoint connected subgraphs

    - If `n.comp.nb(nb_list)$nc == 1`, then you have a fully connected graph

  - Argument Options

    - The [zero.policy]{.arg-text} argument uses zero as the value for neighborless nodes if [TRUE]{.arg-text}, but if [FALSE]{.arg-text} causes `nb2listw` to fail.
    - The [adjust.n]{.arg-text} argument to measures of spatial autocorrelation is by default [TRUE]{.arg-text}, and subtracts the count of neighborless nodes from N in an attempt to acknowledge the reduction in information available.
    - [snap]{.arg-text} allows the shared boundary points to be a short distance from one another. e.g. islands a short distance from the mainland that are essentially connected via ferry, bridge, etc. By using snap, `poly2nb` can make connections that should be there in all practicality. (Also see [Neighbor Lists](geospatial-spatial-weights.qmd#sec-geo-swgt-nbls){style="color: green"} \>\> `poly2nb`)

  - [Example]{.ribbon-highlight}: Using `n.comp.nb` ([source](https://r-spatial.github.io/spdep/reference/compon.html#ref-examples))
  ...
  - [Example]{.ribbon-highlight}: Using [snap]{.arg-text} ([source](https://r-spatial.github.io/spdep/articles/subgraphs.html#no-neighbour-observations))

    ::: panel-tabset
    ## Before snap

    ![](_resources/Geospatial-Spatial-Weights.resources/diag-disjoint-ex-snap-1.png){.lightbox width="232"}
    ...
    - See [Geospatial, Preprocessing \>\> sf](geospatial-processing.qmd#sec-geo-proc-sf){style="color: green"} \>\> Points in Polygons for details on `st_point_on_surface`

    ## Find the Distance to Neighbors
    ...
    - [ynys_mon]{.var-text} is the Welsh name for Anglesey
    - The two neighbors we want the algorithm to detect should be within 280 meters.

    ## After snap

    ![](_resources/Geospatial-Spatial-Weights.resources/diag-disjoint-ex-snap-2.png){.lightbox width="232"}
