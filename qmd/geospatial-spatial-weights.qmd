...
-   [Connectedness]{.underline}
    ...
    -   **Tracts** are the number of links that can be traversed at a certain order (i.e. number of steps away from a particular node)
        ...
        -   This is like The Garden of Forking Paths in Statistical Rethinking where rings are orders and paths are tracts. (See [Statistical Rethinking, Chapter 2 \>\> Counting the Ways](https://ercbk.github.io/Statistical-Rethinking-Notebook/qmd/chapter-2.html#sec-ch2-count){style="color: green"})
    ...
    -   [Example 1]{.ribbon-highlight}: Orders and Tracts Visually
        ...
        filter_node <- function(inner_list) {
        ...
        nb_node_ords_sub <- syr_nb_node[my_orders]
        walk2(
          nb_node_ords_sub,
          my_orders,
          plot_order
    ...
    -   [Example 2]{.ribbon-highlight}: Analyze via Heatmap and Table
        ...
        clean_nb <- function(nb_list, orders) {
        ...
        dat_nb <- 
          ...
          rename("num_tracts" = `...1`,
                 "tot_nodes" = "n") |> 
          mutate(num_tracts = as.integer(num_tracts),
                 order = factor(order, 
                                ordered = TRUE,
                                levels = rev(lags_txt))) |> 
          filter(num_tracts != 0) |> 
          # make sure each order category has full range of num_tracts
          tidyr::complete(order, 
                          num_tracts = 1:max(num_tracts),
                          fill = list(tot_nodes = 0)) |> 
          relocate(order, .before = "num_tracts")
        ```
        ...
        ![](_resources/Geospatial-Analysis.resources/gwgt-diag-conn-heat-1.png){.lightbox width="482"}

        ``` r
        dat_nb |> 
          tidyplot(x = num_tracts, 
                   y = order, 
                   color = tot_nodes,
                   width = 125,
                   height = 125) |> 
          ...
          adjust_legend_title("Total Nodes") |> 
          adjust_y_axis(title = "Order") |> 
          adjust_x_axis(title = "Number of Tracts",
                        position = "top") +
          theme_notebook()
        ```
        ...
        dat_nb_wide <- dat_nb |> 
          arrange(desc(order)) |> 
          tidyr::pivot_wider(names_from = num_tracts, 
