...
-   [Example]{.ribbon-highlight}: Basic ([source](https://isi-iass.org/home/wp-content/uploads/Survey_Statistician_2025_January_N91_06.pdf#page9))
    ...
    svy_nhis <- 
      as_survey_design(
        .data = dat_nhis,      # Dataset containing all variables
    ...
    -   The weights, stratification, and cluster id variable are automatically included in the NHIS dataset.
    -   The Variance Estimation guide actually gives this code for creating this object. See [Surveys, Census Data \>\> IPUMS \>\> Design Information](surveys-census-data.qmd#sec-surv-cens-ipums-di){style="color: green"}

    ## Recode Missing to NA

    ``` r
    # Get codes for BMICALC
    ddi_nhis <- ipumsr::read_ipums_ddi("../../Data/ipums/nhis_00002.xml")
    ipumsr::ipums_var_info(ddi_nhis, vars = "BMICALC")$val_labels 
    #> # A tibble: 2 × 2
    #>     val lbl           
    #>   <dbl> <chr>         
    #> 1     0 NIU           # <1>
    #> 2   996 Not calculable # <2>

    svy_adults_nhis <-   # <3>
      svy_nhis |> 
      filter(AGE >= 18) |>
      mutate(BMI = ifelse(BMICALC == 996, 
                          NA, 
                          BMICALC))
    ```
    ...
    2.  A value of 996 says that the BMI wasn't calculable for other reasons (e.g. confidentiality concerns). You can read about this in the Description on the Variable Information page or through `ipumsr::impums_var_desc`
    3.  By filtering adults we take care of the 0 values. Then the 996 values are replaced with NAs
    ...
    svy_avg_bmi_long_nhis <- 
      ...
      pivot_longer(cols = everything(), 
               ...
               names_sep = "_" ) |> 
      pivot_wider(names_from = "uncertainty", 
                  values_from = "value") |>
      rename(value = `NA`, 
             upper = upp, 
             lower = low) |> 
    ...
    #>    <dbl> <dbl> <int>    <dbl>       <dbl>
    #> 1      1     1  1953     27.9       0.137
    #> 2      1     2  2277     27.3       0.170
    #> 3      2     1  2713     28.4       0.111
    #> 4      2     2  3187     28.4       0.147
    #> 5      3     1  4584     28.3       0.101
    #> 6      3     2  5408     28.3       0.118
    #> 7      4     1  3161     27.9       0.113
    #> 8      4     2  3686     27.2       0.126
    ```
