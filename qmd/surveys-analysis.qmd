...
* Pairwise Likelihood (original, mathy [paper](https://www150.statcan.gc.ca/n1/pub/12-001-x/2013002/article/11887-eng.pdf), factor analysis w/ordinal data [paper](https://www.diva-portal.org/smash/get/diva2:458843/FULLTEXT01.pdf), usage on customer survey data ($) [paper](https://onlinelibrary.wiley.com/doi/abs/10.1002/asmb.2147))
	...
	* Example: in an education study of students, schools (first stage sampling units) may be selected with probabilities proportional to school size and students (second stage units) within selected schools by stratified random sampling.
		* see [Surveys, Sampling Methods](Surveys, Sampling Methods) >> Probabilistic Sampling Methods >> Multi-Stage Sampling
...
* **Ceiling or floor effects** occur when the tests or scales are relatively easy or difficult such that substantial proportions of individuals obtain either maximum or minimum scores and that the true extent of their abilities cannot be determined.
	* Sounds like censoring (See [Regression, Other](Regression, Other) >> Censored and Truncated Data)
...
* Types
	...
	* **(Inverse) Probability Weights** ([wiki](https://en.wikipedia.org/wiki/Inverse_probability_weighting)) - [{{balance}}]{style='color: goldenrod'} refers to this type as "inverse propensity weights"
		...
		* Used to reduce bias when respondents have different probabilities of selection
			...
			Calculate Weights
			![](./_resources/Surveys,_Analysis.resources/Screenshot (1194).png)
			* di is ...?
	...
	* Steps:
		* EDA:
			...
			* Summary Statistics
				...
				* ASMD (Absolute Standardized Mean Deviation) measures the difference between the sample and target for each covariate.
					...
					* Also see
						* [Post-Hoc Analysis, general](Post-Hoc Analysis, general) >> Bayesian >> Cohen's D, SSMD
						* [Post-Hoc Analysis, Mixed Effects](Post-Hoc Analysis, Mixed Effects) >> Cohen's D
			* Visualizations
		...
		* Calculate Weights:
			...
			* Preprocessing ("using best practices in the field"):
				...
				* Missing values  - adds a column '\_is\_na' to any variable that contains missing values
			...
			* Methods
				* **Inverse Propensity Weighting (IPW)**
					...
					* See above, Types >> (Inverse) Probability Weights
						Using LASSO logistic regression keeps the inflation of the variance as minimal as possible while still addressing the meaningful differences in the covariates between the sample and the target
						
					* Design Effect (`max_de`) for tuning penalty factor, _λ,_ and the trimming ratio parameter
						...
						* `max_de=None` - optimization is performed by cross-validation of the logistic model
							* penalty factor, _λ,_ is chosen when the MSE is at most 1 standard error from the minimal MSE
...
* Tidymodels
	* Tidymodels
		...
		* Importance weights only affect the model estimation and _supervised_ recipes steps (i.e. depend on the outcome variable).
...
* Description
	...
	* "class\_1" (80 obs) is severely imbalanced with "class\_2" (4920)
		* class\_1 observations get a weight of 60 since 4920/80 = 61.5 which is ~ 60
* `recipe` will automatically detect the weights (pretty sure it doesn't matter whether on no "case\_wts" is included in formula, e.g. class ~ .)
	* Since these are performance weights and step\_ns and step\_normalize don't depend on the outcome variable (i.e. supervised), case weights are not used in these transformations.
* Steps
	* add "case\_wts" variable to df
