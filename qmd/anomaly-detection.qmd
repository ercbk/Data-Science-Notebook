...
-   From Sci Kit Learn ([link](https://scikit-learn.org/stable/auto_examples/miscellaneous/plot_anomaly_comparison.html))\
    ![](./_resources/Anomaly_Detection.resources/image.3.png){width="532"}
...
-   ML Algorithms (Descriptions from BRDAD [paper](https://arxiv.org/abs/2312.01046))
    ...
    -   Local Outlier Factor (LOF) is a distance-based algorithm that measures the local deviation of the density of a given data point with respect to its neighbors.
    ...
    -   Isolation Forest (iForest) is a forest-based algorithm that works by randomly partitioning features of the data into smaller subsets and distinguishing between normal and anomalous points based on the number of "splits" required to isolate them, with anomalies requiring fewer splits.

    -   One-class SVM (OCSVM) is a kernel-based algorithm which tries to separate data from the origin in the transformed high-dimensional predictor space.
...
-   Use average event rates to filter out false positives and predict an upcoming event\
    ![](./_resources/Anomaly_Detection.resources/1-X1pTyyPhi0hl-CiIROWfeQ.png){width="632"}
...
-   Track variable importance over time to narrow the field of potential causes of an event\
    ![](./_resources/Anomaly_Detection.resources/1--cpQNiLGOBxSWftqLA4fhQ.png){width="632"}
    ...
    -   Use to examine false positives and actual events\
        ![](./_resources/Anomaly_Detection.resources/1-x9wbLjOpCCPq4DfEcvYLuw.png){width="632"}
...
-   Rolling Spectral Entropy\
    ![](./_resources/Anomaly_Detection.resources/image.2.png){width="632"}\
    ![](./_resources/Anomaly_Detection.resources/image.1.png){width="632"}
    ...
    -   [Example]{.ribbon-highlight}\
        ![](./_resources/Anomaly_Detection.resources/image.png){width="532"}
...
-   The goal to breakdown an anomaly (e.g. manufacturing process outage) into constituent parts (sensor readings).\
    ![](./_resources/Anomaly_Detection.resources/1-98b6-GxeBzVmrSQ6U3cvfg.png){width="632"}
...
-   Horizon Chart\
    ![](./_resources/Anomaly_Detection.resources/Screenshot%20(830).png){width="432"}
...
-   Strip Chart
    -   Similar to horizon in that y-axis values get binned and represented by colors\
        ![](./_resources/Anomaly_Detection.resources/1-3jM_tlCOIRWb3d21SSUGHw.png){width="632"}
    -   [Example]{.ribbon-highlight}\
        ![](./_resources/Anomaly_Detection.resources/1-ifvTLRyv8SCBcOhPsOS21g.png){width="632"}
        -   Colors
            -   Blue - low, Gold - medium, Red - high
        -   Columns of red indicate shocks (e.g. around 2022-11-15)
    -   [Example]{.ribbon-highlight}\
        ![](./_resources/Anomaly_Detection.resources/1-hlMumRn0MBT6GMhJy6LPpA.png){width="632"}
        -   Colors
            -   Blue - low, Gold - medium, Red - high
...
-   Misc
    -   Also see [Feature Reduction \>\> Autoencoders](feature-reduction.qmd#sec-feat-red-autoenc){style="color: green"}
