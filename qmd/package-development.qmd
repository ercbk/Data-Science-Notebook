...
- Packages
  ...
  - [{]{style="color: #990000"}[cobertura-action](https://github.com/5monkeys/cobertura-action){style="color: #990000"}[}]{style="color: #990000"} ([Thread](https://bsky.app/profile/jakub-sobolewski.bsky.social/post/3ld3ufgkbek26)) - Github Action that provides private code coverage
    - [{usethis::use_coverage}]{style="color: #990000"} uses codecov.io which evidently collects your code
  - [{]{style="color: #990000"}[consolidatePack](https://cran.r-project.org/package=consolidatePacks){style="color: #990000"}[}]{style="color: #990000"} - Consolidates the functions of an external package into your package. This may be necessary when the package is decommissioned by CRAN, and you do not want your dependent package to also be decommissioned.
    - The functions in this package recursively retrieve dependencies in the external package.
    - It also performs the other needed bookkeeping, such as retrieving .Rd files in the man subdirectory.
  - [{]{style="color: #990000"}[doclisting](https://cran.r-project.org/web/packages/doclisting/index.html){style="color: #990000"}[}]{style="color: #990000"} - Generate 'Rd' markup to list methods for a generic function. Makes it easier to document S3, S4, and S7 generics by automatically finding and linking to method documentation
  - [{]{style="color: #990000"}[goodpractices](https://docs.ropensci.org/goodpractice/){style="color: #990000"}[}]{style="color: #990000"} - Give advice about good practices when building R packages. Advice includes functions and syntax to avoid, package structure, code complexity, code formatting, etc
  - [{]{style="color: #990000"}[lang](https://cran.r-project.org/web/packages/lang/index.html){style="color: #990000"}[}]{style="color: #990000"} - Translates R help documentation on the fly by using a Large Language model of your choice.
  - [{]{style="color: #990000"}[lifecycle](https://lifecycle.r-lib.org/){style="color: #990000"}[}]{style="color: #990000"} - Managing Function Deprecation
    - Communicate changes to users gracefully with lifecycle badges and function messages.
  - [{]{style="color: #990000"}[mini-roxygen](https://github.com/eitsupi/mini-roxygen){style="color: #990000"}[}]{style="color: #990000"} - Supports a minimal subset of roxygen comments, derives everything statically, and never loads or compiles the package. Some documentation that depends on evaluation is out of reach, but Rd generation itself takes only as long as the required static processing.
    - roxygen2 cannot produce a single Rd file until its Rust code has compiled, and without a warm cache, that compile alone easily takes twenty minutes.
  ...
  - [{]{style="color: #990000"}[pkgndep](https://jokergoo.github.io/pkgndep/){style="color: #990000"}[}]{style="color: #990000"} - Analyzing Dependency Heaviness of R Packages
    - The “heaviness” for a package means, the number of additional dependency packages it brings to.
  - [{]{style="color: #990000"}[pkgdocs](https://repos.openanalytics.eu/docs/pkgdocs/){style="color: #990000"}[}]{style="color: #990000"} ([Video](https://www.youtube.com/watch?v=pqTWkMcy0lY)) - Modular approach to package documentation
    ...
    - Optimized to use with Hugo websites ([Themes](https://themes.gohugo.io/), [Deploy](https://gohugo.io/host-and-deploy/), [Components](https://gohugo.io/hugo-modules))
      - Recommended to use [docsy](https://github.com/google/docsy) them (used by ShinyProxy, RDepot). For additional filing see [Look and Feel](https://www.docsy.dev/docs/adding-content/lookandfeel/) section of the docs.
    - Example of multi-package project, [craneserver](https://craneserver.net/documentation/) (See R Client and Python Client pages)
  ...
  - [{]{style="color: #990000"}[tinyrox](https://cran.r-project.org/web/packages/tinyrox/index.html){style="color: #990000"}[}]{style="color: #990000"} - A deterministic, dependency-free documentation generator for R packages. Generates valid Rd files and NAMESPACE from 'roxygen2'-style comments using only base R.
    - Supports a strict subset of tags with no markdown parsing, no inference magic, and explicit-only behavior.
- Resources
  - [R Packages](https://r-pkgs.org/)
  - [R Documentation (.Rd) Syntax](https://cran.r-project.org/doc/manuals/R-exts.html#Writing-R-documentation-files-1) - Has all the styling tidbits like: `\eqn`, `\deqn`, `\item`, `\describe`, `\tabular`, etc.
- Tools
  - [great docs](https://posit-dev.github.io/great-docs/) - Posit solution for Python package websites
  - [revdeprun](https://crates.io/crates/revdeprun) - Easy reverse dependency checks for R with **cloud**-ready (container) environment setup (don't run locally)
    - `tools::package_dependencies("packagename", recursive=T)` probably does something similar
...
- Licenses
  - Notes from
    - [Pick a License, Not Any License](https://blog.stephenturner.us/p/pick-a-license-not-any-license) - Also has links to license resources
  - CRAN
    - Roughly 7 in every 10 CRAN package licenses are *copyleft*.
      - Affects adapting packages
      - Includes GPL, Mozilla Public License, EUPL, CeCILL, or Creative Commons license with a ShareAlike attribute
    - Only 18 (\< 0.1%) of \~23K packages have Creative Commons with a NonCommercial attribute (i.e. the vast majority of CRAN package are safe to use in commerical products)
  - [**Copyleft**]{style="color: #009499"} - Open-source licensing practices that require derivative works to use the same or compatible license terms.
    - Copyleft licenses can be “strong” or “weak”, depending on what they consider to be derivative work.
  - Anaconda
    - See also [Retrospective on Anaconda’s 2024 Licensing Changes: What They Mean and Smarter Alternatives](https://licenseware.io/retrospective-on-anacondas-2024-licensing-changes-what-they-mean-and-smarter-alternatives/)
    - In 2020, Anaconda introduced a commercial terms-of-service model for its package repository. Under these terms, organizations with more than 200 employees are generally required to purchase a license to download packages from the Anaconda-maintained Defaults channel.
    - In July 2025, Anaconda updated their license again. The current terms now indicate that certain non-profit organizations and academic institutions may be exempt from the commercial license requirements
  - Business lawyers *don't* like GPL licenses ([source](https://josiah.rs/posts/gpl-cost/))
    - They *do* like licenses like BSD-3, Apache-2, MIT, etc
...
- Misc
  ...
  - Not including a function in the Reference page but exporting it
    - e.g. a print method for a function class

    - Add this line somewhere in the functions roxygen2 docs:

      ``` r
      #' @keywords internal
      ```

    - See [{]{style="color: #990000"}[ebtools::print.skim_arrow](https://github.com/ercbk/ebtools/blob/df7999f331850db71cec33fa0913fb3bd580fa9b/R/skim-arrow.R#L179){style="color: #990000"}[}]{style="color: #990000"}
...
- Misc
  ...
  - Replace `@importFrom` with explicit `package::function()` calls
