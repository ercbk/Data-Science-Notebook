...
-   [Example 3]{.ribbon-highlight}: Universal Kriging
    ...
    -   A grid of square cells will overlap the German border. So to get an accureate area, all cell areas must be within the border. Therefore, we need to chop of the parts of border cells that cross the border.
    -   `st_intersect` is a binary predictate where a 1 indicates a grid cell that crosses the German bordern and 0 indicates the cell does *not* cross the border
    ...
    # sanity check
    sum(grid_de$pop_dens * grid_de$area, na.rm = TRUE) # verify
    # 80323301  [1]
    ...
    -   Assumes that the population within a source polygon is uniformly distributed, calculates the geometric intersection between source and target, and sums up the proportional values.
    ...
    ``` r
    # grid_de is a stars object which is a list w/attr. select outputs a list w/attr. st_extract combines the values from the stars object to the locations specified by a sf object and     returns a sf dataframe.
    sf_no2_popdens <- 
      grid_de |>
      select("pop_dens") |>
      st_extract(sf_no2) |>
