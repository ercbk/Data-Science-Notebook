...
-   Notes from
    ...
    -   [ArcGIS Pro: How Kriging works](https://pro.arcgis.com/en/pro-app/latest/tool-reference/3d-analyst/how-kriging-works.htm)
...
-   Resources
    ...
    -   [Spatial Sampling With R, Ch. 21](https://dickbrus.github.io/SpatialSamplingwithR/Introkriging.html)

-   **Ordinary Kriging (OK)** - Aassumes that the mean of the study variable is the same everywhere. Considered a reasonable assumption unless there's a theoretical reason to dismiss it

-   **Universal Kriging** - Assumes that there is an overriding trend in the data—for example, a prevailing wind — and it can be modeled by a deterministic function, a polynomial.

    -   This polynomial is subtracted from the original measured points, and the autocorrelation is modeled from the random errors.
    -   Once the model is fit to the random errors and before making a prediction, the polynomial is added back to the predictions to give meaningful results.
    -   Universal kriging should only be used when you know there is a trend in your data and you can give a scientific justification to describe it.
...
-   [{gstat}]{style="color: #990000"} Kriging Functions
    ...
    -   The `krige` documentation is somewhat lacking since the sf method isn't there (most workflows probably involve a sf object). The examples in the docs and in the SDS book don't include the arguments either, but you can figure (most of) them out. If you use `showMethods` it shows you the available classes.
        ...
        -   Typing `gstat:::krige.sf(` in RStudio and hitting tab shows it has these arguments: `krige(formula, locations, newdata,..., nsim)`. The docs say the dots arguments are passed to the `gstat` function.
        -   The first argument of `gstat` is [g]{.arg-text}. It's description is obscure, and its code is worse. Anyways, this is where the variogram model evidently goes.
...
-   Empirical (aka Sample or Experimental) Variogram
...
-   The empirical variogram cannot be computed at *every* distance, $h$, and due to variation in the estimation, it is not ensured that it is a *valid* (?) variogram which is needed for interpolation models. To ensure a valid variogram, models (aka Theoretical Variograms) are used to estimate it.
...
-   [Example 2]{.ribbon-highlight}: Block Kriging NO~2~ in Germany\
    ...
    -   By including the all the data in the means calculations, kriging achieves substantially smaller standard errors than those calculated for sample means.
    :::
