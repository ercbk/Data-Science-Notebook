...
-   [{]{style="color: #990000"}[SpatialDownscaling](https://cran.r-project.org/web/packages/SpatialDownscaling/index.html){style="color: #990000"}[}]{style="color: #990000"} - Methods for Spatial Downscaling Using Deep Learning
    -   The aim of the spatial downscaling is to **increase the spatial resolution of the gridded geospatial input data.**
    -   Contains two deep learning based spatial downscaling methods: super-resolution deep residual network (SRDRN) and UNet ([Paper](https://arxiv.org/abs/2512.13753))
...
-   [{]{style="color: #990000"}[spatialRF](https://blasbenito.github.io/spatialRF/){style="color: #990000"}[}]{style="color: #990000"} - Explanatory spatial regression models by combining **Random Forest** with spatial predictors that help the model reduce the spatial autocorrelation of the residuals and return honest variable importance scores.
...
-   Resources
    ...
    -   [Spatial Data Analysis With Terra, Ch. 4 Interpolation](https://rspatial.org/analysis/4-interpolation.html)
...
-   [Example]{.ribbon-highlight}: Exponential Model\
    ...
    -   Additional `variogram` arguments:
        ...
        -   [width]{.arg-text}: The binwidth distance for the location pairs. (Default: [cutoff]{.arg-text} divided by 15)
            -   See Bin Calculations \>\> What Should My Bin Width be? below
...
-   [What should my bin width be?]{.underline}
    ...
    -   Workflow
        ...
        4.  Interpret results
            ...
            | Sensitivity outcome | What you observe | What the data are telling you | Correct interpretation | Implication for final model |
            |---------------|---------------|---------------|---------------|---------------|
            | **Range stable across Δ ≥ Δ_min, no nugget** | Similar range for multiple bin widths | Distance-decay is identifiable under continuity | Spatial correlation exists at that scale *conditional on smoothness* | Use Δ_min; simple spatial model acceptable |
            | **Range unstable for Δ ≈ Δ_min but stabilizes for larger Δ** | Range varies wildly at small bins, settles as Δ increases | Short-lag bins too noisy; geometry limits resolution | Fine-scale structure not supported by sampling design | Use larger Δ in final model |
            | **Range stable w.r.t. Δ but shifts substantially when nugget added** | Nugget absorbs short-lag variance; range increases | Short-lag variability not forced to be spatial | Spatial scale not identified without continuity assumption | Report range *with nugget* and qualify interpretation |
            | **Range unstable w.r.t. both Δ and nugget** | No consistent range under any specification | No clear distance-decay signal | Spatial dependence weak or absent | Avoid spatial model; or use strong regularization |
            | **Removing first bin has little effect** | Range essentially unchanged | No single-bin leverage | Instability is structural, not outlier-driven | Keep bins; focus on model assumptions |
            | **Removing first bin changes range materially** | Large shift in range | First bin dominates fit | Short-scale behavior unidentified | Either increase Δ or include nugget |
            | **Nugget ≈ large fraction of sill** | Nugget comparable to psill | Most variability non-spatial at observed scale | Distance explains limited variance | Expect weak kriging gain |
            | **Nugget ≈ 0 with stable range** | Nugget not selected by optimizer | Smooth spatial process supported | Strong spatial continuity | Spatial prediction likely effective |
