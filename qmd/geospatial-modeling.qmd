...
-   [Example]{.ribbon-highlight}: Exponential Model\
    ...
    -   I think the `vgm` arguments (besides [model]{.arg-text}) specify starting values in the estimation process. It is possible to call `vgm("exp")` and function will use an algorithm to choose starting values for that model.
        -   Model types combined, e.g. `c("Exp", "Sph")`, in which case the best fitting is returned
