...
-   [Example 3]{.ribbon-highlight}: Universal Kriging
    ...
    ###### Using Area Weighted Aggregation
    ...
    sf_pop_poly <- sf_pop
    st_geometry(sf_pop_poly) <- polys

    # derive bounding box from sf_pop_poly
    bb <- st_bbox(sf_pop_poly)
    ...
    r100 <- rast(
      ...
      resolution = 100,
      crs = st_crs(sf_pop_poly)$wkt
    )

    pop_r <- rasterize(vect(sf_pop_poly), r100, field="Einwohner", fun="sum")

    # should match exactly (aside from floating-point rounding)
    global(pop_r, "sum", na.rm = TRUE)[1]
    #>                sum
    #> Einwohner 80324282
    sum(sf_pop_poly$Einwohner)
    #> [1] 80324282
    ...
    #> [1] 80324279
    sum(sf_pop_poly$Einwohner, na.rm = TRUE)
    #> [1] 80324282
    ...
    # 1. Assign the calculated density back to the original stars object
    ...
    # 2. Plot the stars object directly
    ggplot() +
      geom_stars(data = grid_masked, aes(fill = pop_dens)) + 
      geom_sf(data = shp_de, fill = NA, color = "black", linewidth = 0.5) +
      scale_fill_viridis_c(
    ...
    -   Could also use `st_interpolate_aw` from [{sf}]{style="color: #990000"} which takes polygons as the source, but I found this less accurate and slower (i.e. more computationally expensive
        -   Example:

            ``` r
            grid_sf <- st_interpolate_aw(
              sf_pop_poly["Einwohner"], 
              grid_sf, 
              extensive = TRUE
            )
            ```

            -   [extensive = TRUE]{.arg-text} since population is an extensive variable (i.e. count)
