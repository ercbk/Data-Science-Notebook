...
-   Assumptions
    ...
    -   Assumes the mean is constant between samples and is independent of location
...
-   Preprocessing
    ...
    -   Detrending ([trend.beta]{.arg-text}) - Often improves variogram model regression, but it also removes/diminishes the effects of spatially correlated covariates
        -   Recommend to store the detrended linear effects for later by applying your own `lm`, prior to variogram fitting, e.g. `resids_detrend <- lm(resp ~ long + lat, data)$residual`
    -   Fixing the range can be a performance saver for larger data
...
-   [Example]{.ribbon-highlight}: Ordinary Kriging of NO~2~ in Germany\
    ...
    -   The `krige` documentation is very obsure about what the names of the arguments are. Some stay with this function. Some go to another function (`gstat`), but the documentation of that function is also obsure. Usually you can figure it out by the order of the arguments but not here. It seems like you just throw stuff in there, and I guesss the function will figure out what it is depending on the class of the input.
    -   [NO2 \~ 1]{.arg-text} specifies an intercept-only (unknown, constant mean) model for (aka ordinary kriging model)
