...
- **Custom Cost Functions**
  ...
  - Resources
    - Applied Machine Learning Using mlr3 in R, [Ch.13.1](https://mlr3book.mlr-org.com/chapters/chapter13/beyond_regression_and_classification.html#sec-cost-sens)
  - Examples
...
- [**Proper Scoring Rule**]{style="color: #009499"} (SO [discussion](https://stats.stackexchange.com/questions/312780/why-is-accuracy-not-the-best-measure-for-assessing-classification-models) + Harrell links, [Paper](https://sites.stat.washington.edu/raftery/Research/PDF/Gneiting2007jasa.pdf)) - A score assesses whether a probabilistic forecast is close to the true distribution. A score that is minimized in expectation if the predictive density is the true density.
...
- **Prevalence**
  ...
  - See [{]{style="color: #990000"}[alternativeROC](https://cran.r-project.org/web/packages/alternativeROC/index.html){style="color: #990000"}[}]{style="color: #990000"} which plots PPV or NPV criteria, which accounts for prevalence, in the receiver operating characteristic (ROC) space
    - "Where clinical practitioners will often gauge the merits of a test in terms of prevalence-dependent metrics like positive predictive value (PPV) or negative predictive value (NPV), test developers will usually use other statistics, such as the area under the receiver operating characteristic (AUROC, also referred to as the *c*-statistic or the AUC), which are considered prevalence independent, to do the same."
...
- **Eigenvalues Entropy (EVE)**

  - [{]{style="color: #990000"}[eve](https://cran.r-project.org/web/packages/eve/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2511.01904)) - The Eigenvalues Entropy as a Classifier Evaluation Measure
  - Used as an evaluation measure for a binary or a multi-class problem
  - The confusion matrix entries are first adjusted for each class and an eigenvalue decomposition method is used to obtain an evaluation measure which especially varies with the off-diagonal entries of the confusion matrix.
  - Formula\
    ...
    &P = MD^{-1} \\
    &B = \frac{(P + P^T)}{2} \\
    &\text{eve}(B) = - \sum_{i = 1}^n \frac{\eta_i \log(\eta_i)}{\log(n)}
    \end{align}$$
    - $M$ is the confusion matrix
      - The sum of the entries in the row $i$ and the column $j$ of the matrix $M$ are denoted $m_{i .}$ and $m_{. j}$ , respectively, and $m_{. j}$ is the size of the j^th^ class.
    - $P$ is the scaled version of $M$
      - The entries of $P$ are $p_{ij} = m_{ij} / m_{.j}$ where $\sum_{i=1}^n p_{ij} = 1$
      - $D^{−1} = \text{diag}(1 / m_{.1}, 1 / m_{.2}, \ldots, 1 / m_{.n})$ is a diagonal matrix
      - Scaling the entries is useful for an imbalanced classification problem where a direct use of the entries $m_{ij}$ may have a negative influence on some evaluation measures.
    - $B$ is the real-valued, symmetric version of $P$
    - $\eta_i = \lambda_i / n^*$
      - $\lambda_i$ is a positive-valued eigenvalue of $B$
      - $n^*$ is the sum of the positive-valued eigenvalues.
  - Range: \[0, 1\]
    - EVE is one when all off-diagonal entries of the Confusion Matrix (CM) are zero.
    - Its value decreases continuously towards zero when the off-diagonal entries increase to be the same as the diagonal entries for all classes independently of their sizes (bad prediction results).
  - The bounds of the eigenvalues (`eve.bounds`) can also be used to judge a classifier. The range of these bounds is small (\<2) for a good classifier.
  - Issues
    - Eigenvalue decomposition can be computationally intensive
    - For a binary problem is it makes no difference between a random assignment for the two classes and the results where one class is wrongly predicted and the other is correctly predicted, presence of negative eigenvalue. Since it is shown that the sum of the eigenvalues for a binary problem is related to the AUC and the Gini index, a negative eigenvalue will also lead to a small value for these two measures.
  - Pros
    - EVE results show a better performance for balanced and imbalanced situations.
    - The *estimated* confusion matrix, $\tilde M$, can be used to improve other classification scores: $\tilde M = D^{1/2}\;M\;D^{-1/2}$
      - Paper showed $\tilde M$ improved the MCC score for multi-class example.
...
- [Misc]{.underline}
  - Packages
    - [{]{style="color: #990000"}[RobustMetrics](https://cran.r-project.org/web/packages/RobustMetrics/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://arxiv.org/abs/2404.07661)) - Calculates robust (to imbalance) versions of Matthews Correlation Coefficient (MCC) and F-Beta Scores
      - Also plots the receiver operating characteristic curve (ROC curve) together with the recall / 1-precision curve
...
- [Guidelines for AUPRC vs AUROC]{.underline}
  ...
  - ROC Surface (ROCS)
    - [{]{style="color: #990000"}[ROCS](https://cran.r-project.org/web/packages/ROCS/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://pubmed.ncbi.nlm.nih.gov/22792381/))
    - An ROC surface (ROCS) uses the true positive rate (TPR), false positive rate (FPR), and true discovery rate (TDR) (i.e. adds TDR axis to the ROC))
    - In \[imbalanced data\] situations, the false discovery rate (FDR) could become unacceptably high. Thus, only a small corner of the ROC curve is relevant in practical terms, and the AUC can no longer summarize the effectiveness of the separation.
    - Unlike the ROC curve, the ROC surface and its associated quantities are dependent upon the class ratio. The results are meaningful only when the class ratio of the training data is close to that of future data.
      - In the case that the user foresees a discrepancy, the issue can be addressed by (repeated) resampling from the training data to achieve targeted class ratios and building ROCS.
    - Both VUS and FCAUS seem to both be usable for the same purpose and situation, except the FCAUS requires the user to supply the FDR threshold.
    - Volume Under the Surface (VUS)
      - VUS can be viewed as a weighted version of the AUC of the regular ROC curve, with the weight at every TPR level being the corresponding TDR values. Therefore, it gives higher weight to higher TDR regions.
      - Maximum VUS = 1: Which is achieved when the true positive and true negative samples can be perfectly separated, no matter what the class ratio is
      - Minumum VUS = 0: Which is the scenario where the two distributions are inseparable, and the class ratio (# true negatives/# true positives) approaches infinity.
      - Significance is determined through a permutation test. The proportion of the sampled permutations with VUS values larger than the observed value is taken as the p-value of the one-sided test
    - FDR-Controlled Area Under the Curve (FCAUC)
      - The area under the traditional ROC curve corresponding to a pre-determined FDR (1-TDR) threshold
      - Min and max values should 1 and 0 also.

## Multinomial {#sec-diag-class-multinom .unnumbered}

- [Misc]{.underline}
  - Packages
    - [{]{style="color: #990000"}[gofcat](https://cran.r-project.org/web/packages/gofcat/index.html){style="color: #990000"}[}]{style="color: #990000"} - Goodness-of-Fit Measures for Categorical Response Models
      - Currently supports `serp`, `clm`, `polr`, `multinom`, `mlogit`, `glm`, and `vglm`
    ...
    - [{]{style="color: #990000"}[multChernoff](https://cran.r-project.org/web/packages/multChernoff/index.html){style="color: #990000"}[}]{style="color: #990000"} - Computes finite-sample tail bounds for the **likelihood ratio test (LRT)** under multinomial sampling.
      - These bounds can be used to obtain conservative p-values and critical values when the **sample size is not large relative to the alphabet size**, so standard chi-squared asymptotics may be inaccurate.
  - Also see
...
- [Misc]{.underline}
  ...
  - [{]{style="color: #990000"}[ROCModels](https://cran.r-project.org/web/packages/ROCModels/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides a unified framework for constructing, visualizing, and comparing parametric, nonparametric, semiparametric, and Bayesian ROC curves.
    - Helps researchers identify and implement ROC inference methods most suitable for their data.
...
- Also see
  ...
  - A tutorial on calibration measurements and calibration models for clinical prediction models ([A tutorial on calibration measurements and calibration models for clinical prediction models](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7075534/))
...
- Notes from [The Integrated Calibration Index (ICI) and related metrics for quantifying the calibration of logistic regression models](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6771733/)
...
- [Bootstrap CIs for ICI, E50, E90, Emax]{.underline}
  ...
  - [Example]{.ribbon-highlight}: Using `rms::val.prob` with a `glm` model ([article](https://darrendahly.github.io/post/homr/))
...
- **Weak** - means that, on average, the model does not over or underestimate risk and does not give overly extreme (too close to 0 and 1) or modest (too close to disease prevalence or incidence) risk estimates.
  ...
  - slope - target value of 1
    - A logistic regression model, Y \~ Yhat, is fit to get the calibration slope
...
- [Example]{.ribbon-highlight} 2 ([Calibration: the Achilles heel of predictive analytics](https://bmcmedicine.biomedcentral.com/articles/10.1186/s12916-019-1466-7))\
