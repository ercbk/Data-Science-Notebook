...
- Packages
  - [{]{style="color: #990000"}[mall](https://mlverse.github.io/mall/){style="color: #990000"}[}]{style="color: #990000"} - Text analysis by using rows of a dataframe along with a pre-determined (depending on the function), one-shot prompt. The prompt + row gets sent to an Ollama LLM for the prediction
    ...
    - Features
      ...
      - Custom prompt
  - [{]{style="color: #990000"}[quallmer](https://cran.r-project.org/web/packages/quallmer/index.html){style="color: #990000"}[}]{style="color: #990000"} - Qualitative Analysis with Large Language Models
    ...
    - [{]{style="color: #990000"}[quallmer.app](https://cran.r-project.org/web/packages/quallmer.app/index.html){style="color: #990000"}[}]{style="color: #990000"} - Companion package to 'quallmer' providing an interactive 'shiny' application for manual coding, reviewing large language model (LLM) generated annotations, and computing inter-rater reliability metrics
  ...
  - [{]{style="color: #990000"}[SemanticDistance](https://reilly-conceptscognitionlab.github.io/SemanticDistance/){style="color: #990000"}[}]{style="color: #990000"}
  ...
  - [{]{style="color: #990000"}[text2vec](https://text2vec.org/){style="color: #990000"}[}]{style="color: #990000"}
    ...
    - Vectorization: Vocabulary-based, N-Grams, Feature Hashing
    - Basic Transformations: TF-IDF, Normalization
...
- Resources
  - [Text Classification for Organizational Researchers: A Tutorial](https://journals.sagepub.com/doi/10.1177/1094428117719322)
    - [{]{style="color: #990000"}[textclassificationtutorial](https://cran.r-project.org/web/packages/textclassificationtutorial/index.html){style="color: #990000"}[}]{style="color: #990000"}
    - Covers HTML text extraction, sentence segmentation, text preprocessing, document-term matrices, TF-IDF, keyword extraction, cosine similarity, stratified cross-validation, classification metrics, and a multinomial Naive Bayes classifier
- Typical Workflow ([source](https://text2vec.org/vectorization.html)):
  1.  Construct a document-term matrix (DTM) or term-co-occurrence matrix (TCM) from input documents.
      - i.e. Vectorize text by creating a map from words or n-grams to a vector space.
      - Tokenize (+ (optionally) pruning) or N-Grams (+ (optionally) hashing) $\rightarrow$ Create DTM $\rightarrow$ Transform DTM (e.g. normalization, tf-idf)
  2.  Fit (including tuning and validating) a model to that DTM.
      - These models might include text classification, topic modeling, sentiment analysis, similarity search, etc.
