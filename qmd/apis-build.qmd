...
- Packages
  ...
  - [{]{style="color: #990000"}[drogonR](https://github.com/Zabis13/drogonR){style="color: #990000"}[}]{style="color: #990000"} - High-performance HTTP server for R, powered by the Drogon C++ framework.
    - Rrovides a plumber-style API for building REST services and APIs from R, with substantially higher throughput. The Drogon, Trantor and JsonCpp sources are bundled and built statically — no external installation of Drogon is required.
...
- Comparisons
  ...
  - Python's FastAPI vs GO ([source](https://habr.com/ru/articles/981176/))
    - Go (net/http)
...
- GraphQL
  - Packages
    - [gqlr](https://schloerke.com/gqlr/)- A GraphQL Implementation in R
      - Has the full functionality of GraphQL, but doesn't require users to write full object definitions when they can already be defined with GraphQL syntax.
  - GraphQL is a data “query language created by Facebook in 2015 for describing the capabilities and requirements of data models for client‐server applications”
  - Provides a more flexible way to ask for data through an API
  - [Tutorials](https://graphql.org/learn/)
  - Traditional REST API:
    - You call `/api/users/123` → get back a big JSON object with everything about that user

    - You call `/api/users/123/posts` → get back all their posts

    - You call `/api/posts/456/comments` → get back comments for a specific post

    - You're stuck with whatever the API developers decided to give you.
  - GraphQL API:
    - You send ONE request to `/graphql` with a query describing exactly what you want

    - The GraphQL server figures out how to get that data (maybe from PostgreSQL, maybe from MongoDB, maybe from other APIs, maybe from a cache)
...
- [Serialization]{.underline}
  - Also see [JSON](json.qmd#sec-json){style="color: green"} for fast serialization packages

  - Packages

    - [{]{style="color: #990000"}[RProtoBuf](https://cran.r-project.org/web/packages/RProtoBuf/index.html){style="color: #990000"}[}]{style="color: #990000"} - A serialization format (Protocol Buffers) developed by Google and used widely within Google, and by numerous other projects
...
- [Example]{.ribbon-highlight}: Basic Async
  ...
  - The mirai object is converted into a promise using the promise pipe operator from [{promises}]{style="color: #990000"}.
