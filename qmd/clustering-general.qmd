...
-   Interpetable Clustering
    -   Notes from [Introduction to Interpretable Clustering](https://towardsdatascience.com/introduction-to-interpretable-clustering-d0e07fbd2c99)
        -   Overview of methods with refererences to papers
...
-   Misc
    ...
    -   Packages
        -   [{]{style="color: #990000"}[mclust](https://cloud.r-project.org/web/packages/mclust/index.html){style="color: #990000"}[}]{style="color: #990000"} - Gaussian Mixture Modelling for Model-Based Clustering, Classification, and Density Estimation
            -   Old package that's built as a companion to a book (which I don't have and it's not freely available). It has a vignette, but it just illustrates the available functions. There aren't any descriptions, workflows, or interpretations.
            -   There's an article in JOSS, [Model-based Methods of Classification: Using the mclust Software in Chemometrics](https://www.jstatsoft.org/article/view/v018i06), which seems to give a little more idea on how to use the package.
            -   There's also a website, [mclust.org](https://mclust-org.github.io/), which lists other packages from the group that are associated with GMMs. Their documentation isn't great as well, but I haven't checked the papers associated with them to see if they'd be much help.
