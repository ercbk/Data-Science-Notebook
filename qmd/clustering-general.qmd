...
* Also see
	* Notebook, pgs 57-58
	* [Diagnostics, Clustering](Diagnostics, Clustering)
...
* Cluster Descriptions
	...
	* Run a decision tree on clusters![](./_resources/Clustering.resources/Screenshot (279).png)
		* Each color (orange, blue, green, purple) represents a cluster
		* Explains how clusters were generated
		* [[{treeheatr}]{style='color: #990000'}](https://trang1618.github.io/treeheatr/)![](./_resources/Clustering.resources/image.1.png)
...
* **Gaussian Mixture Models** **(GMM)**
	...
	* Components of the Algorithm
		* "Color" points according to gaussians (clusters)
			...
			* ![](./_resources/Clustering.resources/Screenshot (32).png)
			* 
		* Fitting a Gaussian
			...
			* Find the spread of the points
				...
				* ![](./_resources/Clustering.resources/Screenshot (34).png)
					First equation is the height of Gaussian (multi-variate Gaussian distribution equation). (Second equation is just the familiar 1-D gaussian equation for reference)
				...
				* Partially "colored" points affect spread and center of mass calculations
					...
					* ![](./_resources/Clustering.resources/Screenshot (36).png)
	* Steps
...
* **UMAP**
	...
	* Tuning parameter: neighbors
		* example used 500 iterations (n\_epochs) as limit for convergence
* **K-Means**
	...
	* For choosing the number of clusters, elbow method (i.e. WSS) is usually awful if there are more than few clusters. Recommended: Calinski-Harabasz Index and BIC then Silhouette Coefficient or Davies-Bouldin Index (see [Diagnostics, Clustering](Diagnostics, Clustering)) ([article](https://towardsdatascience.com/are-you-still-using-the-elbow-method-5d271b3063bd))
...
* **Approximate Nearest Neighbor (ANN)**
	...
	* Facebook AI Similarity Search (FAISS)
		...
		* Two approximations available
				Embeddings are clustered and centroids are calculated. The k nearest centroids are returned.
...
* **DBSCAN**![](./_resources/Clustering.resources/1-rLkYqpNmEguxDPuhLaTtVA.png)
	* Misc
		...
		* Packages
			...
			* [[{parameters}]{style='color: #990000'}](https://easystats.github.io/parameters/articles/clustering.html#dbscan) -
				* `n_clusters_dbscan` - Given a "min\_size" (aka _minPts_?), the function estimates the optimal "eps"
	...
	* Tuning
		* _eps_ \- the maximum distance between two samples for one to be considered to be connected to the other
		...
		* _minPts_ (or _min\_samples_) - the minimum number of samples in a neighborhood for a point to be considered as a core point
	...
	* Algorithm
		* For each data point, find the points in the neighborhood within _eps_ distance, and define the core points as those with at least _minPts_ neighbors.![](./_resources/Clustering.resources/Screenshot (1418).1.png)
			* The orange circle represents the _eps_ area
			* If _minPts_ = 4, then the top 4 points are core points because they have at least 4 points overlapping the _eps_ area
		* Define groups of connected core points as clusters.![](./_resources/Clustering.resources/Screenshot (1419).2.png)
			* All the green points have been labelled as core points
		* Assign each non-core point to a nearby cluster if it’s directly reachable from a neighboring core point, otherwise define it as an outlier.![](./_resources/Clustering.resources/Screenshot (1420).png)
	...
	* Advantages
		...
		* Clusters formed by DBSCAN can be any shape, which makes it robust to different types of data.
			* Example: Nested Cluster Structure
				* K-Means![](./_resources/Clustering.resources/Screenshot (1418).png)
					* K-Means wants spherical clusters which makes it grab groups of points it shouldn't
				* DBSCAN![](./_resources/Clustering.resources/Screenshot (1419).png)
					* Able correctly identify the oblong shaped cluster
	* Disadvantages
		* if the data has a very large variation in densities across clusters because you can only use one pair of parameters, _eps_ and _MinPts_, on one dataset
		* it could be hard to define _eps_ without the domain knowledge of the data
