...
-   [Examples]{.underline}
    ...
    -   [Example 2]{.ribbon-highlight}: Classic K-Means ([article](https://www.business-science.io/code-tools/2023/07/06/kmeans-clustering.html), [code](https://github.com/business-science/free_r_tips/blob/master/063_tidyclust/063_tidyclust.R))
        ...
        -   [Example 3]{.ribbon-highlight}: PCA + K-Means
            ...
            -   Other transformations such as square root for discrete variables and logging for skewed contiuous variables are recommended but I didn't worry about that here. (See [Feature Reduction, General \>\> PCA \>\> Preprocessing](feature-reduction-general.qmd#sec-featred-gen-pca-proc))
            ...
            # Choose components that explain ~80-90% of variance
            n_components <- which(summary(res_pca)$importance[3,] >= 0.90)[[1]]
            mat_pca_sub <- res_pca$x[, 1:n_components]
            mat_pca_dist <- dist(mat_pca_sub)
            ```

            -   By looking at the `summary` or the scree plot, we can see that PC1 only accounts around 21.3% of the variance and begins to level off around PC8.
            ...
            tib_clust_stats <- purrr::map(2:n_components, \(k) {
              ...
              tib_cs <- 
                fpc::cluster.stats(mat_pca_dist, res_km$cluster)[c("cluster.number", "within.cluster.ss", "avg.silwidth", "ch")] |> 
                as_tibble() |> 
            ...
            # Perform k-means clustering on PC space
            k <- tib_clust_stats |> 
              filter(ch == max(ch)) |> 
              pull(cluster.number)
            ...
            -   From my reading, the Calinski-Harabasz Index, Silhouette Coefficient, or Davies-Bouldin Index work best with K-Means ([See Diagnostics, Clustering \>\> Spherical/Centroid based](diagnostics-clustering.qmd#sec-diag-clust-sphcent){style="color: green"})
                -   Not sure, if these values are the same as described in that notebook section. I don't think it matters too much in this case though seeing as their was apparent separation possible given the data (See next section) and only 1 reasonable $k$ (2).
            -   Try to find a simple plug and play cluster diagnostic package is difficult. None seem to have been written recently (or with decent documentation), and most require you to use the same package to perform the clustering.
