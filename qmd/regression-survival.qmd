...
- [{]{style="color: #990000"}[FastJM](https://cran.r-project.org/web/packages/FastJM/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements **scalable joint models** for large-scale competing risks time-to-event data with one or multiple **longitudinal** biomarkers using the efficient algorithms
  - The time-to-event process is modeled using a cause-specific Cox proportional hazards model with time-fixed covariates, while longitudinal biomarkers are modeled using linear mixed-effects models.
  - The association between the longitudinal and survival processes is captured through shared random effects.
...
- Log-Rank Test (aka Mantel-Cox test) - tests if two groups survival curves are different
  - Non-Parametric; a special case with one bin\`ary X
...
- [**Censoring Time (C)**]{style="color: #009499"}: Time at which censoring occurs
  ...
  - Plain Censoring: Someone lost to follow-up contributes data up to the last point they were known to be event-free, and is then dropped from the risk set for a time-to-event analysis, rather than counted as "no event" for the rest of the trial.
    - That relies on an assumption that dropout is unrelated to the outcome conditional on measured covariates ("non-informative censoring")
    - If sicker people are disproportionately the ones who disappear, then plain censoring understates the true effect.
  - CONSORT reporting standards require trials to report attrition explicitly, by arm, with reasons where known, e.g. "stopped coming to visits" or "outcome unknown."
    - For hard endpoints like death, many trials can ascertain vital status through a national death registry
  - When dropout looks like it might be informative, trials run sensitivity analyses, multiple imputation, best-case/worst-case bounds, or inverse probability of censoring weighting (IPCW).
- [**Competing Risks**]{style="color: #009499"} - A situation where an event of interest (the primary event) can be precluded or hindered by the occurrence of a different event (a competing event).
  - Also see [Experments, Analysis \>\> Intercurrent Events](experiments-analysis.qmd#sec-exp-anal-intevent){style="color: green"}
...
- Misc
  ...
  - Packages
    ...
    - [{]{style="color: #990000"}[cox.rvph](https://cran.r-project.org/package=cox.rvph){style="color: #990000"}[}]{style="color: #990000"} - Remedying proportional hazards assumption violations of a Cox proportional hazards model using stepwise changepoint and time-varying coefficient methods based on Cox
...
- Packages
  ...
  - [{]{style="color: #990000"}[SuperSurv](https://yuelyu21.github.io/SuperSurv/){style="color: #990000"}[}]{style="color: #990000"} - A Unified Ecosystem for Machine Learning, Ensembles, and Interpretability in Survival Data
...
- Packages
  ...
  - [{]{style="color: #990000"}[RFmstate](https://cran.r-project.org/web/packages/RFmstate/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fits cause-specific random survival forests for flexible multistate survival analysis with covariate-adjusted transition probabilities computed via product-integral.
    - State transitions are modeled by random forests. Subject-specific transition probability matrices are assembled from predicted cumulative hazards using the product-integral formula
