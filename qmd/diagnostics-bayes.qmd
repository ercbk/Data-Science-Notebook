...
- Packages
  - [{]{style="color: #990000"}[bridgesampling](https://cloud.r-project.org/web/packages/bridgesampling/index.html){style="color: #990000"}[}]{style="color: #990000"} - For conducting Bayesian model comparisons using bridge sampling.
    - In most applications, the marginal likelihood is not analytically tractable and must be approximated using numerical methods, e.g. bridge sampling.
    - Allows one to compute marginal likelihoods, Bayes factors, and posterior model probabilities
...
### Misc {#sec-diag-bay-conv-misc .unnumbered}

- Language: Refer to the *inference algortihm* not converging (instead of the model), since convergence depends not only on just the model but also on the data and the parameterization.
  - "The same model and different parameterization or data lead to different posterior geometry. For the same posterior, different iterative algorithms or algorithm choices can also lead to different convergence problems." ([Vehtari](https://statmodeling.stat.columbia.edu/2026/04/02/is-your-model-converging/))
...
- [Effective Sample Size (ESS)]{.underline}
  ...
  - [Vehtari post](https://statmodeling.stat.columbia.edu/2025/11/27/effective-sample-size/): Using phrasing, "the number of effective samples ...sounds like some samples are effective and some are not, but the effectiveness is not a property of individual samples but the whole sample. ....That’s why we recommend shortening the effective sample size as ESS (not n_eff)."
...
- There are also PPD calibration diagnostics in [{bayesplot}]{style="color: #990000"}

  - See this Vehtari [thread](https://bsky.app/profile/avehtari.bsky.social/post/3mtxzc4colc26) for more details
