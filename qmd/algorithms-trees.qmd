...
- [{]{style="color: #990000"}[ggparty](https://cran.r-project.org/web/packages/ggparty/index.html){style="color: #990000"}[}]{style="color: #990000"} - [{ggplot}]{style="color: #990000"} **visualizations** for the [{partykit}]{style="color: #990000"}
...
- [{]{style="color: #990000"}[longitree](https://cran.r-project.org/web/packages/longitree/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements tree-based methods for **longitudinal** data.
  - Constructs decision trees that evaluate both the main effect of a covariate and its interaction with time through a weighted splitting criterion
  - Supports single-tree construction, bootstrap-based multiple-tree selection, and tree visualization
...
- [Classification Procedure]{.underline}
  1.  Calculate *Shannon Entropy* of dependent variable ($Y$):\
      ...
      - In general the Shannon Entropy equation is\
        ...
        $$E(S) = -\sum_{i=1}^{c} p_i \log_2 p_i$$ where $p$ is a probability and $c$ is the number of classes for the variable and $S$ = subset of data or the node.
...
- A **rule** is the full logical statement that defines the path to the terminal nodes.
- Packages
  - Models
  ...
  - Visualization
    - [{]{style="color: #990000"}[lorax](https://lorax.tidymodels.org/){style="color: #990000"}[}]{style="color: #990000"} - Converts tree models from {tidymodels} to {partykit} so they can be visualized and have features extracted.
      - [`extract_rules()`](https://lorax.tidymodels.org/reference/extract_rules.html) helps write out the logical paths to the terminal nodes.
      - [`active_predictors()`](https://lorax.tidymodels.org/reference/active_predictors.html) enumerates which predictors were actually used in a split.
      - [`var_imp()`](https://generics.r-lib.org/reference/var_imp.html) is a wrapper for any importance method contained in the package. This is a wrapper that enables a common interface to the scores.
...
4.  Sum all tree weights, $w_{it}$, for that training observation and divide by the number of trees to get the forest-weight for that training observation.\
    ...
    - In the paper, this process is described by a more compact notation: $$w_i^{\text{forest}}(z) =
