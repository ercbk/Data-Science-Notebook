...
-   Residual plots - model_performance
    -   (line) plot (shows resid distribution),
    -   (boxplot) plot(, geom = "boxplot").
        -   Smaller distr = better overall mod usually.
        -   Quantiles relate to size of resid.
        -   Largest residuals (largest errors) near 1 and smallest near zero.
        -   Y axis is percent of the total residuals.
        -   Red dot represents rmse (aka resid std dev) which I'm not sure what its location is supposed to say about the model.
...
-   LIFT charts - classification **GOF measure** - type = "LIFT" - Rate of Positive Prediction (RPP) plotted (y-axis) vs number of True Positives (TP) (x-axis) on a threshold t.\
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/unknown_filename.png) where P is the total positive classifications (TP + FP) predicted for that threshold and N is the total negative (TN + FN). The ideal model is represented with a orange/yellow curve. The model closer to the ideal curve is considered the better classifier.
...
-   Interactions
    ...
    -   [Example]{.ribbon-highlight}: Years on hormonal contraceptives (continuous) interacts with STDs (binary)\
        ![](./_resources/Diagnostics,_Model_Agnostic.resources/image.png)
    -   Interpretation
...
-   Cluster by shap value\
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/image.1.png)
...
-   Waterfall plots\
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/waterfall.png)
    ...
    -   Interpretation example\
        ![](./_resources/Diagnostics,_Model_Agnostic.resources/image.2.png)
-   force plot - Examines influences behind one predicted value that you input to the function.\
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/force_median.png)
...
-   Extreme values
    ...
    -   Check if extreme values of the sample distribution are covered by the alluvial\
        ![](./_resources/Diagnostics,_Model_Agnostic.resources/unnamed-chunk-11-1.png)
