...
-   Packages
    -   [{]{style="color: #990000"}[e2tree](https://cran.r-project.org/web/packages/e2tree/index.html){style="color: #990000"}[}]{style="color: #990000"} - Explain and interpret decision tree ensemble models using a single tree-like structure.
        -   A new way of explaining an ensemble tree trained through 'randomForest' or 'xgboost' packages.
    -   [{]{style="color: #990000"}[midr](https://ryo-asashi.github.io/midr/){style="color: #990000"}[}]{style="color: #990000"} - Implements ‘Maximum Interpretation Decomposition’ (MID), a functional decomposition technique that finds an optimal additive approximation of the original model. This approximation is achieved by minimizing the squared error between the predictions of the black-box model and the surrogate model.
        -   [{]{style="color: #990000"}[midnight](https://ryo-asashi.github.io/midnight/){style="color: #990000"}[}]{style="color: #990000"} - Implements a [{parsnip}]{style="color: #990000"} engine for the [{midr}]{style="color: #990000"} package, allowing users to seamlessly fit, tune, and evaluate MID (Maximum Interpretation Decomposition) models with [{tidymodels}]{style="color: #990000"} workflows.
    -   [{]{style="color: #990000"}[mrIML](https://cran.r-project.org/web/packages/mrIML/index.html){style="color: #990000"}[}]{style="color: #990000"} - Multi-Response (Multivariate) Interpretable Machine Learnin
-   These measures can't be used in a causal interpretation, but understanding the relationship between the target and predictors for a particular model is beneficial for multiple reasons. ([source](https://arxiv.org/abs/2501.07602))
    -   Model Assessment (e.g., is the model using variables in a scientifically reasonable manner?)
    -   Data Insight (e.g., what patterns in the data did the model find useful for prediction?)
    -   Building Trust in a Prediction (e.g., do we feel comfortable making a decision based on how the model made use of the data to produce a prediction?)
...
-   [**The first step is always going to be creating an "explain" object**]{.underline}
    ...
    -   [{]{style="color: #990000"}[DALEXtra](https://modeloriented.github.io/DALEXtra/){style="color: #990000"}[}]{style="color: #990000"} has helper functions set-up to support [{{sklearn}}]{style="color: goldenrod"}, [{{keras}}]{style="color: goldenrod"}, [{h2o}]{style="color: #990000"}, [{mlr3}]{style="color: #990000"}, [{tidymodels}]{style="color: #990000"}, and [{xgboost}]{style="color: #990000"} models.

### Explanatory Levels {#sec-diag-modagn-dalex-expllev .unnumbered}

![](./_resources/Diagnostics,_Model_Agnostic.resources/dalex-1-xai-pyramid.png){.lightbox width="632"}
...
### Instance Level {#sec-diag-modagn-dalex-instlev .unnumbered}

-   Use Case:
    -   Investigate extreme response values
    -   Investigate observations not predicted well by your model

#### Break-Down (BD) {#sec-diag-modagn-dalex-instlev-bd .unnumbered}
...
-   [Interpretation]{.underline}
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/BDjohnyExample-1.png){.lightbox width="532"}
...
-   When interactions, whether explicit like in linear regression models or implicit like in Random Forest models, are present in the model, the order that the predictors are presented to the DALEX function affects the estimated contribution.
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-bd-ordering.png){.lightbox width="432"}
...
-   [Example]{.ribbon-highlight}: Assume Interactions
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-ibd-1.png){.lightbox width="432"}
...
#### Lime {#sec-diag-modagn-dalex-instlev-lime .unnumbered}
...
-   See [Ch. 9.6](https://ema.drwhy.ai/LIME.html#LIMERcode) for discussion of some of the differences in LIME implementation between [{]{style="color: #990000"}[lime](https://lime.data-imaginist.com/){style="color: #990000"}[}]{style="color: #990000"}, [{]{style="color: #990000"}[localModel](https://modeloriented.github.io/localModel/){style="color: #990000"}[}]{style="color: #990000"}, and [{]{style="color: #990000"}[iml](https://christophm.github.io/iml/){style="color: #990000"}[}]{style="color: #990000"}.

-   [Example]{.ribbon-highlight}: [{localModel}]{style="color: #990000"}
    ...
    -   [type = "localModel"]{.arg-text} says which package to use

        -   [{lime}]{style="color: #990000"} and [{iml}]{style="color: #990000"} available

    -   [estimated]{.arg-text} shows the coefficients of a LASSO glass-box model

    -   Note the dichotomization of the continuous variable, [age]{.var-text}
        ...
        -   Cutpoint chosen using ceteris-paribus profiles. The point at which the largest change in the response is chosens as the cutpoint.
            -   CP Profile: `localModel::plot_interpretable_feature(locMod_johnny, "age")`
                ...
                ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-lime-locmod-1.png){.lightbox width="432"}
        -   [{iml}]{style="color: #990000"} doesn't transform continuous variables and [{lime}]{style="color: #990000"} uses quartile cutpoints

    -   Visualizaton: `plot(locMod_johnny)`

        ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-lime-locmod-2.png){.lightbox width="432"}

#### Ceteris Paribus (CP) {#sec-diag-modagn-dalex-instlev-cp .unumbered}
...
-   [Example]{.ribbon-highlight}: Compare 2 observations

    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-cp-2obs-1.png){.lightbox width="532"}
    ...
    -   `predict_profile`
        -   [variables]{.arg-text} - (Not used in this example) Names of explanatory variables, for which CP profiles are to be calculated. By default, [variables = NULL]{.arg-text} and the profiles are constructed for all variables, which may be time consuming.
        -   [variable_splits]{.arg-text} is an optional argument for providing a custom range of predictor values.
            -   By default the function uses the range of values in the training data which is obtained throught the [explainer]{.arg-text} object.
            -   Also, limits the computations to the variables specified in the data.frame
    -   `plot`
        -   [color = "\_ids\_"]{.arg-text} specifies that more than one observation is being compared

-   [Example]{.ribbon-highlight}: Compare Multiple Models

    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-cp-multmod-1.png){.lightbox width="532"}
    ...
    -   [age]{.var-text} has a non-linear shape because a spline transform was used

-   [Profile Oscillations]{.underline}
...
-   [Example]{.ribbon-highlight}: Basic
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-cp-oscill-1.png){.lightbox width="382"}
    ...
    -   [Gender]{.var-text} is a CP Profile that you should definitely look at. Followed by [sibsp]{.var-text}, [age]{.var-text}, and probably [class]{.var-text}

    -   [variable_splits]{.arg-text} is optional. Available if you want to use a custom grid of variables and values.

    -   [type]{.arg-text}

        -   [type = "oscillations"]{.arg-text} is used when [variable_splits]{.arg-text} is used. An average residual is what's being calculated, and one main difference in the types is the divisor (e.g. number of unique values, sample size) that's used. Here, I'm guessing this argument value sets the divisor to the number of grid values in [variable_splits]{.arg-text}.

        -   [type = "oscillations_uni"]{.arg-text} assumes a uniform distribution of the "residuals" which are differences between the CP value and prediction for that instance.

            -   Filters variables to only unique values

            -   Quicker to compute

        -   [type = "oscillations_emp"]{.arg-text} assumes an empirical distribution of the "residuals"
...
#### Local Diagnostics {#sec-diag-modagn-dalex-instlev-locdiag .unnumbered}
...
-   [Example]{.ribbon-highlight}
    -   Local-Fidelity
        ...
        ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-local-fidelity-1.png){.lightbox width="532"}
        ...
        -   There is a [nbins]{.arg-text}argument which has a default of twenty. This chart many need a few more bins

    -   Local-Stability

        ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-local-stab-1.png){.lightbox width="532"}
...
![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-PredvsFitted1-1.png){.lightbox width="432"}
...
-   [Histogram Plot]{.underline}
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-ResidualHist1-1.png){.lightbox width="432"}
...
-   [Boxplot]{.underline}
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-ResidualBoxplot1-1.png){.lightbox width="432"}
...
-   [Residuals vs Observation ID]{.underline}
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-ResidualvsID-1.png){.lightbox width="432"}
...
-   [Residuals vs Fitted]{.underline} - `plot(md_rf, variable = "y_hat", yvariable = "residuals")`
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-ResidualvsFitted-1.png){.lightbox width="432"}
...
-   [Residuals vs Observed]{.underline} - `plot(md_rf, variable = "y", yvariable = "residuals")`
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-scatterResidual1-1.png){.lightbox width="432"}
...
-   [Abs Residuals vs Fitted]{.underline} - `plot(md_rf, variable = "y_hat", yvariable = "abs_residuals")`
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-ScaleLocation1-1.png){.lightbox width="432"}
...
-   [Example]{.ribbon-highlight}
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-featureImportance-1.png){.lightbox width="432"}
...
-   [Use Cases]{.underline}
    ...
    -   Disagreement between profiles may suggest a way to improve a model. If a PD profile of a simpler, more interpretable model disagrees with a profile of a flexible model, this may suggest a variable transformation that can be used to improve the interpretable model. For example, if a random forest model indicates a non-linear relationship between the dependent variable and an explanatory variable, then a suitable transformation of the explanatory variable may improve the fit or performance of a linear-regression model.
        -   Example:
            ...
            ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-pdp-mod-comp-1.png){.lightbox width="502"}
...
-   [Checks]{.underline}
    ...
    -   Check CP profile for consistent behavior
        ...
        -   Solutions (If not parallel):
            ...
            -   Cluster CP profiles - Cluster the CP profiles (e.g. k-means) and the entroids will be the PD lines
                -   [Example]{.ribbon-highlight}
                    ...
                    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-pdp-cluster-1.png){.lightbox width="332"}
            ...
            -   Grouped-by CP profiles - Group CP profiles by a moderator variable
                ...
                -   [Example]{.ribbon-highlight}
                    ...
                    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-pdp-groupby-1.png){.lightbox width="332"}
        ...
        -   [Example]{.ribbon-highlight}: RF survival model
            ...
            ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-pdp-check-1.png){.lightbox width="432"}
...
#### Funnel Plot {#sec-diag-modagn-dalex-datlev-funnel .unnumbered}

-   From [{]{style="color: #990000"}[DALEXtra](https://modeloriented.github.io/DALEXtra/){style="color: #990000"}[}]{style="color: #990000"}, lets us find subsets of data where one of models is significantly better than other ones.
...
-   [Example]{.ribbon-highlight}
    ...
    ![](_resources/Diagnostics,_Model_Agnostic.resources/dalex-funnel-1.png){.lightbox width="632"}
    ...
    -   [nbins]{.arg-text} is the number of bins to partition continuous variables into.
    -   [measure_function]{.arg-text} is the metric used to judge each model. Here RMSE is used.
    -   [Length of segment is the difference in prediction performance. The longer the segments are, the better that model is at predicting for those subsets of data.]{.underline}
    -   For the most expensive and the cheapest (i.e. first and last bin of [m2.price]{.var-text}), the linear regression model performs best, while the random forest model performs best average priced (i.e. middle 3 bins of [m2.price]{.var-text}) apartments.
    -   The linear regression is extremely better at predicting prices for apartments in the Srodmiescie [district]{.var-text}.
...
-   Global Triplot\
    ![](_resources/Diagnostics,_Model_Agnostic.resources/triplot-global-1.png){.lightbox width="632"}
...
-   LIFT charts - classification **GOF measure** - type = "LIFT" - Rate of Positive Prediction (RPP) plotted (y-axis) vs number of True Positives (TP) (x-axis) on a threshold t.\
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/unknown_filename.png){.lightbox width="232"} where P is the total positive classifications (TP + FP) predicted for that threshold and N is the total negative (TN + FN). The ideal model is represented with a orange/yellow curve. The model closer to the ideal curve is considered the better classifier.
...
-   Packages
    ...
    -   [{]{style="color: #990000"}[qshap](https://cran.r-project.org/web/packages/qshap/index.html){style="color: #990000"}[}]{style="color: #990000"} - Computes feature-specific R-squared (R2) contributions for boosting tree models using a Shapley-value-based decomposition of the total R-squared in polynomial time.
        -   Supports models fitted with 'XGBoost' and 'LightGBM'
        -   Provides efficient parallel implementations suitable for large-scale problems.
        -   Multiple visualization tools (ggplots) are included for interpreting and communicating feature contributions
    -   [{]{style="color: #990000"}[shapr](https://norskregnesentral.github.io/shapr/){style="color: #990000"}[}]{style="color: #990000"}, [{{]{style="color: goldenrod"}[shaprpy](https://norskregnesentral.github.io/shapr/shaprpy.html){style="color: goldenrod"}[}}]{style="color: goldenrod"} - Implements an enhanced version of the Kernel SHAP method, for approximating Shapley values, with a strong focus on conditional Shapley values.
        -   Accounts for any feature dependence, and thereby produces more accurate estimates of the true Shapley values
        -   Offers a variety of methods for estimating contribution functions, enabling accurate computation of conditional Shapley values across different feature types, dependencies, and distributions
-   Articles
...
-   Interactions
    ...
    -   [Example]{.ribbon-highlight}: Years on hormonal contraceptives (continuous) interacts with STDs (binary)\
        ![](./_resources/Diagnostics,_Model_Agnostic.resources/image.png){.lightbox width="432"}
    -   Interpretation
...
-   Cluster by shap value\
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/image.1.png){.lightbox width="532"}
...
-   Waterfall plots\
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/waterfall.png){.lightbox width="632"}
    ...
    -   Interpretation example\
        ![](./_resources/Diagnostics,_Model_Agnostic.resources/image.2.png){.lightbox width="532"}
-   force plot - Examines influences behind one predicted value that you input to the function.\
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/force_median.png){.lightbox width="632"}
...
-   [Interpretable Machine Learning: A Guide for Making Black Box Models Explainable](https://christophm.github.io/interpretable-ml-book/) (ebook)
...
-   Extreme values
    ...
    -   Check if extreme values of the sample distribution are covered by the alluvial\
        ![](./_resources/Diagnostics,_Model_Agnostic.resources/unnamed-chunk-11-1.png){.lightbox width="332"}
...
-   Example - mlbench::BostonHousing; lstat(outcome var) percentage of lower status of the population; rf parsnip model
    ...
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/alluv1-1.png){.lightbox width="532"}
...
-   Example: mtcars; disp (outcome var)
    ...
    ![](./_resources/Diagnostics,_Model_Agnostic.resources/README-unnamed-chunk-5-1.png){.lightbox width="532"}
