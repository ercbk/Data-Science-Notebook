# Documents {#sec-quarto-docs .unnumbered}

## Misc {#sec-quarto-docs-misc .unnumbered}
...
- Packages
  ...
  - [{]{style="color: goldenrod"}[brand_yml](https://posit-dev.github.io/brand-yml/){style="color: goldenrod"}[}]{style="color: goldenrod"} - Create reports, apps, dashboards, plots and more that match your company’s brand guidelines with a single `_brand.yml` file
    - For Quarto dashboards, websites, reports, etc.
      - See [Brand Extensions Docs](https://prerelease.quarto.org/docs/extensions/brand.html)
    - For Python shiny, seaborn, matplotlib, etc.
    - For R shiny, ggplot2, plot, RMarkdown
  - [{]{style="color: #990000"}[froggeR](https://github.com/kyleGrealis/froggeR){style="color: #990000"}[}]{style="color: #990000"} - Reusable templates and project structures
  ...
  - [{]{style="color: #990000"}[cocoon](https://jeffreyrstevens.github.io/cocoon/){style="color: #990000"}[}]{style="color: #990000"} - Extract, Format, and Print Statistical Output into inline Rmd/qmd documents
  - [{]{style="color: #990000"}[r2resize](https://r2resize.obi.obianom.com/index.html){style="color: #990000"}[}]{style="color: #990000"} - Elements (pop-ups, cards, images, etc.) with various interactive resizing features
...
- Tools
  ...
  - [Panache](https://panache.bz/) - A language server, formatter, and linter for Pandoc, Quarto, and R Markdown.
    - Built for fast, reliable formatting and robust parsing across syntax that many generic Markdown tools struggle with, including fenced divs, grid tables, definition lists, citations, and inline footnotes.
...
- For large documents (modularity, avoid caching) ([source](https://bsky.app/profile/rmkubinec.bsky.social/post/3lwh6g2fmfc2v))

  1.  Use R scripts that do data processing and modeling
  2.  From those scripts, save plots and other outputs as RDS files
  3.  In document, load RDS objects and make modifications.

- Using yaml style for chunk options\
  ![](./_resources/Quarto-Documents.resources/FAiWv2DXMAEwZJH%20(2).jpg){.lightbox width="541"}
...
## Syntax {#sec-quarto-docs-syntax .unnumbered}
...
## Functions {#sec-quarto-docs-fun .unnumbered}
...
- Converting Quarto to R
  ...
  - It preserves cells options, commenting cells with [eval: false]{.arg-text}, and ignoring content having [purl: false]{.arg-text}

  - For documents using advanced knitr features like `child=` chunks or `knitr::read_chunk()`, `knitr::purl()` remains the recommended approach as it handles these through actual document processing.

- Converting R to Quarto
  ...
  - For further details, see [vignette](https://quarto-dev.github.io/quarto-r/articles/r-scripts.html)
...
## YAML {#sec-quarto-docs-yaml .unnumbered}

- Set global chunk options in yaml\
  ![](./_resources/Quarto-Documents.resources/image.png){.lightbox width="283"}
...
- YAML Examples
  ...
  - Example\
    ![](./_resources/Quarto-Documents.resources/image.1.png){.lightbox width="334"}

- For Standalone HTML Documents with htmlwidgets, make sure to set [embed-resources: true]{.arg-text}
...
- Switch for light/dark mode ([source](https://mine-cetinkaya-rundel.github.io/quarto-tip-a-day/posts/17-dark-mode/))
  ...
  - Places a button at the top of the document that enables the reader to select between the two modes

  - See source for details on customization

  - See [Theme Helpers](https://quarto-dev.github.io/quarto-r/articles/theme-helpers.html) for setting themes for both light/dark in tables, plots, and {brand.yml}

## Chunk Options {#sec-quarto-docs-chunk .unnumbered}
...
## Knitr Hooks {#sec-quarto-docs-hook .unnumbered}

- [Docs](https://yihui.org/knitr/hooks/)
...
- [Chunk Hooks]{.underline}
  ...
  - [Example]{.ribbon-highlight}: Chunk Timer
    ...
    - Output\
      ![](_resources/Quarto-Documents.resources/chunk-hook-timer-out-1.png){.lightbox width="432"}
- [Option Hooks]{.underline}
  - Runs before code in code chunk runs

  - [Example]{.ribbon-highlight}: Figure String Interpolation ([source](https://mastodon.social/@eliocamp/115624313746671987))
...
## Multiple Programming Languages {#sec-quarto-docs-rpy .unnumbered}
...
- Using `?` operator to use a variable from a R chunk in a SQL chunk ([source](https://raw.githubusercontent.com/tomsing1/blog/refs/heads/main/posts/ducklake/index.qmd), [article](https://tomsing1.github.io/blog/posts/ducklake/#instantiating-a-ducklake), [docs](https://bookdown.org/yihui/rmarkdown/language-engines.html#sql))
  ...
  - [con]{.var-text} is the db connection variable
...
- Use embed shortcode to add more than one programming language engine to a document ([source](https://remlapmot.github.io/post/2025/multi-engine-quarto/))
...
## Layouts {#sec-quarto-docs-lay .unnumbered}

- 2 cols (1 col: text, 1 col: image)\
  ![](_resources/Quarto-Documents.resources/quarto-2col-text-img.png){.lightbox width="554"}
...
- Nested Tabs\
  ![](./_resources/Quarto-Documents.resources/image.3.png){.lightbox width="511"}\
  ![](./_resources/Quarto-Documents.resources/image.2.png){.lightbox width="391"}

- Callout Blocks

  - Resources

    - [Docs](https://quarto.org/docs/authoring/callouts.html). [SCSS variables](https://quarto.org/docs/output-formats/html-themes.html#callouts)
...
- Landscape Mode
  ...
  - [Docs](https://quarto.org/docs/authoring/article-layout.html#landscape-mode)
...
## Automation {#sec-quarto-docs-auto .unnumbered}

- Misc

  - Using External R FIles ([source](https://nrennie.rbind.io/blog/parameterized-plots-reports-r-quarto/))
    ...
    - Chunk Option
      ...
      - The file path should be relative to where your Quarto document is, and you need to make sure you set [eval: true]{.arg-text} either in the code block, or document-wide.
  ...
  - Use dataframes as parameter inputs
    ...
    - Using serialization
      ...
      - [Explanation](https://forum.posit.co/t/param-converted-from-data-frame-to-list/155556/9), `toJSON` [workaround](https://www.jhelvy.com/blog/2023-02-28-parameterized-pdfs-with-quarto/#passing-data-frames-as-parameters)
  ...
  - Print out the code of a function from a file ([source](https://www.tjmahr.com/unenv/#forwarding-a-function-definition-into-a-knitr-code-chunk))\
    ![](_resources/Quarto-Documents.resources/auto-print-code-1.jpg){.lightbox width="432"}

    ```` markdown
    ```{{r setup, include=FALSE}}
    knitr::opts_chunk$set(echo = TRUE)

    inline_function_df <- function(f) {
      lines <- attr(f, "srcref")[c(1,3])],
      source <- attr(attr(f, "srcref"), "srcfile")
      source$lines[seq(lines[1], lines[2])]
    }
    ...
    ```{{r, code = inline_function_def(fit_main_model)}}
    ```
    ````
  ...
  - Inspect document before rendering
  ...
  - Pass database credentials/password via user prompt ([source](https://bsky.app/profile/rmkubinec.bsky.social/post/3mb5gmnno7s2g))

    ``` r
    quarto::quarto_render(
      input = "results/initial-results.qmd",
      execute_params = list(db_password = rstudioapi::askForPassword())
    )
    ```

- Include shortcode to copy/paste the content of another quarto document ([Docs](https://quarto.org/docs/authoring/includes.html), [Article](https://posit.co/blog/quarto-meta-shortcode-variables/))
  ...
  - [Example]{.ribbon-highlight}: Text with parameters
    ...
    - Source Document (`_snippets/facts.qmd`)
      ...
      - [state]{.var-text}, [square-miles]{.var-text}, [size-rank]{.var-text}, and [population]{.var-text} are parameters whose values will be set in the the main document.
  ...
  - Using External Quarto Files ([Docs](https://quarto.org/docs/authoring/includes.html)):
...
- Iteration and Parameterization
  ...
  - Misc
    ...
    - Notes from
      ...
      - [Rennie FAQ](https://nrennie.rbind.io/blog/parameterized-plots-reports-r-quarto/)
    ...
    - Might need to use `fs::file_move` to move processed reports to another folder.
      ...
      - [Helper function](https://github.com/jhelvy/jph/blob/master/R/quarto_render_move.R) that moves rendered files with base R file maintenance functions
    ...
    - **Plot Bug During Iteration** ([source](https://nrennie.rbind.io/blog/parameterized-plots-reports-r-quarto/#multiple-reports), [github issue](https://github.com/quarto-dev/quarto-r/issues/205))
...
## WebR {#sec-quarto-docs-webr .unnumbered}
...
- [arrow](https://emilhvitfeldt.github.io/quarto-arrows/) - Add arrows to documents, slides, typst, etc. (curvature, color, head symbol, etc. set interactively)
- [bluesky-comments](https://github.com/quarto-ext/bluesky-comments) - When someone visits your \[Quarto\] web page, any replies to the linked post are pulled from Bluesky and shown directly in the page
- [closeread](https://closeread.netlify.app/) ([video](https://www.youtube.com/watch?v=KqLxy66B3lQ&ab_channel=PositPBC)) - Scrollytelling
  - Rennie example ([code](https://github.com/nrennie/scrollytelling), [source](https://fosstodon.org/@nrennie/113913420516653587))

    ::: {layout-ncol="2"}
    ![](_resources/Quarto-Documents.resources/ext-clre-rennie-ex-1.png){.lightbox group="clre-renn-1" width="332"}

    ![](_resources/Quarto-Documents.resources/ext-clre-rennie-ex-2.png){.lightbox group="clre-renn-1" width="332"}
    :::

    - Clean look with text and visuals separated
    - Shows how scrolling can zoom into different areas of a chart
- [custom-callout](https://quarto.thecoatlessprofessor.com/custom-callout/) - Allows you to customize callouts through yaml settings instead of inside the callout code or through css/scss.
...
- [diagram](https://emilhvitfeldt.github.io/quarto-diagrams/) - Adds a collection of diagram styles to your documents and Reveal.js presentations.
- [marimo](https://marimo-team.github.io/quarto-marimo/) - Full-featured notebook. Similar to Jupyter but has reactive cells — changing the value of variable in a cell will change it in other cells. I think it might be like an Observable notebook for Python
...
- [svelte](https://quarto-svelte.jamesgoldie.dev/) - Lets you seamlessly write and include [Svelte](https://svelte.dev/) components, like charts and other visuals, in your Quarto website and documents.
  - It's like adding shiny inputs and charts with just some js code.
- [Quarto Theme Tweaker](https://github.com/EmilHvitfeldt/quarto-theme-tweaker) - A Positron/VS Code extension for live Sass theming of Quarto documents.
  - Sliders and color pickers bound to Sass variables let you tweak a rendered Quarto document and see the changes instantly, then write the values back into custom.scss.
- [toggle](https://quarto.thecoatlessprofessor.com/toggle/) - Adds a button to your *code blocks* that shows or hides the output on demand.

## Slides {#sec-quarto-docs-slid .unnumbered}

- Packages
  - [{]{style="color: #990000"}[deckroadmap](https://codingtigertang.github.io/deckroadmap/){style="color: #990000"}[}]{style="color: #990000"} - Adds PowerPoint-style roadmap footers to Quarto and R Markdown Reveal.js slides
- Resources
  - [Slidecrafting](https://slidecrafting-book.com/)
- If you want to use inline code in a reveal-js without any other R code chunks, make sure to specify [engine: knitr]{.arg-text} in your YAML ([source](https://bsky.app/profile/collinberke.bsky.social/post/3lo2qyakuc22b))
