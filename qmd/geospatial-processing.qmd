...
### Aggregations {#sec-geo-proc-sf-agg .unnumbered}
...
-   [Example]{.ribbon-highlight}: Aggregating into Quadrants ([source](https://www.redhat.com/en/ansible-collaborative))\
    ...
    -   sf has an internal `aggregate` function that unions all the counties into the quadrants and aggregates the variable accordingly.
        -   It takes apart the geometry variable and builds it up again to get `st_union` to produce the correct result. It's essentially a few lines of nested base r code, but it's a bit nasty.
        -   It also uses [by_feature = TRUE]{.arg-text} which is not the default and I'm not currently sure what that does. I think it causes `st_union` to act different in the `aggregate,` because the object was coerced into a sfc class
        -   A simple `group_by` + `summarize(geom = st_union(geom))` (or using `mutate`, `group_split`, `group_map`, etc.) will NOT produce a matching result. (although it's close I think)
