...
-   [Example 3]{.ribbon-highlight}: Aggregate Using Weighted Median and Mean ([SDS Ch.16](https://r-spatial.org/book/16-SpatialRegression.html#boston-house-value-dataset))
    ...
    A lot of the models that use [Spatially Structured Random Effects](regression-spatial.qmd#sec-reg-spat-me-ssre){style="color: green"} need some extra manipulation of [{spdep}]{style="color: #990000"}'s neighbor list to get a spatial weights object into an acceptable format. Adding rownames to neighbor list is usually part of that operation.
    ...
    -   `card` gets the neighbor vectors for each [NOX_ID]{.var-text}
    :::
