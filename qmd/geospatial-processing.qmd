...
- Packages
  - [{]{style="color: #990000"}[apache.sedona](https://github.com/apache/sedona/tree/master/R){style="color: #990000"}[}]{style="color: #990000"} - Sparklyr extension for processing geospatial data
    - [Example]{.ribbon-highlight}: Larger-than-memory spatial/KNN join
      ...
      url = "https://github.com/geoarrow/geoarrow-data/releases/download/v0.2.0/microsof"
      ...
      - Requires setting a memory limit

      </Details>
  - [{]{style="color: #990000"}[chopin](https://docs.ropensci.org/chopin/){style="color: #990000"}[}]{style="color: #990000"} - Automates parallelization in spatial operations with chopin functions as well as sf/terra functions.
    - Raster resolution: Suggests 100 meters as a threshold. Rasters with resolution *coarser than 100 meters* and a few layers would be better for the direct call of `exactextractr::exact_extract`.
    - Set `sf::sf_use_s2(FALSE)` - Works best with two-dimensional (planar) geometries. Running any chopin functions at spherical or three-dimensional (e.g., including M/Z dimensions) geometries may produce incorrect or unexpected results.
  - [{]{style="color: #990000"}[geographiclib](https://hypertidy.github.io/geographiclib/){style="color: #990000"}[}]{style="color: #990000"} - R interface to GeographicLib for precise geodetic calculations on the WGS84 ellipsoid
    - Geodesic computations (distance, bearing, paths, intersections)
    - Map projections (UTM/UPS, Transverse Mercator, Lambert Conformal Conic, and more)
    - Grid reference systems (MGRS, Geohash, GARS, Georef)
    - Coordinate conversions (geocentric, local Cartesian)
    - Polygon area on the WGS84 ellipsoid
...
- [PMTiles](https://github.com/protomaps/PMTiles) - A single-file archive format for tiled data. A PMTiles archive can be hosted on a commodity storage platform such as S3, and enables low-cost, zero-maintenance map applications that are "serverless" - free of a custom tile backend or third party provider. ([Docs](https://docs.protomaps.com/))
  ...
  - Packages
    - [{]{style="color: #990000"}[freestiler](https://walker-data.com/freestiler/){style="color: #990000"}[}]{style="color: #990000"} - A tiling engine that is written in Rust and optimized for speed and performance.
      - Generate tiles directly from R sf objects and Python GeoDataFrames.
      - DuckDB integration supports local DuckDB databases, GeoParquet files, and even shapefiles
      - Default output is MapLibre Tiles (MLT) (packaged as PMTiles) which is a next-generation tile format that optimizes for performance and data compression
    - [{]{style="color: #990000"}[pmtiles](https://github.com/walkerke/pmtiles){style="color: #990000"}[}]{style="color: #990000"}
  ...
  - Resources
    - [Example](https://gist.github.com/walkerke/cf87df489be8065635cf9b0c0dee34f5) loading pmtiles and using w/[{mapgl}]{style="color: #990000"}
    - [Complete project](https://github.com/vehicletrends/hhi-map) using vehicle trends data.
      - [Creates PMTiles](https://github.com/vehicletrends/hhi-map/blob/main/prep-data.R) using [{pmtiles}]{style="color: #990000"} from parquet data
      - [Directions](https://github.com/vehicletrends/hhi-map/blob/main/upload.md) for uploading pmtiles to cloudflare r2
...
- Geopackage
  ...
  - Check for available layers, Read, and Write

    ``` r
    st_layers("path/to/nc.gpkg")
    # Driver: GPKG 
    # Available layers:
    #   layer_name geometry_type features fields crs_name
    # 1    nc.gpkg Multi Polygon      100     14    NAD27

    nc <- st_read("path/to/nc.gpkg")
    ...
    - [append]{.arg-text}: Allows `st_write` to either append records to an existing layer or replace it
...
- Misc
  ...
  - If your computation takes in the 30 sec to a few minutes range, then use duckdb and its spatial extensions (See [Databases, DuckDB \>\> Geospatial](db-duckdb.qmd#sec-db-duckdb-geo){style="color: green"}, [Apache \>\> Arrow \>\> Geospatial](apache-arrow.qmd#sec-apache-arrow-geo){style="color: green"})
...
- Packages
  - [{]{style="color: #990000"}[crstools](https://cran.r-project.org/web/packages/crstools/index.html){style="color: #990000"}[}]{style="color: #990000"} - Choose the appropriate map projection for a given application, visualise the resulting distortion, and georeference data from unknown projections
  - [{]{style="color: #990000"}[parzer](https://docs.ropensci.org/parzer/){style="color: #990000"}[}]{style="color: #990000"} - Parses messy geographic coordinates
    - [Example]{.ribbon-highlight}:

      ``` r
      data("wind", package = "gstat")
      as.character(wind.loc[["Latitude"]][[1]])
      #> [1] "51d56'N"

      as.numeric(sp::char2dms(as.character(
        wind.loc[["Latitude"]][[1]]
      )))
      #> [1] 51.93333

      parzer::parse_lat(as.character(
        wind.loc[["Latitude"]][[1]]
      ))
      #> [1] 51.93333
      ```
...
- Valid longitude should be between -180 and 180 (or 0-360 for some systems)
- Valid latitude should be between -90 and 90.
- Formats
...
- The United Nations General Assembly adopted a new world map that reflects countries’ true relative sizes
  - Example:
...
- AKA geodetic coordinates, ellipsoidal
...
- **X → Easting** (distance east from the zone’s central meridian, usually in meters)

- **Y → Northing** (distance north from the equator, usually in meters)
...
- Europe:
  ...
  - Pan-Europe, European Union
    - [EPSG:3035](https://epsg.io/3035) - ETRS89-LAEA
  - Central Europe (between 6°E and 12°E) (Think Western Nordic, Denmark, Germany, Northern Italy)
    - [EPSG:25832](https://epsg.io/25832) - Uses datum ETRS89 and UTM zone 32N projection
      - Preferred standard for European mapping applications.
    - EPSG:32632 - Uses datum WGS84 and UTM zone 32N projection
...
- [Docs](https://r-spatial.github.io/sf/)
...
- sf vs sfc
  ...
  - sfc is a geometry column (just spatial data)
  - sf is a data frame + geometry (full spatial dataset)
...
- Create sf objects
  ...
  - With point locations ([source](https://r-spatial.org/book/07-Introsf.html#creation))\
    ![](_resources/Geospatial,_Processing.resources/sf-obj-components.png){.lightbox width="532"}
...
### Functions {#sec-geo-proc-sf-funs .unnumbered}

- [tidyverse functions for sf objects](https://r-spatial.github.io/sf/reference/tidyverse.html)

  - `summarize` has 2 extra arguments
    - [do_union]{.arg-text} (default [TRUE]{.arg-text}) determines whether grouped geometries are unioned on return, so that they form a valid geometry
    - [is_coverage]{.arg-text} (default [FALSE]{.arg-text}) in case the geometries grouped form a coverage (do not have overlaps), setting this to `TRUE` speeds up the unioning

- `st_agr` - Gets or sets the “attribute-geometry relationship” (AGR) — that is, how each non-geometry attribute (column) relates to its geometry column

  - Certain geometric operations — like `st_union`, `st_intersection`, or `st_join` — may need to know how to handle attributes when geometries are modified or combined.

  - Possible Values

    | AGR value | Meaning | Example use |
    |------------------------|------------------------|------------------------|
    | ["constant"]{.arg-text} | The attribute is constant within the geometry. | A country’s name, ISO code, land use, soil type (it doesn’t change across the polygon). |
    | ["aggregate"]{.arg-text} | The attribute is *summed* or *aggregated* when geometries are combined. | Population, GDP, or area — if you merge polygons, the attribute should be added. |
    | ["identity"]{.arg-text} | Identifier | An ID or index variable (e.g., grid cell ID). |

  - Examples

    - Single Attribute AGR

      ``` r
      # find cells that cross border or don't
      is_cross_brdr <- 
        st_intersects(grid_de["ID"], 
                      st_cast(st_union(shp_de), "MULTILINESTRING"),
                      as_points = FALSE) 
      # subset only cells with 1 (i.e. cross border)
      sf_grid_de <- 
      ...
      - From [Geospatial, Modeling \>\> Interpolation \>\> Kriging \>\> Examples](geospatial-modeling.qmd#sec-geo-gmod-interp-krig-exs){style="color: green"} \>\> Example 3
      - There's only 1 *attribute* in this sf object, and it's a cell ID for a grid. Later `st_intersection` is used, so the AGR gets set to "identity."

    - Set AGRs for multiple attributes

      ``` r
      library(sf)

      # Simple example with two attributes
      nc <- st_read(system.file("shape/nc.shp", package="sf"), quiet = TRUE)
      nc_small <- nc[1:3, c("NAME", "AREA", "geometry")]
      ...
      st_agr(nc_small) <- c(NAME = "constant", AREA = "aggregate")
      st_agr(nc_small)
      #>      NAME      AREA 
      #>  constant aggregate 
      #> Levels: constant aggregate identity
...
- [Distances]{.underline}
  ...
  - `st_crs(sf_obj)$units` tells what unit of measurement is that coordinate system
    - You are not required to designate units in functions such as `st_buffer`, because sf will assume the units based on the Coordinate Reference System (CRS).
...
- [Example]{.ribbon-hilghlight}: Find counties within 50km from Orange County ([source](https://r-spatial.org/book/07-Introsf.html#tidyverse))\
  ![](_resources/Geospatial,_Processing.resources/sf-meas-ex1-orangebuffer-1.png){.lightbox width="532"}
...
- [Buffer]{.underline}\
  ...
  - [Example 1]{.ribbon-highlight}: Point Buffers ([source](https://bookdown.org/michael_bcalles/gis-crash-course-in-r/toolkit.html#buffers))\
  ...
  - [Example 2]{.ribbon-highlight}: Negative Polygon Buffer ([source](https://spencerschien.info/post/spatial_buffer/))\
    ![](_resources/Geospatial,_Processing.resources/sf-buffer-ex2-inverse.png){.lightbox width="224"}
    ...
    - The plot looks like the border is blue and the polygon is yellow, but the blue actually shows the original polygon (i.e., Milwaukee city limits), and **the yellow shows a negative buffer**, or a buffer inside the polygon.

  - [Example 3]{.ribbon-highlight}: Using a Single Side Buffer for Partitioning ([source](https://spencerschien.info/post/spatial_buffer/))

    ::: panel-tabset
    ## Data

    The locations are polling stations in Milwaukee along with interstate I43.

    The goal is to partition the Milwaukee polygon into roughly the east and west sides of I43

    ![](_resources/Geospatial,_Processing.resources/sf-trans-buf-ex3-data.png){.lightbox width="204"}
    ...
    mke |> 
      ggplot() +
      geom_sf() +
      geom_sf(data = mke_interstates) +
      geom_sf(data = polls, size = .75, color = "red") +
      theme_void()
    ```

    - The black lines are the interstates that run through Milwaukee and the red dots are polling stations.

    ## Partition using single side buffer

    ![](_resources/Geospatial,_Processing.resources/sf-trans-buf-ex3-intersected.png){.lightbox width="332"}
    ...
    - [singleSide = TRUE]{.arg-text} will be limit the buffer to one side of the line. Negative distance values will buffer on one side, positive on the other.
    - Here the -20K meters produces a buffer on the right side of I43. When that's intersected with the Milwaukee polygon, we get the space of Milwaukee that east of I43.

    ## Visualize partitioned locations

    ![](_resources/Geospatial,_Processing.resources/sf-trans-buf-ex3-partitioned.png){.lightbox width="341"}
    ...
    - After joining southeast milwaukee partition to the polling data, an indicator variable can be created.
    :::
...
- [Intersection]{.underline}

  ![st_intersection](_resources/Geospatial,_Processing.resources/sf-intersect-1.png){.lightbox width="482"}

  - `st_intersect` - A binary predicate (i.e. logical) that tests whether a geometry/cell in a grid/etc. intersects another geometry/cell in a grid/etc. Outputs 1/0.

  - `st_intersection` - A geometric processing step that actually computes the intersection shapes.

  - [Example]{.ribbon-highlight}: `st_intersection` ([source](https://bookdown.org/michael_bcalles/gis-crash-course-in-r/toolkit.html#intersection))\
  ...
  - [Example]{.ribbon-highlight}: `st_intersects` and `st_intersection` (From [Geospatial, Modeling \>\> Interpolation \>\> Kriging \>\> Examples](geospatial-modeling.qmd#sec-geo-gmod-interp-krig-exs){style="color: green"} \>\> Example 3)![](_resources/Geospatial,_Processing.resources/sf-trans-inter-ex2-1.png){.lightbox width="332"}
    ...
    grid_cross_brdr <- st_intersection(sf_grid_de, st_union(shp_de)) # <3>
    plot(grid_cross_brdr)
    ```

    1.  `st_intersect` finds grid cell IDs that intersect with country border (`st_union(shp_de)`)

    2.  Subset only cells that cross the border and creates a sf object for them

    3.  `st_intersection` takes those border cells and the polygons from the shape file and computes the intesection polygons (I think the colors are possibly referring to the cell IDs?)

### Joins {#sec-geo-proc-sf-join .unnumbered}

- Also see

  - [Aggregations](geospatial-processing.qmd#sec-geo-proc-agg){style="color: green"} \>\> Example 5 \>\> Area Weighted Aggregation (`st_equals`)
  - [Geospatial, Analysis \>\> Descriptive](geospatial-analysis.qmd#sec-geo-anal-desc){style="color: green"} \>\> Average Elevation for Areas (`st_intersects`)

- Packages

  - [{]{style="color: #990000"}[fjoin](https://cran.r-project.org/web/packages/fjoin/index.html){style="color: #990000"}[}]{style="color: #990000"} - [{data.table}]{style="color: #990000"} extension that supports sf/sfc object joins (retains attributes)

- A spatial join is similar to a starndard dataframe join, but the criterion to join records is not equality of attributes but a spatial predicate. (e.g. st_intersects, st_equals, etc)

  - `st_intersects` is the default

- The concepts of “left”, “right”, “inner”, or “full” joins remain identical to the non-spatial join as the options for handling records that have no spatial match

- [Example]{.ribbon-highlight}: [largest = TRUE]{.arg-text} ([source](https://r-spatial.org/book/07-Introsf.html#spatial-joins))\
  ![](_resources/Geospatial,_Processing.resources/sf-join-largest-1.png){.lightbox width="632"}
  ...
  gr <- st_sf(
    label = apply(
      expand.grid(1:10, LETTERS[10:1])[,2:1],
      1, 
      paste0, 
      collapse = ""
    ),
  ...
  - The colored grid gets joined to the north carolina polygons and since [largest = TRUE]{.arg-text}, each grid cell gets matched to the polygon (aka record) with the largest overlap

### Bounding Boxes {#sec-geo-proc-sf-bbox .unnumbered}
...
- [Example]{.ribbon-highlight}: Basic by polygon, by grid cell
...
- [Example 1]{.ribbon-highlight}: Aggregate extensive variable into quadrants ([source](https://www.redhat.com/en/ansible-collaborative))\
  ...
  - Counties are grouped by their position (according to the quadrant in which the county centroid is with respect to ellipsoidal coordinate POINT(-79, 35.5)
    - `aggregate`'s [by]{.arg-text} argument can also take a shape file object
...
- [Example 5]{.ribbon-highlight}: From points of 100m cells to 10km cells

  - This data and aggregation are part of [Geospatial, Modeling \>\> Interpolation \>\> Kriging \>\> Examples](geospatial-modeling.qmd#sec-geo-gmod-interp-krig-exs){style="color: green"} \>\> Example 3
  - Also includes the population density calculation
  ...
  - The population data file consists of the value and point geometries (even though the data is for a 100m grid cells)
  - The goal is to create a population density variable, so the hard part is calculating a reasonably accurate area from square grid cells (i.e. the overlapping parts of the border cells need to be cut off)
    - Aggregation to the 10km cell grid is relatively straightforward. No 100m cell grid need be created. The point geometries are evidently enough.
  - Aggregation and area-correction are performed by 2 different methods: manually and area-weighted aggregation. Two different area-weighted aggregation workflows are demonstrated: via raster and via polygon.
  - tl;dr Although the area-weighted raster method is pretty fast, the manual way is probably fastest (didn't time it). While the area-weighted method is more accurate and less complicated code-wise, the manual way is probably accurate enough for most use cases.
  ...
  # germany shape file with polygons
  shp_de <- 
    read_sf("https://raw.githubusercontent.com/edzer/sdsr/main/data/de_nuts1.gpkg") |> 
    st_transform(crs)
  ...
  # download population data for 100m grid cells. Comes with point geometry
  sf_pop <- local({ 
    load(url("https://raw.githubusercontent.com/edzer/sdsr/main/data/ch12.rda"), 
         envir = environment()) 
    get(ls()[2]) 
  ...
  - Loads NO~2~ data, Germany shape file, and population data for 100m grid cells.
  - A 10km x 10km grid is created from the shape file

  ## Manually with Mixed Polygons

  A normal aggregation procedure is performed. Then, border cells have their overlapping parts lopped off. Then, the corrected area is calculated.

  ###### Aggregate from 100m to 10km cells
  ...
  - The population data is aggregated to the 10km cell grid.

  ###### Fix Overlapping Border Cells

  ``` r
  # create column-wise, cell ID matrix
  ...
  is_cross_brdr <- 
    st_intersects(grid_de["ID"], 
                  st_cast(st_union(shp_de), "MULTILINESTRING"),
                  as_points = FALSE) 
  # subset border cells from grid
  sf_grid_de <- 
    st_as_sf(grid_de["ID"], na.rm = FALSE)[lengths(is_cross_brdr) > 0,]
  st_agr(sf_grid_de) = "identity"
  # create intersection polygons from border cells
  grid_cross_brdr <- st_intersection(sf_grid_de, st_union(shp_de))
  ```

  - A grid of square cells will overlap the German border. So to get an accurate area, all cell areas must be within the border. Therefore, we need to chop of the parts of border cells that cross the border.
  - `st_intersect` is a binary predictate where a 1 indicates a grid cell that crosses the German border and 0 indicates the cell does *not* cross the border
    - `st_union` creates a shape (linestring) that's just the German country border (no provinces)
  - agr needs set for `st_intersection` (See [Geospatial, Preprocessing \>\> sf \>\> Functions](geospatial-processing.qmd#sec-geo-proc-sf-funs){style="color: green"} \>\> `st_agr`)
  - `st_intersection` creates intersection polygons of the German border grid cells (See [Geospatial, Preprocessing \>\> sf \>\> Transformations](geospatial-processing.qmd#sec-geo-proc-sf-trans){style="color: green"} \>\> Intersection \>\> Example 2) (i.e. parts of cells outside the German border get chopped off)

  ###### Calculate Density

  ![](_resources/Geospatial-Modeling.resources/krig-ex3-dens-man-1.png){.lightbox group="krig-ex3" width="332"}

  ``` r

  # fill (inside-border) area matrix cells with cell area (10km x 10km)
  grid_de$area <- st_area(grid_de)[[1]] + units::set_units(grid_de$values, m^2)
  # fill (border) area matrix cells with intersection cell areas
  grid_de$area[grid_cross_brdr$ID] <- st_area(grid_cross_brdr)

  # calculate population density
  grid_de$pop_dens <- grid_agg_pop$Einwohner / grid_de$area
  # sanity check
  est_pop <- sum(grid_de$pop_dens * grid_de$area, na.rm = TRUE) # verify
  est_pop
  # 80323301  [1]
  true_pop <- sum(sf_pop$Einwohner, na.rm = TRUE)
  true_pop
  # [1] 80324282
  # relative error
  (true_pop - as.numeric(est_pop)) * 100 / true_pop
  # [1] 0.001221299
  ```

  - Inside Border: First the cell area (10km x 10km) is calcuated and that value is inserted where values are 0 (cells inside the German border) and not [NA]{.arg-text} (outside border)
    - `st_area` fills a 65 x 87 matrix with the cell area value
    - `set_units` sets the m^2^ units attribute to the matrix in [grid_de]{.var-text} so it can be added to the area matrix.
    - So now the addition of these two terms results in NA + area = NA; 0 + area = area
  - Border: Calculates the intersection border cell areas and assigns those area values to border cell ids in `grid_de$area`
  - This method produces a relative error of around 0.001% which should be good enough for most work.
  ...
  - Redistributes the population from the source polygons (100m cells) to the target polygons (10km cells) based on the proportion of overlap. It takes care of the overlapping border problem and aggregation all at once.
  - Assumes that the population within a source polygon is uniformly distributed and calculates the geometric intersection between source and target. Then, sums up the proportional values.
  - Process
    - *Calculates precise coverage fractions*: For each 100m population raster cell, determine exactly what fraction of that cell falls within each 10km grid polygon. This is done using computational geometry rather than simple point-in-polygon tests.
    - *Weights values by coverage*: Each raster cell's value (population count) is multiplied by its coverage fraction within the polygon.
    - *Sums the weighted values*: All weighted values are summed to get the total for each 10km grid cell.
  - This method produces what's effectively 100% accuracy, although it's much more computationally intensive
    - The tiny difference (3 people) is likely due to floating-point precision or edge effects at the outer boundary.

  ###### [{exactextractr}]{style="color: #990000"} (Raster)

  ![](_resources/Geospatial-Modeling.resources/krig-ex3-dens-aw-1.png){.lightbox group="krig-ex3" width="332"}

  ``` r
  library(exactextractr); library(terra)

  # Convert to sf polygons
  grid_sf <- st_as_sf(grid_de, na.rm = FALSE)
  grid_sf$ID <- seq_len(nrow(grid_sf))

  grid_sf_pop <- sf_pop |>
    st_buffer(dist = 50, endCapStyle = "SQUARE", nQuadSegs = 1)

  # derive bounding box from grid_sf_pop
  bb <- st_bbox(grid_sf_pop)

  # Create 100m raster aligned to pop cells
  r100 <- 
    rast(
      xmin = bb["xmin"], xmax = bb["xmax"],
      ymin = bb["ymin"], ymax = bb["ymax"],
      resolution = 100,
      crs = st_crs(grid_sf_pop)$wkt
  )
  ...
  # should match exactly (aside from floating-point rounding)
  global(pop_r, "sum", na.rm = TRUE)[1]
  #>                sum
  #> Einwohner 80324282
  sum(grid_sf_pop$Einwohner)
  #> [1] 80324282

  grid_sf$Einwohner <- exact_extract(pop_r, grid_sf, "sum")

  sum(grid_sf$Einwohner, na.rm = TRUE)
  #> [1] 80324279
  sum(grid_sf_pop$Einwohner, na.rm = TRUE)
  #> [1] 80324282

  # area in km²
  grid_sf$area_km2 <- units::set_units(st_area(grid_sf), "km^2") |> units::drop_units()
  grid_sf$pop_dens <- grid_sf$Einwohner / grid_sf$area_km2
  ```

  - This raster method took 138.14 sec (almost entirely by `rasterize`) from `bbox` to `exact_extract`
  ...
  ``` r
  # Assign the calculated density back to the original stars object
  # (This works because grid_sf was created from grid_de with na.rm=FALSE, so the cell order matches)
  grid_de$pop_dens <- grid_sf$pop_dens
  grid_masked <- grid_de[shp_de]

  # Plot the stars object directly
  ggplot() +
    geom_stars(data = grid_masked, 
               aes(fill = pop_dens)) + 
    geom_sf(data = shp_de, 
            fill = NA, 
            color = "black", 
    ...
    labs(
      title = "Population Density (10 km Grid)",
      subtitle = "Area-weighted interpolation from 100 m population data"
    )
  ```

  </Details>

  ###### [{chopin}]{style="color: #990000"} (Polygon)

  ![](_resources/Geospatial-Modeling.resources/krig-ex3-dens-aw-2.png){.lightbox group="krig-ex3" width="332"}
  ...
  # 100m cell grid fpr population data
  grid_sf_pop <- sf_pop |>
    st_buffer(dist = 50, endCapStyle = "SQUARE", nQuadSegs = 1)
  ...
  aw <- summarize_aw(
    x = grid_sf,
    y = grid_sf_pop,
    target_fields = "Einwohner",
    id_x = "ID",
    fun = "sum"
  )

  # sanity check
  sum(aw$Einwohner, na.rm = TRUE)
  # [1] 80324279
  sum(sf_pop$Einwohner,   na.rm = TRUE)
  # [1] 80324282

  # calculate population density per km^2
  aw$area <- st_area(aw)
  aw$pop_density <- aw$Einwohner / (as.numeric(aw$area) / 1e6)
  ```

  - This polygon method took 505.92 sec which was just the `summarize_aw` function
  - Could also use `st_interpolate_aw` from [{sf}]{style="color: #990000"}, but I had trouble with it. This `summarize_aw` from [{chopin}]{style="color: #990000"} worked for me more easily.
  ...
  ``` r
  # Since grid_sf was created from grid_de with the same cell order
  # Just directly assign where we have matches
  grid_sf$pop_dens <- NA_real_

  # Use spatial join to match aw back to grid_sf
  matched <- st_join(grid_sf, aw, join = st_equals)
  grid_sf$pop_dens <- matched$pop_density

  # Now assign to stars and mask so only Germany shows
  grid_de$pop_dens <- grid_sf$pop_dens
  grid_masked <- grid_de[shp_de]
...
- [Example 6]{.ribbon-highlight}: Mean across regions per day; [{stars}]{style="color: #990000"} ([source](https://r-spatial.org/book/07-Introsf.html#example-aggregating-air-quality-time-series))

  ::: {layout-ncol="2"}
  ![](_resources/Geospatial,_Processing.resources/agg-ex6-mean-per-region-day-1.png){.lightbox group="agg-ex6" width="532"}

  ![](_resources/Geospatial,_Processing.resources/agg-ex6-mean-per-region-day-2.png){.lightbox group="agg-ex6" width="532"}
  :::
  ...
  # convert to space x time matrix to stars
  ...
  # aggregate and map
  (star_reg_mean_pm10 <- aggregate(
    star_air, 
    DE_NUTS1, # germany's provincial polygons; loads alongs w/air
    mean, 
    na.rm = TRUE
  ))
  ...
  # brandenburg
  xts::as.xts(star_reg_mean_pm10)[,4] |> 
    plot(main = de_nuts1$NAME_1[4])
  ```

  </Details>

  - For details on the conversion from matrix to [{stars}]{style="color: #990000"} object, see [Geospatial, Spatio-Temporal \>\> Data Formats](geospatial-spat-temp.qmd#sec-geo-sptemp-datform){style="color: green"} \>\> Time-wide \>\> Example 2
  - Since this is a [{stars}]{style="color: #990000"} object and [time]{.var-text} is a dimension along with space ([geom]{.var-text}), it also is a grouping variable. So we get means for polygons per day.
