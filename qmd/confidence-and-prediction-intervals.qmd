...
-   [**Coverage**]{style="color: #009499"} or [**Empirical Coverage**]{style="color: #009499"}: The level of coverage *actually observed* when evaluated on a dataset, typically a holdout dataset not used in training the model. Rarely will your model produce the Expected Coverage exactly
    ...
    -   [**Conditional Coverage**]{style="color: #009499"}: The coverage for each individual class of the outcome variable.
...
-   Packages
    -   [{{]{style="color: goldenrod"}[mapie](https://mapie.readthedocs.io/en/latest/){style="color: goldenrod"}[}}]{style="color: goldenrod"} - Handles scikit-learn, tf, pytorch, etc. with wrappers. Computes conformal PIs for Regression, Classification, and Time Series models.
        ...
        -   Classification
            ...
            -   LAC tends to produce empty prediction sets when the model is uncertain, for example at the border between two classes. The rest of the methods try to resolve this issue with RAPS being the most complex.
-   Notes from
    -   [How to Handle Uncertainty in Forecasts: A deep dive into conformal prediction](https://towardsdatascience.com/how-to-handle-uncertainty-in-forecasts-86817f21bb54)
        -   The conformal score formula used in this article, $s_i = |\;y_i - \hat p_i(y_i\;|\;X_i)\;|$ where $y_i$ is the observed class and $\hat p$ is the predicted probability, has the same results as to the one below, but it's not workable in production since there is no *observed* class.
...
-   [LAC (aka Score Method) Process]{.underline}
    ...
    3.  On the calibration (aka validation) set, compute the conformal scores [only for the observed class]{.underline} (i.e. true label) for each observation\
        $$
        s_{i, j}  = 1 - \hat p_{i,j}(y_i | X_i)
        $$
        -   Variables
            -   $s_{i,j}$: Conformal Score for the i^th^ observation and class $j$
            -   $y_i$: Observed Class
    ...
    4.  Order the conformal scores from highest to lowest
    5.  Calculate the critical value or threshold for a chosen $\alpha$\
    ...
    6.  Predict on the Test set and calculate conformal scores [for each class]{.underline}
    7.  For each test set observation, select classes that have scores below the threshold score as the model prediction.
...
-   [Example]{.ribbon-highlight}: LAC Method, Multinomial
    ...
    -   Threshold is then used to get predicted labels of the test set
        ...
        -   Conformal scores are calculated for each potential class using the predicted probabilities on the test set
...
-   [Example]{.ribbon-highlight}: Multinomial - Threshold per Class
