...
- Packages
  - [{]{style="color: #990000"}[brulee](https://brulee.tidymodels.org/){style="color: #990000"}[}]{style="color: #990000"} - [{tidymodels}]{style="color: #990000"} interface to [{torch}]{style="color: #990000"}
    - Contains several basic modeling functions that use the torch package infrastructure, such as: neural networks, linear regression, logistic regression, and multinomial regression.
  - [{]{style="color: #990000"}[keras](https://keras3.posit.co/){style="color: #990000"}[}]{style="color: #990000"} - R interface to keras
  - [{]{style="color: #990000"}[kindling](https://kindling.joshuamarie.com/){style="color: #990000"}[}]{style="color: #990000"} ([overview](https://statsandr.com/blog/you-can-do-more-for-neural-networks-in-r-with-kindling/), [bayesian](https://statsandr.com/blog/bayesian-neural-networks-in-tidymodels-with-kindling/)) - A higher-level interface built on top of [{torch}]{style="color: #990000"}
    - Multiple architectures available: feedforward networks (MLP/DNN/FFNN) and recurrent variants (RNN, LSTM, GRU)
    - Native support for titanic ML frameworks (currently supports [{tidymodels}]{style="color: #990000"} (and [{mlr3}]{style="color: #990000"} soon) workflows and pipelines
  - [{]{style="color: #990000"}[luz](https://mlverse.github.io/luz/){style="color: #990000"}[}]{style="color: #990000"} - A higher level API for [{torch}]{style="color: #990000"} providing abstractions to allow for much less verbose training loops
  - [{]{style="color: #990000"}[mlr3torch](https://mlr3torch.mlr-org.com/){style="color: #990000"}[}]{style="color: #990000"} - Deep Learning with [{torch}]{style="color: #990000"} and [{mlr3}]{style="color: #990000"}
  - [{]{style="color: #990000"}[torch](https://torch.mlverse.org/docs/){style="color: #990000"}[}]{style="color: #990000"} - R interface to Torch
- Resources
  ...
  - [Hands-On Machine Learning With R, Ch. 13](https://bradleyboehmke.github.io/HOML/deep-learning.html) - Deep learning with [{keras}]{style="color: #990000"}
...
## Basic Feed Forward {#sec-dl-gen-bff .unnumbered}

![](_resources/DL,_General.resources/bff-overview.png){.lightbox group="bff-ex1" width="532"}

- Notes from [3Blue1Brown series](https://www.youtube.com/playlist?list=PLZHQObOWTQDNU6R1_67000Dx_ZCJB-3pi) which uses the MNIST dataset and a neural network with 2 hidden layers with sigmoid activation functions (transform values to be between 0 and 1) and softmax output layer (probabilites).

### Structure {#sec-dl-gen-bff-str .unnumbered}

- An image is matrix where each cell has a brightness value\
  ![](_resources/DL,_General.resources/bff-data.png){.lightbox group="bff-ex1" width="532"}
  - There will be 784 input values for the first hidden layer.
  - 0.58 is one of brightness values that was at the top of the "9"
- The activation equation for one neuron/node\
  ![](_resources/DL,_General.resources/bff-activation-eq-1node.png){.lightbox group="bff-ex1" width="532"}
  - This equation produces the activation value for one neuron
  - $w_i$ are the weights and -10 is the bias ($b_i$). These are parameters that get optimized in each layer
  - The -10 bias value says the weighted sum needs to be greater than 10 for the neuron to be "activated."
  - The weighted sum plus bias is essentially just a regression equation which is fed into the sigmoid (activation function) which transforms it into a value between 0 and 1
- The activation equation for one layer\
  ![](_resources/DL,_General.resources/bff-activation-1layer.png){.lightbox group="bff-ex1" width="532"}
  - Each row in the weights matrix is a neuron (e.g. 2nd row is the 2nd neuron)
  - For the *first* fully connected hidden layer, there will be 784 weights and 1 bias value for each node. If there are 16 neurons in this hidden layer, then this layer has (784 x 16) + 16 = 12,560 parameters.
- Overall\
  ![](_resources/DL,_General.resources/bff-str-overall-param-count.png){.lightbox group="bff-ex1" width="332"}
  - Since it's the *second* hidden layer, there's only 16 inputs (activation values), so it's 16 x 16 total weights. Then the last layer, which is a softmax, has the 10 possible numbers in MNIST. With the biases, this gives a grand total of 13,002 parameters that need to be estimated.

### Gradient Descent {#sec-dl-gen-bff-gd .unnumbered}

- Method used to select weight and bias values that minimize a cost function

- Cost Function

  ::: {layout-ncol="2"}
  ![](_resources/DL,_General.resources/bff-grad-cost-1.png){.lightbox group="bff-ex1" width="432"}

  ![](_resources/DL,_General.resources/bff-grad-cost-2.png){.lightbox group="bff-ex1" width="432"}
  :::

  - Left: Shows a network after 1 epoch which has a brier score of 3.37. This is the average of the squared differences between the predicted probability and the truth.
  - Right: Shows a trained network that has brier score of 0.03.
  - The Brier Score is the cost function that we want to minimize

- Simple 1-D Illustration

  ::: {layout-ncol="2"}
  ![](_resources/DL,_General.resources/bff-grad-deriv-1.png){.lightbox group="bff-ex1" width="432"}

  ![](_resources/DL,_General.resources/bff-grad-deriv-2.png){.lightbox group="bff-ex1" width="432"}
  :::

  - The cost is the Y-axis and the weight value is the X-axis
  - Left: If the cost is some value, how do we mathematically find out whether we should lessen the weight value or increase it?
  - Right: By taking the derivative of the Cost function with respect to $w$ and plugging in $w$, we get a positive value. This value is of course the slope of the tangent at that point.
  - Since the slope is *positive*, we know to *lessen* the weight value in order to move towards a local minimum of the cost function

- Simple 3-D Illustration

  ![](_resources/DL,_General.resources/bff-grad-3d.png){.lightbox group="bff-ex1" width="432"}

  - This visualizes, in general, the process in which the gradient descent method takes steps (learning rate) towards a local minimum of the cost function (where $\Delta C$ is the gradient)

- The gradient the weights vector tells us how to adjust the weights.\
  ![](_resources/DL,_General.resources/bff-grad-w-adj.png){.lightbox group="bff-ex1" width="432"}![](_resources/DL,_General.resources/bff-grad-w-adj-2.png){.lightbox group="bff-ex1" width="432"}

  - The negative of the gradient is the value that should be added to weight.
    - This is also used to adjust the bias value
  - The gradient values that get applied to the weights are actually averages of the gradients calculated across all images in the training data.
  - A nudge to weight with a gradient of 3.20 has 32 times more influence on the cost than a weight with a gradient of 0.10.

### Backpropagation Math

- AKA **Backward Pass**. The process by which all the weights and biases get adjusted using the gradients.

- Overview

  ![](_resources/DL,_General.resources/bff-back-overview.png){.lightbox group="bff-ex1" width="532"}

  - Bottom Right: For simplicity, only one neuron's/node's activation value from the last hidden layer ($a^{(L-1)}$) and the output layer ($a^{(L)}$) are used
  - Left Bottom: Shows the parameters (weights, bias, activation values) and the hierarchical nature (chain rule) of the gradient calculation.
    - Ignore the little number lines. They were for animating how the change in one parameter affected others below it.
  - Left Top: Shows the calculation of the weight gradient that gets used to calculate the final activation value before it gets fed into the softmax function
  - Top Right: Shows the equations used in the derviative calculations — the cost function ($C_0$), the weighted sum (weights, biases) ($z^{(L)}$), and the full activation value equation ($a^{(L)}$) that includes the activation function (sigmoid in this case).

- Calculate Derivatives

  ![](_resources/DL,_General.resources/bff-back-deriv.png){.lightbox group="bff-ex1" width="532"}

  - Top: Shows the derivatives of the components for the gradient for the weights
  - Bottom Left: Just shows the whole gradient vector that includes all the neurons/nodes and gradients for weights and biases
  - The gradient for the bias is very similar. You just replace $\frac{\partial z^{(L)}}{\partial w^{(L)}}$ with $\frac{\partial z^{(L)}}{\partial b^{(L)}}$ in the chain rule equation. This results in a derivative equal to 1 instead of $a^{(L-1)}$.
  - Remember, that actually, the gradients in the gradient vector would be averages of gradients across all images (samples) in the training data.
...
- L1 and L2 regularization
  ...
  - L2 regularization may not be necessary to achieve a highly accurate model when applying batch normalization or dropout but Guo et al. empirically demonstrated the model tends to be less calibrated without using L2 regularization. ([source](https://arxiv.org/abs/2308.01222))
  - L1
    - Cost Function, J\
...
- Packages
  - [{]{style="color: #990000"}[multiRL](https://cran.r-project.org/web/packages/multiRL/index.html){style="color: #990000"}[}]{style="color: #990000"} - Modularizes the Markov Decision Process (MDP) into six core components, enabling users to flexibly construct the Rescorla-Wagner Model for Multi-Armed Bandit tasks
