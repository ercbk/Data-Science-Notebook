...
- The activation equation for one neuron/node\
  ...
  - The weighted sum plus bias is essentially just a regression equation which is fed into the sigmoid (activation function) which transformed it into a value between 0 and 1
...
- Simple 1-D Illustration
  ...
  - Since the slope is *positive*, we know to *lessen* the weight value to move towards a local minimum of the cost function
...
- The process by which all the weights and biases get adjusted using the gradients.
...
- Overview
  ...
  - Left Bottom: Shows the parameters (weights, bias, activation values) and the hierarchical nature of the gradient calculation.
