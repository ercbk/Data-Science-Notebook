...
- Packages
  ...
  - [{]{style="color: #990000"}[autocodebook](https://github.com/patriciafortesm/autocodebook){style="color: #990000"}[}]{style="color: #990000"} - Wraps 'dplyr' verbs (mutate, summarise, filter) to automatically capture variable metadata (type, source columns, categories, and source code), producing a codebook and eligibility tracking table with zero manual documentation. Works with both 'sparklyr' (tbl_spark) and local data frames.
  ...
  - [{]{style="color: #990000"}[data.checker](https://onsdigital.github.io/data.checker/){style="color: #990000"}[}]{style="color: #990000"} - Helps with boilerplate data checks. It enables you to automate fundamental data checks which, while simple, can be time-consuming to implement. ([{pointblank}]{style="color: #990000"} backend)
  - [{]{style="color: #990000"}[datadiff](https://cran.r-project.org/web/packages/datadiff/index.html){style="color: #990000"}[}]{style="color: #990000"} - A comprehensive data validation package that allows comparing datasets using configurable validation rules defined in 'YAML' files. Built on top of [{pointblank}]{style="color: #990000"}
...
- Use a forecasting method to generate prediction intervals. If data values for a variable fall outside the PIs, a data quality alert is triggered
  ...
  - If you have hundreds of variables, managing data quality rules for each column may be untenable
    - Use unsupervised data monitoring:
      ...
      - No anomalous records
        - A machine learning algorithm that identifies changes in continuous distributions, categorical values, time durations, or even relationships between columns
        - Score severity of anomaly somehow
  - Create an application (or subscribe to a service) that enables a user to adjust commonly changed rules/thresholds without writing code
    ...
    - The types of changes users often make include:
      ...
      - Easy reversion to prior state (requires a log/versioning)
  - For data that is critical, checks should be included in the pipeline
    - If a check fails, you could run automated tasks to fix the bad data, abort the remainder of the DAG (sometimes no data is better than bad data), or quarantine bad records using SQL produced in the API to query for good and bad data
...
- \[Two-Phase WAP\]{.underline} ![](_resources/Production,_Data_Validation.resources/pat-wap-2.webp){.lightbox width="532"}
...
- \[One-Phase WAP (Zero-Copy WAP)\]{.underline} ![](_resources/Production,_Data_Validation.resources/pat-wap-3.webp){.lightbox width="432"}
...
### Misc {#sec-prod-datval-pb-misc .unnumbered}
...
- Supports any kind of backend that dbplyr/dplyr supports (e.g. duckdb, postgres)
- Report Glossary
...
### Basic Workflow {#sec-prod-datval-pb-wkflw .unnumbered}
...
### Operations {#sec-prod-datval-pb-ops .unnumbered}

- [EDA your data]{.underline}
  - `scan_data` outputs a throrough EDA report either through viewer/browser or exported to a HTML file.

  - [{]{style="color: goldenrod"}[pointblank::col_summary_tbl](https://posit-dev.github.io/pointblank/user-guide/col-summary-tbl.html){style="color: goldenrod"}[}]{style="color: goldenrod"}, [{]{style="color: goldenrod"}[pointblank::missing_vals_tbl](https://posit-dev.github.io/pointblank/user-guide/missing-vals-tbl.html){style="color: goldenrod"}[}]{style="color: goldenrod"} perform a similar functionality in the python version of the package

  - [section = "OVICMS"]{.arg-text} (default) says what information to include in the report

    - ["O"]{.arg-text}: ["overview"]{.arg-text}
    - ["V"]{.arg-text}: ["variables"]{.arg-text}
    - ["I"]{.arg-text}: ["interactions"]{.arg-text}
    - ["C"]{.arg-text}: ["correlations"]{.arg-text}
    - ["M"]{.arg-text}: ["missing"]{.arg-text}
    - ["S"]{.arg-text}: ["sample"]{.arg-text}

  - Some dropdowns in the report include plots (e.g. histograms for categorical variables)

  - Just printing the [ptblank_tbl_scan]{.arg-text} class object (e.g. [tbl_scan]{.var-text} in example) will render the report in the

    - Seems to work better when viewed in a browser window

  - [Example]{.ribbon-highlight}: Export to file
    ...
    ```
- [Autogenerate Workflow]{.underline}
  - `draft_validation` creates a workflow (agent, validation steps, interrogate) by first scanning your data via `scan_data` and generating validation steps based on what it finds.
  ...
  - [Example]{.ribbon-highlight}:
    ...
    - The output will be an R script called `storms-validation.R`
- [Set Test Failure Thresholds]{.underline}
  ...
  - Local Thresholds (i.e. an individual test)
    ...
    - Setting `actions` at the individual validation step using absolute threshold values (e.g., a single test unit failing will trigger the `warn` state)
- [Getting data extracts for failed rows]{.underline}
  - From agent
    ...
    - Outputs a list of tibbles/dataframes with the rows that failed that particular validation step
...
- [Getting sundered data]{.underline}
  - Allows you to get the "usable" data (i.e. data that passed all the checks) after it's been interrogated
...
### Validation Functions {#sec-prod-datval-pb-vfunc .unnumbered}
...
- General Function Arguments
  ...
  - [preconditions]{.arg-text} allows you to apply a transformation to the data prior to the validation step
    ...
    col_vals_between(
      ...
      left = 6,
      right = 12,
      preconditions = ~ x |> dplyr::mutate(a = log(a))
      # or
      # precondions = function(x) dplyr::mutate(x, a = log(a))
    )
    ```
...
- Using values other than *literal* values
  ...
  - Other tables (e.g. same number of columns as the penguin dataset)
    ...
    ```
- Use the predicate/logical version of a function
  - Useful for tests within a function or a script in programming
  - All validation functions come with a `test_*` version that outputs [TRUE]{.arg-text}/[FALSE]{.arg-text}
    - [Example]{.ribbon-highlight}:
      ...
      ```
- Use functions within a [{testthat}]{style="color: #990000"} workflow
  - Useful for datasets that are included in packages
  - Each function has a `expect_*` version which allows it to be used in [{testthat}]{style="color: #990000"} unit testing (testthat has functions with the same prefix)
  - These functions can be used in the standard testthat workflow (in a `test-<name>.R` file, inside the `tests/testthat` folder).
  - The big difference here is that instead of testing function outputs, we are testing data tables.
  - Use `write_testthat_file` to create a `testthat.R` file using the agent from the `draft_validation` file
    - [Example]{.ribbon-highlight}:
      - Production a workflow file
      ...
      - Open the workflow file (`game_revenue-validation.R`) and execute the line at the bottom

        ``` r
        write_testthat_file(agent = agent, name = "game_revenue", path = "."
        ```

        - This will create the `test-game_revenue.R` file.
        - You can run this file by opening it, and using the 'Run Tests' button in the RStudio toolbar.

### Examples {#sec-prod-datval-pb-exs .unnumbered}
