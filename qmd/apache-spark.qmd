...
* Alternative: [{]{style='color: #990000'}[multiplyr](https://multidplyr.tidyverse.org/){style='color: #990000'}[}]{style='color: #990000'}
	* Option for data with > 10M rows and you only have access to one machine
...
* Potential issues with transferring R code to Spark
	1. Can't subset reference dataframe columns
...
* **Hive partitions** - used to split the larger table into several smaller parts based on one or multiple columns (partition key, for example, date, state e.t.c). The hive partition is similar to table partitioning available in SQL server or any other RDBMS database tables.
	* embeds field names and values in path segments, such as "/year=2019/month=2/<hash> or part\_<number>.parquet".
	**HDFS \-** Hadoop Distributed File System is a data storage system used by Hadoop. It provides flexibility to manage structured or unstructured data.
		_Storing large amounts of financial transactional data in an HDFS to query using Hive QL._
		
	**Kafka \-** More complex to work with than NiFi as it doesn’t have a user interface (UI), mainly used for real-time streaming data. It is a messaging system first created by LinkedIn engineers.
		_Streaming real-time weather events using Kafka_
...
* Distributed computation is performed by
	...
	* ![](./_resources/Apache,_Spark.resources/1-UeNRlmH00Xb1uxlyijMzhg.png)
...
1. The numbers 1 through 9 are _partitioned_ across three storage instances.
...
4. The task _executes_ a sorting operation within a partition.
	* Tasks are denoted as f(x) in the Spark webui DAG
5. The result is then _shuffled_ to the correct machine to finish the sorting operation across the entire dataset, which completes a stage.
	1. The sorted results can be optionally _cached_ in memory to avoid rerunning this computation multiple times.
6. Finally, a small subset of the results is _serialized_, through the network connecting the cluster machines, back to the driver node to print a preview of this sorting example.

* Spark Job![](./_resources/Apache,_Spark.resources/1-FdGrw0cwu-z5qaeMm88Hww.png)
...
* Spark Transformation types: ![](./_resources/Apache,_Spark.resources/1-u69zDQgxukpRQTiZZ6Alqg.png)
...
* **Lazy Evaluation** - triggering processing only when a Spark action is run and not a Spark transformation
...
* install
	* spark\_install has a version argument if you want a specific version
...
* `Service 'sparkDriver' failed after 16 retries`
	...
	* Solutions
		* Exporting the corresponding SPARK\_LOCAL\_IP environment variable that is loaded when the JVM is initialized on worker nodes
		* Setting the corresponding spark.driver.bindAddress configuration in the SparkSession (note that this approach will override SPARK\_LOCAL\_IP environment variable)
...
* **data skew**
	* The OOM error can occur on joins of very large tables or very large table + medium table with skewed data because of unevenly distributed keys
		...
		* When they say "skewed", they mean something closer to imbalanced. Partitions and join/group\_by variables are usually discrete and this seems like a imbalanced categorical variable issue
		Examples:
	...
	* Other symptoms
		...
		* e.g. most tasks finish within a reasonable amount of time, only to have one task take forever![](./_resources/Apache,_Spark.resources/image.png)
	...
	* Solution: Adaptive Query Execution (AQE)
		...
		* Set _spark.sql.adaptive.enabled_ = TRUE in the config
	...
	* Solution: "Salt" the join key(s)
		...
		* Join tables then perform calculations (e.g. group\_by(join\_id) %>% count)
	...
	* Solution: Salted Sort Merge Join
		...
		* Process Option 1
			* An additional column ‘_salt key’_ is introduced in one of the skewed input datasets.
			* A number is randomly assigned from a selected range of salt key values for the ‘_salt key_’ column to every record
			...
			* To produce the final joined output, all the partial joined outputs are combined together using the _Union_ operator.
		* Process Option 2![](./_resources/Apache,_Spark.resources/image.2.png)
			* An additional column ‘_salt key’_ is introduced in one of the skewed input datasets.
			* A number is randomly assigned from a selected range of salt key values for the ‘_salt key_’ column to every record
			* A for-loop is initiated on salt key values in the selected range. For every salt key value:
				* The second non skewed input dataset is enriched with the current iterated salt key value by repeating the the same value in the new ‘_salt_’ column to produce a partial salt enriched dataset.
			* All these partial enriched datasets are combined using the _Union_ operator to produce a combined salt enriched dataset version of the second non-skewed dataset.
...
Optimization
![](./_resources/Apache,_Spark.resources/1-0a3eoFBqKLS-lajp4uk4_w.png)
...
sparklyr.cores.local` <- 6
        conf# Apache, Spark

sparklyr.shell.driver-memory` <- "6G"
        conf# Apache, Spark
...
* Create a _config.yml_ file in the working dir or a parent dir. Then there's no need to specify the config parameter when creating a connection
	* You can also specify an alternate configuration filename or location by setting the file parameter in `spark_config()`
	* Can also change the default configuration by changing the value of the R\_CONFIG\_ACTIVE environment variable. See the GitHub [rstudio/config](https://oreil.ly/74jIL) repo for additional information
...
* Partitioning
	...
	* Types
		...
		* **Explicit**: Manaully setting the number of partitions (i.e. repartitioning)
			* Useful when when you have many more or far fewer compute instances than data partitions. In both cases, it can help to _repartition_ data to match your cluster resources
			* Various data functions, like `spark_read_csv()` or `copy_to()`, already support a repartition parameter to request that Spark repartition data appropriately
				* Also,`spark_df %>% sdf_repartition(4)`(e.g. repartitions a Spark DataFrame to have 4 partitions)
					* Should be shown as the _spark.sql.shuffle.partitions_ parameter in the webui or config
...
* Caching
	...
	* Inspect RDDs through the UI (Storage Tab >> <RDD name>)![](./_resources/Apache,_Spark.resources/tuning-cache-rdd-web-resized.png)
	* Checkpoints
...
* Shuffling![](./_resources/Apache,_Spark.resources/1-pGL7i8B1ALEUosUhbXqYEQ.png)
	...
	* Broadcast Joins
		* \*\* Think this only needs to be explicitly specified for Spark 2.x. Spark 3.0 has Adaptive Query Execution (AQE) which uses the optimal type of join automatically. \*\*
			* [Video](https://www.youtube.com/watch?v=PiZcQKbomDU)
			* May need Databricks (runtime >7.0) to get AQE and set _spark.sql.adaptive.enabled_ = TRUE in the config
		...
		* Can be set according to difference in size in bytes using _spark.sql.autoBroadcastHashJoin_ (not sure if this is available in [{sparklyr}]{style='color: #990000'}
		...
		* Adjust (increase?) _spark.sql.autoBroadcastJoinThreshold_ to get smaller tables broadcasted
...
* Iterative Broadcast Joins
	...
	* Process![](./_resources/Apache,_Spark.resources/image.1.png)
...
* Union Operator (e.g. `df = df1.union(df2).union(df3)` ) ([article](https://towardsdatascience.com/boosting-spark-union-operator-performance-optimization-tips-for-improved-query-speed-9123ae6ada80))
	...
	* Typical Use Case![](./_resources/Apache,_Spark.resources/image.3.png)
...
* iris\_ref
	* list object in R environment that is a reference to iris\_tbl in spark
...
```
# combo: manipulate ref objs and create new object in spark and R
full_products <- dplyr::copy_to(sc, products_ref %>%
...
* copy\_to also can be used to create new objects in spark from reference objects in R
	* products\_ref, departments\_ref, and aisles\_ref are reference objects for tables inside spark
	* products\_full is the name we want for the new table inside spark
	* full\_products will be the reference object for products\_full
...
* products\_ref is the reference object
* sc is the connection object
* products\_tbl is the name we want for the table inside spark
...
```
pred_iris <- ml_predict(model_iris, test_iris) %>%
                dplyr::collect()
```
...
* percentile returns a list so `explode`  (unnesting/flattening) coerces it into a vector
	* I found an example of "explode" in [{sparklyr}]{style='color: #990000'} docs but no function documentation. Funciton also available in {sparkr} and {sparklyr.nested}
...
* Can use mutate, group\_by, summarize, across, etc.
...
* where full\_product is the name of the table _in the cluster_


**Modeling**

* Partition, Register, Create Reference Object
	1. sdf\_partition creates partition reference object for train, test sets
	2. sdf\_register takes the partition reference object and "registers" the splits in the cluster
		* Not sure if partition\_iris is in the cluster, but I think when the "registering" occurs is when the splitting takes place inside the cluster and separate objects are created.
...
                ml_decision_tree(response = "Species", features = c("Petal_Width", "Petal_Length"))
test_iris <- tbl(sc, "spark_testing_iris")
```

* test\_iris and tidy\_iris are reference objects
* I don't think model\_iris is a reference object. It's a spark class objecti but I believe it's in your environment and not on the cluster
...
* Notes from class
	* She had RStudio server and Spark running on EC2 instance
		* Guessing Hadoop is also installed when spark is installed and they're like peas and carrots
	* Her data came from a S3 bucket and is ingested in Hadoop Distributed File System (HDFS)
		* she wrote a custom function to browse files in HDFS while in RStudio and get the path to the file she wants
			* paths start with "stackspace" and ends with file.csv
		* she then uses spark\_read\_csv with that path to load the data into spark
			* need to add "hdfs:///" to the beginning of the path
	* Her spark connection uses a yarn-client

```
    sc <- spark_connect(master = "yarn-client")
```

* In her .Rprofile she has Sys.setenv(SPARK\_HOME="/opt/spark")
...
* Start a stream - `stream_read_csv(sc, "<input folder path>/")`
	* Make sure path to folder (e.g. "stream\_input/") ends with a backslash
...
* **Sparklint**![](./_resources/Apache,_Spark.resources/screen_shot.png)
...
* **webui**
	...
	* default URL: http:driver\_host:4040
...
* Stream metrics to **Graphana**![](./_resources/Apache,_Spark.resources/Screenshot (480).png)
	* Example: This sends metrics to [influxdb](https://docs.influxdata.com/influxdb/v1.8/supported_protocols/graphite/) using a [Graphite](https://graphite.readthedocs.io/en/latest/overview.html) "protocol" which can be visualized in [Graphana](https://grafana.com/)![](./_resources/Apache,_Spark.resources/Screenshot (484).png)![](./_resources/Apache,_Spark.resources/Screenshot (482).png)
