...
## Misc {#sec-apache-spark- .unnumbered}
...
-   Manipulate ref objs and create new object in spark and R
    ...
    -   `copy_to` also can be used to create new objects in spark from reference objects in R
        -   [products_ref]{.var-text}, [departments_ref]{.var-tx}, and aisles_ref are reference objects for tables inside spark
...
-   [Example]{.ribbon-highlight}:
    ...
    -   `percentile` returns a list so `explode` (unnesting/flattening) coerces it into a vector
        -   I found an example of "explode" in [{sparklyr}]{style="color: #990000"} docs but no function documentation. Function also available in {sparkr} and {sparklyr.nested}
...
-   [Sparklint]{.underline} ![](./_resources/Apache,_Spark.resources/screen_shot.png){width="632"}
