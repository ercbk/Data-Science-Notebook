...
-   Packages
    ...
    -   [{]{style="color: #990000"}[sparkxgb](https://spark.posit.co/packages/sparkxgb/){style="color: #990000"}[}]{style="color: #990000"} - XGBoost in Spark
...
-   Tools
    ...
    -   [Apache Comet](https://datafusion.apache.org/comet/) - A high-performance accelerator for Apache Spark, built on top of the powerful Apache DataFusion query engine.
...
-   Distributed computation is performed by
    ...
    -   **Serializing** - Transforms data to be sent over the network to other workers or back to the driver node.\
        ![](./_resources/Apache,_Spark.resources/1-UeNRlmH00Xb1uxlyijMzhg.png){.lightbox width="532"}
...
-   Spark Job\
    ![](./_resources/Apache,_Spark.resources/1-FdGrw0cwu-z5qaeMm88Hww.png){.lightbox width="532"}
...
-   Spark Transformation types:\
    ![](./_resources/Apache,_Spark.resources/1-u69zDQgxukpRQTiZZ6Alqg.png){.lightbox width="532"}
...
### Disk Spill {#sec-apache-spark-errors-datsp .unnumbered}

-   Notes from [Spark Performance Tuning: Spill](https://selectfrom.dev/spark-performance-tuning-spill-7318363e18cb)
...
-   Monitoring
    ...
    -   Tables (e.g. large partitions leading to disk spillage)\
        ![](_resources/Apache,_Spark.resources/spill-mon-jobs-1.webp){.lightbox width="532"}
        -   Summary\
            ![](_resources/Apache,_Spark.resources/spill-mon-stats-sum-1.webp){.lightbox width="532"}
        -   Aggregated\
            ![](_resources/Apache,_Spark.resources/spill-mon-stats-agg.webp){.lightbox width="532"}
        -   Task\
            ![](_resources/Apache,_Spark.resources/spill-mon-stats-task-1.webp){.lightbox width="532"}
        -   SQL Plan (On sort step)\
            ![](_resources/Apache,_Spark.resources/spill-mon-sqlplan-1.webp){.lightbox width="332"}
-   Actions that can generate spill
    -   Data Skew
    -   Reducing [spark.sql.shuffle.partitions]{.arg-text}leads to bigger file sizes per partition.
    ...
    -   Setting [spark.sql.files.maxPartitionBytes]{.arg-text} to high (default is 128 MBs).
...
-   Solutions:
    -   Fix Skew
    -   Process less data per task, which can be achieved reducing file size per partition via [spark.shuffle.partitions](https://spark.apache.org/docs/latest/sql-performance-tuning.html#other-configuration-options) or [repartition](https://spark.apache.org/docs/3.1.3/api/python/reference/api/pyspark.sql.DataFrame.repartition.html).
        -   See [Optimization \>\> Partitioning](apache-spark.qmd#sec-apache-spark-opt-part){style="color: green"}
    -   Increase the RAM to core ratio in your compute, i.e. add more memory per worker (increase worker size).
    -   Adjust [spark.sql.files.maxPartitionBytes]{.arg-text} - Influences size of partitions when reading in files. Parquet files are already efficient in this respect but CSVs and others may benefite.
        -   See [Optimization \>\> Partitioning](apache-spark.qmd#sec-apache-spark-opt-part){style="color: green"}
    -   Increase [spark.shuffle.spill.fraction]{.arg-text}: Specifies the fraction of executor memory that can be used for shuffle data before spilling starts. Default: 0.20 (20%)
        -   [Example]{.ribbon-highlight}: If executor memory is 10GB and spark.shuffle.spill.fraction is 0.2, then spilling to disk starts when shuffle data reaches 2GB (20% of 10GB).
        -   Increasing:
...
![](./_resources/Apache,_Spark.resources/1-0a3eoFBqKLS-lajp4uk4_w.png){.lightbox width="632"}
...
-   [spark.sql.files.maxPartitionBytes]{.arg-text} - Influences size of partitions when reading in files. Parquet files are already efficient in this respect but CSVs and others may benefite.
    ...
    -   Best Practices:
        ...
        -   Consider other configuration options like [spark.sql.files.openCostInBytes]{.arg-text} and [spark.sql.files.minPartitionNum]{.arg-text} for fine-tuning.
...
-   Inspect RDDs through the UI (Storage Tab \>\> <RDD name>)\
    ![](./_resources/Apache,_Spark.resources/tuning-cache-rdd-web-resized.png){.lightbox width="532"}
-   Checkpoints
...
![](./_resources/Apache,_Spark.resources/1-pGL7i8B1ALEUosUhbXqYEQ.png){.lightbox width="532"}
...
-   Process\
    ![](./_resources/Apache,_Spark.resources/image.1.png){.lightbox width="475"}
...
### Avoid Loops {#sec-apache-spark-opt-loop .unnumbered}
...
-   [Example]{.ribbon-highlight}: pyspark
...
-   [Example]{.ribbon-highlight}: sparklyr
...
-   Typical Use Case\
    ![](./_resources/Apache,_Spark.resources/image.3.png){.lightbox width="532"}
...
-   [Sparklint]{.underline}\
    ![](./_resources/Apache,_Spark.resources/screen_shot.png){.lightbox width="632"}
...
-   [Graphana]{.underline}
    ...
    -   Visualizes metrics streaming to it ![](./_resources/Apache,_Spark.resources/Screenshot%20(480).png){.lightbox width="532"}
        -   [Example]{.ribbon-highlight}: This sends metrics to [influxdb](https://docs.influxdata.com/influxdb/v1.8/supported_protocols/graphite/) using a [Graphite](https://graphite.readthedocs.io/en/latest/overview.html) "protocol" which can be visualized in [Graphana](https://grafana.com/)\
            ![](./_resources/Apache,_Spark.resources/Screenshot%20(484).png){.lightbox width="532"}\
            ![](./_resources/Apache,_Spark.resources/Screenshot%20(482).png){.lightbox width="532"}
...
-   [Misc]{.underline}
    ...
    -   [Thread](https://bsky.app/profile/sarahkb.bsky.social/post/3lcweogwlzs2g) on advantages and disadvantages
        -   tl;dr Probably too pricey unless you're a large enterprise company but feature rich
    -   {sparkR} and [{sparklyr}]{style="color: #990000"} are already installed on a databricks instance
    -   Databricks Connect is now able to run regular Python code inside Spark. [{sparklyr}]{style="color: #990000"} takes advantage of this capability by having Python transport and run the R code. It does this via the [{{rpy2}}]{style="color: goldenrod"}.
        -   [{{rpy2}}]{style="color: goldenrod"} will have to be manually installed on the Spark cluster. (See [docs](https://docs.databricks.com/en/libraries/cluster-libraries.html) for instructions)
...
-   [UDFs]{.underline}
    -   `spark_apply`
        ...
        -   [columns]{.arg-text}: Specifying a schema makes it run faster. Otherwise, it looks at the first 10 rows and tries to figure out what the column types are.
        -   [arrow_max_records_per_batch]{.arg-text}: Controls then number of parallel jobs.
            -   Arrow is installed on all clusters by default. So instead of using [group_by]{.arg-text} to determine how the data will be partitioned, it will controlled by this argument.
            -   The equivalent Spark setting is [spark.sql.execution.arrow.maxRecordsPerBatch]{.arg-text}
        -   Currently only works on Databricks “Single Access” mode and not “Shared Access” mode.
            -   If `mapInPandas` and `applyInPandas` are ever implemented in Shared Access, then this will mean `spark_apply` will also.
...
-   [Example]{.ribbon-highlight}: [{pyspark}]{style="color: goldenrod"} ([source](https://dataengineeringcentral.substack.com/p/650gb-of-data-delta-lake-on-s3-polars))
    ...
    agg = (
        df.withColumn("post_date", F.to_date("created_at_ts"))
          .groupBy("post_date")
          .count()
    ...
    (agg.coalesce(1)
        ...
        .option("header", "true")
        .csv("s3://confessions-of-a-data-guy/bingbong/posts_per_day_csv_out"))
    ```

    -   Loads a Delta Lake table from S3 using Spark’s native Delta reader.
    -   Converts the string timestamp to a proper Spark timestamp column.
    -   Extracts the date portion for grouping.
    -   Computes daily post counts with a `groupBy().count()`.
    -   `coalesce(1)` reduces the number of partitions to exactly one which is then written to S3 as a CSV
...
### Spark Connect {#sec-apache-spark-cloud-sparkconn .unnumbered}

-   A decoupled client-server architecture that allows remote connectivity to Spark clusters using the DataFrame API (Requires \> Spark 3.4)
-   The separation between client and server allows Spark and its open ecosystem to be leveraged from everywhere. It can be embedded in modern data applications, in IDEs, Notebooks and programming languages.
    -   A spark server typically uses over 1.5 GB of memory. With Spark Connect, the client typical memory consumption is 200 MB.
-   [Overview](https://spark.apache.org/docs/latest/spark-connect-overview.html), [Article](https://towardsdatascience.com/adopting-spark-connect-cdd6de69fa98)
