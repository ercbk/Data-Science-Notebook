...
-   Packages
    ...
    -   [{]{style="color: #990000"}[config](https://rstudio.github.io/config/){style="color: #990000"}[}]{style="color: #990000"} - For manage environment specific configuration values.
...
-   Questions
    ...
    -   Hardware?: You simply can’t put every model on a GPU; you’d go bankrupt. You need a Tiered Strategy: run your simple “fallback” models on cheap CPUs, and reserve the expensive GPUs for the heavy “money-maker” models.
...
-   Components
    ...
    -   Fallback Model: At scale, a 1-second lag in your fallback mechanism is a failure. When a model "fails" (e.g. drifts past the threshold or gets bad data), the switch to the fallback model should happen in milliseconds.
    -   Safety Net:
...
-   Using Config Files
    ...
    -   [Example]{.ribbon-highlight}: [{config}]{style="color: #990000"} for path creation
        ...
        -   When working with databases locally, make sure to add \*.db to .gitignore.
        ...
        db_file_def <- here::here(cfg_def$duckdb$path, cfg_def$duckdb$name)
        db_file_def
        #> [1] "C:/Users/user/Documents/R/Projects/Data-Science-Notebook/db/dev.duckdb"

        cfg_prod <- config::get(config = "production")
        cfg_prod
        #> duckdb
        #> $duckdb$path
        #> [1] "db"
        ...
        db_file_prod <- here::here(cfg_prod$duckdb$path, cfg_prod$duckdb$name)
        db_file_prod
        #> [1] "C:/Users/user/Documents/R/Projects/Data-Science-Notebook/db/prod.duckdb"
        ```

        -   `here` returns the root of your R project, no matter what your current working directory is within the project.
        -   Parameters are concatenated with the local OS path separator as needed (‘/’ or ‘\\’).
        :::
