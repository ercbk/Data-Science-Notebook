...
-   Establishing study objectives is the first step in designing a survey. Defining the target population should be the second step.
...
-   [**Frame**]{style="color: #009499"} - All the elements from the target population *that can be listed* constitute the frame. This will give each element a non-zero probability of selection for inclusion in the survey sample. The frame is used to draw the sample.
...
-   [**Sampling Frame**]{style="color: #009499"} - A frame that *represents* the population of interest. An exhaustive list of all the individuals which comprise the target population (Also see [Sources of Error](surveys-design.qmd#sec-surv-design-soe){style="color: green"} \>\> Coverage or Frame Error)\
    ...
    -   Examples: area frames for in-person interviews, random-digit dialing (RDD) frames for telephone survey samples, and a variety of lists used for all types of surveys.
...
-   [**Target Population**]{style="color: #009499"} - Often similar to the inferential population (population of inference) but excludes some elements that would be very difficult or costly to include on the frame.
...
## Biases {#sec-surv-design-respsc-biases .unnumbered}
...
-   [Acquiescence Bias]{.underline}
    ...
    -   Solution:
        ...
        -   [Example]{.ribbon-highlight}\
            ![](./_resources/Surveys,_Design.resources/1-TyDxWQMp1rrNhnOErUnrGA.png){width="532"}
