...
* For **sampling methods**, see [Experiments, General](Experiments, General)
...
* **Total Survey error** - accumulation of all errors that may arise in the design, collection, processing, and analysis of survey data. It includes sampling variability, interviewer effects, frame errors, response bias, and non-response bias
	* Non-Sampling Error
		...
		* **Coverage or Frame error** typically results from the frame construction process. Discrepancies between the (theoretical) target population of a survey and the frame which is used to draw a sample are equivalent to statistical errors.
			* See [Surveys, Sampling Methods](Surveys, Sampling Methods) >> Terms >> sampling frame
...
* **item-specific (IS)** \- multiple choice response format![](./_resources/Surveys,_Design.resources/0-R4C3yXJTPpBUZnHM.png)
* **Agree/Disagree (A/D)** \- response format where the response are degrees of strength of agreement or disagreement![](./_resources/Surveys,_Design.resources/0-11yLyNXjXnQYLTxd.png)
...
* Types
	...
	* Semantic differential scales  ![](./_resources/Surveys,_Design.resources/Screen-Shot-2016-03-07-at-11.png)
	...
	* Issues
		* Ordinal and not Interval
			* Answers are not all equidistant
				* respondents may perceive (4) Strongly Agree and (3) Agree very similarly and thus the difference between these two options might be much smaller than the difference between (3) Agree and (2) Disagree, despite having the same distance.![](./_resources/Surveys,_Design.resources/1-0L81UdRm3R57VmustUwKCw.png)
			...
			* **Acceptable analysis methods**:
				...
				* graphical tools such as bar charts and correlation matrix plots
...
* **Guttman Scale**![](./_resources/Surveys,_Design.resources/Screenshot (367).png)
...
* **Net Promoter Score (NPS)**
	...
	* NPS = percent\_promoters - percent\_detractors
	...
	* Alternative: Use sentiment analysis on customer reviews and social media posts to generate a proxy for NPS based on sentiment scores.
		* See [Algorithms, Product](Algorithms, Product) >> Net Promoter Score
...
* **Acquiescence Bias**
	...
	* Solution:
		...
		* Example![](./_resources/Surveys,_Design.resources/1-TyDxWQMp1rrNhnOErUnrGA.png)
