...
- **Ansari–Bradley Test**
  ...
  - Packages
    ...
    - [{]{style="color: #990000"}[abwm](https://cran.r-project.org/web/packages/abwm/index.html){style="color: #990000"}[}]{style="color: #990000"} - Performs the two-sample Ansari–Bradley test for univariate, distinct data **in the presence of missing values**
...
- Packages
  - [{]{style="color: #990000"}[BlockwiseRankTest](https://cran.r-project.org/web/packages/BlockwiseRankTest/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements a two-sample test that handles block-wise missingness.
    - It identifies missing-data patterns, constructs a (blockwise) dissimilarity matrix, induces ranks via a k-nearest neighbor style graph, and computes a quadratic statistic under two versions: the congregated form (‘con’) and vectorized form (‘vec’).
...
- **Robust LePage Tests**
  ...
  - Increases statistical power under weak null hypotheses — that is, when unequal variances exist in location-testing or unequal medians exist
...
- [Cochran's Q]{.underline}

  - Extension of McNemar's test. Used to determine whether the proportion of “successes” is equal across three or more groups (e.g. treatments) in which the same individuals appear in each group.
  ...
  - Often it is used to assess if different observers of the same phenomenon have consistent results (interobserver variability).
    ...
    - e.g. repeated time points, successive treatment phases, or the same cases judged against three or more criteria/raters ([source](https://casrai.org/guides/cochrans-q-test))

  - The Friedman test or Durbin test can be used when the response is not binary but ordinal or continuous

  - Primary Assumption

    - Subjects (blocks) are independent of each other. Within a subject, the k answers are of course dependent, and this is exactly what the test accounts for. Between subjects, however, independence is required

      - The answers of one respondent must not influence those of another respondent.
      - As for many statistical tests, this assumption is verified based on the design of the experiment rather than via a formal test, and a random sample of respondents answering individually is generally sufficient.

  - Hypotheses

    - H~0~: The proportion of “successes” is the same in all groups
    - H~a~: The proportion of “successes” is different in at least one of the groups
    - If the Null is rejected, `rstatix::pairwise_mcnemar_test` can drill down further and find the group(s) that is different

  - [Example]{.ribbon-highlight}: Multiple Treatments ([source](https://www.statology.org/cochrans-q-test/))

    ``` r
    dat <- data.frame(
      student = rep(1:20, each = 3),
    ...
    DescTools::CochranQTest(outcome ~ technique | student, data = dat)
    #> Cochran's Q test
    #> 
  ...
  - [Example]{.ribbon-highlight}: Repeated Measures ([source](https://www.r-bloggers.com/2026/08/cochrans-q-test-in-r-or-the-extension-of-mcnemars-test-for-more-than-two-groups/))
    ...
    dat_long |> 
      rstatix::cochran_qtest(opinion ~ time | respondent)
    ## # A tibble: 1 × 6
    ...
    - 200 randomly selected citizens are asked whether they are in favor of a new policy in their city (answer “Yes” or “No”). In order to see whether the effect of the debate persists over time, these citizens are asked this exact same question at three different points in time:
      - *Before* a public debate on this policy
      - Right *after* this debate
      - One month later (*follow up*)
    - [time]{.var-text} is the label for each measurement taken
    - [opinion]{.var-text} is the binary outcome variable
    - A p-value less than 0.05 says that the proportions of [yes]{.arg-text} between [before]{.arg-text}, [after]{.arg-text}, and [follow-up]{.arg-text} are not the same and at least one has a statistically different proportion.
