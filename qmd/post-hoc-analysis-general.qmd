...
-   McNemar Test: The McNemar test is used to compare proportions in a paired design (related samples) where you have a binary outcome measured twice within the same group. (See above)

-   Binomial Test**:** The binomial test is used when you have only one sample and want to compare the proportion of success against a known or hypothesized proportion (one-sample test).
