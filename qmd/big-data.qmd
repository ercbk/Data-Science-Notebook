...
- Benchmarks
  ...
  - [Data Engineering Central](https://dataengineeringcentral.substack.com/p/650gb-of-data-delta-lake-on-s3-polars) tested libraries on lazily aggregating Delta Lake files in AWS S3 on a *single node* (2025)
    - Uses 650GB of social media data and aggregates daily number of posts
    - Uses an EC2 c5.4xlarge instance with 16 vCPU and 32GB of RAM
    - Results
      - [{pyspark}]{style="color: goldenrod"}: \~60 min
      - [{polars}]{style="color: goldenrod"}: 12 min
      - [{daft}]{style="color: goldenrod"}: 50 min
      - [{duckdb}]{style="color: goldenrod"}: 16 min
    - [{pyspark}]{style="color: goldenrod"} and [{daft}]{style="color: goldenrod"} are built for distributed computing, so it's not totally surprising they sucked. The main takeaway is that large data engineering tasks can be handled on a single node on cheapish hardware.
...
- [{]{style="color: #990000"}[cuplyr](https://github.com/bbtheo/cuplyr){style="color: #990000"}[}]{style="color: #990000"} - A dplyr backend for GPU acceleration via RAPIDS cuDF

  - Can provide significant speedups on larger datasets (typically \>10M rows) without requiring major code changes.
...
- [FastR](https://github.com/oracle/fastr) - An implementation of R that can JIT compile R code to run on the [Graal VM](https://www.graalvm.org/). It requires a different runtime (i.e. you can't just run your normal R installation) and has other trade-offs, including warm-up cycles and compatibility limitations[8](https://github.com/brodieG/r2c#user-content-fn-3-1b1cfa42663b7f54a3fec029a434eb09). But otherwise you type in what you would have in normal R and see some impressive speed-ups

  - When considering using matrix operations such as `cholesky` decomposition in place of [{Matrix}]{style="color: #990000"}, you should determine if your use case is using dense ([{Rfast}]{style="color: #990000"}) or sparse matrices ([{Matrix}]{style="color: #990000"}) and whether you also require other functions like `solve` ([{Matrix}]{style="color: #990000"}).

- [{]{style="color: #990000"}[kit](https://cran.r-project.org/web/packages/kit/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fast (implemented in C) vectorized and nested switches, some parallel (row-wise) statistics, and some utilities such as efficient partial sorting and unique values.

- [{]{style="color: #990000"}[matrixStats](https://cran.r-project.org/web/packages/matrixStats/index.html){style="color: #990000"}[}]{style="color: #990000"}: Efficient row-and column-wise (weighted) statistics on matrices and vectors, including computations on subsets of rows and columns.
...
- [{polars}]{style="color: #990000"}: Arrow product; uses SIMD which is a low-level vectorization that can be used to speed up simple operations like addition, subtraction, division, and multiplication
  ...
  - Polars Cloud can perform distributed computing
...
- [{]{style="color: #990000"}[r2c](https://github.com/brodieG/r2c){style="color: #990000"}[}]{style="color: #990000"}: Fast grouped statistical computation; currently limited to a few functions, sometimes faster than [{collapse}]{style="color: #990000"}

- [{]{style="color: #990000"}[Rfast](https://cran.r-project.org/web//packages/Rfast/index.html){style="color: #990000"}[}]{style="color: #990000"}, [{]{style="color: #990000"}[Rfast2](https://cran.r-project.org/web/packages/Rfast2/index.html){style="color: #990000"}[}]{style="color: #990000"}: A collection of fast functions for data analysis.

  - Rfast - Column- and row- wise means, medians, variances, minimums, maximums, many t, F and G-square tests, many regressions (normal, logistic, Poisson), are some of the many fast functions

  - Rfast2 - Regressions: Zero-Inflated, Hurdle, Repeated Measures, Tobit, Multinomial. Models: k-NN, PCA, Discriminant Analysis, Naive Bayes. Other: Hotelling, Moran's I, Mahalanobis depth, Kaplan-Meir, Partial Correlation, Welch, cross-validataion, and much more.
...
- Also see [SQL \>\> Tools](sql.qmd#sec-sql-tools){style="color: green"}
...
- Scatter plots
  ...
  - [{]{style="color: #990000"}[ggrastr](https://github.com/VPetukhov/ggrastr){style="color: #990000"}[}]{style="color: #990000"}
    ...
    - [Example]{.ribbon-highlight}
      ...
      ```
  - [{]{style="color: #CE3375"}[deepscatter](https://github.com/nomic-ai/deepscatter){style="color: #CE3375"}[}]{style="color: #CE3375"} ([Thread](https://twitter.com/benmschmidt/status/1561720471497875456) (using Arrow, duckdb)) - Zoomable, animated scatterplots in the browser that scales over a billion point
- [{]{style="color: #990000"}[dbplot](https://edgararuiz.github.io/dbplot/){style="color: #990000"}[}]{style="color: #990000"} - Plots data in databases and Spark Clusters
...
- [{]{style="color: #990000"}[h2o::h2o.aggregator](https://docs.h2o.ai/h2o/latest-stable/h2o-docs/data-science/aggregator.html){style="color: #990000"}[}]{style="color: #990000"} ([article](https://www.h2o.ai/blog/visualizing-large-datasets-with-h2o-3/))- Reduces data size to a representive sample, then you can visualize a clustering-based method for reducing a numerical/categorical dataset into a dataset with fewer rows
  - A count column is added to show how many rows is represented by the exemplar row (I think)
...
- [{]{style="color: #990000"}[mosaicr](https://dfalbel.github.io/mosaicr/index.html){style="color: #990000"}[}]{style="color: #990000"} - A framework for linking databases and interactive views
...
- [{]{style="color: goldenrod"}[plotly-resampler](https://github.com/predict-idlab/plotly-resampler){style="color: goldenrod"}[}]{style="color: goldenrod"} - Improves the scalability of Plotly for visualizing large time series datasets.
  - Dynamically aggregates time-series data respective to the current graph view, ensuring efficient and responsive updates during user interactions like panning or zooming via callbacks.
- [{]{style="color: #990000"}[shinyHugePlot](https://cran.r-project.org/web/packages/shinyHugePlot/index.html){style="color: #990000"}[}]{style="color: #990000"} - A tool to plot data with a large sample size using 'shiny' and 'plotly'. Relatively small samples are obtained from the original data using a dynamic sampler algorithm (Uses [{plotly-resampler}]{style="color: goldenrod"})

## Modeling {#sec-bgdat-mod .unnumbered}
...
- [{]{style="color: #990000"}[BayesForge](https://cran.r-project.org/web/packages/BayesForge/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Website](https://s-sosa.com/BF/)) - A high-performance probabilistic programming library that aims to unify the modeling experience by providing an intuitive model-building syntax together with the flexibility of low-level abstraction coding.
  - Built on top of Numpyro, TensorFlow Probability, and Jax for cpu, gpu and tpu vectorization and parallelization
...
- [{]{style="color: #990000"}[cuda.ml](https://mlverse.github.io/cuda.ml/){style="color: #990000"}[}]{style="color: #990000"} ([Intro](https://opensource.posit.co/blog/2026-08-21_cuda-ml-0-4-0/)) - A simple and intuitive R interface for [RAPIDS cuML](https://github.com/NVIDIA/cuml). RAPIDS cuML is a suite of GPU-accelerated machine learning libraries powered by CUDA
  - Can use direct R functions or work through [{parsnip}]{style="color: #990000"} and [{tidymodels}]{style="color: #990000"}
  - Many algorithms for clustering, dimension reduction, ML, and regression (linear, logistic, regularized, multinomial)
- [{]{style="color: #990000"}[dbreg](https://github.com/grantmcdermott/dbreg){style="color: #990000"}[}]{style="color: #990000"} - Leverages the power of databases to run regressions and fixed effects regressions on very large datasets, which may not fit into R's memory.
  - Various acceleration strategies allow for highly efficient computation, while robust standard errors are computed from sufficient statistics.
  - DuckDB is the default backend , but users can also specify alternative database backends
- [{]{style="color: goldenrod"}[duckreg](https://github.com/apoorvalal/duckreg){style="color: goldenrod"}[}]{style="color: goldenrod"} - Run stratified/saturated regressions out-of-memory with duckdb
  - Python version of [{dbreg}]{style="color: #990000"}
...
- [{]{style="color: #990000"}[fastglm](https://jaredhuling.org/fastglm/){style="color: #990000"}[}]{style="color: #990000"} - A re-write of `glm` using RcppEigen designed to be computationally efficient and algorithmically stable
  - Also includes Negative Binomial, Zero-Inflated, and Hurdle models
- [{]{style="color: #990000"}[glm4](https://cran.r-project.org/web/packages/glm4/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fits Generalised Linear Models (GLMs) with sparse and dense 'Matrix' matrices for memory efficiency
- [{]{style="color: #990000"}[irls](https://cran.r-project.org/web/packages/irls/index.html){style="color: #990000"}[}]{style="color: #990000"} - Generalized Linear Models via Iteratively Reweighted Least Squares. Supports Logistic, Poisson, and Gamma
...
- [{]{style="color: #990000"}[rcppmlpackexamples](https://eddelbuettel.r-universe.dev/rcppmlpackexamples#readme){style="color: #990000"}[}]{style="color: #990000"} - Provides examples for accessing the C++ API of [{mlpack}]{style="color: #990000"} from the C++ side of an R package.
  - [{]{style="color: #990000"}[mlpack](https://cran.r-project.org/web/packages/mlpack/index.html){style="color: #990000"}[}]{style="color: #990000"} - A fast, flexible machine learning library, written in C++, that aims to provide fast, extensible implementations of cutting-edge machine learning algorithms. Uses Rcpp.
...
- [{]{style="color: #990000"}[sqlm](https://usrbinr.codeberg.page/sqlm/){style="color: #990000"}[}]{style="color: #990000"} - Fits linear regression models on datasets residing in SQL databases (DuckDB, Postgres, Snowflake, BigQuery, etc.) without pulling data into R memory
  - Integrates with [{broom}]{style="color: #990000"} and [{orbital}]{style="color: #990000"}
  - Supports all interaction, variable transform, formula, dot expansion etc. `lm` syntax
  - Handles date and categorical variables
  - Can fit multiple models using `dplyr::group_by`
