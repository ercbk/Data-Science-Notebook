...
- Packages
  ...
  - [{]{style="color: #990000"}[deadwood](https://cran.r-project.org/web/packages/deadwood/index.html){style="color: #990000"}[}]{style="color: #990000"} - Outlier Detection via Trimming of Mutual Reachability Minimum Spanning Trees
  ...
  - [{]{style="color: #990000"}[rrobot](https://cran.r-project.org/web/packages/rrobot/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides robust outlier detection techniques for identifying anomalies in multivariate data, with a focus on methods that remain effective under non-Gaussian distributions.
...
- Papers
  - [A test statistic, h ∗ , for outlier analysis](https://arxiv.org/abs/2508.06792)
    - A parametric, frequentist approach for evaluating global outliers without the normality assumption.
    - No code but includes a few relatively straightforward (at first glance) formulas for variations of h\*, the calculation of confidence intervals and degrees of freedom, and presents a table of significance levels
- Resources
...
- Packages
  - [{]{style="color: #990000"}[robscale](https://cran.r-project.org/web/packages/robscale/index.html){style="color: #990000"}[}]{style="color: #990000"} - High-performance C++ implementation (via 'Rcpp') of the robust location and scale M-estimators
  ...
  - [{]{style="color: #990000"}[robusttseq](https://cran.r-project.org/package=robusttseq){style="color: #990000"}[}]{style="color: #990000"} - Provides robust statistical methods for analyzing numeric data, including robust estimation of location and scale using Huber M-estimators and a robust two-sample t-test
  - [{]{style="color: #990000"}[sgmean](https://cran.r-project.org/package=sgmean){style="color: #990000"}[}]{style="color: #990000"} - Calculation of a "trimmed" mean that applies a weighted reduction to boundary values rather than removing them entirely
...
- [Hodges–Lehmann Estimator]{.underline}
  - Packages: [{]{style="color: #990000"}[DescTools::HodgesLehmann](https://andrisignorell.github.io/DescTools/reference/HodgesLehmann.html){style="color: #990000"}[}]{style="color: #990000"}, [{Hmisc::pMedian}]{style="color: #990000"}
  ...
  - Preferred for small data, symmetric distributions, and extreme outliers.
    - Although Harrell likes it for asymmetric distributions
  - Both the classic exact Hodges-Lehmann confidence interval (which assumes symmetry of the population distribution) and the BCa bootstrap method have excellent coverage. ([Harrell](https://www.fharrell.com/post/aci/))
...
- [{]{style="color: #990000"}[kvr2](https://indenkun.github.io/kvr2/){style="color: #990000"}[}]{style="color: #990000"} - Has a robust R^2^ for linear models fitted using data with outliers
- Papers
  - [Outlier-robust Bayesian Multivariate Analysis with Correlation-intact Sandwich Mixture](https://arxiv.org/abs/2508.18004)
- Regression
