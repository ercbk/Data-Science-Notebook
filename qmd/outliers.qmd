...
-   Grubbs's Test
    ...
    -   Test either a maximum or minimum point
        ...
        -   If you suspect multiple points, you have remove the max/min points above/below the suspect point. Then test the subsetted data. Repeat as necessary H0: There is no outlier in the data.

        Ha: There is an outlier in the data.

        Test statistics

        ![](./_resources/Outliers.resources/Screenshot%20(721).png)![](./_resources/Outliers.resources/Screenshot%20(720).png)

        -   Statistics for whether the minimum or maximum sample value is an outlier

    -   The maximum value is outlier if![](./_resources/Outliers.resources/Screenshot%20(722).png)
    ...
    -   `outliers::grubbs.test(x, type = 10, opposite = FALSE, two.sided = FALSE)` x: a numeric vector of data values
...
-   Dixon's Test
    ...
    -   H0: There is no outlier in the data. Ha: There is an outlier in the data.
...
-   Winsorization
    -   A typical strategy is to set all outliers (values beyond a certain threshold) to a specified percentile of the data
      -   [Example]{.ribbon-highligh}: A 90% winsorization would see all data below the 5th percentile set to the 5th percentile, and data above the 95th percentile set to the 95th percentile. 
    -   Packages
...
-   Isolation Forests - see [Anomaly Detection](Anomaly%20Detection)
-   Support Vector Regression (SVR) - see [Algorithms, ML](Algorithms,%20ML) \>\> Support Vector Machines \>\> Regression
...
-   Huber Regression
    -   See [Loss Functions](Loss%20Functions) \>\> Huber Loss
