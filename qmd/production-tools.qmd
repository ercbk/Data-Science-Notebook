...
* ELT/ETL Operations
	* dbt
	* AWS DataBrew
	Model Experimentation/Version Tracking
	* DVC
...
* Overview of some 2021 tools
	Descriptions in [article](https://continual.ai/post/the-modern-data-stack-ecosystem-fall-2021-edition?utm_source=Data_Elixir&utm_medium=social)
	![](./_resources/Production,_Tools.resources/617add909a1e134d1f9e14b9_Modern-20Data-20Stack-20F.png)
...
* Open-source platform for authoring, scheduling, and executing data pipelines.
	* Features for managing and monitoring data pipelines, including integration with various data storage and processing technologies.
		Similar to the Unix cron utility — you write scripts and schedule them to run every X minutes.
	...
	Server-based
		remains active even when not running jobs --> continually incurring cost
...
* Cloud-based data integration service that makes it easy to move data between data stores.
	...
	* Integrates with other AWS services, such as S3 and Redshift, making it a convenient choice for users of the AWS ecosystem.
	...
	Cost
		Charged by Data Processing Unit (DPU) multiplied by usage hours
...
* Monitoring
	* Cloudwatch
	* GlueStudio within Glue![](./_resources/Production,_Tools.resources/0-oFpwsH3cYg7dCw6e.png)
...
* Output
	* "create\_pytrends\_report-deployment.yaml" file and a ".prefectignore" created in the current directory.
		* "create\_pytrends\_report-deployment.yaml":  specifies where a flow’s code is stored and how a flow should be run.
...
* DAGs
	...
	* Manipulate dependencies via drag and drop![](./_resources/Production,_Tools.resources/image.2.png)
		* mage-ai keeps track of the UI changes the user made and automatically builds the dependencies DAG into the YAML file. (./pipelines/{your\_awesome\_pipeline\_name}/metadata.yaml)
	* Visualize data in each block
		* Helpful for inspecting your input data and further validating the transformation.
		* Once the chart has been created, it will also be attached to the current block as the downstream\_blocks.
...
ELT/ETL Operations
**[dbt](https://www.getdbt.com/)** - see [DB, dbt](DB, dbt)
**AWS DataBrew**
...
* **Prometheus**
	...
	* Process![](./_resources/Production,_Tools.resources/promarchitecture.png)
