...
- Packages
  ...
  - [{]{style="color: #990000"}[irlba](https://cran.r-project.org/web/packages/irlba/index.html){style="color: #990000"}[}]{style="color: #990000"} - Fast Truncated Singular Value Decomposition and Principal Components Analysis for Large Dense and Sparse Matrices
  - [{]{style="color: goldenrod"}[open_nipals](https://github.com/johnsonandjohnson/open_nipals){style="color: goldenrod"}[}]{style="color: goldenrod"} ([JOSS](https://joss.theoj.org/papers/10.21105/joss.08598)) - Implements the nonlinear iterative partial least squares (NIPALS) algorithm (in contrast to SVD) for principal component analysis (PCA) and partial least squares (PLS) regression in a scikit-learn compatible fashion
    - The NIPALS algorithm has especially beneficial properties when it comes to peak memory consumption scaling and accuracy.
    - Since it is an iterative algorithm, the runtime requirement can be balanced with the desired accuracy target. In general, it is a favorable option at a small number of latent variables (n_components \< 10) and a high numerical accuracy requirement.
    - Integrates missing value imputation
  ...
  - [{]{style="color: #990000"}[msPCA](https://cran.r-project.org/web/packages/msPCA/index.html){style="color: #990000"}[}]{style="color: #990000"} - Sparse Principal Component Analysis with Multiple Principal Components
    - The algorithm uses an iterative deflation heuristic with a truncated power method applied at each iteration to compute sparse principal components with controlled sparsity.
...
- Creates a subset of variables that maximizes the covariance with the initial variable set, in order to store as much information as possible in a lower dimension.
...
- Also see
  - [Clustering, General \>\> K-Means](clustering-general.qmd#sec-clust-gen-kmeans){style="color: green"} \>\> Example 3
- Packages
  - [{]{style="color: #990000"}[booklet](https://alexym1.github.io/booklet/index.html){style="color: #990000"}[}]{style="color: #990000"} - Rewrite of [{FactoMineR}]{style="color: #990000"} with the goal to make the package more intuitive and easier to use.
  ...
  - [{]{style="color: #990000"}[factoextra](https://rpkgs.datanovia.com/factoextra/index.html){style="color: #990000"}[}]{style="color: #990000"} - Extract and visualize the output of exploratory multivariate data analyses
  ...
  - [{]{style="color: #990000"}[DLMRMV](https://cran.r-project.org/web/packages/DLMRMV/index.html){style="color: #990000"}[}]{style="color: #990000"} - Distributed Linear Regression Models with Response Missing Variables
    - For the PCA-based missing data with high correlation (HC)
  - [{]{style="color: #990000"}[PCAmixdata](https://cran.r-project.org/web/packages/PCAmixdata/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements principal component analysis, orthogonal rotation and multiple factor analysis for a mixture of quantitative and qualitative variables
- Resources
  - [Interactively Exploring High-Dimensional Data and Models in R, Ch. 4](https://dicook.github.io/mulgar_book/4-pca.html)
...
- (Possibly) defaults to using all available cores since it uses BLAS
  - Depends on the R compilation

  - To stop it from using all cores
...
6.  Equation for the ith value of the PC1 vector: $\text{PC1}_i = V_{(,1)} \cdot A_{(i,)}$
    ...
    - [Example]{.ribbon-highlight}: ([source](https://little-book-of-r-for-multivariate-analysis.readthedocs.io/en/latest/src/multivariateanalysis.html#principal-component-analysis))

      ``` r
      dat_wine_stand <- as.data.frame(scale(wine[2:14]))
      pca_wine <- prcomp(dat_wine_stand)    

      # PC1 Loadings; one for each variable
      pca_wine$rotation[,1]
      #>       V2           V3           V4           V5           V6           V7
      ...
      calcpc <- function(.data, .loadings) {
        ...
        # Calculate the value of the component for each observation
        for (i in 1:num_obs) {
          valuei <- 0
          for (j in 1:num_vars) {
            valuei <- valuei + (.loadings[j] * .data[i,j]) # <1>
          }
          pc[i] <- valuei
        }
        return(pc)
      ...
      # PC1
      calcpc(dat_wine_stand, pca_wine$rotation[,1])
      ```

      1.  Calculates the value of the PC which is a linear combination of the loadings and a row of the data frame. The for-loop recursively calculates this sum one (loading value \* variable value) combination at a time.

      - The first principal component represents a contrast between the concentrations of V8, V7, V13, V10, V12, and V14 (negative loadings) and the concentrations of V9, V3 and V5 (positive loadings).

    - $\text{PC2}$ is similar except $V_{(,2)}$ is used, e.g . `pca_wine$rotation[,2]`
...
- Elements in a PC vector are called **scores** and elements in the V eigenvector are called **loadings**.
  - The score plot can reveal distinct groupings and outliers,
  ...
  - A component can be summarized as a contrast between variables with high magnitude positive loadings (if any) and variables with high magnitude negative loadings (if any)
...
- Misc
  - Packages
    - [{]{style="color: #990000"}[booklet](https://alexym1.github.io/booklet/index.html){style="color: #990000"}[}]{style="color: #990000"} - Rewrite of [{FactoMineR}]{style="color: #990000"} with the goal to make the package more intuitive and easier to use.
      - Functions for principal component analysis (PCA) when variables are quantitative, correspondence analysis (CA) when variables are categorical, Multiple Factor Analysis (MFA) when variables are structured in groups.
      - Models compatible with visualization function from factoextra
    - [{]{style="color: #990000"}[BSTFA](https://cran.r-project.org/web/packages/BSTFA/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements Bayesian spatio-temporal factor analysis models for multivariate data observed across space and time
    - [{]{style="color: #990000"}[factoextra](https://rpkgs.datanovia.com/factoextra/index.html){style="color: #990000"}[}]{style="color: #990000"} - Extract and visualize the output of exploratory multivariate data analyses
      - Principal Component Analysis (PCA), Correspondence Analysis (CA), Multiple Correspondence Analysis (MCA), , Hierarchical Multiple Factor Analysis (HMFA), Factor Analysis of Mixed Data (FAMD)
    ...
    - [{]{style="color: #990000"}[factorana](https://cran.r-project.org/web/packages/factorana/index.html){style="color: #990000"}[}]{style="color: #990000"} - A flexible framework for estimating factor models with multiple latent variables.
      - Supports linear, probit, ordered probit, and multinomial logit model components
    ...
    - [{]{style="color: #990000"}[ItemRest](https://cran.r-project.org/web/packages/ItemRest/index.html){style="color: #990000"}[}]{style="color: #990000"} - Automated Item Removal Strategies for Exploratory Factor Analysis
      - Helps researchers identify low-quality items using statistical criteria and simulate the impact of different removal combinations on the factor structure and internal consistency of the scale.
    - [{]{style="color: #990000"}[LFM](https://cran.r-project.org/web/packages/LFM/index.html){style="color: #990000"}[}]{style="color: #990000"} ([Paper](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5036464))- Laplace Factor Model Analysis and Evaluation
      - Facilitates the application of Sparse Online Principal Component (SOPC), Incremental Principal Component (IPC), Projected Principal Component (PPC), Perturbation Principal Component (PPC), Stochastic Approximation Principal Component (SAPC), Sparse Principal Component (SPC)
    - [{]{style="color: #990000"}[projectLSA](https://cran.r-project.org/web/packages/projectLSA/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides an interactive Shiny-based toolkit for conducting latent structure analyses
      - Includes Latent Profile Analysis (LPA), Latent Class Analysis (LCA), Latent Trait Analysis (LTA/IRT), Exploratory Factor Analysis (EFA), Confirmatory Factor Analysis (CFA), and Structural Equation Modeling (SEM)
    ...
    - [{]{style="color: #990000"}[Rnest](https://cran.r-project.org/web//packages/Rnest/index.html){style="color: #990000"}[}]{style="color: #990000"} - Next Eigenvalue Sufficiency Test to determine the number of dimensions to retain in exploratory factor analysis
    - [{]{style="color: #990000"}[sparseGFM](https://cran.r-project.org/web/packages/sparseGFM/index.html){style="color: #990000"}[}]{style="color: #990000"} - Sparse Generalized Factor Models with Multiple Penalty Functions
      - Features cross-validation for regularization parameter selection using Sparsity Information Criterion (SIC), automatic determination of the number of factors via multiple information criteria, and specialized algorithms for row-sparse loading structures.
  - Resources
...
- Misc
  - Packages
    - [{]{style="color: #990000"}[h2o.deeplearning](https://docs.h2o.ai/h2o/latest-stable/h2o-docs/data-science/deep-learning.html){style="color: #990000"}[}]{style="color: #990000"}
      - [Example Script](https://github.com/h2oai/h2o-3/blob/master/h2o-r/tests/testdir_algos/deeplearning/runit_deeplearning_autoencoder_large.R), [Stacked Autoencoder Example Script](https://github.com/h2oai/h2o-3/blob/master/h2o-r/tests/testdir_algos/deeplearning/runit_deeplearning_stacked_autoencoder_large.R)
      - [Autoencoder-specific parameters](https://docs.h2o.ai/h2o/latest-stable/h2o-docs/data-science/deep-learning.html#autoencoder-specific-parameters)
  - Resources
    - Hands-On Machine Learning with R (2020), [Ch. 19](https://bradleyboehmke.github.io/HOML/autoencoders.html)
      - Tutorial for [{h2o}]{style="color: #990000"} encoders
    - [PCA vs Autoencoders for Dimensionality Reduction](https://www.r-bloggers.com/2018/07/pca-vs-autoencoders-for-dimensionality-reduction/)
      - Tutorial with keras coded autoencoder.
