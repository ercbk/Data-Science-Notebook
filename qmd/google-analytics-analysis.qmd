...
* `Example 2` (unique dimension combinations; same)![](./_resources/Google,_Analytics,_Analysis.resources/1-fiYCABwsy7E3q1aSakXhIw.png)
	* The _quick view_ event of "Cap Black" has been fired 1045 times but only 121 of those unique events
	* A view filtered by _Category_, _Action_, and _Label_ has the same values for _unique events_, _sessions_, and _unique dimension combinations_
		* Index = Label
* `Example 3` (unique dimension combinations; different)![](./_resources/Google,_Analytics,_Analysis.resources/1-EB77kQcgcIjxa-SEUo7xBQ.png)![](./_resources/Google,_Analytics,_Analysis.resources/1-OqVnQAsNhVM-8upgWJJhTQ.png)
	* Think this is a breakdown of 1 session/user
	* (Variables or top) The view is filter by _Page_ and _Category, Action, and Label_
		* _Unique Events_ and _Unique Dimension Combinations_ match
	* (right or bottom) The view is filter by _Page_ and only _Category_ and _Action_
		* _Unique Events_ and _Unique Dimension Combinations_ do NOT match
	* Interpretation
		* Left or Top (index = page, event label)
			* Total _Unique Events_ answers, "How many total unique quickviews (event) were there per label per page?"
				* Labels can be repeatedly counted, just not on the same page
			* Total _Unique Dimension Combinations_ answers, ditto
		* Right or Bottom (index = page)
			* Total _Unique Events_ still answers the same question as above as if event label were included in the view
			* Total _Unique Dimension Combinations_ answers, "How many total unique pages was a quickview (event) used?"
* `Example 4` (multiple users)![](./_resources/Google,_Analytics,_Analysis.resources/1-i3A7OH9-tOroT9FBvuALdw.png)
...
* `Example 5` (Index = Device)![](./_resources/Google,_Analytics,_Analysis.resources/1-HME6mi_gzeYnWUHF3NVFVA.png)
		...
		* Steps
			* Create a custom report with Page, Event Category, Event Action, Event Label
				* For Page = "Sock" there are 100 sessions (i.e. the number of people who saw the "Add to Cart" button)
					* This wasn't covered in detail, but in the examples, session always equaled unique dim combos. So I'm guessing this number and the one below aren't on the same lines of the report.
				* For Page = "Sock" and Event Action = "Add to Cart Click" there are 80 Unique Dimension Combinations.
			* Calculate IR = (80 \* 100) / 100 = 80%
	* Average Click Rate
		* Calculate avg\_cr = total events / unique dimension combinations
...
* `Example 8`
	...
	* Steps:
		...
		* For Return criteria, under Others, choose the app\_remove event.
* `Example 9`
	...
	* Solution framework
		...
		4. Segment by demographic, region, device, platform, language, etc.
...
* Outputs: date and users\_installed
...
* Outputs: country and percent\_users
* "\*" - tables come in an intraday format (one table for each day), you can combine several tables in one query using the wildcard symbol
	* Best to combine with `_TABLE_SUFFIX` in order to include a date range, because if you have a lot of data, "\*" can be very expensive to execute without it
...
* BigQuery SQL + Firebase activated App
* Outputs: category and percent\_users
* "\*" - tables come in an intraday format (one table for each day), you can combine several tables in one query using the wildcard symbol
	* Best to combine with `_TABLE_SUFFIX` in order to include a date range, because if you have a lot of data, "\*" can be very expensive to execute without it
...
* Outputs: daily\_active\_users
* Also Reports >> Lifecycle >> Engagement
* DAU calculation depends on definition of Active User
	* Here Active User = a user who did any “user\_engagement” action in the day
...
* BigQuery SQL + Firebase activated App
* Outputs: percent\_users\_7\_days
...
* BigQuery SQL + Firebase activated App
* Outputs: percent\_users\_crashes
...
* `Example 20`
	* Which webpages are viewed most often?![](./_resources/Google,_Analytics,_Analysis.resources/image.png)
...
* View of nesting schema for event\_params![](./_resources/Google,_Analytics,_Analysis.resources/image.1.png)
	* Each event\_param has a key (name of event) and a value
...
* In rows, transition matrix has the “current” page of reference, or `page_title_adjusted` as rows of a Pivot Table, and in columns — a page that follows, or `next_page`. The values are the number of transitions, `record_count` from row to column. (see right side panel) ![](./_resources/Google,_Analytics,_Analysis.resources/image.2.png)
	* CTE column locations
		* page\_title\_adjusted --> unnested\_events\_categorised
		* next\_page --> ranked\_screens
		* record\_count --> not in query
...
* Misc
	* From [Looking for Power User Journeys](https://towardsdatascience.com/looking-for-power-user-journeys-in-e-commerce-746f5f68b697)
	* Grouping by user\_pseudo\_id, visitID constitutes a session
...
* CTE
	...
	* unnested\_events
		* unnests event parameters to get to relevant keys and values
			* See previous example for schema of event\_params
	* unnested\_events\_categorized
		* categorizing Page Titles into PDPs and PLPs
		* Creates a new categorical column, "page\_title\_adjusted," that has value, "PDP," when a substring in "page\_location" is one of a set of words, and "PLP" when it's not, and the value of page\_title otherwise.
			* PLP is the product listing page and PDP is the product details page (i.e. items on the PLP)
			* See [SQL](SQL) >> Strings for details on the various functions that are used
	* ranked\_screens
		* prepares additional data points for analytics to understand transitions between the previous, current and following pages
	...
	* screen\_summary
		...
		* Think ARRAY\_AGG puts all the webpage names in a list/array for each session
	* screen\_summary\_agg
		* ARRAY\_TO\_STRING coerces the list/array of webpage names to a string with a ">>" delimiter between array values
* Final query
	* Creates column for the first time each page is viewed in each session
		* Note that it also groups by "page\_title" instead of just the session variables, "user\_pseudo\_id" and visitID.
...
* `Example 22`
	* Session Conversion Rate/User Conversion Rate Calculation (Page View)
		* From [Looking for Power User Journeys](https://towardsdatascience.com/looking-for-power-user-journeys-in-e-commerce-746f5f68b697)
		* is\_checkout\_session: `CONTAINS_TEXT(screens_on_visit, 'Checkout Confirmation')`
...
* `Example 23`
	...
	* Questions
		...
		* How _their conversion_ looks like vs the users who don’t do what you want them to do
	...
	* Process:
		...
		* Examine conversion range between those that visit the page/use the feature and those that don't
			* Example: Over this period,![](./_resources/Google,_Analytics,_Analysis.resources/image.3.png)
		...
		* When you find a sizable difference between these groups. Ask:
			...
			* Where they come from?
			- [ ] How can you help them connect with the product they may be interested in?
				* Beware: making the page visitation/feature usage _mandatory_ (e.g. a user must visit the login page before purchasing) usually will **decrease** conversion
...
* `Example 24`
	...
	* From [Looking for Power User Journeys](https://towardsdatascience.com/looking-for-power-user-journeys-in-e-commerce-746f5f68b697)
	* Also see [Product Development](Product Development) >> Metrics >> Growth Metrics >> Power User Curve
	...
	* Identify features correlated with conversion (example 23) and moniter these features' power user curves to measure progress in producing more power users and therefore more conversions
		* Can also be broken down be cohorts (see [Algorithms, Product](Algorithms, Product) >> Cohort Analysis)
	* Product Details Pages (PDP)![](./_resources/Google,_Analytics,_Analysis.resources/image.4.png)
		* Around 50% of users in January 2021 viewed 2 PDPs
		* See example 21 >> CTE >> unnested\_events\_categorized for SQL code to create a PDP category. (collapses various product detail webpages into a PDP category)
...
* `Example 25`
	...
	* PLP: % session vs first seen rank during a session![](./_resources/Google,_Analytics,_Analysis.resources/image.5.png)
		* Y axis: % of sessions
		* X axis: "page\_rank\_session\_first\_seen"
		...
		* Interpretation: a large proportion of user sessions are viewing the PLP _very_ early in the user journey, they're likely landing on this page from paid search (e.g. Google)
			* i.e. the ranks with the highest % session values are low ranks (Top 5 ranks are 1,3,4,5,6)
	* Store Search Page: % session vs first seen rank during a session![](./_resources/Google,_Analytics,_Analysis.resources/image.6.png)
	...
	* Checkout page: % session vs first seen rank during a session![](./_resources/Google,_Analytics,_Analysis.resources/image.7.png)
		* Same axis descriptions as in PLP example
...
* `Example 26`: Segment User Purchases as New Customers or Returning Customers
	...
	* Build a base table with orders by customer and date![](./_resources/Google,_Analytics,_Analysis.resources/image.8.png)
...
* Label customers as 'new' or 'returning'![](./_resources/Google,_Analytics,_Analysis.resources/image.9.png)
