...
-   [Example 1]{.ribbon-highlight}: NHIS via Website
    ...
    -   A closer look at the ID variables:
        ...
        -   [NHISHID]{.var-text} - A unique id to each household in a given survey year. Used to link IPUMS NHIS data with additional household-level data elements, from the NHIS source public use data files, that are not currently available in IPUMS.
            ...
            -   If and when the public use data files become available, there is a guide for using these variables to merge data in the User Guide. The link to the User Guide is in left side panel of the NHIS homepate (See [Design Information](surveys-ensus-data.qmd#sec-surv-cens-ipums-di){style="color: green"} for a pic)
...
-   [Example 2]{.ribbon-highlight}: NHIS via [{ipumsr}]{style="color: #990000"}
    ...
    -   Find the collection name of the NHIS survey
        ...
        -   I want NHIS survey, so [nhis]{.arg-text} is what I want
    ...
    -   Submit Request
        ...
        -   The extract won’t be downloadable immediately. They'll notify you via email or you can check via function.
            -   The output can be subset if your script is noninteractive, e.g. `status_sub_nhis$status` which will say "completed" when ready. Other possible values: "queued", "started", "produced", "canceled", and "failed".
    ...
    -   Download Data
        ...
        -   The data file name is `nhis_00002_csv.gz`. Since you can download previously requested extracts (e.g. download_extract("nhis:1") for the first nhis requested extract), I'm the 2 in the file name refers to the fact that this is my second NHIS request.
            -   My history can be checked with `get_extract_history` which retrieves prior extracts, their survey years, and their variables.
