...
-   [Example 4]{.ribbon-highlight}: Median Age Estimates by PUMA with MOE
    ...
    -   [{srvyr}]{style="color: #990000"} automatically calculates the standard errors
    -   Tabulated median ages are not identical to published estimates, but are very close. Only use PUMS calculated statistics if they aren't available in the regular ACS surveys.
...
-   Workaround for Trump's Executive Order that halts access the Census Bureau's shapefiles

    -   Currently [{tigris}]{style="color: #990000"} pulls from the Census Bureau's ftp server, but that's down. The API still works though
    ...
    -   [Example]{.ribbon-highlight}: Census Tracts for Cook County, IL and the 2023 ACS
        ...
        -   Go TIGERweb and find study, year, and geography\
        ...
        -   Choose the desired resolution\
            ...
            -   1 to 5M would be a higher resolution
