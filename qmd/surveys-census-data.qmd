...
# Census and Other Surveys {#sec-surv-cens .unnumbered}

## Misc {#sec-surv-cens-misc .unnumbered}

- Also see [Geospatial, Mapping](geospatial-mapping.qmd#sec-geo-map){style="color: green"}
- Notes from
...
- Packages
  - [{]{style="color: #990000"}[tidycensus](https://walker-data.com/tidycensus/){style="color: #990000"}[}]{style="color: #990000"} - Allows users to easily interface with a select number of the US Census Bureau’s data APIs and return tidyverse-ready data frames, optionally with simple feature geometry included
    ...
    - [{]{style="color: #990000"}[censuschat](https://github.com/smach/censuschat){style="color: #990000"}[}]{style="color: #990000"} - LLM chat that answers census questions
      - Web, console, and programmatic interfaces
      - Besides the natural language answer, it provides:
  ...
  - [{]{style="color: #990000"}[jollofR](https://cran.r-project.org/web/packages/jollofR/index.html){style="color: #990000"}[}]{style="color: #990000"} - Automatic disaggregation of small area population estimates by demographic groups (e.g., age, sex, race, marital status, educational level, etc) along with the estimates of uncertainty, using advanced Bayesian statistical modelling approaches based on integrated nested Laplace approximation (INLA)
  ...
  - [{]{style="color: #990000"}[nlfh](https://cran.r-project.org/web/packages/nlfh/index.html){style="color: #990000"}[}]{style="color: #990000"} - Nonlinear Fay-Herriot Models for Small Area Estimation
    - A two-stage hierarchical model developed in the US Census Bureau to make estimates for populations in geographic regions\\
    - Essentially a random effects model and needs to pass Durbin–Wu–Hausman test (See [Mixed Effects, General \>\> Considerations](mixed-effects-general.qmd#sec-me-gen-consid){style="color: green"} \>\> Fixed Effects or Random Effects?)
    - The parameters of the distributions within the groups are often assumed to be independent, or it is assumed that they are correlated to those measured for another variable.
...
- Resources
  ...
  - [OpenCensus](https://opencensus.us/) ([Thread](https://bsky.app/profile/peternka.bsky.social/post/3mvppjt43yk2o)) - Using LLMs to digitize every pre-1950 U.S. census as free, open data, starting with 1940
    - Should (eventually) overlap with IPUMS and add additional fields that haven't been transcribed before
- State Issues
  - Alaska
...
- [Behavioral Risk Factor Surveillance System (BRFSS)](https://www.cdc.gov/brfss/)
  - Collects prevalence data among adult U.S. residents regarding their risk behaviors and preventive health practices that can affect their health status.
  - CDC’s state-level telephone health survey. CDC established it in 1984, collects data in all 50 states, the District of Columbia, and participating US territories, and completes more than 400,000 adult interviews each year, which makes it the largest continuously conducted health survey system in the world.
  - Packages
    - [{]{style="color: #990000"}[brfssdata](https://muntasirmasum.github.io/brfssdata/){style="color: #990000"}[}]{style="color: #990000"}
...
- [The National Health and Nutrition Examination Survey (NHANES)](https://www.cdc.gov/nchs/nhanes/about/) ([Docs](https://wwwn.cdc.gov/nchs/nhanes/default.aspx), [History](https://archive.cdc.gov/#/details?q=nhanes&start=0&rows=10&url=https://www.cdc.gov/nchs/nhanes/history.htm), [Linkage Table](https://www.cdc.gov/nchs/data/datalinkage/LinkageTable_1.pdf))
  - Collects data about the health and nutritional status of adults and children in the United States.
    - *Easily* available data is from 1999-2023 although the history of the program goes back to the 1960s with different versions of survey throughout (See History).
  - Data about what participants eat, drink, and take as supplements to determine how many nutrients are in their diet.
  ...
  - [{]{style="color: #990000"}[nhanesdata](https://www.kylegrealis.com/nhanesdata/){style="color: #990000"}[}]{style="color: #990000"} - Retrieve pre-processed datasets from reliable cloud storage with automatic type reconciliation and integrated search tools for variables and datasets. Simplifies NHANES data workflows by handling cycle management and maintaining data consistency across survey waves. Builds on [{]{style="color: #990000"}[nhanesA](https://cran.r-project.org/web/packages/nhanesA/index.html){style="color: #990000"}[}]{style="color: #990000"}.
    - All datasets are hosted on reliable cloud storage with fast access, and all survey cycles are already merged. Solves two issues:
      - CDC server reliability issues that break reproducible research
      - Cycle suffix confusion, where finding DEMO, DEMO_B, DEMO_C, all the way through DEMO_L makes data discovery a scavenger hunt
    - Integrates with [{srvyr}]{style="color: #990000"} for analysis
  - [{]{style="color: #990000"}[mapnhanespa](https://cran.r-project.org/package=mapnhanespa){style="color: #990000"}[}]{style="color: #990000"} - Maps physical activity from the National Health and Nutrition Examination Survey ('NHANES') study into population-based quantiles
...
- [Survey of Consumer Finances (SCF)](https://www.federalreserve.gov/econres/scfindex.htm)
  - One of the highest-quality data sources for information on U.S. households’ balance sheets, pensions, income, and demographic characteristics
  - Conducted by the U.S. Treasury Department
  - [{]{style="color: #990000"}[scf](https://cran.r-project.org/web/packages/scf/index.html){style="color: #990000"}[}]{style="color: #990000"}
...
- Also see [Post-Hoc, General \>\> Dichotomous Data \>\> Mean (Probability of Event + CI](post-hoc-analysis-general.qmd#sec-phoc-gen-dd-poe){style="color: green"} \>\> Small/FInite Population \>\> Comparative Study of Confidence Intervals for Proportions in Complex Sample Surveys for CI estimation of small population
