...
-   [Example 2]{.ribbon-highlight}: Interactions
    ...
    -   The converged models had two warnings:
        -   "algorithm stopped at boundary value" which indicates that one or more parameter estimates hit a boundary during optimization.
            -   This likely means some estimates became very large (approaching infinity) or hit computational limits. `summary(m_bin_inter)` and `summary(m_spl_inter)` showed out of both models the largest magnitude of an effect is only 1.99. No extreme parameter estimates likely indicates no separation has occurred. A stable solution was found and the effect estimates should be reliable.
    ...
    -   The cells in the \<10 column have low counts. We could collapse the \<10 category with the 10-30 category, but most of the 10-30 cells already have at least 50% of the counts. Therefore collapsing may not be optimal
    -   Instead, the outer bins are given observations from the inner bin.

    ###### Refit

    ::: {layout-ncol="2"}
    ![Balanced](_resources/Regression,_Logistic.resources/logbin-ex2-modchk-bal-1.png){.lightbox group="logbin-ex2-refit-1" width="632"}

    ![Centered](_resources/Regression,_Logistic.resources/logbin-ex2-modchk-cent-1.png){.lightbox group="logbin-ex2-refit-1" width="632"}
    :::
    ...
    #>                                                  Estimate Std. Error z value Pr(>|z|)  
    #> (Intercept)                                       -0.1912     0.2606  -0.733   0.4633  
    #> passengerClass2nd                                  0.2231     0.2607   0.856   0.3921  
    #> passengerClass3rd                                 -0.5068     0.3229  -1.569   0.1165  
    #> splines::ns(age_cent, df = 3)1                    -0.3164     0.2179  -1.452   0.1464  
    #> splines::ns(age_cent, df = 3)2                    -0.6137     0.5887  -1.043   0.2971  
    #> splines::ns(age_cent, df = 3)3                    -0.7903     0.3922  -2.015   0.0439 *
    #> passengerClass2nd:splines::ns(age_cent, df = 3)1  -0.2732     0.4751  -0.575   0.5653  
    #> passengerClass3rd:splines::ns(age_cent, df = 3)1  -0.3454     0.5510  -0.627   0.5307  
    #> passengerClass2nd:splines::ns(age_cent, df = 3)2  -2.0218     0.8822  -2.292   0.0219 *
    #> passengerClass3rd:splines::ns(age_cent, df = 3)2  -1.9240     1.3772  -1.397   0.1624  
    #> passengerClass2nd:splines::ns(age_cent, df = 3)3  -1.3304     1.2511  -1.063   0.2876  
    #> passengerClass3rd:splines::ns(age_cent, df = 3)3  -1.6697     2.1582  -0.774   0.4391  
    #> ---
    #> Null deviance: 1412.8  on 1044  degrees of freedom
    #> Residual deviance: 1239.2  on 1033  degrees of freedom
    #> AIC: 1263.2
    ...
    #> ----------------------------------------------------------------------------------------------------------------------
    #> m_bin_int_bal  |   glm | 1287.5 (<.001) | 1287.7 (<.001) | 1332.1 (0.009) |           0.175 | 0.457 | 1.000 |    0.607
    #> m_spl_int_cent |   glm | 1263.2 (>.999) | 1263.5 (>.999) | 1322.6 (0.991) |           0.206 | 0.451 | 1.000 |    0.593
    #> 
    #> Name           | Score_log | Score_spherical |   PCP
    #> ----------------------------------------------------
    #> m_bin_int_bal  |  -247.482 |           0.004 | 0.582
    #> m_spl_int_cent |      -Inf |       9.764e-04 | 0.592
    ```

    -   The spline model seems to have a slightly better AICc and R^2^ but slightly worse BIC. Pretty much no difference.
    -   In the model summries, there's very little difference (i.e. in the ten-thousandths decimal place) in the estimates or standaard errors.
    -   For the centered age model, the marginal means, slopes, and their contrasts are the same. Only thing you need to do for any table or visualization is back-transform the centered values back to the orginal ages (add mean to value).
