...
- Sample size requirements
  - [Harrell](https://hbiostat.org/rmsc/lrm.html#sec-lrm-n)
  ...
  - When $n \lt \frac{p}{2}$, there is almost no relationship between true and estimated ORs ([Harrell](https://hbiostat.org/hdata/#required-sample-size-as-a-function-of-p))
  - If $n$ is too low for $p$, see [Harrell's checklist](https://hbiostat.org/hdata/#what-to-do-if-n-is-too-low-for-p).
...
- [Brier Score]{.underline} (See [Diagnostics, Classification \>\> Scores](diagnostics-classification.qmd#sec-diag-class-scores){style="color: green"})
- [Cessie-Houwelingen-Copas-Hosmer Unweighted Sum of Squares Test]{.underline}
  - Seems to be more popular than the Cessie-Houwelingen Test below
    - Probably due to how popular the Hosmer-Lemeshow test was and the paper he wrote recommending this test instead.
  - Packages
    - [DescTools::HosmerLemeshowTest(model, X)](https://andrisignorell.github.io/DescTools/reference/HosmerLemeshowTest.html)
      - [X]{.arg-text} is a matrix (or maybe dataframe) with the covariates of the model and *must* be specified for the proper test (and not the flawed Hosmer-Lemeshow test)
    - Harrellverse: `rms::residuals(rms::lrm(Y ~ X, data = simulated_data, x=TRUE, y=TRUE), type = "gof")`
  - From the example in the [{DescTools}]{style="color: #990000"} docs, p-value \< 0.05 indicates a good fit
- [Cessie-Houwelingen Test]{.underline}
  - Notes from [Introduction to the Cessie-Houwelingen Test Statistic](https://szlo.ai/posts/005_blog_cessie/)

  - A comparison of the observed values to the standardized residuals after weighting them by the inverse of the variance of a kernel smoothing function

    - The kernel smoothing is particularly important as it accounts for the continuous attributes of covariates, enabling a more detailed and nuanced assessment of how well the model performs
  ...
  - Algorithm
    ...
    1.  Standardize the Residuals\
        ...
        - $\pi(X_i)$ is the predicted probability
        - Dividing by the standard deviation of the predicted probability, ${\sqrt{\pi(X_i)(1 - \pi(X_i))}}$, standardizes the residuals

    2.  Apply Kernel Smoothing to the Standardized Residuals\
        ...
        - $K_h$ is the (uniform) kernel function with bandwidth $h$
        - The bandwidth determines the size of the region over which the residuals are averaged and the kernel function determines the weighting

    3.  Calculate the Test Statistic\

        $$T = n \sum_{i=1}^n \hat{r}(X_i)^2 U(X_i)$$

        - $U(x_i)$ is the inverse of the variance of the smoothed residual at $X_i$
        - $n$ is the sample size.

  - Indicates how much we trust the estimate of the smoothed residual at each observation.

    - When squared residuals are weighted by the inverse of their variances, observations with more precise estimates (i.e., lower variance) contribute more to the test statistic, while observations with less precise estimates (i.e., higher variance) contribute less.
  ...
  - Code
    ...
    - Bandwidth is not required but is available for adjustment

    </Details>

  - Guide: p-value \< 0.05 indicates the model does **NOT** fit the data well

    - Given how the p-value is interpreted in the similar test above, might want to double check this from the references in the article or test it out on some sim data (could use that [{DescTools}]{style="color: #990000"} example data)
- [Residual Deviance (G2)]{.underline}
  - -2 \* LogLikelihood(proposed_mod)))
- Null Deviance
  - -2 \* LogLikelihood(null_mod)))
  - i.e. deviance for the intercept-only model
- [McFadden's Pseudo R^2^]{.underline} = (LL(null_mod) - LL(proposed_mod)) / LL(null_mod))
  ...
  - [Example]{.ribbon-highlight}: Getting the p-value
    ...
    ```
- [Compare nested models]{.underline}
  - Models
...
- Outcome vs Predictor with Logistic Curve
  ...
  - [Example 2]{.ribbon-highlight}: ([source](https://aditya-dahiya.github.io/projects_presentations/data_vizs/tidy_pima_diabetes_study.html))\
    ![](_resources/Regression,_Logistic.resources/viz-out-pred-rugs-by-cat.png){.lightbox width="386"}
