...
-   [Example 1]{.ribbon-highlight}: GAM with Varying Intercepts ([source](https://freerangestats.info/blog/2024/12/23/depression-and-vote))\
    ...
    votes_dep_dat <- mutate(votes_dep_dat, 
    ...
    summary(mod_quasi_bin_inter$gam)
    ...
    -   The `gam` function can be used to fit this model by using `s(state_name, bs = "re")`, but it's better to use `gamm`. `gamm` fits the model by iterating back and forth between `nlme::lme` and `gam`.

        -   Note that it's possible to use `gamm` + `s(var, bs = "re")` to fit the model. This results in fewer effective degrees of freedom (edf) used to fit the spline (i.e. smoother curve) than with `gam`. But it's recommended to fit the random effects using the [random]{.arg-text} argument which feeds the specification to `lme` which is a much faster fit.

        -   The effect size is also slightly lower (15.7091 vs 16.3333) but it's the same model
