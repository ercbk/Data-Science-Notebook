...
-   [**Sharding**]{style="color: #009499"} - Horizontal Partitioning --- divides the data horizontally and usually on different database instances, which reduces performance pressure on a single server.\
    ![](./_resources/DB,_Engineering.resources/1-UQi2bsYKzzuKHv4WWkEW1A.png){.lightbox width="532"}
...
-   Tools:
    -   [Apache Kafka]{.underline} - Flexible, connects to app servers, other microservices, databases, sensor networks, financial networks, etc. and can feed the data to same types of systems including analytical tools.
    ...
    -   [Apache Flink]{.underline} - Processes streaming data with lower latency than Spark Streaming, especially at high throughputs. Less likely to duplicated data. Uses SQL. Steeper learning curve given its more advanced features.
