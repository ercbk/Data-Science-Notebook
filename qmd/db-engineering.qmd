...
-   Packages
    -   [{]{style="color: goldenrod"}[daft](https://github.com/Eventual-Inc/Daft){style="color: goldenrod"}[}]{style="color: goldenrod"} - A distributed query engine for large-scale data processing using Python or SQL, implemented in Rust.
        -   [Example 1]{.ribbon-highlight}: Lazily aggregate delta lake files in AWS S3 ([source](https://dataengineeringcentral.substack.com/p/650gb-of-data-delta-lake-on-s3-polars))
        ...
        -   [Example 2]{.ribbon-highlight}: Process parquet files in S3 w/batches ([article](https://dataengineeringcentral.substack.com/p/1tb-of-parquets-single-node-benchmark?utm_source=multiple-personal-recommendations-email&utm_medium=email&triedRedirect=true), [repo](https://github.com/danielbeach/duckdbAndDaftEat1TB/blob/main/daft_main.py))
            ...
            -   Runs a query on 1TB of parquet files in a S3 bucket using compute with 16 CPUs and 64 GB of RAM. Took almost 30 min. (vs 20 min with duckdb)

            </Details>
    -   [{]{style="color: goldenrod"}[dlt](https://dlthub.com/){style="color: goldenrod"}[}]{style="color: goldenrod"} ([Docs](https://dlthub.com/docs/intro)) - Moves data from source (mysql, API, S3 bucket, etc.) to a storage destination (duckdb, snowflake ([article](https://seifert.substack.com/p/can-you-run-dlt-inside-snowflake)))
...
-   The 80/20 rule for data pipelines: 80% of your runtime typically comes from database queries, network I/O, disk I/O, and inefficient data processing patterns.
    -   Python/R code execution? Usually under 5% of total runtime. Loops and refactoring functions typically only run once per pipeline.

    -   Meanwhile, a single unoptimized SQL query burns compute credits and delays every downstream job.

    -   Simple timing of pipeline components can usually help you discover the bottleneck
...
-   [**Backfilling**]{style="color: #009499"} - The process of retroactively processing historical data through a data pipeline that was designed for processing current or streaming data. Typically, this is a separate flow sending data into your existing tables. ([source](https://docs.databricks.com/aws/en/ldp/flows-backfill))
...
-   [**CRUD**]{style="color: #009499"} - Refers to the four basic functions of data management in databases: Create (add new data), Read (retrieve existing data), Update (modify existing data), and Delete (remove data).
...
-   [**Schemas**]{style="color: #009499"}
    ...
    -   Database Schema - A term for a collection of database objects. These are generally used to logically separate data within the database and apply access controls.
        ...
        -   Stores all tables related to a project
...
-   [**Time Travel**]{style="color: #009499"} - The ability to query your data as it existed at any previous point in time.
...
-   Also see
    ...
    -   [Databases, BigQuery \>\> Optimization](db-bigquery.qmd#sec-db-bigqZ-opt){style="color: green"}
    -   [SQL \>\> Best Practices](sql.qmd#sec-sql-bestp){style="color: green"}

-   Optimize or not?\
    ![](_resources/DB,_Engineering.resources/xkcd-optimize-or-not-1.png){.lightbox}

    -   Runtime Impact
        -   If Impact \< 2 hours/week saved: Don't optimize yet.
        -   If Impact \> 1 day/week saved: Optimize now.
        -   If you're being yelled at: Optimize now
    -   Pipeline Frequency
        -   Once a day? Optimization ROI is probably low
        -   Every 5 minutes? The sweet spot
        -   Ad-hoc analysis? Don't bother
    -   How long does it take for the pipeline to run?
        -   \<5 minutes for job than runs hourly? That's fine
        -   4 hours for a daily overnight job? Also probably fine
        -   15 minutes for something users are waiting on? Problem
    -   Business Criticality
        -   Locks downstream pipelines? High impact
...
-   Common Bottlenecks
    ...
    -   Cartesian joins
    -   Loading data just to filter it in Python/R
...
-   Avoid disk operations, make sure that you look out for hints & information in the `EXPLAIN PLAN` of your query. (e.g. using SORT without an index)\
    ...
    -   Also see
        ...
        -   [SQL \>\> Functions](sql.qmd#sec-sql-funs){style="color: green"} \>\> EXPLAIN, EXPLAIN ANALYZE
...
-   [Indexes]{.underline}
    ...
    -   [Cluster](https://www.postgresql.org/docs/current/sql-cluster.html) a table according to an index
        ...
        -   Also see [Databases, BigQuery \>\> Optimization](db-bigquery.qmd#sec-db-bigqZ-opt){style="color: green"} \>\> Partition and Cluster
...
## Pipelines and Architectures {#sec-db-eng-piparch .unnumbered}

-   Also see

    -   [Streaming](db-engineering.qmd#sec-db-eng-stream){style="color: green"} \>\> Architectures
    -   [Databases, Warehouses \>\> Pipeline Strategies](db-warehouses.qmd#sec-db-ware-pipstrat){style="color: green"}
    -   [Databases, Lakes \>\> Lakehouses](db-lakes.qmd#sec-db-lakes-lkhs){style="color: green"} \>\> Pipeline

-   Notes from

    -   [10 Pipeline Design Patterns for Data Engineers](https://pipeline2insights.substack.com/p/10-pipeline-design-patterns-for-data)

-   Resources

    -   Bookmarks (Data \>\> Storage \>\> Pipelines)
    -   R \>\> Documents \>\> Articles \>\> 2024-Data-Pipeline-Summer

-   [ETL (Extract, Transform, Load)]{.underline}\
    ![](_resources/DB,_Engineering.resources/piparch-etl-1.webp){.lightbox width="432"}
...
-   [ELT (Extract, Load, Transform)\
    ]{.underline}![](_resources/DB,_Engineering.resources/piparch-elt-1.webp){.lightbox width="432"}
...
-   [EtLT (Extract, Transform, Load, Transform)]{.underline}\
    ![](_resources/DB,_Engineering.resources/piparch-etlt-1.webp){.lightbox width="432"}
...
-   [Data Mesh]{.underline}\
    ![source](_resources/DB,_Engineering.resources/term-data-mesh-1.webp){.lightbox alt="source" width="432"}

    -   Decentralized design where data is owned and managed by teams across the organisation that understands it the most, known as domain-driven ownership.
    ...
    -   Raw data is stored in a shared data lake using open formats (e.g., Parquet), and teams process it independently into analysis-ready datasets.
    -   Benefits:
...
-   Architectures
    ...
    -   Streaming\
        ![](_resources/DB,_Engineering.resources/stream-arch-stream-1.webp){.lightbox width="432"}
    ...
    -   Lambda\
        ![](_resources/DB,_Engineering.resources/stream-arch-lambda-1.webp){.lightbox width="432"}
    ...
    -   Kappa\
        ![](_resources/DB,_Engineering.resources/stream-arch-kappa-1.webp){.lightbox width="432"}
