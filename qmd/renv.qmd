...
-   Packages
    ...
    -   [{]{style="color: #990000"}[rv](https://a2-ai.github.io/rv-docs/){style="color: #990000"}[}]{style="color: #990000"} - Alternative to [{renv}]{style="color: #990000"}, written in Rust, similar to [{uv}]{style="color: goldenrod"}
        -   Articles
            ...
            -   [R the Software Engineering Way: Chapter 2](https://deadsimpletech.com/blog/r_the_software_way_2)
        -   Comparison
            -   Package Management
            ...
            -   Dockerfile for Ubuntu ([source](https://deadsimpletech.com/blog/rv_in_docker))
                ...
                -   `RUN mv ~/.local/bin/rv /usr/local/bin/rv`
                    -   rv installs itself in a local user directory and adds itself to PATH using the `.bashrc` file (or the equivalent if you're using zsh or fish), but inside a container, PATH doesn't act as expected when certain infrastructure isn't installed.
                    -   Therefore it's easier to copy the rv executable to where all the other pertinent executables are.
                -   `rv sync` installs exactly the same versions of exactly the same packages that you have installed locally into your container.
                -   `rv activate` initiates the project
...
-   Shiny
    -   Spatial packages (terra, raster) require explicit declaration in DESCRIPTION - Indirect dependencies must be captured in `renv.lock` with `renv::snapshot` ([source](https://stevenponce.netlify.app/projects/standalone_visualizations/sa_2025-12-15.html))
