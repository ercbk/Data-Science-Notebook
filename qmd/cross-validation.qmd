...
- Packages
  ...
  - [{]{style="color: #990000"}[correctR](https://hendersontrent.github.io/correctR/){style="color: #990000"}[}]{style="color: #990000"} - Corrected test statistics for comparing machine learning models on correlated samples
    - It answers the question: "Model A's average score is better than model B's average score — is that difference in average scores statistically significant?"
    - For model comparison, it allows you to perform a t-test on fold metric scores.
      - Along with k-fold cv, it can perform t-tests on repeated k-folds and resamples.
  - [{]{style="color: #990000"}[modeltuning](https://cran.r-project.org/web/packages/modeltuning/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides a lightweight framework for model selection and hyperparameter tuning in R.
    - Offers intuitive tools for grid search, cross-validation, and combined grid search with cross-validation that work seamlessly **with virtually any modeling package**.
    - Vignette on scaling with [{paws}]{style="color: #990000"} and an EC2 instance
- Papers
  - [Irredundant k-Fold Cross-Validation](https://arxiv.org/abs/2507.20048)
    - Each sample is used once for training and once for testing
    - Performs a tiny bit worse than tradtitional k-fold, but it is substantially less computationally intensive. On average, it reduces the total validation time by a factor of approximately 2.9 compared to traditional k-fold.
      - Experiment included datasets with sizes from 1,385 samples to 19,020 samples
    - No code but the algorithm and concept seems clear enough to replicate.
...
- Packages
  ...
  - [{]{style="color: #990000"}[nestedcv](https://cran.r-project.org/web/packages/nestedcv/index.html){style="color: #990000"}[}]{style="color: #990000"} - Implements nested k\*l-fold cross-validation for **lasso** and **elastic-net** regularised linear models via the {glmnet} package and other **machine learning models** via {caret}.
    - Cross-validation of '**glmnet' alpha** mixing parameter and embedded fast filter functions for feature selection are provided.
    - Implements a method using outer CV to measure unbiased model performance metrics when fitting **Bayesian linear and logistic regression shrinkage models** using the horseshoe prior over parameters to encourage a sparse model
...
- Misc
  - Packages
    - [{]{style="color: #990000"}[modernBoot::moving_block_boot](https://cran.r-project.org/web/packages/modernBoot/refman/modernBoot.html#moving_block_boot){style="color: #990000"}[}]{style="color: #990000"} - The moving block bootstrap divides the time series into overlapping blocks of length [block_size]{.arg-text} and resamples these blocks with replacement. This preserves short-range dependence while allowing the empirical sampling distribution to reflect dependence.
  - Resources
    - [Cross-Validation using {forecast}](https://robjhyndman.com/hyndsight/tscv/)
...
- Nested Rolling-Origin Cross-Validation
  - A nested cross-validation strategy tailored to time series data, employing a rolling-origin scheme to prevent look-ahead bias
  - [Code](https://github.com/cgeraldes/BTWAR), [Paper](https://www.mdpi.com/2227-7390/14/3/479) for an AR package that uses it. Need go through it and see if I generalize it for use with other models.

## Spatial and Spatio-Temporal

- Misc
  - Packages
    - [{]{style="color: #990000"}[mlr3spatiotempcv](https://mlr3spatiotempcv.mlr-org.com/){style="color: #990000"}[}]{style="color: #990000"} - Extends the **mlr3** machine learning framework with spatial and spatio-temporal resampling methods to account for the presence of spatiotemporal autocorrelation (STAC) in predictor variables
...
- KNNDM
  - [kNNDM CV: k-fold nearest-neighbour distance matching cross-validation for map accuracy estimation (Linnenbrink et al 2024)](https://gmd.copernicus.org/articles/17/5897/2024/)
  - Spatial CV method that matches folds to the prediction scenario using distance structure (either in geographic or predictor space)
  - Available in [{mlr3spatiotempcv}]{style="color: #990000"}
  - Performs well in various spatial distributions ([source](https://jakubnowosad.com/ml4eo2026workshop/docs/workshop.html#/section-6))\
    ![](_resources/Cross-Validation.resources/spat-knndm-spatdisteval.png){.lightbox width="632"}
