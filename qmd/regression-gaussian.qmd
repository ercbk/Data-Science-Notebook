...
* "_For estimation of causal effects, it does typically not suffice to well control for confounders, we also need a sufficiently strong source of exogenous variation for our explanatory variable of interest."_
	* from http://skranz.github.io//r/2021/01/18/EmpEconB.html?utm_source=feedburner&utm_medium=feed&utm_campaign=Feed%3A+skranz_R+%28Economics+and+R+%28R+Posts%29%29
	* Even if you have good proxy variables for the true confounder(s) and use them as controls, estimated effects of explanatory variables with low s.d. (with mean = 0, e.g. low 0.02, high 1) will still be inflated
	If the regression conditions aren't met - for instance, if heteroskedasticity is present - then the OLS estimator is still unbiased but it is no longer best. Instead, a variation called generalized least squares (GLS) will be Best Linear Unbiased Estimator (BLUE)
...
* Assumptions
	...
	* Homoscedasticity of errors (or, _equal variance_ around the line)
...
* Preliminary 
	...
	4. remove predictors with zero variance recipes::step\_zv(all\_predictors()).
...
* Outcome variable is multi-modal
	* [Regression, Other](Regression, Other) >> Multi-Modal
	* Solutions:
		* Quantile Regression
		* Mixture models
...
* Misc
	* see [Feature Engineering, General](Feature Engineering, General) >> Numeric >> Transformations
* Standardization
	
	* ns have first been centered by their means
	...
	* Methods
		...
		* centering and scaling
			* only for numeric predictors
		* median and MAD
			* only for numeric predictors
			* robust
		* mean and (2 \* std dev)
			* Gelman (2008)
			* for numerics and factors
...
1. Harrell says use all variables unless number of variables , p  > (nrow(df) / 15), then use Regularized Regression
2. See Harrell sample size biostat paper (above). Has overview of approaches (I think).
3. Use best subsets with Mallow's CP, AIC, and BIC as metrics for choosing best model. See R notebook, pg 121 and bkmk regression --> multivariable folder for PSU stat 501
...
* Intercept
	...
	* If variables are _not_ centered so that the predictors have mean 0, this is the expected value of Yi when the predictors are set to 0.
...
* Standardized predictors
	...
	* Coefficients
		* _similar_ to Pearson Correlation Coefficients (r-scores)
	...
	* Does one coefficient have a larger effect than the other?
		* Example: does extra hours work have a statistically larger effect on salary than does number\_compliments\_to\_the\_boss
			* from https://shouldbewriting.netlify.app/posts/2021-02-16-testing-coefficients-equality/
...
* quadratic
	...
	* β1 and β2 are linear and square components of the regression curve, respectively
