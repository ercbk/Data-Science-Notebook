...
## Misc {#sec-doc-misc-misc .unnumbered}

- Packages

  - [{]{style="color: #990000"}[containr](https://erwinlares.github.io/containr/){style="color: #990000"}[}]{style="color: #990000"} - Containerize Your 'R' Project
    - The core function, `generate_dockerfile`, analyzes an 'R' project's environment and dependencies via an [{renv}]{style="color: #990000"} lock file and generates a ready-to-use 'Dockerfile' that encapsulates the computational setup.
  ...
  - [{]{style="color: #990000"}[sysreqr](https://choxos.github.io/sysreqR/){style="color: #990000"}[}]{style="color: #990000"} - Helps R users on GNU/Linux find the system packages they need before, or after, an R package installation fails.
    - It runs offline by default, generates portable shell, Docker, and CI commands, and never edits operating system state itself; the user stays in control.
...
- Resources
  ...
  - [DevOps for Data Science, Ch.6](https://do4ds.com/chapters/sec1/1-6-docker.html)
  - [Reproducible analytical pipelines with R, Ch.14](https://raps-with-r.dev/repro_cont.html)
...
- Docker images built on arm64 chips (aka apple silicon on Macs) won't run on amd64 chips (i.e. intel, amd) which a lot of cloud compute uses, so you have to build a multiplatform image or an image for a specifc brand of chip.

  - [Example]{.ribbon-highlight}: ([source](https://aeturrell.com/blog/posts/build-a-cloud-api/build-a-cloud-api.html#building-the-docker-image-for-google-cloud-run))
...
## BuildKit {#sec-doc-misc-bldkit .unnumbered}
...
## Multi-Stage Dockerfile {#sec-doc-misc-mstage .unnumbered}
...
## Optimizations {#sec-doc-misc-opt .unnumbered}
...
## Snippets {#sec-doc-misc-snip .unnumbered}

- Model API ([source](https://do4ds.com/chapters/sec1/1-6-docker.html#lab-putting-an-api-in-a-container), [repo](https://github.com/alexkgold/do4ds/tree/main/_labs/lab6))
  1.  API using [{vetiver}]{style="color: goldenrod"}
      ...
      - Will load model after container starts
  ...
  2.  Dockerfile
      ...
      - [{vetiver}]{style="color: goldenrod"} can produce this automatically
      - See repo for `requirements.txt`
  ...
  3.  Build and Run Container
      ...
      1.  A volume is necessary because the model lives at `/data/model` on our host machine. But the API inside the container is looking for `/data/model` *inside* the container. We need to make sure that the directory exists and has the model in it.
