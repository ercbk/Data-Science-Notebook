...
-   `{split}` similar to the `{multline}` environment, however, split allows for alignment using `&`, and must be used inside another math environment such as `{equation}`.
-   No idea how to get the top equation flush with the left side
