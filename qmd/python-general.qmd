...
- Misc
  ...
  - T-Strings (v3.14+)
    - [Video](https://pym.dev/t-strings-in-python/)

    - Output is a template object instead of a string (like F-strings)

    - Allow a utility to pre-process the interpolations within the template, in whatever way they'd like, before actually making a string.
...
- Assignment\
  ![](./_resources/Python,_General.resources/image.7.png){.lightbox}
...
### Misc {#sec-py-gen-uv-misc .unnumbered}

- [Github](https://github.com/astral-sh/uv), [Docs](https://docs.astral.sh/uv/)

- Integration Guides: [Docker](https://docs.astral.sh/uv/guides/integration/docker/), [Jupyter](https://docs.astral.sh/uv/guides/integration/jupyter/), [marimo](https://docs.astral.sh/uv/guides/integration/marimo/), [pre-commit](https://docs.astral.sh/uv/guides/integration/pre-commit/), [GitHub Actions](https://docs.astral.sh/uv/guides/integration/github/), [GitLab CI/CD](https://docs.astral.sh/uv/guides/integration/gitlab/), [alternative package indexes](https://docs.astral.sh/uv/guides/integration/alternative-indexes/), [PyTorch](https://docs.astral.sh/uv/guides/integration/pytorch/), [FastAPI](https://docs.astral.sh/uv/guides/integration/fastapi/), [AWS Lambda](https://docs.astral.sh/uv/guides/integration/aws-lambda/)
...
### Operations {#sec-py-gen-uv-ops .unnumbered}

- Update uv: `uv self update`

- Installing one or more specific versions: `uv python install <version, constraints, etc.>`

  - e.g. `uv python install pypy3.8`
  - To uninstall: `uv python uninstall 3.8`

- List all available installations: `uv python list`

- Return path of python executables: `uv python find 3.8`

  - Returns the path of python 3.8

- Start project:

  - Create directory and add project files: `uv init hello-world`
    - Or add project files in the working directory: `uv init`
    - Creates a `pyproject.toml`, a git repo (with Python-specific `.gitignore`), a `README.md`, `.python-version` and a `main.py` files by default
  - Add a dependency: `uv add requests`
    - Creates `.venv` and `uv.lock` files
    - Adds requests library to the dependencies section in `pyproject.toml` and in `renv.lock`
    - The `renv.lock` is cross platform, so you can dev on windows and deploy on linux
...
- Add and Remove Dependencies:
  ...
  - Updates the lockfile and project environment

- List dependencies in lock file

  ``` bash
  uv tree --depth=1
  #> sandbox v0.1.0 (virtual environment)
  #> ├── numpy v2.3.5
  #> ├── polars v1.35.2
  #> ├── scikit-learn v1.7.2
  #> └── statsmodels v0.14.5
...
- Update a library: `uv lock --upgrade-package requests`

  - It will attempt to update the specified package to the latest compatible version, while keeping the rest of the lockfile intact.
  - To a specific version: `uv lock --upgrade-package requests==2.31.0`

- Spin up a quick REPL with a [temporary python version](https://valatka.dev/2025/01/12/on-killer-uv-feature.html) and packages

  - e.g `uv run --python 3.12 --with pandas python`

- Create a package from your project: `uv build`

  - Builds a binary `.whl` file

- Initialize a package project: `uv init --package lifeweeks`
  ...
  - See [uv, part 2: building and publishing packages](https://blog.stephenturner.us/p/uv-part-2-building-and-publishing-packages) for details on how to publish to PyPI

- Scripts

  - Run a script or command: `uv run example.py`

    - Runs any command/script in the venv automatically, even if it's not activated
    - Run jupyter in the current project: `uv run --with jupyter jupyter notebook`
      - Doesn't add jupyter and its dependencies to the project
    - Run with a specific version of Python: `uv run --python 3.9.21 main.py`
    - Run with a package(s) that you've added to a script (i.e. `import rich`) but haven't added as a dependency: `uv run --with rich --with requests --python 3.9 main.py`

  - [--script]{.arg-text}

    - Add this flag to other commands for script-specific operations
    ...
    - [Example]{.ribbon-highlight}: `uv add` ([source](https://natknight.xyz/til-uv-script))

      - You want to add a dependency to `foo.py`
      ...
      - After running `uv add --script foo.py numpy`, your script becomes
...
- Install and Run a Tool from CLI

  - Notes from [uv, part 1: running scripts and tools](https://blog.stephenturner.us/p/uv-part-1-running-scripts-and-tools)
  ...
  - Run Voila on a Jupyter Notebook

    ``` bash
    uvx --with jupyter,numpy,bqplot voila bqplot.ipynb
    ```

    - Since [{voila}]{style="color: goldenrod"} requires jupyter and the packages imported in the notebook to create the web app, [--with]{.arg-text} is required along with extra dependencies.
...
- Misc
  - [Reference](https://docs.python.org/3/library/venv.html), [Basics](https://docs.python.org/3/tutorial/venv.html)
  - Typically shipped with Python
  - Don't mix pip, venv and Anaconda. Avoid Anaconda if you can. If you have to use Anaconda, don't use pip and venv. Limit yourself to Anaconda tools.
- Create
  - Windows: `python3 -<py version> -m venv <env name>`
  ...
  - Mac/Linux: `python3.8 -m venv <env name>`
...
- Remove a venv: `rm -rf <env name>` in the project folder with the env_name folder in it.
...
- [Type Hinting]{.underline}
  ...
  - Resources
    ...
    - Video: [Gimme a (Type) Hint in Python](https://www.youtube.com/watch?v=E2d-tMRFDN8&ab_channel=MattLayman)
...
- Pre-allocate memory to optimize performance of for-loops
  ...
  # good
  result = np.zeros(10)  # Pre-allocate

  for i in range(10):
      result[i] = i ** 2 
  ```

  - Also see [Python, Numpy \>\> Create or Coerce](python-numpy.qmd#sec-py-numpy-crecoe){style="color: green"} \>\> Create an empty array for specifying columns and types for even more efficiency
...
- `try` + `except`
  ...
  - Also see plenty of examples in other notes. Search "try:"
