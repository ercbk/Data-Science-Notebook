...
-   Return path of python executables: `uv python find`

-   Start project:

    -   Create the directory: `uv init hello-world`
    -   Create in the working directory: `uv init`
    -   Creates a `.venv`, `pyproject.toml`, a git repo (with Python-specific `.gitignore`), a `README.md`, `uv.lock`, and a `main.py` by default
    -   The lock file is cross platform, so you can dev on windows and deploy on linux
