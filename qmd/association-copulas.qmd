# Copulas {#sec-assoc-cop .unnumbered}

## Misc {#sec-assoc-cop-misc .unnumbered}

-   Used to create joint distributions that can be used to describe associated variables that may not be from the same distribution. The joint distribution captures a specified dependence structure (e.g. asymmetrical or tail correlation) between variables.
...
-   General use-case pattern: fit a copula to correlated real-world variables, then sample from it to run simulations. For example:
    ...
    -   Feed values for your economic predictors that indicate an economic state (e.g. recession) to your model and forecast the response to see how that group of stocks reacts.
    -   Used in finance for non-linear and tail risk qualities that currently don't take autocorrelation into account
...
-   Probability Integral Transform (PIT) — the key mechanism underlying Sklar's Theorem:
    -   Let $X$ be continuous with CDF $F_X(x)$, then $F_X(X) \sim \mathcal{U}(0,1)$ — says any continuous random variable can be transformed to a uniform one by plugging it into its own CDF.
    -   Conversely, $P(F_X^{-1}(U) \le x) = F_X(x)$ — says plugging a $\mathcal{U}(0,1)$ variable into the quantile function $F_X^{-1}$ produces a draw from the original distribution.
    -   This is why each $u_i$ in the copula formula is the result of passing your data through its ECDF, always yielding $\mathcal{U}(0,1)$ values.
-   For two random variables $S_1$, $S_2$ in $[-\infty, \infty]$. $S_1$ and $S_2$ have their own fixed, continuous CDFs, $F_1$, $F_2$.
    ...
    -   Now take the uniformly distributed quantile random variable, $U_1(S_1)$, $U_2(S_2)$. For every pair, $(u_1, u_2)$, drawn from the pair's quantile, we define the \*bivariate copula, $C: [0,1] \times [0,1] \rightarrow [0,1]$ as:
        ...
        -   Where $F_1^{-1}$ and $F_2^{-1}$ are inverses (i.e. solved for S) of the marginal CDFs, $F_1$ and $F_2$. Each CDF of your data is called a "marginal" (short for marginal distribution).
...
-   [Elliptical]{.underline}
    ...
    -   Gaussian - uses Gaussian inverse CDF and a correlation matrix\
        $$
        C_{\text{gauss}}(u_1,\ldots,u_m;R) = \Phi_R(\Phi^{-1}(u_1),\ldots,\Phi^{-1}(u_m))
        $$
        -   $\Phi_R$ is the joint CDF of the multivariate Gaussian such that\
            $$
            \begin{align}
            &\mu = 0, \Sigma = R \\
            &\mbox{diag}(R) = 1
            \end{align}
            $$
        -   $\Phi^{-1}$ is the quantile function of the standard Gaussian
        -   Note: each $u_i$ in this formula is NOT a raw data variable. It is the result of passing a data variable through its ECDF, which by the PIT always yields a $\mathcal{U}(0,1)$ value. So the formula is equivalent to $\Phi_R(\Phi^{-1}(F_1(X_1)), \ldots, \Phi^{-1}(F_d(X_d)))$ where $X_i$ is a data variable and $F_i$ its ECDF.
...
-   [Example]{.ribbon-highlight}
    ...
    -   Defining a Gaussian Copula for these 2 random variables\
        $$
        P(X\le x, Y\le y) = \Phi_R(\Phi^{-1}(F_X(x)), \Phi^{-1}(F_Y(y)))
        $$
