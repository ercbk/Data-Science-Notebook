    ...
    -   [System and Software Requirements](https://www.tensorflow.org/install/pip#windows-wsl2)
        ...
        -   On Windows (not WSL2)
            -   Check NVIDIA driver version
                ...
                -   I did not check or do anything about:

                    -   CUDA® Toolkit 12.3
                    -   cuDNN SDK 8.9.7
                    -   *(Optional)* TensorRT to improve latency and throughput for inference
            -   Install the Latest Microsoft Visual C++ Redistributable version
                -   I don't think this is required for WSL2 installations, but the video guy installed it, so I will too.
    -   [Misc \>\> Update Python \>\> Linux](misc.qmd#sec-misc-updatepy-lin){style="color: green"} to update Python on WSL2
-   Python
...
-   Process
    ...
    -   Install TensorFlow
        ...
        -   It will download and install a bunch of packages includind CUDA packages, keras, and tensorflow. The tensorflow takes an relatively long time to install (5min?) to the point in makes you question everything is okay.

        -   The second command will print some additional stuff that I don't understand, but the last line is what's important. It should say "GPU".
...
-   R
    ...
    -   Inside the activated virtual environment (e.g. [tf_env]{.var-text}), start R, Then, install packages and register the R kernel with Jupyter.

        ``` r
        install.packages(c("IRkernel", "reticulate"))
        install.packages("tensorflow") # make sure this installs last
        ...
        -   You won't get an output after registering, but you can check that kernel got registered inside bash: `jupyter kernelspec list`.
    ...
    -   Inside the venv, goto your desired project directory, and open up VSCode/Positron, `code .` / `positron .` .
