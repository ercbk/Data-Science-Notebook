...
- Current Page: pg 105 (124) chapter 4.1
  - In python, pg 97 (122) chapter 4.1
- Also see [Deep Learning, General \>\> Basic Feed Forward](https://ercbk.github.io/Data-Science-Notebook/qmd/dl-general.html#sec-dl-gen-bff){style="color: green"}
  - Shows the basic structure and has an explainer on gradient descent and backpropagation.
- Components
...
- **Learning** - Finding the optimal values for the weights ($w_i$) and biases ($b_i$) in a neural network.

- **Batch** (or **Mini-Batch**) - A small set of samples (typically between 8 and 128) that are processed simultaneously by the model.
...
- **Multilabel Classification** - A classification task where each input sample can be assigned multiple labels.
  ...
  - For instance, a given image may contain both a cat and a dog and should be annotated with both the “cat” label and the “dog” label. The number of labels per image is usually variable.
...
- A **gradient** is the derivative of a tensor operation (or tensor function). Gradients are just the generalization of the concept of derivatives to functions that take tensors as inputs.\
  ...
  - Another way of saying that is “If you added 1 to x before plugging it into the function, this is how much w would change, if the function was a straight line”
    - Essentially the way regression coefficients are interpretted.
  - The gradient of a tensor function represents the curvature of the multidimensional surface described by the function
  - For a specific point, the gradient is a vector that points in the direction of the biggest increase in the function, or equivalently, in the steepest uphill direction
...
- **Sample** (aka **Input**) - One data point that goes into your model.
...
- **Stochastic Gradient Descent (SGD)**
  ...
  - Mini-Batch SGD
    1.  Draw a random batch of training samples, $x$, and corresponding targets, $y_{\text{true}}$.
    ...
    5.  Move the parameters a little in the opposite direction from the gradient — for example, $W = W – (\text{learning_rate} \times \text{gradient})$ — thus reducing the loss on the batch a bit. The learning rate would be a scalar factor modulating the “speed” of the gradient descent process.
...
- **Momentum**
  ...
  - It updates the weight parameter $w$ based not only on the current gradient value but also on the previous parameter update
...
- **Backpropagation**
  ...
  - Example:
    ...
    - If there are multiple paths linking the two nodes of interest, a and b, then `grad(b, a)` is obtained by summing the contributions of all the paths

## Tensors

- **Activations** - The values in the neurons/nodes of a layer. It's the result of the weighted sum after it's been fed into the activation function

  - Example: $0.2 = \sigma (w_o a_0 + w_1a_1 + \cdots + w_{n-1}a_{n-1} + b)$
    - Where $0.2$ is the activation, $w_i$ are weights, $a_i$ are inputs or activations from the previous layer, $b$ is the bias, and $\sigma$ is the sigmoid activation function
...
- Components
  ...
  - Rank - (number of axes) - `length(dim(train_images))`
...
- Data Types and Their Ranks
  ...
  - *Vector data* (e.g. tabular data): Rank 2 tensors of shape (samples, features), where each sample is a vector of numerical attributes (“features”)
  ...
  - *Times-series data* or *sequence data*: Rank 3 tensors of shape (samples, timesteps, features), where each sample is a sequence (of length timesteps) of feature vectors
    - Each sample is a matrix (rank 2, time steps (rows) by features (columns)). All the samples make up the rank 3 tensor.
    - e.g. Stock data where the current, lowest, highest stock price (columns) is recorded every minute. There are 390 minutes (rows) in a trading day and the dataset has 250 days of data. The shape would be (250, 390, 3), i.e. each matrix is (390, 3) and there are 250 matrices.
    - e.g. Social media posts where each post is a sequence of 280 characters (rows). Each character can be encoded as a binary vector of length 128 (columns) (an all-zeros vector except for a 1 entry at the index corresponding to that character). For a dataset of 1M posts, the shape would be (1M, 280, 128), i.e. each matrix is (280, 128) and there are 1M matrices.
...
- [Example]{.ribbon-highlight}: `layer_dense(units = 512, activation = "relu")`
  ...
  - Then, an addition (+) between the resulting matrix and a vector $b$
...
- Takes as input, one or more tensors and outputs one or more tensors
...
- Classes
  ...
  - `Variable` is a specific kind of tensor meant to hold mutable state — for instance, the weights of a neural network.
    - Process
      ...
      3.  Inside the scope, apply some tensor operations to the variable
      4.  Use the tape to retrieve the gradient of the output [y]{.var-text} with respect to variable [x]{.var-text}
...
- `%<-%` - A multi-element assignment operator

  - e.g. `c(c(train_data, train_labels), c(test_data, test_labels)) %<-% imdb`
  - In python, `(train_data, train_labels), (test_data, test_labels) = imdb.load_data(num_words=10000)`
...
- `fit` - Implements the training loop
  ...
  - Arguments
    ...
    - [validation_data = list(val_inputs, val_targets)]{.arg-text}: Used only to monitor the validation loss and metrics
      - Can also use `evaluate(model, val_inputs, val_targets, batch_size = 128)`
...
- `predict`
  ...
  - [batch_size]{.arg-text} - Allows you to iterate over the new data in small batches

  - You can also just use the model object, e.g. `model(newdata)`, but this can lead to problems if new data is large.

## Examples

- [Example 1]{.ribbon-highlight}: Basic MNIST Classification
  ...
  - Notice that the only assignment is for the model specification. Usually a S4 (or S7 now) is used for OOP, but not here.
    - If you assign the fitted model to an object, it will print the metrics.
  - Layers
  ...
  - Predicting on the test set
    ...
    - A 2-dim array is returned where each number of index $i$ in that array ([predictions\[1, \]]{.var-text}) corresponds to the probability that digit image [test_digits\[1, \]]{.var-text} belongs to class $i$.
  ...
  - Evaluation
    - The test set accuracy turns out to be 97.9% — "that’s quite a bit lower than the training set accuracy (98.9%)} (Overfitted)
      - If you say so, Chollet.
  :::

- [Example 2]{.ribbon-highlight}: Basic MNIST Classification (python)

  ::: panel-tabset
  ## Data and Process

  Same as Example 1 except in Python pg 27 (52) of book
  ...
  #>  2.6657624e-08, 3.8127661e-07], dtype=float32)code
...
- [Example 3]{.ribbon-highlight}: Binary Classification
  ...
  - [num_words = 10000]{.arg-text} means you’ll keep only the top 10,000 most frequently occurring words in the training data. There are 88,585 unique words in the training data
  - [train_data]{.var-text} and [test_data]{.var-text} are lists of reviews; each review is a list of word indices (encoding a sequence of words).
    - First review decoded: "this film was just brilliant casting location scenery story direction everyone's really suited the part they played and you could just imagine being there robert ? is an amazing actor and now the same being director"
  - [train_labels]{.var-text} and [test_labels]{.var-text} are lists of 0s and 1s, where 0 stands for negative and 1 stands for positive
...
- Process
  ...
  - Inside the venv:
    ...
    - Register your venv with jupyter
      ...
      ``` bash
      python3 -m ipykernel install --user --name=.venv --display-name="Python 3.12 (.venv)"
      ```
  ...
  - VSCode
    ...
    - In WSL2 venv, navigate to the directory that you want to do work in.
      ...
      ``` bash
      cd Documents/R/Projects/deeplearning-with-r/
      ```
    ...
    - In VSCode, hit ctrl + k, ctrl + o to open a folder. Copy the path from WSL2 where you've navigated to the project directory and paste in the VSCode window
      ...
      ```         
      /mnt/c/Users/user/Documents/R/Projects/deeplearning-with-r
      ```
...
- R
  ...
  - Inside the activated virtual environment (e.g. [tf_env]{.var-text})
    ...
    - Goto your desired project directory, and open up VSCode/Positron, `code .` / `positron .` .
      ...
      - Currently Positron won't automatically connect to WSL when it opens.
...
- Multiple venvs
  - Currently, for some reason the second venv I created that had tensorflow wasn't able to use the GPU even though I used the exact set-up process.

  - Applying this bash code, which specifies the location of the installed nvidia libraries, allows python to use the GPUs inside the venv

    - Unfortunately, this doesn't work when R uses it.

  - Append this lines to activate (See [CLI, Linux \>\> Commands \>\> Files/Directories](https://ercbk.github.io/Data-Science-Notebook/qmd/cli-linux.html#sec-cli-lin-com-file){style="color: green"} for details on heredocs)

    ``` bash
    cat << 'EOF' >> .venv/bin/activate 
    _SITE=$(python3 -c "import site; print(site.getsitepackages()[0])")
    export LD_LIBRARY_PATH=$(find "$_SITE/nvidia" -name "lib" -type d | tr '\n' ':')$LD_LIBRARY_PATH
    unset _SITE
    EOF
    ```

    - The heredoc line that allows you to write mulitple lines to the activate file
    - Line 1: Uses the site module to find where pip installs packages for this venv and stores it in the \_SITE variable (underscore indicating that's it's temporary)
    - Line 2:
      - Using `find`, it finds every directory named [lib]{.var-text} under the [nvidia]{.var-text} packages (e.g. `nvidia/cudnn/lib`, `nvidia/cuda_runtime/lib`, etc.)
      - `| tr '\n' ':'` — `find` returns one path per line, but [LD_LIBRARY_PATH]{.var-text} expects colon-separated paths, so `tr` converts the newlines to colons
      - `$LD_LIBRARY_PATH` at the very end preserves whatever was already in [LD_LIBRARY_PATH]{.var-text} instead of replacing it
      - `export` makes the variable visible to child processes — without it, tensorFlow wouldn't see it
    - Line 3: Removes [\_SITE]{.var-text} (clean-up, good practice)
    - Line 4: Dilimiter label indicating an end to heredoc

## Workflows

- Positron
  - Example: Continue work in jupyter notebook
    - `Ctrl + Alt + o` \>\> Connect to WSL

      - Alt: Ctrl + Shift + p \>\> Search "remote menu" or "wsl" \>\> Connect to WSL

    - `Ctrl s, Ctrl o` \>\> Enter the path to the project into the dialog box, e.g. `/mnt/c/Users/user/Documents/R/Projects/deeplearning-with-r`
