...
- Data Types and Their Ranks
  ...
  - *Times-series data* or *sequence data*: Rank 3 tensors of shape (samples, timesteps, features), where each sample is a sequence (of length timesteps) of feature vectors
    ...
    - e.g. Social media posts where each post is a sequence of 280 characters (rows). Each character can be encoded as a binary vector of length 128 (columns) (an all-zeros vector except for a 1 entry at the index corresponding to the character). For a dataset of 1M posts, the shape would be (1M, 280, 128), i.e. each matrix is (280, 128) and there are 1M matrices.
