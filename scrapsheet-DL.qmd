...
- For $\frac{\partial C_0}{\partial b^{(L)}}$, it's the same except $a^{(L)}$ is 1
...
- Example 3: Binary Classification
  ...
  - [train_data]{.var-text} and [test_data]{.var-text} are lists of reviews; each review is a list of word indices (encoding a sequence of words).
    ...
    - Firest review decoded: "this film was just brilliant casting location scenery story direction everyone's really suited the part they played and you could just imagine being there robert ? is an amazing actor and now the same being director"
